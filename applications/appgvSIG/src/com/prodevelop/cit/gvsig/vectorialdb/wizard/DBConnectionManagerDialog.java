...
/**
 * Single connection manager main dialog. Lists available connections. Open
 * connections are marked "[C]" before their names.
 * 
 * @author jldominguez
 * 
 */
public class DBConnectionManagerDialog extends JPanel implements IWindow,
	...
	private static Logger logger = Logger
			.getLogger(DBConnectionManagerDialog.class.getName());
	private JScrollPane connectionsScrollPane = null;
	private JButton closeButton = null;
	private JButton removeButton = null;
	private JButton newButton = null;
	private JTree connectionsTree = null;
	private WindowInfo winfo = new WindowInfo(WindowInfo.MODALDIALOG); // all
																		// false
																		// except
																		// MODAL
	...
	private void registerIcons() {
		PluginServices.getIconTheme().register("conn-add",
				getClass().getClassLoader().getResource("images/addconn.png"));
		PluginServices.getIconTheme().register("conn-del",
				getClass().getClassLoader().getResource("images/delconn.png"));
		PluginServices.getIconTheme().register(
				"conn-mod",
				getClass().getClassLoader().getResource(
						"images/commandstack.png"));
		PluginServices.getIconTheme().register(
				"conn-swi",
				getClass().getClassLoader()
						.getResource("images/connswitch.png"));
	}
	...
	private JButton getRemoveButton() {
		if (removeButton == null) {
			...
			removeButton.setToolTipText(PluginServices.getText(this, "remove"));

			ImageIcon ico = PluginServices.getIconTheme().get("conn-del");
			removeButton.setIcon(ico); // .setText(PluginServices.getText(this,
										// "add"));
	...
	private JButton getNewButton() {
		if (newButton == null) {
			...
			newButton.setToolTipText(PluginServices.getText(this, "add"));

			ImageIcon ico = PluginServices.getIconTheme().get("conn-add");
			newButton.setIcon(ico); // .setText(PluginServices.getText(this,
									// "add"));
		}
	...
	private DefaultMutableTreeNode getTreeRoot() {
		...
		while (iter.hasNext()) {
			...
			VectorialDBConnectionTreeNode node = new VectorialDBConnectionTreeNode(
					k);
			ArrayList cc = (ArrayList) aux_nodes.get(k);

			for (int i = 0; i < cc.size(); i++) {
				ConnectionWithParams conwp = (ConnectionWithParams) cc.get(i);
				VectorialDBConnectionTreeLeaf leaf = new VectorialDBConnectionTreeLeaf(
						conwp);
				node.add(leaf);
			}
	...
	private void addConnection(String _drvName, String _port, String _host,
			String _dbName, String _sche, String _user, String _pw,
			String _conn_usr_name, boolean is_conn) throws DBException {
		SingleDBConnectionManager.instance().getConnection(_drvName, _user,
				_pw, _conn_usr_name, _host, _port, _dbName, _sche, is_conn);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (src == connSwitchButton) {
			...
			if (!_cwp.isConnected()) {
				...
				dlg.setLocationRelativeTo((Component) PluginServices
						.getMainFrame());
		...
		if (src == newButton) {
			...
			if (newco.isOkPressed()) {
				...
				try {
					addConnection(_drvname, _port, _host, _dbname, _sche,
							_user, _pw, _conn_usr_name, hasToBeCon);
				} catch (DBException e) {
					showConnectionErrorMessage(e.getMessage(), false);
				}
			}
		...
		if (src == editButton) {
			...
			if (modifyco.isOkPressed()) {
				...
				try {
					...
					addConnection(_drvname, _port, _host, _dbname, __sche,
							_user, _pw, _conn_usr_name, hasToBeCon);
					saveConnections();
				} catch (DBException e) {
					showConnectionErrorMessage(e.getMessage(), false);
					...
					try {
						addConnection(old_cwp.getDrvName(), old_cwp.getPort(),
								...
								old_cwp.getSchema(), old_cwp.getUser(),
								old_cwp.getPw(), old_cwp.getName(),
								old_was_open);
	...
	private JButton getSwitchConnButton() {
		if (connSwitchButton == null) {
			...
			connSwitchButton
					.setBounds(new java.awt.Rectangle(10, 380, 106, 26));

			connSwitchButton.setToolTipText(PluginServices.getText(this,
					"connect")
					+ "/"
					+ PluginServices.getText(this, "disconnect"));

			ImageIcon ico = PluginServices.getIconTheme().get("conn-swi");
			connSwitchButton.setIcon(ico); // .setText(PluginServices.getText(this,
											// "add"));
	...
	private void saveConnections() {
		if (this.dbConnectionExtension == null) {
			this.dbConnectionExtension = (SingleVectorialDBConnectionExtension) PluginServices
					.getExtension(SingleVectorialDBConnectionExtension.class);
		}
	...
	private JButton getEditButton() {
		if (editButton == null) {
			...
			editButton.setToolTipText(PluginServices.getText(this,
					"edit_settings"));
			ImageIcon ico = PluginServices.getIconTheme().get("conn-mod");
			editButton.setIcon(ico); // .setText(PluginServices.getText(this,
										// "add"));
		}
