...
public class WizardVectorialDB extends WizardPanel implements ActionListener,
	...
	protected void loadVectorialDBDatasourcesCombo() {
		...
		for (int i = 0; i < conn.length; i++) {
			Driver _drv = SingleDBConnectionManager.getInstanceFromName(conn[i]
					.getDrvName());
	...
	public IVectorialDatabaseDriver getDriverFromName(String drvname)
			throws DriverLoadException {
		IVectorialDatabaseDriver resp = (IVectorialDatabaseDriver) LayerFactory
				.getDM().getDriver(drvname);
		return resp;
	}
	...
	public FLayer getLayer() {
		try {
			...
			String strEPSG = ProjectionUtils.getAbrev(getMapCtrl()
					.getViewPort().getCrs());
			...
			for (int i = 0; i < count; i++) {
				...
				UserTableSettingsPanel userTableSettingsPanel = item
						.getUserTableSettingsPanel(ProjectionUtils
				...
				// Change: we parse tableName to extract schema (or owner in
				// Oracle)
			...
			NotificationManager.addError(
					"Error al cargar la capa: " + e.getMessage(), e);
		}
	...
	private void updateTableList(ConnectionWithParams src) {
		...
		try {
			...
			drv = (IVectorialDatabaseDriver) LayerFactory.getDM().getDriver(
					drvName);
		} catch (Exception e) {
		...
		if (!(drv instanceof IVectorialDatabaseDriver)) {
			logger.error("Unexpected driver type (not a DefaultDBDriver driver)");
		...
		for (int i = 0; i < _tablnames.length; i++) {
			try {
				if (drv.canRead(conex, _tablnames[i])) {
					lmodel.addElement(new TablesListItem(_tablnames[i], drv,
							conex, getMapCtrl(), this));
				}
			} catch (SQLException e) {
				// if getTableNames works fine this exception should never be
				// caught
				logger.error("While getting table names: " + e.getMessage(), e);
			}
		}
	...
	public void setSettingsPanels(TablesListItem actTable) throws DBException {
		if (actTable == null) {
			setEmptyPanels();
	...
	/**
	 * @return Message describing why layers don't have good settings or null if
	 *         layers are ok.
	 */
	public String[] validateLayerSettings() {
		...
		String strEPSG = ProjectionUtils.getAbrev(getMapCtrl().getViewPort()
				.getCrs());
		...
		for (int i = 0; i < count; i++) {
			...
			try {
				utsp = item.getUserTableSettingsPanel(ProjectionUtils
						.getAbrev(getMapCtrl().getViewPort().getCrs()));
				layerName = utsp.getUserLayerName();
				_driver = getDriverFromName(selectedDataSource.getDrvName());
			} catch (Exception ex) {
				addErrorToList(listresp, layerName,
						"Error_while_creating_driver");
				continue; // go to next
			}
			...
			if (_driver instanceof IVectorialJDBCDriver) {
				...
				logger.warn("Did not validate settings for driver: "
						+ _driver.getClass().getName());
				continue;
			}
	...
	private void addErrorToList(ArrayList lis, String lyr, String msg) {
		String addstr = PluginServices.getText(this, "Capa") + ": " + lyr
				+ "\n" + msg;
		lis.add(addstr);
	}
