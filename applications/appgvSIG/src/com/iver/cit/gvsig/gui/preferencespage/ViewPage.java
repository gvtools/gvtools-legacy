...
public class ViewPage extends AbstractPreferencePage {
		...
		btnChangeProjection.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				ISelectCrsPanel panel = CRSSelectPanel.getUIFactory()
						.getSelectCrsPanel(
								ProjectionUtils.getCRS(lblDefaultProjection
										.getText()), false);
				PluginServices.getMDIManager().addWindow(panel);
				if (panel.isOkPressed()) {
					lblDefaultProjection.setText(ProjectionUtils.getAbrev(panel
							.getCrs()));
				}
			}
		});
	...
	public void initializeValues() {
		...
		Project.setDefaultCrs(ProjectionUtils.getCRS(lblDefaultProjection
				.getText()));
		...
		MapContext.ZOOMOUTFACTOR = Double.parseDouble(txtZoomOutFactor
				.getText());
		;
		...
		// Map units
		if (Project.getDefaultMapUnits() <= distanceNames.length
				&& distanceNames.length != jCmbMapUnits.getItemCount()) {
			((DefaultComboBoxModel) jCmbMapUnits.getModel())
					.removeAllElements();
			// default map units
			for (int i = 0; i < distanceNames.length; i++) {
				((DefaultComboBoxModel) jCmbMapUnits.getModel())
						.addElement(PluginServices.getText(this,
								distanceNames[i]));
			}
		...
		// Distance units
		if (Project.getDefaultDistanceUnits() <= distanceNames.length
				&& distanceNames.length != jCmbDistanceUnits.getItemCount()) {
			((DefaultComboBoxModel) jCmbDistanceUnits.getModel())
					.removeAllElements();
			// default distance units
			for (int i = 0; i < distanceNames.length; i++) {
				((DefaultComboBoxModel) jCmbDistanceUnits.getModel())
			...
			jCmbDistanceUnits.setSelectedIndex(Project
					.getDefaultDistanceUnits());
		}
		String[] names = MapContext.getAreaNames();

		// Distance area
		if (Project.getDefaultDistanceArea() <= names.length
				&& names.length != jCmbDistanceArea.getItemCount()) {
			((DefaultComboBoxModel) jCmbDistanceArea.getModel())
					.removeAllElements();
			for (int i = 0; i < names.length; i++) {
				((DefaultComboBoxModel) jCmbDistanceArea.getModel())
						.addElement(PluginServices.getText(this, names[i])
	...
	public void storeValues() throws StoreException {
		...
		try {
			...
			Project.setDefaultDistanceUnits(jCmbDistanceUnits
					.getSelectedIndex());
