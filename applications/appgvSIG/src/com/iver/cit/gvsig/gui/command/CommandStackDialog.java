...
public class CommandStackDialog extends DefaultBean implements SingletonWindow,
	...
	private static final ImageIcon imodify = PluginServices.getIconTheme().get(
			"edition-modify-command");
	private static final ImageIcon iadd = PluginServices.getIconTheme().get(
			"edition-add-command");

	private static final ImageIcon idel = PluginServices.getIconTheme().get(
			"edition-del-command");
	...
	public CommandStackDialog() {
		...
		initialize();
		// System.err.println("Identificaci�n del objeto en constructor = "+
		// cr.toString());
	}

	public void setModel(CommandRecord cr1) {
		// System.err.println("Identificaci�n del objeto en setModel = "+
		// cr1.toString());
		...
		tc.setCellRenderer(new DefaultTableCellRenderer() {
			public Component getTableCellRendererComponent(JTable table,
				...
				if (CommandStackDialog.this.cr.getPos() < row) {
					...
					/*
					 * if (cr.getPos()==row){ label.setBackground(Color.orange);
					 * }else{
					 */
	...
	public WindowInfo getWindowInfo() {
		WindowInfo m_viewinfo = new WindowInfo(WindowInfo.ICONIFIABLE
				| WindowInfo.MODELESSDIALOG | WindowInfo.RESIZABLE
				| WindowInfo.PALETTE);
		m_viewinfo.setTitle(PluginServices.getText(this, "pila_de_comandos"));
		return m_viewinfo;
	}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.commands.CommandListener#executeCommand
	 * (com.iver.cit.gvsig.fmap.edition.commands.CommandEvent)
	 */
	...
	private void refreshScroll() {
		Dimension size = new Dimension(jSlider.getPreferredSize().width,
	...
	private void initSlider() {
		...
		jSlider.setPreferredSize(new Dimension(
				jSlider.getPreferredSize().width,
				((getTable().getRowCount()) * getTable().getRowHeight())));
		...
		jSlider.addChangeListener(new javax.swing.event.ChangeListener() {
			...
			public void stateChanged(javax.swing.event.ChangeEvent e) {
				int value = (int) (getJSlider().getValue()
						* cr.getCommandCount() * 0.01);
				try {
					if (!refreshing)
	...
	private void refreshControls() {
		int normalizedValue = (int) ((currentValue / (float) cr
				.getCommandCount()) * 100);
