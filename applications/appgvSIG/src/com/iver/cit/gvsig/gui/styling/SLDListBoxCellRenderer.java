...
public class SLDListBoxCellRenderer extends JPanel implements ListCellRenderer {
	...
	public SLDListBoxCellRenderer() {
		...
		/*
		 * canvas = new JSVGCanvas(); // canvas.setMySize(getWidth()-3,
		 * getHeight()-3); canvas.setMySize(new Dimension(getWidth()-3,
		 * getHeight()-3)); // add(canvas,BorderLayout.CENTER);
	...
	public Component getListCellRendererComponent(JList list, Object value,
		...
		/*
		 * try { canvas.setURI( new
		 * File("D:/java/eclipse30/eclipse/workspace/FMap 03/docs/fill1.svg"
		 * ).toURL().toString() ); } catch (MalformedURLException e1) { // TODO
		...
		/*
		 * setIcon(icon); if (icon != null) {
		 */
		// setText(pet);
		setFont(list.getFont());
		repaint();
		/*
		 * } else { // setUhOhText(pet + " (no image available)",
		 * list.getFont()); }
		 */
	...
	protected void paintComponent(Graphics g) {
		 ...
		 * 
		 * renderer.updateOffScreen(getWidth()-1, getHeight()-1);
		 * 
		...
		try {
			gvtRoot.setTransform((ViewBox.getViewTransform(null, elt,
					getWidth() - 1, getHeight() - 1, ctx)));
			gvtRoot.paint(g2);
		} catch (Exception e) {
			Rectangle bounds = getBounds();
			JLabel errLbl = new JLabel(
					"<html><b>" + "Cannot render north" + /*
