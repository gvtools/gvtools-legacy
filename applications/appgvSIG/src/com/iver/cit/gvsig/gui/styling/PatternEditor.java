...
public class PatternEditor extends JPanel {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i <= separations; i++) {
			if (i % subDividersPerDivider == 0) {
				g.drawLine(hGap + separation * i, vGap, hGap + separation * i,
						vGap + halfHeight - 3);
			} else {
				g.drawLine(hGap + separation * i, vGap + (halfHeight / 2), hGap
						+ separation * i, vGap + halfHeight - 3);
			}
		...
		for (int i = 0; i < states.length; i++) {
			...
			g.fillRect(hGap + separation * i, (vGap) + height / 2, separation,
					halfHeight);

		}
		g.setColor(Color.BLACK);
		bounds.setLocation(0, 0);
		g.drawRect(hGap, (vGap) + height / 2, separation * separations,
				halfHeight);

		if (initSelection != -1 && endSelection != -1) {
			g.setColor(Color.YELLOW);
			bounds.setLocation(0, 0);
			g.drawRect(hGap + initSelection * separation, (vGap) + height / 2,
					separation * (endSelection - initSelection + 1), halfHeight);
		}
	...
	private class MyMouseListener implements MouseListener, MouseMotionListener {
		...
		private synchronized void doIt() {
			...
			if (!isValidRect(endRect) || !isValidRect(initialRect))
				return;
