...
public class TableOperations extends Extension implements ExpressionListener {
	...
	// private Table vista;
	private String defaultSourceTable = null;

	/*
	 * execute with a default source table name; this is useful for calling JOIN
	 * from the layer context menu in the TOC
	 */
	public void execute(String actionCommand, String defaultTable) {
		defaultSourceTable = defaultTable;
		this.execute(actionCommand);
	}
	...
	public void execute(String actionCommand) {
		...
		ProjectTable[] pts = (ProjectTable[]) project.getDocumentsByType(
				ProjectTableFactory.registerName).toArray(new ProjectTable[0]);
			...
			try {
				...
				sourceTable
						.setModel(new TableSelectionModel(pts, PluginServices
								.getText(this, "seleccione_tabla_origen")));
				...
				final AndamiWizard wiz = new AndamiWizard(
						PluginServices.getText(this, "back"),
				...
				wiz.addWizardListener(new WizardListener() {
					...
					public void finished(WizardEvent w) {
						...
						ProjectTable sourceProjectTable = (ProjectTable) sourceTable
								.getSelected();
						...
						ProjectTable targetProjectTable = (ProjectTable) targetTable
								.getSelected();
						...
						sourceProjectTable.setLinkTable(sds2.getName(), field1,
								field2);
						((ProjectExtension) PluginServices
								.getExtension(ProjectExtension.class))
								.getProject().setLinkTable();
					}
					...
					public void next(WizardEvent w) {
						...
						if (w.currentStep == 1) {
							ProjectTable pt = (ProjectTable) sourceTable
									.getSelected();
							...
							try {
								...
								SelectableDataSource sds = pt.getModelo()
										.getRecordset();
								String fieldName = (String) firstTableField
										.getSelected();
								int fieldIndex = sds
	...
	public boolean isVisible() {
		 ...
		 * ProjectView pv = view.getModel(); FLayer[] seleccionadas =
		 * pv.getMapContext().getLayers() .getActives();
		 * 
		 * if (seleccionadas.length == 1) { if (seleccionadas[0] instanceof
		 * AlphanumericData) { return true; } } }
		 */
		return false;
		// }
	}
	...
	public void execJoin(ProjectTable sourceProjectTable, String field1,
			String prefix1, ProjectTable targetProjectTable, String field2,
			String prefix2) {
		...
		String sql = "custom com_iver_cit_gvsig_arcjoin tables '" + tableName1
				+ "', '" + tableName2 + "' values(" + field1 + ", " + field2
				+ ");";
		PluginServices.getLogger().debug(sql);

		try {
			SelectableDataSource result = new SelectableDataSource(LayerFactory
			...
			if (currentJoinedTables != null) {
				...
				System.arraycopy(currentJoinedTables, 0, joinedTables, 0,
						currentJoinedTables.length);
			} else {
				joinedTables = new String[1];
			}
			joinedTables[joinedTables.length - 1] = targetProjectTable
					.getName();
			...
			NotificationManager.addError(
					PluginServices.getText(this, "Error_loading_driver"), e);
		} catch (ReadDriverException e) {
			NotificationManager.addError(PluginServices.getText(this,
					"Error_reading_from_the_driver"), e);
		} catch (SemanticException e) {
			throw new RuntimeException(e);
		} catch (EvaluationException e) {
			NotificationManager.addError(PluginServices.getText(this,
					"Error_evaluationg_expression"), e);
		}
	}
	...
	/**
	 * <p>
	 * Renames fields in a JOIN, so that fields from DataSource1 become
	 * sourcePrefix_field1, sourcePrefix_field2, etc, and fields from TABLE2
	 * become targetPrefix_field1, targetPrefix_field2, etc. ProjectTable
	 * aliases are used to rename fields.
	 * </p>
	 * 
	 * @param targetPt
	 *            The ProjectTable whose fields are to be renamed
	 * @param ds1
	 *            The first datasource in the join
	 * @param prefix1
	 *            The prefix to apply to fields in ds1
	 * @param ds2
	 *            The second datasource in the join
	 * @param prefix2
	 *            The prefix to apply to fields in ds2
	 */
	private void renameFields(ProjectTable targetPt, DataSource ds1,
			String prefix1, DataSource ds2, String prefix2) {
		try {
			...
			while (i < aliases.length) {
				if (!prefix2.equals(""))
					aliases[i] = prefix2
							+ "_"
							+ sds.getFieldName(i).substring(
									ArcJoinDataSource.prefix.length());
				else
					aliases[i] = sds.getFieldName(i).substring(
							ArcJoinDataSource.prefix.length());
				sds.setFieldAlias(i, aliases[i]);
				i++;
			}
			...
			PluginServices.getLogger().error("Error renaming fields in a JOIN",
					e);
		} catch (Exception e) {
			// just log the error, it's not bad if fields couldn't be renamed
			PluginServices.getLogger().error("Error renaming fields in a JOIN",
	...
	/**
	 * Ensure that field name only has 'safe' characters (no spaces, special
	 * characters, etc).
	 */
