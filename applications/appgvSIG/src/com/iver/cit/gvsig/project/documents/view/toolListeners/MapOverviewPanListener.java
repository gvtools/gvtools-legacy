...
/**
 * <p>
 * Listener for moving the extent of the associated {@link MapOverview
 * MapOverview} object according the movement between the initial and final
...
public class MapOverviewPanListener implements PanListener {
	 ...
	 */
	private final Image icursor = PluginServices.getIconTheme()
	 ...
	 * associated {@link MapOverview MapOverview} object.
	 * </p>
	 * 
	 * @param mapControl
	...
	/*
	 * public void move(MoveEvent event) throws BehaviorException { if
	 * (!checkModifiers(event.getEvent().getModifiers())){ return; }
	 * 
	 * ViewPort vp = mapControl.getMapContext().getViewPort(); MapOverview
	 * mov=(MapOverview) this.mapControl; ViewPort
	 * vpView=mov.getAssociatedMapContext().getViewPort(); if (vp.getExtent() !=
	 * null) { Point2D p = vp.toMapPoint(event.getTo()); Rectangle2D r =
	 * (Rectangle2D) vpView.getExtent().clone(); r.setRect(p.getX() -
	 * (r.getWidth() / 2), p.getY() - (r.getHeight() / 2), r.getWidth(),
	 * r.getHeight()); //vpView.setExtent(r); double scale; double escalaX;
	 * double escalaY; double newHeight; double newWidth; double xCenter =
	 * r.getCenterX(); double yCenter = r.getCenterY();
	 * 
	 * escalaX = mapControl.getWidth() / r.getWidth(); escalaY =
	 * mapControl.getHeight() / r.getHeight(); Rectangle2D adjustedExtent = new
	 * Rectangle2D.Double();
	 * 
	 * if (escalaX < escalaY) { scale = escalaX; newHeight =
	 * mapControl.getHeight() / scale; adjustedExtent.setRect(xCenter -
	 * (r.getWidth() / 2.0), yCenter - (newHeight / 2.0), r.getWidth(),
	 * newHeight); } else { scale = escalaY; newWidth = mapControl.getWidth() /
	 * scale; adjustedExtent.setRect(xCenter - (newWidth / 2.0), yCenter -
	 * (r.getHeight() / 2.0), newWidth, r.getHeight()); }
	...
	public void move(MoveEvent event) throws BehaviorException {
		...
		if (vp.getExtent() != null && vpView.getExtent() != null) {
			...
			// centro del recuadro
			realRectangle.setRect(thePoint.getX() - (width / 2),
					thePoint.getY() - (height / 2), width, height);
	...
	private boolean checkModifiers(MouseEvent event) {
		 ...
		 * 
		 * ** No se porque el boton derecho del raton devuelve** un modificador
		 * 'Meta + BUTTON3'. Pensaba que deberia** devolver 'BUTTON2' ????
		 */
