...
public class FLayoutFunctions {
	...
	public void setSelect() {
		...
		for (int i = 0; i < fframes.length; i++) {
			...
			int difx = (layout.getLayoutControl().getLastPoint().x - layout
					.getLayoutControl().getFirstPoint().x);
			int dify = (layout.getLayoutControl().getLastPoint().y - layout
			...
			if (((Math.abs(difx) > 3) || (Math.abs(dify) > 3))
				...
				// //cuadrar con la cuadrícula dibujada y después en el
				// mouseReleased guardar ese rectángulo de la misma forma.
			...
			if (layout.getLayoutControl().getReSel() == null) {
				...
				rect.setFrameFromDiagonal(layout.getLayoutControl()
						.getFirstPoint(), layout.getLayoutControl()
						.getLastPoint());
			} else {
	...
	/**
	 * Añade un fframe al Layout del tipo preseleccionado y abre el diálogo para
	...
	//
	// // if (layout.isAdjustingToGrid()) {
	// // //cuadrar con la cuadrícula dibujada y después en el mouseReleased
	// guardar ese rectángulo de la misma forma.
