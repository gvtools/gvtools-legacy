...
public class Table extends JPanel implements SingletonWindow, EditionTable,
	...
	protected void refreshControls() {
		try {
			...
			if (mF != null) {
				String title = PluginServices.getText(this, "Tabla") + ": "
						+ model.getName();
			...
			if (model.getAssociatedTable() != null) {
				this.fmap = ((FLayer) model.getAssociatedTable())
						.getMapContext();
			} else {
				this.fmap = null;
			}
			...
			for (int i = 0; i < model.getMapping().length; i++) {
				...
				// System.err.println("Table.Dentro de refreshControls. column="
				// + column.toString());
	...
	public javax.swing.JTable getTable() {
		if (table == null) {
			...
			table.getSelectionModel().addListSelectionListener(
					new ListSelectionListener() {
						public void valueChanged(ListSelectionEvent e) {
							...
							DefaultListSelectionModel model = (DefaultListSelectionModel) table
									.getSelectionModel();
			...
			// table.setDefaultEditor(Object.class, new DefaultEditor(tf));
			// table.setDefaultEditor(BooleanValue.class,new
			// BooleanTableCellEditor(table));
			// table.setDefaultEditor(StringValue.class,new
			// DefaultCellEditor(tf));
			// table.setDefaultEditor(Number.class,new
	...
	/*
	 * private void refresh() throws DriverException{ //dw.commitTrans();
	 * //model.getModelo().stop(); //dw.beginTrans(); //DataSourceDataModel
	 * dsdm=(DataSourceDataModel)getTable().getModel();
	 * //dsdm.fireTableDataChanged(); }
	 */
	/*
	 * public void addEmptyRow() throws DriverException{ ValueCollection
	 * valuePK=new ValueCollection(); valuePK.setValues(new
	 * Value[]{ValueFactory.createValue(dw.getRowCount())});
	 * dw.insertEmptyRow(valuePK); refresh(); }
	 */
	/*
	 * public void copySelectedRows() throws DriverException{ int[]
	 * sel=getSelectedRowIndices(); for(int i=0;i<sel.length;i++){
	 * rowsCopied.add(getValueRow(sel[i])); } }
	 */
	/*
	 * public void addSelectionToEnd() throws DriverException { for (int
	 * i=0;i<rowsCopied.size();i++){
	 * dw.insertFilledRow((Value[])rowsCopied.get(i)); } refresh(); }
	 */
	/*
	 * public void delSelectionRow() throws DriverException{ int[]
	 * sel=getSelectedRowIndices(); for(int i=sel.length-1;i>=0;i--){
	 * dw.deleteRow(sel[i]); } refresh(); }
	 */
	/*
	 * public boolean isCopy(){ return !rowsCopied.isEmpty(); }
	 */
	/*
	 * public void addSelectionToRow() throws DriverException { int[]
	 * sel=getSelectedRowIndices();
	 * dw.insertFilledRow((Value[])rowsCopied.get(i),sel[0]); refresh(); }
	 */
		...
		try {
			...
			if ((lyr != null)
				...
				VectorialEditableAdapter vea = (VectorialEditableAdapter) lyr
						.getSource();
				ISpatialWriter spatialWriter = (ISpatialWriter) vea
						.getOriginalDriver();
				 ...
				 * i++) { gdbmswriter.process(getModel().getModelo().getRow(i));
				 * }
				 * 
	 ...
	 * @throws IOException
	 */
	public void addRow(IRow[] rows) throws ValidateRowException,
			ReadDriverException, ExpansionFileWriteException {
		IEditableSource ies = getModel().getModelo();
		if (rows == null) {
			...
			if (getModel().getAssociatedTable() != null) {
				JOptionPane
						.showMessageDialog(
								...
								"No se puede añadir una fila a una tabla asociada a una capa.");
				return;
			}
			IRow row;
			int numAttr = getModel().getModelo().getRecordset().getFieldCount();
			...
			for (int i = 0; i < rows.length; i++) {
				ies.addRow(((IRowEdited) rows[i]).getLinkedRow(),
						"Pegar filas", EditionEvent.ALPHANUMERIC);
			}
	 ...
	 * @throws IOException
	 */
	public void copyRow() throws ReadDriverException,
	...
	public void removeRow() throws ReadDriverException,
		...
		if (orderIndexes != null) {
			...
			while (iterator.hasNext()) {
				...
				getModel().getModelo().removeRow(integer.intValue(),
						"Eliminar fila", EditionEvent.ALPHANUMERIC);
			}
		} else {
			for (int i = index.length - 1; i >= 0; i--) {
				getModel().getModelo().removeRow((int) index[i],
						"Eliminar fila", EditionEvent.ALPHANUMERIC);
			}
		}
	...
	public void addColumn(FieldDescription newField) {
		...
		if (getTable().getCellEditor() != null)
			getTable().getCellEditor().cancelCellEditing();
		getModel().setModel(edAdapter); // Para que se recalculen los campos.
										// TODO: Limpiear todo esto
		// refresh();
		refreshControls();
	}
	...
	public void removeColumn() {
		...
		try {
			...
			for (int i = selectedFields.nextSetBit(0); i >= 0; i = selectedFields
					.nextSetBit(i + 1)) {
			...
			edAdapter.endComplexRow(PluginServices.getText(this,
					"remove_fields"));
			clearSelectedFields();
			getModel().setModel(edAdapter); // Para que se recalculen los
											// campos. TODO: Limpiear todo esto
			// refresh();
			refreshControls();
		} catch (ReadDriverException e) {
			e.printStackTrace();
	 ...
	 * @throws IOException
	 */
	public void pasteRow() throws ValidateRowException, ReadDriverException,
	...
	public void windowActivated() {
		// if (isEditing() && getModel().getModelo() instanceof
		...
		if (e.getChangeType() == EditionEvent.CANCEL_EDITION) {
			try {
				...
				NotificationManager.showMessageError(PluginServices.getText(
						this, "Error creating table aliases"), e1);
			}
			refreshControls();
		}
	...
	public void afterFieldEditEvent(AfterFieldEditEvent e) {
		try {
			...
			NotificationManager.showMessageError(PluginServices.getText(this,
					"Error creating table aliases"), e1);
		}
		clearSelectedFields();
		// refresh();
		refreshControls();
