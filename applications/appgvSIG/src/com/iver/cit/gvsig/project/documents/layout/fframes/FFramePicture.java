...
public class FFramePicture extends FFrame {
				...
				if ((rv == null) || (getQuality() == PRESENTACION)) {
					if (!isSVG) {
						...
						try {
							...
							AffineTransform xpos = AffineTransform
	 ...
	 * @param rv
	 *            Rectángulo que forma la parte visible del Layout.
	 */
	private void drawSVG(Graphics2D g, Rectangle2D rect, Rectangle2D rv) {
		if (rv != null && !rv.intersects(rect))
			return;
		if (gvtRoot != null) {
			// Si no tiene atributo "viewBox" el método ViewBox.getViewTransform
			// devuelve una transformación que no hace nada
			if (elt.hasAttribute("viewBox")) {
				...
				ataux.concatenate(AffineTransform.getScaleInstance(scaleX,
						scaleY));
	...
	public XMLEntity getXMLEntity() throws SaveException {
		...
		try {
			xml.putProperty("m_path",
					PathGenerator.getInstance().getPath(m_path));
	...
	public Image load(String file) {
		...
		if (iString.endsWith("jpg") || iString.endsWith("jpeg")
			...
			tmpIcon = new ImageIcon(Jimi.getImage(file, Jimi.VIRTUAL_MEMORY)); // new
																				// ImageIcon(f.getPath());
	...
	public void setXMLEntity03(XMLEntity xml, Layout l) {
		...
		this.m_path = PathGenerator.getInstance().getAbsolutePath(
				xml.getStringProperty("m_path"));
