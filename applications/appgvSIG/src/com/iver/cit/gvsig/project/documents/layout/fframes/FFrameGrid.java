...
public class FFrameGrid extends FFrame implements IFFrameViewDependence {
	...
	public void draw(Graphics2D g, AffineTransform at, Rectangle2D rv,
		...
		Font font = new Font(this.font.getFamily(), this.font.getStyle(),
				scaledFontSize);
		...
		// Dibuja los márgenes
		double valueIntervalX = ((extentX / intervalX) - restX) * intervalX
				- intervalX;
		x = x - intervalX;
		double topX = extentX;
		// if (showCornerText){
		...
		while (x < extent.getMaxX()) {
			if (x > topX) {
				...
				g.drawLine((int) p2.getX(), (int) p2.getY(), (int) p2.getX(),
						(int) p2.getY() + 5);
				TextLayout textaux = new TextLayout(
						String.valueOf(valueIntervalX), font, frc);
		...
		double valueIntervalY = ((extentY / intervalY) - restY) * intervalY
				- intervalY;
		y = y - intervalY;
		double topY = extentY;
		// if (showCornerText){
		...
		while (y < extent.getMaxY()) {
			if (y > topY) {
				...
				TextLayout textaux = new TextLayout(
						String.valueOf(valueIntervalY), font, frc);
				double w = textaux.getBounds().getWidth();
				double h = textaux.getBounds().getHeight();
				g.setColor(textColor);
				...
				if (x > extentX) {
					while (y <= extent.getMaxY()) {
						if (y >= extentY) {
							...
							IGeometry geom = createLine((int) antPoint.getX(),
			...
			while (x <= extent.getMaxX()) {
				if (x > extentX) {
					while (y <= extent.getMaxY()) {
						if (y >= extentY) {
							Point2D p = vp.fromMapPoint(x, y);
							IGeometry geom = createPoint((int) p.getX(),
									(int) p.getY());
	...
	public XMLEntity getXMLEntity() throws SaveException {
		...
		try {
			...
			// xml.putProperty("lineColor",
			// StringUtilities.color2String(lineColor));
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		this.symbolLine = (ILineSymbol) SymbologyFactory.createSymbolFromXML(
				xml.getChild(0), "line");
		this.symbolPoint = (IMarkerSymbol) SymbologyFactory
				.createSymbolFromXML(xml.getChild(1), "point");
		// this.showCornerText=xml.getBooleanProperty("showCornerText");
		this.font = new Font(xml.getStringProperty("familyFont"),
				xml.getIntProperty("styleFont"), 8);
	...
	public void setBoundBox() {
		...
		Rectangle2D extent = fframeview.getMapContext().getViewPort()
				.getAdjustedExtent();
		...
		double dX = FLayoutUtilities.toSheetDistance(pixelsX, layout
				.getLayoutControl().getAT());
		double dY = FLayoutUtilities.toSheetDistance(pixelsY, layout
				.getLayoutControl().getAT());
		Rectangle2D rBound = new Rectangle2D.Double(r.getMinX() - dY,
				r.getMinY() - dX, r.getWidth() + dY * 2, r.getHeight() + dX * 2);
