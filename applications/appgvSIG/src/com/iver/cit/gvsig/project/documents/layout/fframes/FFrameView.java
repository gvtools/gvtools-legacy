...
public class FFrameView extends FFrame implements ViewPortListener,
	...
	public void setView(ProjectView v) {
		...
		if (m_bLinked) {
			if (getTypeScale() == AUTOMATICO) {
				m_fmap = v.getMapContext().createNewFMap(
			...
			try {
				...
				NotificationManager.addError(
						"Cuando se añade una vista al Layout", e1);
			}
		}
	}
							...
							if (origin != null
								...
								if (r.getWidth() > getLayout().getWidth()
									...
									try {
										...
										mapContext.getMapContextDrawer()
												.clean();
										mapContext.draw(m_image, gimg,
												getScale());
									} catch (ReadDriverException e) {
										e.printStackTrace();
									}
	...
	private void printX(Graphics2D g, AffineTransform at) {
		...
		if (theBackColor != null) {
			g.setColor(theBackColor);
			g.fillRect((int) r.x, (int) r.y, viewPort.getImageWidth(),
					viewPort.getImageHeight());
		}
			...
			if (view != null) {
				...
				for (int i = 0; i < views.size(); i++) {
					if (view.getName().equals(
							((ProjectView) views.get(i)).getName())) {
	...
	public void extentChanged(ExtentEvent e) {
		if (getTypeScale() == AUTOMATICO) {
			...
			m_fmap.getViewPort().setExtent(
					new Rectangle2D.Double(oldExtent.getX() + xDif, oldExtent
							.getY() + yDif, oldExtent.getWidth(), oldExtent
							.getHeight()));
			if (getLayout() != null) {
				getLayout().getLayoutControl().setStatus(
						LayoutControl.DESACTUALIZADO);
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (xml.contains("viewName")) {
			view = (ProjectView) project.getProjectDocumentByName(
	...
	public IFFrame cloneFFrame(Layout layout) {
		...
		FFrameView frame = (FFrameView) FrameFactory
	...
	public void refreshOriginalExtent() {
		if (getTypeScale() == AUTOMATICO) {
			view.getMapContext().getViewPort()
					.setExtent(getMapContext().getViewPort().getExtent());
			view.getMapContext().getViewPort().refreshExtent();
		} else if (getTypeScale() == MANUAL) {
			Rectangle2D oldExtent = view.getMapContext().getViewPort()
					.getExtent();
			...
			view.getMapContext()
					.getViewPort()
					.setExtent(
							new Rectangle2D.Double(oldExtent.getX() + xDif,
									oldExtent.getY() + yDif, oldExtent
											.getWidth(), oldExtent.getHeight()));
			view.getMapContext().getViewPort().refreshExtent();
		}
	}
}
