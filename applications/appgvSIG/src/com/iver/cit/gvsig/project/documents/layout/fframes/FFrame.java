 ...
 * todos los FFrames que extenderan de este, dejando uno como m�todo abstracto
 * para implementar por todos los FFrames para ser dibujados.
 * 
...
public abstract class FFrame implements IFFrame {
	...
	private final Cursor curNEResize = Toolkit.getDefaultToolkit()
			.createCustomCursor(iNEResize, new Point(16, 16), "");

	private static final Image iEResize = PluginServices.getIconTheme()
			.get("eresize-icon").getImage();
	private final Cursor curEResize = Toolkit.getDefaultToolkit()
			.createCustomCursor(iEResize, new Point(16, 16), "");

	private static final Image iNResize = PluginServices.getIconTheme()
			.get("nresize-icon").getImage();
	private final Cursor curNResize = Toolkit.getDefaultToolkit()
			.createCustomCursor(iNResize, new Point(16, 16), "");

	private static final Image iMove = PluginServices.getIconTheme()
			.get("move-icon").getImage();
	private final Cursor curMove = Toolkit.getDefaultToolkit()
			.createCustomCursor(iMove, new Point(16, 16), "");

	private static final Image iSEResize = PluginServices.getIconTheme()
			.get("sereresize-icon").getImage();
	private final Cursor curSEResize = Toolkit.getDefaultToolkit()
			.createCustomCursor(iSEResize, new Point(16, 16), "");
	...
	public void drawHandlers(Graphics2D g) {
		...
		g.fillRect((int) r.getCenterX() - (size / 2), (int) r.getY() - size,
				size, size);
		atRotate.transform(
				new Point2D.Double(r.getCenterX() - (size / 2), r.getY() - size),
				p);
	...
	public Rectangle2D getMovieRect(int difx, int dify) {
		...
		lastMoveRect = new Rectangle2D.Double(this.getBoundingBox(null).x,
	 ...
	 * transformada afin que se debe de aplicar y el rect�ngulo que se debe de
	 * dibujar. M�todo que dibuja sobre el graphics que se le pasa como
	 * par�metro, seg�n la transformada afin que se debe de aplicar y el
	 * rect�ngulo que se debe de dibujar.
	 * 
	 * @param g
	 *            Graphics
	 * @param at
	 ...
	 * el �ltimo boundingbox que se calcul�.
	 * 
	 * @param at
	 *            Transformada af�n
	 * 
	...
	/**
	 * Crea un Objeto FFrame seg�n el tipo que sea, a partir de la
	 * informaci�n del XMLEntity.
	 * 
	 * @param xml
	 *            XMLEntity
	 * @param p
	 ...
	 *             DOCUMENT ME!
	 */
	// public static IFFrame createFFrame(XMLEntity xml, Project p,Layout
	// layout)
	// throws OpenException {
	// IFFrame fframe = null;
	...
	public void drawSymbolTag(Graphics2D g) {
		...
		try {
			Image image = PluginServices.getIconTheme().get("symboltag-icon")
					.getImage();
	...
	public static IFFrame createFromXML(XMLEntity xml, Project p, Layout layout)
		...
		try {
			...
			ExtensionPoint extPoint = ((ExtensionPoint) extensionPoints
					.get("FFrames"));
			try {
				flf = (FrameFactory) extPoint.create(xml
						.getStringProperty("className"));
			} catch (InstantiationException e) {
				NotificationManager
						.addError("Clase de FFrame no reconocida", e);
			} catch (IllegalAccessException e) {
				NotificationManager
						.addError("Clase de FFrame no reconocida", e);
			}
			...
			frame.setBoundBox(new Rectangle2D.Double(
					xml.getDoubleProperty("x"), xml.getDoubleProperty("y"), xml
							.getDoubleProperty("w"), xml.getDoubleProperty("h")));
