...
public class FFrameGraphics extends FFrame implements IFFrameEditableVertex {
	...
	public void update(int type, AffineTransform at) {
		...
		int shapeType = -1;
		/*
		 * (jaume) moved to ISymbol switch (m_type) { case (POINT):
		 * 
		 * FSymbol symbolPoint= new FSymbol(FConstant.SHAPE_TYPE_POINT,
		 * m_color); symbolPoint.setSize((int)
		 * FLayoutUtilities.fromSheetDistance( size, at)); m_symbol=symbolPoint;
		 * break;
		 * 
		 * case (RECTANGLE): FSymbol symbolRectangle= new
		 * FSymbol(FConstant.SYMBOL_TYPE_FILL, m_color);
		 * symbolRectangle.setColor(null); m_symbol=symbolRectangle; break;
		 * 
		 * case (LINE): FSymbol symbolLine= new
		 * FSymbol(FConstant.SYMBOL_TYPE_LINE, m_color); m_symbol=symbolLine;
		 * break;
		 * 
		 * case (POLYLINE): FSymbol symbolPolyline= new
		 * FSymbol(FConstant.SYMBOL_TYPE_LINE, m_color);
		 * m_symbol=symbolPolyline; break;
		 * 
		 * case (POLYGON): FSymbol symbolPolygon= new
		 * FSymbol(FConstant.SYMBOL_TYPE_FILL, m_color);
		 * symbolPolygon.setStyle(FConstant.SYMBOL_STYLE_FILL_SOLID);
		 * symbolPolygon.setOutlined(true);
		 * symbolPolygon.setOutlineColor(m_color); symbolPolygon.setColor(null);
		 * m_symbol=symbolPolygon; break;
		 * 
		 * case (CIRCLE): FSymbol symbolCircle= new
		 * FSymbol(FConstant.SYMBOL_TYPE_FILL, m_color);
		 * symbolCircle.setStyle(FConstant.SYMBOL_STYLE_FILL_SOLID);
		 * symbolCircle.setOutlined(true);
		 * symbolCircle.setOutlineColor(m_color); symbolCircle.setColor(null);
		 * m_symbol=symbolCircle; break; }
		 */
		switch (m_type) {
		case (POINT):
	...
	public void draw(Graphics2D g, AffineTransform at, Rectangle2D rv,
		...
		if (intersects(rv, rect)) {
			...
			// BasicStroke basicStroke = new
			// BasicStroke((float)FLayoutUtilities.fromSheetDistance(stroke,at)/100);
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (xml.getChildrenCount() == 1) {
			...
			if (m_type == LINE) {
			...
			} else if (m_type == RECTANGLE || m_type == RECTANGLEGROUP) {
				...
				geometry.addPoint(new Point2D.Double(r.getCenterX(), r
						.getCenterY()));

				if (r.getWidth() < r.getHeight()) {
					geometry.addPoint(new Point2D.Double(r.getMaxX(), r
							.getCenterY()));
				} else {
					geometry.addPoint(new Point2D.Double(r.getCenterX(), r
							.getY()));
	 ...
	 * 
	 * @param size
	 *            entero que representa el tamaño del punto.
	 */
	...
	public void setBoundBox(Rectangle2D r) {
		...
		if (getBoundBox().getWidth() != 0) {
			...
			AffineTransform trans1 = AffineTransform.getTranslateInstance(
					-getBoundBox().getX(), -getBoundBox().getY());
	...
	public void paint(Graphics2D g, AffineTransform at) {
		if (geometryEdit != null) {
			...
			if (getRotation() != 0) {
				...
				/*
				 * FShape m_shape = null; GeneralPathX polyLine = new
				 * GeneralPathX(geometryEdit.getShape());
				 * polyLine.transform(at); m_shape = new FPolyline2D(polyLine);
