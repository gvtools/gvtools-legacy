...
public class CopyPasteLayersUtiles {
		...
		for (int i = 0; i < actives.length; i++) {
			...
			if (actives[i].getParentLayer().getParentLayer() == null) {
				if (!this.addToXMLLayer(actives[i], xmlLayers, xmlTables,
						xmlDataSources))
					return null;
			}// If the layer is not a root layer, is copied if its parent layer
				// is not active
	...
	private boolean addToXMLTable(ProjectTable pt, XMLEntity xmlTables,
		...
		if (xmlTables.findChildren("name", pt.getName()) != null)
			return true;
		...
		try {
			...
			if (pt.getLinkTable() != null) {
				if (xmlTables.findChildren("name", pt.getLinkTable()) == null) {
					ProjectTable ptLink = (ProjectTable) project
							.getProjectDocumentByName(pt.getLinkTable(),
