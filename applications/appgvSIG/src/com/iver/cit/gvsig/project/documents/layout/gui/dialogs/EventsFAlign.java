...
public class EventsFAlign implements ActionListener {
	...
	private void alignCenterVL() {
		double xcenter = 0;
		xcenter = m_layout.getLayoutContext().getAtributes().m_sizePaper
				.getAncho() / 2;
	...
	private void alignRightL() {
		double xmax = 0;
		xmax = m_layout.getLayoutContext().getAtributes().m_sizePaper
				.getAncho();
	...
	/**
	 * Desplaza los fframes seleccionados a la parte superior del fframe que más
	 * arriba este colocado.
	 */
	...
	private void alignUpL() {
		...
		efs.endComplexCommand(PluginServices
				.getText(this, "align_to_layout_up"));
	}
	...
	private void alignCenterHL() {
		double ycenter = 0;
		ycenter = m_layout.getLayoutContext().getAtributes().m_sizePaper
				.getAlto() / 2;
	...
	/**
	 * Distribuye los fframes seleccionados de forma equidistante y vertical, de
	 * izquierda a derecha.
	 */
	...
	/**
	 * Distribuye los fframes seleccionados en el Layout de forma equidistante y
	...
	/**
	 * Distribuye los fframes seleccionados en el Layout de forma equidistante y
	 * vertical, desde arriba hacia abajo.
	 */
	...
	/**
	 * Distribuye los fframes seleccionados en el Layout de forma equidistante y
	 * vertical, desde bajo hacia arriba.
	 */
	...
	/**
	 * Cambia la altura de los fframes seleccionados por la altura del mas alto.
	 */
	...
	private void spaceRight() {
		...
		if (j.hasNext()) {
			...
			while (j.hasNext()) {
				...
				total += (fframeS.getBoundBox().getMinX() - fframeA
						.getBoundBox().getMaxX());
				fframeA = fframeS;
			}
		...
		if (inLayout) {
			total += (xmin + (m_layout.getLayoutContext().getAtributes().m_sizePaper
					.getAncho() - xmax));
			num += 2;
		}
		...
		if (k.hasNext()) {
			...
			if (inLayout) {
				Rectangle2D.Double r = (Rectangle2D.Double) fframeA
						.getBoundBox().clone();
				r.x = dist;
			...
			while (k.hasNext()) {
				...
				Rectangle2D.Double r = (Rectangle2D.Double) fframeS
						.getBoundBox().clone();
	...
	private void spaceDown() {
		...
		if (j.hasNext()) {
			...
			while (j.hasNext()) {
				fframeS = (IFFrame) treemap.get(j.next());
				total += (fframeS.getBoundBox().getMinY() - fframeA
						.getBoundBox().getMaxY());
				fframeA = fframeS;
			}
		...
		if (inLayout) {
			total += (ymin + (m_layout.getLayoutContext().getAtributes().m_sizePaper
					.getAlto() - ymax));
			num += 2;
		}
