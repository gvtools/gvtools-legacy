...
public class FFrameScaleBar extends FFrame implements IFFrameViewDependence {
	 ...
	 * 
	 * @param f
	 *            FFrameView sobre el que obtener la escala.
	 */
	 ...
	 * 
	 * @param s
	 *            número de intervalos a la izquierda.
	 */
		...
		if (intersects(rv, r)) {
			if ((fframeview == null) || (fframeview.getMapContext() == null)) {
				...
				case (NUMERO):
					...
					if (properties != null) {
						PrintQuality pq = (PrintQuality) properties
						...
						if (pq.equals(PrintQuality.NORMAL)) {
							...
							// unitFactor *= 72; (which is the same than doing
							// nothing)
						}
					}
					...
					if (scl < 1) {
						String unknowScale = PluginServices.getText(this,
								"escala_desconocida");
						if (fm.stringWidth(unknowScale) > (d * 0.8)) {
							double dif = fm.stringWidth(unknowScale)
									/ (d * 0.8);
					...
					if (fm.stringWidth(String.valueOf(m_interval)) > (d * 0.8)) {
						double dif = fm.stringWidth(String.valueOf(m_interval))
								/ (d * 0.8);
						m_f = new Font(m_f.getName(), m_f.getStyle(),
								(int) (m_f.getSize() / dif));
						g.setFont(m_f);
	 ...
	 * centímetros) del fframe del cual con una transformación se podrá calcular
	 * el BoundingBox (en pixels).
	 * 
	 * @param r
	...
	private void drawBar(int type, Graphics2D g, AffineTransform at) {
		...
		Rectangle2D.Double rectotal = (FLayoutUtilities.fromSheetRect(
				new Rectangle2D.Double((rect.x + difL), (rect.y + difUp),
		...
		for (int i = 0; i < m_numinterval; i++) {
			Rectangle2D.Double recder = FLayoutUtilities
					.fromSheetRect(
							new Rectangle2D.Double(
									(rect.x + (difL) + ((m_numUnit * i) + (m_hasleft * m_numUnit))),
									(rect.y + (difUp)), (m_numUnit),
									(rect.height - (difUp + difDown)) / 2), at);
			Rectangle2D.Double recderB = FLayoutUtilities
					.fromSheetRect(
							new Rectangle2D.Double(
									(rect.x + (difL) + ((m_numUnit * i) + (m_hasleft * m_numUnit))),
									(rect.y + difUp)
			 ...
			 * recderB.getMaxX()) - rectotal.getMaxX(); }} if (recderB.getMaxY()
			 * != rectotal.getMaxY()) { recderB.height = (recderB.height +
			 * recderB.getMaxY()) - rectotal.getMaxY(); }
			 */
			if ((m_numleft % 2) == 0) {
				if ((i % 2) == 0) {
			...
			if (isAboveIntervals()) {
				p = new Point2D.Double(recder.x - difpos, r.getMinY()
						+ (r.getHeight() / DIFUP));
			} else {
		...
		if (isAboveIntervals()) {
			p = new Point2D.Double(rectotal.getMaxX() - difpos, r.getMinY()
					+ (r.getHeight() / DIFUP));
		} else {
		...
		for (int i = 0; i < m_numleft; i++) {
			Rectangle2D.Double reciz = FLayoutUtilities.fromSheetRect(
					new Rectangle2D.Double(
							(rect.x + difL + ((m_numUnit / m_numleft) * i)),
							(rect.y + difUp), (m_numUnit / numleft),
							(rect.height - (difUp + difDown)) / 2), at);
			Rectangle2D.Double recizB = FLayoutUtilities.fromSheetRect(
					new Rectangle2D.Double(
							(rect.x + difL + ((m_numUnit / m_numleft) * i)),
							(rect.y + difUp)
									+ ((rect.height - (difUp + difDown)) / 2),
		...
		if (m_numleft > 0) {
			...
			if (isAboveIntervals()) {
				p = new Point2D.Double(rectotal.x - difpos, r.getMinY()
						+ (r.getHeight() / DIFUP));
			} else {
		...
		// En el caso de que se pida como númro de intervalos a la izquierda del
		// 0, se reajusta el tamaño del rectángulo exterior de la escala
		// gráfica.
		if (m_numleft == 0) {
			Rectangle2D.Double recAux = FLayoutUtilities.fromSheetRect(
					new Rectangle2D.Double(
	 ...
	 * la descripción, siendo por defecto la escala 1:.
	 * 
	 * @param g
	 *            Graphics sobre el que se dibuja.
	 * @param rec
	 *            Rectángulo que ocupa la escala gráfica.
	 */
	...
	/**
	 * Devuelve el porcentaje por el cual hay que multiplicar el intervalo para
	 * conseguir un intervalo redondeado, de momento con una cifra
	...
	public XMLEntity getXMLEntity() throws SaveException {
		...
		try {
			...
			xml.putProperty("textcolor",
					StringUtilities.color2String(textcolor));
	...
	public void setXMLEntity03(XMLEntity xml, Layout l) {
		...
		fframeview = (FFrameView) l.getLayoutContext().getFFrame(
				xml.getIntProperty("index"));

		if (xml.contains("description")) { // Comprobar que es de la versión que
											// cambia el diálogo.
			this.barcolor = StringUtilities.string2Color(xml
					.getStringProperty("barcolor"));
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (xml.contains("showDescription")) { // Comprobar que es de la versión
												// que cambia el diálogo.
			this.barcolor = StringUtilities.string2Color(xml
					.getStringProperty("barcolor"));
	 ...
	 * 
	 * @param showNameUnits
	 *            True si se muestra el nombre de las unidades de medida.
	 */
	 ...
	 *            True si se muestra arriba la descripción.
	 */
	...
	/**
	 * Inserta si se muestran los valores de los intervalos a la parte de arriba
	 * de la escala o debajo.
	 * 
	 * @param aboveIntervals
	 *            True si se muestran los valores de los intervalos a la parte
	 *            de arriba de la escala gráfica.
	 */
	...
	/**
	 * Inserta si el nombre se muestra a la parte de arriba de la escala gráfica
	 * o a la parte de abajo.
	 * 
	 * @param aboveName
