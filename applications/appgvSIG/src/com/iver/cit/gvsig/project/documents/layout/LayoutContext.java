...
public class LayoutContext {
	 ...
	 *            True True if clean the selection of the fframes already added
	 *            and false if intends to maintain the same selection.
	 * @param select
	...
	private ArrayList allFFrames(IFFrame[] fframes, ArrayList all) {
		for (int i = 0; i < fframes.length; i++) {
			if (fframes[i] instanceof FFrameGroup) {
				ArrayList groupFrames = allFFrames(
						((FFrameGroup) fframes[i]).getFFrames(), all);
