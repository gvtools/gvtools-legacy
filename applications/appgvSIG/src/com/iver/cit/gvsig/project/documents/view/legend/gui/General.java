...
/**
 * This class implements an useful and intuitive graphic interface to change
 * some properties of the layer. This class extends AbstractThemeManager. The
 * properties that allow modified are the name of the layer, the scale, if the
 * user want to use Spatial index or not and the HyperLink. Also shows a scroll
 * with a little resume with the properties of the layer.
 * 
 * @author jmorell
 * 
...
public class General extends AbstractThemeManagerPage {
	...
	public void setModel(FLayer layer) {
		...
		// if (PluginServices.getMDIManager().getActiveWindow() instanceof
		// BaseView ){
		// BaseView theView =
	...
	private GridBagLayoutPanel getPnlScale() {
		if (pnlScale == null) {
			...
			aux2.addComponent(
					PluginServices.getText(this, "este_por_encima_de") + " 1:",
					aux);
			aux = new JPanel(new FlowLayout(FlowLayout.LEFT));
			...
			aux2.addComponent(
					PluginServices.getText(this, "este_por_debajo_de_") + " 1:",
					aux);
	...
	private JScrollPane getScrlProperties() {
		if (scrlProperties == null) {
			...
			scrlProperties.setBorder(BorderFactory
	...
	private JRadioButton getRdBtnShowAlways() {
		if (rdBtnShowAlways == null) {
			...
			rdBtnShowAlways.setText(PluginServices.getText(this,
					"Mostrar_siempre"));
	...
	private JRadioButton getRdBtnDoNotShowWhen() {
		if (rdBtnDoNotShow == null) {
			...
			rdBtnDoNotShow.setText(PluginServices.getText(this,
	...
	private JCheckBox getJCheckBoxSpatialIndex() {
		if (jCheckBoxSpatialIndex == null) {
			...
			jCheckBoxSpatialIndex.setText(PluginServices.getText(this,
	...
	private void showLayerInfo() {
		try {
			...
			if (info == null) {
				...
				CoordinateReferenceSystem viewPortCrs = layer.getMapContext()
						.getCrs();
				...
				if (!layer.getCrs().getName().equals(viewPortCrs.getName())) {
					...
					info += PluginServices.getText(this, "Extent")
							...
							+ PluginServices.getText(this, "Superior") + ":\t"
							+ nativeLayerExtent.getMaxY() + "\n\t"
							+ PluginServices.getText(this, "Inferior") + ":\t"
							+ nativeLayerExtent.getMinY() + "\n\t"
							+ PluginServices.getText(this, "Izquierda") + ":\t"
							+ nativeLayerExtent.getMinX() + "\n\t"
							+ PluginServices.getText(this, "Derecha") + ":\t"
							+ nativeLayerExtent.getMaxX() + "\n";

				}
				if (layer instanceof FLyrVect) {
					ReadableVectorial rv = ((FLyrVect) layer).getSource();
					if (rv instanceof VectorialEditableAdapter) {
						rv = ((VectorialEditableAdapter) ((FLyrVect) layer)
					...
					if (rv instanceof VectorialFileAdapter) {
						...
						DBLayerDefinition dbdef = ((VectorialDBAdapter) rv)
								.getLyrDef();
						info = info + "\n" + rv.getDriver().getName() + "\n";

						try {
							info = info + PluginServices.getText(this, "url")
						...
						info = info + PluginServices.getText(this, "Tabla")
					...
					info += "\n" + PluginServices.getText(this, "type") + ": "
							+ ((FLyrVect) layer).getTypeStringVectorLayer()
							+ "\n";
	 ...
	 */
	private IMonitorableTask getCreateSpatialIndexTask()
	...
	public void applyAction() {
		if (isScaleActive()) {
			try {
				layer.setMinScale((nf.parse(getTxtMaxScale().getText()))
						.doubleValue());
			...
			try {
				layer.setMaxScale((nf.parse(getTxtMinScale().getText()))
						.doubleValue());
			} catch (ParseException ex) {
		...
		if (layer instanceof FLyrVect) {
			...
			if (getSelectedLinkField() != null) {
				...
				vectlyr.getLinkProperties().setField(
				...
				System.out.println("Extensión: "
						+ vectlyr.getLinkProperties().getExt());
				System.out.println("Campo: "
						+ vectlyr.getLinkProperties().getField());
				System.out.println("Tipo: "
						+ vectlyr.getLinkProperties().getType());
			}
		}
	}
