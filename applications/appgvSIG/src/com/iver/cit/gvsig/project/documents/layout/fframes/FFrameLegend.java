...
public class FFrameLegend extends FFrame implements IFFrameViewDependence {
	 ...
	 * 
	 * @param f
	 *            FFrameView para obtener los nombres de las capas.
	 */
	...
	public void draw(Graphics2D g, AffineTransform at, Rectangle2D rv,
		...
		if (intersects(rv, re)) {
			...
			} else if (getQuality() == BORRADOR) { // Si se selecciona la
													// calidad BORRADOR.
				drawDraft(g);
			}
		}
	...
	private void drawLegendOrToFFrame(Graphics2D g, Rectangle2D re, double h,
		...
		int l = 0;
		// ////Se recorren los layers dibujando el símbolo y su nombre sobre el
		// graphics
		for (int i = layers.getLayersCount() - 1; i >= 0; i--) {
			...
			if (b) {
				if (layer instanceof FLayers) {
					...
					if (image != null) {
						FFramePicture picture = (FFramePicture) FrameFactory
								.createFrameFromName(FFramePictureFactory.registerName);
						...
						picture.setImage(bi);
						double dY = n[0] * h;
						Rectangle2D rectImage = new Rectangle2D.Double(
								re.getX(), re.getY() + dY, re.getWidth(), h);
						if (layout != null) {
							picture.setBoundBox(FLayoutUtilities.toSheetRect(
									rectImage, layout.getAT()));
							picture.setPath(path);
							layout.addFFrame(picture, false, true);
						} else {
							picture.setBoundBox(FLayoutUtilities.toSheetRect(
									rectImage, new AffineTransform()));
					...
					double yl = (re.getY() + dY);
					if (layout != null) {
						toFFrameText(layout, layer.getName(), re, sizefont, xl,
								yl, h);
					} else {
	...
	/**
	 * Dibuja sobre el graphics el nombre de la capa que se pasa como parámetro.
	 * 
	 * @param g
	 ...
	 *            Nombre de la capa.
	 * @param re
	 *            Rectángulo a ocupar por el nombre.
	 * @param sizefont
	 *            tamaño de la fuente.
	 * @param x
	 *            Posición X.
	...
	private void toFFrameText(LayoutContext layout, String name,
		...
		// Font f = getFont(sizefont);
		Rectangle2D rAux = new Rectangle2D.Double(x + re.getWidth() / 4,
				((y + (h / 2)) - h), re.getWidth() - re.getWidth() / 4, h);
		FFrameText text = (FFrameText) FrameFactory
				.createFrameFromName(FFrameTextFactory.registerName);
	 ...
	 *            Graphics2D.
	 * @param re
	 *            Rectángulo a cubrir por el símbolo.
	 * @param x
	 *            Posición X.
	 * @param y
	 *            Posición Y.
	...
	private void drawSymbolLegend(Graphics2D g, Rectangle2D re, double x,
		...
		Rectangle rectangle = new Rectangle((int) x,
				(int) ((y + (h / 2)) - haux), (int) wl, (int) haux);
		try {
	 ...
	 * 
	 * @param layout
	 *            Layout sobe el que se añade el FFrame.
	 * @param x
	 *            Posición X.
	 * @param y
	 *            Posición Y.
	...
	private void toFFrameSymbol(LayoutContext layout, Rectangle2D re, double x,
		...
		Rectangle2D rAux3 = new Rectangle2D.Double(x, ((y + (h / 2)) - haux),
				wl, haux);
		FFrameSymbol symbol = (FFrameSymbol) FrameFactory
				.createFrameFromName(FFrameSymbolFactory.registerName);
	...
	private int getSizeNum(FLayers layers) {
		...
		// ///Aquí hay que calcular cuantos layers y sublayers hay, para saber
		// que distancias dejar entre uno y otro.
		// /y además el tamaño de cada uno de ellos para saber que anchura
		// dejar.
		int l = 0;
		for (int i = layers.getLayersCount() - 1; i >= 0; i--) {
			FLayer layer = layers.getLayer(i);
	 ...
	 * 
	 * @param layout
	 *            Layout sobre el que añadir los FFrame nuevos y sobre el que
	 *            elimnar el FFrameLegend anterior.
	 */
	 ...
	 * .awt.Graphics2D, java.awt.geom.AffineTransform)
	 */
