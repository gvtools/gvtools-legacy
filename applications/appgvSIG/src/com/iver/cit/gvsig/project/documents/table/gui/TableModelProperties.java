...
public class TableModelProperties extends AbstractTableModel {
	 ...
	 * 
	 * @param t
	 *            Tabla del proyecto que se quiere mostrar
	...
	public int getRowCount() {
		try {
			...
			NotificationManager.addError(
					"No se pudo leer la información de la tabla", e);
			return 0;
		}
	}
	...
	public Object getValueAt(int rowIndex, int columnIndex) {
		if (columnIndex == 0) {
			...
			try {
				...
				NotificationManager.addError(
						"Error accediendo al nombre del campo", e);
				return "error!";
			}
		} else {
			// rowIndex++;
	...
	public int[] getFieldMapping() {
		...
		int index = 0;
		for (int i = visibles.nextSetBit(0); i >= 0; i = visibles
				.nextSetBit(i + 1)) {
			mapping[index] = i;
			index++;
		}
