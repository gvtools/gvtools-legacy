...
public class MapProperties extends JPanel implements IWindow {
	 ...
	 * @param m
	 *            mapa cuya información se presenta
	 */
	...
	private void initialize() {
		...
		jLabel9.setText(map.getModel().getLayoutContext().getAtributes()
				.getNameUnit());
		jLabel6.setText(map.getModel().getLayoutContext().getAtributes()
				.getNameUnit());

		getTxtHorizontal().setText(
				Double.toString(map.getModel().getLayoutContext()
						.getAtributes().getNumUnitsX()));
		getTxtVertical().setText(
				Double.toString(map.getModel().getLayoutContext()
						.getAtributes().getNumUnitsY()));

		getChkActiveGrid().setSelected(
				map.getModel().getLayoutContext().isAdjustingToGrid());
	...
	private javax.swing.JLabel getJLabel8() {
		if (jLabel8 == null) {
			...
			jLabel8.setText(PluginServices.getText(this, map.getModel()
					.getLayoutContext().getAtributes().getNameUnit()));
		}
		return jLabel8;
	}
	...
	private javax.swing.JCheckBox getChbActiveRuler() {
		if (activeRuler == null) {
			...
			activeRuler.setSelected(map.getModel().getLayoutContext()
					.getRuler());
	...
	private JCheckBox getChbShowGrid() {
		if (showGrid == null) {
			...
			showGrid.setSelected(map.getModel().getLayoutContext()
					.isGridVisible());
	...
	private JCheckBox getChbEditable() {
		if (chbEditable == null) {
			...
			chbEditable.setSelected(map.getModel().getLayoutContext()
					.isEditable());
		...
		if (accept == null) {
			...
			okAction = new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					map.getModel().getLayoutContext()
							.setAdjustToGrid(chkActiveGrid.isSelected());
					map.getModel().getLayoutContext()
							.setGridVisible(showGrid.isSelected());
					map.getModel().getLayoutContext()
							.setRuler(activeRuler.isSelected());
					map.getModel().getLayoutContext()
					...
					try {
						...
						if (sh.length() != 0) {
							...
							map.getModel().getLayoutContext().getAtributes()
									.setNumUnitsX(f);
						}
						...
						PluginServices.getMDIManager().closeWindow(
								MapProperties.this);
						map.getModel().getLayoutContext()
								.setGridVisible(getChbShowGrid().isSelected());
