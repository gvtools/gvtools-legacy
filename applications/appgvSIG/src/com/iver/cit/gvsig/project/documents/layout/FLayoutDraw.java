...
public class FLayoutDraw {
	 ...
	 * @param g2
	 * @param imgBase
	 *            Si es null, estás imprimiendo. Si no, la usas para el código
	 *            de optimización.
	 * 
	...
	public void drawLayout(Graphics2D g2, BufferedImage imgBase)
		...
		for (int i = 0; i < fframes.length; i++) {
			...
			// ESTILO MAC
			/*
			 * if (f instanceof FFrameView) { FFrameView fframe = (FFrameView)
			 * f; BufferedImage img = new BufferedImage((int)
			 * layoutControl.getWidth(), (int) layoutControl.getHeight(),
			 * BufferedImage.TYPE_INT_ARGB);
			 * 
			 * 
			 * fframe.draw(img.createGraphics(), at,
			 * layoutControl.getVisibleRect(), imgBase); g2.drawImage(img, 0, 0,
			 * layoutControl); fframe.setBufferedImage(img); } else { f.draw(g2,
			 * at, layoutControl.getVisibleRect(),imgBase); }
			 */
	...
	public void drawRectangle(Graphics2D g) {
		...
		Rectangle2D.Double rectBig = new Rectangle2D.Double(rLayout.getX(),
				rLayout.getY(), rLayout.getWidth(), rLayout.getHeight());
	 ...
	 *            Graphics2d sobre el que se dibuja.
	 * @param dist
	 *            distancia en centímetros de una línea a otra.
	 * @param init
	 *            inicio de la línea.
	 * @param end
	 *            fin de la línea.
	 * @param y
	 *            rectángulo, dentro del cual se dibujan las líneas.
	 * @param desp
	 *            Desplazamiento.
	 */
	private void drawLineY(Graphics2D g, double dist, int init, int end,
		...
		if (distY > 4) {
			...
			int num = 0;
			double iniY = 40;
			Point2D.Double pfin = FLayoutUtilities.fromSheetPoint(
					new Point2D.Double(layoutControl.getWidth(), layoutControl
							.getHeight()), at);
	 ...
	 * 
	 * try { // Open the image file // InputStream is = new BufferedInputStream(
	 * // new FileInputStream("filename.gif"));
	 * 
	 * // Prepare the output file to receive the postscript OutputStream fos =
	 * new BufferedOutputStream( new FileOutputStream("filename.ps"));
	 * 
	 * // Find a factory that can do the conversion //DocFlavor flavor =
	 * DocFlavor.INPUT_STREAM.GIF; DocFlavor flavor =
	 * DocFlavor.SERVICE_FORMATTED.PRINTABLE; StreamPrintServiceFactory[]
	 * factories = StreamPrintServiceFactory.lookupStreamPrintServiceFactories(
	 * flavor, DocFlavor.BYTE_ARRAY.POSTSCRIPT.getMimeType());
	 * 
	 * if (factories.length > 0) { StreamPrintService service =
	 * factories[0].getPrintService(fos);
	 * 
	 * // Create the print job DocPrintJob job = service.createPrintJob(); Print
	 * print= (Print)PluginServices.getExtension(
	 * com.iver.cit.gvsig.Print.class); print.setLayout(layout); Doc doc = new
	 * SimpleDoc((Printable)print, flavor, null); //Doc doc = new SimpleDoc(is,
	 * flavor, null); // Monitor print job events; for the implementation of
	 * PrintJobWatcher, // see e702 Determining When a Print Job Has Finished
	 * //PrintJobWatcher pjDone = new PrintJobWatcher(job); // Actualizar
	 * attributes PrintRequestAttributeSet att = layout.getAtributes()
	 * .toPrintAttributes(); // Print it job.print(doc, att);
	 * 
	 * // Wait for the print job to be done //pjDone.waitForDone(); // It is now
	 * safe to close the streams }
	 * 
	 * // is.close(); fos.close(); } catch (PrintException e) { } catch
	 * (IOException e) { } }
	 */

	/*
	 * public void printLayout(Layout layout) { l = layout;
	 * 
	 * try { printerJob.setPrintable((Printable) PluginServices.getExtension(
	 * com.iver.cit.gvsig.Print.class));
	 * 
	 * //Actualizar attributes PrintRequestAttributeSet att =
	 * layout.getAtributes() .toPrintAttributes(); DocFlavor flavor =
	 * DocFlavor.SERVICE_FORMATTED.PRINTABLE;
	 * 
	 * if (m_cachePrintServices == null) { m_cachePrintServices =
	 * PrintServiceLookup.lookupPrintServices(flavor, null); }
	 * 
	 * PrintService defaultService = null;
	 * 
	 * if (m_cachePrintService == null) { defaultService =
	 * PrintServiceLookup.lookupDefaultPrintService(); }
	 * 
	 * if (m_cachePrintService == null) { m_cachePrintService =
	 * ServiceUI.printDialog(null, 200, 200, m_cachePrintServices,
	 * defaultService, flavor, att); }
	 * 
	 * if (m_cachePrintService != null) { DocPrintJob jobNuevo =
	 * m_cachePrintService.createPrintJob(); PrintJobListener pjlistener = new
	 * PrintJobAdapter() { public void printDataTransferCompleted(PrintJobEvent
	 * e) { System.out.println("Fin de impresión"); } };
	 * 
	 * jobNuevo.addPrintJobListener(pjlistener);
	 * 
	 * Doc doc = new SimpleDoc((Printable) PluginServices.getExtension(
	 * com.iver.cit.gvsig.Print.class), flavor, null); jobNuevo.print(doc, att);
	...
	public void toPS(File ps) {
		...
		try {
			...
			if (factories.length > 0) {
				...
				PrintRequestAttributeSet att = attributes.toPrintAttributes();
				// Print it
				job.print(doc, att);
		 ...
		 * PrintServiceLookup.lookupDefaultPrintService(); DocPrintJob
		 * printerJob = defaultPrintService.createPrintJob();
		 * 
		 * try { DocFlavor flavor=DocFlavor.URL.POSTSCRIPT; if
		 * (!defaultPrintService.isDocFlavorSupported(flavor)) {
		 * System.err.println
		 * ("The printer does not support the appropriate DocFlavor"); }else {
		 * 
		 * SimpleDoc simpleDoc; simpleDoc = new SimpleDoc(ps.toURL(),flavor,
		 * null); printerJob.print(simpleDoc, null); } } catch
		 * (MalformedURLException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } catch (PrintException e) { // TODO
