 ...
 * 
 * If the feature Info comes from a special layer which has registered the way
 * to visualize itself, adds a panel that the layer should provide otherwise
 * this viewer will add a panel to visualize HTML or a special viewer to show
 * XML.
 * 
 * @author laura
 * 
...
public class InfoToolViewer extends JPanel {
	...
	private void initilizeLayerListModel() {
		...
		FormatListModel model = new FormatListModel(
				(String[]) layerNames.toArray(new String[0]));
	...
	private void updateViewer(int changedIdx) {
		...
		if (layer instanceof InfoByPoint) {
			...
			if (XULPanelClass != null
					&& defaultLayer.getProperty("infoTool.XULFile") != null) {
				try {
					IXULInfoToolSupport infoPanel = (IXULInfoToolSupport) XULPanelClass
							.newInstance();
					infoPanel.setXULFile((String) defaultLayer
							.getProperty("infoTool.XULFile"));
			...
			if (defaultLayer.getProperty("customPanel") != null) {
				...
				if (item.toString().toLowerCase().endsWith("</html>"))// if
																		// (item.toString().toLowerCase().startsWith(
																		// "<html>"))
				{
