...
/**
 * Menu de botón derecho para el TOC. Se pueden añadir entradas facilmente desde
 * una extensión, creando una clase derivando de TocMenuEntry, y añadiendola en
 * estático (o en tiempo de carga de la extensión) a FPopupMenu. (Las entradas
 * actuales están hechas de esa manera).
 * 
...
public class FPopupMenu extends JPopupMenu {
	...
	static {
		/*
		 * Cambiados FPopupMenu.addEntry(new FSymbolChangeColorTocMenuEntry());
	...
	public static Object getEntry(String className) {
		...
		try {
			action = (OldTocContextMenuAction) ((ExtensionPoint) extensionPoints
					.get("View_TocActions")).create(className);
	...
	private IContextMenuAction[] getActionList() {
		...
		if (contains) {
			while (iter.hasNext()) {
				action = null;
				try {
					action = (AbstractTocContextMenuAction) this.extensionPoint
				...
				if (action != null
					...
					if (action.isVisible((ITocItem) this.getNodeUserObject(),
							this.selecteds)) {
			...
			IContextMenuAction[] result = (IContextMenuAction[]) Array
					.newInstance(IContextMenuAction.class,
