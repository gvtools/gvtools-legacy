...
public class FFrameText extends FFrame {
	...
	public void draw(Graphics2D g, AffineTransform at, Rectangle2D rv,
		...
		if (intersects(rv, raux)) { // comprueba que no cae fuera de la pantalla
			...
			if (m_text.isEmpty()) {
				...
				if (m_f != null) {
					// Aquí se ajusta a partir de las características de la
					// fuente, una nueva fuente con el tamaño ajustado.
					m_f = new Font(m_f.getFontName(), m_f.getStyle(),
							scaledFontSize);
				} else {
					// Aquí pasa cuando se abre el diálogo para seleccionar un
				...
				if (hasTitle()) {
					...
					if (isSurrounded()) {
						...
						gap += (int) (FLayoutUtilities.fromSheetDistance(
				...
				if (isSurrounded()) {
					...
					// Recalibro el rectangulo para establecer el área donde se
					// dibujan las fuentes
					// al área marcada con el ratón menos la distancia al marco
					// especificada
					raux.setRect(raux.getX() + scaledCellPaddingmm + bordermm,
							raux.getY() + scaledCellPaddingmm + bordermm,
							raux.getWidth() - (scaledCellPaddingmm * 2)
									- bordermm * 2, raux.getHeight()
									- (scaledCellPaddingmm * 2) - bordermm * 2);
	 ...
	 * 
	 * @param r
	 *            Rectángulo sobre el que dibujar.
	 * @param sfs
		...
		if (isFontSizeFixed()) {
			...
			for (int i = 0; i < m_text.size(); i++) {
				if (!((String) m_text.get(i)).equals("")
					...
					double difH = (txtBound.getHeight() * m_text.size())
							/ (r.getHeight());
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (xml.contains("frameColor")) {
			this.frameColor = StringUtilities.string2Color(xml
					.getStringProperty("frameColor"));
		}

		if (xml.contains("titleColor")) {
			this.titleColor = StringUtilities.string2Color(xml
					.getStringProperty("titleColor"));
		}
	...
	/**
	 * True if the FFrameText is set to draw an involving rectangle, or false if
	 * not.
	 * 
	 * @return boolean
	 */
