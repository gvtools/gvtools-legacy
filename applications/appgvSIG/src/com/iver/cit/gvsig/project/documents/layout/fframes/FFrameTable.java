...
public class FFrameTable extends FFrameGroup implements IFFrameGroupSelectable {
	...
	public void calculateTable(Rectangle2D r) {
		...
		for (int i = 0; i < numColumns; i++) {
			for (int j = 0; j < rows; j++) {
				...
				FFrameBasic basic = (FFrameBasic) FrameFactory
						.createFrameFromName(FFrameBasicFactory.registerName);
	...
	/*
	 * public int getSelected() { // if (!selectFFrameBasic) return
	 * super.getSelected(); // IFFrame[] fframes=getFFrames(); // for (int i =
	 * 0;i<fframes.length;i++){ // int selection=fframes[i].getSelected(); // if
	 * (selection>0){ // return selection; // } // } // return 0;
	 * 
	 * }
	 */
	...
	public void drawHandlersGroup(Graphics2D g) {
		...
		for (int i = 0; i < fframes.length; i++) {
			...
			if (fframe.getSelected() != IFFrame.NOSELECT) {
				...
				atRotate.transform(
						new Point2D.Double(r.getX() - size / 2, r.getY() - size
								/ 2), p);
				...
				g.fillRect((int) r.getMaxX() - size / 2, (int) r.getY() - size
						/ 2, size, size);
				atRotate.transform(
						new Point2D.Double(r.getMaxX() - size / 2, r.getY()
								- size / 2), p);
				...
				g.fillRect((int) r.getX() - size / 2, (int) r.getMaxY() - size
						/ 2, size, size);
				atRotate.transform(
						new Point2D.Double(r.getX() - size / 2, r.getMaxY()
								- size / 2), p);
				...
				g.fillRect((int) r.getMaxX() - size / 2, (int) r.getCenterY()
						- (size / 2), size, size);
				atRotate.transform(
						new Point2D.Double(r.getMaxX() - size / 2, r
								.getCenterY() - (size / 2)), p);
	...
	/*
	 * public void drawHandlersGroup(Graphics2D g) { g.setColor(Color.gray);
	 * IFFrame[] fframes=getFFrames(); for (int i = 0; i < fframes.length; i++)
	 * { IFFrame fframe = fframes[i]; if
	 * (fframe.getSelected()!=IFFrame.NOSELECT){ fframe.drawHandlers(g); } } }
	 */
