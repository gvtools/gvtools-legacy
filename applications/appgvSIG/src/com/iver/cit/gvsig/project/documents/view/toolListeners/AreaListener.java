			...
			mF.getStatusBar()
					.setMessage(
							...
							"P:"
									+ nf.format(distAll
											/ MapContext
													.getDistanceTrans2Meter()[distanceUnits])
									+ "");
			mF.getStatusBar()
					.setMessage("5",
					...
					// / new Point2D.Double(
					// / event.getXs()[event.getXs().length - 2].doubleValue(),
					// / event.getYs()[event.getYs().length -
					// 2].doubleValue())))/FMap.CHANGEM[mapCtrl.getViewPort().getDistanceUnits()])
					// + "");
							"A:"
									+ nf.format(returnArea(new Point2D.Double(
											event.getXs()[event.getXs().length - 2]
													.doubleValue(),
											event.getYs()[event.getYs().length - 2]
													.doubleValue()))
											/ Math.pow(
													MapContext
															.getAreaTrans2Meter()[distanceArea],
													2)) + "");
			mF.getStatusBar().setMessage("distancearea",
					MapContext.getAreaAbbr()[distanceArea]);
		}
	}
}
