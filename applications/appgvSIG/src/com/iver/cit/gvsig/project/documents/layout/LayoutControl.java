...
public class LayoutControl extends JComponent implements LayoutDrawListener {
	...
	// .createCustomCursor(Toolkit.getDefaultToolkit().createImage(new
	// MemoryImageSource(
	...
	public LayoutControl(Layout layout) {
		...
		LayoutAddOverViewListenerImpl laovl = new LayoutAddOverViewListenerImpl(
				layout);
		addLayoutTool("layoutaddoverview", new LayoutRectangleBehavior(laovl));

		LayoutAddPictureListenerImpl lapl = new LayoutAddPictureListenerImpl(
				layout);
		addLayoutTool("layoutaddpicture", new LayoutRectangleBehavior(lapl));

		LayoutAddNorthListenerImpl lanorthl = new LayoutAddNorthListenerImpl(
				layout);
		...
		LayoutAddLegendListenerImpl lall = new LayoutAddLegendListenerImpl(
				layout);
		...
		LayoutAddPointListenerImpl lapointl = new LayoutAddPointListenerImpl(
				layout);
		addLayoutTool("layoutaddpoint", new LayoutPointBehavior(lapointl));

		LayoutAddLineListenerImpl lalinel = new LayoutAddLineListenerImpl(
				layout);
		addLayoutTool("layoutaddline", new LayoutPointBehavior(lalinel));

		LayoutAddPolygonListenerImpl lapolygonl = new LayoutAddPolygonListenerImpl(
				layout);
		addLayoutTool("layoutaddpolygon", new LayoutPointBehavior(lapolygonl));

		LayoutAddPolylineListenerImpl lapolylinel = new LayoutAddPolylineListenerImpl(
				layout);
		addLayoutTool("layoutaddpolyline", new LayoutPointBehavior(lapolylinel));

		LayoutAddCircleListenerImpl lacirclel = new LayoutAddCircleListenerImpl(
				layout);
		addLayoutTool("layoutaddcircle", new LayoutPointBehavior(lacirclel));

		LayoutAddRectangleListenerImpl larectanglel = new LayoutAddRectangleListenerImpl(
				layout);
		...
		LayoutViewZoomInListenerImpl lvzil = new LayoutViewZoomInListenerImpl(
				layout);
		addLayoutTool("layoutviewzoomin", new LayoutViewZoomBehavior(lvzil));

		LayoutViewZoomOutListenerImpl lvzol = new LayoutViewZoomOutListenerImpl(
				layout);
		...
		LayoutEditGraphicsListenerImpl leditl = new LayoutEditGraphicsListenerImpl(
				layout);
	...
	/**
	 * Returns the rectangle that represents the sheet. In pixels.
	 * 
	...
	/**
	 * Inserts the rectangle that represents the sheet. In pixels.
	 * 
	 */
	...
	public void setTool(String toolName) {
		...
		LayoutBehavior layoutTool = (LayoutBehavior) namesLayoutTools
				.get(toolName);
	...
	public void setPosition(Point point2) {
		if (layoutContext.isAdjustingToGrid()
			...
			position = FLayoutUtilities.getPointGrid(point2, layoutContext
					.getAtributes().getNumUnitsX(), layoutContext
					.getAtributes().getNumUnitsY(), layoutContext.getAT());
		} else {
			position = point2;
		}
	}
