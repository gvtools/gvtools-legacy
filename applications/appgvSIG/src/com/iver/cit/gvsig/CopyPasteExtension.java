...
public class CopyPasteExtension extends Extension {
	...
	public void initialize() {
		 ...
		 * 
		 * 
		 * // TOC MyTocMenuEntry copy = new CopyTocMenuEntry(); MyTocMenuEntry
		 * cut = new CutTocMenuEntry(); MyTocMenuEntry paste = new
		 * PasteTocMenuEntry(); Utiles utiles = new Utiles();
		 * copy.setUtiles(utiles); cut.setUtiles(utiles);
		 * paste.setUtiles(utiles);
		 * 
		 * extensionPoints.add("View_TocActions","Copy",
		 * "Copy selectes layers to system clipboard",copy);
		 * extensionPoints.add(
		 * "View_TocActions","Cut","Cut selectes layers to system clipboard",
		 * cut); extensionPoints.add("View_TocActions","Paste",
		 * "Paste layers from system clipboard",paste);
		 * 
		 * 
		 * //FPopupMenu.addEntry(copy); //FPopupMenu.addEntry(cut);
		 * //FPopupMenu.addEntry(paste);
		 * 
		 * 
		 * // ProjectWindow CopyProjectElement copyProjectElement = new
		 * CopyProjectElement(); CutProjectElement cutProjectElement = new
		 * CutProjectElement(); PasteProjectElement pasteProjectElementView =
		 * new PasteProjectElement(); PasteProjectElement
		 * pasteProjectElementTable = new PasteProjectElement();
		 * PasteProjectElement pasteProjectElementMap = new
		 * PasteProjectElement();
		 * 
		 ...
		 * 
		 * 
		 * extensionPoints.add("DocumentActions_View","Copy",
		 * "Copy selectes documento to system clipboard",copyProjectElement);
		 * extensionPoints.add("DocumentActions_View","Cut",
		 * "Cut selectes documento to system clipboard", cutProjectElement);
		 * extensionPoints
		 * .add("DocumentActions_View","Paste","Paste views from system clipboard"
		 * ,pasteProjectElementView);
		 * 
		 * 
		 * extensionPoints.add("DocumentActions_Table","Copy",
		 * "Copy selectes documento to system clipboard",copyProjectElement);
		 * extensionPoints.add("DocumentActions_Table","Cut",
		 * "Cut selectes documento to system clipboard", cutProjectElement);
		 * extensionPoints
		 * .add("DocumentActions_Table","Paste","Paste tables from system clipboard"
		 * ,pasteProjectElementTable);
		 * 
		 * extensionPoints.add("DocumentActions_Map","Copy",
		 * "Copy selectes documento to system clipboard",copyProjectElement);
		 * extensionPoints.add("DocumentActions_Map","Cut",
		 * "Cut selectes documento to system clipboard", cutProjectElement);
		 * extensionPoints
		 * .add("DocumentActions_Map","Paste","Paste maps from system clipboard"
		 * ,pasteProjectElementMap);
		 */
	}
...
abstract class MyDocumentAction extends AbstractDocumentContextMenuAction
		implements IExtensionBuilder {
...
class PasteProjectElement extends MyDocumentAction {
	...
	public String getDescription() {
		// FIXME: Falta claves
		// return
		// PluginServices.getText(this,"tooltip_pegar_desde_el_portapapeles");
		return null;
	}
	...
	public boolean isEnabled(ProjectDocument item,
		...
		if (this.utiles.getXMLEntityChildOfType(xml, this.type) == null)
			return false;
		return true;
	}
...
class PasteTocMenuEntry extends MyTocMenuEntry {
	...
	private XMLEntity getCheckedXMLFromClipboard() {
		...
		if (this.utiles.getXMLEntityChildOfType(xml, "layers") == null)
			return null;
	...
	public void execute(ITocItem item, FLayer[] selectedItems) {
		...
		if (isOK) {
			...
			if (view instanceof BaseView)
				((ProjectDocument) ((BaseView) view).getModel())
						.setModified(true);
		}
	}
...
class Utiles {
	...
	/*
	 * 
	 * ====================================== Comportamiento del Pegar
	 * documentos: ======================================
	 * 
	 *  Pegar vista. Si ya existe una vista en el proyecto con el mismo nombre.
	 * 1. Abortar 2. Pedir nuevo nombre. Que hacemos con las tablas asociadas.
	 * No se pegan Si alguna de las tablas a pegar de las que van en el
	 * portapapeles ya existen en el proyecto. 1. abortamos 2. Informamos al
	 * usuario y no se pegan las tablas.
	 * 
	 * Pegar tabla. Si alguna de las tablas existe. Se pega igualmente
	 * (apareceran tablas duplicadas)
	 * 
	 * Pegar mapa. Si el mapa ya existe en el proyecto. 1. Abortar 2. renombrar
	 * el mapa Si alguna vista ya existe en el proyecto. 1. Abortar 2. Usar la
	 * vista que ya existe en el proyecto y no pegar la nueva vista. Si alguna
	 * de las tablas a pegar de las que van en el portapapeles ya existen en el
	 * proyecto. 1. abortamos 2. Informamos al usuario y no se pegan las tablas.
	 */
	...
	/**
	 * Genera un XMLEntity con la informacion necesaria para copiar los
	 * elementos de selectedItems en otro proyecto
	 */
	...
	public XMLEntity generateXMLCopyViews(ProjectView[] selectedItems) {
		...
		for (int i = 0; i < selectedItems.length; i++) {
			if (!this.addToXMLView(selectedItems[i], xmlViews, xmlTables,
					xmlDataSources))
				return null;
	...
	public XMLEntity generateXMLCopyMaps(ProjectMap[] selectedItems) {
		...
		for (int i = 0; i < selectedItems.length; i++) {
			if (!this.addToXMLMap(selectedItems[i], xmlMaps, xmlViews,
					xmlTables, xmlDataSources))
				return null;
	...
	public XMLEntity generateXMLCopyTables(ProjectTable[] selectedItems) {
		...
		for (int i = 0; i < selectedItems.length; i++) {
			if (!this.addToXMLTable(selectedItems[i], xmlTables,
					xmlDataSources, null))
				return null;
		}
	...
	public XMLEntity generateXMLCopyLayers(FLayer[] actives) {
		...
		for (int i = 0; i < actives.length; i++) {
			if (!this.addToXMLLayer(actives[i], xmlLayers, xmlTables,
					xmlDataSources))
				return null;
	...
	public boolean loadLayersFromXML(XMLEntity xml, FLayers root) {
		...
		XMLEntity xmlDataSources = this.getXMLEntityChildOfType(xml,
				"dataSources");
		 ...
		 * 
		 * Hashtable tablesConfits =
		 * this.getConflicts(xmlTables,project.getTables());
		 */
	...
	public boolean loadViewsFromXML(XMLEntity xml) {
		...
		Hashtable viewsConflits = this.getConflicts(xmlViews,
				project.getDocumentsByType(ProjectViewFactory.registerName));

		Hashtable tablesConflits = this.getConflicts(xmlTables,
				project.getDocumentsByType(ProjectTableFactory.registerName));
	...
	public boolean loadMapsFromXML(XMLEntity xml) {
		...
		Hashtable mapsConflits = this.getConflicts(xmlMaps,
				project.getDocumentsByType(ProjectMapFactory.registerName));
			...
			while (en.hasMoreElements()) {
				...
				if (newName == null) {
					JOptionPane.showMessageDialog(
							...
							PluginServices.getText(this, "pegar_mapas"),// titulo
							JOptionPane.ERROR_MESSAGE);
				} else if (newName.equalsIgnoreCase(map
						.getStringProperty("name"))) {
					JOptionPane.showMessageDialog(
							(Component) PluginServices.getMainFrame(),
							"<html>"
		...
		if (viewsConflits != null && viewsConflits.size() > 0) {
			int option = JOptionPane.showConfirmDialog(
					...
					"<html>"
							...
							+ "<br>"
							+ PluginServices.getText(this, "desea_continuar")
							+ "</html>", PluginServices.getText(this,
							"pegar_mapas"),// titulo
					JOptionPane.YES_NO_OPTION);
			if (option != JOptionPane.YES_OPTION) {
				return false;
	...
	/**
	 * Devuelve las claves de conflits ordenados en orden inverso. Las claves se
	 * esperan que sean instancias de Integer
	 */
	...
	private boolean addToXMLMapDependencies(ProjectMap map, XMLEntity xmlViews,
			XMLEntity xmlTables, XMLEntity xmlDataSources) {
		...
		for (int i = 0; i < components.length; i++) {
			if (components[i] instanceof FFrameView) {
				...
				if (findChildInXML(xmlViews, "name", view.getName()) == null) {
					if (!this.addToXMLView(view, xmlViews, xmlTables,
							xmlDataSources))
						return false;
				}
			}
	...
	private boolean addToXMLMap(ProjectMap map, XMLEntity xmlMaps,
			XMLEntity xmlViews, XMLEntity xmlTables, XMLEntity xmlDataSources) {
		try {
			xmlMaps.addChild(map.getXMLEntity());

			return this.addToXMLMapDependencies(map, xmlViews, xmlTables,
					xmlDataSources);
	...
	private boolean addToXMLView(ProjectView view, XMLEntity xmlViews,
			XMLEntity xmlTables, XMLEntity xmlDataSources) {
		try {
			xmlViews.addChild(view.getXMLEntity());

			if (!this.addToXMLLayerDependencies(view.getMapContext()
					.getLayers(), xmlTables, xmlDataSources))
				return false;

			if (view.getMapOverViewContext() != null) {
				return this.addToXMLLayerDependencies(view
						.getMapOverViewContext().getLayers(), xmlTables,
						xmlDataSources);
			} else {
				return true;
			}
	...
	private Hashtable getConflicts(XMLEntity xml, ArrayList elements) {
		if (xml == null || xml.getChildrenCount() < 1)
			return null;
		...
		try {
			...
			for (int i = 0; i < numDataSources; i++) {
				...
				if (dsFactory.getDriverInfo(name) == null) {
					if (child.getStringProperty("type").equals(
							"otherDriverFile")) {
					...
					} else if (child.getStringProperty("type").equals(
							"sameDriverFile")) {
						/*
						 * String layerName =
						 * child.getStringProperty("layerName"); ProjectView
						 * vista =
						 * project.getViewByName(child.getStringProperty(
						 * "viewName")); FLayer layer =
						 * vista.getMapContext().getLayers
						 * ().getLayer(layerName);
						 * 
	...
	private boolean addTables(XMLEntity xmlTables) {
		try {
			...
			for (int i = 0; i < numTables; i++) {
				try {
					ProjectTable ptable = (ProjectTable) ProjectTable
							.createFromXML(xmlTables.getChild(i), project);
					project.addDocument(ptable);
					/*
					 * if (ptable.getSeedViewInfo()!=null &&
					 * ptable.getAndamiView()!=null) { // open the view, if it
					 * was open, and restore its dimensions
					 * PluginServices.getMDIManager
					 * ().addView(ptable.getAndamiView());
					 * PluginServices.getMDIManager
					 * ().changeViewInfo(ptable.getAndamiView(),
					 * ptable.getSeedViewInfo()); }
					 */
	...
	private boolean addLayers(XMLEntity xmlLayers, FLayers root) {
		try {
			...
			for (int i = 0; i < numLayers; i++) {
				...
				if (!root.addLayerFromXMLEntity(child, null))
					return false;
			}
			return true;
		} catch (Exception e) {
	...
	private boolean addViews(XMLEntity xmlViews) {
		try {
			...
			for (int i = 0; i < numLayers; i++) {
				...
				ProjectView pv = (ProjectView) ProjectView.createFromXML(child,
						project);
	...
	private boolean addMaps(XMLEntity xmlMaps) {
		try {
			...
			for (int i = 0; i < numLayers; i++) {
				...
				ProjectMap pm = (ProjectMap) ProjectMap.createFromXML(child,
						project);
	...
	private boolean addToXMLDataSource(SourceInfo source,
			XMLEntity xmlDataSources, Project project) {
		if (project == null) {
	...
	private boolean addToXMLTable(ProjectTable pt, XMLEntity xmlTables,
			XMLEntity xmlDataSources, Project project) {
		if (project == null) {
			project = this.getProject();
		}
		if (findChildInXML(xmlTables, "name", pt.getName()) != null)
			return true;
		...
		try {
			...
			if (pt.getAssociatedTable() != null) {
				this.addToXMLDataSource(pt.getAssociatedTable().getRecordset()
						.getSourceInfo(), xmlDataSources, project);
			}
			...
			if (pt.getLinkTable() != null) {
				if (findChildInXML(xmlTables, "name", pt.getLinkTable()) == null) {
					...
					if (!this.addToXMLTable(ptLink, xmlTables, xmlDataSources,
							project))
						return false;
				}
			}
	...
	private boolean addToXMLLayerDependencies(FLayer lyr, XMLEntity xmlTables,
			XMLEntity xmlDataSources) {
		try {
			Project project = this.getProject();
			...
			if (lyr instanceof FLayers) {
				...
				for (int i = 0; i < count; i++) {
					...
					this.addToXMLLayerDependencies(subLyr, xmlTables,
							xmlDataSources);
				}

			} else if (lyr instanceof AlphanumericData) {
				if (!this.addToXMLDataSource(((AlphanumericData) lyr)
						.getRecordset().getSourceInfo(), xmlDataSources,
						project
				...
				if (pt != null) {
					if (!this.addToXMLTable(pt, xmlTables, xmlDataSources,
							project))
						return false;
				}
	...
	private boolean addToXMLLayer(FLayer lyr, XMLEntity xmlLayers,
			XMLEntity xmlTables, XMLEntity xmlDataSources) {
		try {
			xmlLayers.addChild(lyr.getXMLEntity());

			return this.addToXMLLayerDependencies(lyr, xmlTables,
					xmlDataSources);
