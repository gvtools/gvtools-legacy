...
public class makeEPSG {
	...
	public static void main ( String args[]) {
		...
		if ( args.length != 1 ) {
			System.out.println ("Convert an SQL script to HSQLDB execute statements (one per line)");
			System.out.println ("Usage: java makeEPSG <input>");
			System.out.println ("The output will be a file called 'ESPG.sql' in the current directory.");
			System.out.println ("All text processing will be done in ISO-8859-15.\n");
			System.out.println ("This program was designed to be run on preprocessed input that was");
			System.out.println ("prepared as described in the document 'PrepareForHSQL.xml', which");
			System.out.println ("is part of the Geotools project source files.\n");
			System.out.println ("Once the EPSG PostgreSQL scripts have been modified as described in above");
			System.out.println ("document, merge them into one file, in the correct order:");
			System.out.println ("  cat EPSG_Tables.sql EPSG_Data.sql EPSG_FKeys.sql > input.sql\n");
			System.out.println ("Then run input.sql through this program to obtain the properly formatted EPSG.sql");
			System.out.println ("file. EPSG.sql can then be injected into gvSIG's geotools-epsg-hsql-2.1.1_gvsig.jar");
		...
		try {
			...
			if ( inFile.exists() ) {			
				System.out.println ( "Processing (" + isr.getEncoding() + "): " + args[0] );
				...
				while ( line != null ) {			
					...
					if ( upperCase.startsWith("INSERT INTO")) {						
						...
						final String tableName = line.substring(12,upperCase.indexOf("VALUES")).trim();
						if ( lastInsert.equals( tableName ) )  {//write into open statement
							...
							line = new String ("insert into " + tableName + " values" + token2);
					...
					if ( !line.trim().endsWith(terminator) ) {
						...
						i ++;
						output=output+line.substring(0, line.lastIndexOf(";"));
						//Careful with "\n" and "," at end of line
						if ( needsComma == true ) {
								out.write(",");
								needsComma = false;
