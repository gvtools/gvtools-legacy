...
public class LayerOrderExtension extends Extension implements
	...
	// Not necessary: the page gets automatically registered by implemented
	// IPreferenceExtension
	...
	private void registerDefaultManager() {
		...
		ExtensionPointsSingleton.getInstance().add(
				DefaultOrderManager.getExtensionPointName(), manager.getCode(),
				manager);
	...
	private void initDefaultOrderManager() {
		...
		if (xml != null
				&& xml.contains(DefaultOrderManager.getExtensionPointName())) {
			String code = xml.getStringProperty(DefaultOrderManager
			...
			try {
				...
				if (ep.containsKey(code)) {
					ep.addAlias(code,
							DefaultOrderManager.getDefaultManagerKey());
				}
			} catch (Exception ex) {
	...
	public void setDefaultOrderManager(OrderManager manager) {
		...
		if (ep != null) {
			try {
				...
				ep.addAlias(manager.getCode(),
						DefaultOrderManager.getDefaultManagerKey());
			} catch (KeyException e) {
			} // this can't happen, we have ensured the key is present
		}

		PluginServices ps = PluginServices.getPluginServices(this);
		XMLEntity xml = ps.getPersistentXML();
		xml.putProperty(DefaultOrderManager.getExtensionPointName(),
				manager.getCode());
	}
	...
	/**
	 * Checks whether an OrderManager was configured as default in the extension
	 * point.
	 * 
