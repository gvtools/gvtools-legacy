...
public class SQLQueryValidation {
	...
	private int[] errorPosition; // [0] -> line, [1] -> column ( (-1, -1) -> no
									// error ) ( (-2, -2) -> error: query =
									// null)
	 ...
	 * @param _onlyWhereStatement
	 *            If the query is only the part of WHERE ... in a query
	 */
	...
	public boolean validateQuery() {
		...
		if (query == null) {
			...
			errorMessage = new String(PluginServices.getText(null,
					"queryIsNull"));
		...
		// Converts all apostrophes between aphostrophes to double inverted
		// commas ( " )
		int index = 0;
		...
		// Converts all ocurrences of the symbol " ( double inverted commas ) to
		// space because Zql doesn't support that symbol
		if (onlyWhereStatement)
			completeQuery = preQuery
					+ formatted_query.trim().replaceAll("\"", " ");
		else
			completeQuery = formatted_query.trim().replaceAll("\"", " ");

		if ((completeQuery.length() > 0)
				&& (completeQuery.charAt(completeQuery.length() - 1) != ';'))
			completeQuery += ";";
		...
		try {
			...
			token = new String(errorMessage.substring(ini_pos,
					errorMessage.indexOf('\"', ini_pos)));

			// Get the line an column where the syntax error starts
			String line = new String(errorMessage.substring(e.getMessage()
					.indexOf("line"), errorMessage.indexOf(',')));
			line = line.substring(line.indexOf(' ') + 1, line.length());

			String column = new String(e.getMessage().substring(
					errorMessage.indexOf("column"), errorMessage.indexOf('.')));
			...
			if (onlyWhereStatement) {
				this.errorPosition[1] -= this.preQueryLenght; // Substract the
																// lenght of the
																// pre-query
																// added
			}
	...
	/**
	 * Returns an string with an error message if there has been an error, or
	 * 'null' if there hasn't been
	 * 
	...
	/**
	 * Returns an string with a text describing the (first) position of the
	 * error
	 * 
	 * @return An string or null
	 */
	...
	/**
	 * Returns the error position (line, column) or (-1, -1) if there hasn't
	 * been any error
	 * 
	 ...
	 * 
	 * @param position
	 *            An array with 2 elements: (row, column)
	 */
	private void defineErrorPositionAsMessageAttribute(int position[]) {
		// Defines the error message
		errorPositionAsMessage = new String(
				PluginServices.getText(null, "line") + ": " + errorPosition[0]
