...
public class SingleVectorialDBConnectionExtension extends Extension {
	private static Logger logger = Logger
			.getLogger(SingleVectorialDBConnectionExtension.class.getName());

	/**
	 * This method simply loads the connections stored in gvSIG's persistence
	 * file as closed connections
	 */
		...
		PluginServices.getIconTheme()
				.register(
						...
						this.getClass().getClassLoader()
								.getResource("images/conn.png"));
		PluginServices.getIconTheme().register(
				"disconn-image",
				this.getClass().getClassLoader()
						.getResource("images/disconn.png"));
	...
	public static void saveAllToPersistence() {
		...
		XMLEntity xml = PluginServices.getPluginServices(ext)
				.getPersistentXML();
	...
	private void loadPersistenceConnections() {
		...
		try {
			...
			System.err.println("Error while getting projects db-connections: "
					+ e.getMessage());
		...
		PluginServices ps = PluginServices
				.getPluginServices("com.iver.cit.gvsig.jdbc_spatial");
		if (ps == null)
			return;
		...
		try {
			...
			System.err
					.println("Error while getting projects jdbc-connections: "
							+ e.getMessage());
	...
	private void addDisconnected(ConnectionSettings _cs) {
		try {
			SingleDBConnectionManager.instance().getConnection(_cs.getDriver(),
