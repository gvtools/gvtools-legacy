...
/**
 * This class create a query using the gazetteer query language used by ADL
 * 
 * 
 ...
 */
	...
	public String getQuery(GazetteerQuery query) {
		...
		if (query.getName() != null) {
			if (query.getOptions().getSearch().isWithAccents()) {
				...
				filter.addClauses("title", query.getName(),
						query.getNameFilter(), "and");
			}
		}

		if ((query.getFeatures() != null) && (query.getFeatures().length != 0)) {
			if (!(query.getFeatures()[0].getName().equals("ThesaurusRoot")))
				filter.addClauses("thesaurus",
						query.getFeatures()[0].getName(), "E", "and");
		}
		...
		if ((query.getCoordinates() != null) && (query.isCoordinatesClicked())) {
			...
			String sCoordinates = sWest + "," + sSouth + " " + sEast + ","
					+ sNorth;

			filter.addClauses("coordinates", sCoordinates, "E",
					getRelation(query.getCoordinatesFilter()));
		}
	 ...
	 * 
	 * @return String Possible values: overlaps within contains
	 * @param relacion
	 */
