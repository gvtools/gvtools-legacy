...
import es.gva.cit.gazetteer.drivers.GazetteerCapabilitiesParser;
...
public class WfsgCapabilitiesParser extends GazetteerCapabilitiesParser {
	 ...
	 * 
	 * @see
	 * es.gva.cit.gazetteer.drivers.GazeteerCapabilitiesParser#parseCapabilities
	...
	protected void parseCapabilities(XMLNode node) {
		if ((node == null) || !(node.getName().equals("WFS_Capabilities"))) {
			capabilities.setServerMessage(Messages
					.getText("errorNotSupportedProtocol"));
	...
	private void parseProjection() {
		driver.setProjection(XMLTree.searchNodeValue(getRootNode(),
				"FeatureTypeList->FeatureType->SRS"));
	}
	...
	private FeatureType[] parseFeatures() {
		XMLNode[] nodes = XMLTree.searchMultipleNode(getRootNode(),
	...
	private FeatureType parseFeature(XMLNode featureNode) {
		...
		f.setCoordinates(new Coordinates(XMLTree.searchNodeAtribute(
				featureNode, "LatLongBoundingBox", "minx"), XMLTree
				.searchNodeAtribute(featureNode, "LatLongBoundingBox", "miny"),
				XMLTree.searchNodeAtribute(featureNode, "LatLongBoundingBox",
						"maxx"), XMLTree.searchNodeAtribute(featureNode,
