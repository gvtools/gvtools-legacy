 ...
 *   dac@iver.es
 */
package es.gva.cit.gazetteer.idec.parsers;
...
public class IdecFeatureParser {
	...
	public static Feature[] parse(XMLNode node) {
		...
		XMLNode[] nodeFeatures = XMLTree.searchMultipleNode(node,
				"SOAP-ENV:Body->ns1:getCoordenadesUTMResponse->return->item");
		Feature[] features = new Feature[nodeFeatures.length];

		// New Version
		if (features.length == 0) {
			nodeFeatures = XMLTree.searchMultipleNode(node,
					"soapenv:Body->multiRef");
		...
		for (int i = 0; i < nodeFeatures.length; i++) {
			...
			Point2D.Double point = new Point2D.Double(
					Double.parseDouble(XMLTree.searchNodeValue(nodeFeatures[i],
							"X")), Double.parseDouble(XMLTree.searchNodeValue(
							nodeFeatures[i], "Y")));
