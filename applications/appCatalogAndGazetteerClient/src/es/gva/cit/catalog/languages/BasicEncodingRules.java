...
/**
 * This class is used to create a Basic Encoding Rules (BER) query. See the
...
public class BasicEncodingRules extends AbstractGeneralLanguage {
	 ...
	 * 
	 * @param use
	 *            It is a number that represent an attribute
	 *            (4=Title,62=abstract,...)
	 * @param structure
	 *            It defines the attribute type (1=Phrase,2=wors,...)
	 * @param relation
	 *            Relation between the attribute and the query
	 *            (1=LessThan,3=equal,...)
	 * @param line
	 *            String with the user introduced value
	 * @param concordancia
	 *            Relationship between different words of the same field (more
	 *            than one words) E,A o Y --> Exact, All, anY
	 * @param operator
	 *            Relationship between fields (title, abstract) 'and' or 'or'
	 */
	...
	/**
	 * It realize the same function than the "addClauses(String use, String
	 * structure String relation,String line, String concordancia)" function,
	 * but the words to find are in a vector.
	 * 
	 * 
	 * @param use
	 * @param structure
	...
	protected void addCurrentClauseQuery(String operator) {
		if (currentClause != null) {
			if (currentQuery == null) {
				...
				currentQuery = "@" + operator + " " + currentQuery + " "
						+ currentClause;
			}
		}
	}
