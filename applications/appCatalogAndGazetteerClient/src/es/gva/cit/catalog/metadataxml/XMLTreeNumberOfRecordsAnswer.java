...
public class XMLTreeNumberOfRecordsAnswer {
	 ...
	 * @param lastRecord
	 */
	public static XMLNode getNode(int numberOfRecords, int firstRecord,
			int lastRecord) {
		String message = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>"
				+ "<Elements> " + "<NumberOfRecords>"
				+ String.valueOf(numberOfRecords) + "</NumberOfRecords>"
				+ "<FirstRecord>" + String.valueOf(firstRecord)
				+ "</FirstRecord>" + "<LastRecord>"
				+ String.valueOf(lastRecord) + "</LastRecord>" + "</Elements>";
	...
	public static int getNumberOfRecords(XMLNode node) {
		return Integer.parseInt(XMLTree
				.searchNodeValue(node, "NumberOfRecords"));
	}
