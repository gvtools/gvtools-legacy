 ...
 *   dac@iver.es
 */
package es.gva.cit.catalog.z3950.filters;

import org.gvsig.i18n.Messages;

import es.gva.cit.catalog.filters.AbstractFilter;
import es.gva.cit.catalog.languages.BasicEncodingRules;
...
public class Z3950Filter extends AbstractFilter {
	 ...
	 * 
	 * @see
	 * es.gva.cit.catalog.filters.IFilter#getQuery(es.gva.cit.catalog.querys
	 * .CatalogQuery)
	 */
	public String getQuery(es.gva.cit.catalog.querys.CatalogQuery query) {
		...
		if (query.isMinimized()) {
			if (query.getAbstract() != null) {
				filter.addClauses("62", "6", "3", query.getAbstract(), "E",
						"or");
			}
		} else {
			if (query.getAbstract() != null) {
			...
			if (query.getThemeKey() != null) {
				filter.addClauses("2002", "6", "3", query.getThemeKey(), "Y",
						"and");
			}
			// Source Scale Denominator (srcscale, 1024), Numeric String (109)
			if (query.getScale() != null) {
				if (query.getMinScale() != null) {
					filter.addClauses("1024", "109", "4", query.getMinScale(),
							"E", "and");
				}
				if (query.getMaxScale() != null) {
					filter.addClauses("1024", "109", "2", query.getMaxScale(),
							"E", "and");
				}
			}

			if (query.getProvider() != null) {
				filter.addClauses("1005", "6", "3", query.getProvider(), "E",
						"and");
			}
			// Beginning Date (begdate, 1012), Date(210)
			if (query.getDateFrom() != null) {
				filter.addClauses("1012", "210", "18", query.getDateFrom(),
						"E", "and");
			}
			// Ending Date (date, 1012 ), Date(210)
			if (query.getDateTo() != null) {
				filter.addClauses("1012", "210", "14", query.getDateTo(), "E",
						"and");
			}
		}
	 ...
	 * 
	 * @param relation
	 *            String in the combo. Possible values: encloses overlaps
	 *            fullyOutsideOf equals
	 * 
	 * @return String Possible values: 9 -> encloses 7 -> overlaps 10 ->
	 *         fullyOutsideOf 3 -> equals
	 * @param translator
	 * @param relacion
	 */
