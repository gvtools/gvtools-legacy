 ...
 */
public abstract class CSWCatalogServiceDriver extends
	...
	public DiscoveryServiceCapabilities getCapabilities(URI uri)
		...
		nodes = new HTTPGetProtocol().doQuery(url,
				CSWAbstractMessages.getHTTPGETCapabilities(), 0);
	 ...
	 * 
	 * @see
	 * es.gva.cit.catalog.drivers.ICatalogServiceDriver#getRecords(java.net.URI,
	 * es.gva.cit.catalog.querys.CatalogQuery, int)
	 */
	public GetRecordsReply getRecords(URI uri, CatalogQuery query,
		...
		Collection nodes = new java.util.ArrayList();
		try {
			System.out.println(getMessages(uri).getHTTPPostGetRecordsMessage(
					capabilities, getQuery(), getServerData(), firstRecord));
			nodes = new HTTPPostProtocol().doQuery(
					url,
					getMessages(uri).getHTTPPostGetRecordsMessage(capabilities,
							getQuery(), getServerData(), firstRecord),
					firstRecord);
	...
	/**
	 * This function returns the number of records that have been retrieved. It
	 * Reads a Node value.
	 * 
	 * 
	...
	protected int getNumberOfRecords(XMLNode node) {
		int numberOfRecords = getNumberOfRecords(node, "csw:SearchResults",
				"numberOfRecordsMatched");
		if (numberOfRecords == -1)
			numberOfRecords = getNumberOfRecords(node, "SearchResults",
					"numberOfRecordsMatched");
	...
	/**
	 * @return the class to generate the messages. It depends of the server
	 *         version
	 * @throws NotSupportedVersionException
	 */
	private CSWAbstractMessages getMessages(URI uri)
			throws NotSupportedVersionException {
		if (servers.containsKey(uri)) {
			return (CSWAbstractMessages) servers.get(uri);
		}
		CSWAbstractMessages messages = CSWMessagesFactory.getMessages(
				capabilities.getVersion(), (CSWAbstractProfile) getProfile());
