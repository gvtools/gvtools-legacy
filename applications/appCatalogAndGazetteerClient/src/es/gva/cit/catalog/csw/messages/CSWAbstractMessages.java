...
public abstract class CSWAbstractMessages {
	...
	/**
	 * The get capabilities operation started with lower case. There are some
	 * servers that has this bug
	 * 
	 * @return
	 */
	public static NameValuePair[] getHTTPGETCapabilitiesLower() {
		NameValuePair nvp1 = new NameValuePair("request",
				CSWConstants.OPERATION_GETCAPABILITIESToLOWER);
		NameValuePair nvp2 = new NameValuePair("service",
				ServerData.SERVER_SUBTYPE_CATALOG_CSW);
		NameValuePair nvp3 = new NameValuePair("acceptFormats", "text/xml");
		return new NameValuePair[] { nvp1, nvp2, nvp3 };
	...
	public static NameValuePair[] getHTTPGETDescribeRecord(String version) {
		NameValuePair nvp1 = new NameValuePair("request",
				CSWConstants.OPERATION_DESCRIBERECORD);
		NameValuePair nvp2 = new NameValuePair("service",
				ServerData.SERVER_SUBTYPE_CATALOG_CSW);
	 ...
	 * @return
	 */
	public String getHTTPPostGetRecordsMessage(CSWCapabilities capabilities,
			CatalogQuery query, ServerData serverData, int firstRecord) {
	...
	protected String createGetRecordsHeader(int firstRecord) {
		...
		buffer.append(" " + CSWConstants.CSW_SERVICE + "=\""
				+ ServerData.SERVER_SUBTYPE_CATALOG_CSW + "\" ");
		buffer.append(CSWConstants.CSW_VERSION + "=\""
				+ capabilities.getVersion() + "\" ");
		buffer.append(CatalogConstants.XML_NS + "=\""
				+ CSWConstants.CSW_NAMESPACE_URI + "\" ");
		buffer.append(CatalogConstants.XML_NS + ":"
				+ CSWConstants.CSW_NAMESPACE + "=\""
				+ CSWConstants.CSW_NAMESPACE_URI + "\" ");
		buffer.append(CatalogConstants.XML_NS + ":"
				+ CSWConstants.OGC_NAMESPACE + "=\""
				+ CSWConstants.OGC_NAMESPACE_URI + "\" ");
		buffer.append(CatalogConstants.XML_NS + ":"
				+ CSWConstants.GML_NAMESPACE + "=\""
		...
		if (capabilities.getOutputFormat() != null) {
			buffer.append(CSWConstants.CSW_OUTPUTFORMAT + "=\""
					+ capabilities.getOutputFormat() + "\" ");
		}
		if (getOutputSchema(capabilities.getOutputSchema()) != null) {
			buffer.append(CSWConstants.CSW_OUTPUTSCHEMA + "=\""
					+ getOutputSchema(capabilities.getOutputSchema()) + "\" ");
		} else {
		...
		buffer.append("</" + CSWConstants.CSW_NAMESPACE + ":"
				+ CSWConstants.OPERATION_GETRECORDS + ">");
		return buffer.toString();
	}

	/**
	 * Returns a common reslut type if the getCapabilities doesn't have one.
	 * Sometimes it works.
	 * 
	 * 
	 * @return Just one String
	 * @param resultType
	 *            The array of result types parsed.
	 */
	 ...
	 * 
	 * @param OutputFormat
	 *            The array of outputFormats parsed.
	 * @return Just one String
	 * @param outputSchemas
	 */
	...
	/**
	 * @return the Query xml sub tree of the GetRecords operation
	 */
	...
	/**
	 * @return the Constarint xml sub tree of the GetRecords operation
	 */
	protected String createGetRecordsConstraint() {
		StringBuffer buffer = new StringBuffer();
		buffer.append("<" + CSWConstants.CSW_NAMESPACE + ":"
				+ CSWConstants.CONSTRAINT);
	...
	/**
	 * Creates the filter encoding. It can be overridden by the children
	 * classes.
	 * 
	 * @return The filter encoding to use
	 */
	...
	protected String createFilterEncoding() {
		...
		if (query.getTitle() != null) {
			filter.addClauses(profile.getTitle(), query.getTitle(),
					query.getTitleFilter(), FilterEncoding.PROPERTY_IS_LIKE,
					FilterEncoding.TYPE_LITERAL, FilterEncoding.AND);
		}
		if (query.isMinimized()) {
			if (query.getAbstract() != null) {
				filter.addClauses(profile.getAbstract(), query.getAbstract(),
			...
			if ((query.getTopic() != null)
					&& (!query.getTopic().equals(
							Messages.getText("cathegoryAny")))) {
				filter.addClauses(profile.getTopic(),
						profile.getTopicValue(query.getTopic()),
			...
			if (query.getProvider() != null) {
				filter.addClauses(profile.getProvider(), filter.getWildCard()
						+ query.getProvider() + filter.getWildCard(),
						FilterEncoding.EXACT_WORDS,
		...
		if ((query.getCoordenates() != null) && (query.isCoordinatesClicked())) {
			filter.addBoundingBox(query.getCoordenates(),
					profile.getCoordinates(),
