 ...
 */
	...
	public NameValuePair[] getHTTPGETRecords(CatalogQuery query, int firstRecord) {
		...
		NameValuePair nvp2 = new NameValuePair("version", driver.getVersion());
		NameValuePair nvp3 = new NameValuePair("query",
				new SRWFilter().getQuery(query));
		...
		NameValuePair nvp6 = new NameValuePair("startRecord", new String(
				new Integer(firstRecord).toString()));
	...
	public String getSOAPCapabilities() {
		String soapMessage = "<SRW:explainRequest xmlns:SRW=\"http://www.loc.gov/zing/srw/\">"
				+ "<SRW:version>"
				+ driver.getVersion()
				+ "</SRW:version>"
				+ "</SRW:explainRequest>";
	...
	public String getSOAPRecords(CatalogQuery query, int firstRecord) {
		...
		String soapMessage = "<srw:searchRetrieveRequest xmlns:srw=\"http://www.loc.gov/zing/srw/\">"
				+ "<srw:query><![CDATA["
				+ new IDEESRWFilter().getQuery(query)
				+ "]]></srw:query>"
				...
				+ "</srw:recordSchema>"
				+ "<srw:resultSetTTL>"
				+ driver.getResultSetTTL()
				+ "</srw:resultSetTTL>" + "</srw:searchRetrieveRequest>";
		return SOAPProtocol.setSOAPMessage(soapMessage, null);
	}
}
