 ...
 *   dac@iver.es
 */
package es.gva.cit.catalog.srw.parsers;
 ...
 */
	...
	public boolean parse(XMLNode node) {
		...
		prefix = prefix + ":";
		driver.setOutputSchema(XMLTree.searchNodeValue(node, prefix
				+ "record->" + prefix + "recordSchema"));
		driver.setOutputFormat(XMLTree.searchNodeValue(node, prefix
				+ "record->" + prefix + "recordPacking"));
		driver.setStartPosition("1");
		driver.setMaxRecords("10");
		driver.setRecordXPath("");
		driver.setResultSetTTL("0");
		String title = XMLTree.searchNodeValue(node, prefix + "record->"
				+ prefix + "recordData->explain->databaseInfo->title");

		String description = XMLTree.searchNodeValue(node, prefix + "record->"
				+ prefix + "recordData->explain->databaseInfo->description");
