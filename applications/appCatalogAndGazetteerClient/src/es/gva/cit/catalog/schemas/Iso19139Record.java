...
public class Iso19139Record extends Record {
	...
	public Iso19139Record(URI uri, XMLNode node) {
		super(uri, node);
		setTitle(node
				.searchNodeValue("identificationInfo->MD_DataIdentification->citation->CI_Citation->title->CharacterString"));
		setAbstract_(node
				.searchNodeValue("identificationInfo->MD_DataIdentification->abstract->gco:CharacterString"));
		setPurpose(node.searchNodeValue("dataIdInfo->idPurp"));
		setKeyWords(node
				.searchMultipleNodeValue("identificationInfo->MD_DataIdentification->descriptiveKeywords->MD_Keywords->keyword->gco:CharacterString"));
	...
	private Resource[] getResources(String label) {
		...
		for (int i = 0; i < resources.length; i++) {
			resources[i] = new Resource(XMLTree.searchNodeValue(nodes[i],
					"linkage->URL"), XMLTree.searchNodeValue(nodes[i],
					"protocol->gco:CharacterString"), XMLTree.searchNodeValue(
					nodes[i], "name->gco:CharacterString"),
