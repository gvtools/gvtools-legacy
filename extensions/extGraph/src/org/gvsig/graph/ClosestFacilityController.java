...
public class ClosestFacilityController implements IFlagListener {
	...
	private static final Logger logger = Logger
			.getLogger(ClosestFacilityController.class.getName());
	 ...
	 * 
	 * @param listener
	 *            the ClosestFacilityListener to be added
	 */
	 ...
	 * 
	 * @param listener
	 *            the ClosestFacilityListener to be removed
	 */
	...
	private void informListeners(int reason, GvFlag flag1, GvFlag flag2) {
		...
		case EVENTS_REMOVED:
			...
			break;
		case EVENT_MODIFIED:
			logger.info("Modificación del evento " + flag1 + " pasa a ser: "
					+ flag2);
			while (it.hasNext()) {
				((IClosestFacilityListener) it.next()).eventModified(flag1,
						flag2);
			}
			break;
		case FACILITY_ADDED:
		...
		case FACILITY_MODIFIED:
			logger.info("Modificación del proveedor " + flag1 + " pasa a ser: "
					+ flag2);
			while (it.hasNext()) {
				((IClosestFacilityListener) it.next()).facilityModified(flag1,
						flag2);
			}
			break;
		case SOLVEDFACILITY_ADDED:
			...
			while (it.hasNext()) {
				((IClosestFacilityListener) it.next())
						.addedSolvedFacility(flag1);
			}
			break;
	 ...
	 * 
	 * @param network
	 *            The networ wich will be used in the ClosestFacilityController
	 */
	 ...
	 * 
	 * @param maxFacilitiesNumber
	 *            sets the maximum number of facilities that will be considered
	 *            in the solution
	 */
	public void setMaxFacilitiesNumber(int maxFacilitiesNumber) {
		this.maxFacilitiesNumber = (maxFacilitiesNumber <= this.toolMaxFacilitiesNumber) ? maxFacilitiesNumber
				: this.toolMaxFacilitiesNumber;
	...
	/**
	 * Set the default maximum number of facilities to be considered in the
	 * solution
	 * 
	 * @param toolMaxFacilitiesNumber
	 *            the default maximum number of facilities to be considered in
	 *            the solution
	 */
	...
	/**
	 * Gets the default maximum number of facilities to be considered in the
	 * solution
	 * 
	 * @return the default maximum number of facilities to be considered in the
	 *         solution
	 */
	...
	/**
	 *  It won't be considered in the solution facilities that have a much
	 * higher cost than this property
	 * 
	...
	/**
	 * It won't be considered in the solution facilities that have a much higher
	 * cost than this property
	 * 
	 * @param facilitiesMaxLimit
	 *            the maximum cost
	 */
	...
	/**
	 * Determines if the direction of the route is from the facility to the
	 * event
	 * 
	 * @return true if the direction of the route is from the facility to the
	...
	/**
	 * Determines if the direction of the route is from the event to the
	 * facility
	 * 
	 * @return true if the direction of the route is from the event to the
	 ...
	 * 
	 * @param flag
	 *            the graphics will be centered on this flag
	 */
	...
	/**
	 * The graphics used in the ClosestFacilityController will be centered on
	 * the minimum rectangle that contains all the flags
	 * 
	 * @param flags
	 *            the array of flags
	 */
	 ...
	 * 
	 * @param event
	 *            the event used to find the closest facility
	 */
	...
	private GvFlag createFlagFromGeometry(Geometry geom, double tol)
		...
		if (geom instanceof Point || geom instanceof MultiPoint) {
			...
			for (int j = 0; j < coords.length; j++) {
				...
				if (flag == null) {
					// segundo intento:
					flag = this.network.createFlag(coords[j].x, coords[j].y,
							4 * tol);
				}
	...
	public int loadFacilitiesFromLayer(FLyrVect layerFacilities,
			boolean onlySelectedFacilities) throws ReadDriverException,
			GraphException {
		...
		double realTol = this.mpCtrl.getViewPort().toMapDistance(
		...
		if (fieldDesc != null) {
			for (int i = 0; i < fieldDesc.length && fieldDescriptionIndex == -1; i++) {
				if (fieldDesc[i].getFieldName().equalsIgnoreCase("descript")
						&& fieldDesc[i].getFieldType() == Types.VARCHAR) {
		...
		if (onlySelectedFacilities) {
			...
			for (int i = selection.nextSetBit(0), j = 0; i >= 0; i = selection
					.nextSetBit(i + 1), j++) {
				flag = this.createFlagFromGeometry(layerFacilities.getSource()
						.getShape(i).toJTSGeometry(), realTol);
				if (flag == null)
					rejectedFacilities++;
				else {
					if (fieldDescriptionIndex >= 0
							&& fieldDescriptionIndex < fieldDesc.length) {
						Value[] values = ds.getRow(i);
						if (values[fieldDescriptionIndex] instanceof StringValue) {
							flag.setDescription(((StringValue) values[fieldDescriptionIndex])
									.toString());
						}
					} else {
		...
		logger.info(ps.getText("facilities_loaded_in") + " "
				+ (System.currentTimeMillis() - init) + " millis");
		return rejectedFacilities;
	}

	private SelectableDataSource getRecordset(IndexedShpDriver shpDriver)
		...
		if (shpDriver instanceof ExternalData) {
			...
			String name = LayerFactory.getDataSourceFactory()
					.addFileDataSource(driverName, dataFile.getAbsolutePath());
			// CHEMA: AUTOMATIC DATA SOURCE
			// ds = new
			// SelectableDataSource(LayerFactory.getDataSourceFactory().createRandomDataSource(name,
	...
	public void loadEventsFromFile(File file) throws FileNotFoundException,
			NullPointerException, GraphException, ReadDriverException,
			DriverLoadException, NoSuchTableException, Exception {
		...
		if (!file.exists()) {
			throw new FileNotFoundException(ps.getText("the_file") + " "
					+ file.getName() + " " + ps.getText("doesnt_exist"));
		}
		...
		if (shpDriver.getShapeType() == FShape.POINT) {
			...
			if (fieldDesc != null) {
				for (int i = 0; i < fieldDesc.length
						&& fieldDescriptionIndex == -1; i++) {
					if (fieldDesc[i].getFieldName()
							.equalsIgnoreCase("descript")
							&& fieldDesc[i].getFieldType() == Types.VARCHAR) {
						fieldDescriptionIndex = i;
					}
				}
			...
			for (int i = 0; i < shapeCount; i++) {
				/*
				 * FPoint2D
				 * point=(FPoint2D)shpDriver.getShape(i).getInternalShape();
				 * flag=this.network.createFlag(point.getX(), point.getY(), 10);
				 * this.network.addFlag(flag); //this.addEvent(flag);
				 */
				...
				for (int j = 0; j < coords.length; j++) {
					...
					if (flag == null) {
						// segundo intento:
						flag = this.network.createFlag(coords[j].x,
								coords[j].y, 4 * realTol);
						if (flag == null) {
							throw new NullPointerException(ps.getText("event")
									+ " " + i + " "
									+ ps.getText("out_of_the_network") + "."
									+ " " + ps.getText("tolerance") + "="
									+ realTol);
							// NotificationManager.addError("No se puedo situar el registro "
							// + i
							// +"Por favor, compruebe que está encima de la red o aumente la toleracina.",
							// e);
						}
					}
					if (flag != null) {
	...
	public void createSHPEvents(File file) throws InitializeWriterException,
			StartWriterVisitorException, StopWriterVisitorException,
			ProcessWriterVisitorException {
		...
		for (int i = 0; i < events.length; i++) {
			Value[] rowValues = new Value[] {
					ValueFactory.createValue(events[i].getIdFlag()),
					ValueFactory.createValue(events[i].getDescription()) };
			IFeature feat = new DefaultFeature(
					ShapeFactory.createPoint2D(new FPoint2D(events[i]
							.getOriginalPoint())), rowValues, "" + rowIndex);
			DefaultRowEdited row = new DefaultRowEdited(feat,
					IRowEdited.STATUS_ADDED, rowIndex);
	...
	/**
	 * @param idFacility
	 *            the id of the facility to be returned
	 ...
	 * 
	 * @param idFacility
	 *            the id of the facility to be removed
	 */
	...
	/*
	 * public GvFlag getSelectedEvent(){ return this.cfd.getSelectedEvent(); }
	 */
	...
	private void addSortedSolvedFacility(GvFlag flag) {
		...
		if (flagCost <= this.facilitiesMaxLimit && flagCost >= 0
				&& this.solvedFacilities.size() < this.maxFacilitiesNumber) {
			for (index = 0; !added && index < this.solvedFacilities.size(); index++) {
				if (flagCost <= ((GvFlag) this.solvedFacilities.get(index))
						.getCost()) {
	...
	private void addSortedSolvedRoute(GvFlag flag, Route route)
			throws IndexOutOfBoundsException {
		boolean added = false;
		double routeCost = route.getCost();
		int index = 0;
		if (routeCost <= this.facilitiesMaxLimit
				&& this.solvedFacilities.size() < this.maxFacilitiesNumber) {
			for (index = 0; !added && index < this.solvedFacilities.size(); index++) {
				if (routeCost <= ((GvFlag) this.solvedFacilities.get(index))
						.getCost()) {
					this.addSolvedFacility(index, flag);
	...
	/**
	 * Gets the facility stored in the specified index. Remember that the solved
	 * facilities are stored in sorted mode, from the closest to the farthest
	 * 
	 * @param index
	 *            the position where is the facility
	 * @return the facility
	 * @throws IndexOutOfBoundsException
	 *             when the index is less than 0 and bigger than the facilities
	 *             number
	 */
	...
	private Route calculateRoute(Network network, GvFlag source,
			GvFlag destination) throws GraphException {
		...
		String fieldStreetName = (String) network.getLayer().getProperty(
		...
		logger.info("Ruta desde " + source + " hasta " + destination
				+ " calculada en " + (System.currentTimeMillis() - iniTime)
				+ " millis");
		return route;
	}

	/**
	 * Gets the route stored in the specified index. If the route doesn't exist
	 * it will be created with the source event specified when the route was
	 * solved and the facility that correspond to the route. Then, when the
	 * route is created, is stored for future accesses.
	 * 
	 * @param index
	 *            the position where is the route
	 * @return the route
	 * @throws GraphException
	 * @throws IndexOutOfBoundsException
	 *             when the specified index is less than 0 and bigger than the
	 *             number of solved facilities
	 */
	public Route getSolvedRoute(int index) throws GraphException,
	...
	/**
	 * Gives all the facilities sorted by the cost field from closest to
	 * farthest
	 * 
	...
	/**
	 * Shows an animation wich points with a circle to the position where is the
	 * flag
	 * 
	 * @param flag
	 *            the pointed flag
	 * @param color
	 *            the color of the circle wich will point the flag
	 * @param maxCount
	 *            level of transparency of the circle
	 */
	public void flashFlag(GvFlag flag, Color color, int maxCount) {
		NetworkUtils.flashPoint(this.mpCtrl, color, maxCount, flag
				.getOriginalPoint().getX(), flag.getOriginalPoint().getY());
	}
	...
	/**
	 * Finds and stores in sorted mode the closest facilities. Cost field is
	 * used to sort the facilities from the closest to the farthest.
	 * 
	 * @throws GraphException
	...
	public void solve() throws GraphException, ReadDriverException {
		...
		if (this.facilities.size() > 0) {
			this.maxFacilitiesNumber = (this.maxFacilitiesNumber <= 0 || this.maxFacilitiesNumber > this.toolMaxFacilitiesNumber) ? this.toolMaxFacilitiesNumber
					: this.maxFacilitiesNumber;
			this.facilitiesMaxLimit = (this.facilitiesMaxLimit <= 0) ? Double.MAX_VALUE
					: this.facilitiesMaxLimit;
			if (this.getSourceEvent() != null) {
				if (this.toEvent) {
					ShortestPathSolverAStar solverToEvent = new ShortestPathSolverAStar();
					// this.network.setLayer(lyr);
					...
					while (it.hasNext()
							&& this.solvedFacilities.size() < this.maxFacilitiesNumber) {
						flag = (GvFlag) it.next();
						route = this.calculateRoute(this.network, flag,
								this.solutionEventSelected);
		...
		this.network.addFlagListener(this);
		logger.info("Solución obtenida en: "
				+ (System.currentTimeMillis() - initialTime) + " millis");
	}
}