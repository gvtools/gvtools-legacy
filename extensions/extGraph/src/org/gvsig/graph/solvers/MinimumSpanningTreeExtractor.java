 ...
 *         design networks (water networks, cable networks), because you can
 *         reach all node points with minimum length cost. (Offer service to the
 *         whole area, but expending less money on cable and holes).
 * 
 */
		...
		try {
			...
			if (nodeEnd.getBestCost() < Double.MAX_VALUE) {
				...
				if (nodeOrig.getBestCost() < Double.MAX_VALUE) {
					...
					// writePartialEdge(idEdge, geom, edge, nodeOrig, nodeEnd,
					// idFlag, edge.);
