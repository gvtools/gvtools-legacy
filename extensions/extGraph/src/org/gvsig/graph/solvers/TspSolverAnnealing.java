...
public class TspSolverAnnealing {
	...
	/*
	 * float calcula_dist_ordenacion(int v[], int bVolverOrigen) { float dist,
	 * distTot; int i;
	 * 
	 * distTot = odMatrix[origenTSP][v[0]]; // Origen al primer punto for (i =
	 * 0; i< numElemTSP-1;i++) { dist = odMatrix[v[i]][v[i+1]]; distTot =
	 * distTot + dist; }
	 * 
	 * // desde y hasta el almacen (distancia al primero y al último if
	 * (bVolverOrigen) { dist = odMatrix[v[numElemTSP-1]][origenTSP]; distTot =
	 * distTot + dist; } else { dist = odMatrix[v[numElemTSP-1]][destinoTSP];
	 * distTot = distTot + dist; } return distTot; }
	 */
	...
	/*
	 * Local Search Heuristics b-------a b a . . => .\ /. . d...e . . e...d . ./
	 * \. . . c f c-------f
	 */
	double getThreeWayCost(int[] p) {
		...
		// b va a ser el nuevo origen => sumamos su distancia a nuestro origen
		// fijo
		...
		// AÑADIR DIFERENCIA DE COSTE A LOS NODOS INMOVILES. (ORIGEN Y
		// ¿DESTINO?)
	}
	...
	double getReverseCost(int[] p) {
		...
		if (bReturnToOrigin) // Miramos la distancia al cero
		{
			...
			if (p[0] == fin || p[1] == fin) // también hay que mirar la
											// distancia al destino desde el
											// último punto
			{
				Dant = D(iorder[fin], origenTSP);
				if (p[0] == fin)
	...
	double annealing() {
		...
		/*
		 * Set up first eulerian path iorder to be improved by simulated
		 * annealing.
		 */
		/*
		 * bool conEulerian = true; if (conEulerian) findEulerianPath();
		 */
		...
		for (T = T_INIT; T > FINAL_T; T *= COOLING) /* annealing schedule */
		{
			...
			for (j = 0; j < TRIES_PER_T; j++) {
				...
				if (pathlen < bestlen) {
					pathlen = pathLength(); // Calculamos la distancia de
											// verdad, por si no interesa
											// hacer el cambio. pathlen es en
											// realidad una estimación
					if (pathlen < bestlen) {
						bestlen = pathlen;
						for (i = 0; i < n; i++)
							vTSP[i + 1] = iorder[i];
	...
	/**
	 * This function will use annealing only if numStops >= 6. If numStops < 6,
	 * it will use direct calculation, ensuring the optimum result.
	 * 
	 * @return
	 */
	...
	void permut(int v[], int m, boolean bVolverOrigen) {
		...
		if (m > 0)
			...
			// Calculamos su distancia y si es menor que la que tenemos
			// guardada, guardamos el nuevo vector.
