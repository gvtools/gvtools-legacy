...
public class ServiceAreaExtractor2 implements IDijkstraListener {
	...
	private void processEdgeForPolygon(GvEdge edge, GvNode nodeOrig,
		...
		if (nodeEnd.getBestCost() > nodeOrig.getBestCost()) {
			...
			if (indexInterval != -1) {
				...
				writeTotalEdge(edge.getIdArc(), geom, edge, nodeOrig, nodeEnd,
						idFlag);
			}
			double maxCost = costs[costs.length - 1];
			...
			if (indexInterval < costs.length - 1) {
				// Caso límite externo
				if ((costAux > maxCost) && (nodeOrig.getBestCost() < maxCost)) {
					...
					if (nodeOrig.getBestCost() == 0) {
						// Caso particular: empezamos en un tramo y terminamos
						// en ese mismo tramo
						if (edge.getDirec() == 0) // ponemos el pct como estaba.
													// En el nuevo
													// getPartialString ya lo
													// tiene en cuenta
							pct = 1 - pct;
						...
						partial = NetworkUtils.getPartialLineString(jtsGeom,
								pct1, pct2, edge.getDirec());
						writePartialEdge2(edge.getIdArc(), partial, edge,
								nodeOrig, nodeEnd, idFlag, maxCost);
					} else {
						partial = NetworkUtils.getPartialLineString(jtsGeom,
								pct, edge.getDirec());
						writePartialEdge(edge.getIdArc(), partial, edge,
								nodeOrig, nodeEnd, idFlag, maxCost);
					}
				...
				if ((nodeOrig.getBestCost() < maxCost) && (costAux > maxCost)) {
	...
	private void writePartialEdge(int i, LineString partial, GvEdge edge,
	...
	private void writePartialEdge2(int i, LineString jtsGeom, GvEdge edge,
			GvNode nodeOrig, GvNode nodeEnd, int idFlag, double maxCost)
			throws ProcessWriterVisitorException {
	...
	private void addUniqueNode(double x, double y, double z) {
		...
		if (!borderCoords.contains(c)) {
			...
			if (borderCoords.size() == 2) {
				...
				fullExtent.setFrameFromDiagonal(cAuxF1.x, cAuxF1.y, cAuxF2.x,
						cAuxF2.y);
	...
	/**
	 * FIXME: CHANGE THE NAME OF THIS METHOD. Now, it writes al nodes with cost
	 * < maxCost
	 * 
	 * @return
	...
	private void writeTotalEdge(int i, IGeometry geom, GvEdge edge,
		...
		values[2] = ValueFactory.createValue(nodeOrig.getBestCost());
		values[3] = ValueFactory.createValue(nodeOrig.getAccumulatedLength());
		values[4] = ValueFactory.createValue(nodeOrig.getBestCost()
				+ edge.getWeight());
		values[5] = ValueFactory.createValue(nodeOrig.getAccumulatedLength()
				+ edge.getDistance());
		values[6] = ValueFactory.createValue(idFlag);
		...
		if (bDoCompactArea) {
			// This code is necessary ONLY if you want to triangulate also with
			// interior points
			// (To draw a 3D view, for example.
			// TODO: Use borderPoints in triangulation instead of "nodes"
			// Origin
			addUniqueNode(nodeOrig.getX(), nodeOrig.getY(),
					nodeOrig.getBestCost());
	...
	private void insertVisitedEdge(GvEdge edge) {
		...
		if (!visitedEdges.containsKey(key)) {
			// En el constructor calculamos el porcentaje recorrido y lo
			// guardamos
			visitedEdges.put(key, new VisitedEdge(edge));
			// System.out.println("idEdge adjacent= " + edge.getIdEdge());
		} else {
			// Recuperamos el visiteEdge que habíamos metido y miramos sus
			// porcentajes
			// recorridos. Si entre los dos porcentajes NO suman más de uno,
			// quiere decir
			// que ahí se queda un tramo central al que no llegamos ni desde un
			// lado ni
			// desde el otro. Por eso guardamos cada trocito al que hemos
	...
	public void writeServiceArea() throws BaseException {
		...
		for (Map.Entry<String, VisitedEdge> entry : keySet) {
			...
			try {
				...
				// if (nodeOrig.getBestCost() < maxCost) { // FIXME: Creo que
				// este if no tiene sentido.
				// // A ese tramo hemos llegado parcialmente
				// // Recuperamos su distancia y etiquetamos.
				// writePartialEdge(edge.getIdArc(), geom, edge, nodeOrig,
		...
		for (int j = serviceAreaPolygons.size() - 1; j >= 0; j--) {
			...
			if (bDoCompactArea) {
				...
				while (it.hasNext()) {
					...
					// TODO: Optimizar este polígono. Quitar bucles y comprobar
					// smallSegments,
	...
	private void addExteriorPoints() {
		int numSteps = 30; // por ejemplo, 30 puntos por cada lado del
							// rectángulo
		...
		for (int i = 0; i < numSteps; i++) {
			...
			Coordinate cUp = new Coordinate(x, fullExtent.getMaxY() + 10.0,
					Double.MAX_VALUE);
			Coordinate cDown = new Coordinate(x, fullExtent.getMinY() - 10.0,
					Double.MAX_VALUE);
			Coordinate cLeft = new Coordinate(fullExtent.getMinX() - 10.0, y,
					Double.MAX_VALUE);
			Coordinate cRight = new Coordinate(fullExtent.getMaxX() + 10.0, y,
					Double.MAX_VALUE);
