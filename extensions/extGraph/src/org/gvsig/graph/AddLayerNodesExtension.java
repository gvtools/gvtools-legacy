 ...
 * @author fjp
 * 
 *         Extension to add nodes as a layer. It will be useful to have a good
 *         view of the network, and allows to edit and fix bugs.
 */
public class AddLayerNodesExtension extends Extension {
	 ...
	 */
	private void add_nodes_layer(MapControl mapCtrl, Network net)
			throws GraphException {
		NodesDriver driver = new NodesDriver(net);
		FLyrVect lyr = (FLyrVect) LayerFactory.createLayer("tmpNodes", driver,
				net.getLayer().getCrs());
		URL legendPath = this.getClass().getClassLoader()
				.getResource("images/nodos.gvl");
		...
		BeforeSavingListener savingListener = new MyBeforeSavingListener(lyr,
