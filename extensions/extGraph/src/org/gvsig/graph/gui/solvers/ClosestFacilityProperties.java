...
public class ClosestFacilityProperties extends JPanel implements IWindow,
		ActionListener {
	...
	private void initComponents() {
		...
		if (linesLayerName == null)
			linesLayerName = "<sin especificar>";
		...
		try {
			...
			if (fieldName != null)
				comboCostFieldValues.setSelectedItem(fieldName);
		} catch (NullPointerException except) {
		...
		try {
			int roundValue = ((Integer) this.properties.get("ROUND_VALUE"))
					.intValue();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.buttonAccept) {
			...
			try {
				costFieldName = (String) this.comboCostFieldValues
						.getSelectedItem();
			...
			try {
				this.properties.put("ROUND_VALUE", Integer
						.valueOf(this.comboRoundValues.getSelectedIndex()));
			} catch (NullPointerException except) {
