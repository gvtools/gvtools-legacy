...
/**
 * Component that shows in a combobox all the TOC's layers, and allows user to
 * select one of them.
 * 
...
public class LayerSelectionPanel extends JPanel implements IWindow {
	...
	/**
	 * Optinal filter to show in the combo box layers of a given geometry typ0e
	 * */
	ArrayList<Integer> geometryType = new ArrayList<Integer>();

	/**
	 * If it is not null, we'll filter layers to only show types of this class
	 * */
	Class layerType;
	private WindowInfo wi;

	public LayerSelectionPanel(FLayers tocLayers, String title,
	...
	public FLayer getLayer() {
		...
		if (selectedLayer != null) {
			if (!(selectedLayer.trim().equals(""))) {
				// TODO: Creo que sería conveniente revisar getLayer para que
				// tenga en cuenta los grupos de capas.
	...
	public void initialize() {
		...
		contentPanel.addComponent(PluginServices.getText(null, "layers") + ":",
				comboBox);
			...
			while (lyrIterator.hasNext()) {
				...
				if (layerType != null) {
					...
					if (layer instanceof FLyrVect) {
						...
						try {
							// Para que no salte el error por culpa del
							// AutomaticDataSource.
							...
							for (int j = 0; j < geometryType.size(); j++) {
								if (lyrVect.getShapeType() == geometryType
										.get(j))
									list.add(layer);
							}
