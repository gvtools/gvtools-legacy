...
public class ODMatrixTask extends AbstractMonitorableTask {
	...
	public ODMatrixTask(Network net, GvFlag[] originFlags,
		...
		setFinalStep(originFlags.length); // Importante: Fijarlo en el
											// constructor, para que esté
											// establecido antes del run() <br>
	...
	public void run() throws Exception {
		try {
			...
			for (int i = 0; i < originFlags.length; i++) {
				...
				System.out.println("Punto " + i + " de " + originFlags.length
						+ ". " + (t2 - t1) + " msecs.");

				for (int j = 0; j < destinationFlags.length; j++) {
					selectedWriter.saveDist(i, j,
							destinationFlags[j].getCost(),
