...
public class ServiceAreaPage1 extends JWizardPanel implements KeyListener,
	...
	private void initialize() {
		...
		this.labelTitle = new JLabel(
				"<html><b>Edición de costes de cada área</b></html>");
		this.labelAreaMainCost = new JLabel(
				"<html>Costes <b>primarios</b> de área:</html>");
		this.labelErrorMainCosts = new JLabel();
		this.labelErrorMainCosts.setOpaque(true);
		this.iconErrorCost = PluginServices.getIconTheme().get(
				"service_area_wrong_costs");
		this.scrollAreaMainCosts = new JScrollPane();
		this.scrollAreaMainCosts
		...
		this.labelAreaSecondaryCosts = new JLabel(
				"<html>Costes <b>secundarios</b> de área:</html>");
		...
		this.scrollAreaSecondaryCosts.getViewport().add(
				this.textAreaSecondaryCosts);
		this.checkSameCosts = new JCheckBox(
				"<html><p align='left'>Utilizar los costes del</p><p align='left'>punto seleccionado</p><p align='left'>para el resto</p><p align='left'>de puntos</p></html>");
		...
		this.tablePoints.enablePijamaEffect(Color.white, new Color(229, 251,
				252));
		...
		this.labelTitle.setBounds(BORDER_HGAP, BORDER_VGAP,
				this.labelTitle.getFontMetrics(this.labelTitle.getFont())
						.stringWidth(this.labelTitle.getText()),
				PREFERRED_HEIGHT);
		this.labelAreaMainCost.setBounds(
				BORDER_HGAP,
				this.labelTitle.getY() + this.labelTitle.getHeight()
						+ COMPONENT_VGAP,
				this.labelAreaMainCost.getFontMetrics(
						this.labelAreaMainCost.getFont()).stringWidth(
						this.labelAreaMainCost.getText()), 23);
		this.labelErrorMainCosts.setBounds(this.labelAreaMainCost.getWidth()
				+ this.labelAreaMainCost.getX() + COMPONENT_HGAP,
				this.labelAreaMainCost.getY(), 23, 23);
		this.scrollAreaMainCosts.setBounds(
				BORDER_HGAP,
				this.labelAreaMainCost.getY()
						+ this.labelAreaMainCost.getHeight() + COMPONENT_VGAP,
				200, 70);
		this.checkSameCosts.setBounds(this.scrollAreaMainCosts.getX()
				+ this.scrollAreaMainCosts.getWidth() + BORDER_HGAP,
				this.scrollAreaMainCosts.getY(), this.checkSameCosts
						.getFontMetrics(this.checkSameCosts.getFont())
						.stringWidth(this.checkSameCosts.getText()),
				this.scrollAreaMainCosts.getHeight());
		this.labelAreaSecondaryCosts
				.setBounds(
						BORDER_HGAP,
						this.scrollAreaMainCosts.getY()
								+ this.scrollAreaMainCosts.getHeight()
								+ COMPONENT_VGAP,
						this.labelAreaSecondaryCosts.getFontMetrics(
								this.labelAreaSecondaryCosts.getFont())
								.stringWidth(
										this.labelAreaSecondaryCosts.getText()),
						PREFERRED_HEIGHT);
		this.labelErrorSecondaryCosts.setBounds(
				this.labelAreaSecondaryCosts.getWidth()
						+ this.labelAreaSecondaryCosts.getX() + COMPONENT_HGAP,
				this.labelAreaSecondaryCosts.getY(), 23, 23);
		this.scrollAreaSecondaryCosts.setBounds(BORDER_HGAP,
				this.labelAreaSecondaryCosts.getY()
						+ this.labelAreaSecondaryCosts.getHeight()
						+ BORDER_VGAP, 200, 70);

		this.panelNorth.setPreferredSize(new Dimension(0, this.labelTitle
				.getHeight()
	...
	public void next() {
		...
		if (model.getEnabledRowsCount() > 0) {
			if (!model.isUniqueMainCostEnabled()
					|| !model.isUniqueSecondaryCostEnabled()) {
				boolean flag = true;
				...
				for (i = 0; i < rowCount && flag; i++) {
					if (model.isRowEnabled(i)) {
						if (String.valueOf(model.getValueAt(i, 3)).trim()
								.equals("")
				...
				if (flag) {
					...
					JOptionPane.showMessageDialog(
							this,
							"La fila con ID "
									+ Integer.valueOf(String.valueOf(model
											.getValueAt(i, 1)))
									+ " no tiene todos los costes asignados",
							"Costes no asignados", JOptionPane.WARNING_MESSAGE);
				}
			} else {
				super.next();
			}
			...
			JOptionPane.showMessageDialog(this,
					"No hay ninguna fila habilitada", "Advertencia",
	...
	public void keyTyped(KeyEvent e) {
		if (e.getSource() == this.textAreaMainCosts) {
			try {
				AbstractPointsModel model = this.owner.getController()
						.getModel();
				double[] costs = NetworkUtils.string2doubleArray(
						this.textAreaMainCosts.getText() + e.getKeyChar(),
						model.COSTS_SEPARATOR);
				...
				if (this.selectedRowIndex >= 0
						&& this.selectedRowIndex < this.owner.getController()
					...
					s = s.trim();
					if (s.endsWith(model.COSTS_SEPARATOR))
						s = s.substring(0, s.length() - 1);
					this.owner.getController().getModel()
							.setValueAt(s, this.selectedRowIndex, 3);
					this.tablePoints.updateUI();
				}
			} catch (NumberFormatException except) {
				this.labelErrorMainCosts.setIcon(this.iconErrorCost);
				this.labelErrorMainCosts
						.setToolTipText("Hay un error en los costes primarios");
			...
			try {
				AbstractPointsModel model = this.owner.getController()
						.getModel();
				double[] costs = NetworkUtils.string2doubleArray(
						this.textAreaSecondaryCosts.getText() + e.getKeyChar(),
						model.COSTS_SEPARATOR);
				...
				if (this.selectedRowIndex >= 0
					...
					if (s.endsWith(model.COSTS_SEPARATOR))
						s = s.substring(0, s.length() - 1);
					this.owner.getController().getModel()
							.setValueAt(s, this.selectedRowIndex, 4);
					this.tablePoints.updateUI();
				}
			} catch (NumberFormatException except) {
				this.labelErrorSecondaryCosts.setIcon(this.iconErrorCost);
				this.labelErrorSecondaryCosts
						.setToolTipText("Hay un error en los costes secundarios");
				this.textAreaSecondaryCosts.setBackground(new Color(255, 210,
						210));
			}
		}
	}
	...
	public void mouseClicked(MouseEvent e) {
		if (e.getSource() == this.tablePoints) {
			...
			if (this.selectedRowIndex >= 0
					&& this.selectedRowIndex < this.owner.getController()
							.getModel().getRowCount()) {
				String s = String.valueOf(this.owner.getController().getModel()
						.getValueAt(this.selectedRowIndex, 3));
				this.textAreaMainCosts.setText(s.trim());
				s = String.valueOf(this.owner.getController().getModel()
						.getValueAt(this.selectedRowIndex, 4));
		...
		if (e.getSource() == this.checkSameCosts) {
			...
			if (this.checkSameCosts.isSelected()) {
				if (this.tablePoints.getSelectedRowCount() > 0) {
					String mainCosts = String.valueOf(model.getValueAt(
							this.tablePoints.getSelectedRow(), 3)), secondaryCosts = String
							.valueOf(model.getValueAt(
									this.tablePoints.getSelectedRow(), 4));
					if (!mainCosts.trim().equals("")
							&& !secondaryCosts.trim().equals("")) {
						try {
							model.enableUniqueMainCost(String.valueOf(model
									.getValueAt(
											this.tablePoints.getSelectedRow(),
											3)));
							model.enableUniqueSecondaryCost(String
									.valueOf(model.getValueAt(
											this.tablePoints.getSelectedRow(),
											4)));
							this.tablePoints.setRowBackground(
									this.tablePoints.getSelectedRow(),
									new Color(237, 240, 184));
							this.tablePoints.updateUI();
						...
						JOptionPane
								.showMessageDialog(
										this,
										"El coste primario de la fila seleccionada no es correcto",
										"Coste incorrecto",
										JOptionPane.ERROR_MESSAGE);
						...
						JOptionPane
								.showMessageDialog(
										this,
										"El coste secundario de la fila seleccionada no es correcto",
										"Coste incorrecto",
										JOptionPane.ERROR_MESSAGE);
						this.checkSameCosts.setSelected(false);
					}
					...
					JOptionPane
							.showMessageDialog(this,
									"No se puede asginar un coste único porque no hay ninguna fila seleccionada");
					this.checkSameCosts.setSelected(false);
				}
			} else {
	...
	class ColoredTable extends JTable {
		...
		public void setRowBackground(int rowIndex, Color color)
			...
			if (this.getModel().getRowCount() > rowIndex) {
				...
				throw new IndexOutOfBoundsException("La fila " + rowIndex
						+ " no existe");
			}
		}
		...
		public Component prepareRenderer(TableCellRenderer renderer, int row,
			...
			if (!returnComp.getBackground().equals(getSelectionBackground())
					&& this.coloredRowIndex == row) {
				returnComp.setBackground(rowColor);
			}

			if (this.pijamaEffectEnabled
					&& !returnComp.getBackground().equals(
							getSelectionBackground()) && row != coloredRowIndex) {
				if (row % 2 == 0) {
