...
public abstract class AbstractPointsModel extends AbstractTableModel {
	...
	protected String getTotalCosts(String costs) throws NumberFormatException{
		if(!costs.trim().equals("")){
			double[] costsArray=NetworkUtils.string2doubleArray(costs, COSTS_SEPARATOR);
			String s="";
	...
	public void enableUniqueMainCost(String cost) throws InvalidCostException{
		try{
			...
			throw new InvalidCostException("El coste principal único es incorrecto"); 
		}
	}
	...
	public void enableUniqueSecondaryCost(String cost) throws InvalidCostException{
		try{
			this.uniqueSecondaryCosts=this.getTotalCosts(cost);
		}
		catch(NumberFormatException except){
			throw new InvalidCostException("El coste secundario único es incorrecto"); 
		}
	}
