 ...
 * Based in LineSequence from Java Topology Suite.
 * The idea is to avoid loops. Later, we will check this loops to see if the
 * CompactArea contains all good edges and disjoint bad edges.
 * First, we need a well oriented graph. Then, we iterate for nodes, and check if we
 * arrive to any already visited. If yes, then we will set the end point of this edge
 * to the next non visited node.
 */
	    ...
	    for (Iterator i1 = sequences.iterator(); i1.hasNext(); ) {
	      ...
	      for (Iterator i2 = seq.iterator(); i2.hasNext(); ) {
	        ...
	        if (!isNodeVisited(de.getToNode(), numSeq)) {
	        	...
	        	else // Hemos saltado uno o varios nodos, así que no hay lineString debajo
	        	{
