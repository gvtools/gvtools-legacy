...
//    protected PersonalLogger logger = new PersonalLogger(DebugUserIds.OLE);
    ...
    public DelaunayCalculator(List<DelaunayPunkt> punktefeld, ErrorHandler errHandler) {
        super();
        this.pointsArray = (DelaunayPunkt[])punktefeld.toArray(new DelaunayPunkt[0]);
...
//        this.monitor = monitor;
    ...
    public DelaunayCalculator(DelaunayPunkt[] punktefeld, ErrorHandler errHandler) {
        super();
...
//        this.monitor = monitor;
    ...
    protected void findNextNeighborsRecursive( int down, int up ) throws Exception {
        if (down < up){
            ...
            double dividingXValue  = ((PirolPoint)this.pointsArray[dividingIndex]).getX();
            ...
            while ( !found && lowerLimitIndex > down ){
                if ( Math.abs(dividingXValue - ((PirolPoint)this.pointsArray[lowerLimitIndex]).getX()) > this.minimalDistance) {
                    found = true;
                    break;
                }
                lowerLimitIndex--;
            ...
            while (!found && upperLimitIndex < up ){
                if ( Math.abs(dividingXValue - ((PirolPoint)this.pointsArray[upperLimitIndex]).getX()) > this.minimalDistance) {
                    found = true;
                    break;
                }
                upperLimitIndex++;
...
//                    this.logger.printError(e.getMessage() + ", " + borderLinePoints.get(i).getClass().getName() + " vs. DelaunayPunkt");
                    e.printStackTrace();
                }
                for (int j=0; j<borderLinePoints.size(); j++){
                    if ( i==j ) continue;
                    jPoint = ((DelaunayPunkt)borderLinePoints.get(j));
                    if (iPoint.isNeighborOf(jPoint)) continue;
                    ...
                    if ( Math.abs(iPoint.getY()-jPoint.getY()) < this.minimalDistance ){
                        ...
                        if ( tmpDistance < this.minimalDistance ){
                            if (tmpDistance==0){
                                ...
                                this.point1 = null;
                                this.point2 = null;
                                throw new IllegalArgumentException("two-points-with-identical-coordinates");
                            }
    ...
    protected void calculateDiagramIterative( DelaunayPunkt p1, DelaunayPunkt p2, DelaunayPunkt alterPunkt, int triangleCounter ) throws Exception{
        ...
        while ( !currentList.isEmpty() ){
            ...
            while (continueQuery){
                ...
                DelaunayLoopItem[] currentItems = currentList.toArray(new DelaunayLoopItem[0]);
                
                for (int itemIndex=0; itemIndex<currentItems.length; itemIndex++){
                    number ++;
                    item = currentItems[itemIndex];
                    newResults = this.buildTriangles( item.getPunkt1(), item.getPunkt2(), item.getAlterPunkt(), number );
                    newList.addAll(newResults);
                }
                
                if ( newList.isEmpty() && this.kantenCreated==1 && this.limitingAngle > Double.MIN_VALUE ){
...
//            this.monitor.report(this.kantenCreated, this.estimatedLineNum, PirolPlugInMessages.getString("done"));
    ...
    protected List<DelaunayLoopItem> buildTriangles(DelaunayPunkt p1, DelaunayPunkt p2, DelaunayPunkt alterPunkt, int nummerDreieck) throws Exception{
...
*/
            ...
            if (cosGamma >= -1.0 && cosGamma <= 1.0){
                ...
                System.out.println("schrott winkel: " + cosGamma + ", " + a + ", " + b + ", " + c);
                continue;
            }
    ...
    public boolean createDelaunayNet() throws Exception{
        ...
        while (!neighborsFound){
            try {
                ...
                System.out.println( (numPts-this.numPoints) + " point(s) was/were kicked out");
            }
            numPts = this.numPoints;
        }
