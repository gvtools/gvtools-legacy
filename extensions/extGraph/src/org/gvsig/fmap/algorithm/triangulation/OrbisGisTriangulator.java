 ...
 * 
 *         This algorithm is suitable to use breakLines (very useful for
 *         professional use). The algorithm removes duplicated points and may
 *         insert others to deal with breakLines. This is the reason to clean
 *         initial points and create a new TIN from the result.
 * 
 */
public class OrbisGisTriangulator extends AbstractTriangulator {
	...
	public TIN calculateTriangulation() {
		...
		try {
			final Triangulation t = new Triangulation(
					tin.vertices.toArray(new Vertex[0]), breakLineList);
			t.triangulate();
