...
import org.gvsig.fmap.algorithm.triangulation.pirol.comparisonAndSorting.Sortable;

import com.vividsolutions.jts.geom.Coordinate;

/**
 * Base class for objects that describe an n-dimensional point. There are
 * derived classes e.g. to suit the requirements of a triangulation or an
 * interpolation. Offers methods e.g. to determine distances between different
 * punkt objects or to mark and unmark punkt objects.
 * 
 * punkt objects implement Sortable and have a choseable natural order
 * (implement Comparable), so they can be sorted with the tools from the Java
...
public class PirolPoint extends Sortable {
	...
	public static final PirolPoint NULLPUNKT = new PirolPoint(new double[] {
			0.0, 0.0, 0.0 });
	...
	/**
	 * creates a punkt object that represents a vector (x,y(,z)) that shows the
	 * direction that leads from point "from" to point "to".
	 * 
	 * @param from
	 *            source points of the vector
	 * @param to
	...
	public static PirolPoint createVector(PirolPoint from, PirolPoint to) {
		...
		for (int i = 0; i < dim; i++) {
			try {
				vectorComponents[i] = to.getCoordinate(i)
						- from.getCoordinate(i);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 * share any references with the original
	 * 
	 * @param pkt
	 *            the punkt object to clone
	...
	/**
	 * Method to create a deep copy of the calling punkt object, that doesn't
	 * share any references with the original
	 * 
	...
	/**
	 * set boolean flag of the object, to e.g. mark if this point was already
	 * passed or whatever.
	 * 
	 * @param marked
	...
	public boolean isOn(PirolPoint p) {
		try {
			if (this == p) {
				return true;
		...
		try {
			if (this == p) {
				return true;
					...
					&& !(this.getIndex() < 0)) {
				return true;
			} else if (Math.min(p.getDimension(), this.getDimension()) >= 3
	...
	/**
	 * get the <code>nr</code>-th coordinate. This method is used within getX(),
	 * getY(), getZ().
	 * 
	 * @return the <code>nr</code>-th coordinate
	 * @throws RuntimeException
	 *             if the point has less coordniates than <code>nr</code> or if
	 *             <code>nr</code> is less than zero
	 */
	public double getCoordinate(int nr) throws RuntimeException {
		if (dimension > nr) {
			return this.coordinates[nr];
		}
		throw new RuntimeException("invalid dimension: " + nr + " (vs. "
				+ this.dimension + ")");
	}

	/**
	 * set the <code>nr</code>-th coordinate. This method is used within setX(),
	 * setY(), setZ().
	 * 
	 * @param newCoord
	 *            new Coordinate
	 ...
	 * The distance will be calculated in as many dimensions as both points
	 * have. If the two points have a different amount of coordinates, this will
	 * happen:
	 * <code>int checkDim = Math.min(this.dimension, p.getDimension());</code>
	 * 
	 * @param p
	 *            the other point
	...
	public int compareTo(Object arg0) {
		if (this.getSortFor() != ((PirolPoint) arg0).getSortFor()) {
			System.err.println("Can't compare for coordinates " + this.sortFor
					+ " and " + ((PirolPoint) arg0).getSortFor());
			return 0;
		}

		double local, remote;
		/*
		 * changed by oster if getSortFor is SORTFOR_XY then sum x and y
		 * coordinate!
		 */
		try {
			if (this.getSortFor() != CoordinateComparator.SORTFOR_XY) {
				local = this.getCoordinate(this.sortFor);
				remote = ((PirolPoint) arg0).getCoordinate(((PirolPoint) arg0)
						.getSortFor());
			} else {
				local = this.getCoordinate(CoordinateComparator.SORTFOR_X)
						+ this.getCoordinate(CoordinateComparator.SORTFOR_Y);
				remote = ((PirolPoint) arg0)
						.getCoordinate(CoordinateComparator.SORTFOR_X)
						+ ((PirolPoint) arg0)
								.getCoordinate(CoordinateComparator.SORTFOR_Y);
			}
		} catch (Exception e) {
