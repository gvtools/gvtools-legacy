...
public class ValueRange {
	...
	public ValueRange(double minValue, boolean includeMin, double maxValue,
			boolean includeMax) {
	...
	public boolean isResponsibleForValue(double value) {
		boolean minOk = (this.includingMin) ? value >= this.minValue
				: value > this.minValue;
		boolean maxOk = (this.includingMax) ? value <= this.maxValue
				: value < this.maxValue;
		return minOk && maxOk;
	}
	...
	public String toString() {
		return (this.includingMin ? "[" : "(")
				+ ValueRange.doubleToString(this.minValue, 5) + " - "
				+ ValueRange.doubleToString(this.maxValue, 5)
				+ (this.includingMax ? "]" : ")");
	}

	/**
	 * Creates a String representing a given double with a max. number of
	 * <code>postkomma</code> digits after the decimal separator.
	 * 
	 * @param toString
	 *            double to be represented
	 * @param postkomma
	 *            numbers of postkomma digits in the String
	...
	protected static String doubleToString(double toString, int postkomma) {
		...
		if (pointPos > -1) {
			...
			if (EPos < 0)
				EPos = representation.length();

			if (EPos > pointPos + postkomma) {
				representation = representation.substring(0, pointPos
						+ postkomma)
						+ representation.substring(EPos);
			}
		}
