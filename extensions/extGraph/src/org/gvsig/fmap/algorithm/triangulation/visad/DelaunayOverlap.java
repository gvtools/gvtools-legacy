...
/**
 * DelaunayOverlap quickly constructs a triangulation of a series of partially
 * overlapping two-dimensional gridded sets.
 * <P>
 * 
 * Grids should be aligned so that they overlap in a roughly vertical column
 * (i.e., as samples[*][1] increases or decreases, the grid number increases or
 * decreases respectively). DelaunayOverlap does not handle grids which overlap
...
public class DelaunayOverlap extends Delaunay {
	...
	/*
	 * This method could result in overlapping triangles in the triangulation in
	 * the intersecting section of two grids. For "bow-tie" shaped data, this
	 * problem is unlikely to occur, but for data with the left and right bowed
	 * inwards, overlapping triangles are more likely. The latter type of data
	 * is also more likely to have a greater number of Type 3 lost points.
	 */
	...
	public DelaunayOverlap(double[][] samples, int lenx, int leny)
		...
		for (int curgrid = 0; curgrid < numgrids; curgrid++) {
			for (int gy = 0; gy < leny - 1; gy++) {
				for (int gx = 0; gx < lenx - 1; gx++) {
					...
					if (((v10x - v00x) * (v11y - v10y) - (v10y - v00y)
							* (v11x - v10x) > 0 != sig)
							|| ((v11x - v10x) * (v01y - v11y) - (v11y - v10y)
									* (v01x - v11x) > 0 != sig)
							|| ((v01x - v11x) * (v00y - v01y) - (v01y - v11y)
									* (v00x - v01x) > 0 != sig)
							|| ((v00x - v01x) * (v10y - v00y) - (v00y - v01y)
									* (v10x - v00x) > 0 != sig)) {
						throw new SetException(
								"DelaunayOverlap: samples from grid " + curgrid
		...
		for (curgrid = 1; curgrid < numgrids; curgrid++) {
			...
			for (int line = 0; line < leny; line++) {
				...
				for (int pix = 0; pix < lenx; pix++) {
					...
					// look through all applicable previous grids until box
					// found
					boolean found = false;
					for (int g = prevgrid; g < curgrid; g++) {

						// search grid #g for containing grid box of current
						// point
						int gx = foundx[g];
						int gy = foundy[g];
						int ogx, ogy;
						...
						while (!found) {
							...
							case 0: // inside this box
								...
								if (glen[qg] > gsize[qg]) {
									int[] newg = new int[gsize[q] + gsize[q]
											+ 1];
									System.arraycopy(grid[qg], 0, newg, 0,
											gsize[qg]);
									grid[qg] = newg;
									gsize[qg] += gsize[qg];
								}
							...
							case 1:
							case 11: // up from this box
								gy--;
								break;
							case 2:
							case 7: // right from this box
								gx++;
								break;
							...
							if (ogx == gx && ogy == gy && !found)
								break;
						}

						// no need to check remaining grids if a grid box was
						// located
						if (found)
							break;
					}
				...
				// if entire scan line wasn't in a previous grid, advance
				// prevgrid
				prevgrid = lfound;
			}
		...
		for (curgrid = 0; curgrid < numgrids - 1; curgrid++) {
			...
			for (int pix = 0; pix < lenx; pix++) {
				...
				while (!found) {
					...
					// determine marching direction
					switch (((ax - bx) * (y - by) - (ay - by) * (x - bx) < 0 == sig ? 0
							: 1)
							+ ((bx - dx) * (y - dy) - (by - dy) * (x - dx) < 0 == sig ? 0
									: 2)
							+ ((dx - cx) * (y - cy) - (dy - cy) * (x - cx) < 0 == sig ? 0
									: 4)
							+ ((cx - ax) * (y - ay) - (cy - ay) * (x - ax) < 0 == sig ? 0
									: 8)) {
		...
		for (curgrid = 0; curgrid < numgrids; curgrid++) {
			for (int gy = 0; gy < leny - 1; gy++) {
				for (int gx = 0; gx < lenx - 1; gx++) {
					int npts = (curgrid == numgrids - 1) ? 0 : glen[leng
							* curgrid + (lenx - 1) * gy + gx];
					trisize += 2 * npts + 2;
				}
			}
		}
		...
		for (curgrid = 0; curgrid < numgrids; curgrid++) {
					...
					default: // triangulate first two points
						...
						for (int i = 2; i < glen[qg]; i++) {
							...
							for (itnum = 0; itnum < maxit && !found; itnum++) {
								...
								double p12x = p1x - p2x;
								double p12y = p1y - p2y;
								double c0 = p01x * (p0y - pty) - p01y
										* (p0x - ptx);
								double c1 = p12x * (p1y - pty) - p12y
										* (p1x - ptx);
								double c2 = p02x * (pty - p2y) - p02y
										* (ptx - p2x);
								boolean t0 = (c0 == 0 || c0 > 0 == p01x * p02y
										- p01y * p02x > 0);
								boolean t1 = (c1 == 0 || c1 > 0 == p12x * p01y
										- p12y * p01x > 0);
								boolean t2 = (c2 == 0 || c2 > 0 == p02x * p12y
										- p02y * p12x > 0);
							...
							if (!found)
								...
								// the first new triangle overwrites the old
								// triangle
								tri[curtri][0] = pt;
								...
								walk[w2][we2] = trilen;
								trilen++;
							} else if (curtri == tlr[qmg][0]
									|| curtri == tlr[qmg][1]) {
								// tlr[qmg][0]: curtri is on top edge of grid
								// box
								// tlr[qmg][1]: curtri is on left edge of grid
								// box
								...
								// otherwise, curtri is not on border of grid
								// box
			...
			if (curgrid == 0) {
				...
				if (samples[1][leftedge[ledges - 1]] < ty != sig
					...
					int sg = (min + max) / 2;
					int itnum = 0;
					while (samples[1][leftedge[sg + 1]] < ty == samples[1][leftedge[sg]] < ty
							&& itnum < ledges) {
					...
					// perform cross-product test to verify that points are
					// correct
					int pt = curgrid * lenp - lenx + 1;
					double px = samples[0][pt];
					double py = samples[1][pt];

					if ((tx - cx) * (ty - c1y) - (ty - cy) * (tx - c1x) > 0 == (px - cx)
							* (py - c1y) - (py - cy) * (px - c1x) > 0) {
						// inverted overlap, must adopt slightly different
						// strategy
						start2 = ledges - 1;
						...
						while (samples[1][lenx * sg + offst] < ll == samples[1][lenx
								* sg + offst2] < ll
								&& itnum < leny) {
				...
				if (samples[1][rightedge[redges - 1]] < ty != sig
					...
					int sg = (min + max) / 2;
					int itnum = 0;
					while (samples[1][rightedge[sg + 1]] < ty == samples[1][rightedge[sg]] < ty
							&& itnum < redges) {
				...
				while (base1 != end1 || base2 != end2 || b2lbr != 2) {
					...
					if (diag) { // diagonal goes from base1 to oneUp2
						...
						if (broken[curgrid][base1x == 0 ? 0 : base1x + 1] < base1y - 1
							...
							oneUp1x = base1x;
							oneUp1y = base1y - 1;
						} else if (base1y == leny - 1
								|| (base1x < lenx - 1 && broken[curgrid][base1x + 1] < base1y)) {
							// move right
							oneUp1x = base1x + 1;
							oneUp1y = base1y;
							down = false;
					...
					for (int gy = 0; gy <= broken[curgrid][gx + 1]; gy++) {
						...
						for (int pt = 0; pt < glen[qg]; pt++) {
							...
							for (itnum = 0; itnum < trilen && !located; itnum++) {
								...
								// test whether point is contained in current
								// triangle
								double tval0 = (Bx - Ax) * (Py - Ay)
										- (By - Ay) * (Px - Ax);
								double tval1 = (Cx - Bx) * (Py - By)
										- (Cy - By) * (Px - Bx);
								double tval2 = (Ax - Cx) * (Py - Cy)
										- (Ay - Cy) * (Px - Cx);
								boolean test0 = (tval0 == 0)
										|| ((tval0 > 0) == ((Bx - Ax)
												* (Cy - Ay) - (By - Ay)
												* (Cx - Ax) > 0));
								boolean test1 = (tval1 == 0)
										|| ((tval1 > 0) == ((Cx - Bx)
												* (Ay - By) - (Cy - By)
												* (Ax - Bx) > 0));
								boolean test2 = (tval2 == 0)
										|| ((tval2 > 0) == ((Ax - Cx)
												* (By - Cy) - (Ay - Cy)
												* (Bx - Cx) > 0));
				...
				for (int pix = 1; pix < lenx - 1; pix++) {
					...
					if (!ptfell[q] && broken[curgrid][pix] >= line
						...
						for (itnum = 0; itnum < trilen && !located; itnum++) {
							...
							// test whether point is contained in current
							// triangle
							double tval0 = (Bx - Ax) * (Py - Ay) - (By - Ay)
									* (Px - Ax);
							double tval1 = (Cx - Bx) * (Py - By) - (Cy - By)
									* (Px - Bx);
							double tval2 = (Ax - Cx) * (Py - Cy) - (Ay - Cy)
									* (Px - Cx);
							boolean test0 = (tval0 == 0)
									|| ((tval0 > 0) == ((Bx - Ax) * (Cy - Ay)
											- (By - Ay) * (Cx - Ax) > 0));
						...
						if (itnum < trilen) {
							...
							tri[curtri][2] = q;
							// walk[curtri][0] = T0; <-- already true
							// if (TE0 >= 0) walk[T0][TE0] = curtri; <-- already
							// true
							walk[curtri][1] = trilen;
							walk[curtri][2] = trilen + 1;
	...
	static final double[][] m_samples = { // grid 1 x-coordinates
			{ 65, 142, 215, 315, 373, 435, 39, 118, 202, 320, 373, 455, 40,
					114, 206, 307, 384, 457, 66, 128, 208, 308, 380,
					436,
					// grid 2 x-coordinates
					83, 144, 201, 293, 354, 433, 60, 135, 202, 285, 355, 456,
					59, 136, 204, 284, 362, 456, 75, 138, 207, 285, 363, 438,
					// grid 3 x-coordinates
					90, 153, 217, 292, 358, 441, 61, 145, 216, 292, 366, 452,
					55, 143, 213, 295, 373, 463, 80, 148, 217, 295, 375, 444 },
			// grid 1 y-coordinates
			{ 67, 87, 103, 104, 72, 42, 122, 148, 160, 165, 156, 109, 212, 211,
					203, 200, 204, 211, 282, 263, 248, 243, 256,
					287,
					// grid 2 y-coordinates
					166, 187, 201, 207, 185, 155, 230, 235, 240, 241, 235, 210,
					283, 275, 270, 267, 273, 280, 338, 310, 299, 297, 305, 331,
					// grid 3 y-coordinates
					247, 270, 277, 276, 265, 233, 295, 319, 325, 322, 306, 281,
					368, 371, 368, 362, 372, 376, 464, 431, 417, 418, 424, 455 } };
