 ...
 *  $Id: DelaunayPunkt.java 2446 2006-09-12 12:57:25Z LBST-PF-3\orahn $s
 */
...
public class DelaunayPunkt extends PirolPoint {
	...
	public void compile() {
		if (!this.wasCompiled) {
			this.nachbarVonArray = (DelaunayPunkt[]) this.nachbarVon
					.toArray(new DelaunayPunkt[0]);
			this.bildetKantenArray = (PirolEdge[]) this.bildetKanten
					.toArray(new PirolEdge[0]);
			this.gehoertZuDreieckArray = (Integer[]) this.belongsToDreieck
	...
	public void attendsEdge(PirolEdge k) {
		if (!this.wasCompiled && !this.bildetKanten.contains(k)) {
			...
			throw new RuntimeException(
					"this object was compiled already - can not add nay lines anymore");
	}
	...
	public PirolEdge[] getBildetKantenArray() {
		if (!this.wasCompiled)
			return (PirolEdge[]) bildetKanten.toArray(new PirolEdge[0]);
	...
	public void setAttendsTriangle(int nrDreieck) {
		if (!this.belongsTo(nrDreieck) && !this.wasCompiled)
			this.belongsToDreieck.add(new Integer(nrDreieck));
		else if (this.wasCompiled)
			throw new RuntimeException(
					"can not do this after this point was compiled");
	}
	...
	protected boolean belongsTo(int dreieckTriangle) {
		...
		while (iter.hasNext()) {
			...
			if (nummer == dreieckTriangle)
				return true;
		}
		return false;
	}

	public boolean attendsTriangleTogetherWith(DelaunayPunkt p1,
			DelaunayPunkt p2) {
		Integer[] triangleIndices = null;
		int nummer;

		if (!this.wasCompiled)
			triangleIndices = (Integer[]) this.belongsToDreieck
					.toArray(new Integer[0]);
		else
		...
		for (int i = 0; i < triangleIndices.length; i++) {
			...
			if (p1.belongsTo(nummer) && p2.belongsTo(nummer))
				return true;
		}
		return false;
	}
	...
	public void setNeighborOf(PirolPoint p) {
		if (!this.wasCompiled)
			...
			throw new RuntimeException(
					"this point was already compiled, can not add further neighbors!");
	}
	...
	public boolean istNachbarVon(int pIndex) {
		if (!this.wasCompiled) {
			...
			while (iter.hasNext()) {
				if (((PirolPoint) iter.next()).getIndex() == pIndex)
					return true;
			}
		} else {
			int numNeighbors = this.nachbarVonArray.length;
	...
	public DelaunayPunkt[] getTheOther2PointsFromDreieckTriangle(int nr) {
		...
		if (!this.wasCompiled)
			pointsArray = (DelaunayPunkt[]) this.nachbarVon
					.toArray(new DelaunayPunkt[0]);
		else
	...
	public Integer[] getDreieckTrianglesBelongingToArray() {
		if (!this.wasCompiled)
			return (Integer[]) belongsToDreieck.toArray(new Integer[0]);
