...
/**
 * 
 * Class that describes a single line, specified by it's starting and end point.
 * If offers methods e.g. to find intersection points with other Kante objects
 * or to determine on which side of the Kante object a given punkt object
 * resides.
 * 
 * @author Ole Rahn <br>
 * <br>
...
public class PirolEdge {
	...
	public static PirolEdge KANTE_X0Y0ToX0Y1 = new PirolEdge(
			PirolPoint.NULLPUNKT, new PirolPoint(new double[] { 0, 1 }), true,
			true);
	public static PirolEdge KANTE_X0Y0ToX1Y0 = new PirolEdge(
			PirolPoint.NULLPUNKT, new PirolPoint(new double[] { 1, 0 }), true,
			true);
	...
	public PirolEdge(PirolPoint anfang, PirolPoint ende,
			boolean anfangUnbegrenzt, boolean endeUnbegrenzt) {
		this.anfang = anfang;
		this.ende = ende;
	...
	public PirolEdge(PirolPoint anfang, double steigung, double laenge) {
		...
		try {
			this.ende = new PirolPoint(new double[] { anfang.getX() + dx,
					anfang.getY() + dy });
		} catch (Exception e) {
	...
	/**
	 * creates a new {@link PirolEdge} object, that has a length equal to
	 * <code>lineToShift</code>'s length and is parallel to
	 * <code>lineToShift</code>.
	 * 
	 * @param lineToShift
	 *            the line to be shifted
	 * @param shiftingVector
	 *            x,y,z component for the shifting (will be added to
	 *            <code>lineToShift</code>'s starting and end point).
	 * @return a new, shifted {@link PirolEdge} object
	 * @throws Exception
	 */
	public final static PirolEdge shiftLine(PirolEdge lineToShift,
			PirolPoint shiftingVector) throws Exception {
		...
		for (int i = 0; i < 2; i++) {
			...
			for (int dim = 0; dim < coords.length; dim++) {
				coords[dim] = currPkt.getCoordinate(dim)
						+ shiftingVector.getCoordinate(dim);
			}
		...
		return new PirolEdge(spkt, epkt, lineToShift.isAnfangUnbegrenzt(),
				lineToShift.isEndeUnbegrenzt());
	}

	/**
	 * switch starting and end point of <code>this</code> {@link PirolEdge}
	 * instance.
	 * 
	 */
	...
	public static PirolEdge kreiereKanteDurchPunktInnerhalbBegrenzung(
			PirolPoint p, double steigung, Envelope begrenzung)
			throws Exception {
	...
	public boolean isParallelZu(PirolEdge k, double infinityFactor)
			throws Exception {
		...
		if (this.getAnfang().distanceTo(sp) > infiniteDistance
				&& this.getEnde().distanceTo(sp) > infiniteDistance) {
			// as good as parallel
			return true;
		}

		// logger.printDebug("not parallel: " + this.getAnfang().distanceTo(sp)
		// + ", " + this.getEnde().distanceTo(sp) + ", infinity: " +
		// infiniteDistance);
	...
	public double getSteigung() throws Exception {
		...
		if (deltaY > 0)
			...
			throw new Exception(
					"wasn't able to calculate the increase of line (starting point == end point)");

	}

	public static PirolPoint getSchnittpunkt(PirolEdge k1, PirolEdge k2)
			throws Exception {
		PirolPoint a = k1.getAnfang();
		PirolPoint ab = new PirolPoint(
				new double[] { k1.getEnde().getX() - a.getX(),
						k1.getEnde().getY() - a.getY() });
		PirolPoint c = k2.getAnfang();
		PirolPoint cd = new PirolPoint(
				new double[] { k2.getEnde().getX() - c.getX(),
						k2.getEnde().getY() - c.getY() });
		...
		if (ab.getX() == 0 && cd.getX() != 0) {
			...
			if (a.getX() != c.getX())
				return null;
			else if ((c.getY() >= a.getY() && c.getY() <= a.getY() + ab.getY())
					|| (c.getY() <= a.getY() && c.getY() >= a.getY()
							+ ab.getY()))
				return new PirolPoint(new double[] { a.getX(), c.getY() });
			else if ((a.getY() >= c.getY() && a.getY() <= c.getY() + cd.getY())
		...
		/*
		 * else if ( ab.getY() && cd.getY()==0 ){ // sollte nicht auftreten m =
		 * 0; }
		 */
		else if (ab.getX() != 0
				&& (cd.getX() * ab.getY() / ab.getX()) != cd.getY()) {
			m = (a.getY() - c.getY() + ((c.getX() - a.getX()) * ab.getY())
					/ ab.getX())
					/ (cd.getY() - (cd.getX() * ab.getY() / ab.getX()));
		} else {
			return null;
		}
		...
		if (ab.getX() == 0 && ab.getY() != 0 && cd.getX() != 0) {
			n = (a.getY() - c.getY() - ((a.getX() - c.getX()) * cd.getY() / cd
					.getX())) / (-ab.getY());
		} else if (ab.getX() != 0) {
		...
		PirolPoint schnittpunkt = new PirolPoint(new double[] {
				a.getX() + n * ab.getX(), a.getY() + n * ab.getY() });
	...
	public double getABFaktorZumNormalenFaktor(PirolPoint pkt) throws Exception {
		...
		PirolPoint ab = new PirolPoint(new double[] { b.getX() - a.getX(),
				b.getY() - a.getY() });
		// N ist normale zu ab
		PirolPoint N = new PirolPoint(new double[] { ab.getY(), -ab.getX() });
		// berechne faktor y mit dem N multipliziert werden muss, um punkt3 zu
		// erreichen

		if (ab.getX() != 0) {
			double Xx = (c.getX() - a.getX() - normalenFaktor * N.getX())
					/ ab.getX();
			return Xx;
		}
		double Yx = (c.getY() - a.getY() - normalenFaktor * N.getY())
				/ ab.getY();
		return Yx;
	}
	...
	public double getNormalenFaktorZu(PirolPoint pkt) throws Exception {
		...
		/*
		 * # (C) # / | \ # / | \ # / |y*N \ # (A)______|_________(B) # ab ist
		 * vektor punkt1 -> punkt2 = a -> b
		 */
		...
		try {
			if (ab.getX() != 0 && ab.getY() != 0)
				ergebnis = ((punkt1.getY() - punkt3.getY()) - (punkt1.getX() - punkt3
						.getX()) * ab.getY() / ab.getX())
						/ (N.getX() * ab.getY() / ab.getX() - N.getY());
			else if (ab.getX() == 0)
				ergebnis = (punkt3.getX() - punkt1.getX()) / N.getX();
			else
				ergebnis = (punkt3.getY() - punkt1.getY()) / N.getY();
		} catch (Exception e) {
			// faengt division durch null fehler ab, die hier aber nicht
			// auftauchen koennen sollten
			ergebnis = 1;
		...
		/*
		 * if (Double.isNaN(ergebnis)){ logger.printError("got Nan!");
		 * logger.printDebug(this.toString());
		 * logger.printDebug(pkt.toString()); logger.printDebug(ab.toString());
		 * logger.printDebug("---"); }
		 */
	...
	public PirolEdge getNormalenKante(double laenge) throws Exception {
		...
		PirolPoint ab = new PirolPoint(new double[] {
				punkt2.getX() - punkt1.getX(), punkt2.getY() - punkt1.getY() });
		PirolPoint N = new PirolPoint(new double[] { ab.getY(), -ab.getX() });
		...
		PirolEdge nKante = new PirolEdge(new PirolPoint(new double[] {
				punkt2.getX(), punkt2.getY() }), new PirolPoint(new double[] {
				punkt2.getX() + N.getX(), punkt2.getY() + N.getY() }), false,
				false);
	...
	public String toString() {
		return "Kante<" + this.punktIndexA + "," + this.punktIndexB + ">["
				+ this.getAnfang().toString() + "," + this.getEnde().toString()
				+ "]";
	}
	...
	public double getLaenge() throws Exception {
		...
		throw new Exception(PirolEdge.class.getName()
				+ ": Starting point or end point not specified!");
	}
