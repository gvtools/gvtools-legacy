...
public class GeometryInfoListener implements PointListener {
	...
	/**
	 * When the users click over the view the point is caught and handled in
	 * this method, which will look for alfanumeric information at that feature
	...
	public void point(PointEvent event) throws BehaviorException {
		...
		List<FLyrVect> activeLyrs = FLyrUtil.getActiveVectorialLyrs(mapCtrl
				.getMapContext());
		String text = "";
		...
		if (numActiveLyrs > 0) {
			try {
				for (int i = 0; i < numActiveLyrs; i++) {
					...
					Rectangle2D rect = FGeometryUtil.getSnapRectangle(
							mapPoint.getX(), mapPoint.getY(), tol);
					IFeatureIterator iterator = lyr.getSource()
							.getFeatureIterator(rect, null, null, false);
					text += "<h3><b>" + PluginServices.getText(this, "LAYER")
							+ "</b>: " + lyr.getName() + "</h3>";
					int numFeatures = 0;
					text += "<table border=\"1\">";
					while (iterator.hasNext()) {
						...
						Geometry geometry = NewFConverter.toJtsGeometry(feature
								.getGeometry());
						text += "<tr><td><b>"
								+ PluginServices.getText(this, "FEATURE_ID")
								+ "</b>: " + feature.getID() + "</td></tr>";
						text += "<tr>";
						text += "<td><b>"
								+ PluginServices.getText(this, "AREA")
								+ "</b>: " + nf.format(geometry.getArea())
								+ "</td></tr>";
						text += "<tr><td><b>"
								+ PluginServices.getText(this, "PERIMETER")
								+ "</b>: " + nf.format(geometry.getLength())
								+ "</td>";
						text += "</tr>";

						text += "<tr>";
						text += "<td><b>" + PluginServices.getText(this, "WKT")
								+ "</b>: " + geometry.toText() + "</td>";
						text += "</tr>";
					}// while
					if (numFeatures == 0) {
				...
				if (text != "") {
					HTMLPanel panel = new HTMLPanel(PluginServices.getText(
							this, "GEOMETRY_INFO"), text);
