...
public class NewTopologyRulePanel extends JPanel implements IWindow {
	...
	/**
	 * Relates the user friendly rule name (showed in the rules' combobox) with
	 * the class that implements this rule
	 */
	 ...
	 * @param layers
	 */
	public NewTopologyRulePanel(List<FLyrVect> lyrsOfTopology,
	...
	private boolean checkTwoLyrRule() {
		Class clazz = ruleName_ruleClass.get(topologyRulesCombo
				.getSelectedItem());
	...
	/**
	 * Enables or disables second layer combo box, in function of the selected
	 * rule applies to one or to two layers.
	 */
	private void updateSecondLayerComboStatus() {
		if (!checkTwoLyrRule()) {
			lyrCombo2.setEnabled(false);
			lyrCombo2.setToolTipText(PluginServices.getText(null,
			...
			try {
				...
				for (int i = 0; i < numLyrs; i++) {
					...
					if (tempLyr == lyrCombo.getSelectedLayer())// we jump the
																// selected
																// origin layer
						continue;
	...
	private void updateRuleDescription() {
		...
		try {
			...
			htmlViewer.setText("<p>"
					+ PluginServices.getText(this, "UNAVAILABLE_DESCRIPTION")
					+ "</p>");
		}
	}
	...
	private void initialize() {
		...
		rightPanel = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		acceptCancelPanel = new AcceptCancelPanel(new ActionListener() {// ok
					public void actionPerformed(ActionEvent e) {
						...
						if (ruleClass != null) {
							...
							if (ITwoLyrRule.class.isAssignableFrom(ruleClass)) {
								if (lyrCombo2.isEnabled()) {
									params.put("destinationLyr",
											lyrCombo2.getSelectedLayer());
								} else {
									String errorMessage = PluginServices
											.getText(this,
													"no_hay_capas_suficientes_para_esta_regla");
									String errorTitle = PluginServices.getText(
											this, "error_creando_regla");
							...
							try {
								newTopologyRule = TopologyRuleFactory
										.createRule(ruleClass, params, null);
							} catch (TopologyRuleDefinitionException e1) {
								e1.printStackTrace();
								String errorMessage = PluginServices.getText(
										this, e1.toString());
								String errorTitle = PluginServices.getText(
										this, "error_creando_regla");
								GUIUtil.getInstance().messageBox(errorMessage,
										errorTitle);
								return;
							}
	...
	private ComboBoxModel createRulesCbModelForInputLyr(FLyrVect inputLyr) {
		...
		if (topologyRules == null) {
			cbModel = new DefaultComboBoxModel(
					new Object[] { PluginServices.getText(null,
	...
	// Class clazz =
	// ruleName_ruleClass.get(topologyRulesCombo.getSelectedItem());
	// IOneLyrRule rule;
	// try {
	// rule = (IOneLyrRule) clazz.newInstance();
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices.getText(this, "Topology_rules"));
