 ...
 * 
 */
public class TopologyPropertiesPanel extends JPanel implements
	...
	private void initialize() {
		...
		final TopologyContentProvider contentProvider = new TopologyContentProvider(
				originalTopology);
		// General tab
		generalTab = new GeneralTabPanel(readOnly);
		getJTabbedPane().addTab(PluginServices.getText(this, "General"),
				generalTab);
		 ...
		 * public void selectionEvent(SelectedLayerEvent event) { FLyrVect
		 * eventLyr = (FLyrVect) event.lyrOfEvent; String warningMessage = "";
		 * String title = ""; if(event.eventType ==
		 * SelectedLayerEvent.REMOVED_EVENT_TYPE){ List<ITopologyRule>
		 * rulesToRemove = originalTopology.getRulesByLyr(eventLyr);
		 * 
		 * if(rulesToRemove.size() > 0){ warningMessage +=
		 * PluginServices.getText(this, "Remove_Lyr_WARNING1"); for(int i = 0; i
		 * < rulesToRemove.size() - 1; i++){ warningMessage +=
		 * rulesToRemove.get(i).getDescription(); warningMessage += ";"; }
		 * warningMessage += rulesToRemove.get(rulesToRemove.size() -
		 * 1).getName()+"\n"; }
		 * 
		 * warningMessage += PluginServices.getText(this,
		 * "Remove_Lyr_WARNING2"); title = PluginServices.getText(this,
		 * "warning"); boolean deleteLyr = GUIUtil.getInstance().
		 * optionMessage(warningMessage, title);
		 * 
		 * if(deleteLyr) originalTopology.removeLayer(eventLyr); }else
		 * if(event.eventType == SelectedLayerEvent.ADDED_EVENT_TYPE){ boolean
		 * addLyr = true; if(originalTopology.getStatus() ==
		 * ITopologyStatus.VALIDATED_WITH_ERRORS){ warningMessage =
		 * PluginServices.getText(this, "AddLyrDeleteErrors_WARNING"); title =
		 * PluginServices.getText(this, "warning"); addLyr =
		 * GUIUtil.getInstance().optionMessage(warningMessage, title); }
		 * 
		 * if(addLyr){ originalTopology.addLayer(eventLyr);
		 * errorPanel.updateErrorTable(); } }
		 * 
		 * generalTab.updateContent();
		 * rulesTabPanel.updateContent(contentProvider); }});
		 */
		getJTabbedPane().addTab(PluginServices.getText(this, "Layers"),
				lyrsPanel);
		 ...
		 * public void selectionEvent(SelectedRuleEvent event) { ITopologyRule
		 * eventRule = event.getRule(); if(event.getEventType() ==
		 * SelectedRuleEvent.RULE_REMOVED){
		 * originalTopology.removeRule(eventRule);
		 * 
		 * generalTab.updateContent();
		 * rulesTabPanel.updateContent(contentProvider); }
		 * 
		 * }}); rulesTabPanel.addRuleSelectionListener(new
		 * RuleSelectionListener(){ public void selectionEvent(SelectedRuleEvent
		 * event) { generalTab.updateContent(); }});
		 */
		getJTabbedPane().addTab(PluginServices.getText(this, "Rules"),
				rulesTabPanel);

		// Topology errors tab
		errorPanel = new TopologyErrorPanel(
				originalTopology.getErrorContainer());
		getJTabbedPane().addTab(PluginServices.getText(this, "Errors"),
				errorPanel);
	}
			...
			jTabbedPane.addChangeListener(new ChangeListener() {
				public void stateChanged(ChangeEvent e) {
					...
					if (acceptPanel instanceof AcceptCancelPanel) {
						((AcceptCancelPanel) acceptPanel).setOkButtonEnabled(!(tabs
								.getSelectedComponent() instanceof WizardPanel));
					}
				...
				public void actionPerformed(ActionEvent arg0) {
					if (PluginServices.getMainFrame() != null) {
						PluginServices.getMDIManager().closeWindow(
								(IWindow) TopologyPropertiesPanel.this);
					} else {
	...
	public WindowInfo getWindowInfo() {
		...
		// WindowInfo.RESIZABLE | WindowInfo.PALETTE);
		m_viewinfo
				.setTitle(PluginServices.getText(this, "Topology_properties"));
	...
	private void verifyChanges() throws CloneNotSupportedException {
		...
		if (generalTab.isChanged()) {
			...
			// Meter aquí un mensaje que avise de que si se cambia la tolerancia
			// de cluster se va a resetear
		...
		GUIUtil.getInstance().updateTopologyInToc(
				originalTopology.getMapContext(), originalTopology);
	...
	class GeneralTabPanel extends GridBagLayoutPanel {
		...
		public int getMaxNumberOfErrors() {
			int maxNumberErrors;
			try {
				maxNumberErrors = Integer.parseInt(maxNumberOfErrorsTextField
						.getText());
			} catch (NumberFormatException e) {
				maxNumberErrors = -1;
			}
		...
		private void initialize() {
			...
			firstPanel.add(new JLabel(PluginServices.getText(this,
					"topology_name")));
			...
			secondPanel.add(new JLabel(PluginServices.getText(this,
					"cluster_tolerance")));
			secondPanel.add(Box.createHorizontalStrut(5));
			clusterTextField = new JTextField(
					Double.toString(clusterTolerance), 20);
			...
			thirdPanel.add(new JLabel(PluginServices.getText(this,
					"topology_status")));
			...
			fourthPanel.add(new JLabel(PluginServices.getText(this,
					"max_number_of_errors")));
			fourthPanel.add(Box.createHorizontalStrut(5));
			maxNumberOfErrorsTextField = new JTextField(
					Integer.toString(maxNumberOfErrors), 20);
			...
			fithPanel.add(new JLabel(PluginServices.getText(this,
					"number_of_layers")));
			...
			sixthPanel.add(new JLabel(PluginServices.getText(this,
					"number_of_rules")));
			...
			if (readOnly) {
				...
				clusterTextField.getDocument().addDocumentListener(
						changeListener);
				maxNumberOfErrorsTextField.getDocument().addDocumentListener(
						changeListener);
			}
		}
		...
		private String getStatusAsText() {
			...
			case Topology.NOT_VALIDATED:
				topologyText = PluginServices.getText(this,
			...
			case Topology.VALIDATING:
				topologyText = PluginServices
						.getText(this, "VALIDATING_STATUS");
				break;
		...
		public void updateContent() {
			...
			clusterTextField.setText(Double.toString(originalTopology
					.getClusterTolerance()));
			String statusText = getStatusAsText();
			statusLabel.setText("<html><b>" + statusText + "</b></html>");
			maxNumberOfErrorsTextField.setText(Integer
					.toString(originalTopology.getMaxNumberOfErrors()));
			nLyrsLabel.setText("<html><b>" + originalTopology.getLayerCount()
					+ "</b></html>");
			nRulesLabel
					.setText(String.valueOf(originalTopology.getRuleCount()));
		}
	}
