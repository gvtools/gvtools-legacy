...
public class IntersectionNodeVisitor implements FeatureVisitor {
	 ...
	 * @param dangleTol
	 */
	public IntersectionNodeVisitor(FeatureProcessor processor,
			FeaturePersisterProcessor2 intersectsProcessor,
			boolean cleanOnlySelection, ILayerDefinition layerDefinition,
			FLyrVect firstLayer, SelectableDataSource source,
			SnappingCoordinateMap snapCoordMap, double dangleTol,
			double fuzzyTol, FeatureProcessor danglesProcessor) {
	...
	public void visit(IGeometry g, final int index) throws VisitorException,
		...
		SnappingNodeMap nodes = new SnappingNodeMap(new NodeFactory(),
				snapTolerance);
		/*
		...
		try {
			CleanFeatureVisitor cleanFeatureVisitor = new CleanFeatureVisitor(
					index, onlySelection, layerToClean, recordset, jtsGeo,
					nodes, snapCoordMap, intersectProcessor);
			strategy.process(cleanFeatureVisitor, g.getBounds2D());

			/*
			 * Primero hay que 'ordenar' los nodos, dentro de la geometria
			 * jtsGeo, que es con la que estan intersectando
			 */
			...
			if (nodes.values().size() > 0) {
				...
				for (int i = 0; i < coords.length - 1; i++) {
					...
					while (it.hasNext()) {
						...
						Point point = geomFact
								.createPoint(node.getCoordinate());
						/*
						 * Hay que comprobar si para la linea obtenida, las
						 * coordenadas del nodo intersectan. La siguiente
						 * condicion viene a comprobar lo mismo
						 */
						if (line.distance(point) < 0.0000001) {
							/*
							 * Si intersecta, se crea una feature, desde la
							 * ultima coordenada insertada, hasta el nodo
							 */
							insertCoordinate(auxCoords, node.getCoordinate());
							if (auxCoords.size() > 1) {
								LineString lineToInsert = geomFact
										.createLineString(getCoordsAsArray(auxCoords));
								if (lineToInsert.getLength() >= dangleTolerance) {
									IFeature feature = createFeature(
											lineToInsert, index);
									featureProcessor.processFeature(feature);
									auxCoords.clear();
									insertCoordinate(auxCoords,
											node.getCoordinate());
								} else {
				...
				if (auxCoords.size() > 1) {
					...
					if (lineToInsert.getLength() >= dangleTolerance) {
						...
						if (!isAPoint(lineToInsert)) {
							IFeature feature = createFeature(lineToInsert,
									index);
							dangleProcessor.processFeature(feature);
						}
					}
			...
			//
			// // It computes linear distance of a point on the given jtsGeo
			// linear
			// // geometry
			// boolean rightGeometry = true;
			// if (nodes.values().size() > 0) {
			...
			// ArrayList<LineIntersection> nodeIntersections = new
			// ArrayList<LineIntersection>();
			...
			// rightGeometry = false;
			// LineIntersectionComparator comp = new
			// LineIntersectionComparator();
			// Collections.sort(nodeIntersections,comp);
			//
			// LinearLocation lastLocation = null;
			// LineIntersection lastIntersection = null;
			// LocationIndexedLine indexedLine = new
			// LocationIndexedLine(jtsGeo);
			// for (int i = 0; i < nodeIntersections.size(); i++) {
			// Geometry solution = null;
			// LineIntersection li = (LineIntersection)
			// nodeIntersections.get(i);
			//
	 ...
	 * 
	 * @param nodes
	 *            - SnappingNodeMap con los nodos encontrados que intersectan
	 *            con la geometria
	 * @param geom
	 *            - geometria
	 */
	private SnappingNodeMap sortNodes(SnappingNodeMap nodes, Geometry geom) {
		...
		GeometryFactory geomFact = new GeometryFactory();
		SnappingNodeMap auxSNM = new SnappingNodeMap(new NodeFactory(),
				snapTolerance);
		...
		for (int i = 0; i < coords.length - 1; i++) {
			...
			for (int j = 0; j < nodes.values().size(); j++) {
				...
				while (it.hasNext()) {
					...
					if (line.distance(point) < 0.0000001) {
						/*
						 * Y luego que sea el que menor distancia tiene al
						 * primero de los vertices de la linea
						 */
		...
		/*
		 * Ahora se itera a la 'inversa' para introducir en el map los nodos,
		 * segun se han ido 'ordenando'
		 */
	...
	public String getProcessDescription() {
		return "Eliminando pseudonodos de la capa vectorial '"
				+ this.layerToClean.getName() + "'";
	}
	...
	/**
	 * Crea una feature en el 'recordset' con la geometria recibida, y las
	 * propiedades de la feature de indice 'firstLayerIndex', correspondiente a
	 * la capa 'firstLayer'
	 * 
	 * @param jtsGeometry
	 *            - nueva geometria
	 * @param firstLayerIndex
	 *            - indice de la feature en la capa original (firstLayer)
	...
	private IFeature createFeature(Geometry jtsGeometry, int firstLayerIndex)
		...
		featureAttr[featureAttr.length - 2] = valFact
				.createValue(firstLayerIndex);
