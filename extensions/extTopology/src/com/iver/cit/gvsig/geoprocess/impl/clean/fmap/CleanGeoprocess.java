...
public class CleanGeoprocess extends AbstractGeoprocess {
	...
	public ILayerDefinition createLayerDefinition() {
		if (resultLayerDefinition == null) {
			try {
				...
				FieldDescription[] fields = resultLayerDefinition.getFieldsDesc();
	...
	public void setParameters(Map params) throws GeoprocessException {
		...
		if (createLyrsWithError != null)
			this.createLyrsWithErrorGeometries = createLyrsWithError.booleanValue();
	...
	public FLayer getResult() throws GeoprocessException {
		if(getOnlyFirstResultLayer) {
			...
			if(this.createLyrsWithErrorGeometries){
				MapContext map = ((View)PluginServices.getMDIManager().getActiveWindow()).getModel().getMapContext();
				...
				SimpleMarkerSymbol symIntNode =(SimpleMarkerSymbol) SymbologyFactory.createDefaultSymbolByShapeType(FShape.POINT, Color.RED);
				
				dangleLines = (FLyrVect) createLayerFrom(this.dangleWriter);
				ILineSymbol symDangleEdge = (ILineSymbol)SymbologyFactory.createDefaultSymbolByShapeType(FShape.LINE, Color.BLUE);
				
				try {
					if(pseudonodes.getSource().getShapeCount() != 0){
						solution.addLayer(pseudonodes);
						symIntNode.setSize(8);
						pseudonodes.setLegend(new SingleSymbolLegend(symIntNode));
					}
					
					if(dangleLines.getSource().getShapeCount() != 0) {
						solution.addLayer(dangleLines);
						dangleLines.setLegend(new SingleSymbolLegend(symDangleEdge));
					}
	...
	private class LineCleanTask extends AbstractMonitorableTask implements IPipedTask{
		...
		/**
		 * Recorre todas las geometrias de la capa para fusionar sus nodos en caso de que sea necesario
		 * @param layer - capa a 'corregir'
		 */
			...
			try {
				...
				for(int i=0;i<rv.getShapeCount();i++) {
					if(!isCanceled()) {
						reportStep();
						/*
						 * Se itera para cada registro de la capa (se controla tb si
						 * el geoproceso solo debe actuar sobre los seleccionados)
						 */
						boolean ahead = false;
						if(selection  != null) {
							if(selection.get(i))
						...
						if(ahead) {
							...
							for(int j=0;j<shape.getSelectHandlers().length;j++) {
								Point2D point = shape.getSelectHandlers()[j].getPoint();
		...
		/**
		 * Fusiona el nodo que se encuentra en las coordenadas del punto 'p' con las geometrias
		 * que se encuentren dentro de un radio menor a la tolerancia fuzzy
		 * 
		 * @param layer - capa clonada de la original sobre la que se realiza el geoproceso
		 * @param p - Punto del registro que se esta analizando
		 * @param id - Identificador en la capa del registro que se esta analizando
		 * @param indHandler - indice del vertice (punto) en la polilinea
		 */
		private void mergeNode(FLyrVect layer, Point2D p, int id, int indHandler) {
			try {
				...
				if(intersections != null) {
					for(int i=0;i<intersections.size();i++) {
						if(p.distance(intersections.get(i))<fuzzyTol) {
							Coordinate coord = new Coordinate(intersections.get(i).getX(),intersections.get(i).getY());
				...
				FBitSet bitSet = layer.queryByShape(circle,DefaultStrategy.INTERSECTS);
				...
				if(found.size() == 1) {
					/*
					 * Si solo encuentra un registro, puede significar dos cosas:
					 * 1 - Que no hay otro registro con el que fusionar
					 * 2 - Que hay que fusionar la geometria del registro consigo misma
					 */
					if(found.get(0) == id){
						IGeometry geom = rv.getShape(id);
						MultiLineString multiLine = (MultiLineString)geom.toJTSGeometry();
						...
						if(closest != null) {
							/*
							 * Si la distancia del punto mas cercano al nodo es menor que la tolerancia
							 * fuzzy, hay que cambiar las coordenadas del nodo, y situarlo sobre la
							 * coordenada calculada
							 */
							if(p.distance(closest.x,closest.y)<fuzzyTol) {
								updateFeature(rv, layer, closest, id, indHandler);
								if(intersections == null) {
									intersections = new ArrayList<Point2D>();
									intersections.add(new Point2D.Double(closest.x,closest.y));
								}
								else
									intersections.add(new Point2D.Double(closest.x,closest.y));
							}
					...
					for(int i=0;i<found.size();i++) {
						...
						MultiLineString jtsGeomFeat = (MultiLineString)geomFeat.toJTSGeometry();
						Coordinate pointC = new Coordinate(p.getX(),p.getY());
						Coordinate newClosest = getClosestPoint(jtsGeomFeat.getCoordinates(),pointC);
						if(newClosest != null)
							if(p.distance(newClosest.x,newClosest.y)<fuzzyTol) 
								if(closest != null) {
									if(p.distance(newClosest.x,newClosest.y)<p.distance(closest.x,closest.y)) {
										updateFeature(rv, layer, newClosest, id, indHandler);
										closest = newClosest;
									}
								}
								else {
		 ...
		 * @param coords - conjunto de coordenadas de la polilinea
		 * @param pointC - coordenadas del punto
		 * @return - coordenadas del punto mas cercano; null en caso de 
		 * que el punto mas cercano sea una de las coordenadas de 'coords'
		 */
		private Coordinate getClosestPoint(Coordinate[] coords,Coordinate pointC) {
			Coordinate closest = null;
			for(int i=0;i<coords.length-1;i++) {
				Coordinate c1 = coords[i];
				Coordinate c2 = coords[i+1];
				/*
				 * Primero se comprueba que los vertices esten dentro de la tolerancia
				 * fuzzy. En ese caso, se considerara que el vertice es el punto
				 * mas cercano
				 */
				if(c1.distance(pointC) < fuzzyTol && !isSamePoint(c1,pointC)) {
				...
				else if(c2.distance(pointC) < fuzzyTol && !isSamePoint(c2,pointC)) {
					if(closest != null) {
					...
					/*
					 * Hay que comprobar que ninguno de los puntos del segmento de la polilinea
					 * 'coincide' con las coordenadas del nodo 
					 */
					if(!isSamePoint(pointC, c1) && !isSamePoint(pointC, c2)) {
						...
						if(closest != null) {
							if(line.closestPoint(pointC).distance(pointC) < closest.distance(pointC))
		...
		/**
		 * Actualiza la feature 'feat' en la capa 'layer' cambiando su geometria. Al vertice de indice
		 * 'indHandler' le asigna las nuevas coordenadas 'closest'
		 * @param rv - Feature de la que se va a modificar la geometria
		 * @param layer - capa sobre la que se esta trabajando
		 * @param closest - nuevas coordenadas del nodo
		 * @param idFeature - id de la feature en la capa
		 * @param indHandler - indice del nodo a modificar
		 * @throws ReadDriverException 
		 * @throws ExpansionFileReadException 
		 */
		private void updateFeature(ReadableVectorial rv,FLyrVect layer,Coordinate closest,int idFeature, int indHandler) throws ExpansionFileReadException, ReadDriverException {
			...
			DefaultFeature df = new DefaultFeature(geometry,newFeat.getAttributes(),newFeat.getID());
			DefaultRowEdited dre = new DefaultRowEdited(df,DefaultRowEdited.STATUS_MODIFIED,idFeature);
			VectorialEditableAdapter adapter = (VectorialEditableAdapter)layer.getSource();
			
			try {
				adapter.modifyRow(dre.getIndex(),dre.getLinkedRow(),"",EditionEvent.GRAPHIC);
		...
		/**
		 * Comprueba si dos coordenadas corresponden al mismo punto. Si su distancia es menor que
		 * 0.000000001 metros, se asume que es el mismo punto
		 * @param c1 - coordenada 1
		 * @param c2 - coordenada 2
		 * @return TRUE si ambas coordenadas corresponden al mismo punto. FALSE en caso contrario
		 */
		...
		public void run() throws Exception {
			/*
			 * Se clona la capa inicial y su seleccion, para fusionar nodos con otros nodos o con
			 * segmentos, segun la tolerancia 'fuzzy'. De esta manera se pueden modificar
			 * las geometrias de la capa 'original' y tenerlas preparadas para aplicarles
			 * la separacion de pseudonodos.
			 */
			...
			/*
			 * Se exporta a otra capa (pq aunque se clone, el fichero fisico del disco es el mismo)
			 * De esta manera, ya se puede trabajar con la capa auxiliar sin que los cambios de 
			 * geometria afecten a la capa inicial
			 */
			...
			PluginServices.getExtension(StartEditingClean.class).execute("STARTEDITING");
			
			/*
			 * Despues de haber fusionado los nodos, hay que guardar la capa auxiliar,
			 * para que los cambios en las geometrias tengan efecto. De este modo, esta
			 * capa sera la que reciba el siguiente codigo para tratar los pseudonodos
			 */
			mergeNodes(auxLayer);
			PluginServices.getExtension(StopEditingClean.class).execute("STOPEDITING");
			...
			String prefName = resultLayerName.substring(0,resultLayerName.indexOf(".shp"));
			String temp = System.getProperty("java.io.tmpdir") + "/" + prefName + "_intersections.shp";
			...
			((ShpWriter) intersectionsWriter).initialize((LayerDefinition) intersectDefinition);
			((SHPLayerDefinition) intersectDefinition).setFile(newFile);

			ShpSchemaManager interSchMg = new ShpSchemaManager(newFile.getAbsolutePath());
			interSchMg.createSchema(intersectDefinition);

			intersectsProcessor = new FeaturePersisterProcessor2(intersectionsWriter);
			
			dangleWriter = new ShpWriter();
			String temp2 = System.getProperty("java.io.tmpdir") + File.separatorChar + prefName + "_dangles.shp";
			...
			FieldDescription[] dangleFields = auxLayer.getRecordset().getFieldsDescription().clone();
			dangleDefinition.setFieldsDesc(dangleFields);
			
			((ShpWriter) dangleWriter).initialize((LayerDefinition) dangleDefinition);
			((SHPLayerDefinition) dangleDefinition).setFile(dangleFile);

			ShpSchemaManager dangleSchMg = new ShpSchemaManager(dangleFile.getAbsolutePath());
			...
			IntersectionNodeVisitor visitor = new IntersectionNodeVisitor(processor, intersectsProcessor, onlyFirstLayerSelection, resultLayerDefinition, auxLayer, auxLayer.getRecordset(), coordMap,dangleTol,fuzzyTol,dangleProcessor);
			...
			if(fromBuild) {
				...
				String path = resPath.substring(0,resPath.indexOf(shpFile.getName())) + "build.shp";
				...
				params.put("createlayerswitherrors", new Boolean(createLyrsWithErrorGeometries));
		 ...
		 * @param layer - capa a exportar
		 * @return la nueva capa que ya apunta fisicamente al shape que se acaba de crear
		 * @throws ReadDriverException
		 * @throws BaseException
		 */
		private FLyrVect exportAuxLayer(FLyrVect layer) throws ReadDriverException, BaseException {
			...
			try {
				...
				String temp = System.getProperty("java.io.tmpdir") + "/" + "auxLayer.shp";
				File newFile = new File(temp);
				IndexedShpDriver drv = new IndexedShpDriver();
				if (!newFile.exists()) {
					try {
						newFile.createNewFile();
						File newFileSHX=new File(newFile.getAbsolutePath().replaceAll("[.]shp",".shx"));
						newFileSHX.createNewFile();
						File newFileDBF=new File(newFile.getAbsolutePath().replaceAll("[.]shp",".dbf"));
						newFileDBF.createNewFile();
					} catch (IOException e) {
						throw new FileNotFoundDriverException("SHP",e,newFile.getAbsolutePath());
					}
				}
				drv.open(newFile);
		...
		public String getNote() {
			String cleaningText1 = PluginServices.getText(this, "Limpiando_lineas");
			String cleaningText2 = PluginServices.getText(this, "Merge_geometry_nodes");
			String of = PluginServices.getText(this, "de");
			String retText1 = cleaningText1 + " " + (getCurrentStep()-(getFinishStep()/2)) + " " + of + " " + (getFinishStep()/2);
			String retText2 = cleaningText2 + " " + getCurrentStep() + " " + of + " " + (getFinishStep()/2);
