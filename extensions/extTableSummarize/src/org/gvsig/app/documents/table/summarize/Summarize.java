...
public class Summarize {
	...
	public void summarizeToWriter(IWriter writer)
			throws InitializationException, GroupByFieldNotExistsException,
			SummarizeException, DBFExportException, GroupingErrorException {
		if (operations == null) {
		...
		try {
			...
			summarizeToWriter(writer, outputFieldNames.toArray(new String[0]),
					outputTypeList.toArray());
		} catch (ReadDriverException e) {
			throw new SummarizeException(e);
		}
	}

	public void summarizeToDbf(File dbfFile) throws InitializationException,
			GroupByFieldNotExistsException, SummarizeException,
			DBFExportException, GroupingErrorException {
		if (operations == null) {
			throw new InitializationException("Operation not initialized");
		}
		if (sds == null) {
		...
		try {
			createFieldsAndTypes(operations, outputFieldNames, outputTypeList);
			IWriter writer = createDbfFile(dbfFile,
	...
	protected void summarizeToWriter(IWriter writer, String[] outputFieldNames,
			int[] fieldTypes) throws SummarizeException, DBFExportException,
			GroupingErrorException {
		try {
			initWriter(writer);
			calculateStats(getOperations(), groupByField, outputFieldNames,
					writer);
			finishWriter(writer);
	...
	protected void createFieldsAndTypes(
			ArrayList<SelectedStatistics> operations,
			ArrayList<String> outputFieldNames, IntList fieldTypes)
			throws ReadDriverException, GroupByFieldNotExistsException,
			SummarizeException {
		...
		String groupByName = createUniqueFieldName(outputFieldNames, "",
				PluginServices.getText(this, "GROUP"), maxlength);
		outputFieldNames.add(groupByName);
		fieldTypes.add(sds.getFieldType(groupByFieldIdx));

		String countName = createUniqueFieldName(outputFieldNames, "",
				PluginServices.getText(this, "COUNT"), maxlength);
		...
		for (int i = 0; i < operations.size(); i++) {
			...
			if (operations.get(i).isMin()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "MIN"), maxlength);
			...
			if (operations.get(i).isMax()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "MAX"), maxlength);
				outputFieldNames.add(newField);
				fieldTypes.add(currFieldType);
			}
			if (operations.get(i).isMean()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "MEAN"), maxlength);
				outputFieldNames.add(newField);
			...
			if (operations.get(i).isSum()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "SUM"), maxlength);
				outputFieldNames.add(newField);
				fieldTypes.add(currFieldType);
			}
			if (operations.get(i).isSd()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "STDEV"), maxlength);
				outputFieldNames.add(newField);
				fieldTypes.add(Types.DOUBLE);
			}
			if (operations.get(i).isVar()) {
				String newField = createUniqueFieldName(outputFieldNames,
						currentFieldName,
						"_" + PluginServices.getText(this, "VAR"), maxlength);
				outputFieldNames.add(newField);
				fieldTypes.add(Types.DOUBLE);
			}
		}
	...
	/**
	 * <p>
	 * Creates a new field name (not contained in the existingFields list),
	 * based in the provided baseName and suffix and with the provided maximum
	 * length. The following pattern is used to create the new names:
	 * <p>
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * <p>
	 * Examples of created names:
	 * </p>
	 * 
	 * <pre>
	 * baseName: "TERRAIN", suffix: "_MAX", maxLength: 11
	...
	public String createUniqueFieldName(ArrayList<String> existingFields,
			String baseName, String suffix, int maxLength)
			throws SummarizeException {

		// ensure we don't try to take a substring longer than baseName.length
		int auxLength = Math
				.min(maxLength - suffix.length(), baseName.length());
		...
		// ensure the index length is not longer than the maximum allowed field
		// length
		...
		do {
			...
			// ensure we don't try to take a substring longer than
			// baseName.length
			auxLength = Math.min(maxLength - Integer.toString(index).length()
	...
	public void calculateStats(ArrayList<SelectedStatistics> operations,
		...
		try {
			indiceColumnaMaestra = getDataSource().getFieldIndexByName(
					groupByField);

			// ONLY IF THE USER HAS SELECTED SOME ROWS
			if (dss.cardinality() > 0) {

				for (int i = dss.nextSetBit(0); i >= 0; i = dss
						.nextSetBit(i + 1)) {
					// get all the distinct groups
					if (!claves.contains(getDataSource().getFieldValue(i,
							indiceColumnaMaestra)))
						claves.add(getDataSource().getFieldValue(i,
								indiceColumnaMaestra));
				}
			} else {
				// ONLY IF THE USER HAS NOT SELECTED ANY ROW
				for (int i = 0; i < getDataSource().getRowCount(); i++) {
					// get all the distinct groups (this may be done with a
					// SELECT DISTINCT)
					if (!claves.contains(getDataSource().getFieldValue(i,
							indiceColumnaMaestra)))
						claves.add(getDataSource().getFieldValue(i,
			...
			for (int z = 0; z < claves.size(); z++) {
				...
				// GROUP BY column
				value[indice] = (Value) claves.get(z);
				indice++;

				for (int i = 0; i < operations.size(); i++) {

					DataSource ds = getSelectedAndFilteredRows(claves.get(z),
							operations.get(i).getColumnName(), groupByField,
							getDataSource());
					...
					SelectionFieldIterator iterator = new SelectionFieldIterator(
							ds, selection, operations.get(i).getColumnName());
					...
					if (i == 0) {
						value[indice] = ValueFactory
								.createValue(numFilteredRows);
						indice++;
					}

					if (operations.get(i).isMin()) {
						BigDecimal min = statsCalculator.min();
						value[indice] = ValueFactory.createValueByType(min
								.toString(), operations.get(i).getColumnType());
						indice++;
					}
					if (operations.get(i).isMax()) {
						BigDecimal max = statsCalculator.max();
						value[indice] = ValueFactory.createValueByType(max
								.toString(), operations.get(i).getColumnType());
						indice++;
					}
					if (operations.get(i).isMean()) {
						BigDecimal mean = statsCalculator.mean();
						value[indice] = ValueFactory.createValueByType(
								mean.toString(), Types.DOUBLE);
						indice++;
					}
					if (operations.get(i).isSum()) {
						BigDecimal sum = statsCalculator.sum();
						value[indice] = ValueFactory.createValueByType(sum
								.toString(), operations.get(i).getColumnType());
						indice++;
					}
					if (operations.get(i).isSd()) {
						BigDecimal stdDeviation = statsCalculator
								.stdDeviation();
						value[indice] = ValueFactory.createValueByType(
								stdDeviation.toString(), Types.DOUBLE);
						indice++;
					}
					if (operations.get(i).isVar()) {
						BigDecimal variance = statsCalculator.variance();
						value[indice] = ValueFactory.createValueByType(
								variance.toString(), Types.DOUBLE);
						indice++;
					}
				}
	...
	public IWriter createDbfFile(File targetFile, String[] fieldNames,
			int[] fieldTypes) throws DBFExportException {
		FileDriver driver = null;
		try {
			driver = new DBFDriver();

			driver.createSource(targetFile.getAbsolutePath(), fieldNames,
					fieldTypes);
	 ...
	 */
	protected void writeRow(IWriter writer, Value[] values, int index)
			throws VisitorException {
		IFeature feat = new DefaultFeature(null, values,
	...
	/**
	 * Devuelve un iterador con las filas de la tabla que cumplen con la
	 * condicion
	 * 
	 * @param clave
	 * @param columnNumber
	 *            The number of the field whose statistics are to be calculated
	 * @param masterColumnName
	 *            The name of the field which will be used to define the groups
	 * @param originalDS
	 * @return
	 * @throws GroupingErrorException
	 */
	protected DataSource getSelectedAndFilteredRows(Object clave,
			String statsColumnName, String masterColumnName,
			SelectableDataSource originalDS) throws GroupingErrorException {

		try {
			int masterIndex = originalDS.getFieldIndexByName(masterColumnName);
			String fieldType = FieldDescription.typeToString(originalDS
					.getFieldType(masterIndex));
			DataSource ds;
			String query;
			if (fieldType.equals("Double")) {
				query = "select * from '" + originalDS.getName() + "' where (("
						+ masterColumnName + " - " + clave
						+ ") between -1 and 1);";
				...
				if (clave instanceof Boolean) {
					...
					bvalue = ((BooleanValue) clave).getValue() ? Boolean.TRUE
							: Boolean.FALSE;
				} else if (clave == null) {
					// nothing to do
				} else {
					throw new GroupingErrorException(
							"Illegal value for boolean: "
									+ clave.getClass().getName() + ": " + clave);
				}
				if (bvalue == null) {
					query = "select * from '" + originalDS.getName()
							+ "' where " + masterColumnName + " = " + clave
							+ ";";
				} else if (bvalue.booleanValue()) {
					query = "select * from '" + originalDS.getName()
							+ "' where " + masterColumnName + " = ( 1 = 1 );";
				} else {
					query = "select * from '" + originalDS.getName()
							+ "' where " + masterColumnName + " = ( 1 != 1 );";
				}
				...
				query = "select * from '" + originalDS.getName() + "' where "
						+ masterColumnName + " = '"
						+ clave.toString().replaceAll("'", "''") + "';";

			}
			logger.debug(query);
			return LayerFactory.getDataSourceFactory().executeSQL(query,
