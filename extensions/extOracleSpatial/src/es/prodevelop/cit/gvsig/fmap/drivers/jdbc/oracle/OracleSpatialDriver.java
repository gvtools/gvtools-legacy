...
/**
 * Vectorial driver to access Oracle databases geometries Should work on Oracle
 * Locator.
 * 
 * It contains switches to test different modules to perform the translation
 * oracle structs --> gvsig geometries:
 * 
 * - Parsing the structs directly. - Using Oracle's JGeometry static methods -
 ...
 * 
 */
public class OracleSpatialDriver extends DefaultJDBCDriver implements
		IDelayedDriver, ICanReproject, IWriteable {
	private static Logger logger = Logger.getLogger(OracleSpatialDriver.class
			.getName());
	...
	/**
	 * This method is called when the user creates a new oracle table from a
	 * vectorial layer
	 * 
	 * @param params
	 *            this array simply contains the parameters <tt>Connection</tt>
	 *            and <tt>DBLayerDefinition</tt>
	 */
	...
	public void setData(IConnection _conn, DBLayerDefinition lyrDef) {
		...
		// without problems:
		ConnectionWithParams _cwp = SingleDBConnectionManager.instance()
				.findConnection(conn);
		host = _cwp.getHost();
		port = _cwp.getPort();
		dbName = _cwp.getDb();
		...
		try {
			if (conn.isClosed()) {
				...
				_cwp = SingleDBConnectionManager.instance().getConnection(
						_cwp.getDrvName(), _cwp.getUser(), _cwp.getPw(),
						_cwp.getName(), _cwp.getHost(), _cwp.getPort(),
						_cwp.getDb(), _cwp.getSchema(), true);
			}
		...
		if ((workingAreaInViewsCS != null) && isTableHasSrid()
				&& (getEpsgSRID() != null)) {
			...
			if (workingAreaInViewsCS != null) {
				workingAreaInTablesCS = (Rectangle2D) workingAreaInViewsCS
						.clone();
				workingAreaInTablesCSStruct = shapeToStruct(
						workingAreaInTablesCS, FShape.NULL, isTableHasSrid(),
						false, true);
			}
		}
	...
	/**
	 * Utility method to load IDs in a different thred, so that gvsig's gui does
	 * not get blocked.
	 * 
	 */
	public void getMetaDataInThisThread() throws SQLException {
		...
		if (delay < ID_MIN_DELAY) {
			logger.info("Ids thread delayed by: " + (ID_MIN_DELAY - delay)
					+ " ms.");
			try {
		...
		if (!hasRealiableExtent) {
			full_Extent = getEstimatedExtent(getTableName(), geoColName, conn,
					20, 1.5, isGeogCS);
		}
	...
	private void loadSdoMetadata() {
		try {
			Statement _st = ((ConnectionJDBC) conn).getConnection()
					.createStatement();
			...
			if (_rs.next()) {
				...
				isGeogCS = OracleSpatialUtils.getIsGCS(oracleSRID,
						isTableHasSrid());
				...
				if (!hasRealiableExtent) {
					full_Extent = getFastEstimatedExtent(getTableName(),
							geoColName, conn, 20, 10, isGeogCS);
				}
	 ...
	 * @param use_gtools
	 *            switch to use geotools classes or not
	 * @return the geometry
	 * @throws SQLException
	 */
	...
	private IGeometry getFMapGeometry(STRUCT st, boolean complex, int rec_iter) {
		...
		try {
			...
			/*
			 * if (rec_iter > 4) { logger.error(lastGeom); return new
			 * FNullGeometry(); }
			 */

			int full_gtype = ((NUMBER) the_data[0]).intValue();
			// int jgtype = ((NUMBER) the_data[0]).intValue() % 1000;
			int fshape_type = OracleSpatialUtils.oracleGTypeToFShapeType(
					full_gtype, complex);
			...
			case FShape.MULTI:
				...
				if (collec_val == OracleSpatialUtils.COLLECTION_VALUE_MERGE_COLLECTION_IN_POLYGON) {
					ig = OracleSpatialUtils
							.mergePolygons((FGeometryCollection) ig);
				}
				break;
			...
			case FShape.LINE:
				ig = OracleSpatialUtils.getFMapGeometryMultiLineString(
						the_data, dim);

				break;

			case FShape.POLYGON:
				ig = OracleSpatialUtils.getFMapGeometryMultipolygon(the_data,
						dim);
		...
		if (d instanceof ARRAY) {
			...
			if (d instanceof NUMBER) {
				...
				try {
					...
					resp = "============= DATUM: " + n + " ; VALUE = "
							+ d.toString() + " (CAST TO INT FAILED)\n";
				}
			} else {
				resp = "============= DATUM: " + n + " ; VALUE = "
						+ d.toString() + " (NOT NUMBER)\n";
			}
		}
		return resp;
	...
	private void oneRowMetadata() {
		try {
			...
			this.setAdaptedFetchSize(OracleSpatialUtils
					.estimateGoodFetchSize(metaData));

			userName = ((ConnectionJDBC) conn).getConnection().getMetaData()
					.getUserName();

			// geoColInd = _rs.findColumn(geoColName);
			oneBasedGeoColInd = metaData.getColumnCount() + 1;

			DatabaseMetaData dbmd = ((ConnectionJDBC) conn).getConnection()
					.getMetaData();
			...
			getIdFieldNames();
			adjustLyrDef();
			_rs.close(); // st no debe cerrarse ya que las llamadas a metadata
							// lo encesitan
		} catch (SQLException se) {
	...
	/**
	 * Gets the feature iterator for a given SQL sentence (ignores previous
	 * filters and uses directly that sentence to query the table).
	 */
	...
	public IFeatureIterator getFeatureIterator(Rectangle2D r, String strEPSG)
		...
		STRUCT local_st = shapeToStruct(r, FShape.NULL, isTableHasSrid(),
				false, true);
	...
	/**
	 * This method reverts to the one without the fields specification. The
	 * fields have been selected from the start.
	 */
	public IFeatureIterator getFeatureIterator(Rectangle2D r, String strEPSG,
		...
		if ((alphaNumericFieldsNeeded == null)
				|| (alphaNumericFieldsNeeded.length == 0)) {
			...
			return new OracleSpatialFeatureIterator(this, localrs, _st,
					oneBasedGeoColInd, use_geotools, true,
					alphaNumericFieldsNeeded);
		}
	}
	...
	private String getSqlFor(String[] alphaNumericFieldsNeeded, String sdo_inter) {
		...
		String custom_sel = getCustomSelect(alphaNumericFieldsNeeded,
				sdo_inter, idswhere);
		return custom_sel;
	}
	 ...
	 * This should be a rare way to get the geometries. The standard way is by
	 * using the iterators.
	 */
	public IGeometry getShape(int _ind) throws ReadDriverException {
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) conn)
					.getConnection().prepareStatement(_sql);
			ps.setObject(1, r_id);
	...
	private void setIdRowTable(boolean only_test) throws SQLException {
		...
		try {
			...
			/*
			 * boolean needsCollectionLayer =
			 * OracleSpatialUtils.hasSeveralGeometryTypes(types, false);
			 * 
			 * if (needsCollectionLayer) { shapeType = FShape.MULTI; }
			 */
		} catch (SQLException e) {
	...
	public int getFieldType(int idField) throws ReadDriverException {
		...
		try {
			...
			// we must add this entry because we did not remove the 'geometry'
			// column
			...
			if ((__type == Types.INTEGER) || (__type == Types.SMALLINT)
					|| (__type == Types.TINYINT) || (__type == Types.BIGINT)
	...
	public Value getFieldValue(long rowIndex, int field_Id)
		...
		try {
			...
			if (!_r.next()) {
				_r.close();
				ps.close();
				throw new SQLException("No row for ROWID: " + _id.toString()
						+ ". Possibly deleted from another app.");
			}

		} catch (SQLException se) {
			logger.error("While getting row " + rowIndex + " : "
					+ se.getMessage());
			return ValueFactory.createNullValue();
		}
	...
	private IGeometry getFMapGeometryCollection(Datum[] the_data, int dim,
		...
		try {
			...
			ords_of_groups = OracleSpatialUtils.getOrdOfGroups(all_ords,
					elems_info_aray);
		...
		for (int i = 0; i < elems_info_aray.length; i++) {
			_elems_info_aray[i] = OracleSpatialUtils
	...
	private String getMainSelect(String viewsdo, String idsLoadWhere) {
		...
		if (isGeogCS) {
			String vport = "sdo_filter(" + geoColName
					+ ", SDO_CS.VIEWPORT_TRANSFORM(" + viewsdo + ", "
					+ oracleSRID + "), 'querytype=window') = 'TRUE'";
	...
	private String getCustomSelect(String[] atts, String viewsdo,
			String idsLoadWhere) {
		String resp = "";
		...
		if (isGeogCS) {
			...
			resp = "select " + atts_enum + " from " + getTableName()
					+ " c where ";
			if (idsLoadWhere.length() > 0) {
				resp = resp + " (" + idsLoadWhere + ") AND ";
			}
	...
	private int oracleTypeToFShapeTypeExceptPointTypes(int type) {
		 ...
		 * 
		 * GTYPE_COLLECTION collection geometry type GTYPE_CURVE curve geoemtry
		 * type GTYPE_MULTICURVE multi-curve geometry type GTYPE_MULTIPOINT
		 * multi-point geometry type GTYPE_MULTIPOLYGON multi-polygon geometry
		 * type GTYPE_POINT point geometry type GTYPE_POLYGON polygon geometry
		 * type
		 * 
		 * Tipos gvSIG FShape
		 * 
		 * NULL = 0; POINT = 1; LINE = 2; POLYGON = 4; TEXT = 8; MULTI = 16;
		 * MULTIPOINT = 32; CIRCLE = 64; ARC = 128; ELLIPSE=256; Z=512
		 */
		switch (type) {
	...
	private String getValidViewConstructor(STRUCT _st, String ora_srid,
			boolean _hassrid, boolean _isgeocs) {
	...
	private Object[] getViewResultSet(STRUCT geoStruct, String fixsql,
		...
		try {
			_stmnt = ((ConnectionJDBC) conn).getConnection().createStatement(
					ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
			...
			logger.error("Tablename: " + getTableName()
					+ ". Error while getting main cursor: " + se.getMessage(),
					se);
			throw new ReadDriverException(getName(), se);
		}

		// this method returns the statement too, so that it can be closed
		// afterwards
		_resp[0] = _rs;
		_resp[1] = _stmnt;
	...
	private String getSdoConstructor(STRUCT geoStruct, boolean hasSrid,
		...
		try {
			...
			Datum[] vertices = ((ARRAY) geoStruct.getOracleAttributes()[4])
					.getOracleArray();
			...
			logger.error("While getting sdo contructor: " + ex.getMessage());
		}
	...
	private String getIdsQueryWhereClause(boolean with_where) {
		...
		if (workingAreaInTablesCSStruct == null) {
			...
			if (emptyWhereClause) {
				resp = resp + _where + "(sdo_relate(" + geoColName + ", "
						+ waqry
						+ ", 'mask=anyinteract querytype=window') = 'TRUE')";
			} else {
				resp = resp + _where + "((" + getWhereClauseWithoutWhere()
	...
	public static String getTableCreationSql(DBLayerDefinition dbLayerDef) {
		...
		String pk = "CONSTRAINT " + getDerivedNAme(table_name, "PK")
				+ " PRIMARY KEY (\""
				+ OracleSpatialDriver.DEFAULT_ID_FIELD_CASE_SENSITIVE
				+ "\") ENABLE";
	...
	public static String getIndexCreationSql(DBLayerDefinition dbLayerDef) {
		String resp = "CREATE INDEX "
				+ getDerivedNAme(dbLayerDef.getTableName(), "SX") + " ON "
	 ...
	 */
	public static String getMetadataUpdateSql(String schema, String tName,
		...
		String resp = "INSERT INTO " + USER_ORACLE_GEOMETADATA_VIEW + " "
				...
				+ dim_name[0] + "', " + minx + ", " + maxx + ", "
				+ OracleSpatialUtils.ORACLE_GEOM_METADATA_TOLERANCE + " ), "
	...
	public static String getRowInsertSql(IFeature feat,
		...
		for (int i = 0; i < fieldsDescr.length; i++) {
			...
			if (!isOracleAllowedFieldname(name))
				continue;
			if (!isUserEditableType(ftype, name, _geoColName))
				continue;
			// ------------------------------------------------------
		...
		resp = resp + " " + geo_val + " )";
		/*
		 * String test = "SDO_UTIL.APPEND(SDO_GEOMETRY(" + "2002, NULL, NULL," +
	...
	private static boolean isUserEditableType(int ftype, String item_name,
			String geo_name) {
		...
		if ((ftype == Types.BINARY) || (ftype == Types.ARRAY)
				|| (ftype == Types.BLOB) || (ftype == Types.CLOB)
				|| (ftype == Types.STRUCT)) {
			return false;
		}
		return true;
		...
		for (int i = 0; i < _fieldsDescr.length; i++) {
			...
			if (isStructAndNotGeoField(_fieldsDescr[i].getFieldType(), name,
				...
				logger.info("Field: " + name
						+ " will not be updated (it's a struct).");
				continue;
			}
	 ...
	 *            layer definition
	 * @param id
	 *            ROWID of the record to be deleted
	...
	/**
	 * Utility function to translate a SRS code from EPSG to Oracle. Uses a
	 * datasource based on a DBF file.
	 * 
	 * @param epsg
	 *            the EPSG code
	...
	public static String epsgSridToOracleSrid(String _epsg) throws Exception {
		...
		try {
			...
			if (ds.getRowCount() > 1) {
				// logger.error("===============");
				logger.warn("DBF file is wrong?: More than one preferred Oracle Spatial code found for EPSG code: "
						+ epsg);
	...
	/**
	 * Utility function to translate a SRS code from Oracle to EPSG. Uses a
	 * datasource based on a DBF file.
	 * 
	 * @param ora
	 *            the Oracle code
	...
	public static String oracleSridToEpsgSrid(String ora) throws Exception {
		...
		try {
			...
			if (ds.getRowCount() > 1) {
				// logger.error("===============");
				logger.warn("DBF file is wrong?: More than one EPSG code found for Oracle Spatial code: "
						+ ora);
	...
	/**
	 * This methos creates the datasource used to translate the SRS codes: EPSG
	 * <--> Oracle.
	 * 
	 * It's called from several places, so checks that the datasource does not
	 * exist.
	 */
	...
	/**
	 * Utility function. Says whether a given field name can be a user field
	 * name or not (for example, "ROWID" is not a valid one because it's a
	...
	private Rectangle2D getBoundingFromSample(int n_max) {
		...
		try {
			...
			logger.error("While getting sample full extent: " + se.getMessage());
		}
	...
	/**
	 * Does what it says, puts the LinearRing in counter clock wise order.
	 * 
	 * @param ls
	 *            The ring to set.
	 * @param gf
	...
	/**
	 * Does what it says, reverses the order of the Coordinates in the
	 * linestring.
	 * 
	 * @param ls
	 *            The ls to reverse.
	...
	public static STRUCT iGeometryToSTRUCT(IGeometry ig, int _forced_type,
			IConnection _conn, String _o_srid, boolean withSrid,
	...
	public static STRUCT shapeToStruct(Shape shp, int forced_type,
			IConnection _conn, String o_srid, boolean hasSrid,
			boolean agu_bien, boolean isView, boolean _isGeoCS) {
		int _srid = -1;
		...
		if (shp instanceof Rectangle2D) {
			return OracleSpatialUtils.rectangleToStruct((Rectangle2D) shp,
	...
	public String[] getTableNames(IConnection conn, String catalog)
		...
		try {
			...
			ResultSet rs = null;
			rs = getTableNamesFromTable(dbmd.getTables(catalog,
					ORACLE_GEO_SCHEMA, ALL_ORACLE_GEOMETADATA_VIEW, types),
					((ConnectionJDBC) conn).getConnection());
			...
			while (rs.next()) {
				String nomCompleto = rs.getString("OWNER") + "."
	...
	private void writeMetaTableToLog(Connection con, String tname) {
		...
		logger.debug("======================================================");
		logger.debug("=     " + ALL_ORACLE_GEOMETADATA_VIEW
				+ "  (1 EVERY 10 TABLES) ========");
		logger.debug("======================================================");
	...
	public String[] getIdFieldsCandidates(IConnection conn, String table_name)
			throws DBException {
		try {
			String rowid_avail_test = "SELECT ROWID FROM " + table_name
					+ " WHERE ROWNUM = 1";
			Statement _st = ((ConnectionJDBC) conn).getConnection()
					.createStatement();
	...
	/**
	 * Gets the field names that can act as geometry fields (queries the user's
	 * geographic metadata).
	 */
	...
	private String[] checkIndexes(IConnection c, String[] all,
			String long_table_name) throws DBException {
		...
		try {
			...
			for (int i = 0; i < all.length; i++) {
				...
				String qry = "SELECT SRID, DIMINFO FROM "
						+ ALL_ORACLE_GEOMETADATA_VIEW + " WHERE TABLE_NAME = "
						+ "'" + t.toUpperCase() + "' AND COLUMN_NAME = '"
						+ all[i].toUpperCase() + "'";

				Statement _st = ((ConnectionJDBC) c).getConnection()
						.createStatement();
				ResultSet _rs = _st.executeQuery(qry);
				if (_rs.next()) {
					...
					if (allowsGeoQueries(((ConnectionJDBC) c).getConnection(),
							long_table_name, all[i], _srid, len)) {
						good_ones.add(all[i]);
					}
				}
				_rs.close();
	...
	private boolean allowsGeoQueries(Connection c, String _t, String gf,
			String _srid, int dims) {
		String p = getPointConstructor(dims, _srid);
		String qry = "";
		qry = "SELECT * FROM " + _t.toUpperCase() + " WHERE SDO_RELATE(" + "\""
				+ gf + "\", " + p + ", 'mask=TOUCH') = 'TRUE'"
				+ " AND ROWNUM = 1";
	...
	private String getPointConstructor(int dims, String _srid) {
		...
		return "MDSYS.SDO_GEOMETRY("
				...
				+ _srid
				+ ", NULL, "
				+ "MDSYS.SDO_ELEM_INFO_ARRAY(1, 1, 1), MDSYS.SDO_ORDINATE_ARRAY("
				+ coord + "))";
	}
		...
		try {
			...
			for (int i = 0; i < ret.length; i++) {
				if (rsmd.getColumnType(i + 1) == Types.NUMERIC) {
					...
					if (scale >= 0) {
						String prec_dec = " (" + rsmd.getPrecision(i + 1)
								+ ", " + scale + ")";
	...
	/**
	 * Allows the method to decide what to do with the ID field name (remove/add
	 * it from the user selected fields).
	 * 
	 * @param flds
	 * @param idf
	 * @return the possibly modified field names
	...
	/**
	 * This method is called when the user removes the layer from the view. If
	 * the IDs were being loaded, the driver will check this field and will let
	 * the thread 'die' quietly.
	 * 
	 */
	public void remove() {
		cancelIDLoad = true;
	...
	private Rectangle2D getFastEstimatedExtent(String tname, String gfield,
			IConnection c, int sample_size, double enlargement, boolean is_geo) {

		Rectangle2D world = new Rectangle2D.Double(-180, -90, 360, 180);
		Rectangle2D resp_aux = null;
		String qry = "SELECT " + gfield + " FROM " + tname
				+ " WHERE ROWNUM <= " + sample_size;
		ResultSet _rs = null;

		try {
			PreparedStatement _st = ((ConnectionJDBC) c).getConnection()
	...
	private Rectangle2D getEstimatedExtent(String tname, String gfield,
			IConnection c, int sample_size, double enlargement, boolean is_geo) {
	...
	private void setLyrDefFieldTypes(int[] tt) {
		...
		if (sz_tt != sz_fd) {
			logger.error("Field count does not match. lyrDef has " + sz_fd
					+ " fields," + " but this method was called with " + sz_tt
					+ " items (?)");
			sz = Math.min(sz_fd, sz_tt);
		}

		for (int i = 0; i < sz; i++)
			lyrDef.getFieldsDesc()[i].setFieldType(tt[i]);
	}
					...
					if (obj instanceof STRUCT) {
						...
						if (obj instanceof TIMESTAMP) {
							...
							try {
								...
								return ValueFactory.createValueByType(
										objToString, _type);
							} catch (Exception ex) {
	...
	public Value[] getAttributes(ResultSet rs, boolean use_main_metadata) {
		...
		try {
			...
			logger.error("While getting resultset attribute values: "
					+ se.getMessage());
			res = new Value[fcount];
			for (int i = 0; i < fcount; i++)
