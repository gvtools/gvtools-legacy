...
public class OracleSpatialUtils {
	private static Logger logger = Logger.getLogger(OracleSpatialUtils.class
			.getName());
	...
	public static STRUCT fShapeToSTRUCT(Object fshp, IConnection c, int srid,
			boolean agu_b, boolean hasSrid, boolean is_geodet,
			int forced_shptype) throws SQLException {
		...
		if (fshp instanceof FPoint2D) { // point 2/3d
			...
			if (fshp instanceof FPolygon2D) { // polygon 2/3d
				...
				if (forced_shptype % FShape.Z == FShape.POLYGON) {
					...
					resp = multiLineToMultiPolygonStruct((FShape) fshp, srid,
							three, c, agu_b, hasSrid, is_geodet);
	...
	private static STRUCT multiPoint2DToStruct(FMultiPoint2D mp2d,
		...
		StructDescriptor dsc = StructDescriptor.createDescriptor(
				"MDSYS.SDO_GEOMETRY", ((ConnectionJDBC) c).getConnection());
	...
	private static STRUCT getCircleAsStruct(FCircle2D fcirc, int srid,
		...
		Coordinate[] three_points = getThreePointsOfCircumference(
				fcirc.getCenter(), fcirc.getRadio());
		...
		STRUCT resp;
		StructDescriptor dsc = StructDescriptor.createDescriptor(
				"MDSYS.SDO_GEOMETRY", ((ConnectionJDBC) _conn).getConnection());
		Object[] obj = new Object[5];
	...
	private static ArrayList getPolygonsEasily(FShape mpolygon, boolean isgeo) {
		...
		if (arrayCoords != null) {
			...
			try {
				...
				if (threed) {
					z = getZ((FPolygon3D) mpolygon, start_ind, end_ind,
							new_size);
				}
	...
	private static STRUCT getMultiPointAsStruct(Coordinate pnt, int srid,
		...
		STRUCT ords_st = new STRUCT(ord_dsc,
				((ConnectionJDBC) _conn).getConnection(), ords);
	...
	private static STRUCT getMultiPolygonAsStruct(FShape mpol, int srid,
			boolean threed, IConnection _conn, boolean agu_bien,
			boolean hasSrid, boolean isgeo) throws SQLException {
	...
	private static STRUCT multiLineToMultiPolygonStruct(FShape mpol, int srid,
		...
		for (int i = 0; i < n_sh; i++)
			no_holes.add(new ArrayList());

		return getMultiPolygonAsStruct(all_ls, no_holes, srid, threed, _conn,
				agu_bien, hasSrid);
	}
	...
	public static STRUCT appendGeometriesInStruct(FGeometryCollection co,
			int _forced_type, IConnection _conn, String _o_srid,
	...
	private static STRUCT appendStructs(STRUCT st1, STRUCT st2,
			IConnection _conn) {
		try {
			...
			STRUCT resp = new STRUCT(dsc,
					((ConnectionJDBC) _conn).getConnection(), atts);
	...
	public static ARRAY getDevelopedOrdsArray(ARRAY ords, int dim) {
		...
		try {
			...
			for (int i = 0; i < 5; i++) {
				resp[i * 2] = ((i == 2) || (i == 3)) ? corners[dim]
						: corners[0];
			}
			for (int i = 0; i < 5; i++) {
				resp[1 + i * 2] = ((i == 1) || (i == 2)) ? corners[dim + 1]
						: corners[1];
			}
	...
	private static int getLineToType(Datum[] infos, int i) {
		...
		try {
			if ((((NUMBER) infos[(3 * i) + 2]).intValue() == 2)
				...
				if ((((NUMBER) infos[(3 * i) + 1]).intValue() == 1003)
	...
	private static boolean addOrdsToGPX(ExtendedGeneralPathX gpx,
			int zero_based_start, int zero_based_include_end, Datum[] ords,
			int d, int ltype, boolean do_the_move, boolean must_do_first) {
	...
	private static void arcTo(GeneralPathX gpx, double x, double y, double x2,
			double y2, int ints) {
		...
		for (int i = 0; i < ints; i++) {
			gpx.lineTo(cent.getX() + r * Math.cos(angle_ini + i * anginc),
	...
	public static STRUCT reprojectGeometry(IConnection conn, STRUCT fromStruct,
			String toSrid) {
		...
		try {
			PreparedStatement _st = ((ConnectionJDBC) conn).getConnection()
	...
	public static void removeStructFields(DBLayerDefinition def, String[] arr) {
		...
		FieldDescription[] flds_new = (FieldDescription[]) aux
	...
	public static void setUpperCase(DBLayerDefinition def) {
		...
		if (aux != null)
			def.setCatalogName(aux.toUpperCase());

		aux = def.getSchema();
		if (aux != null)
			def.setSchema(aux.toUpperCase());
	}
	...
	public static IGeometry getFMapGeometryMultipolygon(Datum[] the_data,
			int dim) {
		IGeometry ig = null;
	...
	public static IGeometry getFMapGeometryMultiLineString(Datum[] the_data,
			int dim) {
		ExtendedGeneralPathX gpx = OracleSpatialUtils.structToGPX(the_data);
		IGeometry ig = null;
		double[] ords = null;
	...
	private static IGeometry getFMapGeometrySdoPoint(Datum[] the_data, int d) {
		...
		try {
			...
			logger.error(
					"While getting sdo point ordinates: " + se.getMessage(), se);
		}
	...
	public static int isCollection(Datum[] the_data) {
		...
		for (int i = 2; i < size; i++) {
			...
			if ((_item_mod != second) && (!((_item == 2003) && (second == 2)))
		...
		try {
			while (i < size) {
				...
				if (newaux[0].intValue() == aux[0].intValue()) {
					...
					/*
					 * if (((NUMBER) newaux[1]).intValue() == 1003 || ((NUMBER)
					 * newaux[1]).intValue() == 2003) { resp.add(aux); aux =
					 * getNthGroupOfThree(all_elem, i); } else { aux =
					 * appendDatArrays(aux, newaux); }
					 */
				}
	...
	public static STRUCT rectangleToStruct(Rectangle2D r, boolean hasSrid,
		...
		if ((_isGeogCS) && (isView)) {
			// absurd cases for geodetic (user chose wrong epsg when loading the
			// table)
			if (r.getMinX() >= 180) {
		...
		try {
			...
			// StructDescriptor.createDescriptor("STRUCT", conn);
			StructDescriptor dsc = StructDescriptor.createDescriptor(
					"MDSYS.SDO_GEOMETRY",
					((ConnectionJDBC) __conn).getConnection());

			resp = new STRUCT(dsc, ((ConnectionJDBC) __conn).getConnection(),
					new_obj);
		} catch (Exception ex) {
			logger.error(
