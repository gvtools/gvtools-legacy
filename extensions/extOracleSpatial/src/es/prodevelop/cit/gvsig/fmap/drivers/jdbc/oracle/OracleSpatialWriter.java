...
/**
 * Oracle Spatial geometry writer. Used during editing and when a vectorial
 * layer has to be exported to Oracle.
 * 
 * @author jldominguez
 * 
 */
public class OracleSpatialWriter extends AbstractWriter implements
		ISpatialWriter, IFieldManager {

	public static final String NAME = "Oracle Spatial Writer";
	private static Logger logger = Logger.getLogger(OracleSpatialWriter.class
			.getName());
	 ...
	 * 
	 * @param rowcount
	 *            the table's current row countt
	 */
	...
	public void initialize(ITableDefinition lyrD)
		...
		String[] forbidden = { dbdef.getFieldID(), dbdef.getFieldGeometry(),
				OracleSpatialDriver.ORACLE_ID_FIELD };

		ConnectionJDBC _conn = (ConnectionJDBC) dbdef.getConnection();
		fieldManager = new OracleFieldManager(_conn.getConnection(),
	...
	public static boolean createEmptyTable(DBLayerDefinition lyr_def,
			IConnection the_conn, Rectangle2D rect) throws SQLException {
		...
		try {
			...
			logger.error("Unknown EPSG code: " + srid_epsg
					+ ", using it as oracle code in empty table");
			// with_srid = false;
			_ora_srid = srid_epsg;
		}
		...
		String _sql_table_exists = OracleSpatialDriver
				.getTableExistsSql(lyr_def);

		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_table_exists);
			ps.execute();
			ps.close();
		...
		if (table_exists) {
			throw new SQLException("Table " + lyr_def.getTableName()
					+ " already exists. Creation cancelled.");
		}

		String _sql_rem_meta = OracleSpatialDriver
				.getRemoveMetadataSql(lyr_def);
		...
		String _sql_meta = OracleSpatialDriver.getMetadataUpdateSql(
				lyr_def.getUser(), lyr_def.getTableName(), _ora_srid, rect, 2,
				with_srid);
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_drop);
			ps.execute();
			ps.close();
		} catch (SQLException ex) {
			logger.info("Table did not exist: " + lyr_def.getTableName());
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_creation);
			ps.execute();
			ps.close();
		} catch (SQLException ex) {
			logger.error(
					"Error while executing SQL for table creation: "
							+ ex.getMessage(), ex);
			return false;
		}
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_rem_meta);
			ps.execute();
			ps.close();
		} catch (SQLException ex) {
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_meta);
			ps.execute();
			ps.close();
		} catch (SQLException ex) {
			logger.error("Error while executing SQL for metadata insertion: "
					+ ex.getMessage(), ex);
			return false;
		}
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) the_conn)
					.getConnection().prepareStatement(_sql_index);
			ps.execute();
			ps.close();
		} catch (SQLException ex) {
			logger.error(
					"Error while executing SQL for index creation: "
							+ ex.getMessage(), ex);
			return false;
		}
	...
	public void preProcess() throws StartWriterVisitorException {
		...
		if (tableCreation) {
			...
			try {
				oracleSRID = OracleSpatialDriver
						.epsgSridToOracleSrid(srid_epsg);
			} catch (Exception e1) {
				// not found
				oracleSRID = srid_epsg;
				logger.warn("unknown EPSG code: " + srid_epsg
						+ ", using it as ora code");
				// storeWithSrid = false;
			}

			String _sql_rem_meta = OracleSpatialDriver
					.getRemoveMetadataSql((DBLayerDefinition) tableDef);
			String _sql_drop = OracleSpatialDriver
					.getDropTableSql((DBLayerDefinition) tableDef);
			String _sql_creation = OracleSpatialDriver
					.getTableCreationSql((DBLayerDefinition) tableDef);
			String _sql_index = OracleSpatialDriver
					.getIndexCreationSql((DBLayerDefinition) tableDef);

			int dim_aux = dimensions;

			String _sql_meta = OracleSpatialDriver.getMetadataUpdateSql(
					driver.getUserName(),
					((DBLayerDefinition) tableDef).getTableName(), oracleSRID,
					bbox, dim_aux, storeWithSrid);
			...
			try {
				...
				logger.error("Error while executing SQL for table creation: "
						+ ex.getMessage(), ex);
				throw new StartWriterVisitorException(
						"Oracle layer for table: " + tableDef.getName(), ex);
			}
			...
			try {
				...
				logger.error("While altering table: "
						+ getTableDefinition().getName());
				throw new StartWriterVisitorException("TABLE: "
						+ getTableDefinition().getName(), e);
			}
		}
		...
		try {
			...
			logger.error("Error while setting auto commit FALSE: "
					+ e.getMessage());
			throw new StartWriterVisitorException("Oracle layer for table: "
					+ tableDef.getName(), e);
		}
	...
	public void process(IRowEdited _row) throws VisitorException {
		...
		case IRowEdited.STATUS_ADDED:
			...
			/*
			 * // TODO when addRowInCreation() is implemented: if
			 * (tableCreation) { addRowInCreation(_row); } else { addRow(_row);
			 * }
			 */
			break;
	...
	private void addRow(IRowEdited irow) throws VisitorException {
		...
		try {
			...
			// STRUCT st = driver.shapeToStruct(shp, lyrShapeType, conn,
			// oracleSRID, false)
			STRUCT st;

			if ((oracleSRID == null)
					|| !differentSRS(driver.getDestProjectionOracleCode(),
							oracleSRID)
			// (driver.getDestProjectionOracleCode().compareToIgnoreCase(oracleSRID)
			// == 0)
			) {

				st = OracleSpatialDriver.iGeometryToSTRUCT(_ig, lyrShapeType,
						conn, oracleSRID, storeWithSrid, aguBien, isGeoCS);
			} else {
				String viewSrid = driver.getDestProjectionOracleCode();
				boolean isViewSridGedetic = driver.getIsDestProjectionGeog();
				st = OracleSpatialDriver.iGeometryToSTRUCT(_ig, lyrShapeType,
						conn, viewSrid, storeWithSrid, aguBien,
						isViewSridGedetic);
			...
			if (st == null) {
				...
				ps = ((ConnectionJDBC) conn).getConnection().prepareStatement(
						_sql_insert);
			} else {
				_sql_insert = OracleSpatialDriver
						.getRowInsertSql(ifeat, (DBLayerDefinition) tableDef,
								rowIndex, geoColName, "?");
				ps = ((ConnectionJDBC) conn).getConnection().prepareStatement(
						_sql_insert);
				ps.setObject(1, st);
			...
			logger.error(
					"Error while executing SQL for row insertion: "
							+ ex.getMessage() + " SQL: " + _sql_insert, ex);
			throw new VisitorException("Oracle layer for table: "
					+ tableDef.getName(), ex);
		}
	}
	...
	private boolean differentSRS(String srsa, String srsb) {
		...
		if (srsa == null || srsb == null || srsa.length() == 0
				|| srsb.length() == 0) {
			return false;
		} else {
			if (srsa.compareToIgnoreCase(srsb) == 0)
				return false;
			String aux = "";
			try {
	...
	private void deleteRow(IRowEdited irow) throws VisitorException {
		...
		String _sql_delete = OracleSpatialDriver.getRowDeleteSql(
				(DBLayerDefinition) tableDef, id);

		IConnection conn = ((DBLayerDefinition) tableDef).getConnection();

		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) conn)
					.getConnection().prepareStatement(_sql_delete);
			ps.execute();
			ps.close();
			...
			logger.error(
					"Error while executing SQL for row deletion: "
							+ ex.getMessage(), ex);
			throw new VisitorException("Oracle layer for table: "
					+ tableDef.getName(), ex);
		}
	...
	private void updateRow(IRowEdited irow) throws VisitorException {
		...
		try {
			java.sql.PreparedStatement ps = ((ConnectionJDBC) conn)
					.getConnection().prepareStatement(_sql_update);
			...
			logger.error(
					"Error while executing SQL for row update: "
							+ ex.getMessage(), ex);
			throw new VisitorException("Oracle layer for table: "
					+ tableDef.getName(), ex);
		}
	...
	public void postProcess() throws StopWriterVisitorException {
		if (tableCreation) {
			...
			try {
				...
				logger.error("!!! While performing table creation MAIN COMMIT: "
						+ e.getMessage());
				throw new StopWriterVisitorException("Oracle layer for table: "
						+ tableDef.getName(), e);
			}
		}
	}
