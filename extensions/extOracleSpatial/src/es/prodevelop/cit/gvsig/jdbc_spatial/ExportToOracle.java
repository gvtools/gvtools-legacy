...
public class ExportToOracle {
		...
		try {
			...
			while (!valid_name) {
				tableName = JOptionPane.showInputDialog(
						...
						PluginServices.getText(this, "intro_tablename"));
				valid_name = ((tableName == null) || ((tableName.length() <= (OracleSpatialDriver.MAX_ID_LENGTH - 3))
				...
				if (!valid_name) {
					if (tableName.length() > (OracleSpatialDriver.MAX_ID_LENGTH - 3)) {
						JOptionPane.showMessageDialog(null, PluginServices
								.getText(this, "nombre_demasiado_largo"),
								PluginServices.getText(this, "error"),
								JOptionPane.ERROR_MESSAGE);
					} else {
						JOptionPane.showMessageDialog(null, PluginServices
								.getText(this, "nombre_no_valido"),
								PluginServices.getText(this, "error"),
								JOptionPane.ERROR_MESSAGE);
					}
				}
			...
			ConnectionChooserPanel dlg = new ConnectionChooserPanel(
			...
			if (!bFound) {
				...
				newFields[numFieldsAnt]
						.setFieldName(OracleSpatialDriver.DEFAULT_ID_FIELD_CASE_SENSITIVE);
				dbLayerDef.setFieldsDesc(newFields);
			}

			// addStartIfNotPresent(dbLayerDef,
			// OracleSpatialDriver.ORACLE_ID_FIELD);
			// addEndIfNotPresent(dbLayerDef,
			...
			String strSRID = ProjectionUtils.getAbrev(layer.getCrs())
					.substring(5);
			strSRID = ProjectionUtils.getAbrev(mapContext.getCrs())
					.substring(5);
			dbLayerDef.setSRID_EPSG(strSRID);
			dbLayerDef.setConnection(conex);

			OracleSpatialWriter writer = (OracleSpatialWriter) LayerFactory
					.getWM().getWriter("Oracle Spatial Writer");
			...
			/*
			 * PostProcessSupport.clearList(); Object[] p = new Object[1]; p[0]
			 * = params; PostProcessSupport.addToPostProcess(oDriver, "setData",
			 * p, 1);
			 */
	...
	private void writeFeatures(MapContext mapContext, FLyrVect layer,
		...
		PluginServices.cancelableBackgroundExecution(new OracleWriteTask(
				mapContext, layer, (OracleSpatialWriter) writer, reader,
				setDataParams));
	}
