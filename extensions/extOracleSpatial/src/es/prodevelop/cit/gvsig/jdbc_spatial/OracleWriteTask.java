...
public class OracleWriteTask extends AbstractMonitorableTask {
	private static Logger logger = Logger.getLogger(OracleWriteTask.class
			.getName());
	FLyrVect lyrVect;
	...
	public OracleWriteTask(MapContext mapc, FLyrVect lyr,
		...
		try {
			...
			if (shp instanceof FShape3D)
				dims = 3;

		} catch (Exception ex) {
			logger.error("While getting dim of sample geometry: "
					+ ex.getMessage());
		}
		...
		if (bitSet.cardinality() == 0) {
			...
			for (int i = 0; i < rowCount; i++) {
				...
				setNote(PluginServices.getText(this, "exporting_") + " "
						+ (i + 1) + " "
						+ PluginServices.getText(this, "geometries"));
