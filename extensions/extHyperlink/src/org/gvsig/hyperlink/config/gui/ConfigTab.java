...
public class ConfigTab extends AbstractThemeManagerPage implements IWindow,
	...
	private String[] getActionNames() {
		if (actionNames == null) {
			ExtensionPoint actions = (ExtensionPoint) ExtensionPointsSingleton
					.getInstance().get("HyperLinkAction");
			String[] actionArray = new String[actions.size()];
			Iterator actionsIter = actions.values().iterator();
			int i = 0;
			while (actionsIter.hasNext()) {
				ILinkActionManager action = (ILinkActionManager) actionsIter
						.next();
	...
	private String[] getCandidateFields() {
		...
		try {
			...
			PluginServices.getLogger().warn(
					"Hyperlink: error getting candidate fields", e);
	...
	public void applyAction() {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			config.addLink(getActionCodes()[row.getSelectedAction()],
					row.getSelectedField(), row.getExtension());
		}
	...
	protected JButton getAddActionButton() {
		if (jbt_addAction == null) {
			jbt_addAction = new JButton(PluginServices.getText(this,
					"Add_action"));
	...
	protected JButton getRemoveActionButton() {
		if (jbt_removeAction == null) {
			jbt_removeAction = new JButton(PluginServices.getText(this,
					"Remove_action"));
	...
	public WindowInfo getWindowInfo() {
		if (_windowInfo == null) {
			_windowInfo = new WindowInfo(WindowInfo.ICONIFIABLE
	...
	protected JButton getAcceptButton() {
		if (jbt_accept == null) {
			jbt_accept = new org.gvsig.gui.beans.swing.JButton(
	...
	protected JButton getCancelButton() {
		if (jbt_cancel == null) {
			jbt_cancel = new org.gvsig.gui.beans.swing.JButton(
	...
	protected JCheckBox getEnabledCheckBox() {
		if (jcb_enabled == null) {
			jcb_enabled = new JCheckBox(PluginServices.getText(this,
