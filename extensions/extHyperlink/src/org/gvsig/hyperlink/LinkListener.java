...
public class LinkListener implements PointListener {
	...
	public LinkListener(MapControl mc, ManagerRegistry registry) {
		...
		formatManagers = (ExtensionPoint) ExtensionPointsSingleton.getInstance().get(LinkControls.ACTIONSEXTENSIONPOINT);
		initialize();
	}
	
	private void initialize() {
		URL url = PluginServices.getPluginServices(this).getClassLoader().getResource("images/LinkCursor.gif");
		if (url!=null) {
			ImageIcon img = new ImageIcon(url);
			cur = Toolkit.getDefaultToolkit().createCustomCursor(img.getImage(), new Point(16, 16), "Hyperlink");
		}
		else {
		...
		for (int layerCount = 0; layerCount < sel.length; layerCount++) {
			...
			try {
				...
				if (theLayer.getProperty(LinkControls.LAYERPROPERTYNAME)!=null &&
					...
					LayerLinkConfig layerLink = (LayerLinkConfig) theLayer.getProperty(LinkControls.LAYERPROPERTYNAME);
					if (layerLink.isEnabled() &&
							layerLink.linkCount()>0) { // there is some link configured for this layer
						...
						for (int actionCount=0; actionCount<layerLink.linkCount(); actionCount++) {
							...
							for (int i=0; i<links.length; i++) {
								if (links[i]!=null) {
									actionManager = (ILinkActionManager) formatManagers.get(linkConfig.getActionCode());
									...
									else { // delegate in the format manager to show the file
