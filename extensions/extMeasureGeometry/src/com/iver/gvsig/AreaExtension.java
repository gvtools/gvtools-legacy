...
/**
 * Extension responsible for calculating the area and the perimeter of the
 * geometries selected, only when these they be of type polygon. When there is
 * not an active layer, or some of the geometries selected be not of type
 * polygon this tool will remain in not visible way.
 * 
			...
			if (layersActives.length == 1) {
				if (layersActives[0] instanceof FLyrVect) {
					...
					try {
						...
						if (FShape.CIRCLE == type || FShape.ELLIPSE == type
								|| FShape.POLYGON == type) {
							return true;
						}
					} catch (ReadDriverException e) {
