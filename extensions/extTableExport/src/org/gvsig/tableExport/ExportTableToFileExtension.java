...
/**
 * Extensión que permite exportar una tabla a cualquier driver de fichero que
 * soporte escritura.
 * 
 * @author jmvivo
 */
public class ExportTableToFileExtension extends Extension {
	...
	/**
	 * @param actionCommand
	 *            Debe de estar compuesto por tres parámentros separados por '|'
	 *            dónde el primero es el nombre del driver, el segundo la
	 *            extensión de los ficheros y el tercero el nombre del tipo.
	 *            Ejemplo:<br>
	...
	public void execute(String actionCommand) {
		...
		if (params.length != 3) {
			NotificationManager.showMessageError("Parametros no validos: "
					+ actionCommand, null);
			return;
	...
	public void exportToFile(Table table, String driverName,
			String fileExtension, String fileDescription) throws Exception {
		FileDriver driver = (FileDriver) LayerFactory.getDM().getDriver(
				driverName);
		if (driver == null) {
	...
	public void exportToFile(Table table, FileDriver driver,
			String fileExtension, String fileDescription) throws Exception {
		File file = this.askForFileName(fileExtension, fileDescription);
		exportToFile(table, driver, file);
	}

	public void exportToFile(Table table, FileDriver driver, File file)
			throws Exception {
		...
		try {
			if (!file.exists()) {
				...
				// Builds a valid empty dbf file. We need it for call to the
				// method Driver.open
				driver.createSource(file.getAbsolutePath(),
						new String[] { "0" }, new int[] { Types.INTEGER });
				file.createNewFile();
			...
			throw new Exception("Error preparando el fichero destino", e); // TODO
																			// intenacionalizacion??
		}
	...
	private File askForFileName(String ext, String extDescription) {
		...
		if (ext.equals("dbf")) {
			extDescription = PluginServices.getText(this, "Ficheros_dbf");
		}
	...
	private class SourceIterator implements Iterator {
		...
		public IFeature nextFeature() throws DriverException,
				ReadDriverException {
