...
public class GeoTransformProcess extends RasterProcess {
	...
	/**
	 * Metodo que recoge los parametros del proceso de transformación <LI>gpcs:
	 * lista de puntos de control</LI>> <LI>orden: orden del polinomio de
	 * transformacion</LI>
	 */
	public void init() {
		...
		// Chequear si el numero de puntos es suficiente para determinar la
		// transformacion de orden n.
		if (!enoughPoints()) {
	...
	/**
	 * @return true si se proporciona el numero minimo de puntos de entrada para
	 *         la transformación de orden seleccionado. false en caso contrario.
	 * */
			...
			for (int i = 0; i <= orden; i++)
				for (int j = 0; j <= i; j++) {
					...
					for (int v = 0; v < gpcs.length; v++)
						matrixDx[filas][k] += (Math.pow(
								gpcs[v].pixelPoint.getX(), i - j) * Math.pow(
								gpcs[v].pixelPoint.getX(), exp[filas][0]))
								* (Math.pow(gpcs[v].pixelPoint.getY(), j) * Math
										.pow(gpcs[v].pixelPoint.getY(),
												exp[filas][1]));
					...
					if (filas == 0) {
						...
						for (int v = 0; v < gpcs.length; v++)
							result[k] += (Math.pow(gpcs[v].pixelPoint.getX(), i
									- j) * Math.pow(gpcs[v].pixelPoint.getX(),
									exp[filas][0]))
									* (Math.pow(gpcs[v].pixelPoint.getY(), j) * Math
											.pow(gpcs[v].pixelPoint.getY(),
													exp[filas][1]))
									* gpcs[v].mapPoint.getX();
					}
				}
		}
	...
	// TO DO: Ver la manera de unificar con setDxGeneral(Parametrizar un metodo
	// general)..... Estudiar optimizaciones!!!
	...
	public void calculateRMSerror() {
		...
		for (int im_point = 0; im_point < numgpcs; im_point++) {
			...
			for (int i = 0; i < minGPC; i++) {
				xEvaluate[im_point] += coefX[i]
						* Math.pow(gpcs[im_point].pixelPoint.getX(), exp[i][0])
						* Math.pow(gpcs[im_point].pixelPoint.getY(), exp[i][1]);
				yEvaluate[im_point] += coefY[i]
						* Math.pow(gpcs[im_point].pixelPoint.getX(), exp[i][0])
						* Math.pow(gpcs[im_point].pixelPoint.getY(), exp[i][1]);

			}

			xError[im_point] = Math.pow(xEvaluate[im_point]
					- gpcs[im_point].mapPoint.getX(), 2);
			rmsxTotal += xError[im_point];
			yError[im_point] = Math.pow(yEvaluate[im_point]
					- gpcs[im_point].mapPoint.getY(), 2);
			rmsyTotal += yError[im_point];
			rms[im_point] = Math.sqrt(xError[im_point] + yError[im_point]);
		...
		for (int i = 0; i < numgpcs; i++) {
			System.out.print("\n");
			System.out
					.print((new Double(gpcs[i].pixelPoint.getX()).toString() + "\t\t"));
			System.out
					.print((new Double(gpcs[i].pixelPoint.getY()).toString() + "\t\t"));
			System.out
					.print((new Double(gpcs[i].mapPoint.getX()).toString() + "\t\t"));
			System.out
					.print((new Double(gpcs[i].mapPoint.getY()).toString() + "\t\t"));
