...
public class GeoTransformDataResult {
	...
	/**
	 * @return array con evaluacion de las coordenadas y de la lista de puntos
	 *         de control.
	 * */
	...
	/**
	 * Función que evalua el polinomio de transformación para un punto
	 * especifico
	 * 
	 * @param x
	...
	public double[] evaluate(double x, double y) {
		...
		for (int i = 0; i < polxCoef.length; i++) {
			eval[0] += polxCoef[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
			eval[1] += polyCoef[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
		}
		return eval;
	}
