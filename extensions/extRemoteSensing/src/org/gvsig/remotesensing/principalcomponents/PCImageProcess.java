...
/**
 * Proceso de contrucción de la imagen resultante del analisis de componentes
 * principales a partir de los componentes seleccionados.
 * 
 * @params <LI>FLyrRasterSE "inputRasterLayer": Capa raster de entrada</LI> <LI>
 *         PCStatistics "statistics": Estadísticas del Análisis de C.P. (ej.:
 *         las generadas por PCStatisticsProcess)</LI> <LI>boolean[]
 *         "selectedBands": Bandas del raster original que se tienen en cuenta
 *         para la transformación</LI> <LI>boolean[] "selectedComponents":
 *         Componentes que se usarán para consturir la imagen transformada</LI>
 *         <LI>String "outputPath": Ruta completa al fichero de salida del
 *         proceso</LI>
 * 
 * @result <LI>FLyrRasterSE: Raster transformado</LI>
 * 
...
public class PCImageProcess extends RasterProcess {
	...
	public void process() throws InterruptedException {
		...
		// Se determina el numero de bandas de la imagen resultante, en funcion
		// de la seleccion.
		int numbandas = 0;
		...
		rasterResult = RasterBuffer.getBuffer(IBuffer.TYPE_FLOAT,
				inputGrid.getLayerNX(), inputGrid.getLayerNY(), bandas.length,
				true);
		...
		if (inputGrid.getRasterBuf().getDataType() == RasterBuffer.TYPE_BYTE) {
			// Construccion de la imagen en funcion de los componentes
			// seleccionados.
						...
						if (selectedPCs[k]) {
							for (int s = 0; s < bandCount; s++) {
								if (selectedPCs[s]) {
									...
									try {
										valor += (double) inputGrid
												.getCellValueAsByte(j, i)
												* pcStatistics
														.getAutoVectorsMatrix()
														.get(s, resultOrden[k]);
	...
	private void createLayer() {
		try {
			...
			grw = GeoRasterWriter.getWriter(writerBufferServer, layerName,
					rasterResult.getBandCount(),
					inputRasterLayer.getAffineTransform(),
					rasterResult.getWidth(), rasterResult.getHeight(),
					rasterResult.getDataType(),
					GeoRasterWriter.getWriter(layerName).getParams(), null);
			grw.dataWrite();
			grw.setWkt(inputRasterLayer.getWktProjection());
			grw.writeClose();

			outputRasterLayer = FLyrRasterSE.createLayer(
					RasterUtilities.getFileNameFromCanonical(layerName),
					layerName, null);
