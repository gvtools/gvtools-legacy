...
/**
 * PCStatisticsProcess es la clase que implementa el proceso cálculo de
 * estadísticas avanzado para el análisis de componentes principales. Se calcula
 * la matriz de varianza-covarianza, los atovalores y autovectrores.
 * 
 * @parms <LI>FLyrRasterSE "inputRasterLayer": Capa raster de entrada</LI> <LI>
 *        boolean[] "selectedBands": Bandas del raster original que se tienen en
 *        cuenta para la transformación</LI>
 * 
 * @result <LI>PCStatistics: Estadísticas reusltantes del ACP.</LI>
 * 
...
public class PCStatisticsProcess extends RasterProcess {
	...
	public Object getResult() {
		if (reusltStatistics == null)
			reusltStatistics = new PCStatistics(autoVectorMatrix, autovalors,
					coVarMatrix);
	...
	private double[][] covarianceOptimize() throws InterruptedException {
		...
		int iValues = 0;
		buildGrid();
		double[][] coV = new double[inputGrid.getRasterBuf().getBandCount()][inputGrid
				.getRasterBuf().getBandCount()];
					...
					for (int k = 0; k < inputGrid.getNX(); k++) {
						for (int l = 0; l < inputGrid.getNY(); l++) {
							try {
								...
								valorBandai = inputGrid
										.getCellValueAsByte(k, l)
										- inputGrid.getMeanValue();
								inputGrid.setBandToOperate(j);
								valorBandaj = inputGrid
										.getCellValueAsByte(k, l)
										- inputGrid.getMeanValue();
							} catch (GridException e) {
								RasterToolsUtil.messageBoxError(PluginServices
										.getText(this, "grid_error"), this, e);
							}
				...
				for (int j = i; j < bandCount; j++) {
					...
					for (int k = 0; k < inputGrid.getNX(); k++) {
						for (int l = 0; l < inputGrid.getNY(); l++) {
							try {
								inputGrid.setBandToOperate(i);
								valorBandai = inputGrid.getCellValueAsShort(k,
										l) - inputGrid.getMeanValue();
								inputGrid.setBandToOperate(j);
								valorBandaj = inputGrid.getCellValueAsShort(k,
										l) - inputGrid.getMeanValue();
							} catch (GridException e) {
								RasterToolsUtil.messageBoxError(PluginServices
										.getText(this, "grid_error"), this, e);
				...
				for (int j = i; j < bandCount; j++) {
					...
					for (int k = 0; k < inputGrid.getNX(); k++) {
						for (int l = 0; l < inputGrid.getNY(); l++) {
							try {
								inputGrid.setBandToOperate(i);
								valorBandai = inputGrid.getCellValueAsInt(k, l)
										- inputGrid.getMeanValue();
								inputGrid.setBandToOperate(j);
								valorBandaj = inputGrid.getCellValueAsInt(k, l)
										- inputGrid.getMeanValue();
							} catch (GridException e) {
								RasterToolsUtil.messageBoxError(PluginServices
										.getText(this, "grid_error"), this, e);
				...
				for (int j = i; j < bandCount; j++) {
					...
					for (int k = 0; k < inputGrid.getNX(); k++) {
						for (int l = 0; l < inputGrid.getNY(); l++) {
							try {
								inputGrid.setBandToOperate(i);
								valorBandai = inputGrid.getCellValueAsFloat(k,
										l) - inputGrid.getMeanValue();
								inputGrid.setBandToOperate(j);
								valorBandaj = inputGrid.getCellValueAsFloat(k,
										l) - inputGrid.getMeanValue();
							} catch (GridException e) {
								RasterToolsUtil.messageBoxError(PluginServices
										.getText(this, "grid_error"), this, e);
				...
				for (int j = i; j < bandCount; j++) {
					...
					for (int k = 0; k < inputGrid.getNX(); k++) {
						for (int l = 0; l < inputGrid.getNY(); l++) {
							try {
								inputGrid.setBandToOperate(i);
								valorBandai = inputGrid.getCellValueAsDouble(k,
										l) - inputGrid.getMeanValue();
								inputGrid.setBandToOperate(j);
								valorBandaj = inputGrid.getCellValueAsDouble(k,
										l) - inputGrid.getMeanValue();
							} catch (GridException e) {
								RasterToolsUtil.messageBoxError(PluginServices
										.getText(this, "grid_error"), this, e);
	...
	public void process() throws InterruptedException {
		...
		EigenvalueDecomposition eigenvalueDecomp = new EigenvalueDecomposition(
				coVarMatrix);
		// Autovectores
