...
public class MosaicProcess extends RasterProcess {
	...
	/**
	 * Inicialización de los parámetros layers - FLayers con los layers
	 * seleccionados para el mosaico.
	 * 
	...
	public void init() {
		...
		for (int i = 0; i < inputRasterLayers.length; i++) {
			dsetCopy = ((FLyrRasterSE) inputRasterLayers[i]).getDataSource()
					.newDataset();
			BufferFactory bufferFactory = new BufferFactory(dsetCopy);
			bufferFactory.setAdjustToExtent(false);
			bufferFactory.setDrawableBands(inputRasterLayers[i]
					.getRenderBands());
			if (!RasterBuffer.loadInMemory(dsetCopy))
				bufferFactory.setReadOnly(true);

			try {
				extendS[i] = new GridExtent(inputRasterLayers[i]
						.getFullExtent().getMinX(), inputRasterLayers[i]
						.getFullExtent().getMinY(), inputRasterLayers[i]
						.getFullExtent().getMaxX(), inputRasterLayers[i]
						.getFullExtent().getMaxY(), inputRasterLayers[i]
						.getAffineTransform().getScaleX(), inputRasterLayers[i]
						.getAffineTransform().getScaleY());

				bufferFactory.setAreaOfInterest(extendS[i].getMin().getX(),
						extendS[i].getMax().getY(), extendS[i].getMax().getX(),
						extendS[i].getMin().getY(), extendS[i].getNX(),
						extendS[i].getNY());
				buffers[i] = bufferFactory.getRasterBuf();
				buffers[i] = ((RasterBuffer) buffers[i]).getAdjustedWindow(
						extendS[i].getNX(), extendS[i].getNY(),
				...
				if (buffers[i].getDataType() != DataBuffer.TYPE_BYTE) {
					...
					leParams = LinearStretchParams.createStandardParam(
							inputRasterLayers[i].getRenderBands(), 0.0,
		...
		try {
			mosaicGrid = new Grid(fullExtend, fullExtend, IBuffer.TYPE_BYTE,
					new int[] { 0, 1, 2 });
			...
			for (int i = 0; i < buffers.length; i++) {
				for (int band = 0; band < resultbandCount; band++) {
					...
					for (int row = 0; row < buffers[i].getHeight(); row++) {
						...
						percent = (int) (progress * 100 / ((mosaicGrid
								.getLayerNY() * resultbandCount)))
								/ buffers.length;
					}
				}
			}
	...
	/**
	 * Método que establece para la coordenada x,y el valor máximo de todos los
	 * valores para ese píxel en cualquiera de las imagenes.
	 * 
	 * @param cordenada
	 *            x
	 * @param coordenada
	 ...
	 * */
	public void setValueMax(int x, int y, int band, int layer)
			throws InterruptedException {

		byte result = Byte.MIN_VALUE;
		byte data = Byte.MIN_VALUE;
		Point2D worldPoint = (Point2D) extendS[layer]
				.getWorldCoordsFromGridCoords(new GridCell(x, y, (double) 0));
		;
		int numLayers = buffers.length;

		for (int i = 0; i < numLayers; i++) {
			if (extendS[i].isAt(worldPoint)) {
				GridCell cell = extendS[i]
						.getGridCoordsFromWorldCoords(worldPoint);
				if (!(cell.getX() >= buffers[i].getWidth() || cell.getY() >= buffers[i]
						.getHeight()))
					data = (byte) RemoteSensingUtils.getCellValueInLayerCoords(
							buffers[i], cell.getX(), cell.getY(), band);
	 ...
	 * @throws InterruptedException
	 * */
	public void setValueMin(int x, int y, int band, int layer)
			throws InterruptedException {
	...
	/**
	 * Método que establece para la coordenada x,y el valor medio de todos los
	 * valores para ese píxel en cualquiera de las imagenes. Si el valor en
	 * cualquiera de las imagenes es noData no es tenido en cuenta
	 * 
	 * @param cordenada
	 *            x
	 * @param coordenada
	 ...
	 * @throws InterruptedException
	 * */
	public void setValueMean(int x, int y, int band, int layer)
			throws InterruptedException {
		...
		for (int i = 0; i < numLayers; i++) {
			if (extendS[i].isAt(worldPoint)) {
				...
				if (!(cell.getX() >= buffers[i].getWidth() || cell.getY() >= buffers[i]
						.getHeight()))
					result += RemoteSensingUtils.getCellValueInLayerCoords(
							buffers[i], cell.getX(), cell.getY(), band);
				buffTotales++;
			}
		}
	 ...
	 * en caso de solape. Se parte de un array de buffer ordenados, de tal
	 * manera que el primer elemento corresponde a la capa situada mas al
	 * frente. El último por contra es el situado al fondo.
	 * 
	 * @param cordenada
	 *            x
	 * @param coordenada
	 ...
	 * @throws InterruptedException
	 * */
	public void setValueFront(int x, int y, int band, int layer)
			throws InterruptedException {
		Point2D worldPoint = (Point2D) extendS[layer]
				.getWorldCoordsFromGridCoords(new GridCell(x, y, (double) 0));
		;
		...
		for (int i = 0; i < numLayers; i++) {
			if (extendS[i].isAt(worldPoint)) {
				...
				if (!(cell.getX() >= buffers[i].getWidth() || cell.getY() >= buffers[i]
						.getHeight()))
					result = (byte) RemoteSensingUtils
							.getCellValueInLayerCoords(buffers[i], cell.getX(),
									cell.getY(), band);
				break;
			}
		}
	 ...
	 * @throws InterruptedException
	 * */
	public void setValueBack(int x, int y, int band, int layer)
			throws InterruptedException {
		Point2D worldPoint = (Point2D) extendS[layer]
				.getWorldCoordsFromGridCoords(new GridCell(x, y, (double) 0));
		;
	...
	private void calculateCoords(int x, int y, int layer) {
		...
		cell = mosaicGrid.getGridExtent().getGridCoordsFromWorldCoords(
				worldPoint);
		if (lastY == cell.getY())
	 ...
	 * 
	 * @param layers
	 *            que intervienen en la operacion.
	...
	private GridExtent calculateExtend(FLyrRasterSE layers[]) {
		...
		for (int i = 0; i < layers.length; i++) {
			...
			double csY = ((FLyrRasterSE) layers[i]).getWCHeight()
					/ ((FLyrRasterSE) layers[i]).getPxHeight();
	...
	public void createLayer() {
		try {
			...
			writerBufferServer = new WriterBufferServer(
					mosaicGrid.getRasterBuf());
			AffineTransform aTransform = new AffineTransform(
					fullExtend.getCellSize(), 0.0, 0.0,
					-fullExtend.getCellSize(), fullExtend.getMin().getX(),
			...
			grw.dataWrite();
			grw.setWkt((String) ((FLyrRasterSE) inputRasterLayers[0])
					.getWktProjection());
			grw.writeClose();
			mosaicGrid.getRasterBuf().free();
			outputRasterLayer = FLyrRasterSE.createLayer(fileName.substring(
	...
	/*
	 * // Identificación de zonas de solapamiento public boolean
	 * getSolapes(FLyrRasterSE raster1, FLyrRasterSE raster2){
	 * 
	 * Grid grid1=null, grid2=null, aux=null;; IRasterDataSource dsetCopy =
	 * null; dsetCopy =raster1.getDataSource().newDataset(); BufferFactory
	 * bufferFactory = new BufferFactory(dsetCopy);
	 * 
	 * IRasterDataSource dsetCopy2 = null; dsetCopy2
	 * =raster2.getDataSource().newDataset(); BufferFactory bufferFactory2 = new
	 * BufferFactory(dsetCopy2);
	 * 
	 * 
	 * if (!RasterBuffer.loadInMemory(dsetCopy))
	 * bufferFactory.setReadOnly(true);
	 * 
	 * try { grid1 = new Grid(bufferFactory,raster1.getRenderBands()); grid2=
	 * new Grid(bufferFactory2,raster2.getRenderBands()); } catch
	 * (RasterBufferInvalidException e) { e.printStackTrace(); }
	 * 
	 * // En grid1 la imagen con la cordenada x menor.
	 * if(grid2.getGridExtent().getMin().getX()<
	 * grid1.getGridExtent().getMin().getX()) { try { grid1 = new
	 * Grid(bufferFactory2,raster2.getRenderBands()); grid2= new
	 * Grid(bufferFactory,raster1.getRenderBands()); } catch
	 * (RasterBufferInvalidException e) { e.printStackTrace(); }
	 * 
	 * }
	 * 
	 * double xmin= grid1.getGridExtent().getMin().getX(); double xmax=
	 * grid1.getGridExtent().getMax().getX(); double ymin=
	 * grid1.getGridExtent().getMin().getY(); double ymax=
	 * grid1.getGridExtent().getMax().getY();
	 * 
	 * double xmin2= grid2.getGridExtent().getMin().getX(); double ymin2=
	 * grid2.getGridExtent().getMin().getY();
	 * 
	 * if(!(xmin2>xmin && xmin2<xmax)){
	 * System.out.print("Las imagenes no se solapan en las X"); return false; }
	 * 
	 * if(!(ymin2>ymin && ymin2<ymax)){
	 * System.out.print("Las imagenes no se solapan en las Y"); return false; }
	 * 
	 * // Detectado el solapamiento System.out.print("Rango x["+ xmin2 + ","+
	 * Math.min(xmax,grid2.getGridExtent().getMax().getX())+"].");
	 * System.out.print("Rango y["+ ymin2 + ","+
	 * Math.min(ymax,grid2.getGridExtent().getMax().getY())+"].");
	 * 
	 * return true; }
	 */
