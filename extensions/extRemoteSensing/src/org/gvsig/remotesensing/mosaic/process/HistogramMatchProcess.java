...
/**
 * Clase que implementa el proceso de Histogram Matching para un conjuto de
 * imagenes que se quieren hacer corresponder con un histogram de referencia. El
 * proceso tomara como parametros un conjunto de capas y un histograma maestro y
 * ejecutara de forma secuencial un filtro de histogramMatch para cada una de
 * ellas. El proceso se realiza sobre las bandas de visulaización.
 * 
 * @params <LI>FLyrRasterSE[] "inputRasterLayers": Capas raster de entrada</LI>
 *         <LI>Histogram "masterHistogram": Histograma con el que se hace la
 *         correspondencia</LI> <LI>int "numbandas": PROVISIONAL.</LI> <LI>
 *         String "outputPath": Ruta completa al fichero de salida del proceso</LI>
 * 
 * @result <LI>outputRassterLayers[]: Capas raster resultantes</LI>
 * 
...
public class HistogramMatchProcess extends RasterProcess {
	...
	/**
	 * Metodo en el que se recogen los parámetros para el proceso layers
	 * conjunto de capas a las que se aplica el proceso histogramMaster
	 * histograma de referencia view vista sobre la que se cargan los resultados
	 * */
	...
	/**
	 * Proceso. Para cada capa de las contenidas en layers, se aplica un filtro
	 * de histogramMatching entre la capa y el histograma de referencia pasado
	 * como paramero.
	 * */
		...
		try {
			...
			for (int i = 0; i < inputRasterLayers.length; i++) {
				...
				if (inputRasterLayers[i] != null) {
					...
					dsetCopy = inputRasterLayers[i].getDataSource()
							.newDataset();
					...
					if (inputBuffer.getDataType() != DataBuffer.TYPE_BYTE) {
						...
						inputBuffer = (IBuffer) linearStretchEnhancementFilter
								.getResult("raster");
					}
					...
					outputRassterLayers[i] = createLayer(path + File.separator
							+ "hist" + i + ".tif", i);
				}
			}
			...
			RasterToolsUtil.messageBoxError(
					PluginServices.getText(this, "filter_error"), this, e2);
		} finally {
	...
	public String getLog() {
		return PluginServices.getText(this, "hiatogramMatchLog")
				+ layerInProcess + "  (" + countInProcess + "/" + totalcount
				+ ")";
	...
	public FLyrRasterSE createLayer(String path, int i) {
		...
		try {
			...
			lyr = FLyrRasterSE
					.createLayer(path.substring(
							path.lastIndexOf(File.separator) + 1, endIndex),
							path, null);
