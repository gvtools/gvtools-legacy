...
public class FeatherProcess extends RasterProcess {
	...
	public void init() {
		...
		for (int i = 1; i < inputRasterLayers.length; i++) {
			if (Math.abs(cellSizeX) > Math.abs(inputRasterLayers[i]
					.getAffineTransform().getScaleX()))
				cellSizeX = inputRasterLayers[i].getAffineTransform()
						.getScaleX();
			if (Math.abs(cellSizeY) > Math.abs(inputRasterLayers[i]
					.getAffineTransform().getScaleY()))
				cellSizeY = inputRasterLayers[i].getAffineTransform()
						.getScaleY();
		}
	...
	public void process() throws InterruptedException {
		...
		for (int l = 0; l < inputRasterLayers.length; l++) {
			try {
				inputGrids[l] = new Grid(inputRasterLayers[l].getDataSource(),
						new int[] { 0, 1, 2 }, resultGridExtent);
				inputGrids[l]
	...
	private void createLayer() {
		...
		try {
			...
			if (view == null)
				return;
			view.getModel().getMapContext().getLayers()
					.addLayer(outputRasterLayer);
		...
		try {
			for (int i = 0; i < borders.length; i++) {
				...
				for (int band = 0; band < resultbandCount; band++) {
					...
					resultCellVale[band] = resultCellVale[band] + cellValue
							* distances[i];
				}
			}
