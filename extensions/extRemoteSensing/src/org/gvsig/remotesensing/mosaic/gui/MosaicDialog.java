...
public class MosaicDialog extends DefaultButtonsPanel implements
		PropertyListener, IWindow, IWindowListener, ButtonsPanelListener,
		ActionListener, FocusListener, PropertyChangeListener, IProcessActions {
	...
	public JPanel getOutputOptionsPanel() {
		...
		if (nombreCapa == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "salida"));
			topBorder.setTitlePosition(TitledBorder.TOP);
			...
			gridBagConstraints.anchor = GridBagConstraints.WEST;
			gridBagConstraints.insets = new java.awt.Insets(3, 3, 3, 3);
			nombreCapa.add(new JLabel((PluginServices.getText(this, "name"
		...
		if (getRadioFile().isSelected()) {
			...
			for (int i = 0; i < extList.length; i++) {
				...
				if (extList[i].toLowerCase().equals("tif")) {
					...
					fileFilter.addExtension((String) "tiff");
				}
				if (extList[i].toLowerCase().equals("jpg")) {
					fileFilter.addExtension((String) "jpeg");
				}
				if (extList[i].toLowerCase().equals("mpl")) {
					fileFilter.addExtension((String) "mpr");
				}
	...
	/**
	 * Acciones al presionar los botones aceptar, aplicar o cancelar Cancelar se
	 * cierra el dialogo Aceptar se comprueba las opciones seleccionadas. si la
	 * opcion de histogramMatching se encuentra seleccionada, si no se dispone
	 * del hostograma de referencia.Obtenido el histograma (con un proceso
	 * histogramProcess), se lanza el proceso de HistogramMatching para cada una
	 * de las capas seleccionadas. Una vez finalizado, se comprueba que la
	 * opción de mosaicos se encuentra activa. En caso afirmativo se lanza el
	 * proceso de mosaico con los parámetros correspondientes.
	 * */
	...
	private void applyHistogramMatchProcess() {
		try {
			FLyrRasterSE capas[] = new FLyrRasterSE[getPanelSouth().getLayers()
					.getLayersCount()];
			hmMasterRaster = (FLyrRasterSE) getPanelSouth().getLayers()
					.getLayer(
							(String) getPanelSouth().getMasterImageCombo()
									.getSelectedItem());
			...
			Histogram histogramReference = bufferFactory.getRasterBuf()
					.getHistogram();

			// Histogram
			// histogramReference=hmMasterRaster.getDataSource().getHistogram();
			for (int i = 0; i < getPanelSouth().getLayers().getLayersCount(); i++)
				if (!hmMasterRaster.getName().equals(
						getPanelSouth().getLayers().getLayer(i).getName())) {
					capas[i] = (FLyrRasterSE) getPanelSouth().getLayers()
							.getLayer(i).cloneLayer();
				} else
					masterRasterPosition = i;

			File tempHMDir = new File(Utilities.createTempDirectory()
					+ File.separator + "HMResults");
			tempHMDir.mkdir();
			...
			RasterToolsUtil.messageBoxError(PluginServices.getText(this,
					"apply_histogramMatching_filter_error"), this, e2);
		}
	}
	...
	public void loadLayerInToc(String fileName) {
		if (!new File(fileName).exists())
			return;
		try {
			RasterToolsUtil.loadLayer(PluginServices.getMDIManager()
					.getWindowInfo(view).getTitle(), fileName, null);
	...
	public void end(Object param) {
		...
		case 2: // MOSAICING
			...
			File tempDir = new File(Utilities.createTempDirectory()
					+ File.separator + "HMResults");
	...
	private void applyBasicProcess(FLyrRasterSE inputRasterLayers[]) {
		...
		proceso.addParam("methodCode", new Integer(getPanelSouth()
				.getComboOverlapFunction().getSelectedIndex()));
		...
		if (e.getSource() == getPanelSouth().getCheckFeathering()) {
			if (getPanelSouth().getCheckFeathering().isSelected()) {
				...
				if (getPanelNorth().getModeloTabla().getRowCount() > 1) {
					getPanelNorth().getModeloTabla().setValueAt(
	...
	/** Acciones al aplicar */
	void apply() {
		applyHistogramM = getPanelSouth().getCheckHistogramMatching()
				.isSelected();

		if (applyHistogramM) {
			currentProcess = this.HM;
			try {
				FLyrRasterSE rasterReferencia = (FLyrRasterSE) getPanelSouth()
						.getLayers().getLayer(
								(String) getPanelSouth().getMasterImageCombo()
										.getSelectedItem());
				// Si el histograma no esta calculado se calcula
				HistogramProcess histoCalculus = new HistogramProcess();
				histoCalculus.addParam("histogramable",
						(IHistogramable) rasterReferencia.getDataSource());
				...
				RasterToolsUtil.messageBoxError(PluginServices.getText(this,
						"error_getting_referenceHistogram"), this, e2);
			}
		} else {
			currentProcess = this.PREMOSAICING;
			inputRasterLayers = new FLyrRasterSE[getPanelSouth().getLayers()
					.getLayersCount()];
			for (int i = 0; i < inputRasterLayers.length; i++)
				inputRasterLayers[i] = (FLyrRasterSE) getPanelSouth()
						.getLayers().getLayer(i);
