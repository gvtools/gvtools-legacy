...
public class MethodMosaicPanel extends JPanel implements ActionListener {
	...
	/**
	 * Constructor param nameLayersAviable array con los nombres de las capas
	 * que intervendran en el proceso de mosaico//setBorder(topBorder);
	 * 
	 * */
	...
	private void initUI() {
		...
		TitledBorder topBorder = BorderFactory
				.createTitledBorder(PluginServices.getText(this, "methods"));
	...
	public JPanel getNorthPanel() {
		if (northPanel == null) {
			...
			northPanel.setBorder(new CompoundBorder(topBorder, new EmptyBorder(
					2, 2, 2, 2)));
			...
			constrains.gridx = 0;
			constrains.gridy = 0;
			panel.add(new JLabel(PluginServices.getText(this, "image_master")),
					constrains);
	...
	public JPanel getSouthPanel() {
		if (southPanel == null) {
			...
			southPanel.setBorder(new CompoundBorder(topBorder, new EmptyBorder(
					2, 2, 2, 2)));
	...
	public JCheckBox getCheckHistogramMatching() {
		if (checkHistogramMatching == null) {
			checkHistogramMatching = new JCheckBox(PluginServices.getText(this,
	...
	public JCheckBox getCheckFeathering() {
		if (checkFeathering == null) {
			checkFeathering = new JCheckBox(PluginServices.getText(this,
					"feathering"), false);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(getCheckHistogramMatching())) {
			getMasterImageCombo().setEnabled(
					getCheckHistogramMatching().isSelected());
		}
		...
		if (e.getSource().equals(getCheckFeathering())) {
			...
			getTableContainer().getTable().setEnabled(
					getCheckFeathering().isSelected()
							|| getRButtomCutLine().isSelected());
			getComboOverlapFunction().setEnabled(
					!getCheckFeathering().isSelected());
		...
		if (e.getSource().equals(getRButtomEdge())) {
			view.getMapControl().getMapContext().getGraphicsLayer()
					.clearAllGraphics();
	...
	public JRadioButton getRButtomCutLine() {
		if (rButtomCutLine == null) {
			rButtomCutLine = new JRadioButton(PluginServices.getText(this,
					"cut_line_Feathering"));
	...
	public JRadioButton getRButtomEdge() {
		if (rButtomEdge == null) {
			rButtomEdge = new JRadioButton(PluginServices.getText(this,
					"edge_Feathering"));
	...
	private JPanel getPanelOverlapFunction() {
		if (panelOverlapFunction == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "overlap"));
			panelOverlapFunction.setBorder(new CompoundBorder(topBorder,
					new EmptyBorder(5, 2, 2, 2)));
			panelOverlapFunction.setSize(300, 80);
			BorderLayout bd = new BorderLayout();
			bd.setHgap(10);
			panelOverlapFunction.setLayout(bd);
			panelOverlapFunction.add(
					new JLabel(PluginServices.getText(this, "function")),
					BorderLayout.WEST);
			panelOverlapFunction.add(getComboOverlapFunction(),
					BorderLayout.CENTER);
	...
	public void updateRoisTable() {
		...
		for (int i = 0; i < nameLayers.length; i++) {
			raster = (FLyrRasterSE) view.getMapControl().getMapContext()
					.getLayers().getLayer(nameLayers[i]);
			ArrayList roisArray = raster.getRois();
			if (roisArray != null) {
				for (Iterator iter = roisArray.iterator(); iter.hasNext();) {
					roi = (VectorialROI) iter.next();
					;
					for (Iterator iterator = roi.getGeometries().iterator(); iterator
							.hasNext();) {
						...
						if (load && countGeometries == 1) {
							Object row[] = ((IModel) tableContainer.getTable()
									.getTableModel()).getNewLine();
							row[1] = roi.getName() + "-" + raster.getName();
	...
	public void drawRoi() {
		...
		IGeometry geometry = null;
		view.getMapControl().rePaintDirtyLayers();
		GraphicLayer graphicLayer = view.getMapControl().getMapContext()
				.getGraphicsLayer();
	...
	public FLayers getLayers() {
		...
		for (int i = 0; i < nameLayers.length; i++)
			try {
				fLayers.addLayer(view.getMapControl().getMapContext()
						.getLayers().getLayer(nameLayers[i]).cloneLayer());
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public JComboBox getComboOverlapFunction() {
		if (comboOverlapFunction == null) {
			...
			comboOverlapFunction.addItem(new String(PluginServices.getText(
					this, "maximo")));
			comboOverlapFunction.addItem(new String(PluginServices.getText(
					this, "minimo")));
			comboOverlapFunction.addItem(new String(PluginServices.getText(
