...
public class SelectLayersFromViewPanel extends JPanel {
	...
	public SelectLayersFromViewPanel(View view) {
		...
		if (this.view != null)
			/*
			 * make room for at most all raster layers in View + 1 terminator
			 * entry
			 */
			layers = new FLayer[view.getMapControl().getMapContext()
					.getLayers().getLayersCount() + 1];
		int k = view.getMapControl().getMapContext().getLayers()
				.getLayersCount() - 1;

		/* add all raster layers of the current View to layer selection list */
		int j = 0;
		for (int i = 0; i < view.getMapControl().getMapContext().getLayers()
				.getLayersCount(); i++) {
			if ((view.getMapControl().getMapContext().getLayers().getLayer(k)) instanceof FLyrRasterSE) {
				layers[j] = ((FLyrRasterSE) view.getMapControl()
						.getMapContext().getLayers().getLayer(k));
				j++;
			}
			k--;
		}
		layers[j] = null; /* terminate array */
	...
	private String[] getLayerNames() {
		...
		for (i = 0; i < layers.length; i++) {
			if (layers[i] instanceof FLyrRasterSE)
				if (layers[i] != null) /* add only valid (raster) layers */
					capasraster++;
		}
	...
	/**
	 * Medodo que devuelve un array de Strings con en nombre de las capas
	 * seleccionadas
	 * */
	public String[] getSelectedLayers() {

		String[] layersSelectedNames = new String[getModeloTabla()
				.getNumSelected()];
