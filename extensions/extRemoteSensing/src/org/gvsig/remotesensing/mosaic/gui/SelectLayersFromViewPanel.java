...
public class SelectLayersFromViewPanel extends JPanel{
	...
	public SelectLayersFromViewPanel (View view){
		this.view=view;
		if(this.view!=null)
			/* make room for at most all raster layers in View + 1 terminator entry */
			layers= new FLayer[view.getMapControl().getMapContext().getLayers().getLayersCount() + 1];
		...
		for(int i=0;i<view.getMapControl().getMapContext().getLayers().getLayersCount();i++){
			if ( (view.getMapControl().getMapContext().getLayers().getLayer(k)) instanceof FLyrRasterSE ) {
				layers[j]=((FLyrRasterSE)view.getMapControl().getMapContext().getLayers().getLayer(k));
				j++;
			}
			k--;
		}
	...
	public JPanel getImageViewPanel() {
		if(imageViewPanel==null){
			...
			TitledBorder topBorder = BorderFactory.createTitledBorder(PluginServices.getText(this,"seleccion_imagenes"));
