...
/**
 * Panel que implementa el comportamiento básico de una calculadora cuyas
 * variables pueden ser asociadas a las bandas de las capas raster cargadas en
 * una vista.
 * 
...
public class CalculatorPanel extends JPanel implements FocusListener {
	...
	/**
	 * Define los elementos del panel central: JTree, Tabla de Variables y
	 * calculadora
	 * 
	...
	public JPanel getCenterPanel() {
		...
		if (centerPanel == null) {
			...
			getJTableVariables().getTableFormat()
					.getColumn(PluginServices.getText(this, "valor"))
	...
	/**
	 * Crea un panel en el que se agrega el area de texto donde se define la
	 * expresión.
	 * 
	...
	public JPanel getExpresionPanel() {
		...
		TitledBorder topBorder = BorderFactory
				.createTitledBorder(PluginServices.getText(this, "expression"));
		topBorder.setTitlePosition(TitledBorder.TOP);
		// creamos un borde compuesto
		expresionPanel.setBorder(new CompoundBorder(topBorder, new EmptyBorder(
				0, 0, 0, 0)));
		p.add(getSalvar(), "1,0");
	...
	public JCheckBox getJCheckExtent() {
		...
		if (jCheckExtent == null) {
			jCheckExtent = new JCheckBox(PluginServices.getText(this,
					"distinto_extent"), false);
	 ...
	 * bandas correspondientes. Ademas con las funciones, operadores y
	 * constantes estandar.
	 */
	void InicizlizaJTree(JTree jTree) {
		...
		DefaultMutableTreeNode main = new DefaultMutableTreeNode(
				PluginServices.getText(this, "elementos_jtree"));
		DefaultMutableTreeNode node = new DefaultMutableTreeNode(
				PluginServices.getText(this, "capas_jtree"));
		...
		for (i = 0; i < layers.getLayersCount(); i++) {
			...
			if (layers.getLayer(i) instanceof FLyrRasterSE) {
				...
				for (int j = 0; j < ((FLyrRasterSE) layers.getLayer(i))
						.getBandCount(); j++) {
		...
		node = new DefaultMutableTreeNode(PluginServices.getText(this,
				"funciones_jtree"));
	...
	/**
	 * Agrega el texto del árbol sobre el que se ha hecho doble click en el
	 * cuadro de la expresion
	 */
	private void insertTextFromTree(TreePath path) {
		String var;
		boolean yaAsignada = false;
		// Devuelve la ruta de acceso primaria de la ruta de acceso
		// proporcionada.
		...
		if (parent != null
			...
			if (parent
				...
				if (jTableVariables.isEnEspera()) {
					...
					if (checkVisible && !getJCheckExtent().isSelected()) {
						...
						for (int i = 0; i < getJTableVariables()
								.getTableFormat().getRowCount(); i++) {

							layerBand = getJTableVariables().getTableFormat()
							...
							if (i == getJTableVariables().getTableFormat()
									.getRowCount() - 1) {
						...
						if (!yaAsignada) {
							...
							// Se comparan
							layerBand = getJTableVariables().getTableFormat()
									.getValueAt(filaAsignada, 1).toString();
							layerName = layerBand.substring(0,
									layerBand.indexOf("["));
							rasterLayer1 = (FLyrRasterSE) m_MapContext
									.getLayers().getLayer(layerName);

							// Variable pasan como argumento
							layerName = s.substring(0, s.indexOf("["));
							rasterLayer2 = (FLyrRasterSE) m_MapContext
									.getLayers().getLayer(layerName);

							if ((rasterLayer1.getFullRasterExtent().height() == rasterLayer2
									.getFullRasterExtent().height())
									&& (rasterLayer1.getFullRasterExtent()
											.width() == rasterLayer2
											.getFullRasterExtent().width())) {// iguales){
					...
					jTextExpresion.insert(var,
							jTextExpresion.getCaretPosition());
					s = "";
				}
			} else
			...
			if (sParentName.equals("["
					+ PluginServices.getText(this, "elementos_jtree") + ", "
					+ PluginServices.getText(this, "constantes_jtree") + "]")) {
			...
			if (sParentName.equals("["
					+ PluginServices.getText(this, "elementos_jtree") + ", "
					+ PluginServices.getText(this, "funciones_jtree") + "]")) {
				jTextExpresion.setCaretPosition(jTextExpresion
						.getCaretPosition() - 2);
			}
		}
	}
	...
	/**
	 * Agregar la variable y el valor a la tabla, haciendo la comprobacion del
	 * extent
	 * 
	 * @param valor
	...
	private String insertVariableToJTable(String valor) {
		...
		/*
		 * Antes de insertar la variable hay que comprobar si no esta marcada la
		 * casilla que no hay diferente extent caso no permitir distinto extent
		 */

		if (checkVisible && !getJCheckExtent().isSelected()) {
			if (getJTableVariables().getTableFormat().getRowCount() > 0) {
				for (int i = 0; i < getJTableVariables().getTableFormat()
						.getRowCount(); i++) {
					layerBand = getJTableVariables().getTableFormat()
							.getValueAt(i, 1).toString();
					if (!layerBand.equals("")) {
				...
				if (yaAsignada == false) {
					...
					rasterLayer1 = (FLyrRasterSE) m_MapContext.getLayers()
							.getLayer(layerName);

					// Variable que me pasan como argumento
					layerName = valor.substring(0, valor.indexOf("["));
					rasterLayer2 = (FLyrRasterSE) m_MapContext.getLayers()
							.getLayer(layerName);
					// Se compara si tienen las mismas dimensiones
					if ((rasterLayer1.getFullRasterExtent().height() == rasterLayer2
							.getFullRasterExtent().height())
							&& (rasterLayer1.getFullRasterExtent().width() == rasterLayer2
									.getFullRasterExtent().width()))// &&
																	// (qWindow.getNY()==
																	// qWindow1.getNY())
					{
						s = jTableVariables.InsertRow(null, valor);
	...
	/**
	 * Se utiliza para conocer si un determinado valor-->(capa[banda]) tiene una
	 * variable asociada, si no es así se introduce y se le asigna una nueva
	 * variable
	 * 
	 * @param valor
	...
	public void focusLost(FocusEvent e) {
		...
		if (e.getSource() == jTextExpresion) {
			jTableVariables.setEnEspera(false);
			System.out.println("Evento del textarea:   "
					+ jTableVariables.isEnEspera());
		}
	}
	...
	private void loadVarTable() {
		...
		if (varTable.getCont() > 0)
			varTable.getTableFormat().removeRowSelectionInterval(0,
					varTable.getCont() - 1);
		for (Iterator iter = persistentVarTable.keySet().iterator(); iter
				.hasNext();) {
	...
	/**
	 * Actualiza el HasMap de persistencia con los elementos que tiene la tabla
	 * de variables.
	 * 
	 */
	public void updatePersistentVarTable() {
		for (int i = 0; i < getJTableVariables().getTableFormat().getRowCount(); i++) {
			String var = getJTableVariables().getTableFormat().getValueAt(i, 0)
					.toString();
			String value = getJTableVariables().getTableFormat()
					.getValueAt(i, 1).toString();
