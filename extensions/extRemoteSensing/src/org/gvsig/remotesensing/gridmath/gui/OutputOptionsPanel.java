 ...
 * @version 22/10/2007
 */
public class OutputOptionsPanel extends JPanel implements ActionListener,
		FocusListener, KeyListener, PropertyListener {
	...
	public void InicializarOpcion() {
		...
		for (int i = 0; i < cp.getCalculatorPanel().getJTableVariables()
				.getTableFormat().getRowCount(); i++) {
			allAssigned = true;
			if (cp.getCalculatorPanel().getJTableVariables().getTableFormat()
					.getValueAt(i, 1).toString().equals("")) {
				allAssigned = false;
				break;
			}
		}
	...
	private void setAjustInDataExtent() {
		// Si no hay variables asignadas no hace nada. No dejara lanzar el
		// calculo
		if (cp.getCalculatorPanel().getJTableVariables().getTableFormat()
				.getRowCount() > 0
				&& cp.getCalculatorPanel().getJTableVariables()
						.getTableFormat().getValueAt(0, 1).toString() != "") {

			String layerName = cp.getCalculatorPanel().getJTableVariables()
					.getTableFormat().getValueAt(0, 1).toString();
			layerName = layerName.substring(0, layerName.indexOf("["));
			FLyrRasterSE rasterLayer = (FLyrRasterSE) mapContext.getLayers()
					.getLayer(layerName);
			...
			double cellSizeX = Math.abs(rasterLayer.getAffineTransform()
					.getScaleX());
			double cellSizeY = Math.abs(rasterLayer.getAffineTransform()
					.getScaleY());

			for (int i = 0; i < cp.getCalculatorPanel().getJTableVariables()
					.getTableFormat().getRowCount(); i++) {
				layerName = cp.getCalculatorPanel().getJTableVariables()
				...
				cellSizeX = Math.min(cellSizeX,
						Math.abs(rasterLayer.getAffineTransform().getScaleX()));
				cellSizeY = Math.min(cellSizeY,
						Math.abs(rasterLayer.getAffineTransform().getScaleY()));
			}
			...
			getJTextRangoX1().setText(
					String.valueOf(cp.getOutputExtent().minX()));
			getJTextRangoX2().setText(
					String.valueOf(cp.getOutputExtent().maxX()));
			getJTextRangoY1().setText(
					String.valueOf(cp.getOutputExtent().minY()));
			getJTextRangoY2().setText(
					String.valueOf(cp.getOutputExtent().maxY()));
