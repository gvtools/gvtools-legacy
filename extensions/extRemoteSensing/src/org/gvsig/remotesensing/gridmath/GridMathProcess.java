...
/**
 * GridMathProcess implenta la funcionalidad para el cálculo de operaciones
 * entre grids. El proceso permite hacer operaciones matemáticas entre los
 * valores de las bandas de la misma imagen o entre diferentes imágenes bajo
 * ciertas restricciones espaciales (siempre sobre los datos originales).
 * 
...
public class GridMathProcess extends RasterProcess {
	...
	/**
	 * Establece la expresión en el parser
	 * 
	 * @param expression
	 *            expresion
	 */
	...
	private void writeToFile() {
		...
		try {
			...
			aTransform = new AffineTransform(resultExtent.getCellSizeX(), 0.0,
					0.0, -resultExtent.getCellSizeY(), resultExtent.getMin()
							.getX(), resultExtent.getMax().getY());
		...
		try {
			lyr = FLyrRasterSE.createLayer(filePath.substring(
					filePath.lastIndexOf(File.separator) + 1, endIndex),
					new File(filePath), mapContext.getCrs());
	...
	public void process() throws InterruptedException {
		...
		rasterResult = RasterBuffer.getBuffer(RasterBuffer.TYPE_DOUBLE,
				resultExtent.getNX(), resultExtent.getNY(), 1, true);
				...
				for (Iterator iter = params.keySet().iterator(); iter.hasNext();) {
					...
					if (dataType == IBuffer.TYPE_BYTE) {
						value = (byte) RemoteSensingUtils
								.getCellValueInLayerCoords(inputBuffer, x, y, 0);
						if (value != inputBuffer.getNoDataValue()) {
							parser.setVarValue(varName, new Double(value));
							i++;
						} else {
							rasterResult.setElem(y, x, 0,
									rasterResult.noDataValue);
							break;
						}
					} else {
	...
	void loadBuffers() {
		...
		int numVar = gridMathPanel.getCalculatorPanel().getJTableVariables()
				.getTableFormat().getRowCount();
		for (int i = 0; i < numVar; i++) {

			/* DEBUG */
			System.err.println("*** DEBUG: I = " + i + " of " + numVar);

			layerBand = gridMathPanel.getCalculatorPanel().getJTableVariables()
					.getTableFormat().getValueAt(i, 1).toString();
			layerName = layerBand.substring(0, layerBand.indexOf("["));
			nBand = Integer.valueOf(
					layerBand.substring(layerBand.lastIndexOf("Band") + 4,
			...
			try {
				...
				bufferFactory.setAreaOfInterest(minX, minY, maxX, maxY,
						gridMathPanel.getOutputExtent().getNX(), gridMathPanel
								.getOutputExtent().getNY());
			...
			gridMathPanel
					.getCalculatorPanel()
					.getQWindowsHash()
					.put(gridMathPanel.getCalculatorPanel()
		...
		gridMathPanel.getGridMath().setParams(
				gridMathPanel.getCalculatorPanel().getQWindowsHash());

		JEP parser = new JEP();
		parser.setAllowUndeclared(true);
		parser.addStandardFunctions();
		parser.parseExpression(gridMathPanel.getCalculatorPanel()
				.getJTextExpression().getText());

		for (Iterator iter = parser.getSymbolTable().values().iterator(); iter
				.hasNext();) {
			Variable variable = (Variable) iter.next();
			if (!gridMathPanel.getGridMath().getParams()
					.containsKey(variable.getName())) {
				RasterToolsUtil.messageBoxError(
						PluginServices.getText(this, "variables_sin_asignar"),
						this);
				return;
			}
		}
	...
	public int getPercent() {
		...
		/*
		 * if(!loadEnd) return (int)((loadBuffers-1)/numVar)*100; else
		 */if (writerBufferServer != null)
	...
	public String getLog() {
		...
		/*
		 * if(!loadEnd) return
		 * PluginServices.getText(this,"cargando datos")+"...";
		 */
