...
/**
 * 
 * Reescritura de la clase ChartPanel. Se reescribe para personalizar el grafico
 * con opciones de dibujado de rectangulos sobre el area del gráfico.
 * 
...
public class ScatterPlotDiagram extends JPanel implements ChartChangeListener,
		ActionListener, MouseListener, MouseMotionListener, Printable,
		Serializable, IProcessActions {
	 ...
	 *            for the chart.
	 */
	public ScatterPlotDiagram(JFreeChart chart, int width, int height,
	...
	private Point getPointInRectangle(int x, int y, Rectangle2D area) {
		x = (int) Math.max(Math.ceil(area.getMinX()),
				Math.min(x, Math.floor(area.getMaxX())));
		y = (int) Math.max(Math.ceil(area.getMinY()),
				Math.min(y, Math.floor(area.getMaxY())));
		...
		if (this.zoomRectangle != null) {
			...
			if (zoomTrigger1 || zoomTrigger2) {
				if ((hZoom && (e.getX() < this.zoomPoint.getX()))
					...
					if (activeROI != null) {
						Rectangle2D rectangleArea = new Rectangle2D.Double(x,
								y, w, h);
		...
		if ((selection.getHeight() > 0) && (selection.getWidth() > 0)) {
			...
			if (p instanceof ScatterPlotChart) {
				...
				if (z.getOrientation() == PlotOrientation.HORIZONTAL) {
					rangos[0] = z.getRangeX(vLower, vUpper, plotInfo,
							selectOrigin);
					rangos[1] = z.getRangeY(hLower, hUpper, plotInfo,
							selectOrigin);
				} else {
					// devolver las coordenadas del rectangulo.
					rangos[0] = z.getRangeX(hLower, hUpper, plotInfo,
							selectOrigin);
					rangos[1] = z.getRangeY(vLower, vUpper, plotInfo,
							selectOrigin);
				}
			}
	...
	public void doEditChartProperties() {
		...
		/*
		 * ChartEditorManager.getChartEditor(this.chart); int result =
		 * JOptionPane.showConfirmDialog(this, editor,
		 * localizationResources.getString("Chart_Properties"),
		 * JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE); if (result
		 * == JOptionPane.OK_OPTION) { editor.updateChart(this.chart); }
		 */
	}
	...
	protected JPopupMenu createPopupMenu(boolean properties, boolean save,
		...
		JMenuItem newClassItem = new JMenuItem(PluginServices.getText(this,
				"new_class"));
	...
	public void newRoiChart() {
		activeROI = new ROIChart(roiList.getNexColor(),
				roiList.getdefaultName(), bands);
		updateList();
	}
