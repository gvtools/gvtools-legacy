...
public class ManagerROIChartPanelListener implements ButtonsPanelListener,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == tablePanel.getExportButton()) {
			...
			process.addParam("raster", (FLyrRasterSE) tablePanel
					.getScatterPlotPanel().getFLayer());
			process.setActions(this);
			// Mirar si es apropiado siempre mostrar la barra o solo en el caso
			// layer grandes
			process.start();
			...
			try {
				if (tablePanel.getTable().getSelectedRows().length > 0) {
					...
					String roiName = tablePanel.getDiagram()
							.getActiveRoiChart().getName();
					// Borrado de la roi de la lista de rois
					tablePanel.getDiagram().getROIChartList()
							.deleteROI(roiName);

					if (tablePanel.getTable().getRowCount() > 0) {
						int selectedRow = 0;
						roiSelectedName = (String) tablePanel.getTable()
								.getTable().getJTable()
								.getValueAt(selectedRow, 0);
						// Se establece la roi activa.
						tablePanel.getDiagram().setActiveRoi(
								(ROIChart) tablePanel.getDiagram()
										.getROIChartList().getListRois()
										.get(roiSelectedName));
	...
	public void valueChanged(ListSelectionEvent e) {
		try {
			if (tablePanel.getTable().getSelectedRows().length <= 0) {
				...
				tablePanel.getDiagram().setActiveRoi(
						(ROIChart) tablePanel.getDiagram().getROIChartList()
								.getListRois().get(roiSelectedName));
				tablePanel.getDiagram().updateUI();
				// tablePanel.getDiagram().setActiveRoi()
			}
		} catch (NotInitializeException e1) {
	...
	public void tableChanged(TableModelEvent e) {
		...
		if (e.getColumn() == 4) {
			...
			Color color = (Color) tablePanel.getTable().getTable().getJTable()
