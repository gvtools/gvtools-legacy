...
public class RoiFromChartProcess extends RasterProcess {
	 ...
	 * 
	 * @param roiToExport
	 *            roiChart que va a ser a exportar
	 * @grid grid asociado (unicamente dispone de las dos bandas seleccionadas
	 *       en el grafico)
	 * 
	 **/
	 ...
	 * this.roiToExport= roiToExport; this.rasterSE= raster; }
	 */
	...
	/**
	 * Esto tiene que cambiar!!!!!!!! Actualmente solo pinta los pixeles que
	 * pertenecen a la roi Dibujado de la roi
	 * */
	void drawRoi(VectorialROI roi) {
		...
		for (Iterator iterator = vectorialROI.getGeometries().iterator(); iterator
				.hasNext();) {
	...
	/**
	 * Proceso que construye la ROI Vectorial añadiendo un punto por cada pixel
	 * que se encuentra en el conjunto de rangos especificados. Para terminar se
	 * pinta la roi en la vista
	 * */
			...
			for (int j = 0; j < nY; j++) {
				...
				for (int i = 0; i < nX; i++) {
					...
					while (iterator.hasNext()) {
						...
						if (isInside(valorBandX, rango[0])
								&& isInside(valorBandY, rango[1])) {
							mapX = grid.getGridExtent().getMin().getX() + i
									* grid.getCellSize();
							mapY = grid.getGridExtent().getMax().getY() - j
									* grid.getCellSize();
							IGeometry geometry = ShapeFactory.createPoint2D(
									mapX, mapY);
							newRoi.addGeometry(geometry); // Geometria
						}
					}
				}
				percent = (int) j * 100 / nY;
			}
		}
