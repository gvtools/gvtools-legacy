...
/**
 * Clase que define la interfaz que gestiona el grafico de dispersion. Permite
 * la seleccion de las bandas por parte del usuario y el acceso al gestor de
 * ROis del grafico.
 * 
...
public class ScatterPlotPanel extends DefaultButtonsPanel implements
		ActionListener {
	...
	/**
	 * @return Panel con los combos de seleccion de bandas y y botton de acceso
	 *         al gestor de ChartROIs
	 * */
	public JPanel getBandPanel() {
		...
		if (selectionBandPanel == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "bandas"));
			...
			panelX.setLayout(bd);
			bd.setHgap(3);
			panelX.add(new JLabel(PluginServices.getText(this, "ejex")),
					BorderLayout.CENTER);
			...
			panelY.add(new JLabel(PluginServices.getText(this, "ejey")),
					BorderLayout.CENTER);
	...
	public JPanel getPanelRoiButton() {
		...
		if (panelRoiButton == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "rois"));
			topBorder.setTitlePosition(TitledBorder.TOP);
	...
	public JComboBox getComboX() {
		if (comboX == null) {
			DefaultComboBoxModel defComboBand = new DefaultComboBoxModel(
					getBand());
	...
	public ChartScaterPlotPanel getPanelGrafico() {
		if (panelGrafico == null) {
			...
			if (buffer.getBandCount() > 1)
				...
				// si solo hay una banda se construye el grafico con la misma
				// banda en x y en y
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == getComboX()) {
			...
			panelGrafico.updateChart(getColorChart().getColor(),
					getColorBackground().getColor(), getColorExternal()
							.getColor());
	...
	public JPanel getOptionsPanel() {
		if (optionsPanel == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "colors"));
			topBorder.setTitlePosition(TitledBorder.TOP);
			...
			constrains.gridy = 0;
			optionsPanel.add(
					new JLabel(PluginServices.getText(this, "grafico")),
					constrains);
			constrains.gridx = 0;
			constrains.gridy = 1;
			...
			constrains.gridx = 1;
			constrains.gridy = 1;
			optionsPanel.add(new JLabel(PluginServices.getText(this, "fondo")),
					constrains);
			...
			constrains.gridy = 0;
			optionsPanel.add(
					new JLabel(PluginServices.getText(this, "exterior")),
					constrains);
