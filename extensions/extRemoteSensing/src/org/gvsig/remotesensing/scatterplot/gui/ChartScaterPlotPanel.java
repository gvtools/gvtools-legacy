...
/**
 * Clase que define el panel donde aparece el grafico de dispersion para dos
 * bandas, determinadas por las variables bandaX y bandaY. El grafico es un
 * FastScatterPlot de la libreria jfreeChart (personalizada para este apartado).
 * 
...
public class ChartScaterPlotPanel extends JPanel implements IProcessActions {
	 ...
	 *            capa
	 * @param band1
	 *            banda que se representa en el eje x.
	 * @param band2
	 *            banda que se representa en el eje y.
	 * */
	...
	public ScatterPlotDiagram getChart() {
		if (jPanelChart == null) {
			jPanelChart = new ScatterPlotDiagram(chart, (FLyrRasterSE) fLayer,
					new int[] { bandaX, bandaY });
			chart = null;
			jPanelChart.setBorder(javax.swing.BorderFactory.createLineBorder(
	 ...
	 * */
	public void updateChartPanel(Color colorChart, Color colorbackground,
			Color colorExternal) {
		/*
		 * chart.setBackgroundPaint(colorExternal); plot.setPaint(colorChart);
	...
	/**
	 * Actualizacion del grafico. Esta actualizacion se producira cuando haya
	 * variacion en la seleccion de las bandas por parte del usuario.
	 * 
	 * */
	public void updateChart(Color colorChart, Color colorBackground,
			Color colorExternal) {

		NumberAxis domainAxis = new NumberAxis(PluginServices.getText(this,
				"banda") + (bandaX + 1));
		domainAxis.setAutoRangeIncludesZero(false);
		NumberAxis rangeAxis = new NumberAxis(PluginServices.getText(this,
				"banda") + (bandaY + 1));
		...
		/*
		 * setDataChart(fLayer); plot = new ScatterPlotChart(this.data,
		 * domainAxis, rangeAxis);
		 * 
		 * chart = new
		 * JFreeChart(PluginServices.getText(this,"diagrama_dispersion"), plot);
		 * chart.getRenderingHints().clear(); data=null;
		 */
	...
	/**
	 * Metodo que establece los datos a representar. El vector data[][] se
	 * rellena con los valores de cada punto para las bandas bandX y bandY.
	 * 
	...
	public void setDataChart(FLayer fLayer) throws InterruptedException {
		...
		if (dataGrid.getDataType() == IBuffer.TYPE_BYTE) {
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {
					data[0][indice] = dataGrid.getRasterBuf().getElemByte(j, i,
							0) & 0xff;
					data[1][indice] = dataGrid.getRasterBuf().getElemByte(j, i,
							1) & 0xff;
					indice++;
				}
			}
		...
		if (dataGrid.getDataType() == IBuffer.TYPE_SHORT) {
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {
					data[0][indice] = dataGrid.getRasterBuf().getElemShort(j,
							i, 0);
					data[1][indice] = dataGrid.getRasterBuf().getElemShort(j,
							i, 1);
					indice++;
				}
			}
		...
		if (dataGrid.getDataType() == IBuffer.TYPE_INT) {
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {
					data[0][indice] = dataGrid.getRasterBuf().getElemInt(j, i,
							0);
					data[1][indice] = dataGrid.getRasterBuf().getElemInt(j, i,
							1);
					indice++;
				}
			}
		...
		if (dataGrid.getDataType() == IBuffer.TYPE_FLOAT) {
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {
					data[0][indice] = dataGrid.getRasterBuf().getElemFloat(j,
							i, 0);
					data[1][indice] = dataGrid.getRasterBuf().getElemFloat(j,
							i, 1);
					indice++;
				}
			}
		...
		if (dataGrid.getDataType() == IBuffer.TYPE_DOUBLE) {
			for (int j = 0; j < height; j++) {
				for (int i = 0; i < width; i++) {
					data[0][indice] = (float) dataGrid.getRasterBuf()
							.getElemDouble(j, i, 0);
					data[1][indice] = (float) dataGrid.getRasterBuf()
							.getElemDouble(j, i, 1);
					indice++;
				}
			}
