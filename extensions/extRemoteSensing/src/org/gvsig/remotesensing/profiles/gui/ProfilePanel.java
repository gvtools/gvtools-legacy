...
/**
 * Clase que define el panel donde aparece el grafico del perfil espectral de
 * los puntos que seleccione el usuario.
 * 
...
public class ProfilePanel extends DefaultButtonsPanel implements ChangeListener {
	 ...
	 * 
	 * @param flayer
	 *            capa de la que se van a tomar los puntos
	 **/
	...
	public GraphicChartPanel getChart() {
		if (jPanelChart == null) {
			...
			this.jPanelChart.getChart().getChart().getPlot()
	...
	public JPanel getSouthPanel() {
		if (southPanel == null) {
			...
			getTabbedPane().addTab(PluginServices.getText(this, "lineProfile"),
					lineOptionsPanel);
			pointOptionsPanel = new ZProfileOptionsPanel(this);
			pointOptionsPanel.setJPanelChart(getChart());
			getTabbedPane().addTab(PluginServices.getText(this, "zProfile"),
					pointOptionsPanel);
		...
		if (e.getSource().equals(getTabbedPane())) {
			...
			if (nextActiveChart == PANELZPROFILE) {
				...
				try {
					...
					int selectedRow = getLineOptionsPanel().getTable()
							.getSelectedRow();
					getLineOptionsPanel().getJPanelChart();
					if (selectedRow == -1)
						return;
				} catch (NotInitializeException e1) {
					RasterToolsUtil.messageBoxError("tabla_no_inicializada",
							this, e1);
				}
			}
		}
