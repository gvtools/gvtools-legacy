...
public class LineProfileOptionsListener implements ButtonsPanelListener,
	...
	public void actionPerformed(ActionEvent e) {
		...
		// Construccion de la roi (vacia) que espera el dibujado de la geometria
		// de tipo punto
		// selección de la herramienta de dibujado de point

		if (e.getSource() == optionsPanel.getNewButton()) {
			try {
				String roiName = PluginServices.getText(this, "line")
						+ String.valueOf(createRois);
				createRois++;
				Object row[] = ((IModel) optionsPanel.getTable().getModel())
						.getNewLine();
				row[0] = roiName;
				((DefaultTableModel) optionsPanel.getTable().getModel())
						.addRow(row);
				...
				optionsPanel.getTable().setSelectedIndex(
						optionsPanel.getTable().getRowCount() - 1);

			} catch (NotInitializeException e1) {
				RasterToolsUtil.messageBoxError("tabla_no_inicializada", this,
						e1);
			}
		...
		// Seleccionada la banda se actualiza la grafica y los datos en la tabla
		// de la roi seleccionada
		if (e.getSource() == optionsPanel.getComboBands()) {

			try {
				if (optionsPanel.getTable().getRowCount() > 0) {
					ROI lineRoi = (VectorialROI) optionsPanel.getROIs().get(
							optionsPanel.getTable().getSelectedRow());
		...
		if (e.getSource() == optionsPanel.getDeleteButton()) {
			...
			try {
				if (optionsPanel.getTable().getRowCount() > 0) {
					...
					String className = (String) optionsPanel
							.getTable()
							.getModel()
							.getValueAt(
									optionsPanel.getTable().getSelectedRow(), 0);
					optionsPanel.removeROI(className);
					((DefaultTableModel) optionsPanel.getTable().getModel())
							.removeRow(optionsPanel.getTable().getSelectedRow());

					if (optionsPanel.getTable().getRowCount() > 0) {
						optionsPanel.getTable().setSelectedIndex(0);
						ROI lineRoi = (VectorialROI) optionsPanel.getROIs()
								.get(optionsPanel.getTable().getSelectedRow());
						int band = optionsPanel.getComboBands()
								.getSelectedIndex();
						drawChartRoi((VectorialROI) lineRoi, band);
					}
					delete = false;
	...
	public void tableChanged(TableModelEvent e) {
		...
		if (e.getColumn() == 1) {
			String roiName = (String) optionsPanel.getTable().getTable()
			...
			if (optionsPanel.getMapControl() != null) {
				GraphicLayer graphicLayer = optionsPanel.getMapControl()
						.getMapContext().getGraphicsLayer();
				ISymbol symbol = null;
				Color color = (Color) optionsPanel.getTable().getTable()
						.getJTable().getValueAt(e.getFirstRow(), 1);
	...
	public void valueChanged(ListSelectionEvent e) {
		try {
			if (optionsPanel.getTable().getRowCount() > 0 && !delete) {
				VectorialROI lineRoi = (VectorialROI) optionsPanel.getROIs()
						.get(optionsPanel.getTable().getSelectedRow());
				int band = optionsPanel.getComboBands().getSelectedIndex();
	...
	/**
	 * Metodo que se encarga de pintar la roi que se pasa como parametro sobre
	 * el grafico. La banda de la que se tomarán los valores es la que se pasa
	 * como parámetro.
	 * 
	...
	private void drawChartRoi(VectorialROI roi, int band)
		...
		try {
			...
			optionsPanel.getTable().getModel()
