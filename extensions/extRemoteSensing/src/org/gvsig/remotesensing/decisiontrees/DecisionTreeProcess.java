...
/**
 * Proceso para la generación de un raster de clasificación mediante un árbol de
 * decisión.
 * 
	...
	private void loadLAyer() {
		...
		try {
			...
			AffineTransform aTransform = new AffineTransform(
					resultExtent.getCellSizeX(), 0.0, 0.0,
					-resultExtent.getCellSizeY(), resultExtent.getMin().getX(),
					resultExtent.getMax().getY());
			GeoRasterWriter grw = GeoRasterWriter.getWriter(writerBufferServer,
					fileName, rasterResult.getBandCount(), aTransform,
	...
	public void process() throws InterruptedException {
		...
		try {
			...
			for (int x = 0; x < iNX; x++) {
				...
				for (int y = 0; y < iNY; y++) {
					int i = 0;
					for (Iterator iter = varsTable.keySet().iterator(); iter
							.hasNext();) {
