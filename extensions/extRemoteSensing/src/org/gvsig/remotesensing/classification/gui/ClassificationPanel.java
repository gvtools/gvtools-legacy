...
/**
 * Clase que define la interfaz del proceso de clasificacion de una imagen.
 * Recoge la imagen a clasificar con la selección de bandas correspondiente.
 * Posibilita la elección del método empleado y da la posibilidad de manejar la
 * lista de clases que intervienen en el proceso. Permite ajustar algunos
 * parametros del proceso y configurar las opciones de salida.
 * 
 * @author Alejandro Muñoz Sanchez (alejandro.munoz@uclm.es)
 * @author Diego Guerrero Sevilla (diego.guerrero@uclm.es)
 * @version 19/10/2007
 */
public class ClassificationPanel extends DefaultButtonsPanel implements
	...
	private void initialize() {
		...
		this.setBorder(new CompoundBorder(topBorder,
	...
	private JPanel getPanelTab1() {
		if (panelTab1 == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "raster"));
			topBorder.setTitlePosition(TitledBorder.TOP);
			...
			TitledBorder topBorderMethod = BorderFactory
	...
	JPanel getPanelParametros() {
		if (panelParametros == null) {
			...
			panelParametros.add(new JLabel(PluginServices.getText(this,
	...
	JPanel getPanelParametros2() {
		if (panelParametros2 == null) {
			...
			panelParametros2.add(
					new JLabel(PluginServices.getText(this, "numclases")),
					BorderLayout.NORTH);
	...
	/**
	 * @return Scroll con la tabla de clases intervienen en el proceso de
	 *         clasificación.
	 */
	public JScrollPane getScrollTable() {
		...
		if (scrollTable == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder((PluginServices.getText(this, "clases")));
			topBorder.setTitlePosition(TitledBorder.TOP);
			scrollTable.setBorder(new CompoundBorder(topBorder,
	...
	public void accept() {
		...
		FLyrRasterSE rasterLayer = ((FLyrRasterSE) layers
				.getLayer((String) getComboCapas().getSelectedItem()));
		/*
		...
		if (getComboMethods().getSelectedIndex() == 2) {
			...
			process.addParam("dev",
					new Double(Double.parseDouble(getStevField().getText())));
		}
		if (getComboMethods().getSelectedIndex() == 3) {
			process = new NoSupervisedClassificationProcess();
			process.addParam("numClases",
					new Integer(Integer.parseInt(getNumClases().getText())));
		}
	...
	public void actionButtonPressed(ButtonsPanelEvent e) {
		...
		if (e.getButton() == ButtonsPanel.BUTTON_ACCEPT) {
			if (!(getRoisTablePanel().getROIs().isEmpty() && getComboMethods()
					.getSelectedIndex() != 3)) {
				accept();
				close();
			} else
				JOptionPane.showMessageDialog(null,
						PluginServices.getText(this, "no_clases"),
						PluginServices.getText(this, "no_clases"),
		...
		if (e.getButton() == ButtonsPanel.BUTTON_APPLY) {
			if (!(getRoisTablePanel().getROIs().isEmpty() && getComboMethods()
				...
				JOptionPane.showMessageDialog(null,
						PluginServices.getText(this, "no_clases"),
						PluginServices.getText(this, "clasificacion"),
						JOptionPane.WARNING_MESSAGE);
		}
	...
	public JComboBox getComboMethods() {
		if (comboMethods == null) {
			...
			sNames[0] = new String(PluginServices.getText(this,
					"metodo_maxima_probabilidad"));
			sNames[1] = new String(PluginServices.getText(this,
					"metodo_minima_diatancia"));
			sNames[2] = new String(PluginServices.getText(this,
					"metodo_paralelepipedos"));
			sNames[3] = new String(PluginServices.getText(this,
	...
	public void getBandas() {
		...
		if (layers.getLayer((String) getComboCapas().getSelectedItem()) instanceof FLyrRasterSE) {
			...
			try {
				getRoisTablePanel().setFLayer(
						(FLyrRasterSE) layers.getLayer((String) getComboCapas()
								.getSelectedItem()));
