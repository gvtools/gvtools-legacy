 ...
 * @version 19/10/2007
 */
public class ClassificationMinimumDistanceProcess extends
	...
	/**
	 * Método que implementa el clasificador de mínima distancia. Para cada
	 * pixel, obtiene la clase para la que la distancia euclídea respecto al
	...
	public int getPixelClassForTypeByte(byte[] pixelBandsValues) {
		...
		for (int clase = 0; clase < numClases; clase++) {
			...
			for (int i = 0; i < bandCount; i++) {
				y[i] = (pixelBandsValues[i] - means[clase][i])
						* (pixelBandsValues[i] - means[clase][i]);
				// y[i]=Math.pow((pixelBandsValues[i]-means[clase][i]),2);
				distancia += y[i];
			}
			// distancia= Math.sqrt(distancia); //No hacer la raiz: Para
			// comparar da igual la distancia que la distancia al cuadrado...
			if (clase == 0)
	 ...
	 * 
	 * @param array
	 *            de tipo short con los valores del pixel en cada una de las
	 *            bandas
	 * @return clase a la que pertenece el pixel
	 */
	...
	/**
	 * Metodo que recoge los parametros del proceso de clasificacion de mínima
	 * distancia <LI>rasterSE: Capa de entrada para la clasificación</LI> <LI>
