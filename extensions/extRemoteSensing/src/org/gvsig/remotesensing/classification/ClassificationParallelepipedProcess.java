...
public class ClassificationParallelepipedProcess extends
	...
	public int getPixelClassForTypeByte(byte[] pixelBandsValues) {
		for (int clase = 0; clase < numClases; clase++) {
			...
			for (int i = 0; i < bandCount; i++) {
				if (!((medias[clase][i] - stevcoef
						* Math.sqrt(varianza[clase][i]) < pixelBandsValues[i]) && (pixelBandsValues[i] < medias[clase][i]
						+ stevcoef * Math.sqrt(varianza[clase][i])))) {
					inClass = false;
				}
			}
			if (inClass == true)
	...
	/**
	 * Metodo que recoge los parametros del proceso de clasificacion de por
	 * paralelepipedos <LI>rasterSE: Capa de entrada para la clasificación</LI>
	 * <LI>rois: lista de rois</LI> <LI>bandList:bandas habilitadas</LI> <LI>
	 * view: vista sobre la que se carga la capa al acabar el proceso</LI> <LI>
	 * filename: path con el fichero de salida</LI> <LI>stevcoef: coeficiente
	 * que establece el de desviacion maxima</LI>
	 */
	public void init() {
