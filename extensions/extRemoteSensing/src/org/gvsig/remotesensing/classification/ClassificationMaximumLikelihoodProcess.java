...
/**
 * ClassificationMaximumLikelihoodProcess implementa el método de clasificación
...
public class ClassificationMaximumLikelihoodProcess extends
	 ...
	 * pixel, obtiene la clase que minimiza la expresion: -Ln(P(x))=
	 * Ln(|Si|)+Y'*inverse(Si)*Y
	 * 
	 * @param array
	 *            de tipo byte con valores del pixel en cada una de las bandas
	...
	/**
	 * Metodo que implementa el clasificador de maxima probabilidad. Para cada
	 * pixel, obtiene la calase que minimiza la expresion: -Ln(P(x))=
	 * Ln(|Si|)+Y'* inverse(Si)*Y
	 * 
	 * @param array
	 *            de tipo short con valores del pixel en cada una de las bandas
	 * @return clase a la que pertenece el pixel (por el metodo de maxima
	 *         probabilidad)
	 */
	 ...
	 * 
	 * @param array
	 *            de tipo int con valores del pixel en cada una de las bandas
	 * @return clase a la que pertenece el pixel (por el metodo de maxima
	 *         probabilidad)
	 */
	 ...
	 * 
	 * @param array
	 *            de tipo float con valores del pixel en cada una de las bandas
	 * @return clase a la que pertenece el pixel (por el metodo de maxima
	 *         probabilidad)
	 */
	 ...
	 * 
	 * @param array
	 *            de tipo double con valores del pixel en cada una de las bandas
	 * @return clase a la que pertenece el pixel (por el metodo de maxima
	 *         probabilidad)
	 */
	...
	/**
	 * Metodo que recoge los parametros del proceso de clasificacion de máxima
	 * probabilidad <LI>rasterSE: Capa de entrada para la clasificación</LI> <LI>
	 * rois: lista de rois</LI> <LI>bandList:bandas habilitadas</LI> <LI>view:
	 * vista sobre la que se carga la capa al acabar el proceso</LI> <LI>
	 * filename: path con el fichero de salida</LI>
	 */
	public void init() {
	...
	public void process() throws InterruptedException {
		...
		for (int clase = 0; clase < numClases; clase++)
			for (int i = 0; i < bandCount; i++) {
				...
				try {
					...
					RasterToolsUtil
							.messageBoxError(
									PluginServices.getText(this, "grid_error"),
									this, e);
				}
			}
		...
		// Se calculan las inversas de las matrices de Varianza-covarianza de
		// todas las rois y se almacenan en inverseVarCovMAtrix
		Matrix S = null;
		...
		for (int i = 0; i < numClases; i++) {
			...
			if (bandList.length != rasterSE.getBandCount()) {
				/*
				 * Extraer la submatiz correspondiente a las bandas que
				 * intervienen:
				 */
