...
public class FusionListener implements ActionListener,
		PropertiesComponentListener, ChangeListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == fusionDialog.getMainPanel().getJComboMethods()) {
			if (fusionDialog.getMainPanel().getJComboMethods()
					.getSelectedIndex() == 0
					|| fusionDialog.getMainPanel().getJComboMethods()
							.getSelectedIndex() == 1) {
				fusionDialog.getNewLayerPanel().getRadioAllBand()
						.setEnabled(false);
				fusionDialog.getNewLayerPanel().getRadioAsignedBand()
						.setSelected(true);
				fusionDialog.getNewLayerPanel().getRadioAllBand()
						.setSelected(false);
			}
	...
	void updatePanel() {
		...
		fusionDialog.getMainPanel().getPanel(
				fusionDialog.getMainPanel().getJComboMethods()
						.getSelectedIndex());
	...
	public void accept() {
		...
		HashMap params = fusionDialog.getMainPanel().getParameterPanel()
				.getParams();
		...
		if (fusionDialog.getMainPanel().getJComboMethods().getSelectedIndex() == 0) {
			...
			proceso.addParam("high_band", fusionDialog.getMainPanel()
					.getBandPanel().getLayer());
			proceso.addParam("band", new Integer(fusionDialog.getMainPanel()
					.getBandPanel().getModeloTabla().getFirstSelected()));
			proceso.addParam("filename", fusionDialog.getNewLayerPanel()
					.getFileSelected());
			proceso.start();
		} else if (fusionDialog.getMainPanel().getJComboMethods()
				.getSelectedIndex() == 1) {
			...
			proceso.start();
				...
				.getSelectedIndex() == 2)// .getIDPanel()==PluginServices.getText(this,"pc")){
		{
			...
			proceso.start();
				...
				.getSelectedIndex() == 3)// .getIDPanel()==PluginServices.getText(this,"pc")){
		{
			...
			if (fusionDialog.getNewLayerPanel().getRadioAllBand().isSelected()) {
				bands = new int[((FLyrRasterSE) fusionDialog.getLayer())
						.getBandCount()];
				for (int i = 0; i < ((FLyrRasterSE) fusionDialog.getLayer())
						.getBandCount(); i++)
					bands[i] = i;
			} else {
				bands = ((FLyrRasterSE) fusionDialog.getLayer())
						.getRenderBands();
			}
