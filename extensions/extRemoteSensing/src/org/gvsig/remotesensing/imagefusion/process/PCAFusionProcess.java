...
public class PCAFusionProcess extends ImageFusionProcess {
	...
	// bandas para las que se realiza el procedimiento. Pueden ser 3 caso de
	// opcion
	// bandas de visualización o todas las bandas del raster en caso de elegirse
	// la opcion allBands
	int bands = 0;
	// Proceso para el cálculo de estadisticas de componentes principales
	PCStatisticsProcess calcStatisticProcess = null;

	// Proceso para la obtencion de la imagen de componentes principales (imagen
	// tipo float)
	...
	public void process() throws InterruptedException {
		...
		calcStatisticProcess.addParam("selectedBands", new boolean[] { true,
				true, true });
		calcStatisticProcess.run();

		// Imagen resultante componentes principales
		PCStatistics pcStatistics = new PCStatistics(
				calcStatisticProcess.getAutoVectorMatrix(),
				(double[]) (((PCStatistics) (calcStatisticProcess.getResult()))
						.getAutovalues()), null);

		imageProcess = new PCImageProcess();
		imageProcess.addParam("inputRasterLayer",
				calcStatisticProcess.getRasterLayer());
		imageProcess.addParam("statistics", pcStatistics);
		// TODO: Cambiar bandas de visualizacion o todas las bandas
		imageProcess.addParam("selectedComponents", new boolean[] { true, true,
				true });
		imageProcess.addParam("selectedBands",
		...
		double order[][] = new double[autoV.getRowDimension()][autoV
				.getColumnDimension()];
		...
		// Supersampleo de la multiespectral a la pancromatica. A la vez aplicar
		// transformacion inversa PCA-1 y obtener la imagen.
		IBuffer pcTmp = RasterBuffer.getBuffer(RasterBuffer.TYPE_FLOAT,
				highBandGrid.getRasterBuf().getWidth(), highBandGrid
						.getRasterBuf().getHeight(), pcResult.getBandCount(),
				true);
			...
			for (int col = 0; col < pcResult.getWidth(); col++) {
				...
				for (int facx = 0; facx < cellFactorX; facx++) {
					...
					for (int facy = 0; facy < cellFactorY; facy++) {
						...
						pcTmp.setElem(line, colum, 0,
								(float) (getValuePanc(line, colum)));
