...
public class BroveyFusionProcess extends ImageFusionProcess {
	...
	public void process() throws InterruptedException {
		...
		if (inputGrid.getRasterBuf().getDataType() == RasterBuffer.TYPE_BYTE) {
			rasterResult = RasterBuffer.getBuffer(RasterBuffer.TYPE_BYTE,
					highBandGrid.getRasterBuf().getWidth(), highBandGrid
							.getRasterBuf().getHeight(), 3, true);
			for (int iLine = 0; iLine < heightMultiespec; iLine++) {
				pRGBMultiespectralByte = inputGrid.getRasterBuf().getLineByte(
						iLine);
		...
		if (inputGrid.getRasterBuf().getDataType() == RasterBuffer.TYPE_SHORT) {
			...
			rasterResult = RasterBuffer.getBuffer(RasterBuffer.TYPE_SHORT,
					highBandGrid.getRasterBuf().getWidth(), highBandGrid
							.getRasterBuf().getHeight(), 3, true);
			for (int iLine = 0; iLine < heightMultiespec; iLine++) {
				pRGBMultiespectralShort = inputGrid.getRasterBuf()
						.getLineShort(iLine);
		...
		if (inputGrid.getRasterBuf().getDataType() == RasterBuffer.TYPE_INT) {
			...
			rasterResult = RasterBuffer.getBuffer(RasterBuffer.TYPE_INT,
					highBandGrid.getRasterBuf().getWidth(), highBandGrid
							.getRasterBuf().getHeight(), 3, true);
			for (int iLine = 0; iLine < heightMultiespec; iLine++) {
				pRGBMultiespectralInt = inputGrid.getRasterBuf().getLineInt(
						iLine);
		...
		if (inputGrid.getRasterBuf().getDataType() == RasterBuffer.TYPE_FLOAT) {
			...
			rasterResult = RasterBuffer.getBuffer(RasterBuffer.TYPE_FLOAT,
					highBandGrid.getRasterBuf().getWidth(), highBandGrid
							.getRasterBuf().getHeight(), 3, true);
			for (int iLine = 0; iLine < heightMultiespec; iLine++) {
				pRGBMultiespectralFloat = inputGrid.getRasterBuf()
						.getLineFloat(iLine);
	...
	public String getLog() {
		if (writerBufferServer == null)
			return PluginServices.getText(this, "fusionando_imagen") + "..."
					+ "\n\nMétodo seleccionado:"
					+ PluginServices.getText(this, "brovey");
		else
