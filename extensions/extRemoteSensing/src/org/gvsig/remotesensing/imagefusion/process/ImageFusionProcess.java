...
/**
 * ImageFusionProccess es la clase que representa el proceso general de fusion
 * de una imagen
 * 
...
public abstract class ImageFusionProcess extends RasterProcess {
	...
	/**
	 * Metodo que recoge los parametros del proceso de fusion de imagenes. <LI>
	 * rasterSE: Capa de entrada para la fusion</LI> <LI>bandaHigth: Banda alta
	 * resolucion</LI> <LI>metodo: metodo seleccionado</LI> <LI>view: vista
	 * sobre la que se carga la capa al acabar el proceso</LI> <LI>filename:
	 * path con el fichero de salida</LI>
	 */
	public void init() {
		...
		/**
		 * if((rasterSE.getDataSource().getExtent().getLLX() !=
		 * rasterBand.getDataSource().getExtent().getLLX()) ||
		 * (rasterSE.getDataSource().getExtent().getLLX() !=
		 * rasterBand.getDataSource().getExtent().getLLX()) ||
		 * (rasterSE.getDataSource().getExtent().getLLX() !=
		 * rasterBand.getDataSource().getExtent().getLLX()) ||
		 * (rasterSE.getDataSource().getExtent().getLLX() !=
	...
	public void initparams() {
		...
		if (rasterBand == null) {
			try {
				...
				RasterToolsUtil.messageBoxError(
						PluginServices.getText(this, "no_band_selected"), this);
				return;
			} catch (Throwable e) {
				e.printStackTrace();
			}
		...
		try {
			...
			cellFactorX = (double) highBandGrid.getRasterBuf().getWidth()
					/ inputGrid.getRasterBuf().getWidth();
			cellFactorY = (double) highBandGrid.getRasterBuf().getHeight()
					/ inputGrid.getRasterBuf().getHeight();
	...
	public void writeToFile() {
		try {
			...
			grw = GeoRasterWriter.getWriter(writerBufferServer, filename,
					...
					rasterBand.getAffineTransform(), rasterResult.getWidth(),
