...
/**
 * TasseledCapProccess es la clase que implementa el proceso de transformación
 * Tasseled Cap. La Trasformación es aplicable a imagenes LandSat MS, LandSatTM
 * y LandSantETM
 * 
...
public class TasseledCapProcess extends RasterProcess {
	...
	public void setBuffer() {
		...
		try {
			...
			RasterToolsUtil
					.messageBoxError(
							PluginServices.getText(this, "error_preview_stop"),
							this, e);
		}
	}
	...
	public void process() throws InterruptedException {
		if (!exec)
			return;
		setBuffer();
		GridExtent layerExtent = new GridExtent(rasterSE.getFullRasterExtent(),
				rasterSE.getCellSize());

		// TODO: Tipo salida float
		rasterResult = RasterBuffer.getBuffer(IBuffer.TYPE_FLOAT,
				layerExtent.getNX(), layerExtent.getNY(), matrixParams.length,
				true);
				...
				for (int j = 0; j < iNX; j++) {
					for (int k = 0; k < matrixParams.length; k++) {
						for (int s = 0; s < matrixParams[0].length; s++) {
							valor += (double) (buffer.getElemByte(i, j, s) & 0xff)
									* matrixParams[k][s];
						}
		...
		if (buffer.getDataType() == IBuffer.TYPE_SHORT) {
			for (int i = 0; i < iNY; i++) {
				for (int j = 0; j < iNX; j++) {
					for (int k = 0; k < matrixParams.length; k++) {
						for (int s = 0; s < matrixParams[0].length; s++) {
							valor += (double) buffer.getElemShort(i, j, s)
									* matrixParams[k][s];
						}
						rasterResult.setElem(i, j, k, (float) valor);
						valor = 0;
		...
		if (buffer.getDataType() == IBuffer.TYPE_INT) {
			for (int i = 0; i < iNY; i++) {
				for (int j = 0; j < iNX; j++) {
					for (int k = 0; k < matrixParams.length; k++) {
						for (int s = 0; s < matrixParams[0].length; s++) {
							valor += (double) buffer.getElemInt(i, j, s)
									* matrixParams[k][s];
						}
						rasterResult.setElem(i, j, k, (float) valor);
						valor = 0;
		...
		if (buffer.getDataType() == IBuffer.TYPE_FLOAT) {
			for (int i = 0; i < iNY; i++) {
				for (int j = 0; j < iNX; j++) {
					for (int k = 0; k < matrixParams.length; k++) {
						for (int s = 0; s < matrixParams[0].length; s++) {
							valor += (double) buffer.getElemFloat(i, j, s)
									* matrixParams[k][s];
						}
						rasterResult.setElem(i, j, k, (float) valor);
						valor = 0;
		...
		if (buffer.getDataType() == IBuffer.TYPE_DOUBLE) {
			for (int i = 0; i < iNY; i++) {
				for (int j = 0; j < iNX; j++) {
					for (int k = 0; k < matrixParams.length; k++) {
						for (int s = 0; s < matrixParams[0].length; s++) {
							valor += (double) buffer.getElemDouble(i, j, s)
									* matrixParams[k][s];
						}
						rasterResult.setElem(i, j, k, (float) valor);
						valor = 0;
		...
		try {
			grw = GeoRasterWriter.getWriter(writerBufferServer, fileName,
					rasterResult.getBandCount(), aTransform,
	...
	public String getLog() {
		return PluginServices.getText(this, "apply_tasseledCap")
				+ PluginServices.getText(this, "transformation_type")
				+ transformType;
	}
