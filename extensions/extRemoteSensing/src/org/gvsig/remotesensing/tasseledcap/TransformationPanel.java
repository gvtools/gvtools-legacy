...
/**
 * Clase que implementa la interfaz para las transformaciones multiespectrales
 * de Componentes Principales y TasseledCap.
 * 
 * @author aMuÑoz (alejandro.munoz@uclm.es)
 * @version 30/4/2008
 * */

public class TransformationPanel extends DefaultButtonsPanel implements
		PropertyListener, IWindow, IWindowListener, ButtonsPanelListener,
		ActionListener, FocusListener, IProcessActions {
	...
	public WindowInfo getWindowInfo() {
		...
		m_viewinfo.setTitle(PluginServices
				.getText(this, "transformacion_title"));
		m_viewinfo.setX(300);
		return m_viewinfo;
	}
	...
	public JPanel getSelectionImagePanel() {
		if (panelSeleccion == null) {
			...
			TitledBorder topBorder = BorderFactory
					.createTitledBorder(PluginServices.getText(this, "imagen"));
			topBorder.setTitlePosition(TitledBorder.TOP);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getSource() == getRTasseledCapOption()) {
			getTypeTasseledCap().setEnabled(
					getRTasseledCapOption().isSelected());
		}
	...
	public JRadioButton getRPrincipalComponentOption() {
		if (rPrincipalComponentOption == null) {
			rPrincipalComponentOption = new JRadioButton(
					PluginServices.getText(this, "principal_component"));
	...
	public JRadioButton getRTasseledCapOption() {
		if (rTasseledCapOption == null) {
			rTasseledCapOption = new JRadioButton(PluginServices.getText(this,
					"tasseledcap"));
	...
	public JComboBox getTypeTasseledCap() {
		if (typeTasseledCap == null) {
			...
			typeTasseledCap.addItem(new String(PluginServices.getText(this,
					"landsatMS")));
			typeTasseledCap.addItem(new String(PluginServices.getText(this,
					"landsatTM")));
			typeTasseledCap.addItem(new String(PluginServices.getText(this,
	...
	public void aplicar() {
		...
		if (filename != null) {
			...
			if (getRTasseledCapOption().isSelected()) {
				// Comprobaciones
				if (bandList.length != 4
						&& getTypeTasseledCap().getSelectedIndex() == 0) {
					RasterToolsUtil.messageBoxInfo(
							PluginServices.getText(this, "transformMSS_error"),
							"aviso");
					return;
				} else if (bandList.length != 6
						&& getTypeTasseledCap().getSelectedIndex() == 1) {
					RasterToolsUtil.messageBoxInfo(
							PluginServices.getText(this, "transformTM_error"),
							"aviso");
					return;
				} else if (bandList.length != 6
						&& getTypeTasseledCap().getSelectedIndex() == 2) {
					RasterToolsUtil.messageBoxInfo(
							PluginServices.getText(this, "transformETM_error"),
							"aviso");
					return;
				}
					...
					process.addParam("type", new Integer(getTypeTasseledCap()
							.getSelectedIndex()));
	...
	public void end(Object param) {
		...
		PrincipalComponentCalculusPanel altPrincipalComponentCalculusPanel = new PrincipalComponentCalculusPanel(
				view, (PCStatistics) param, statisticsProcess, selectedBands);
		altPrincipalComponentCalculusPanel.setFilename(fileName);
		PluginServices.getMDIManager().addWindow(
				altPrincipalComponentCalculusPanel);
	}
