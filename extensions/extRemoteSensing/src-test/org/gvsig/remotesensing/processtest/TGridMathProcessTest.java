...
/**
 * Este test prueba el proceso de construcción de la imagen resultante al operar
 * las dos bandas, elemento a elemento, de la imagen pc_CreateImageTest.tif.
 * 
...
public class TGridMathProcessTest extends TestCase {
	...
	public void testStack() {
		try {
			...
			cp.getCalculatorPanel()
					.getQWindowsHash()
					.put("b1",
							new Object[] { g.getRasterBuf(), new Integer(2) });
			cp.getCalculatorPanel()
					.getQWindowsHash()
					.put("b2",
							new Object[] { g2.getRasterBuf(), new Integer(2) });
			proceso.addParam("params", cp.getCalculatorPanel()
					.getQWindowsHash());
			proceso.run();
