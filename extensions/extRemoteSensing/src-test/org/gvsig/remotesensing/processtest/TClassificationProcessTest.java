...
/**
 * Este test prueba el proceso de clasificación de una imagen de 4x4 con tres
 * bandas.El proceso de clasificación se realiza por minima distancia, maxima
 * probabilidad y paralelepipedos. Los resultados se comparan con el resultado
 * teórico válido
 * 
...
public class TClassificationProcessTest extends TestCase {
	...
	public void testStack() {
		...
		path.moveTo(dataGrid.getGridExtent().getULX(), dataGrid.getGridExtent()
				.getULY());
		path.lineTo(
				dataGrid.getGridExtent().getULX()
						+ (2 * dataGrid.getCellSize()) - 0.3, dataGrid
						.getGridExtent().getULY());
		path.lineTo(
				dataGrid.getGridExtent().getULX()
						+ (2 * dataGrid.getCellSize()) - 0.3, dataGrid
						.getGridExtent().getULY()
						- 2
						* (dataGrid.getCellSize()) + 0.3);
		path.lineTo(dataGrid.getGridExtent().getULX(), dataGrid.getGridExtent()
				.getULY() - 2 * (dataGrid.getCellSize()) + 0.3);
		path.closePath();
		...
		path2.moveTo(dataGrid.getGridExtent().getLLX(), dataGrid
				.getGridExtent().getLLY());
		path2.lineTo(
				dataGrid.getGridExtent().getLLX()
						+ (2 * dataGrid.getCellSize()) - 0.3, dataGrid
						.getGridExtent().getLLY());
		path2.lineTo(
				dataGrid.getGridExtent().getLLX()
						...
						+ 2
						* (dataGrid.getCellSize()) - 0.3);
		path2.lineTo(dataGrid.getGridExtent().getLLX(), dataGrid
				.getGridExtent().getLLY() + 2 * (dataGrid.getCellSize()) - 0.3);
