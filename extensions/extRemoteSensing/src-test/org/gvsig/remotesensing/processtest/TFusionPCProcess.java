 ...
 * componetes principales calculadas. En el proceso de fusión de imágenes por el
 * método de PCA está básado en transformación inversa PCA.
 * 
...
public class TFusionPCProcess extends TestCase {
	...
	public void setUp() {
		...
		try {
			lyr = FLyrRasterSE.createLayer(path1, path1, null);
	...
	public void testStack() {
		...
		imgProcess
				.addParam("selectedBands", new boolean[] { true, true, true });
		imgProcess.addParam("selectedComponents", new boolean[] { true, true,
				true });
		...
		try {
			RasterBuffer rasterResult = RasterBuffer.getBuffer(
					RasterBuffer.TYPE_FLOAT,
					dataGrid.getRasterBuf().getWidth(), dataGrid.getRasterBuf()
							.getHeight(), dataGrid.getBandCount(), true);
			...
			// Comprobar que el resultado de la imagen original es el mismo que
			// tras aplicar la inversa a las componentes

			for (int i = 0; i < dataGrid.getRasterBuf().getHeight(); i++)
				for (int j = 0; j < dataGrid.getRasterBuf().getHeight(); j++) {
					assertEquals(
							rasterResult.getElemFloat(i, j, 0),
							(float) dataOrigen.getRasterBuf().getElemShort(i,
									j, 0), 0.1);
					assertEquals(
							rasterResult.getElemFloat(i, j, 1),
							(float) dataOrigen.getRasterBuf().getElemShort(i,
									j, 1), 0.1);
					assertEquals(
							rasterResult.getElemFloat(i, j, 2),
							(float) dataOrigen.getRasterBuf().getElemShort(i,
									j, 2), 0.1);
				}
	...
	public float[] solveSystem(Matrix matrix, float columResult[]) {
		...
		// flanagan.math.Matrix matrixFL = new
		// flanagan.math.Matrix(matrix.getArray());
