...
public class WMSLayerNode {
	...
	/**
	 * <p>
	 * min scale for the layer to be visible
	 * </p>
	 */
	...
	/**
	 * <p>
	 * max scale for the layer to be visible
	 * </p>
	 */
	 ...
	 */
	public void addDimension(String name, String units, String unitSymbol,
	...
	/**
	 * Sets the Latitude-Longitude box text to be shown in an user interface
	 * layer descriptor.
	 * 
	 * @param latLonBox
	 */
	...
	/**
	 * Returns the Latitude-Longitude box text to be shown in an user interface
	 * layer descriptor.
	 * 
	 * @return
	 */
	...
	/**
	 * When a server cannot renderize images but just server them in constant
	 * size and BBox, the layer must have this value set in order to correctly
	 * work.
	 * 
	 * @param fixedWidth
	 *            - the constant value for the image width
	 * @param fixedHeight
	 *            - the constant value for the image height
	 */
	...
	public FMapWMSStyle getSelectedStyle() {
		if (styles == null || selectedStyleIndex > styles.size() - 1
	...
	/**
	 * Marks the style of this layer given by its name as selected. If this
	 * layer has no style with this name, then the layer is marked as none
	 * selected.
	 * 
	 * @param style
	 *            name
	...
	/**
	 * Creates a new instance of WMSLayerNode containing a copy of this, but
	 * with no children and parent set.
	 */
	public Object clone() {
		...
		if (styles != null)
			for (int i = 0; i < styles.size(); i++) {
				FMapWMSStyle sty = (FMapWMSStyle) ((FMapWMSStyle) this.styles
						.get(i)).clone();
				sty.parent = this;
		...
		if (dimensions != null)
			for (int i = 0; i < dimensions.size(); i++) {
				...
				clone.dimensions
						.add((IFMapWMSDimension) this.dimensions.get(i));
			}
	...
	public class FMapWMSStyle {
		/*
		 * Please! ensure that the fields are double, int, or Object or
		 * otherwise add the corresponding entry in the clone() method.
		 */
		public String name;
		public String title;
			...
			for (int i = 0; i < fields.length; i++) {
				try {
					...
					if (fields[i].getType().equals(Integer.class)) {
						clazz.getField(fieldName).setInt(clone,
								clazz.getField(fieldName).getInt(this));
						// double entry
					} else if (fields[i].getType().equals(Double.class)) {
						clazz.getField(fieldName).setDouble(clone,
								clazz.getField(fieldName).getDouble(this));
						// any object entry
					} else {
						clazz.getField(fieldName).set(clone,
								clazz.getField(fieldName).get(this));
					}
				} catch (NullPointerException e) {
					NotificationManager.addWarning(
							"Cloning " + "'" + fields[i].getName()
									+ "' field is null" + "(FMapWMSStyle: "
									+ name + ")", e);
				} catch (Exception e) {
					NotificationManager.addWarning(
