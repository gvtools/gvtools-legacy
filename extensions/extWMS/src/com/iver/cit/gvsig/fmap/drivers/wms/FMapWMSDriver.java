...
public class FMapWMSDriver {
	...
	@Deprecated
	public void getCapabilities(URL server) throws WMSException {
	}
	...
	private GetMapRequest createMapRequest(WMSStatus status) {
		...
		if (stylesVector != null && stylesVector.size() > 0) {
			...
			for (int i = 0; i < stylesVector.size(); i++) {
				mapRequest.addLayer(layerNamesVector.get(i),
						stylesVector.get(i));
			}
		} else {
	...
	public File getLegendGraphic(WMSStatus status, String layerName,
			ICancellable cancel) throws WMSException {
	...
	public String getFeatureInfo(WMSStatus _wmsStatus, int i, int j,
			int max_value, ICancellable cancellable) throws WMSException {
