...
public class WMSPropsDialog extends JPanel implements IWindow {
	...
	/**
	 * With getParamsPanel we have access to the map config TabbedPane. If this
	 * panel doesn't exist yet (which can occur when an existing project is
	 * recovered) it is been automatically constructed by connecting to the
	 * server, reloading the necessary data, filling up the content and setting
	 * the selected values that were selected when the projet was saved.
	 * 
	 * 
	 * Since a connection to the server is needed when rebuiliding the panel,
	 * this causes a delay for the panel's showing up or a nullPointer error if
	 * there is no path to the server.
	 * 
	 * 
	 * Con getParamsPanel tenemos acceso a juego de pestaï¿½as de
	 * configuraciï¿½n del mapa. Si este panel todavï¿½a no existe (como puede
	 * ser cuando recuperamos un proyecto guardado) ï¿½ste se crea
	 * automï¿½ticamente conectando al servidor, recuperando los datos
	 * necesarios, rellenando el contenido y dejando seleccionados los valores
	 * que estaban seleccionados cuando se guardï¿½ el proyecto.
	 * 
	 * 
	 * Como para reconstruirse requiere una conexiï¿½n con el servidor esto
	 * causa un retardo en la apariciï¿½n en el toc o un error de nullPointer si
	 * no hay conexiï¿½n hasta el servidor.
	 * 
	 * 
	...
	public WMSParamsPanel getParamsPanel(HashMap info) {
		if (info != null) {
			try {
				...
				WMSLayerNode[] selectedLayers = (WMSLayerNode[]) ((Vector) info
						.get("selectedLayers")).toArray(new WMSLayerNode[0]);
				// boolean any = false;
				// selected layers
				// ((JDnDListModel)
				// toc.getLstSelectedLayers().getModel()).clear();
				for (int i = 0; i < selectedLayers.length; i++) {
					toc.getJustTreeLayer().clearSelection();
					// Se añade a la lista de layers seleccionados
					TreePath path = new TreePath(
							((LayerTreeModel) toc.getJustTreeLayer().getModel())
									.getNodeByName(selectedLayers[i].getName()));
					toc.getJustTreeLayer().addSelectionPath(path);
					toc.addLayer();
					// This block is deleted since this is implemented in the
					// addLayer method.
					// JDnDListModel modelo = (JDnDListModel)
					// toc.getLstSelectedLayers().getModel();
				...
				JOptionPane.showMessageDialog(null,
						"error_comunicacion_servidor", "Error",
	...
	private class CommandListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand() == "CANCEL")
				...
				try {
					((FLyrWMS) fLayer).setFullExtent(wmsParamsTabbedPane
							.getLayersRectangle());
				...
				try {
					if (!wmsParamsTabbedPane.getDisagregatedLayers())
						((FLyrWMS) fLayer).setLayerQuery(wmsParamsTabbedPane
								.getLayersQuery());
					((FLyrWMS) fLayer).setInfoLayerQuery(wmsParamsTabbedPane
							.getQueryableLayerQuery());
					((FLyrWMS) fLayer).setSRS(wmsParamsTabbedPane.getSRS());
					((FLyrWMS) fLayer).setName(wmsParamsTabbedPane
							.getLayerName());
					((FLyrWMS) fLayer).setWmsTransparency(wmsParamsTabbedPane
							.getTransparency());
					((FLyrWMS) fLayer).setStyles(wmsParamsTabbedPane
							.getStyles());
					((FLyrWMS) fLayer).setDimensions(wmsParamsTabbedPane
							.getDimensions());
					((FLyrWMS) fLayer).setFixedSize(wmsParamsTabbedPane
							.getFixedSize());
					...
					if (e.getActionCommand() == "APPLY") {
						...
						BaseView vista = (BaseView) PluginServices
						...
						if (parent != null) {
							...
							mapCtrl.getMapContext()
									.getLayers()
									.replaceLayer(fLayer.getName(),
											wmsParamsTabbedPane.getLayer());
						}
	...
	private FLayers mergeFLayers(FLayers group1, FLayers group2) {
		// FLayers agrupation = new FLayers(group1.getFMap(),
		// group1.getParentLayer());
		FLayer layer;
