...
public class MySqlFeatureIterator implements IFeatureIterator {
	...
	public IFeature next() throws ReadDriverException {
		...
		try {
			...
			IFeature feat = null;
			if (idFieldID != -1) {// TODO Review if we could find problems when
									// the table has only geom and gid
				int fieldId = lyrDef.getIdFieldID();
				Value idValue = regAtt[fieldId];
				String theID = "";
				if (idValue != null)// azabala: sometimes we found problems with
									// gid (null pointer exceptions)
