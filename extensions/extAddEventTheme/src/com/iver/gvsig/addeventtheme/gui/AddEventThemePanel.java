...
/**
 * The AddEventThemePanel class creates a JPanel where the user can input the
 * name of the gvSIG Table from that the plugin will create the new point Layer,
 * and the fields corresponding with the point coordinates.
 * 
 * @author jmorell
 */
public class AddEventThemePanel extends JPanel implements IWindow {
	...
	private String[] getFieldNames() {
		...
		try {
			...
			for (int i = 0; i < ds.getFieldCount(); i++) {
				...
				// if (ds.getFieldType(i)==Types.DOUBLE ||
				// ds.getFieldType(i)==Types.INTEGER ||
	...
	private void createNewLayerFromDataSource() {
		...
		try {
			...
			FLayer capa = null;
			capa = LayerFactory.createLayer(tableName, addEventThemeDriver,
					mapContext.getCrs());
			capa.addLayerListener(new AddEventThemListener());

			// Guardamos el nombre del fichero asociado a la capa de eventos
			SourceInfo srcInfo = ds.getDataSourceFactory().getDriverInfo(
					tableName);
