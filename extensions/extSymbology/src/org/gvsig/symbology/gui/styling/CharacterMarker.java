 ...
 * define a <b>character marker</b>.
 * <p>
 * <p>
 * This functionality is carried out thanks to two tabs (character marker and
 * mask)which are included in the panel to edit the properities of a symbol
 * (SymbolEditor)how is explained in AbstractTypeSymbolEditor.
 * <p>
 * <p>
 * The first tab (Character marker)allows the user to change the font
 * (<b>cmbFonts</b>)of the symbol for the character marker, the size of that
 * (<b>cmbFontSize</b>),the angle (<b>numberAngle</b>)if the user wants to show
 * the character with some rotation,the color (<b>jcc</b>) and the offset
 * (<b>txtYOffset,txtXOffset</b>).
 * <p>
 * <p>
 * The second tab (<b>Mask</b>) modifies attributes of a mask for points such as
 * style,size and symbol (to represent a point in the map).
 * <p>
 * 
 * @see Mask
 ...
 */
public class CharacterMarker extends AbstractTypeSymbolEditor implements
		ActionListener {
	...
	/**
	 * Initializes the parameters that define a charactermarker.To do it, two
	 * tabs are created inside the SymbolEditor panel with default values for
	 * the different attributes of the character marker.This two tabs will be
	 * character marker tab (options of font,size,angle,color and the offset of
	 * the character marker)and a Mask added as a new tab.
	 * 
	 */
	private void initialize() {
		JPanel myTab;
		{
			...
			getJListSymbols().setModel(
					new CharacterListModel(new Font((String) getCmbFonts()
							.getSelectedItem(), Font.PLAIN, 14)));
			...
			aux1.addComponent(PluginServices.getText(this, "size") + ":",
					getCmbFontSize());
			aux1.addComponent(PluginServices.getText(this, "angle") + ":",
					getNumberAngle());

			aux1.addComponent(PluginServices.getText(this, "color") + ":",
					getColorChooser());

			GridBagLayoutPanel aux2 = new GridBagLayoutPanel();
			aux2.addComponent("X:", getTxtXOffset());
			aux2.addComponent("Y:", getTxtYOffset());
			aux2.setBorder(BorderFactory.createTitledBorder(PluginServices
					.getText(this, "offset")));
	...
	/**
	 * JIncrementalNumberField that controls the YOffset for the character that
	 * the user wants to use as a marker.
	 * 
	...
	/**
	 * JIncrementalNumberField that controls the XOffset for the character that
	 * the user wants to use as a marker.
	 * 
	 * @return JIncrementalNumberField
	 */
	...
	/**
	 * JComboBoxFontSizes that allows the user to change the size of the
	 * character selected as a marker
	 * 
	...
	/**
	 * JIncrementalNumberField that controls the angle (if the user wants to
	 * show the character with some rotation) for the character that the user
	 * wants to use as a marker.
	 * 
	 * @return JIncrementalNumberField
	 */
	 ...
	 * for the character marker.Depends on this font, the character will change
	 * its appearance.
	 * 
	...
	/**
	 * Used with the JScrollPane to create the pane that allows the user to
	 * select the character marker.
	 * 
	 * @see getJScrollPane()
	 * @return JList
	 */
	private JList getJListSymbols() {
		if (jListSymbols == null) {
			...
			jListSymbols.addListSelectionListener(new ListSelectionListener() {
				...
				public void valueChanged(ListSelectionEvent e) {
					CharacterItem theChar = (CharacterItem) jListSymbols
							.getSelectedValue();
					if (theChar == null)
						return;
					font = theChar.font;
	...
	public void refreshControls(ISymbol layer) {
		...
		try {
			...
			getCmbFontSize().setSelectedItem(
					new Integer(sym.getFont().getSize()));
			getJListSymbols().setSelectedIndex(sym.getUnicode());
			getNumberAngle().setDouble(
					sym.getRotation() / FConstant.DEGREE_TO_RADIANS);
	...
	public ISymbol getLayer() {
		...
		if (font == null) {
			font = new Font((String) getCmbFonts().getSelectedItem(),
					Font.PLAIN, (int) getCmbFontSize().getSelectedValue());
		}
		layer.setFont(font);
		layer.setUnicode(unicode);
		layer.setColor(getColorChooser().getColor());
		layer.setSize(getCmbFontSize().getSelectedValue());
		layer.setOffset(new Point2D.Double(getTxtXOffset().getDouble(),
				-getTxtYOffset().getDouble()));
		layer.setRotation(getNumberAngle().getDouble()
				* FConstant.DEGREE_TO_RADIANS);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (c.equals(getCmbFonts())) {
			...
			getJListSymbols().setModel(
					new CharacterListModel(new Font(fontName, Font.PLAIN, 10)));
			return;
		}
