 ...
 * <p>
 * <p>
 * This functionality is carried out thanks to two tabs (picture fill and
 ...
 * <p>
 * <p>
 * The first tab (picture fill)permits the user to select the picture for the
 * padding and differentes options to modify it such as the
 * angle(<b>incrAngle</b>) and the scale (<b>incrScaleX,incrScaleY</b>). Also,
 * there is an option to select a color for the fill (<b>jccFillColor</b>).
 * <p>
...
public class PictureFill extends AbstractTypeSymbolEditor implements
	 ...
	 * polygon with a picture style.To do it, two tabs are created (picture fill
	 * and MarkerFillProperties)inside the SymbolEditor panel with default
	...
	private void initialize() {
		...
		btnBrowseFileSelected = new JButton(PluginServices.getText(this,
				"browse"));
		...
		aux3.addComponent(PluginServices.getText(this, "scale") + "X:",
				incrScaleX = new JIncrementalNumberField("1", 20, 0.01,
						Double.POSITIVE_INFINITY, 0.1));
		incrScaleX.setDouble(1);
		aux3.addComponent(PluginServices.getText(this, "scale") + "Y:",
				incrScaleY = new JIncrementalNumberField("1", 20, 0.01,
						Double.POSITIVE_INFINITY, 0.1));
	...
	public ISymbol getLayer() {
		...
		try {
			...
			if (lblFileName.getText().equals(""))
				...
				sym = new PictureFillSymbol(new URL(lblFileName.getText()),
						null);
				if (!lblSelFileName.getText().equals(""))
					sym = new PictureFillSymbol(new URL(lblFileName.getText()),
							new URL(lblSelFileName.getText()));
