 ...
 * polygon with a padding composed of lines
 * <p>
 * <p>
 * This functionality is carried out thanks to a tab (line fill)which is
 ...
 * the color between the lines (<b>jcc</b>),the angle of the
 * line(<b>incrAngle</b>), the offset between paralel lines(<b>incrOffset</b>)
 * and the separation (<b>incrSeparation</b>).
 * 
 * 
 * @see AbstractTypeSymbolEditor
 * @author jaume dominguez faus - jaume.dominguez@iver.es
 */
public class LineFill extends AbstractTypeSymbolEditor implements
		ActionListener {
	 ...
	 * polygon with a simpleline style.To do it, a tab (line fill) is created
	...
	private void initialize() {
		...
		jcc.setAlpha(255);
		content.addComponent(PluginServices.getText(this, "color") + ":", aux);

		aux = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
		aux.add(incrAngle = new JIncrementalNumberField("0", 5));
		content.addComponent(PluginServices.getText(this, "angle") + ":", aux);

		aux = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
		aux.add(incrOffset = new JIncrementalNumberField("0", 5));
		content.addComponent(PluginServices.getText(this, "offset") + ":", aux);

		aux = new JPanel(new FlowLayout(FlowLayout.LEFT, 5, 0));
		aux.add(incrSeparation = new JIncrementalNumberField("5", 5, 1,
