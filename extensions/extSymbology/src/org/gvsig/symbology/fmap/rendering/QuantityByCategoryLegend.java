 ...
 * using a color for the region and , on the other side, using a graduated
 * symbol. Both methods will change (the color or the size of the symbol)
...
public class QuantityByCategoryLegend extends AbstractClassifiedVectorLegend
	...
	private ISymbol defaultSymbol = SymbologyFactory
	 ...
	 * fieldNames is an array of length 2 where the first element is the field
	 * name for the embedded GraduatedSymbolLegend, and the second is the field
	 * name for the embedded colorRamp (VectorialIntervalLegend) legend.
	 */
	public void setClassifyingFieldNames(String[] fieldNames) {
		...
		graduatedSymbol
				.setClassifyingFieldNames(new String[] { fieldNames[0] });
	}
	...
	public void delSymbol(Object key) {
		...
		fireClassifiedSymbolChangeEvent(new SymbolLegendEvent(null, null));
	}
	...
	public String[] getDescriptions() {
		...
		if (desc1.length == 0)
			...
			for (int i = 0; i < descriptions.length; i++) {
				descriptions[i] = (i < desc1.length) ? desc1[i] : desc2[i
						- desc1.length];
			}
			return descriptions;
		}
	...
	public ISymbol[] getSymbols() {
		...
		if (symbols1.length == 0)
			...
			for (int i = 0; i < symbols.length; i++) {
				symbols[i] = (i < symbols1.length) ? symbols1[i] : symbols2[i
						- symbols1.length];
			}
		}
		return symbols;
	...
	public Object[] getValues() {
		...
		if (objects1.length == 0)
			...
			for (int i = 0; i < objects.length; i++) {
				objects[i] = (i < objects1.length) ? objects1[i] : objects2[i
						- objects1.length];
			}
			return objects;
		}
	...
	public void setDataSource(DataSource ds) throws FieldNotFoundException,
			ReadDriverException {
		// TODO remove it when
		// FLyrVect.forTestOnlyVariableUseIterators_REMOVE_THIS_FIELD is removed
	...
	public ISymbol getSymbol(int i) throws ReadDriverException {
		...
		case FShape.POLYGON:
			/*
			 * symbol from the GraduatedSymbolLegend is a marker, but what we
			 * need is a fill symbol. Will use a MarkerFillSymbol to enable
	...
	public ISymbol getSymbolByFeature(IFeature feat) {
		...
		// if (!FLyrVect.forTestOnlyVariableUseIterators_REMOVE_THIS_FIELD) {
		// gsVal =
		// feat.getAttribute(ds.getFieldIndexByName(getClassifyingFieldNames()[0]));
		//
		// } else {
		gsVal = feat.getAttribute(0);
		// }
		sym1 = graduatedSymbol.getSymbolByInterval(graduatedSymbol
				.getInterval(gsVal));

		// } catch (ReadDriverException e) {
		// Logger.getLogger(getClass()).error("Graduated Symbol Legend failed",
		// e);
		// }
		...
		// if (!FLyrVect.forTestOnlyVariableUseIterators_REMOVE_THIS_FIELD) {
		// crVal =
		// feat.getAttribute(ds.getFieldIndexByName(getClassifyingFieldNames()[1]));
		// } else {
		...
		// } catch (ReadDriverException e) {
		// Logger.getLogger(getClass()).error("Color Ramp Symbol Legend failed",
		// e);
		// }
	...
	public void setShapeType(int shapeType) {
		...
		colorRamp.setDefaultSymbol(SymbologyFactory
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		try {
			colorRamp = (VectorialIntervalLegend) LegendFactory
					.createFromXML(xml.getChild(0));
			graduatedSymbol = (GraduatedSymbolLegend) LegendFactory
					.createFromXML(xml.getChild(1));
			colorRamp.setShapeType(shapeType);
