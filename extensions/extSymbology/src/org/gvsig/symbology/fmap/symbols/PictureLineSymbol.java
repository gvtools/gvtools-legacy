...
/**
 * PictureLineSymbol allows to use any symbol defined as an image (by an image
 * file) supported by gvSIG.This symbol will be used as an initial object.The
 * line will be painted as a succession of puntual symbols through the path
 * defined by it(the line).
 * 
...
public class PictureLineSymbol extends AbstractLineSymbol {
	...
	/**
	 * Sets the URL for the image to be used as a picture line symbol (when it
	...
	public ISymbol getSymbolForSelection() {
		if (selectionSym == null) {
			selectionSym = (PictureLineSymbol) SymbologyFactory
	...
	public void draw(Graphics2D g, AffineTransform affineTransform, FShape shp,
		...
		g.setClip(new BasicStroke((float) csWidth, BasicStroke.CAP_ROUND,
				BasicStroke.CAP_ROUND).createStrokedShape(shp));
		...
		while ((cancel == null || !cancel.isCanceled()) && !iterator.isDone()) {
			...
			for (int i = 0; (cancel == null || !cancel.isCanceled())
				...
				rect.setBounds(0, (int) Math.round(height * .5),
						(int) Math.ceil(imageWidth), height);
				try {
					bg.drawInsideRectangle(g, rect, false);
				} catch (SymbolDrawingException e) {
					Logger.getLogger(getClass())
							.warn(Messages
									.getString("label_style_could_not_be_painted"),
									e);
				}
	...
	/**
	 * Returns the path of the image that is used as a picture line symbol (when
	 * it is selected in the map)
	 * 
	...
	public double toCartographicSize(ViewPort viewPort, double dpi, FShape shp) {
		...
		setCartographicSize(CartographicSupportToolkit.getCartographicLength(
				this, width, viewPort, dpi), shp);
		return s;
	}
}
