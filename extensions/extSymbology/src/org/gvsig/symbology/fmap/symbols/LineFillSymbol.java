...
/**
 * LineFillSymbol allows to define a filling pattern composed by lines to
 * complete the padding of the polygon.
 * 
...
public class LineFillSymbol extends AbstractFillSymbol {
	...
	/**
	 * Returns the separation between lines that is used to create the filling
	 * pattern
	 * 
	 * @return
	 */
	...
	/**
	 * Establishes the separation between lines that is used to create the
	 * filling pattern
	 * 
	 * @param separation
	 */
	...
	public void draw(Graphics2D g, AffineTransform affineTransform, FShape shp,
		...
		if (lineSymbol != null) {
			...
			double radius = Math.abs(Math.max(bounds.getHeight(),
					bounds.getWidth()));
			...
			while ((cancel == null || !cancel.isCanceled()) && aux <= radius) {
				...
				lineSymbol.draw(g, null,
						new FPolyline2D(new GeneralPathX(line)), null);
				aux += csSeparation;
			}
			g.rotate(-angle);
			g.translate(-centerX, -centerY);
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (lineSymbolXML != null) {
			setLineSymbol((ILineSymbol) SymbologyFactory.createSymbolFromXML(
					lineSymbolXML, "fill symbol"));
		}

		XMLEntity outlineXML = xml.firstChild("id", "outline");
		if (outlineXML != null) {
			setOutline((ILineSymbol) SymbologyFactory.createSymbolFromXML(
					outlineXML, "outline symbol"));
		}
	...
	public double toCartographicSize(ViewPort viewPort, double dpi, FShape shp) {
		...
		csOffset = CartographicSupportToolkit.getCartographicLength(this,
				offset, viewPort, dpi);

		csSeparation = CartographicSupportToolkit.getCartographicLength(this,
				separation, viewPort, dpi);
		double csLineWidth = CartographicSupportToolkit.getCartographicLength(
				this, symbolLineWidth, viewPort, dpi);
		if (getLineSymbol() != null) {
			getLineSymbol().setCartographicSize(csLineWidth, shp);
		}
		return s;
