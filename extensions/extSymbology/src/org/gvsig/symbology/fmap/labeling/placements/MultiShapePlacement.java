...
public class MultiShapePlacement implements ILabelPlacement {
	 ...
	 * placements to those passed as parameters. Null values are allowed for the
	 * parameters and will cause that no label will be placed when the geometry
	 * belongs to such null values.
	 * 
	 * @param pointPlacement
	 *            , the placement for points
	 * @param linePlacement
	 *            , the placement for lines
	 * @param polygonPlacement
	 *            , the placement for polygons
	 */
	public MultiShapePlacement(ILabelPlacement pointPlacement,
			ILabelPlacement linePlacement, ILabelPlacement polygonPlacement) {
	...
	public ArrayList<LabelLocationMetrics> guess(LabelClass lc, IGeometry geom,
		...
		case FShape.POINT:
			if (pointPlacement != null) {
				return pointPlacement.guess(lc, geom, pc.getPointConstraints(),
						cartographicSymbolSize, cancel, vp);
			}
			break;
		case FShape.LINE:
			if (linePlacement != null) {
				return linePlacement.guess(lc, geom, pc.getLineConstraints(),
						cartographicSymbolSize, cancel, vp);
			}
			break;
		case FShape.POLYGON:
			if (polygonPlacement != null) {
				return polygonPlacement.guess(lc, geom,
						pc.getPolygonConstraints(), cartographicSymbolSize,
						cancel, vp);
			}
			break;
		}
	...
	public boolean isSuitableFor(IPlacementConstraints placementConstraints,
			int shapeType) {
		// TODO shoud I also ask to each placement if it is suitable for these
		// constraints????
