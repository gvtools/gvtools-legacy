...
import org.gvsig.symbology.fmap.rendering.filter.operations.FieldExpressionOperator;
...
import org.gvsig.symbology.fmap.rendering.filter.operations.LabelExpressionOperator;
...
import org.gvsig.symbology.fmap.rendering.filter.operations.NumericalConstant;
...
import org.gvsig.symbology.fmap.rendering.filter.operations.StringConstant;
...
public class LabelExpressionParser implements LabelExpressionParserConstants {
	...
	public LabelExpressionParser(StringReader reader,
			Hashtable<String, Value> symbolsTable) {
	...
	/**
	 * it is default because it is used in the unit tests but this method is not
	 * part of the public API, so don't use it!
	 */
	...
	public static void main(String[] args) throws ParseException,
		...
		System.out.print(((Expression) (parser.getStack().pop())).evaluate()
				.toString());

	}

	/* LABEL EXPRESSION */
	// We have to use this function if we want to evaluate an String which
	// contains different Expressions
	// inside
	...
	final public void EqCompExpression() throws ParseException {
		...
		label_4: while (true) {
			...
			if (x.kind == EQ)
				func = new EqOperator(symbols);
			else if (x.kind == NE)
				func = new NeOperator(symbols);
	...
	final public void ComparisonExpression() throws ParseException {
		...
		label_5: while (true) {
			...
			if (x.kind == LT)
				func = new LessThanOperator(symbols);
			else if (x.kind == LE)
				func = new LessThanOrEqualsOperator(symbols);
			else if (x.kind == GT)
				func = new GreaterThanOperator(symbols);
			else if (x.kind == GE)
				func = new GreaterThanOrEqualsOperator(symbols);
	...
	final public void AdditiveExpression() throws ParseException {
		...
		label_6: while (true) {
			...
			if (x.kind == PLUS)
				func = new AddOperator(symbols);
			else
				func = new MinusOperator(symbols);
	...
	final public void MultiplicativeExpression() throws ParseException {
		...
		label_7: while (true) {
			...
			if (x.kind == MULTIPLY)
				func = new MultOperator(symbols);
			else
				func = new DivOperator(symbols);
	...
	final public void Element() throws ParseException {
		...
		if (jj_2_26(2)) {
			...
			NumericalConstant intC = new NumericalConstant(
					Integer.valueOf(x.image), symbols);
			stack.push(intC);
		} else if (jj_2_28(2)) {
			x = jj_consume_token(FLOATING_POINT);
			NumericalConstant doubleC = new NumericalConstant(
					Double.valueOf(x.image), symbols);
			stack.push(doubleC);
		} else if (jj_2_29(2)) {
			x = jj_consume_token(STRING);
			StringConstant stringValue = new StringConstant(x.image.substring(
					1, x.image.length() - 1), symbols);
	...
	final public void function() throws ParseException {
		...
		if (jj_2_34(2)) {
			...
			argCount = Arguments();
			try {
				function = (Expression) operators.functions.get(functionName)
						.getConstructor(new Class[] { Hashtable.class })
						.newInstance(new Hashtable<String, Value>());
			} catch (Exception e) {
				{
					if (true)
						throw new ParseException("Problem with function '"
								+ functionName + "': " + e.toString());
				}
			}
			if (function == null) {
				if (true)
					throw new ParseException("Nonexisting function name '"
							+ functionName + "'");
			}
	...
	final private boolean jj_3_26() {
		if (jj_scan_token(OPEN_SQ_BRACKETS))
			return true;
