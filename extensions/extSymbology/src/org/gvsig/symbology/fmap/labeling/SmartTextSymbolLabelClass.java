 ...
 * <p>
 * 
 * This is a <b>wrapper</b> to be able to use a SmartTextSymbol as a LabelClass.
 * A SmartTextSymbol uses a line instead of a point. Being in fact a Line and
 * not a Marker, it does not make any sense to have label styles since the label
 * styles are well-defined rectangle areas where texts are placed in fields
 * backgrounded by an image. In this contexts, the area is defined dinamically
 * for each line, and there is no sense to have texts fields. They will be
 * rendered as a single string along a line.<br>
 * </p>
 * <p>
 * The label itself is the SmartTextSymbol, the geometry (the line), the label
 * expression, and the text applied to the symbol.<br>
 * </p>
 * <p>
 * Most of the operations performed by this LabelClass are in fact delegated to
 * the symbol.<br>
 * </p>
 * 
 * 
...
public class SmartTextSymbolLabelClass extends LabelClass {
	...
	public void setTextSymbol(ITextSymbol textSymbol) {
		// // if (! (textSymbol instanceof SmartTextSymbol)) throw new
		// IllegalArgumentException("Only SmartTextSymbol allowed in this context");
