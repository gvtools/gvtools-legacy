...
/**
 * Implements the driver for Styled Layer Descriptor documents in order to allow
 * gvSIG to read or write XML documents of this OGC Standard.
 * 
...
public class FMapSLDDriver implements IFMapLegendDriver {
	...
	/**
	 * Obtains the classifying field names that are used in the .sld document
	 * 
	...
	/**
	 * Returns the classifying field types of the classifying field names that
	 * are used in the .sld document
	 * 
	...
	/**
	 * Returns true if any layer contained in the TOC menu of gvSIG has the same
	 * name that is passed as an argument. Otherwise,false.
	 * 
	 * @param layer
	 * @param name
	 * 
	...
	public Hashtable<FLayer, ILegend> read(FLayers layers, FLayer layer,
		...
		try {
			...
			my_sld = SLDProtocolHandlerFactory
					.createVersionedProtocolHandler(file);
		...
		for (Iterator<ISLDLayer> iterator = sldLayers.iterator(); iterator
				.hasNext();) {
	...
	/**
	 * Takes a SLDLayer and with its information, creates a gvSIG legend.
	 * Depending on if the layer has filters or not, it will be created a single
	 * symbol legend or a filter expression legend
	 * 
	 * @param sldLayer
	 *            layer contained in a .sld document
	 ...
	 */
	private ILegend createLegend(ISLDLayer sldLayer)
			throws ReadDriverException, NumberFormatException,
		...
		// SingleSymbolLegend
		else if ((lyrVect.getShapeType() % FShape.Z) != FShape.MULTI
				&& !sldLayer.layerHasFilterForSymbolizers(lyrVect
						.getShapeType())) {
			SingleSymbolLegend createdLegend = new SingleSymbolLegend();
			ArrayList<ISLDSymbolizer> symbolizers;
			symbolizers = sldLayer.getSymbolizersByShapeType(lyrVect
					.getShapeType());
			IMultiLayerSymbol multiLayerSymbol = SymbologyFactory
					.createEmptyMultiLayerSymbol(lyrVect.getShapeType());
			...
			if (multiLayerSymbol != null) {
				...
				ISymbol defaultSym = (multiLayerSymbol.getLayerCount() > 1) ? multiLayerSymbol
		...
		// VectorialFilterExpressionLegend
			...
			VectorFilterExpressionLegend legend = new VectorFilterExpressionLegend(
					lyrVect.getShapeType(), fieldNames);
			for (int i = 0; i < filters.size(); i++) {
				for (int j = 0; j < filters.get(i).getSymbolizers().size(); j++) {

					ISLDSymbolizer sldSym = filters.get(i).getSymbolizers()
							.get(j);
					ISymbol sym = SLDSymbolizer2ISymbol(sldSym);
					sym.setDescription(filters.get(i).getExpression()
							.toString());
					legend.addSymbol(filters.get(i).getExpression().toString(),
							sym);
				}
			}
			return legend;
	...
	/**
	 * Checks if the field names contained in an sld document appear in the
	 * information of the layer that we are using. If all the field names are
	 * correct the method returns true.Otherwise, false.
	 * 
	 * @param fieldNames
	 * @return
	 * @throws ReadDriverException
	 */
	private boolean checkFieldNames(String[] fieldNames)
			throws ReadDriverException {

		for (int i = 0; i < fieldNames.length; i++) {
			if (((FLyrVect) this.layer).getSource().getRecordset()
					.getFieldIndexByName(fieldNames[i]) == -1)
				return false;
		}
		return true;
	}
	 ...
	 * 
	 * @param symbolizer
	 *            to be transfomed into an ISymbol
	 * @return ISymbol
	 * @throws NumberFormatException
	 * @throws LegendDriverException
	 * @throws UnknownHostException
	 */
	public static ISymbol SLDSymbolizer2ISymbol(ISLDSymbolizer symbolizer)
		...
		if (symbolizer instanceof SLDLineSymbolizer) {
			...
			if (myLine.getStroke().getExpressionDashOffset().getLiteral() != null)
				dashOffset = Float.valueOf(myLine.getStroke()
						.getExpressionDashOffset().getLiteral());
			else
				dashOffset = 0f;
			...
			if (myLine.getStroke().getFloatDashArray().length == 0) {
				...
				dash = myLine.getStroke().getFloatDashArray();

			BasicStroke str = new BasicStroke(width, lineCap, lineJoin, 10,
					dash, dashOffset);

			simplLine.setStroke(str);

			// SimpleLineSymbol
			if (!myLine.getStroke().isHasGraphicFill()
					&& !myLine.getStroke().isHasGraphicStroke()) {
				...
				if (myLine.getStroke().getExpressionOpacity().getLiteral() != null)
					line.setAlpha((int) (255 * Float.valueOf(myLine.getStroke()
							.getExpressionOpacity().getLiteral())));
				if (myLine.getStroke().getExpressionWidth().getLiteral() != null)
				...
				if (myLine.getStroke().getGraphic().getMarks().size() > 0) {
					SLDMark myMark = myLine.getStroke().getGraphic().getMarks()
							.get(0);
					SimpleMarkerSymbol marker = new SimpleMarkerSymbol();
					FExpression size = myLine.getStroke().getGraphic()
							.getExpressionSize();
					FExpression rotation = myLine.getStroke().getGraphic()
							.getExpressionRotation();
					marker = createMarker(myMark, size, rotation);
					line.setLineWidth(marker.getSize());
					marker.setUnit(-1);
					marker.setStyle(SLDUtils.setMarkerStyle(myMark
							.getWellKnownName().getLiteral()));
				...
				else if (myLine.getStroke().getGraphic().getExternalGraphics()
						.size() > 0) {
					SLDExternalGraphic myExtGraphic = myLine.getStroke()
							.getGraphic().getExternalGraphics().get(0);
		...
		// /////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			// SimpleFillSymbol
			if (myPolygon.getFill() != null
					&& myPolygon.getFill().getFillGraphic() == null) {
				...
				if (myPolygon.getFill().getExpressionColor().getLiteral() != null) {
					...
					if (myPolygon.getFill().getExpressionOpacity().getLiteral() != null) {
						...
						int alpha = (int) ((myPolygon.getFill()
								.getFillOpacity()) * 255);
						Color myColor = new Color(color.getRed(),
				...
				if (myFill.getMarks().size() > 0
					...
					if (myPolygon.getStroke() != null) {
						...
						markerFill.setOutline(createOutline(myPolygon
								.getStroke()));
					}
					...
					// Marker->PictureMarkerSymbol
						...
						PictureMarkerSymbol marker = new PictureMarkerSymbol();
						SLDExternalGraphic myExtGraphic = myFill
								.getExternalGraphics().get(0);
						marker = createMarker(myExtGraphic, size, rotation);
						if (marker != null) {
							markerFill.setMarker(marker);
							if (marker.getColor() != null)
		...
		// /////////////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if (myPoint.getGraphic().getMarks().size() > 0) {
				...
				FExpression rotation = myPoint.getGraphic()
						.getExpressionRotation();
			...
			// Marker->PictureMarkerSymbol
				...
				SLDExternalGraphic myExtGraphic = myPoint.getGraphic()
						.getExternalGraphics().get(0);
				FExpression size = myPoint.getGraphic().getExpressionSize();
				FExpression rotation = myPoint.getGraphic()
						.getExpressionRotation();
				PictureMarkerSymbol marker = createMarker(myExtGraphic, size,
						rotation);
				marker.setUnit(-1);
				return marker;
			}
	...
	public static SimpleMarkerSymbol createMarker(SLDMark myMark,
			FExpression size, FExpression rotation)
			throws NumberFormatException, LegendDriverException {
		...
		if (myMark.getFill() != null) {
			if (myMark.getFill().getExpressionColor().getLiteral() != null) {
				...
				if (myMark.getFill().getExpressionOpacity().getLiteral() != null) {
					...
					Color myColor = new Color(color.getRed(), color.getGreen(),
							color.getBlue(), alpha);
	...
	public static PictureMarkerSymbol createMarker(
			SLDExternalGraphic myExtGraphic, FExpression size,
			FExpression rotation) throws LegendDriverException {
		PictureMarkerSymbol marker = new PictureMarkerSymbol();
	...
	public static ILineSymbol createOutline(SLDStroke stroke)
			throws NumberFormatException, LegendDriverException {
		SLDLineSymbolizer line;

		try {
			line = (SLDLineSymbolizer) Class
					.forName(
							classNameForVersion(SLDLineSymbolizer.class,
									sldReadVersion)).newInstance();
	...
	public void write(FLayers layers, FLayer layer, ILegend legend, File file,
		...
		try {
			...
			if (legend.getClass().equals(VectorialIntervalLegend.class)) {
				...
				if (symbols.length == 0) {
					NotificationManager.showMessageInfo(PluginServices.getText(
							this, "not_save_empty_legend"), null);
					return;
				}
				...
				FInterval interval;
				if (symbols[0] != null
						&& symbols[0] instanceof MultiShapeSymbol)
					throw new LegendDriverException(
				...
				for (int i = 0; i < symbols.length; i++) {
					...
					xmlBuilder.writeTag(FilterTags.NAME, "" + interval.getMin()
							+ " - " + interval.getMax());
					...
					xmlBuilder.writeTag(FilterTags.PROPERTYNAME,
							myLegend.getClassifyingFieldNames()[0]);
					...
					ISLDSymbolizer symSLD = (ISLDSymbolizer) ISymbol2SLDSymbolizer(
							symbols[i], sldVersion);
					xmlBuilder.writeRaw(symSLD.toXML());

					xmlBuilder.closeTag();
				}
			} else if (legend.getClass().equals(
				...
				for (int i = 0; i < symbols.length; i++) {
					...
					if (myVal.toString().compareTo("Default") == 0) {
						...
						if (myVal instanceof StringValue)
							xmlBuilder.writeTag(FilterTags.NAME,
									"\"" + myVal.toString() + "\"");
						else
							xmlBuilder.writeTag(
									FilterTags.NAME,
									""
											+ df.format(Double.valueOf(myVal
													.toString())));
				...
				for (int i = 0; i < symbols.length; i++) {
					...
					xmlBuilder.openTag(FilterTags.FILTER);
					xmlBuilder.writeRaw(filterFactory
							.createXMLFromExpression(filter4Symbol));
					xmlBuilder.closeTag();
			...
			// SingleSymbolLegend
				...
				xmlBuilder.openTag(SLDTags.RULE);
				ISLDSymbolizer symSLD = (ISLDSymbolizer) ISymbol2SLDSymbolizer(
						myLegend.getDefaultSymbol(), sldVersion);
				xmlBuilder.writeRaw(symSLD.toXML());
				xmlBuilder.closeTag();
			} else {
				throw new LegendDriverException(
	...
	private static String classNameForVersion(Class sldClass, String sldVersion) {
		...
		// sldClass.getName().lastIndexOf(".", sldClass.getName().length())) +
		// versionString;
	 ...
	 */
	private ISLDSymbolizer ISymbol2SLDSymbolizer(ISymbol symbol,
			String sldVersion) throws LegendDriverException,
		...
		if (symbol instanceof IMultiLayerSymbol) {
			...
			if (symbol instanceof MultiLayerLineSymbol) {
				...
				SLDMultiLineSymbolizer multiSldLine = (SLDMultiLineSymbolizer) Class
						.forName(
								classNameForVersion(
										SLDMultiLineSymbolizer.class,
										sldVersion)).newInstance();
				for (int i = 0; i < multiLine.getLayerCount(); i++) {
					multiSldLine
							.addSldLine((SLDLineSymbolizer) ISymbol2SLDSymbolizer(
									multiLine.getLayer(i), sldVersion));
				}
			...
			// Made up of polygons
				...
				SLDMultiPolygonSymbolizer multiSldPolygon = (SLDMultiPolygonSymbolizer) Class
						.forName(
								classNameForVersion(
										SLDMultiPolygonSymbolizer.class,
										sldVersion)).newInstance();
				for (int i = 0; i < multiFill.getLayerCount(); i++) {
					multiSldPolygon
							.addSldPolygon((SLDPolygonSymbolizer) ISymbol2SLDSymbolizer(
									multiFill.getLayer(i), sldVersion));
				}
			...
			// Made up of markers
				...
				SLDMultiPointSymbolizer multiSldPoint = (SLDMultiPointSymbolizer) Class
						.forName(
								classNameForVersion(
										SLDMultiPointSymbolizer.class,
										sldVersion)).newInstance();
				for (int i = 0; i < multiMarker.getLayerCount(); i++) {
					multiSldPoint
							.addSldPoint((SLDPointSymbolizer) ISymbol2SLDSymbolizer(
									multiMarker.getLayer(i), sldVersion));
				}
		...
		// SLDMultiShapeSymbolizer sldMultiShape = new
		// SLDMultiShapeSymbolizer();
		...
		// Single symbol
			...
			if (symbol instanceof ILineSymbol) {
				...
				SLDLineSymbolizer mySldLine = (SLDLineSymbolizer) Class
						.forName(
								classNameForVersion(SLDLineSymbolizer.class,
										sldVersion)).newInstance();
			...
			// Made up of a polygon
				...
				SLDPolygonSymbolizer mySldPolygon = (SLDPolygonSymbolizer) Class
						.forName(
								classNameForVersion(SLDPolygonSymbolizer.class,
										sldVersion)).newInstance();
				SLDFill fill = (SLDFill) Class.forName(
						classNameForVersion(SLDFill.class, sldVersion))
						.newInstance();
				SLDGraphic graphic = (SLDGraphic) Class.forName(
						classNameForVersion(SLDGraphic.class, sldVersion))
						.newInstance();
				...
				if (myFill instanceof MarkerFillSymbol) {
					...
					fill.setFillGraphic(createMarkerFill(
							markerFill.getMarker(), graphic, sldVersion));
			...
			// Made up of a marker
				...
				SLDPointSymbolizer mySldPoint = (SLDPointSymbolizer) Class
						.forName(
								classNameForVersion(SLDPointSymbolizer.class,
										sldVersion)).newInstance();
	...
	private SLDGraphic createMarkerFill(IMarkerSymbol markerFill,
			SLDGraphic graphic, String sldVersion)
		...
		if (markerFill.getColor() != null) {
			...
			FExpression expressionColor = new FExpression();
			expressionColor.setLiteral(SLDUtils
					.convertColorToHexString(markerFill.getColor()));
			fill.setExpressionColor(expressionColor);
		...
		if (markerFill instanceof MultiLayerMarkerSymbol) {
			...
			SLDMark mark = (SLDMark) Class.forName(
					classNameForVersion(SLDMark.class, sldVersion))
					.newInstance();
			FExpression wellKnownName = new FExpression();
			wellKnownName.setLiteral(SLDUtils.getMarkWellKnownName(myMarker
					.getStyle()));
			...
			myGraphic.addExternalGraphic(createPictureFill(
					myMarkerSymbol.getImagePath(), sldVersion));
		} else if (markerFill instanceof CharacterMarkerSymbol) {
			SLDMark mark = (SLDMark) Class.forName(
					classNameForVersion(SLDMark.class, sldVersion))
					.newInstance();
			FExpression wellKnownName = new FExpression();
			wellKnownName.setLiteral(SLDUtils
					.getMarkWellKnownName(SimpleMarkerSymbol.SQUARE_STYLE));
			mark.setWellKnownName(wellKnownName);
			mark.setFill(fill);
			myGraphic.addMark(mark);
		}
	...
	private SLDExternalGraphic createPictureFill(String imagePath,
			String sldVersion) throws InstantiationException,
			IllegalAccessException, ClassNotFoundException {
		SLDExternalGraphic extGra = (SLDExternalGraphic) Class.forName(
				classNameForVersion(SLDExternalGraphic.class, sldVersion))
				.newInstance();
		try {
			if (imagePath != null) {
				extGra.addOnlineResource(new URL(imagePath));
				extGra.setFormat("image/"
	...
	private SLDStroke createStroke(ILineSymbol myLine, String sldVersion)
		...
		SLDLineSymbolizer mySldLine = (SLDLineSymbolizer) Class.forName(
				classNameForVersion(SLDLineSymbolizer.class, sldVersion))
				.newInstance();
		// If the outline symbol is a multilayer symbol the line is made up of
		// the first
		...
		if (myLine.getColor() != null) {
			stroke.setExpressionColor(SLDUtils.convertColorToHexString(myLine
					.getColor()));
		...
		if (str != null) {
			...
			stroke.setExpressionLineCap(SLDUtils.convertLineCapToString(str
					.getEndCap()));
			stroke.setExpressionLineJoin(SLDUtils.convertLineJoinToString(str
					.getLineJoin()));
		...
		if (myLine instanceof MarkerLineSymbol) {
			...
			float[] dash = {
					Float.valueOf(String.valueOf(myLine.getLineWidth())),
					Float.valueOf(String.valueOf(myMarkerLine.getSeparation())) };
			stroke.setFloatDashArray(dash);
			stroke.setGraphic(createGraphic(myMarkerLine.getMarker(),
					sldVersion));
			...
			SLDGraphic graphic = (SLDGraphic) Class.forName(
					...
					.newInstance();
			graphic.addExternalGraphic(createPictureFill(
					picLine.getImagePath(), sldVersion));
	...
	private SLDGraphic createGraphic(IMarkerSymbol myMarker, String sldVersion)
		...
		graphic.getExpressionSize().setLiteral(
				String.valueOf(myMarker.getSize()));
		graphic.getExpressionRotation().setLiteral(
				String.valueOf(myMarker.getRotation()));
		graphic.getExpressionOpacity().setLiteral(
				String.valueOf(myMarker.getColor().getAlpha() / 255));
		...
		if (myMarker instanceof SimpleMarkerSymbol) {
			...
			SLDStroke str = (SLDStroke) Class.forName(
					classNameForVersion(SLDStroke.class, sldVersion))
					.newInstance();
			SLDMark mark = (SLDMark) Class.forName(
					classNameForVersion(SLDMark.class, sldVersion))
					.newInstance();
			...
			if (marker.getOutlineColor() != null)
				str.setExpressionColor(SLDUtils.convertColorToHexString(marker
						.getOutlineColor()));
			...
			if (marker.getColor() != null) {
				...
				FExpression expressionColor = new FExpression();
				expressionColor.setLiteral(SLDUtils
						.convertColorToHexString(marker.getColor()));
				fill.setExpressionColor(expressionColor);

				FExpression expressionOpacity = new FExpression();
				float opacity = (float) marker.getColor().getAlpha()
						/ (float) 255;
				expressionOpacity.setLiteral(String.valueOf(opacity));
				fill.setExpressionOpacity(expressionOpacity);
			...
			FExpression wellKnownName = new FExpression();
			wellKnownName.setLiteral(SLDUtils.getMarkWellKnownName(marker
					.getStyle()));
			mark.setWellKnownName(wellKnownName);
			...
			if (myMarker.getColor() != null) {
				...
				FExpression expressionColor = new FExpression();
				expressionColor.setLiteral(SLDUtils
						.convertColorToHexString(myMarker.getColor()));
				fill.setExpressionColor(expressionColor);

				FExpression expressionOpacity = new FExpression();
				float opacity = (float) myMarker.getColor().getAlpha()
						/ (float) 255;
				expressionOpacity.setLiteral(String.valueOf(opacity));
				fill.setExpressionOpacity(expressionOpacity);
