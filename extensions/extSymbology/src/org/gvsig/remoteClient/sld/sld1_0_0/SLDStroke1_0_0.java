...
public class SLDStroke1_0_0 extends SLDStroke {
		...
		while (!end) {
			...
			case XMLSchemaParser.START_TAG:
				if (parser.getName().compareTo(SLDTags.GRAPHICFILL) == 0
					...
					String attributeName = parser.getAttributeValue("",
							SLDTags.NAME_ATTR);
					/*
					 * Parche para compatibilizar leyendas generadas con la
					 * 1.1.2 donde el atributo "name" se pone con la primera
					 * letra mayuscula "Name"
					 */
					if (attributeName == null) {
						attributeName = parser.getAttributeValue("",
								SLDTags.NAME_ATTR_VERSAL);
					}
					/* Fin del parche */
					if (attributeName.compareTo(SLDTags.STROKE_WIDTH_ATTR) == 0) {
						...
						try {
							expressionWidth.parse(parser, parser.nextTag(),
									parser.getName());
						...
						setExpressionColor(expressionColor);
							...
							.compareTo(SLDTags.STROKE_OPACITY_ATTR) == 0) {
						...
						setExpressionOpacity(expressionOpacity);
							...
							.compareTo(SLDTags.STROKE_LINEJOIN_ATTR) == 0) {

						FExpression expressionLineJoin = new FExpression();

						try {
							expressionLineJoin.parse(parser, parser.nextTag(),
									parser.getName());
						} catch (XmlPullParserException e) {
							String s = parser.getText().trim();
						...
						if (!SLDUtils.isLineJoin(expressionLineJoin
								.getLiteral()))
							throw new LegendDriverException(
									LegendDriverException.PARSE_LEGEND_FILE_ERROR);
						...
						setExpressionLineJoin(expressionLineJoin);
							...
							.compareTo(SLDTags.STROKE_LINECAP_ATTR) == 0) {

						FExpression expressionLineCap = new FExpression();

						try {
							expressionLineCap.parse(parser, parser.nextTag(),
									parser.getName());
						} catch (XmlPullParserException e) {
							String s = parser.getText().trim();
						...
						setExpressionLineCap(expressionLineCap);
							...
							.compareTo(SLDTags.STROKE_DASHARRAY_ATTR) == 0) {

						FExpression expressionDashArray = new FExpression();
						try {
							expressionDashArray.parse(parser, parser.nextTag(),
									parser.getName());
						...
						if (cad == null /* || "".equals(cad) */) {
							...
							.compareTo(SLDTags.STROKE_DASHOFFSET_ATTR) == 0) {
						FExpression expressionDashOffset = new FExpression();

						try {
							expressionDashOffset.parse(parser,
									parser.nextTag(), parser.getName());
						} catch (XmlPullParserException e) {
							String s = parser.getText().trim();
							expressionDashOffset.setLiteral(s);
						}

						if (!SLDUtils.isANumber(expressionDashOffset
								.getLiteral()))
							throw new LegendDriverException(
									LegendDriverException.PARSE_LEGEND_FILE_ERROR);
	...
	private void parserDashArray(XMLSchemaParser parser, int i, String name)
			throws XmlPullParserException, IOException, LegendDriverException {
		...
		while (!end) {
			...
			case XMLSchemaParser.END_TAG:
				if (parser.getName().compareTo(
						FilterUtils.remNameSpace(SLDTags.CSSPARAMETER)) == 0) {
					parser.next();
				}
			}
			break;
	...
	public String toXML() {
		...
		if (getExpressionWidth().getLiteral() != null) {
			...
			xmlBuilder.writeTag(FilterTags.LITERAL, getExpressionWidth()
					.getLiteral());
			xmlBuilder.closeTag();
		}
		if (getExpressionLineCap().getLiteral() != null) {
			...
			xmlBuilder.writeTag(FilterTags.LITERAL, getExpressionLineCap()
					.getLiteral());
			xmlBuilder.closeTag();
		}
		if (getExpressionLineJoin().getLiteral() != null) {
			...
			xmlBuilder.writeTag(FilterTags.LITERAL, getExpressionLineJoin()
					.getLiteral());
			xmlBuilder.closeTag();
		}
		...
		if (getExpressionDashOffset().getLiteral() != null) {
			...
			xmlBuilder.writeTag(FilterTags.LITERAL, getExpressionDashOffset()
					.getLiteral());
			xmlBuilder.closeTag();
		}
		xmlBuilder.closeTag();
