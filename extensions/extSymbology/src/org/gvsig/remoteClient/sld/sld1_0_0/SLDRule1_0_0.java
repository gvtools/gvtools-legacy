...
public class SLDRule1_0_0 extends SLDRule {
			...
			case XMLSchemaParser.START_TAG:
				if (parser.getName().compareTo(SLDTags.NAME) == 0) {
					...
					setLegendGraphic(legend);
						...
						SLDTags.MINSCALEDENOMINATOR) == 0) {
					setMinScaleDenominator(Double.valueOf(parser.nextText()));
				} else if (parser.getName().compareTo(
						SLDTags.MAXSCALEDENOMINATOR) == 0) {
				...
				// }
					...
					addLineSymbolizer(sldLine);
						...
						.compareTo(SLDTags.POLYGONSYMBOLIZER) == 0) {
				...
				// else
				// if(parser.getName().compareTo(SLDTags.TEXTSYMBOLIZER)==0) {
				// throw new Error
				// ("TextSymbolizer not yet implemented->Rule\n");
				// }
				// else
				// if(parser.getName().compareTo(SLDTags.RASTERSYMBOLIZER)==0) {
				// throw new Error
				// ("RasterSymbolizer not yet implemented->Rule\n");
				// }
	...
	public void applyScale2Symbolizers(double minScaleDenominator,
			double maxScaleDenominator) {

		for (int i = 0; i < getLineSymbolizers().size(); i++) {
			getLineSymbolizers().get(i).setMinScaleDenominator(
					minScaleDenominator);
			getLineSymbolizers().get(i).setMaxScaleDenominator(
					maxScaleDenominator);
		}
		for (int i = 0; i < getPolygonSymbolizers().size(); i++) {
			getPolygonSymbolizers().get(i).setMinScaleDenominator(
					minScaleDenominator);
			getPolygonSymbolizers().get(i).setMaxScaleDenominator(
					maxScaleDenominator);
		}
		for (int i = 0; i < getPointSymbolizers().size(); i++) {
			getPointSymbolizers().get(i).setMinScaleDenominator(
					minScaleDenominator);
			getPointSymbolizers().get(i).setMaxScaleDenominator(
					maxScaleDenominator);
		}
	}
	...
	public void addSymbolizers2Filter() {
		if (getFilter() != null) {
			...
			for (int i = 0; i < getPolygonSymbolizers().size(); i++) {
				getFilter()
						.addSymbolizer2Filter(getPolygonSymbolizers().get(i));
			}
			for (int i = 0; i < getPointSymbolizers().size(); i++) {
