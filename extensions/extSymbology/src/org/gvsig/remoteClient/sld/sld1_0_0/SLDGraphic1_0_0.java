...
public class SLDGraphic1_0_0 extends SLDGraphic {
	...
	public void parse(XMLSchemaParser parser, int cuTag, String tag)
	...
	/**
	 * Parses the xml data retrieved from the SLD, it will parse the Graphic
	 * element</p>
	 * 
	 * @throws LegendDriverException
	 */
	public void parseGraphic(XMLSchemaParser parser, String tag)
			throws IOException, XmlPullParserException, LegendDriverException {
		int currentTag;
		boolean end = false;
		...
		while (!end) {
			...
			case XMLSchemaParser.START_TAG:
				if (parser.getName().compareTo(SLDTags.EXTERNALGRAPHIC) == 0) {
					...
					if (s == null || "".equals(s)) {
						expressionSize.parse(parser, parser.nextTag(),
								parser.getName());
					} else {
						if (SLDUtils.isANumber(s))
					...
					if (s == null || "".equals(s)) {
						expressionRotation.parse(parser, parser.nextTag(),
								parser.getName());
					} else {
						if (SLDUtils.isANumber(s))
	...
	public String toXML() {
		...
		if (getExpressionOpacity().getLiteral() != null)
			xmlBuilder.writeTag(SLDTags.OPACITY, this.getExpressionOpacity()
					.getLiteral());
		if (getExpressionSize().getLiteral() != null)
			xmlBuilder.writeTag(SLDTags.SIZE, getExpressionSize().getLiteral());
		if (getExpressionRotation().getLiteral() != null)
			xmlBuilder.writeTag(SLDTags.ROTATION, getExpressionRotation()
					.getLiteral());
