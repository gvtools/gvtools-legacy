...
public class NormalizationPanel extends javax.swing.JPanel implements IWindow,
	...
	private void initComponents() {
		...
		jTextFieldName.addKeyListener(new KeyListener() {
			public void keyPressed(KeyEvent e) {
				preFieldName = jTextFieldName.getText();
			}
	...
	private void KeyChangesFieldName(KeyEvent evt) {
		...
		int posi = jTextFieldName.getCaretPosition();
		if (tx.trim().length() < 1) {
			jTextFieldName.setText(preFieldName);
			jTextFieldName.setCaretPosition(0);
		} else if (preFieldName.compareTo(jTextFieldName.getText().trim()) == 0) {
			// nothing to do
		} else {
	...
	private void jButLoadActionPerformed(java.awt.event.ActionEvent evt) {
		...
		NormalizationPattern pat = model.loadPatternXML();
