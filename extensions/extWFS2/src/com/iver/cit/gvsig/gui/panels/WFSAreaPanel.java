 ...
 * There are two options to do this: - Indicating the coordinates of the
 * top-left and down-right corners - Selecting the area with some visual tool
 * 
...
public class WFSAreaPanel extends AbstractWFSPanel {
	...
	private final Rectangle DEFAULT_AREA_COORDINATES_PANEL_RECTANGLE = new Rectangle(
			8, 20, 481, 60);
	private final Rectangle DEFAULT_AREA_MAPCONTROL_PANEL_RECTANGLE = new Rectangle(
			8, 115, 481, 265);
	private final Rectangle DEFAULT_UNDO_ICON_BUTTON_RECTANGLE = new Rectangle(
			250, 85, 25, 25);
	// private final Rectangle DEFAULT_REDO_ICON_BUTTON_RECTANGLE = new
	// Rectangle (277, 85, 25, 25);
	// private final Rectangle DEFAULT_MOVE_ICON_TOGGLE_BUTTON_RECTANGLE = new
	// Rectangle (304, 85, 25, 25);
	// private final Rectangle DEFAULT_SCALING_ICON_TOGGLE_BUTTON_RECTANGLE =
	// new Rectangle (331, 85, 25, 25);
	private final Rectangle DEFAULT_MOVE_ICON_TOGGLE_BUTTON_RECTANGLE = new Rectangle(
			277, 85, 25, 25);
	private final Rectangle DEFAULT_SCALING_ICON_TOGGLE_BUTTON_RECTANGLE = new Rectangle(
			304, 85, 25, 25);
	private final Rectangle DEFAULT_PAN_ICON_BUTTON_RECTANGLE = new Rectangle(
			250, 85, 25, 25);
	private final Rectangle DEFAULT_TOOL_JCOMBOBOX_RECTANGLE = new Rectangle(
			10, 87, 230, 21);
	private final Rectangle DEFAULT_ENABLING_CHECKBOX_RECTANGLE = new Rectangle(
			10, 87, 150, 21);
	private final Rectangle DEFAULT_ZOOMINANDOUT_JCOMBOBOX_RECTANGLE = new Rectangle(
			277, 85, 47, 25);
	private final Rectangle DEFAULT_ZOOMPREVIOUS_ICON_BUTTON_RECTANGLE = new Rectangle(
			326, 85, 25, 25);
	private final Rectangle DEFAULT_ZOOMCOMPLETE_ICON_BUTTON_RECTANGLE = new Rectangle(
			353, 85, 25, 25);
	private final Rectangle DEFAULT_ZOOMINANDOUTVIEWCENTERED_JCOMBOBOX_RECTANGLE = new Rectangle(
			380, 85, 47, 25);
	private final Rectangle DEFAULT_OTHER_TOOLS_JCOMBOBOX_RECTANGLE = new Rectangle(
			429, 85, 47, 25);
	...
	private final String ZOOM_IN_TOOL = "zoomIn"; // This constant must be
													// 'zoomIn' for a correct
													// operation of the tools
													// 'Zoom In' and 'Zoom In
													// Map Centered'
	private final String ZOOM_OUT_TOOL = "zoomOut"; // This constant must be
													// 'zoomOut' for a correct
													// operation of the tools
	...
	private void initCoordinates() {
		...
		Rectangle2D r2d = activeView.getMapControl().getViewPort()
				.getAdjustedExtent();
		// Rectangle2D r2d =
		// activeView.getMapControl().getViewPort().getExtent();

		if (r2d != null) {
			previousExtentValids.put(r2d);

			setCoordinates(r2d.getMaxX(), r2d.getMaxY(), r2d.getMinX(),
					r2d.getMinY());
	...
	/**
	 * <p>
	 * Sets the coordinates of all the text fields. It's supposed that all
	 * coordinates are valid.
	 * </p>
	 * 
	 * @param v1x
	 *            value of the x coordinate of the right-up corner of the
	 *            rectangle
	 * @param v1y
	 *            value of the y coordinate of the right-up corner of the
	 *            rectangle
	 * @param v2x
	 *            value of the x coordinate of the left-up corner of the
	 *            rectangle
	 * @param v2y
	 *            value of the y coordinate of the left-up corner of the
	 *            rectangle
	 */
	private void setCoordinates(double v1x, double v1y, double v2x, double v2y) {
		getCoordinatesPanel().getJTextFieldVertex1X().setText(
				Double.toString(v1x));
		getCoordinatesPanel().getJTextFieldVertex1Y().setText(
				Double.toString(v1y));
		getCoordinatesPanel().getJTextFieldVertex2X().setText(
				Double.toString(v2x));
		getCoordinatesPanel().getJTextFieldVertex2Y().setText(
				Double.toString(v2y));
	}
	 ...
	 * 
	 * @param bbox
	 *            the bounding box that represents a rectangle with the
	 *            information of all coordinates
	 */
	...
	private JCheckBox getEnablingJCheckBox() {
		if (jEnablingCheckBox == null) {
			jEnablingCheckBox = new JCheckBox(PluginServices.getText(this,
					"enabled"));
			jEnablingCheckBox.setBounds(DEFAULT_ENABLING_CHECKBOX_RECTANGLE);
			jEnablingCheckBox.setToolTipText(PluginServices.getText(this,
			...
			jEnablingCheckBox.addItemListener(new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					if (e.getStateChange() == ItemEvent.SELECTED) {
						jEnablingCheckBox.setToolTipText(PluginServices
								.getText(this, "disable_filter_by_area"));
						enableCoorderatesRegionOperation();
					} else {
						jEnablingCheckBox.setToolTipText(PluginServices
								.getText(this, "enable_filter_by_area"));
	...
	private JComboBox getJComboBoxToolSelection() {
		if (jComboBoxToolSelection == null) {
			...
			jComboBoxToolSelection.setToolTipText(PluginServices.getText(this,
					"select_a_tool"));
			jComboBoxToolSelection.setEditable(false);
			jComboBoxToolSelection.addItem(new ItemOperation(PluginServices
					.getText(this, "disabled"), this.DISABLED_OPERATION));
			jComboBoxToolSelection.addItem(new ItemOperation(PluginServices
					.getText(this, "define_absolute_coordinates"),
					this.SELECTION_BY_COORDINATES_OPERATION));
			jComboBoxToolSelection.addItem(new ItemOperation(PluginServices
					.getText(this, "define_coordinates_using_view"),
					this.SELECTION_BY_AREA_OPERATION));
			...
			jComboBoxToolSelection.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					switch (((ItemOperation) cb.getSelectedItem())
							.getOperation()) {
					...
					case SELECTION_BY_AREA_OPERATION:
						// Only enable the area operation if there is data
						// loaded in this MapControl
						if (getSelectableMapAreaPanel().getMapContext()
								.getLayers().getLayersCount() > 0) {
							disableCoorderatesRegionOperation();
							enableAreaRegionOperation();
						} else {
							JOptionPane.showMessageDialog(
									jComboBoxToolSelection,
									PluginServices
											.getText(null,
													"there_is_no_layer_loaded_in_the_active_view"),
									PluginServices.getText(null, "information"),
	...
	private void enableCoorderatesRegionOperation() {
		...
		// Update extent (we will use the adjusted extent because increases the
		// usability)
	...
	private void enableAreaRegionOperation() {
		...
		getJComboBoxZoomInAndOutViewCentered().revalidate(); // Force update
																// this
																// component
	...
	/**
	 * Disables the components associated to the selection of area by
	 * coordinates
	 */
	...
	private JButton getJButtonUndo() {
		if (jButtonUndo == null) {
			...
			jButtonUndo.setToolTipText(PluginServices.getText(this,
					"previous_area"));
	...
	// jButtonRedo.setIcon(new
	// ImageIcon(getClass().getClassLoader().getResource("images/edit-redo.png")));
	// jButtonRedo.setToolTipText(PluginServices.getText(this,
	// "following_area"));
	// }
	//
	// return jButtonRedo;
	...
	private JToggleButton getJToggleButtonMove() {
		if (jToggleButtonMove == null) {
			...
			jToggleButtonMove
					.setBounds(DEFAULT_MOVE_ICON_TOGGLE_BUTTON_RECTANGLE);
			jToggleButtonMove.setIcon(PluginServices.getIconTheme().get(
					"WFS-move"));
			jToggleButtonMove.setToolTipText(PluginServices.getText(this,
					"move")
					+ ": "
					+ PluginServices.getText(this, "area_move_explanation"));
			jToggleButtonMove.addItemListener(new ItemListener() {
				 ...
				 */
				public void itemStateChanged(ItemEvent e) {
					// Isn't allowed that the two JToggleButton would be
					// selected (can't move and scale)
	...
	private JToggleButton getJToggleButtonScaling() {
		if (jToggleButtonScaling == null) {
			...
			jToggleButtonScaling
					.setBounds(DEFAULT_SCALING_ICON_TOGGLE_BUTTON_RECTANGLE);
			jToggleButtonScaling.setIcon(PluginServices.getIconTheme().get(
					"WFS-scaling"));
			jToggleButtonScaling.setToolTipText(PluginServices.getText(this,
					"scaling")
					+ ": "
					+ PluginServices.getText(this, "area_scaling_explanation"));
	...
	private JButton getJButtonPan() {
		if (jButtonPan == null) {
			...
			jButtonPan.setToolTipText(PluginServices.getText(this,
					"Desplazamiento"));
	...
	private JComboBoxWithImageIconItems getJComboBoxZoomInAndOut() {
		if (jComboBoxZoomInAndOut == null) {
			...
			jComboBoxZoomInAndOut
					.setBounds(DEFAULT_ZOOMINANDOUT_JCOMBOBOX_RECTANGLE);
			jComboBoxZoomInAndOut.addImageIconItem(new ImageIconItemInfo(
					"images/ZoomIn.png", PluginServices.getIconTheme().get(
							"view-zoom-in"), PluginServices.getText(this,
							"Zoom_Mas"), ZOOM_IN_TOOL));
			jComboBoxZoomInAndOut.addImageIconItem(new ImageIconItemInfo(
					"images/ZoomOut.png", PluginServices.getIconTheme().get(
							"view-zoom-out"), PluginServices.getText(this,
							"Zoom_Menos"), ZOOM_OUT_TOOL));
			jComboBoxZoomInAndOut.addActionListener(new ActionListener() {
				 ...
				 */
				public void actionPerformed(ActionEvent e) {
					JComboBoxWithImageIconItems cb = (JComboBoxWithImageIconItems) e
							.getSource();
					ImageIconItemInfo iiii = (ImageIconItemInfo) cb
							.getSelectedItem();
					getSelectableMapAreaPanel().setTool(
							(String) iiii.getItemValue());
				}
			});
		}
	...
	private JButton getJButtonZoomPrevious() {
		if (jButtonZoomPrevious == null) {
			...
			jButtonZoomPrevious
					.setBounds(DEFAULT_ZOOMPREVIOUS_ICON_BUTTON_RECTANGLE);
			jButtonZoomPrevious.setIcon(PluginServices.getIconTheme().get(
					"view-zoom-back"));
			jButtonZoomPrevious.setToolTipText(PluginServices.getText(this,
					"Zoom_Previo"));
	...
	private JButton getJButtonZoomComplete() {
		if (jButtonZoomComplete == null) {
			...
			jButtonZoomComplete
					.setBounds(DEFAULT_ZOOMCOMPLETE_ICON_BUTTON_RECTANGLE);
			jButtonZoomComplete.setIcon(PluginServices.getIconTheme().get(
					"view-zoom-map-contents"));
			jButtonZoomComplete.setToolTipText(PluginServices.getText(this,
					"Zoom_Completo"));
			jButtonZoomComplete.addMouseListener(new MouseAdapter() {
				 ...
				 */
				public void mouseClicked(MouseEvent e) {
					getSelectableMapAreaPanel().getViewPort().setExtent(
							getSelectableMapAreaPanel().getMapContext()
									.getLayers().getFullExtent());
					BaseView view = ((BaseView) PluginServices.getMDIManager()
							.getActiveWindow());
					view.repaintMap();
				}
	...
	private JComboBoxWithImageIconItems getJComboBoxZoomInAndOutViewCentered() {
		if (jComboBoxZoomInAndOutViewCentered == null) {
			...
			jComboBoxZoomInAndOutViewCentered
					.setBounds(DEFAULT_ZOOMINANDOUTVIEWCENTERED_JCOMBOBOX_RECTANGLE);
			jComboBoxZoomInAndOutViewCentered
					.addImageIconItem(new ImageIconItemInfo(
							"images/zoommas.png", PluginServices.getIconTheme()
									.get("view-zoom-mas"), PluginServices
									.getText(this, "Zoom_Acercar"),
							ZOOM_IN_VIEW_CENTERED_TOOL));
			jComboBoxZoomInAndOutViewCentered
					.addImageIconItem(new ImageIconItemInfo(
							"images/ZoomOut.png", PluginServices.getIconTheme()
									.get("view-zoom-menos"), PluginServices
									.getText(this, "Zoom_Alejar"),
							ZOOM_OUT_VIEW_CENTERED_TOOL));
			jComboBoxZoomInAndOutViewCentered
					.addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent e) {
							...
							if (((String) iiii.getItemValue())
									.compareTo(ZOOM_IN_VIEW_CENTERED_TOOL) == 0) {
	...
	private JComboBoxWithImageIconItems getJComboBoxOtherTools() {
		if (jComboBoxOtherTools == null) {
			...
			jComboBoxOtherTools
					.setBounds(DEFAULT_OTHER_TOOLS_JCOMBOBOX_RECTANGLE);
			jComboBoxOtherTools.addImageIconItem(new ImageIconItemInfo(
					"images/Distancia.png", PluginServices.getIconTheme().get(
							"view-query-distance"), PluginServices.getText(
							this, "medir_distancias"), MEASURE_DISTANCES_TOOL));
			jComboBoxOtherTools.addImageIconItem(new ImageIconItemInfo(
					"images/Poligono16.png", PluginServices.getIconTheme().get(
							"view-query-area"), PluginServices.getText(this,
							"medir_area"), MEASURE_AREA_TOOL));
	...
	public void setExtent(Rectangle2D extent) {
		if (extent != null) {
			...
			if (getSelectableMapAreaPanel().getMapContext().getLayers()
					.getLayersCount() > 0) {
				getSelectableMapAreaPanel().getMapContext().getViewPort()
						.setExtent(extent);
			}
		}
	}
	...
	/**
	 * Gets the extent: a 'Rectangle2D.Double' object reference or 'null' if
	 * there is no data or if the data is invalid or user hasn't defined an area
	 * 
	...
	public Rectangle2D getExtent() {
		...
		if (getCoordinatesPanel().areAllCoordinatesUndefined()) {
			...
			if (getCoordinatesPanel().validVertexes()) {
				// System.out.println("Extent que devuelve: " +
				// getCoordinatesPanel().getExtent());
	...
	/**
	 * Set parent's 'Applicable' button enable or disable according the value of
	 * the parameter
	 * 
	 * @param b
	 *            A boolean value
	...
	private void setApplicable(boolean b) {
		...
		if (!((WFSParamsPanel) panelGroup).getFilterPanel()
	...
	/**
	 * If there is some coordinate text field that has text, returns true; else
	 * returns false
	 * 
	...
	/**
	 * If there is some coordinate text field that isn't undefined, returns
	...
	/**
	 * Restores the inner attribute 'hasUserDefinedAnArea' to its default value
	 * (false)
	 */
	...
	public void updateWFSArea() {
		...
		// To prevent that events that take place (are produced) could be a
		// nuisance to the load of the layers
		this.getSelectableMapAreaPanel().getMapContext().beginAtomicEvent();

		try {
			MapContext mapContext = this.getSelectableMapAreaPanel()
					.getMapContext();
			...
			// Update extent (we will use the adjusted extent because increases
			// the usability)
			Rectangle2D r2D = vP.getAdjustedExtent();
			// Rectangle2D r2D = vP.getExtent();

			if (r2D == null) {
				// End To prevent that events that take place(are produced)
				// could be a nuisance to the load of the layers
				this.getSelectableMapAreaPanel().getMapContext()
						.endAtomicEvent();
			...
			for (i = (numberOfLayers - 1); i >= 0; i--) {
				mapContext.getLayers().addLayer(
						mC.getLayers().getLayer(i).cloneLayer());
			}

			// If has to enable all listeners of all tools on the view area
			if (((ItemOperation) this.getJComboBoxToolSelection()
					.getSelectedItem()).getOperation() == SELECTION_BY_AREA_OPERATION)
				this.getSelectableMapAreaPanel().enableAllMouseListeners();

			// End To prevent that events that take place(are produced) could be
			// a nuisance to the load of the layers
	...
	/**
	 * Represents an object that stores the necessary information for know each
	 * operation of the 'jComboBoxToolSelection'
	 * 
	...
	/**
	 * This class is a panel width four fields for indicate the coordinates of
	 * two points: One for the ritgh-up point and another for the left-down
	 * point of a rectangle area
	 * 
	...
	private class AreaCoordinatesPanel extends JPanel {
		private final Rectangle DEFAULT_JBUTTON_GO_TO_AREA_RECTANGLE = new Rectangle(
				412, 5, 62, 51); // @jve:decl-index=0:
		...
		private final int DEFAULT_COORDINATE_WIDTH = DEFAULT_COORDIANTE_LABEL_WIDTH
				+ DEFAULT_TEXT_FIELDS_WIDTH + 10;
		private final int DEFAULT_COORDINATE_HEIGHT = 25;
		private final Dimension DEFAULT_JLABEL_VERTEX_DIMENSION = new Dimension(
				60, DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT); // @jve:decl-index=0:
		private final int DEFAULT_XY_COORDINATES_WIDTH = 2
				* DEFAULT_COORDINATE_WIDTH
				+ DEFAULT_JLABEL_VERTEX_DIMENSION.width + 10;
		...
		private void initialize() {
			...
			this.setToolTipText(PluginServices.getText(this,
			...
			this.add(getJButtonValidateArea());
		}
		...
		/**
		 * Sets all flags about if a coordinate has change to false (haven't
		 * changed)
		 */
		...
		/**
		 * last_Coordinated_that_Changed This method initializes jPanelVertex1
		 * 
		 * @return javax.swing.JPanel
		 */
		private JPanel getJPanelVertex1() {
			if (jPanelVertex1 == null) {
				...
				jPanelVertex1
						.setBounds(DEFAULT_X_LOCATION,
								DEFAULT_Y_LEFT_UP_CORNER_LOCATION,
								DEFAULT_XY_COORDINATES_WIDTH,
		...
		private JLabel getJLabelVertex1() {
			if (jLabelVertex1 == null) {
				...
				jLabelVertex1.setToolTipText(PluginServices.getText(this,
						"coordinates_from_area_right_up_vertex"));
				jLabelVertex1.setText(PluginServices.getText(this, "vertex")
						+ " 1:");
				jLabelVertex1.setHorizontalAlignment(SwingConstants.LEFT);
				jLabelVertex1.setVerticalAlignment(SwingConstants.CENTER);
				jLabelVertex1.setForeground(new Color(0, 0, 255)); // Blue color
																	// for text
			}
		...
		private JLabel getJLabelVertex2() {
			if (jLabelVertex2 == null) {
				...
				jLabelVertex2.setToolTipText(PluginServices.getText(this,
						"coordinates_from_area_left_bottom_vertex"));
				jLabelVertex2.setText(PluginServices.getText(this, "vertex")
						+ " 2:");
				jLabelVertex2.setHorizontalAlignment(SwingConstants.LEFT);
				jLabelVertex2.setVerticalAlignment(SwingConstants.CENTER);
				jLabelVertex2.setForeground(new Color(128, 64, 0)); // Brown
																	// color for
																	// text
			}
		...
		private JPanel getJPanelVertex1X() {
			if (jPanelVertex1X == null) {
				...
				jPanelVertex1X.setPreferredSize(new Dimension(new Dimension(
						DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
		...
		private JPanel getJPanelVertex1Y() {
			if (jPanelVertex1Y == null) {
				...
				jPanelVertex1Y.setPreferredSize(new Dimension(new Dimension(
						DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
		...
		private JPanel getJPanelVertex2X() {
			if (jPanelVertex2X == null) {
				...
				jPanelVertex2X.setPreferredSize(new Dimension(new Dimension(
						DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
		...
		private JPanel getJPanelVertex2Y() {
			if (jPanelVertex2Y == null) {
				...
				jPanelVertex2Y.setPreferredSize(new Dimension(new Dimension(
						DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
		...
		private JLabel getJLabelVertex1X() {
			if (jLabelVertex1X == null) {
				...
				jLabelVertex1X.setPreferredSize(new java.awt.Dimension(
						DEFAULT_COORDIANTE_LABEL_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jLabelVertex1X.setToolTipText(PluginServices.getText(this,
				...
				jLabelVertex1X.setForeground(new Color(0, 0, 255)); // Blue
																	// color for
																	// text
			}
		...
		private JLabel getJLabelVertex2X() {
			if (jLabelVertex2X == null) {
				...
				jLabelVertex2X.setPreferredSize(new java.awt.Dimension(
						DEFAULT_COORDIANTE_LABEL_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jLabelVertex2X.setToolTipText(PluginServices.getText(this,
				...
				jLabelVertex2X.setForeground(new Color(128, 64, 0)); // Brown
																		// color
																		// for
																		// text
			}
		...
		private JLabel getJLabelVertex1Y() {
			if (jLabelVertex1Y == null) {
				...
				jLabelVertex1Y.setPreferredSize(new java.awt.Dimension(
						DEFAULT_COORDIANTE_LABEL_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jLabelVertex1Y.setToolTipText(PluginServices.getText(this,
				...
				jLabelVertex1Y.setForeground(new Color(0, 0, 255)); // Blue
																	// color for
																	// text
			}
		...
		private JLabel getJLabelVertex2Y() {
			if (jLabelVertex2Y == null) {
				...
				jLabelVertex2Y.setPreferredSize(new java.awt.Dimension(
						DEFAULT_COORDIANTE_LABEL_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jLabelVertex2Y.setToolTipText(PluginServices.getText(this,
				...
				jLabelVertex2Y.setForeground(new Color(128, 64, 0)); // Brown
																		// color
																		// for
																		// text
			}
		...
		/**
		 * Returns a focus listener for validate the text of a JTextField when
		 * that component loses its focus
		 * 
		...
		private FocusListener getFocusListenerForCoordinateValidation() {
			if (focusListenerForCoordinateValidation == null) {
				focusListenerForCoordinateValidation = new FocusListener() {
					 ...
					 * 
					 * @see
					 * java.awt.event.FocusListener#focusGained(java.awt.event
					 * .FocusEvent)
					 */
					 ...
					 * 
					 * @see
					 * java.awt.event.FocusListener#focusLost(java.awt.event
					 * .FocusEvent)
					 */
					public void focusLost(FocusEvent e) {
						...
						// If a coordinate has lost focus, is valid and has
						// changed -> store the new value and set its related
						...
						if (jTF.getName().compareTo(JTEXTFIELD_V1X_NAME) == 0) {
							...
							if (text.compareTo(last_Coordinates[0]) != 0) {
								// If the text written isn't a number, restores
								// the previous value of that coordinate
								...
								// if
								// (getJComboBoxToolSelection().getSelectedIndex()
								// != 0) {
		...
		private JTextFieldWithSCP getJTextFieldVertex1X() {
			if (jTextFieldVertex1X == null) {
				...
				jTextFieldVertex1X.setPreferredSize(new Dimension(
						DEFAULT_TEXT_FIELDS_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jTextFieldVertex1X.setToolTipText(PluginServices.getText(this,
						"x_coordinate_from_area_right_up_vertex"));
				jTextFieldVertex1X
						.addFocusListener(getFocusListenerForCoordinateValidation());
				jTextFieldVertex1X
						.addKeyListener(getKeyListenerForCoordinateValidation());
		...
		private JTextFieldWithSCP getJTextFieldVertex1Y() {
			if (jTextFieldVertex1Y == null) {
				...
				jTextFieldVertex1Y.setPreferredSize(new Dimension(
						DEFAULT_TEXT_FIELDS_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jTextFieldVertex1Y.setToolTipText(PluginServices.getText(this,
						"y_coordinate_from_area_right_up_vertex"));
				jTextFieldVertex1Y
						.addFocusListener(getFocusListenerForCoordinateValidation());
				jTextFieldVertex1Y
		...
		private JTextFieldWithSCP getJTextFieldVertex2X() {
			if (jTextFieldVertex2X == null) {
				...
				jTextFieldVertex2X.setPreferredSize(new Dimension(
						DEFAULT_TEXT_FIELDS_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jTextFieldVertex2X.setToolTipText(PluginServices.getText(this,
						"x_coordinate_from_area_left_bottom_vertex"));
				jTextFieldVertex2X
						.addFocusListener(getFocusListenerForCoordinateValidation());
				jTextFieldVertex2X
		...
		private JTextFieldWithSCP getJTextFieldVertex2Y() {
			if (jTextFieldVertex2Y == null) {
				...
				jTextFieldVertex2Y.setPreferredSize(new Dimension(
						DEFAULT_TEXT_FIELDS_WIDTH,
						DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
				jTextFieldVertex2Y.setToolTipText(PluginServices.getText(this,
						"y_coordinate_from_area_left_bottom_vertex"));
				jTextFieldVertex2Y
						.addFocusListener(getFocusListenerForCoordinateValidation());
				jTextFieldVertex2Y
		...
		/**
		 * Validates the coordinate of a JTextField: returns true if it's valid;
		 * else returns false
		 * 
		 * @param coordinate
		...
		private boolean validateCoordinate(JTextField coordinate) {
			if (coordinate != null) {
				// If the format of the coordinate is incorrect, shows a message
				// warning the user that problem and resets the coordinate text
				if ((coordinate.getText().compareTo("") != 0)
						&& (!StringNumberUtilities
								.isRealNumberWithIntegerExponent(coordinate
										.getText()))) {
					...
					// case 1:
					// coordinate.setText(previous_Coordinate_Value[0]); // Set
					// last value
					// break;
					// case 2:
					// coordinate.setText(previous_Coordinate_Value[1]); // Set
					// last value
					// break;
					// case 3:
					// coordinate.setText(previous_Coordinate_Value[2]); // Set
					// last value
					// break;
					// case 4:
					// coordinate.setText(previous_Coordinate_Value[3]); // Set
					// last value
					// break;
					// default:
					// // Do nothing
		...
		private boolean validateCoordinate(int coordinate_ID) {
			...
			case JTEXTFIELD_V1X:
				...
				if ((text.compareTo("") != 0)
						&& (!StringNumberUtilities
								.isRealNumberWithIntegerExponent(text))) {
					JOptionPane.showMessageDialog(this, PluginServices.getText(
							null, "incorrect_coordinate_format"),
							PluginServices.getText(null, "error"),
							JOptionPane.ERROR_MESSAGE);
					coordinate.setText(previous_Coordinate_Value[0]); // Set
																		// last
			...
			// switch (coordinate_ID) {
			// case 1:
			// coordinate.setText(previous_Coordinate_Value[0]); // Set last
			// value
			// break;
			// case 2:
			// coordinate.setText(previous_Coordinate_Value[1]); // Set last
			// value
			// break;
			// case 3:
			// coordinate.setText(previous_Coordinate_Value[2]); // Set last
			// value
			// break;
			// case 4:
			// coordinate.setText(previous_Coordinate_Value[3]); // Set last
			// value
			// break;
			// default:
			// // Do nothing
		...
		private JButton getJButtonValidateArea() {
			if (jButtonPreviewArea == null) {
				...
				jButtonPreviewArea
						.setBounds(DEFAULT_JBUTTON_GO_TO_AREA_RECTANGLE);
				// jButtonPreviewArea.setToolTipText(PluginServices.getText(this,
				// "to_previsualize"));
				jButtonPreviewArea.setToolTipText(PluginServices.getText(this,
				...
				jButtonPreviewArea.setIcon(PluginServices.getIconTheme().get(
						"layout-show-tag"));
				jButtonPreviewArea.addMouseListener(new MouseAdapter() {
					...
					// */
					// public void mouseClicked(MouseEvent e) {
					// // Only enable the area operation if there is data loaded
					// in this MapControl
					//
					// // Do nothing if there is no layer in the map control
					// if
					// (getSelectableMapAreaPanel().getMapContext().getLayers().getLayersCount()
					// == 0) {
					// return;
					// }
					//
					// if ((validateCoordinate(getJTextFieldVertex1X())) &&
					// (validateCoordinate(getJTextFieldVertex1Y())) &&
					// (validateCoordinate(getJTextFieldVertex2X())) &&
					// (validateCoordinate(getJTextFieldVertex2Y()))) {
					...
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea,
					// "problem_with_the_new_calculated_area") + ": " +
					// PluginServices.getText(jButtonPreviewArea, "v1x") + " < "
					// + PluginServices.getText(jButtonPreviewArea, "v2x"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
					// JOptionPane.ERROR_MESSAGE);
					// restoreAllModifiedCoordinates(); // restores the
					// coordinates
					// return;
					// }
					//
					...
					// PluginServices.getText(jButtonPreviewArea,
					// "problem_with_the_new_calculated_area") + ": " +
					// PluginServices.getText(jButtonPreviewArea, "v1y") + " < "
					// + PluginServices.getText(jButtonPreviewArea, "v2y"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
					// JOptionPane.ERROR_MESSAGE);
					// restoreAllModifiedCoordinates(); // restores the
					// coordinates
					...
					// }
					// else {
					// // If there has been an error -> can't move different
					// distances in X the two vertexes
					// if (sx != sy) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea,
					// "problem_with_the_new_calculated_area") + ": " +
					...
					// else {
					// // If there has been an error -> can't move different
					// distances in Y the two vertexes
					// if (d1 != d2) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea,
					...
					// if (hasChanged_v1X) {
					// if (getDoubleValueOfJTextFieldV1X() <
					// getDoubleValueOfJTextFieldV2X()) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea, "v1x") + " < "
					// + PluginServices.getText(jButtonPreviewArea, "v2x"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
					...
					//
					// if (hasChanged_v1Y) {
					// if (getDoubleValueOfJTextFieldV1Y() <
					// getDoubleValueOfJTextFieldV2Y()) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea, "v1y") + " < "
					// + PluginServices.getText(jButtonPreviewArea, "v2y"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
					...
					//
					// if (hasChanged_v2X) {
					// if (getDoubleValueOfJTextFieldV2X() >
					// getDoubleValueOfJTextFieldV1X()) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea, "v2x") + " > "
					// + PluginServices.getText(jButtonPreviewArea, "v1x"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
					...
					//
					// if (hasChanged_v2Y) {
					// if (getDoubleValueOfJTextFieldV2Y() >
					// getDoubleValueOfJTextFieldV1Y()) {
					// JOptionPane.showMessageDialog(jButtonPreviewArea,
					// PluginServices.getText(jButtonPreviewArea, "v2y") + " > "
					// + PluginServices.getText(jButtonPreviewArea, "v1y"),
					// PluginServices.getText(jButtonPreviewArea, "error"),
		...
		private Rectangle2D getNewRectangleByCoordinates() {
			return new Rectangle2D.Double(getDoubleValueOfJTextFieldV2X(),
					getDoubleValueOfJTextFieldV2Y(),
					getDoubleValueOfJTextFieldV1X()
							- getDoubleValueOfJTextFieldV2X(),
					getDoubleValueOfJTextFieldV1Y()
		...
		/**
		 * Restores the value of all text fields with coordinates modified to
		 * their last value
		 */
		...
		private ValidationInfo validVertexesCoordinates() {
			...
			if (!validateCoordinate(getJTextFieldVertex1X())) {
				if (infoSufix == null) {
					infoPrefix = PluginServices.getText(this,
							"invalid_coordinate") + ": ";
				...
				// infoSufix += ", " + PluginServices.getText(this, "v1x");
				// }
				// return new ValidationInfo(false, PluginServices.getText(this,
				// "invalid_coordinate") + ": " + PluginServices.getText(this,
				// "v1x"));
			}
		...
		/**
		 * Validates that Vertex1 is in the right-up corner and the Vertex2 in
		 * the left-down corner of the area/rectangle <br>
		 * (It's supposed that coordinates of the vertexes are valid) <br>
		 * (It's supposed that going to right or up, the coordinate value
		 * increases)
		 * 
		 * @return A ValidationInfo object reference
		 */
		private ValidationInfo validVertexesPositions() {
			...
			if (getJTextFieldVertex1X().getText().compareTo("") == 0) {
				...
				// }
				// return new ValidationInfo(false, PluginServices.getText(this,
				// "undefined_coordinate") + ": " + PluginServices.getText(this,
				// "v1x"));
			}
			...
			// Check if the vertex V1 is in the right-up corner and the vertex
			// V2 is in the left-down corner
			if (getDoubleValueOfJTextFieldV1X() < getDoubleValueOfJTextFieldV2X()) {
				getJTextFieldVertex1X().setBackground(Color.RED);
				getJTextFieldVertex2X().setBackground(Color.RED);
				return new ValidationInfo(false, PluginServices.getText(this,
						"v1x") + " < " + PluginServices.getText(this, "v2x"));
			}
			...
			if (getDoubleValueOfJTextFieldV1Y() < getDoubleValueOfJTextFieldV2Y()) {
				...
				getJTextFieldVertex2Y().setBackground(Color.RED);
				return new ValidationInfo(false, PluginServices.getText(this,
						"v1y") + " < " + PluginServices.getText(this, "v2y"));
			}
		...
		// if (! validateCoordinate(getJTextFieldVertex1X())) {
		// return new ValidationInfo(false, PluginServices.getText(this,
		// "invalid_coordinate") + ": " + PluginServices.getText(this, "v1x"));
		// }
		//
		// if (! validateCoordinate(getJTextFieldVertex1Y())) {
		// return new ValidationInfo(false, PluginServices.getText(this,
		// "invalid_coordinate") + ": " + PluginServices.getText(this, "v1y"));
		// }
		//
		// if (! validateCoordinate(getJTextFieldVertex2X())) {
		// return new ValidationInfo(false, PluginServices.getText(this,
		// "invalid_coordinate") + ": " + PluginServices.getText(this, "v2x"));
		// }
		//
		// if (! validateCoordinate(getJTextFieldVertex2Y())) {
		// return new ValidationInfo(false, PluginServices.getText(this,
		// "invalid_coordinate") + ": " + PluginServices.getText(this, "v2y"));
		// }
		//
		// // If arrives here, all coordinates are valid
		// return new ValidationInfo(true, "");
		...
		// }
		//
		// // Check if the vertex V1 is in the right-up corner and the vertex V2
		// is in the left-down corner
		// if (getDoubleValueOfJTextFieldV1X() <
		// getDoubleValueOfJTextFieldV2X()) {
		// return new ValidationInfo(false, PluginServices.getText(this, "v1x")
		// + " < " + PluginServices.getText(this, "v2x"));
		// }
		//
		// if (getDoubleValueOfJTextFieldV1Y() <
		// getDoubleValueOfJTextFieldV2Y()) {
		// return new ValidationInfo(false, PluginServices.getText(this, "v1y")
		// + " < " + PluginServices.getText(this, "v2y"));
		// }
		//
		// // If arrives here -> vertexes positions are valid
		// return new ValidationInfo(true, "");
		...
		/**
		 * Validate each coordinate of each vertex and the position of the
		 * vertexes
		 * 
		 * @return A boolean value
		 */
		public boolean validVertexes() {
			...
			if (!v1.isValid()) {
				...
				JOptionPane.showMessageDialog(null, PluginServices.getText(
						null,
						PluginServices.getText(this, "invalid_coordinates")
								+ ": " + v1.getMessage()), PluginServices
						.getText(null, "warning"), JOptionPane.WARNING_MESSAGE);

				// Restore the previous valid extent
				if (previousExtentValids.hasPrevious()) {
					Rectangle2D previousExtent = previousExtentValids
							.removePrev();
				...
				if (!v2.isValid()) {
					...
					// Once has been validated -> allow apply with the new
					// extent, and save it
					if (previousExtentValids.hasPrevious()) {
						Rectangle2D previousExtent = previousExtentValids
								.removePrev();
		...
		/**
		 * <p>
		 * Compares two bi-dimensional rectangles, and returns <code>true</code>
		 * if both are equals.
		 * </p>
		 * 
		 * @param rect1
		 *            first rectangle 2D to compare
		 * @param rect2
		 *            second rectangle 2D to compare
		 * 
		 * @return <code>true</code> if both are equals; otherwise
		 *         <code>false</code>
		 */
		private boolean areEqual(Rectangle2D rect1, Rectangle2D rect2) {
			return ((rect1.getX() == rect2.getX())
					&& (rect1.getY() == rect2.getY())
					&& (rect1.getWidth() == rect2.getWidth()) && (rect1
						.getHeight() == rect2.getHeight()));
		}

		/**
		 * Gets the extent (rectangle) represented by the values of the
		...
		/**
		 * Returns true if there is some coordinate text field with data; else
		 * returns false
		 * 
		 * @return A boolean value
		 */
		public boolean areThereSomeCoordinatesWritten() {
			return ((getJTextFieldVertex1X().getText().compareTo("") != 0)
					| (getJTextFieldVertex1Y().getText().compareTo("") != 0)
					| (getJTextFieldVertex2X().getText().compareTo("") != 0) | (getJTextFieldVertex2Y()
					.getText().compareTo("") != 0));
		}

		/**
		 * Returns true if all coordinate text fields are undefined (without
		 * values)
		 * 
		 * @return A boolean value
		 */
	...
	/**
	 * This class is a MapControl JComponent that has visual information and
	 * allows user interact with some tools and view the results
	 * 
	 * @author Pablo Piqueras Bartolomï¿½ (p_queras_pab@hotmail.com)
	 */
	private class SelectableMapControlAreaPanel extends MapControl {
		...
		public void initialize() {
			...
			/*
			 * Sets a clone of the current active view map context to this
			 * MapControl
			 */
			try {
				MapContext mp = ((BaseView) PluginServices.getMDIManager()
						.getActiveWindow()).getMapControl().getMapContext();

				this.setMapContext(mp.cloneFMap());
				ViewPort vP = this.getViewPort();

				// We will use the adjusted extent because increases the
				// usability
				Rectangle2D r2D = vP.getAdjustedExtent();
				// Rectangle2D r2D = vP.getExtent();
		...
		/**
		 * Adds listeners that allows user interact with it's mouse over this
		 * component and that represents different tools
		 */
		private void addToolsListeners() {
			// MOVEMENT EVENTS LISTENER: sets mouse coordinates to the status
			// bar
			StatusBarListener sbl = new StatusBarListener(this);

			// ZOOM OUT (Presses on the map and it will be centered showing a
			// bigger area)
			// Only pressing, not selecting a rectangle area
			ZoomOutListener zol = new ZoomOutListener(this);
			this.addMapTool(ZOOM_OUT_TOOL, new Behavior[] {
			...
			this.getViewPort().addViewPortListener(new ViewPortListener() {
				 ...
				 * 
				 * @see
				 * com.iver.cit.gvsig.fmap.ViewPortListener#extentChanged(com
				 * .iver.cit.gvsig.fmap.ExtentEvent)
				 */
				public void extentChanged(ExtentEvent e) {
					if (!getEnablingJCheckBox().isSelected())
						return;

					if (getMapContext().getViewPort().getExtents()
							.hasPrevious()) {
						// We will use the adjusted extent because increases the
						// usability
				 ...
				 * 
				 * @see
				 * com.iver.cit.gvsig.fmap.ViewPortListener#backColorChanged
				 * (com.iver.cit.gvsig.fmap.ColorEvent)
				 */
				 ...
				 * 
				 * @see
				 * com.iver.cit.gvsig.fmap.ViewPortListener#projectionChanged
				 * (com.iver.cit.gvsig.fmap.ProjectionEvent)
				 */
		...
		public void disableAllMouseListeners() {
			...
			// Only disable listeners if there are listeners to remove (it's
			// supposed that there are always the same number of mouse
			// listeners, and this listeners
			// are referenciated by this component or by the private attribute
			// 'mouseListeners')
	...
	private class ValidationInfo {
		 ...
		 *            If was valid or not
		 * @param message
		 *            A message associate to the validation (If has been valid,
		 *            this attribute should be "", in the other case should have
		 *            a message with an explanation about the invalidation)
		 */
	...
	protected void initialize() {
		...
		this.setBorder(javax.swing.BorderFactory.createTitledBorder(null,
				PluginServices.getText(this, "select_by_area"),
		...
		// this.getJComboBoxToolSelection().setSelectedIndex(0); // By default
...
// private final Rectangle DEFAULT_BOUNDS = new Rectangle(10, 5, 490, 380);
// private final Rectangle DEFAULT_AREA_COORDINATES_PANEL_RECTANGLE = new
// Rectangle(8, 20, 481, 60);
// private final Rectangle DEFAULT_AREA_MAPCONTROL_PANEL_RECTANGLE = new
// Rectangle(8, 115, 481, 265);
// private final Rectangle DEFAULT_UNDO_ICON_BUTTON_RECTANGLE = new Rectangle
// (250, 85, 25, 25);
// // private final Rectangle DEFAULT_REDO_ICON_BUTTON_RECTANGLE = new Rectangle
// (277, 85, 25, 25);
// // private final Rectangle DEFAULT_MOVE_ICON_TOGGLE_BUTTON_RECTANGLE = new
// Rectangle (304, 85, 25, 25);
// // private final Rectangle DEFAULT_SCALING_ICON_TOGGLE_BUTTON_RECTANGLE = new
// Rectangle (331, 85, 25, 25);
// private final Rectangle DEFAULT_MOVE_ICON_TOGGLE_BUTTON_RECTANGLE = new
// Rectangle (277, 85, 25, 25);
// private final Rectangle DEFAULT_SCALING_ICON_TOGGLE_BUTTON_RECTANGLE = new
// Rectangle (304, 85, 25, 25);
// private final Rectangle DEFAULT_PAN_ICON_BUTTON_RECTANGLE = new
// Rectangle(250, 85, 25, 25);
// private final Rectangle DEFAULT_TOOL_JCOMBOBOX_RECTANGLE = new Rectangle(10,
// 87, 230, 21);
// private final Rectangle DEFAULT_ENABLING_CHECKBOX_RECTANGLE = new
// Rectangle(10, 87, 150, 21);
// private final Rectangle DEFAULT_ZOOMINANDOUT_JCOMBOBOX_RECTANGLE = new
// Rectangle(277, 85, 47, 25);
// private final Rectangle DEFAULT_ZOOMPREVIOUS_ICON_BUTTON_RECTANGLE = new
// Rectangle(326, 85, 25, 25);
// private final Rectangle DEFAULT_ZOOMCOMPLETE_ICON_BUTTON_RECTANGLE = new
// Rectangle(353, 85, 25, 25);
// private final Rectangle DEFAULT_ZOOMINANDOUTVIEWCENTERED_JCOMBOBOX_RECTANGLE
// = new Rectangle(380, 85, 47, 25);
// private final Rectangle DEFAULT_OTHER_TOOLS_JCOMBOBOX_RECTANGLE = new
// Rectangle(429, 85, 47, 25);
//
// private final int DISABLED_OPERATION = 0;
// private final int SELECTION_BY_COORDINATES_OPERATION = 1;
// private final int SELECTION_BY_AREA_OPERATION = 2;
...
// private final String PAN_TOOL = "HAND";
// private final String ZOOM_IN_TOOL = "zoomIn"; // This constant must be
// 'zoomIn' for a correct operation of the tools 'Zoom In' and 'Zoom In Map
// Centered'
// private final String ZOOM_OUT_TOOL = "zoomOut"; // This constant must be
// 'zoomOut' for a correct operation of the tools 'Zoom Out' and 'Zoom Out Map
// Centered'
// private final String ZOOM_IN_VIEW_CENTERED_TOOL = "ZOOM_IN_VIEW_CENTERED";
// private final String ZOOM_OUT_VIEW_CENTERED_TOOL = "ZOOM_OUT_VIEW_CENTERED";
// private final String MEASURE_DISTANCES_TOOL = "MEASURE_DISTANCES";
...
// // Adds JComboBox to select a part of the graphical interface
// // this.add(getJComboBoxToolSelection(), null);
// // this.getJComboBoxToolSelection().setSelectedIndex(0); // By default select
// first element
//
// // Adds JPanel with the view
// // this.add(getSelectableMapAreaPanel(), null);
...
// "select_a_tool"));
// jComboBoxToolSelection.setEditable(false);
// jComboBoxToolSelection.addItem(new ItemOperation(PluginServices.getText(this,
// "disabled"), this.DISABLED_OPERATION));
// jComboBoxToolSelection.addItem(new ItemOperation(PluginServices.getText(this,
// "define_absolute_coordinates"), this.SELECTION_BY_COORDINATES_OPERATION));
// jComboBoxToolSelection.addItem(new ItemOperation(PluginServices.getText(this,
// "define_coordinates_using_view"), this.SELECTION_BY_AREA_OPERATION));
//
// jComboBoxToolSelection.addActionListener(new ActionListener() {
// /*
// * (non-Javadoc)
...
// if (getSelectableMapAreaPanel().getMapContext().getLayers().getLayersCount()
// > 0) {
// disableCoorderatesRegionOperation();
// enableAreaRegionOperation();
// }
// else {
// JOptionPane.showMessageDialog(jComboBoxToolSelection,
// PluginServices.getText(null, "there_is_no_layer_loaded_in_the_active_view"),
// PluginServices.getText(null, "information"),
// JOptionPane.INFORMATION_MESSAGE);
//
// // Select: no tools
...
// this.add(getJButtonZoomComplete(), null);
// this.add(getJComboBoxZoomInAndOutViewCentered(), null);
// getJComboBoxZoomInAndOutViewCentered().revalidate(); // Force update this
// component
// this.add(getJComboBoxOtherTools(), null);
// getJComboBoxOtherTools().revalidate(); // Force update this component
//
...
// */
// public void itemStateChanged(ItemEvent e) {
// // Isn't allowed that the two JToggleButton would be selected (can't move and
// scale)
// if (jToggleButtonMove.isSelected())
// jToggleButtonScaling.setSelected(false);
// }
...
// jComboBoxZoomInAndOut.addImageIconItem(new
// ImageIconItemInfo("images/ZoomIn.png",PluginServices.getIconTheme().get("view-zoom-in"),PluginServices.getText(this,
// "Zoom_Mas"), ZOOM_IN_TOOL));
// jComboBoxZoomInAndOut.addImageIconItem(new
// ImageIconItemInfo("images/ZoomOut.png",PluginServices.getIconTheme().get("view-zoom-out"),
// PluginServices.getText(this, "Zoom_Menos"), ZOOM_OUT_TOOL));
// jComboBoxZoomInAndOut.addActionListener(new ActionListener() {
// /*
// * (non-Javadoc)
// * @see
...
// jComboBoxZoomInAndOutViewCentered.addImageIconItem(new
// ImageIconItemInfo("images/zoommas.png",PluginServices.getIconTheme().get("view-zoom-mas"),
// PluginServices.getText(this, "Zoom_Acercar"), ZOOM_IN_VIEW_CENTERED_TOOL));
// jComboBoxZoomInAndOutViewCentered.addImageIconItem(new
// ImageIconItemInfo("images/ZoomOut.png",PluginServices.getIconTheme().get("view-zoom-menos"),
// PluginServices.getText(this, "Zoom_Alejar"), ZOOM_OUT_VIEW_CENTERED_TOOL));
...
// jComboBoxOtherTools.addImageIconItem(new
// ImageIconItemInfo("images/Distancia.png",PluginServices.getIconTheme().get("view-query-distance"),PluginServices.getText(this,
// "medir_distancias"), MEASURE_DISTANCES_TOOL));
// jComboBoxOtherTools.addImageIconItem(new
// ImageIconItemInfo("images/Poligono16.png",PluginServices.getIconTheme().get("view-query-area"),
// PluginServices.getText(this, "medir_area"), MEASURE_AREA_TOOL));
// jComboBoxOtherTools.addActionListener(new ActionListener() {
// /*
// * (non-Javadoc)
// * @see
...
//
// /**
// * Gets the extent: a 'Rectangle2D.Double' object reference or 'null' if there
// is no data or if the data is invalid or user hasn't defined an area
// *
// * @return java.awt.geom.Rectangle2D or null if the data is invalid
// */
// public Rectangle2D getExtent() {
...
//
// /**
// * If there is some coordinate text field that isn't undefined, returns true;
// else returns false
// *
// * @return A boolean value
// */
...
// this.getSelectableMapAreaPanel().disableAllMouseListeners();
//
// // To prevent that events that take place (are produced) could be a nuisance
// to the load of the layers
// this.getSelectableMapAreaPanel().getMapContext().beginAtomicEvent();
//
// try {
...
//
// if (r2D == null) {
// // End To prevent that events that take place(are produced) could be a
// nuisance to the load of the layers
// this.getSelectableMapAreaPanel().getMapContext().endAtomicEvent();
//
// // No tools enabled
...
// // If has to enable all listeners of all tools on the view area
// if
// (((ItemOperation)this.getJComboBoxToolSelection().getSelectedItem()).getOperation()
// == SELECTION_BY_AREA_OPERATION)
// this.getSelectableMapAreaPanel().enableAllMouseListeners();
//
// // End To prevent that events that take place(are produced) could be a
// nuisance to the load of the layers
...
//
// /**
// * This class is a panel width four fields for indicate the coordinates of two
// points:
// * One for the ritgh-up point and another for the left-down point of a
// rectangle area
// *
// * @author Pablo Piqueras Bartolomï¿½ (p_queras_pab@hotmail.com)
// * @author Jorge Piera Llodrï¿½ (piera_jor@gva.es)
// */
// private class AreaCoordinatesPanel extends JPanel {
// private final Rectangle DEFAULT_JBUTTON_GO_TO_AREA_RECTANGLE = new
// Rectangle(412, 5, 62, 51); // @jve:decl-index=0:
// private final int DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT = 20;
// private final int DEFAULT_COORDIANTE_LABEL_WIDTH = 20;
// private final int DEFAULT_TEXT_FIELDS_WIDTH = 132;
// private final int DEFAULT_X_LOCATION = 8;
...
// DEFAULT_TEXT_FIELDS_WIDTH + 10;
// private final int DEFAULT_COORDINATE_HEIGHT = 25;
// private final Dimension DEFAULT_JLABEL_VERTEX_DIMENSION = new Dimension(60,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT); // @jve:decl-index=0:
// private final int DEFAULT_XY_COORDINATES_WIDTH = 2 * DEFAULT_COORDINATE_WIDTH
// + DEFAULT_JLABEL_VERTEX_DIMENSION.width + 10 ;
//
// private final String JTEXTFIELD_V1X_NAME = "V1X";
// private final String JTEXTFIELD_V1Y_NAME = "V1Y";
...
// if (jPanelVertex1 == null) {
// jPanelVertex1 = new JPanel();
// jPanelVertex1.setBounds(DEFAULT_X_LOCATION,
// DEFAULT_Y_LEFT_UP_CORNER_LOCATION, DEFAULT_XY_COORDINATES_WIDTH,
// DEFAULT_COORDINATE_HEIGHT);
// jPanelVertex1.setLayout(new GridBagLayout());
// jPanelVertex1.add(getJLabelVertex1(), null);
// jPanelVertex1.add(getJPanelVertex1X(), null);
...
// if (jPanelVertex2 == null) {
// jPanelVertex2 = new JPanel();
// jPanelVertex2.setBounds(DEFAULT_X_LOCATION, DEFAULT_Y_LEFT_UP_CORNER_LOCATION
// + DEFAULT_COORDINATE_HEIGHT, DEFAULT_XY_COORDINATES_WIDTH,
// DEFAULT_COORDINATE_HEIGHT);
// jPanelVertex2.setLayout(new GridBagLayout());
// jPanelVertex2.add(getJLabelVertex2(), null);
// jPanelVertex2.add(getJPanelVertex2X(), null);
...
// if (jPanelVertex1X == null) {
// jPanelVertex1X = new JPanel();
// jPanelVertex1X.setPreferredSize(new Dimension(new
// Dimension(DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
// jPanelVertex1X.setLayout(new GridBagLayout());
// jPanelVertex1X.add(getJLabelVertex1X(), null);
// jPanelVertex1X.add(getJTextFieldVertex1X(), null);
// }
...
// if (jPanelVertex1Y == null) {
// jPanelVertex1Y = new JPanel();
// jPanelVertex1Y.setPreferredSize(new Dimension(new
// Dimension(DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
// jPanelVertex1Y.setLayout(new GridBagLayout());
// jPanelVertex1Y.add(getJLabelVertex1Y(), null);
// jPanelVertex1Y.add(getJTextFieldVertex1Y(), null);
...
// if (jPanelVertex2X == null) {
// jPanelVertex2X = new JPanel();
// jPanelVertex2X.setPreferredSize(new Dimension(new
// Dimension(DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
// jPanelVertex2X.setLayout(new GridBagLayout());
// jPanelVertex2X.add(getJLabelVertex2X(), null);
// jPanelVertex2X.add(getJTextFieldVertex2X(), null);
...
// if (jPanelVertex2Y == null) {
// jPanelVertex2Y = new JPanel();
// jPanelVertex2Y.setPreferredSize(new Dimension(new
// Dimension(DEFAULT_COORDINATE_WIDTH, DEFAULT_COORDINATE_HEIGHT)));
// jPanelVertex2Y.setLayout(new GridBagLayout());
// jPanelVertex2Y.add(getJLabelVertex2Y(), null);
// jPanelVertex2Y.add(getJTextFieldVertex2Y(), null);
...
// if (jLabelVertex1X == null) {
// jLabelVertex1X = new JLabel();
// jLabelVertex1X.setPreferredSize(new
// java.awt.Dimension(DEFAULT_COORDIANTE_LABEL_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jLabelVertex1X.setToolTipText(PluginServices.getText(this,
// "x_coordinate_from_area_right_up_vertex"));
// jLabelVertex1X.setText(PluginServices.getText(this, "x") + ":");
...
// if (jLabelVertex2X == null) {
// jLabelVertex2X = new JLabel();
// jLabelVertex2X.setPreferredSize(new
// java.awt.Dimension(DEFAULT_COORDIANTE_LABEL_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jLabelVertex2X.setToolTipText(PluginServices.getText(this,
// "x_coordinate_from_area_left_bottom_vertex"));
...
// if (jLabelVertex1Y == null) {
// jLabelVertex1Y = new JLabel();
// jLabelVertex1Y.setPreferredSize(new
// java.awt.Dimension(DEFAULT_COORDIANTE_LABEL_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jLabelVertex1Y.setToolTipText(PluginServices.getText(this,
// "y_coordinate_from_area_right_up_vertex"));
...
// if (jLabelVertex2Y == null) {
// jLabelVertex2Y = new JLabel();
// jLabelVertex2Y.setPreferredSize(new
// java.awt.Dimension(DEFAULT_COORDIANTE_LABEL_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jLabelVertex2Y.setToolTipText(PluginServices.getText(this,
// "y_coordinate_from_area_left_bottom_vertex"));
...
//
// /**
// * Returns a focus listener for validate the text of a JTextField when that
// component loses its focus
// *
// * @return java.awt.event.FocusListener
// */
// private FocusListener getFocusListenerForCoordinateValidation() {
...
// JTextField jTF = (JTextField)e.getSource();
//
// // If a coordinate has lost focus, is valid and has changed -> store the new
// value and set its related 'hasChanged' flag to true
// String text = jTF.getText();
//
// if (jTF.getName().compareTo(JTEXTFIELD_V1X_NAME) == 0) {
//
...
// jTextFieldVertex1X.setPreferredSize(new Dimension(DEFAULT_TEXT_FIELDS_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jTextFieldVertex1X.setToolTipText(PluginServices.getText(this,
// "x_coordinate_from_area_right_up_vertex"));
...
// jTextFieldVertex1Y.setPreferredSize(new Dimension(DEFAULT_TEXT_FIELDS_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jTextFieldVertex1Y.setToolTipText(PluginServices.getText(this,
// "y_coordinate_from_area_right_up_vertex"));
...
// jTextFieldVertex2X.setPreferredSize(new Dimension(DEFAULT_TEXT_FIELDS_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jTextFieldVertex2X.setToolTipText(PluginServices.getText(this,
// "x_coordinate_from_area_left_bottom_vertex"));
...
// jTextFieldVertex2Y.setPreferredSize(new Dimension(DEFAULT_TEXT_FIELDS_WIDTH,
// DEFAULT_LABELS_AND_TEXT_FIELDS_HEIGHT));
// jTextFieldVertex2Y.setToolTipText(PluginServices.getText(this,
// "y_coordinate_from_area_left_bottom_vertex"));
...
//
// /**
// * Validates the coordinate of a JTextField: returns true if it's valid; else
// returns false
// *
...
// private boolean validateCoordinate(JTextField coordinate) {
// if (coordinate != null) {
// // If the format of the coordinate is incorrect, shows a message warning the
// user that problem and resets the coordinate text
// if ((coordinate.getText().compareTo("") != 0) && (!
// StringNumberUtilities.isRealNumberWithIntegerExponent(coordinate.getText())))
// {
// // JOptionPane.showMessageDialog(this, PluginServices.getText(null,
// "incorrect_coordinate_format"), PluginServices.getText(null, "error"),
// JOptionPane.ERROR_MESSAGE);
// // switch (last_Coordinate_that_Loose_the_Focus) {
// // case 1:
...
//
// if ((text.compareTo("") != 0) && (!
// StringNumberUtilities.isRealNumberWithIntegerExponent(text))) {
// JOptionPane.showMessageDialog(this, PluginServices.getText(null,
// "incorrect_coordinate_format"), PluginServices.getText(null, "error"),
// JOptionPane.ERROR_MESSAGE);
// coordinate.setText(previous_Coordinate_Value[0]); // Set last value
...
//
// // if (coordinate != null) {
// // // If the format of the coordinate is incorrect, shows a message warning
// the user that problem and resets the coordinate text
// // switch (coordinate_ID) {
// // case 1:
// // coordinate.setText(previous_Coordinate_Value[0]); // Set last value
// // break;
...
// // */
// // public void mouseClicked(MouseEvent e) {
// // // Only enable the area operation if there is data loaded in this
// MapControl
// //
// // // Do nothing if there is no layer in the map control
// // if
...
// // }
// // else {
// // // If there has been an error -> can't move different distances in X the
// two vertexes
// // if (sx != sy) {
// // JOptionPane.showMessageDialog(jButtonPreviewArea,
// PluginServices.getText(jButtonPreviewArea,
// "problem_with_the_new_calculated_area") + ": " +
// PluginServices.getText(jButtonPreviewArea, "different_scale_factors"),
// PluginServices.getText(jButtonPreviewArea, "error"),
// JOptionPane.ERROR_MESSAGE);
// // restoreAllModifiedCoordinates(); // restores the coordinates
// // return;
...
// // if (d1 != d2) {
// // JOptionPane.showMessageDialog(jButtonPreviewArea,
// PluginServices.getText(jButtonPreviewArea, "different_distances_in_X"),
// PluginServices.getText(jButtonPreviewArea, "error"),
// JOptionPane.ERROR_MESSAGE);
// // restoreAllModifiedCoordinates(); // restores the coordinates
// // return;
...
// // }
// // else {
// // // If there has been an error -> can't move different distances in Y the
// two vertexes
// // if (d1 != d2) {
// // JOptionPane.showMessageDialog(jButtonPreviewArea,
// PluginServices.getText(jButtonPreviewArea, "different_distances_in_Y"),
// PluginServices.getText(jButtonPreviewArea, "error"),
// JOptionPane.ERROR_MESSAGE);
// // restoreAllModifiedCoordinates(); // restores the coordinates
// // return;
...
// private Rectangle2D getNewRectangleByCoordinates() {
// return new Rectangle2D.Double(getDoubleValueOfJTextFieldV2X(),
// getDoubleValueOfJTextFieldV2Y(), getDoubleValueOfJTextFieldV1X() -
// getDoubleValueOfJTextFieldV2X(), getDoubleValueOfJTextFieldV1Y() -
// getDoubleValueOfJTextFieldV2Y());
// }
//
// /**
// * Restores the value of all text fields with coordinates modified to their
// last value
// */
// private void restoreAllModifiedCoordinates() {
// if (hasChanged_v1X) {
...
//
// /**
// * Validates that Vertex1 is in the right-up corner and the Vertex2 in the
// left-down corner of the area/rectangle <br>
// * (It's supposed that coordinates of the vertexes are valid) <br>
...
// if (getJTextFieldVertex1X().getText().compareTo("") == 0) {
// return new ValidationInfo(false, PluginServices.getText(this,
// "undefined_coordinate") + ": " + PluginServices.getText(this, "v1x"));
// }
//
// if (getJTextFieldVertex2X().getText().compareTo("") == 0) {
// return new ValidationInfo(false, PluginServices.getText(this,
// "undefined_coordinate") + ": " + PluginServices.getText(this, "v2x"));
// }
//
// if (getJTextFieldVertex1Y().getText().compareTo("") == 0) {
// return new ValidationInfo(false, PluginServices.getText(this,
// "undefined_coordinate") + ": " + PluginServices.getText(this, "v1y"));
// }
//
// if (getJTextFieldVertex2Y().getText().compareTo("") == 0) {
// return new ValidationInfo(false, PluginServices.getText(this,
// "undefined_coordinate") + ": " + PluginServices.getText(this, "v2y"));
// }
//
// // Check if the vertex V1 is in the right-up corner and the vertex V2 is in
...
// PluginServices.getText(this, "invalid_coordinates") + ": " +
// v1.getMessage()), PluginServices.getText(null, "warning"),
// JOptionPane.WARNING_MESSAGE);
//
// return false;
// }
...
// JOptionPane.showMessageDialog(null, PluginServices.getText(null,
// PluginServices.getText(this, "invalid_coordinates") + ": " +
// v2.getMessage()), PluginServices.getText(null, "warning"),
// JOptionPane.WARNING_MESSAGE);
//
// return false;
// }
...
//
// /**
// * Gets the extent (rectangle) represented by the values of the coordinate
// text fields
// *
// * @return java.awt.geom.Rectangle2D
// */
...
//
// /**
// * Returns true if there is some coordinate text field with data; else returns
// false
// *
// * @return A boolean value
// */
...
// return ((getJTextFieldVertex1X().getText().compareTo("") != 0) |
// (getJTextFieldVertex1Y().getText().compareTo("") != 0) |
// (getJTextFieldVertex2X().getText().compareTo("") != 0) |
// (getJTextFieldVertex2Y().getText().compareTo("") != 0));
// }
//
// /**
...
// try {
// MapContext mp = ((BaseView)
// PluginServices.getMDIManager().getActiveWindow()).getMapControl().getMapContext();
//
// this.setMapContext(mp.cloneFMap());
// ViewPort vP = this.getViewPort();
//
...
// StatusBarListener sbl = new StatusBarListener(this);
//
// // ZOOM OUT (Presses on the map and it will be centered showing a bigger
// area)
// // Only pressing, not selecting a rectangle area
// ZoomOutListener zol = new ZoomOutListener(this);
// this.addMapTool(ZOOM_OUT_TOOL, new Behavior[]{new PointBehavior(zol), new
...
// // PAN
// PanListener pl = new PanListener(this);
// this.addMapTool(PAN_TOOL, new Behavior[]{new MoveBehavior(pl), new
// MouseMovementBehavior(sbl)});
//
// // MEASURE DISTANCES
// MeasureListener mli = new MeasureListener(this);
// this.addMapTool(MEASURE_DISTANCES_TOOL, new Behavior[]{new
// PolylineBehavior(mli), new MouseMovementBehavior(sbl)});
//
// // MEASURE AREA
// AreaListener ali = new AreaListener(this);
// this.addMapTool(MEASURE_AREA_TOOL, new Behavior[]{new PolygonBehavior(ali),
// new MouseMovementBehavior(sbl)});
//
// this.getViewPort().addViewPortListener(new ViewPortListener() {
// /*
// * (non-Javadoc)
// * @see
// com.iver.cit.gvsig.fmap.ViewPortListener#extentChanged(com.iver.cit.gvsig.fmap.ExtentEvent)
// */
// public void extentChanged(ExtentEvent e) {
...
// * (non-Javadoc)
// * @see
// com.iver.cit.gvsig.fmap.ViewPortListener#projectionChanged(com.iver.cit.gvsig.fmap.ProjectionEvent)
// */
// public void projectionChanged(ProjectionEvent e) {
// }
//
...
// int i;
//
// // Only disable listeners if there are listeners to remove (it's supposed
// that there are always the same number of mouse listeners, and this listeners
// // are referenciated by this component or by the private attribute
// 'mouseListeners')
//
// // Mouse Button Listeners
...
// * @param message A message associate to the validation (If has been valid,
// this attribute should be "", in the other case should have a message with
...
//
// VERSIÓN PREVIA -> ESTA ES LA VERSIÓN BUENA, PERO HAY QUE EVITAR QUE CLONE
// MAPCONTROL
//
//
// /**
...
// }
// else {
// // If there has been an error -> can't move different distances in X the two
// vertexes
// if (sx != sy) {
// JOptionPane.showMessageDialog(jButtonPreviewArea,
// PluginServices.getText(jButtonPreviewArea,
...
// }
// else {
// // If there has been an error -> can't move different distances in Y the two
// vertexes
// if (d1 != d2) {
// JOptionPane.showMessageDialog(jButtonPreviewArea,
// PluginServices.getText(jButtonPreviewArea, "different_distances_in_Y"),
