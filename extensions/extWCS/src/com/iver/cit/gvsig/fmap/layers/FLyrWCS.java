 ...
 * Capa para el WCS.
 * 
 * Las capas WCS son tileadas para descargarlas del servidor. Esto quiere decir
 * que están formadas por multiples ficheros raster. Por esto la fuente de datos
 * raster (IRasterDatasource) de la capa FLyrWCS es un objeto de tipo
 ...
 * repinta ya que en WCS a cada zoom varian los ficheros fuente. La secuencia de
...
public class FLyrWCS extends FLyrRasterSE {
	...
	public FLyrWCS(Map args) throws DriverIOException {
		...
		try {
			...
			// e.printStackTrace();
			throw new DriverIOException("Malformed host URL, '" + host + "' ("
					+ e.toString() + ").");
		}
		try {
			drv = this.getDriver();
			...
			// e.printStackTrace();
			throw new DriverIOException("Can't get driver to host '" + host
					+ "' (" + e.toString() + ").");
		}
		...
		if (wcsNode == null) {
			throw new DriverIOException("The server '" + host
					+ "' doesn't has the coverage '" + sCoverage + "'.");
		}
		...
		try {
			...
			throw new DriverIOException("The server '" + host
					+ "' is not able to load the coverage '" + sCoverage + "'.");
		}

	}

	/**
	 * Clase que contiene los datos de visualización de WCS. Tiene datos que
	 * representan al raster en la vista. Este raster puede estar compuesto por
	 * tiles por lo que valores como el ancho total o el mínimo o máximo deben
	 * ser calculados a partir de todos los tiles visualizados.
	 * 
	...
	private class VisualStatus {
		...
		private double rasterMinX = Double.MAX_VALUE,
				rasterMinY = Double.MAX_VALUE;
	...
	public void draw(BufferedImage image, Graphics2D g, ViewPort viewPort,
		...
		if (isWithinScale(scale)) {
			...
			try {
				if (true) {
					if (viewPort.getImageWidth() <= maxTileDrawWidth
							&& viewPort.getImageHeight() <= maxTileDrawHeight) {
						...
						Rectangle r = new Rectangle((int) p.getX(),
								(int) p.getY(), viewPort.getImageWidth(),
						...
						for (int tileNr = 0; tileNr < tiles.getNumTiles(); tileNr++) {
							...
							try {
								ViewPort vp = tiles.getTileViewPort(viewPort,
										tileNr);
								boolean painted = drawTile(g, vp, cancel,
										tileNr, scale, tileNr);
								if (layerRaster != null
	...
	private void initializeRasterLayer(MultiRasterDataset[][] datasets,
		...
		if (transparency == null)
			transparency = new GridTransparency(getDataSource()
	...
	private boolean drawTile(Graphics2D g, ViewPort vp, Cancellable cancel,
		...
		try {
			...
			wcsStatus.setOnlineResource((String) onlineResources
					.get("GetCoverage"));

			File f = getDriver().getCoverage(wcsStatus,
					new MyCancellable(cancel));
			if (f == null)
				return false;
	...
	private void rasterProcess(String filePath, Graphics2D g, ViewPort vp,
		...
		if (visualStatus.dataType == IBuffer.TYPE_UNDEFINED
				&& layerRaster[nLyr].getDataType() != null)
			visualStatus.dataType = layerRaster[nLyr].getDataType()[0];
		if (visualStatus.bandCount == 0
				&& layerRaster[nLyr].getBandCount() != 0)
		...
		if (filterArguments != null) {
			...
			StatusLayerRaster.enhancedCompV10(filterArguments,
					layerRaster[nLyr], filterListManager);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.raster.layers.FLyrRasterSE#print(java.awt.Graphics2D,
	...
	/**
	 * Returns the XMLEntity containing the necessary info for reproduce the
	 * layer.
	 * 
	...
	public void setXMLEntity(XMLEntity xml) throws XMLException {
		...
		if (xml.contains("wcs.maxRes"))
			maxRes = new Point2D.Double(xml.getDoubleProperty("wcs.maxRes"),
					xml.getDoubleProperty("wcs.maxRes"));
		else if (xml.contains("wcs.maxResX") && xml.contains("wcs.maxResY"))
			maxRes = new Point2D.Double(xml.getDoubleProperty("wcs.maxResX"),
					xml.getDoubleProperty("wcs.maxResY"));
	...
	/**
	 * Remote source layers have a bunch of properties that are required for get
	 * them from the servers. This method supplies a hash table containing any
	 * needed field. This hash table may be used to let the client to connect to
	 * a server and restore a previously saved layer. So, the layer itself may
	 * not be saved to the disk since the actual saved info is just its
	 * properties.
	 * 
	 ...
	 * 
	 * @return String con la extensiï¿½n del fichero de georreferenciaciï¿½n
	...
	private String getExtensionWorldFile() {
		...
		/*
		 * if (format.equals("image/jpeg")) extWorldFile = ".jpgw";
		 */
		return extWorldFile;
	}
	...
	/**
	 * <p>
	 * Gets the max resolution allowed by the coverage. Requesting a higher
	 * resolution than this value does not cause any error, but the info
	 * responsed is just an interpolation. <br>
	 * </p>
	 * 
	 * <p>
	 * In exchange for obtaining a greater file and without additional
	 * information, we can easily fit it into the View. <br>
	 * </p>
	 * 
	 * <p>
	 * Obtiene la resolución máxima soportada por la cobertura. La petición de
	 * una resolución superior a la soportada no provoca ningún error, aunque la
	 * información obtenida sólo es una mera interpolación de información. <br>
	 * </p>
	 * 
	 * <p>
	...
	public FLayer getFileLayer() {
		if (layerRaster != null && layerRaster[0] != null) {
			...
			if (lastNColumns == 0 && lastNRows == 0) { // Una capa sin tilear =>
														// MultirasterDataset
				try {
