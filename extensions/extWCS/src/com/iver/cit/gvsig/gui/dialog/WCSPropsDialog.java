...
public class WCSPropsDialog extends JPanel implements IWindow {
	...
	/**
	 * With getParamsPanel we have access to the coverage config TabbedPane. If
	 * this panel doesn't exist yet (which can occur when an existing project is
	 ...
	 * 
	 * 
	 * Con getParamsPanel tenemos acceso a juego de pestañas de configuración de
	 * la cobertura. Si este panel todavía no existe (como puede ser cuando
	 ...
	 * un retardo en la aparición en el toc o un error de nullPointer si no hay
	...
	public WCSParamsPanel getParamsPanel(Hashtable info) {
		try {
			...
			JOptionPane.showMessageDialog(null, "servidor_wcs_no_responde",
	...
	private class ComandosListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getActionCommand() == "CANCEL") {
				...
				((FLyrWCS) fLayer).setCoverageName(wcsParamsTabbedPane
						.getCurrentCoverageName());
				((FLyrWCS) fLayer).setName(wcsParamsTabbedPane
						.getCoverageName());
				((FLyrWCS) fLayer).setSRS(wcsParamsTabbedPane.getSRS());
				((FLyrWCS) fLayer).setFormat(wcsParamsTabbedPane.getFormat());
				((FLyrWCS) fLayer).setFullExtent(wcsParamsTabbedPane
						.getExtent());
				((FLyrWCS) fLayer).setTime(wcsParamsTabbedPane.getTime());
				((FLyrWCS) fLayer).setParameter(wcsParamsTabbedPane
						.getParameterString());

				if (e.getActionCommand() == "APPLY") {
					com.iver.cit.gvsig.project.documents.view.gui.View vista = (com.iver.cit.gvsig.project.documents.view.gui.View) PluginServices
