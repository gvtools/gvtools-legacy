...
public class WCSZoomToPixelTocMenuEntry extends TocMenuEntry {
	...
	public void actionPerformed(ActionEvent e) {
		...
		double wcOriginCenterX = v.getExtent().getMinX()
				+ ((v.getExtent().getMaxX() - v.getExtent().getMinX()) / 2D);
		double wcOriginCenterY = v.getExtent().getMinY()
				+ ((v.getExtent().getMaxY() - v.getExtent().getMinY()) / 2D);
		...
		getMapContext().getViewPort().setExtent(
				new Rectangle2D.Double(wcDstMinX, wcDstMinY, wcDstWidth,
						wcDstHeight));
