...
public class ViewRasterRequestManager implements IExtensionRequest {
	...
	public Rectangle2D request(Rectangle2D extent)
		...
		if (view.getCanvas().getMinxMaxyUL()) {
			...
			ext = new Extent(extent.getMinX(), extent.getMinY(),
					extent.getMaxX(), extent.getMaxY());
			extSelection = RasterUtilities.calculateAdjustedView(ext,
					lyr.getFullRasterExtent());
		}

		// Obtenemos el viewport y calculamos la matriz de transformación
		ViewPortData vp = new ViewPortData(null, ext, new Dimension(
				view.getCanvasWidth(), view.getCanvasHeight()));
		vp.calculateAffineTransform();

		// Calculamos el punto del canvas de la vista donde se empieza a dibujar
		// el buffer de la imagen
		Point2D pt = new Point2D.Double(extSelection.getULX(),
				extSelection.getULY());
		...
		try {
			...
			BufferedImage drawedImg = (BufferedImage) lyr.getRender().draw(img,
					vp);
			img.drawImage(drawedImg, (int) Math.round(pt.getX()),
					(int) Math.round(pt.getY()), null);
			...
			throw new InvalidRequestException(
					"Error asignando el área de la petición");
