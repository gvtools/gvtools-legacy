 ...
 * pixel de un punto de control. Un punto de control tiene coordenada pixel de
 * imagen de origen y coordenada de destino en coordenadas geográficas. Un
 * GPGraphic tiene asociado un GeoPoint que es el que tiene la información de
 * georreferencia.
 * 
 * 22/12/2007
 * 
...
public class GPGraphic {
	 ...
	 * 
	 * @param type
	 *            El valor de "type" viene definido por las constantes en esta
	 *            clase
	 */
	...
	/**
	 * Dibuja sobre el graphics pasado la cruz del punto que marca el pixel de
	 * la imagen.
	 * 
	 * @param g
	 ...
	 *            Graphics sobre el que se pinta
	 * @param p
	 *            Punto del graphics que coincide con el centro del GP
	 * @param pointNumber
	 *            Numero de punto a mostrar
	 * @param label
	 *            Etiqueta del punto
	 */
	...
	private void drawPixelCrux(Graphics2D g, Point2D p, int pointNumber,
		...
		g.drawOval((int) p.getX() - dpto + 1, (int) p.getY() - dpto + 1,
				DIAM_CIRCLE - 2, DIAM_CIRCLE - 2);
		g.drawLine((int) p.getX() - incr, (int) p.getY() - 1,
		...
		g.drawLine((int) p.getX() - 1, (int) p.getY() - incr,
				(int) p.getX() - 1, (int) p.getY() - 1);
		g.drawLine((int) p.getX() + 1, (int) p.getY() - incr,
				(int) p.getX() + 1, (int) p.getY() - 1);
		...
		g.drawOval((int) p.getX() - dpto, (int) p.getY() - dpto, DIAM_CIRCLE,
				DIAM_CIRCLE);
		g.drawLine((int) p.getX(), (int) p.getY() - dpto - incr,
				(int) p.getX(), (int) p.getY() + dpto + incr);
		g.drawLine((int) p.getX() - dpto - incr, (int) p.getY(), (int) p.getX()
				+ dpto + incr, (int) p.getY());
	...
	private void drawMapCrux(Graphics2D g, Point2D p, int pointNumber,
		...
		int incr = 5;
		g.setColor(Color.WHITE);
		g.drawRect((int) p.getX() - dpto + 1, (int) p.getY() - dpto + 1,
				DIAM_CIRCLE - 2, DIAM_CIRCLE - 2);
		g.drawLine((int) p.getX() - incr, (int) p.getY() - 1,
				(int) p.getX() - 1, (int) p.getY() - 1);
		g.drawLine((int) p.getX() - incr, (int) p.getY() + 1,
		...
		g.drawRect((int) p.getX() - dpto, (int) p.getY() - dpto, DIAM_CIRCLE,
				DIAM_CIRCLE);
		g.drawLine((int) p.getX(), (int) p.getY() - dpto - incr,
				(int) p.getX(), (int) p.getY() + dpto + incr);
		g.drawLine((int) p.getX() - dpto - incr, (int) p.getY(), (int) p.getX()
		...
		if (showNumber) {
			...
			for (int i = -1; i < 2; i++)
				for (int j = -1; j < 2; j++)
					g.drawString(pt, ptX + i, ptY + j);
			g.setColor(new Color(45, 8, 165));
			g.drawString(String.valueOf(pointNumber),
					(int) (p.getX() + dpto + 1), (int) (p.getY() - dpto - 1));
		}
		if (showLabel) {
			// TODO: FUNCIONALIDAD mostrar etiqueta
		}
	 ...
	 * 
	 * @param type
	 *            Tipo de punto definido en las constantes de clase
	 */
	...
	/**
	 * Consulta el flag que informa sobre si se está pintando el punto de
	 * control o está oculto
	 * 
	...
	/**
	 * Asigna el flag que informa sobre si se está pintando el punto de control
	 * o está oculto
	 * 
	 * @param draw
	 *            true para pintar el punto de control y false para ocultarlo
	 */
