...
/**
 * Capa gráfica que se dibuja sobre una vista de zoom un cursor rectangular que
 * representa una ventanta de zoom sobre la vista 22/12/2007
 * 
...
public class ZoomCursorGraphicLayer implements IGraphicLayer {
	 ...
	 * 
	 * @param pX
	 *            Posición en X del cursor en la vista
	 * @param pY
	 *            Posición en Y del cursor en la vista
	 * @param w
	 *            Ancho del cursor en la vista
	 * @param h
	 *            Alto del cursor en la vista
	 * @param listener
	 *            Listener para acciones de finalización de la operación de zoom
	 */
	public ZoomCursorGraphicLayer(int pX, int pY, int w, int h,
		...
		try {
			iconHoriz = new ImageIcon(getClass().getClassLoader().getResource(
					"images/FlechaHorizCursor.gif")).getImage();
			iconVert = new ImageIcon(getClass().getClassLoader().getResource(
					"images/FlechaVertCursor.gif")).getImage();
			iconMove = new ImageIcon(getClass().getClassLoader().getResource(
					"images/FlechaMoveCursor.gif")).getImage();
	 ...
	 * devueltas en referencia a la vista.
	 * 
	 * @return
	 */
	public Rectangle2D getCursorViewCoordinates() {
		return new Rectangle2D.Double(posX - (wCursor >> 1), posY
				- (hCursor >> 1), wCursor, hCursor);
	}
	...
	public void draw(Graphics2D g, Rectangle2D ext, int w, int h) {
		...
		g.drawRect(posX - (wCursor >> 1), posY - (hCursor >> 1), wCursor,
				hCursor);
	...
	public void mouseMoved(MouseEvent e) {
		...
		// Si estamos fuera del área del cuadrado + 2 píxeles ponemos el cursor
		// por defecto y no hacemos nada
		if (e.getX() < (pxLeft - 2) || e.getX() > (pxRight + 2)
				|| e.getY() < (pyUp - 2) || e.getY() > (pyDown + 2)) {
			setOperation(NONE);
		...
		if (e.getX() >= (pxRight - 2) && e.getY() <= (pyUp + 2)) {
			if (iconMove != null)
				canvas.setCursor(Toolkit.getDefaultToolkit()
						.createCustomCursor(iconMove, new Point(16, 16), ""));
		...
		if (e.getX() <= (pxLeft + 1)) {
			if (iconHoriz != null)
				canvas.setCursor(Toolkit.getDefaultToolkit()
						.createCustomCursor(iconHoriz, new Point(16, 16), ""));
		...
		if (e.getY() <= (pyUp + 1)) {
			if (iconVert != null)
				canvas.setCursor(Toolkit.getDefaultToolkit()
						.createCustomCursor(iconVert, new Point(16, 16), ""));
	...
	/**
	 * Asigna el flag que activa y desactiva la interactuación con capa de
	 * control de zoom
	 * 
	 * @param activeEvent
	 */
