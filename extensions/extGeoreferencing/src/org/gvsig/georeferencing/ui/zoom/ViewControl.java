...
/**
 * <P>
 * Control de zoom. Muestra una imagen contenida en un buffer con controles de
 * aumento y disminución del zoom de la misma.
 * </P>
 * <P>
 * El visualizador mostrará el buffer de datos pasado en
 * <code>setDrawParams</code>. En esta llamada asignamos también el Extent del
 * área de datos, tamaño de pixel y punto del buffer que coincidirá con el
 * centro del control de zoom. De esta forma la imagen visualizada se centra
 * sobre este punto. En caso de no tener información geográfica del buffer de
 * datos a visualizar el Extent coincidirá con su tamaño en píxeles y el tamaño
 * de pixel será de 1. El punto donde queramos centrar en este caso se dará
 * también en coordenadas pixel.
 * </P>
 * <P>
 * Hay dos usos posibles en este control al accionar el botón de zoom: el
 * primero de ellos, en caso de que no haya ningún listener del control de zoom
 * registrado. En este caso se aplicará un escalado sobre los datos del buffer
 * pasado por parámetro. En el caso de que haya un listener registrado
 * (IExtensionBuffer) no se aplicará un escalado sobre el buffer sino que se
 * aplicará una escala de 1.0 y se ejecutará el método request del interfaz
 * registrado para que el cliente reciba notificación del nuevo extent.
 * </P>
 * <P>
 * En la inicialización podemos solicitar que los controles de zoom estén a la
 * izquierda, a la derecha o no esten.
 * </p>
 * 
 * 21/12/2007
 * 
...
public class ViewControl extends JPanel implements ActionListener, ToolListener {
	 ...
	 * 
	 * @param right
	 *            true para los botones a derecha y false para la izquierda
	 */
	...
	public void selectTool(Class tool, boolean select) {
		// Si tool es null se desactivan todas y ponemos la seleccionada en el
		// canvas a null
		if (tool == null) {
			for (int i = 0; i < toolList.size(); i++)
		...
		// Si seleccionamos una esta se activa, se asigna al canvas y las otras
		// se desactivan
		if (select) {
			for (int i = 0; i < toolList.size(); i++) {
		...
		// Si quitamos una esta se desactiva y si es la del canvas se quita
		// también
		for (int i = 0; i < toolList.size(); i++) {
			if (tool.isInstance(toolList.get(i))) {
				((BaseViewTool) toolList.get(i)).setActive(false);
	 ...
	 * otra que estuviera activa. Si el argumento es null elimina cualquier
	...
	/**
	 * Registra un listener a la tool especificada por el parámetro de la
	 * llamada.
	 * 
	 * @param tool
	 *            Clase de la tool a la que queremos registrar el listener
	...
	/**
	 * Desactiva las herramientas temporalmente. Guarda el estado en el que
	 * estaban para restaurarlo cuando se invoque a awake
	 */
	 ...
	 * 
	 * @param b
	 *            Constante definida en ZoomControl
	 */
	public void hideButton(int b) {
		...
		case PREV_ZOOM:
			...
			break;
		case PAN:
			this.getBMove().setVisible(false);
			break;
		case LOCATOR:
	...
	public JButton getBCenterLocator() {
		if (bCenterLocator == null) {
			...
			bCenterLocator.setToolTipText(RasterToolsUtil.getText(this,
					"center_locator"));
			bCenterLocator.setPreferredSize(new java.awt.Dimension(25, 25));
			try {
				bCenterLocator.setIcon(RasterToolsUtil
						.getIcon("centerlocator-icon"));
			} catch (NullPointerException e) {
				// Sin icono
			}
	...
	public JButton getBZoomMenos() {
		if (bZoomMenos == null) {
			...
			bZoomMenos.setToolTipText(RasterToolsUtil.getText(this,
					"zoom_menos"));
	...
	public JToggleButton getBSelectZoomArea() {
		if (bSelectZoomArea == null) {
			...
			bSelectZoomArea.setToolTipText(RasterToolsUtil.getText(this,
					"select_zoom_area"));
			bSelectZoomArea.setPreferredSize(new java.awt.Dimension(25, 25));
			try {
				bSelectZoomArea.setIcon(RasterToolsUtil
						.getIcon("selectzoomarea-icon"));
			} catch (NullPointerException e) {
				// Sin icono
			}
	...
	public JButton getBPrevZoom() {
		if (bPrevZoom == null) {
			...
			bPrevZoom
					.setToolTipText(RasterToolsUtil.getText(this, "prev_zoom"));
	...
	/**
	 * Crea el botón de zoom completo. El zoom completo inicializa al primer
	 * zoom que recibió en caso de que no se esté usando un IExtensionRequest,
	 * ya que no tendría conocimiento del tamaño total de capa, sino solo del
	 * buffer que tiene en ese momento. En caso de usar un IExtensionRequest
	 * entonces se hará la petición correspondiente al interfaz.
	 * 
	 * @return JButton
	 */
	public JButton getBFullView() {
		if (bFullView == null) {
			...
			bFullView
					.setToolTipText(RasterToolsUtil.getText(this, "full_view"));
	 ...
	 *            del buffer
	 */
	public void setDrawParams(BufferedImage img, Rectangle2D ext,
	...
	public void setCenter(Point2D center) {
		...
		ext.setRect(ext.getX() + diffX, ext.getY() + diffY, ext.getWidth(),
				ext.getHeight());
		try {
	...
	/**
	 * Registra un objeto IExtensionRequest para que no se aplique un escalado
	 * sobre el buffer pasado por parámetro. Alternativamente a la aplicación de
	 * este escalado se ejecutará el método request del interfaz para que el
	 * cliente pueda pasar un nuevo buffer con escala 1:1 y con la extensión
	 * correspondiente al zoom, Es decir, se deja en manos del cliente la
	 * aplicación del zoom.
	 * 
	 * @param er
	 */
		...
		if (e.getSource() == bCenterLocator) {
			...
			for (int i = 0; i < graphicLayers.size(); i++) {
				if (graphicLayers.get(i) instanceof ZoomCursorGraphicLayer) {
					((ZoomCursorGraphicLayer) graphicLayers.get(i))
							.setCursorPosition(canvas.getWidth() / 2,
									canvas.getHeight() / 2);
					((ZoomCursorGraphicLayer) graphicLayers.get(i))
							.resetCursorSize();
				}
		...
		if (extReq == null) {
			...
			if (e.getSource() == bPrevZoom) {
				...
				if (request != null) {
					if (request instanceof Rectangle2D) {
						try {
							...
							RasterToolsUtil.messageBoxError(
									"error_setview_preview", this);
						}
					}
					if (request instanceof Integer
							&& ((Integer) request).intValue() == FULL_VIEW) {
						try {
							extReq.fullExtent();
						} catch (InvalidRequestException e1) {
							RasterToolsUtil.messageBoxError(
					...
					getZoomRecord().getRequest(); // El que hemos metido al
													// hacer zoom previo no
													// sirve por lo que hay que
													// sacarlo
				}
				return;
			}
	...
	/**
	 * Recarga la vista con los parámetros de tamaño y extensión actuales
	 */
	public void reload() {
	...
	/**
	 * Evento de finalización de la tool que maneja el viewControl y de la capa
	 * con el cursor gráfico que controla el área de la miniimagen asociada
	 */
	public void endAction(ToolEvent ev) {
		if (ev.getSource() instanceof ZoomRectangleTool
				|| ev.getSource() instanceof PanTool) {
			Rectangle2D extent = (Rectangle2D) canvas.getSelectedTool()
					.getResult();
			try {
	 ...
	 * 
	 * @see
	 * org.gvsig.rastertools.georeferencing.ui.zoom.tools.ToolListener#activeTool
	 * (org.gvsig.rastertools.georeferencing.ui.zoom.tools.ToolEvent)
	 */
	 ...
	 * 
	 * @see
	 * org.gvsig.rastertools.georeferencing.ui.zoom.tools.ToolListener#offTool
	 * (org.gvsig.rastertools.georeferencing.ui.zoom.tools.ToolEvent)
	 */
