...
/**
 * Zona de dibujado del raster 21/12/2007
 * 
 * @author Nacho Brodin nachobrodin@gmail.com
 */
public class CanvasZone extends JPanel implements MouseListener,
		MouseMotionListener {
	...
	/**
	 * Último extent aplicado. Si no ha variado el siguiente a aplicar no hace
	 * falta que releamos de nuevo
	 */
	private Rectangle2D lastExtent = null;
	// Ultimo Image con la capa dibujada y con la transformación que tiene en
	// cuenta los desplazamientos dentro de un pixel
	// Este buffer no varia hasta que se hace la siguiente petición de
	// setDrawParams
	private BufferedImage lastImage = null;
	// lastImage sobre el que se pintan las capas gráficas. Este buffer varia en
	// cada repaint. El orden en el que se carga
	// es: se vuelva el lastImage, se pintan las capas gráficas, se pintan las
	// tools.
	...
	/**
	 * Informa de que la esquina superior izquierda corresponde con el valor de
	 * mínimo X y máximo Y. En caso de ser false esta esquina sería de mínimo X
	 * y mínimo Y.
	 */
	...
	/**
	 * Normalmente no se hace una petición al dibujado del raster si el extent
	 * no ha variado. Si esta variable está a true fuerza que haya una petición
	 * de redibujado aunque el extent del raster no haya cambiado. Esto solo se
	 * hace para una petición. La siguiente vuelve a estar a false.
	 */
	...
	public Point2D viewCoordsToWorld(Point2D p) {
		...
		if (minxMaxyUL) // Cuando las Y decrecen de arriba a abajo
			...
			// Cuando las Y crecen de arriba a abajo
	...
	/**
	 * Asigna un nuevo centro de visualización en coordenadas del componente.
	 * 
	 * @param center
	 */
	...
	/**
	 * Asigna un nuevo centro de visualización en coordenadas pixel del área de
	 * dibujado (canvas). El nuevo centro será calculado en coordenadas del
	 * mapa.
	 * 
	 * @param center
	 */
	public void setPixelCenter(Point2D c) {
		...
		this.center = new Point2D.Double(ext.getMinX() + wWC, ext.getMinY()
				- hWC);
		repaint();
	}

	/**
	 * Asigna un nuevo centro de visualización en coordenadas pixel. Esta
	 * llamada tiene en cuenta solo píxeles completos. No centra sobre porciones
	 * de pixel cuando el zoom es mayor de 1:1. El nuevo centro es en
	 * coordenadas del mapa pero siempre centrará en la esquina inferior
	 * izquierda del pixel.
	 * 
	 * @param center
	 */
	public void setPixelCenter(int x, int y) {
		...
		Point2D center = new Point2D.Double(ext.getMinX() + wWC, ext.getMinY()
				- hWC);
		...
		// Después de haber convertido a pixel y redondeado la coordenada a
		// entero volvemos a convertirla en real
	...
	/**
	 * Obtiene el buffer de la vista activa y lo dibuja sobre el panel con los
	 * datos de escala y desplazamiento seleccionados.
	 */
	protected void paintComponent(Graphics g) {
		...
		if (lastImage == null || !equal(lastExtent, ext)) {
			lastImage = new BufferedImage((int) w, (int) h,
					BufferedImage.TYPE_INT_RGB);
			lastImageWithLayers = new BufferedImage((int) w, (int) h,
					BufferedImage.TYPE_INT_RGB);
		}
		...
		for (int i = 0; i < graphicLayers.size(); i++)
			((IGraphicLayer) graphicLayers.get(i)).draw(
					(Graphics2D) graphicsDB, ext, w, h);
	...
	private void showInfo(Graphics g) {
		...
		g.drawString("X: " + MathUtils.clipDecimals(realCoord.getX(), 3), 12,
				18);
		g.drawString("Y: " + MathUtils.clipDecimals(realCoord.getY(), 3), 12,
				32);
		// g.drawString("X': " + MathUtils.tailDecimals(viewCoord.getX(), 3),
		// 12, 46);
		// g.drawString("Y': " + MathUtils.tailDecimals(viewCoord.getY(), 3),
		// 12, 60);
	}

	/**
	 * Compara dos extents y devuelve true si son iguales y false si son
	 * distintos
	 * 
	 * @param e1
	 *            Extent 1
	...
	private boolean equal(Rectangle2D e1, Rectangle2D e2) {
		return (e1 != null && e2 != null && e1.getMinX() == e2.getMinX()
				&& e1.getMinY() == e2.getMinY() && e1.getMaxX() == e2.getMaxX() && e1
					.getMaxY() == e2.getMaxY());
	}

	/**
	 * Obtiene el Extent del canvas. Este canvas tiene un ancho en pixeles de w
	 * y un alto de h. Tiene en cuenta la escala a la que se quiere dibujar para
	 * devolver el extent cuando el zoom ya está aplicado.
	 * 
	 * @param w
	 ...
	 * </P>
	 * <P>
	 * No podemos aplicar un escalado al Graphics y dibujar porque cuando el
	 * zoom es mayor a 1 los pixeles no empiezan a dibujarse siempre en la
	 * esquina superior izquierda y al Graphics solo podemos ordenarle el
	 * dibujado en coordenadas enteras. Para solucionarlo debemos copiar el
	 * trozo de buffer a dibujar teniendo en cuenta el desplazamiento de la
	 * esquina superior izquierda de un pixel.
	 * </P>
	 * 
	 * @param g
	 * @param ext
	...
	private Graphics draw(Rectangle2D ext, double w, double h) {
		if (!equal(lastExtent, ext) || forceRequest) {
			// Hallamos la coordenada pixel del buffer de la esquina superior
			// izquierda del extent
			double pxX = ((ext.getMinX() - extent.getMinX()) * (w / scale))
					/ ext.getWidth();
			double pxY = ((extent.getMinY() - ext.getMinY()) * (h / scale))
					/ ext.getHeight();

			// Creamos el buffer y lo cargamos teniendo en cuenta el
			...
			for (int i = 0; i < w; i++) {
				yValue = pxY;
				for (int j = 0; j < h; j++) {
					if ((int) xValue >= 0 && (int) yValue >= 0
							&& (int) xValue < image.getWidth()
							&& (int) yValue < image.getHeight()) {
						lastImage.setRGB(i, j,
								image.getRGB((int) xValue, (int) yValue));
						lastImageWithLayers.setRGB(i, j,
								image.getRGB((int) xValue, (int) yValue));
					} else {
			...
			((Graphics2D) lastImageWithLayers.getGraphics()).drawImage(
					lastImage, 0, 0, null);
	...
	/**
	 * Asigna el valor para el flag minxMaxyUL. Este flag informa de que la
	 * esquina superior izquierda corresponde con el valor de mínimo X y máximo
	 * Y. En caso de ser false esta esquina sería de mínimo X y mínimo Y.
	 * 
	 * @param v
	 */
	...
	/**
	 * Obtiene el valor para el flag minxMaxyUL. Este flag informa de que la
	 * esquina superior izquierda corresponde con el valor de mínimo X y máximo
	 * Y. En caso de ser false esta esquina sería de mínimo X y mínimo Y.
	 * 
	 * @param v
