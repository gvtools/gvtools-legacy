...
public class GCPTablePanel extends JPanel implements IWindow, ComponentListener {
	...
	private String[] columnNames = { "-", RasterToolsUtil.getText(this, "num"),
			RasterToolsUtil.getText(this, "worldx"),
			RasterToolsUtil.getText(this, "worldy"),
			RasterToolsUtil.getText(this, "imagex"),
			RasterToolsUtil.getText(this, "imagey"), "Error X", "Error Y",
			"RMS", "-" };
	...
	/**
	 * Constructor. Crea la composición de controles de zoom.
	 */
	...
	private void init() throws NotInitializeException {
		...
		gb.fill = GridBagConstraints.BOTH; // El componente se hace tan ancho
											// como espacio disponible tiene
	...
	public TableContainer getTable() throws NotInitializeException {
		if (table == null) {
			...
			table.getTable().getJTable().getColumnModel()
					.getColumn(columnNames.length - 1).setMinWidth(0);
			table.getTable().getJTable().getColumnModel()
					.getColumn(columnNames.length - 1).setMaxWidth(0);
			table.getTable().getJTable().getColumnModel().getColumn(0)
					.setMinWidth(26);
			table.getTable().getJTable().getColumnModel().getColumn(0)
					.setMaxWidth(26);
			table.getTable().getJTable().getColumnModel().getColumn(1)
					.setMinWidth(26);
			table.getTable().getJTable().getColumnModel().getColumn(1)
					.setMaxWidth(26);
	...
	public JButton getEndGeorefButton() {
		if (bEndGeoref == null) {
			...
			bEndGeoref.setToolTipText(RasterToolsUtil.getText(this,
					"end_georef"));
	...
	/**
	 * Obtiene el botón de terminar el test de georreferenciación. Si no existe
	 * se crea.
	 * 
	 * @return JButton
	 */
	public JButton getEndTestButton() {
		if (bEndTest == null) {
			...
			bEndTest.setToolTipText(RasterToolsUtil.getText(this,
					"end_test_georef"));
	...
	public JButton getLoadFromCSVButton() {
		if (bLoadFromAscii == null) {
			...
			bLoadFromAscii.setToolTipText(RasterToolsUtil.getText(this,
					"load_from_ascii"));
			bLoadFromAscii.addActionListener(buttonsListener);
			bLoadFromAscii.setIcon(RasterToolsUtil
					.getIcon("importfromcsv-icon"));
		}
	...
	public JButton getExporToCSVButton() {
		if (bSaveToAscii == null) {
			...
			bSaveToAscii.setToolTipText(RasterToolsUtil.getText(this,
					"save_to_ascii"));
	...
	public JButton getLoadFromXMLButton() {
		if (bLoadFromXml == null) {
			...
			bLoadFromXml.setToolTipText(RasterToolsUtil.getText(this,
					"load_from_xml"));
	...
	public JButton getSaveToXMLButton() {
		if (bSaveToXml == null) {
			...
			bSaveToXml.setToolTipText(RasterToolsUtil.getText(this,
					"save_gcp_to_xml"));
	...
	/**
	 * Obtiene el botón de asignar el punto de control seleccionado en la tabla
	 * sobre una posición de la vista. Si no existe se crea.
	 * 
	 * @return JButton
	 */
	public JToggleButton getToolSelectPointButton() {
		if (bAddPoint == null) {
			...
			bAddPoint.setToolTipText(RasterToolsUtil
					.getText(this, "move-point"));
	...
	public JButton getCenterButton() {
		if (bCenterView == null) {
			...
			bCenterView.setToolTipText(RasterToolsUtil.getText(this,
					"center_view"));
	...
	/**
	 * Cuando la tabla tiene un tamaño mayor de 800 pixeles de ancho se muestran
	 * las columnas de error en X e Y que cuando está minimizada no aparecen.
	 */
	public void componentResized(ComponentEvent e) {
		try {
			Dimension dim = ((GCPTablePanel) e.getSource()).getSize();
			if (dim.getWidth() <= 800) {
				TableColumn col = getTable().getTable().getJTable()
						.getColumnModel().getColumn(6);
				col.setMinWidth(0);
				col.setMaxWidth(0);
				col = getTable().getTable().getJTable().getColumnModel()
						.getColumn(7);
				col.setMinWidth(0);
				col.setMaxWidth(0);
			} else {
	 ...
	 * 
	 * @param row
	 *            Fila de la que se quieren obtener las coordenadas
	 * @return Array con 4 valores coordenadas real X, coordenada real Y,
	 *         coordenadas raster X y coordenada raste Y
	 */
	public double[] getCoordinates(int row) {
		...
		try {
			...
			// Esto es necesario porque aunque se produzca una excepción en la
			// lectura
			...
			if (numberFormatException) {
				...
				try {
					getTable().getModel().setValueAt(new String(xMap + ""),
							row, 2);
					getTable().getModel().setValueAt(new String(yMap + ""),
							row, 3);
					getTable().getModel().setValueAt(new String(xRaster + ""),
							row, 4);
					getTable().getModel().setValueAt(new String(yRaster + ""),
							row, 5);
				} catch (NotInitializeException e) {
					// Si está inicializada sino habría entrado en la excepción
					// de nivel mayor
				}
			}
	 ...
	 * @param rms
	 */
	public void addRow(boolean checked, Point2D map, Point2D pixel, double z,
			double eX, double eY, double rms, long id) {
		try {
	...
	/**
	 * Asigna un valor al punto en coordenadas raster de la tabla. Es
	 * recomendable gastar este y no setValueAt para evitar que se recalculen
	 * los errores en cada inserción de cada celda. De esta forma al actualiza
	 * ambas coordenadas a la vez solo se calculará el error una vez.
	 * 
	 * @param valueX
	 *            Valor en X
	 * @param valueY
	...
	/**
	 * Asigna un valor al punto en coordenadas del mundo de la tabla. Es
	 * recomendable gastar este y no setValueAt para evitar que se recalculen
	 * los errores en cada inserción de cada celda. De esta forma al actualiza
	 * ambas coordenadas a la vez solo se calculará el error una vez.
	 * 
	 ...
	 * setValueAt para evitar que se recalculen los errores en cada inserción de
	 * cada celda. De esta forma al actualiza ambas coordenadas a la vez solo se
	 * calculará el error una vez.
	 * 
	 * @param mapX
	 *            Valor en X en coordenadas del mundo
	 * @param mapY
	 *            Valor en Y en coordenadas del mundo
	 * @param pixelX
	 *            Valor en X en coordenadas del raster
	 * @param pixelY
	 *            Valor en Y en coordenadas del raster
	 * @param row
	 *            Fila a actualizar
	 * @param col
	 *            Columna a actualizar
	 */
	public void updatePoint(Object mapX, Object mapY, Object pixelX,
			Object pixelY, int row) {
		try {
	...
	/**
	 * Actualiza los errores de toda la tabla. Esta función debe ser llamada
	 * cuando ha habido algún cambio en los valores de los puntos.
	 * 
	...
	public void updateErrors() {
		...
		GeoTransformDataResult result = (GeoTransformDataResult) process
				.getResult();
		if (result == null)
			return;
		...
		try {
			...
			for (int i = 0; i < gpList.size(); i++) {
				...
				getTable().setValueAt(new Double(point.getErrorX()),
						point.number, 6);
				getTable().setValueAt(new Double(point.getErrorY()),
						point.number, 7);
				getTable().setValueAt(new Double(point.getRms()), point.number,
						8);
			...
			getTable().getTable().getJTable().getColumnModel().getColumn(8)
					.setCellRenderer(cr);
	...
	/**
	 * Inicializa una fila de la tabla con los valores a 0, el número de punto y
	 * el identificador. Esto es util para la inserción de un nuevo punto antes
	...
	/**
	 * Renderer para cambiar el color a la columna de la tabla 06/02/2008
	 * 
	 * @author Nacho Brodin nachobrodin@gmail.com
	 */
