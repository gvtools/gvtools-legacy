...
public class ViewDialog extends BaseZoomView implements IWindow,
	 ...
	 *            Posición en Y
	 * @param w
	 *            Tamaño en pixeles de ancho
	 * @param h
	 *            Tamaño en pixeles de alto
	 */
	public ViewDialog(int posX, int posY, int w, int h,
			ToolListener zoomCursorGraphicListener) {
	 ...
	 * 
	 * @see
	 * org.gvsig.rastertools.georeferencing.view.BaseZoomView#addGraphicLayer
	 * (org.gvsig.rastertools.georeferencing.ui.zoom.IGraphicLayer)
	 */
	...
	public ZoomCursorGraphicLayer getZoomCursorGraphicLayer() {
		if (zoomCursorGraphic == null) {
			zoomCursorGraphic = new ZoomCursorGraphicLayer(w / 2, h / 2, 12, 6,
					zoomCursorGraphicListener);
	...
	/**
	 * Obtiene las coordenadas de la ventana de zoom. Las coordenadas son
	 * devueltas en referencia a las coordenadas del mundo.
	 * 
	 * @return
	 */
	public Rectangle2D getCursorWorldCoordinates() {
		Rectangle2D r = getZoomCursorGraphicLayer().getCursorViewCoordinates();
		Point2D p1 = zoomPixelControl.getCanvas().viewCoordsToWorld(
				new Point2D.Double(r.getX(), r.getY()));
		Point2D p2 = zoomPixelControl.getCanvas().viewCoordsToWorld(
				new Point2D.Double(r.getX() + r.getWidth(), r.getY()
						+ r.getHeight()));
		return new Rectangle2D.Double(Math.min(p1.getX(), p2.getX()), Math.max(
				p1.getY(), p2.getY()), Math.abs(p2.getX() - p1.getX()),
				Math.abs(p2.getY() - p1.getY()));
	}

	/**
	 * Obtiene las coordenadas de la ventana de zoom. Las coordenadas son
	 * devueltas en referencia a las coordenadas del mundo. Esta llamada ajusta
	 * el tamaño de la ventana a la proporción pasada por parámetro. Es decir,
	 * si pasamos una proporción en la que el ancho es 3 veces mayor que el alto
	 * se ajustará el zoom a esta proporción ya que presupone que la ventana de
	 * destino del zoom va a tener esas proporciones.
	 * 
	 * @return
	 */
	public Rectangle2D getCursorAdjustedWorldCoordinates(int wWindow,
			int hWindow) {
		Rectangle2D r = getZoomCursorGraphicLayer().getCursorViewCoordinates();
		...
		try {
			...
			RasterToolsUtil
					.debug("Redimensión de las ventanas. La altura de estas es menor que cero",
							this, ex);
			return r;
		}
	...
	public WindowInfo getWindowInfo() {
		WindowInfo m_viewinfo = new WindowInfo(WindowInfo.MODELESSDIALOG
				| WindowInfo.RESIZABLE | WindowInfo.NOTCLOSABLE);
