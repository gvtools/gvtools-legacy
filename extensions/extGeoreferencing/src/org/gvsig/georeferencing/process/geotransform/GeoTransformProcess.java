...
public class GeoTransformProcess extends RasterProcess {
			...
			for (int i = 0; i <= orden; i++) {
				for (int j = 0; j <= i; j++) {
					...
					for (int v = 0; v < gpcs.size(); v++) {
						...
						if (point.active) {
							matrixD[filas][k] += (Math.pow(
									point.pixelPoint.getX(), i - j) * Math.pow(
									point.pixelPoint.getX(), exp[filas][0]))
									* (Math.pow(point.pixelPoint.getY(), j) * Math
											.pow(point.pixelPoint.getY(),
													exp[filas][1]));
							matrixD2[filas][k] += (Math.pow(
									point.mapPoint.getX(), i - j) * Math.pow(
									point.mapPoint.getX(), exp[filas][0]))
									* (Math.pow(point.mapPoint.getY(), j) * Math
											.pow(point.mapPoint.getY(),
													exp[filas][1]));
						}
					}
					...
					if (filas == 0) {
						...
						for (int v = 0; v < gpcs.size(); v++) {
							GeoPoint point = gpcs.get(v);
							if (point.active) {
								result[k] += (Math.pow(point.pixelPoint.getX(),
										i - j) * Math.pow(
										point.pixelPoint.getX(), exp[filas][0]))
										* (Math.pow(point.pixelPoint.getY(), j) * Math
												.pow(point.pixelPoint.getY(),
														exp[filas][1]))
										* point.mapPoint.getY();
								result2[k] += (Math.pow(point.mapPoint.getX(),
										i - j) * Math.pow(
										point.mapPoint.getX(), exp[filas][0]))
										* (Math.pow(point.mapPoint.getY(), j) * Math
												.pow(point.mapPoint.getY(),
														exp[filas][1]))
										* point.pixelPoint.getY();
								result3[k] += (Math.pow(
										point.pixelPoint.getX(), i - j) * Math
										.pow(point.pixelPoint.getX(),
												exp[filas][0]))
										* (Math.pow(point.pixelPoint.getY(), j) * Math
												.pow(point.pixelPoint.getY(),
														exp[filas][1]))
										* point.mapPoint.getX();
								result4[k] += (Math.pow(point.mapPoint.getX(),
										i - j) * Math.pow(
										point.mapPoint.getX(), exp[filas][0]))
										* (Math.pow(point.mapPoint.getY(), j) * Math
												.pow(point.mapPoint.getY(),
														exp[filas][1]))
										* point.pixelPoint.getX();
							}
						}
					}
				}
	...
	public double[] solveSystem(Matrix matrix, double columResult[]) {
		...
		if (matrix.det() == 0.0) {
			...
			flanagan.math.Matrix matrixFL = new flanagan.math.Matrix(
					matrix.getArray());
	...
	public void calculateRMSerror() {
		...
		for (int im_point = 0; im_point < numgpcs; im_point++) {
			...
			if (point.active) {
				double tmp[] = getCoordMap(point.pixelPoint.getX(),
						point.pixelPoint.getY());
				double tmp2[] = getCoordPixel(tmp[0], tmp[1]);
				point.setEvaluateX(tmp2[0]);
				point.setEvaluateY(tmp2[1]);
				point.setErrorX(Math.pow(point.getEvaluateX()
						- point.pixelPoint.getX(), 2));
				rmsxTotal += point.getErrorX();
				point.setErrorY(Math.pow(point.getEvaluateY()
						- point.pixelPoint.getY(), 2));
	...
	public double[] getCoordMap(double x, double y) {
		...
		for (int i = 0; i < pixelToMapCoefX.length; i++) {
			eval[0] += pixelToMapCoefX[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
			eval[1] += pixelToMapCoefY[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
		}
		return eval;
	}
	...
	public double[] getCoordPixel(double x, double y) {
		...
		double eval[] = new double[2];
		for (int i = 0; i < pixelToMapCoefX.length; i++) {
			eval[0] += mapToPixelCoefX[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
			eval[1] += mapToPixelCoefY[i] * Math.pow(x, exp[i][0])
					* Math.pow(y, exp[i][1]);
		}
		return eval;
	}
