 ...
 * @version 10/2/2008
 **/
public class GeoreferencingProcess extends RasterProcess implements
		IProcessActions {
	...
	/**
	 * Metodo que recoge los parametros del proceso georreferenciacion de un
	 * raster <LI>rasterSE: capa a georeferenciar</LI> <LI>filename: path con el
	 * fichero de salida</LI> <LI>method: metodo de resampleo</LI>
	 */
	public void init() {
	...
	public void process() throws InterruptedException {
		...
		// Obtenida la transformacion la aplicamos a los puntos extremos de la
		// imagen
		...
		double p4[] = transform.getCoordMap(rasterSE.getPxWidth(),
				rasterSE.getPxHeight());
		...
		gridInterpolated = new GridInterpolated(
				(RasterBuffer) imageGrid.getRasterBuf(),
				imageGrid.getGridExtent(), imageGrid.getGridExtent(), bands);

		// SE ESTABLECE EL METODO DE INTERPOLACION (por defecto vecino mas
		// proximo)
		if (rMethod == GridInterpolated.INTERPOLATION_BicubicSpline)
			gridInterpolated
					.setInterpolationMethod(GridInterpolated.INTERPOLATION_BicubicSpline);
		else if (rMethod == GridInterpolated.INTERPOLATION_Bilinear)
			gridInterpolated
					.setInterpolationMethod(GridInterpolated.INTERPOLATION_Bilinear);
		else
			gridInterpolated
					.setInterpolationMethod(GridInterpolated.INTERPOLATION_NearestNeighbour);
				...
				for (int band = 0; band < bands.length; band++) {
					...
					for (int row = 0; row < gridResult.getLayerNY(); row++) {
						...
						for (int col = 0; col < gridResult.getLayerNX(); col++) {
							coord = transform.getCoordPixel(col * cellsize
									+ minPointX, maxPointY - row * cellsize);
							values[band] = (byte) gridInterpolated._getValueAt(
									coord[0], coord[1]);
							gridResult.setCellValue(col, row,
									(byte) values[band]);
						}
						percent = (int) (progress * 100 / (gridResult
								.getLayerNY() * bands.length));
				...
				for (int band = 0; band < bands.length; band++) {
					...
					for (int row = 0; row < gridResult.getLayerNY(); row++) {
						...
						for (int col = 0; col < gridResult.getLayerNX(); col++) {
							coord = transform.getCoordPixel(col * cellsize
									+ minPointX, maxPointY - row * cellsize);
							values[band] = (short) gridInterpolated
									._getValueAt(coord[0], coord[1]);
							gridResult.setCellValue(col, row,
									(short) values[band]);
						}
						percent = (int) (progress * 100 / (gridResult
				...
				for (int band = 0; band < bands.length; band++) {
					...
					for (int row = 0; row < gridResult.getLayerNY(); row++) {
						...
						for (int col = 0; col < gridResult.getLayerNX(); col++) {
							coord = transform.getCoordPixel(col * cellsize
									+ minPointX, maxPointY - row * cellsize);
							values[band] = (int) gridInterpolated._getValueAt(
									coord[0], coord[1]);
							gridResult.setCellValue(col, row,
									(int) values[band]);
						}
				...
				for (int band = 0; band < bands.length; band++) {
					...
					for (int row = 0; row < gridResult.getLayerNY(); row++) {
						...
						for (int col = 0; col < gridResult.getLayerNX(); col++) {
							coord = transform.getCoordPixel(col * cellsize
									+ minPointX, maxPointY - row * cellsize);
							values[band] = (float) gridInterpolated
									._getValueAt(coord[0], coord[1]);
							gridResult.setCellValue(col, row,
									(float) values[band]);
						}
				...
				for (int band = 0; band < bands.length; band++) {
					...
					for (int row = 0; row < gridResult.getLayerNY(); row++) {
						...
						for (int col = 0; col < gridResult.getLayerNX(); col++) {
							coord = transform.getCoordPixel(col * cellsize
									+ minPointX, maxPointY - row * cellsize);
							values[band] = (double) gridInterpolated
									._getValueAt(coord[0], coord[1]);
							gridResult.setCellValue(col, row,
									(double) values[band]);
						}
	...
	private void generateLayer() {
		...
		AffineTransform aTransform = new AffineTransform(
				newExtend.getCellSize(), 0.0, 0.0, -newExtend.getCellSize(),
				newExtend.getMin().getX(), newExtend.getMax().getY());
		int endIndex = filename.lastIndexOf(".");
		if (endIndex < 0)
			endIndex = filename.length();
		try {
			grw = GeoRasterWriter.getWriter(writerBufferServer, filename,
					gridResult.getRasterBuf().getBandCount(), aTransform,
					gridResult.getRasterBuf().getWidth(), gridResult
							.getRasterBuf().getHeight(), gridResult
							.getRasterBuf().getDataType(), GeoRasterWriter
							.getWriter(filename).getParams(), null);
			grw.dataWrite();
			grw.setWkt(rasterSE.getWktProjection());
			grw.writeClose();
		} catch (NotSupportedExtensionException e1) {
			RasterToolsUtil.messageBoxError(PluginServices.getText(this,
					"error_writer_notsupportedextension"), this, e1);
