...
public class Georeferencing implements ButtonsPanelListener, ToolListener,
	...
	/**
	 * Colocación inicial de las ventanas del espacio de trabajo. Carga la
	 * configuración de ventanas con cartografía de referencia.
	 */
	private void initWindowsWithoutMap() {
		...
		ViewRasterRequestManager viewRasterRequestManager = new ViewRasterRequestManager(
				viewRaster, lyrToGeoref);
		viewRaster.setExtensionRequest(viewRasterRequestManager);
		viewRasterSelectPointTool = new SelectPointTool(viewRaster.getCanvas(),
				this);

		// Inicialización de la ventana del zoom de la vista raster
		zoomRaster = new ZoomDialog(tableWidth, posYViews, smallWindowsWidth,
				smallWindowsHeight);
		zoomRaster.setMinxMaxyUL(false);
		ViewRasterRequestManager zoomRasterRequestManager = new ViewRasterRequestManager(
				zoomRaster, lyrToGeoref);
		zoomRaster.setExtensionRequest(zoomRasterRequestManager);
		zoomRasterSelectPointTool = new SelectPointTool(zoomRaster.getCanvas(),
				this);
		...
		layersPointManager.setTools(viewRasterSelectPointTool, null,
				zoomRasterSelectPointTool, null);
		layersPointManager.registerPointToolListener();

		viewsManager.setViews(viewRaster, null, zoomRaster, null);
		viewsManager.setRequestsManager(viewRasterRequestManager, null,
				zoomRasterRequestManager, null);
		viewsManager.setTablePanel(table);

		try {
			viewRaster.setCursorSize(zoomRaster.getCanvasWidth() / ZOOM_RATIO,
					zoomRaster.getCanvasHeight() / ZOOM_RATIO);
			viewRaster.setCursorPosition(viewRaster.getCanvasWidth() / 2,
					viewRaster.getCanvasHeight() / 2);
			...
			Rectangle2D ext = viewRaster.getCursorAdjustedWorldCoordinates(
					zoomRaster.getCanvasWidth(), zoomRaster.getCanvasHeight());
			zoomRasterRequestManager.initRequest(lyrToGeoref.getFullExtent());
			zoomRasterRequestManager.request(ext);

			layersPointManager.createGCPPIxelLayer(viewRaster,
	...
	private void initWindowsWithMap() {
		...
		for (int i = 0; i < windowList.length; i++) {
			if (windowList[i] instanceof IView
					&& windowList[i].getWindowInfo().getTitle()
							.endsWith(": " + selectedView))
				mapControl = ((IView) windowList[i]).getMapControl();
		}
		...
		table = new GCPTablePanel(smallWindowsWidth, posYViews, tableWidth,
				smallWindowsHeight, this);
		buttonsListener = new ApplicationControlsListener(table, this);
		table.initialize(buttonsListener);
		...
		ViewMapRequestManager viewMapRequestManager = new ViewMapRequestManager(
				viewMap, mapControl);
		...
		// Inicialización de la ventana del zoom de la vista raster
		zoomRaster = new ZoomDialog(smallWindowsWidth + tableWidth, posYViews,
				smallWindowsWidth, smallWindowsHeight);
		zoomRaster.setMinxMaxyUL(false);
		...
		zoomMap = new ZoomDialog(0, posYViews, smallWindowsWidth,
				smallWindowsHeight);
		zoomMap.setShowInfo(true);
		ViewMapRequestManager zoomMapRequestManager = new ViewMapRequestManager(
				zoomMap, mapControl);
		...
		layersPointManager.setTools(viewRasterSelectPointTool,
				viewMapSelectPointTool, zoomRasterSelectPointTool,
				zoomMapSelectPointTool);
		layersPointManager.registerPointToolListener();

		viewsManager.setViews(viewRaster, viewMap, zoomRaster, zoomMap);
		viewsManager.setRequestsManager(viewRasterRequestManager,
				viewMapRequestManager, zoomRasterRequestManager,
				zoomMapRequestManager);
		viewsManager.setTablePanel(table);

		try {
			viewMap.setCursorSize(zoomMap.getCanvasWidth() / ZOOM_RATIO,
					zoomMap.getCanvasHeight() / ZOOM_RATIO);
			viewRaster.setCursorSize(zoomRaster.getCanvasWidth() / ZOOM_RATIO,
					zoomRaster.getCanvasHeight() / ZOOM_RATIO);
			viewMap.setCursorPosition(viewMap.getCanvasWidth() / 2,
					viewMap.getCanvasHeight() / 2);
			viewRaster.setCursorPosition(viewRaster.getCanvasWidth() / 2,
					viewRaster.getCanvasHeight() / 2);
			...
			viewMapRequestManager.initRequest(mapControl.getMapContext()
			...
			Rectangle2D ext = viewMap.getCursorAdjustedWorldCoordinates(
					zoomMap.getCanvasWidth(), zoomMap.getCanvasHeight());
			zoomMapRequestManager.initRequest(ext);

			// Inicializamos el zoom de raster
			ext = viewRaster.getCursorAdjustedWorldCoordinates(
					zoomRaster.getCanvasWidth(), zoomRaster.getCanvasHeight());
			zoomRasterRequestManager.initRequest(lyrToGeoref.getFullExtent());
			zoomRasterRequestManager.request(ext);
			...
			layersPointManager.createGCPPIxelLayer(viewRaster,
					viewRasterRequestManager, zoomRasterRequestManager);
			layersPointManager.createGCPMapLayer(viewMap,
	...
	public void addTestRasterLayer(FLyrRasterSE lyr) {
		viewsManager.addTestRasterLayer(lyr, getOptions().getAlgorithm(),
				getOptions().getInterpolationMethod(),
				getOptions().getDegree(), getOptions().getOutFile());
	}
	...
	/**
	 * Lanzador del cuadro de inicialización de propiedades de
	 * georreferenciación.
	 */
	private void launcher() {
		Project p = ((ProjectExtension) PluginServices
	...
	/**
	 * Captura de eventos de los botones Aceptar y Cancelar del cuadro inicial
	 * con las propiedades de georreferenciación.
	 */
	public void actionButtonPressed(ButtonsPanelEvent e) {
		...
		if (e.getButton() == ButtonsPanel.BUTTON_ACCEPT) {
			if (initDialog != null) {
				...
				getOptions().setInterpolationMethod(
						initDialog.getInterpolationMethod());
				getOptions().setXCellSize(initDialog.getXCellSizeValue());
				getOptions().setYCellSize(initDialog.getYCellSizeValue());
				// Inicializamos la matriz de transformación en la imagen que
				// vamos a georreferenciar para
				// que nos de coordenadas pixel
				if (lyrToGeoref == null)
					RasterToolsUtil.messageBoxError("layer_not_loaded", this);
				else {
					lyrToGeoref.setAffineTransform(new AffineTransform(1, 0, 0,
							1, 0, 0));
	...
	/**
	 * Controla los eventos de finalización de la capa con el cursor gráfico que
	 * controla el área de la miniimagen asociada y la terminación de la
	 * operación de mover punto
	 */
	public void endAction(ToolEvent ev) {
		...
		if (ev.getSource() instanceof ZoomCursorGraphicLayer) {
			try {
				...
				if (viewRaster != null) {
					ext = viewRaster.getCursorAdjustedWorldCoordinates(
							zoomRaster.getCanvasWidth(),
							zoomRaster.getCanvasHeight());
					viewsManager.getZoomRasterRequestManager().request(ext);
				}
				if (viewMap != null) {
					ext = viewMap
							.getCursorAdjustedWorldCoordinates(
									zoomMap.getCanvasWidth(),
									zoomMap.getCanvasHeight());
		...
		if (ev.getSource() instanceof GCPsGraphicLayer) {
			...
			// Se redibujan, se obtienen las coordenadas del punto y se
			// actualiza la tabla
			if (layersPointManager != null)
				layersPointManager.redrawPoints();
			GPGraphic gPoint = ((GCPsGraphicLayer) ev.getSource())
					.getLastPoint();
			...
			try {
				table.updatePoint(new Double(pMap.getX()),
						new Double(pMap.getY()), new Double(pRaster.getX()),
						new Double(pRaster.getY()), gPoint.getGeoPoint().number);
				table.getTable()
						.getTable()
						.getJTable()
						.setRowSelectionInterval(gPoint.getGeoPoint().number,
		...
		if (ev.getSource() instanceof SelectPointTool) {
			try {
				// Se obtiene el punto seleccionado, se actualiza la tabla y se
				// redibujan
				int nPoint = table.getTable().getSelectedRow();
				GeoPoint gp = layersPointManager.getPointByNumber(nPoint);

				if (ev.getSource() == viewRasterSelectPointTool
						|| ev.getSource() == zoomRasterSelectPointTool) {
					Point2D[] p = (Point2D[]) ((SelectPointTool) ev.getSource())
							.getResult();
					table.updateRasterPoint(new Double(p[1].getX()),
							new Double(p[1].getY()), nPoint);
					if (gp != null) {
						gp.pixelPoint = p[1];
						layersPointManager.redrawPoints();
					}
				}
				if (ev.getSource() == viewMapSelectPointTool
						|| ev.getSource() == zoomMapSelectPointTool) {
					Point2D[] p = (Point2D[]) ((SelectPointTool) ev.getSource())
							.getResult();
					table.updateMapPoint(new Double(p[1].getX()), new Double(
							p[1].getY()), nPoint);
					if (gp != null) {
						gp.mapPoint = p[1];
						layersPointManager.redrawPoints();
	...
	/**
	 * <P>
	 * Controla las prioridades de eventos sobre la vista en las herramientas y
	 * capas de selección. Esto es necesario para que no se mezclen eventos
	 * entre capas.
	 * <P>
	 * </P>
	 * Prioridades: </P>
	...
	public void onTool(ToolEvent ev) {
		...
		if (ev.getSource() instanceof SelectPointTool) {
			...
			if (viewRaster != null) {
				viewRaster.getControl().selectTool(ZoomRectangleTool.class,
						false);
		...
		if (ev.getSource() instanceof ZoomRectangleTool) {
			...
			if (viewRaster != null) {
				viewRaster.getControl()
						.selectTool(SelectPointTool.class, false);
				viewRaster.getControl().selectTool(PanTool.class, false);
				viewRaster.getControl().getBMove().setSelected(false);
			}
	...
	public void offTool(ToolEvent ev) {
		...
		// Selección de punto
		/*
		 * if(ev.getSource() instanceof SelectPointTool) { } //Selección de zoom
		 * ventana if(ev.getSource() instanceof ZoomRectangleTool) { }
		 */
	}
	...
	/**
	 * Cuando cambia el zoom de alguna vista hay que recalcular la posición del
	 * cursor de zoom
	 */
	public void zoomViewChanged(ViewEvent ev) {
		Rectangle2D ext = null;
		try {
			if (viewRaster != null && zoomRaster != null
					&& viewsManager.getZoomRasterRequestManager() != null) {
				ext = viewRaster.getCursorAdjustedWorldCoordinates(
						zoomRaster.getCanvasWidth(),
						zoomRaster.getCanvasHeight());
				viewsManager.getZoomRasterRequestManager().request(ext);
			}
			if (viewMap != null && zoomMap != null
					&& viewsManager.getZoomMapRequestManager() != null) {
				ext = viewMap.getCursorAdjustedWorldCoordinates(
						zoomMap.getCanvasWidth(), zoomMap.getCanvasHeight());
				viewsManager.getZoomMapRequestManager().request(ext);
			}
