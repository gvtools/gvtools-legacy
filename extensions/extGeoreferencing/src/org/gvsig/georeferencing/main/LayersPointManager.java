...
/**
 * Clase para realizar operaciones y mantener la sincronización entre las capas
 * de puntos de las dos vistas y las dos ventanas de zoom.
 * 
 * 04/02/2008
 * 
...
public class LayersPointManager {
	 ...
	 * @param zMap
	 */
	public void setLayerPoints(GCPsGraphicLayer vRaster, GCPsGraphicLayer vMap,
			GCPsGraphicLayer zRaster, GCPsGraphicLayer zMap) {
	 ...
	 * @param zMap
	 */
	public void setTools(SelectPointTool vRaster, SelectPointTool vMap,
			SelectPointTool zRaster, SelectPointTool zMap) {
	...
	public void setActiveLayerZoomCursor(boolean active) {
		if (viewRaster != null
				&& viewRaster.getZoomCursorGraphicLayer() != null)
	...
	/**
	 * Recalcula las coordenadas de dibujado de los puntos de control y redibuja
	 * estos en todas las vistas
	 */
	...
	/**
	 * Registra el listener para la herramienta de selección de puntos de
	 * control
	 */
	public void registerPointToolListener() {
		if (zoomMap != null) {
			...
			zoomMap.getControl().registerToolListener(ZoomRectangleTool.class,
					appMain);
		...
		if (zoomRaster != null) {
			...
			zoomRaster.getControl().registerToolListener(
					ZoomRectangleTool.class, appMain);
			zoomRaster.getControl()
					.registerToolListener(PanTool.class, appMain);
		}
		if (viewRaster != null) {
			viewRasterSelectPointTool.setActive(false);
			viewRaster.getControl().addTool(viewRasterSelectPointTool);
			viewRaster.getControl().registerToolListener(
					ZoomRectangleTool.class, appMain);
		...
		if (viewMap != null) {
			...
			viewMap.getControl().registerToolListener(ZoomRectangleTool.class,
					appMain);
	 ...
	 * 
	 * @param map
	 *            Coordenadas del mapa del punto
	 * @param raster
	 *            Coordenadas pixel del punto
	...
	/**
	 * Asigna las coordenadas del mundo real y raster al punto indicado en el
	 * parámetro.
	 * 
	 * @param point
	 *            Punto a asignar las nuevas coordenadas
	 * @param xMap
	 *            Coordenada real en X
	 * @param yMap
	 *            Coordenada real en Y
	 * @param xRaster
	 *            Coordenada pixel en X
	 * @param yRaster
	 *            Coordenada pixel en Y
	 */
	public void setCoordinates(int point, double xMap, double yMap,
			double xRaster, double yRaster) {
	...
	public void setXMapCoordinate(int point, double xMap) {
		double oldMapY = viewRasterGCPs.getPoint(point).getGeoPoint().mapPoint
				.getY();
		viewRasterGCPs.getPoint(point).getGeoPoint().mapPoint.setLocation(xMap,
				oldMapY);
	}
	...
	public void setYMapCoordinate(int point, double yMap) {
		double oldMapX = viewRasterGCPs.getPoint(point).getGeoPoint().mapPoint
				.getX();
		viewRasterGCPs.getPoint(point).getGeoPoint().mapPoint.setLocation(
				oldMapX, yMap);
	}
	...
	public void setXRasterCoordinate(int point, double xRaster) {
		double oldRasterY = viewRasterGCPs.getPoint(point).getGeoPoint().pixelPoint
				.getY();
		viewRasterGCPs.getPoint(point).getGeoPoint().pixelPoint.setLocation(
				xRaster, oldRasterY);
	}
	...
	public void setYRasterCoordinate(int point, double yRaster) {
		double oldRasterX = viewRasterGCPs.getPoint(point).getGeoPoint().pixelPoint
				.getX();
		viewRasterGCPs.getPoint(point).getGeoPoint().pixelPoint.setLocation(
				oldRasterX, yRaster);
	}
	...
	public void setVisiblePoint(boolean visible, int position) {
		if (viewRasterGCPs != null)
			((GPGraphic) viewRasterGCPs.getPointList().get(position))
					.setDraw(visible);
		if (viewMapGCPs != null)
			((GPGraphic) viewMapGCPs.getPointList().get(position))
					.setDraw(visible);
		if (viewRasterGCPsZoom != null)
			((GPGraphic) viewRasterGCPsZoom.getPointList().get(position))
					.setDraw(visible);
		if (viewMapGCPsZoom != null)
			((GPGraphic) viewMapGCPsZoom.getPointList().get(position))
					.setDraw(visible);
	}

	/**
	 * Asigna la numeración a los puntos de control. Tanto a la tabla como a los
	 * GeoPuntos
	 * 
	 * @throws NotInitializeException
	 */
	public void calcPointsNumeration(GCPTablePanel table)
			throws NotInitializeException {
		ArrayList pointList = getPointList();

		for (int i = 0; i < table.getRowCount(); i++) {
			table.setValueAt(new Integer(i), i, 1);
			long id = (long) ((Long) table.getValueAt(i,
					table.getColumnCount() - 1)).longValue();
	...
	/**
	 * Crea una capa de puntos de control y la añade a la vista que se le indica
	 * por parámetro
	 * 
	 * @param view
	 */
	...
	/**
	 * Obtiene un punto (GeoPoint) a partir de su número. Hay que tener en
	 * cuenta que el número de punto puede no coincidir con su posición en el
	 * array.
	 * 
	 * @return GeoPoint
	 */
	...
	/**
	 * Obtiene la posición de un punto a partir de su número. Hay que tener en
	 * cuenta que el número de punto puede no coincidir con su posición en el
	 * array.
	 * 
