 ...
 */
public class ApplicationControlsListener implements ActionListener,
		TableModelListener, ButtonsPanelListener {
	...
	/**
	 * Flag que estará a true después de crear un nuevo punto de control.
	 * Volverá a false cuando se ejecute el evento de inserción de fila en una
	 * tabla. Esto es necesario para que no se centre el punto sobre la vista
	 * después de insertar un nuevo punto. Ya que las vistas se van a la
	 * posición (0,0)
	 */
	 ...
	 * @param tablePanel
	 */
	public ApplicationControlsListener(GCPTablePanel tablePanel,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == tablePanel.getCenterButton()) {
			try {
				getViewsManager().centerToPoint(
						tablePanel.getTable().getSelectedRow());
			} catch (NotInitializeException e1) {
				RasterToolsUtil.messageBoxYesOrNot("table_not_initialize",
						tablePanel, e1);
			}
		}
		...
		if (e.getSource() == tablePanel.getOptionsButton()) {
			if (optionsDialog == null)
				optionsDialog = new GeorefOptionsDialog(
						Georeferencing.DEFAULT_DEGREE, this);
			...
			optionsDialog.getOptionsPanel(this).getCheckOptionsPanel()
					.getAddErrorsCSVCheck()
					.setSelected(getOptions().isAddErrorsCSV());
			optionsDialog.getOptionsPanel(this).getCheckOptionsPanel()
					.getCenterViewCheck()
					.setSelected(getOptions().isCenterView());
			optionsDialog.getOptionsPanel(this).getCheckOptionsPanel()
					.getShowNumberCheck()
					.setSelected(getOptions().isShowNumber());
			optionsDialog.setThresholdError(getOptions().getThreshold());
			optionsDialog.setInterpolationMethod(getOptions()
				...
				try {
					if (!getPointManager().isSelectPointToolSelected()) {
						...
						if (tablePanel.getTable().getRowCount() == 0) {
							RasterToolsUtil.messageBoxError("no_rows",
									tablePanel);
							tablePanel.getToolSelectPointButton().setSelected(
									false);
							return;
						}
						...
						if (tablePanel.getTable().getSelectedRow() == -1) {
							RasterToolsUtil.messageBoxError("no_rows_selected",
									tablePanel);
							tablePanel.getToolSelectPointButton().setSelected(
									false);
							return;
		...
		if (e.getSource() == tablePanel.getExporToCSVButton()) {
			getPersistence().exportToCSV(getPointManager().getPointList(),
					getOptions().isAddErrorsCSV());
		}
		...
		if (e.getSource() == tablePanel.getSaveToXMLButton()) {
			getPersistence().saveToRMF(getPointManager().getPointList(),
					appMain.getLayer().getDataSource());
		}
		...
		if (e.getSource() == tablePanel.getLoadFromXMLButton()) {
			setEnableTableEvent(false);
			getPersistence().loadFromRMF(appMain.getLayer().getDataSource(),
					getPointManager(), tablePanel);
			setEnableTableEvent(true);
		}
	}

	/**
	 * Acciones de finalización de georreferenciación. La secuencia de acciones
	 * es:
	 * <UL>
	 * <LI>Preguntar si está seguro de finalizar.</LI>
	 * <LI>Preguntar si se desea salvar la transformación (Solo en caso de ser
	 * afín).</LI>
	 * <LI>Preguntar si se desea cargar el resultado en la vista (Solo en caso
	 * de ser con mapa de referencia).</LI>
	 * </UL>
	 */
	private void endGeoref() {
		if (RasterToolsUtil.messageBoxYesOrNot("ask_end_georef", null)) {
				...
				if (RasterToolsUtil.messageBoxYesOrNot("ask_load_view", null)) {
					...
					if (appMain.getOptions().getAlgorithm() == Georeferencing.POLYNOMIAL) {
						if (appMain.getLastTestLayer() != null && lyrs != null) {
							appMain.getLayers().addLayer(
									appMain.getLastTestLayer());
						} else {
							RasterToolsUtil.messageBoxInfo(
									"raster_not_created", null);
							return;
						}
					}
				} else {
			...
			if (saveTransform) {
				...
				IRasterDataSource dataSource = appMain.getLastTestLayer()
						.getDataSource();
				try {
					for (int i = 0; i < dataSource.getDatasetCount(); i++) {
						dataSource.saveObjectToRmf(i, RasterDataset.class,
								dataSource.getDataset(i)[0]);
						RasterUtilities.createWorldFile(
								dataSource.getNameDatasetStringList(0, 0)[i],
								dataSource.getAffineTransform(0),
								(int) dataSource.getWidth(),
								(int) dataSource.getHeight());
					}
				} catch (RmfSerializerException e) {
					RasterToolsUtil.messageBoxError("error_salvando_rmf", null,
							e);
				} catch (IOException e) {
					RasterToolsUtil.messageBoxError("error_salvando_rmf", null,
							e);
	...
	/**
	 * Evento de modificación de la tabla de puntos de control. El añadir,
	 * borrar o mover puntos dentro de la tabla hace que estos se creen, borren
	 * o muevan en la lista de GeoPoints.
	 */
	public void tableChanged(TableModelEvent e) {
		...
		try {
			...
			if (e.getType() == TableModelEvent.INSERT) {
				long id = getPointManager().addPoint(new Point2D.Double(0, 0),
						new Point2D.Double(0, 0));
				tablePanel.initializeRow(e.getFirstRow(), id);
				getPointManager().calcPointsNumeration(tablePanel);

				// Activamos la herramienta de pinchar punto en la vista al
				// añadir punto
			...
			if (e.getType() == TableModelEvent.UPDATE) {
				getPointManager().calcPointsNumeration(tablePanel);
				// Revisamos el flag de activo de cada punto poniendo el valor
				// que pone en la entrada de la tabla
				for (int i = 0; i < tablePanel.getRowCount(); i++) {
					boolean active = ((Boolean) tablePanel.getValueAt(i, 0))
							.booleanValue();
					int position = getPointManager()
							.getPointPositionByNumber(i);
					if (position < 0)
						return;
					GeoPoint pt = getPointManager().getGeoPoints()
							.get(position);
					pt.active = active;
					getPointManager().setVisiblePoint(active, position);

					// Para la funcionalidad de cambio de coordenadas desde la
					// tabla: reasignamos el valor
					// de las coordenadas por si han cambiado estas
					double[] values = tablePanel.getCoordinates(i);

					getPointManager().setCoordinates(position, values[0],
	...
	public void actionButtonPressed(ButtonsPanelEvent e) {
		...
		if (e.getButton() == ButtonsPanel.BUTTON_APPLY
			...
			if (optionsDialog != null) {
				...
				getOptions().setBackgroundColor(
						optionsDialog.getBackGroundColor());
				getOptions().setTextColor(optionsDialog.getTextColor());
				getOptions().setAddErrorsCSV(
						optionsDialog.getOptionsPanel(this)
								.getCheckOptionsPanel().getAddErrorsCSVCheck()
								.isSelected());
				getOptions().setCenterView(
						optionsDialog.getOptionsPanel(this)
								.getCheckOptionsPanel().getCenterViewCheck()
								.isSelected());
				getOptions().setShowNumber(
						optionsDialog.getOptionsPanel(this)
								.getCheckOptionsPanel().getShowNumberCheck()
								.isSelected());
				getOptions().setInterpolationMethod(
						optionsDialog.getInterpolationMethod());
				getOptions().setOutFile(
						optionsDialog.getOptionsPanel(this)
								.getOutFileSelectionPanel().getOutFile());
				...
				try {
					getOptions().setThreshold(
							Double.valueOf(
									optionsDialog.getThresholdError()
											.getValue()).doubleValue());
					getOptions().setXCellSize(
							optionsDialog.getOptionsPanel(this)
									.getCellSizePanel().getXCellSizeValue());
					getOptions().setYCellSize(
							optionsDialog.getOptionsPanel(this)
									.getCellSizePanel().getYCellSizeValue());
				} catch (NumberFormatException exc) {
					RasterToolsUtil.messageBoxError("error_numeric_format",
							null, exc);
				}

				getViewsManager().setGraphicsColor(getOptions().getTextColor());
				getViewsManager().setBackgroundColor(
						getOptions().getBackgroundColor());
				getPointManager().setActiveNumberPoint(
						getOptions().isShowNumber());
	 ...
	 * 
	 * @param enableTableEvent
	 *            true para activarlos o false para desactivarlos
	 */
	...
	/**
	 * Obtiene una instancia del objeto que gestiona la persistencia en disco de
