...
/**
 * <p>
 * Panel where user will select the geometric information of the layer, to be
 * added.
 * </p>
 * 
 * @version 23/05/08
...
public class AddGeometricInfoPanel extends JPanel implements IWindow {
	...
	/**
	 * <p>
	 * Creates a new form where user could select the geometric information to
	 * add.
	 * </p>
	 */
	...
	private void initialize() {
		...
		if (!refreshVisibleVectorLayers()) {
			JOptionPane.showMessageDialog(this, PluginServices.getText(this,
					"No_vector_layer_can_be_save_changes"), PluginServices
					.getText(this, "Warning"), JOptionPane.WARNING_MESSAGE);
		} else {
			setVisible(true);
	...
	/**
	 * <p>
	 * This method initializes isEditableLabel.
	 * </p>
	 * 
	...
	private JLabel getIsEditableLabel() {
		if (isEditableLabel == null) {
			isEditableLabel = new JLabel(PluginServices.getText(this,
					"Writable"));
			isEditableLabel.setToolTipText(PluginServices.getText(this,
	...
	/**
	 * <p>
	 * Updates the lists with the geometric properties of <code>layer</code>.
	 * </p>
	 * 
	 * @param layer
	 *            a vector layer
	...
	private void updateEditableLabel(FLyrVect layer) {
		if (layer.isWritable()) {
			// Only can save the changes if layer is writable and its driver is
			// of a PostGree database,
			// shape, or GML
			...
			if (vd.getName().equalsIgnoreCase("gvSIG shp driver")
				...
				DefaultListModel attrsListModel = (DefaultListModel) getAttributesList()
						.getModel();
				attrsListModel.removeAllElements();

				DefaultListModel attrsAddedListModel = (DefaultListModel) getAttributesAddedList()
						.getModel();
	...
	/**
	 * <p>
	 * This method initializes southWestScrollPane.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * This method initializes southCenterPanel.
	 * </p>
	 * 
	 * @return javax.swing.JPanel
	 */
	private JPanel getSouthCenterPanel() {
		if (southCenterPanel == null) {
			...
			rightIconButton.setToolTipText(PluginServices.getText(this,
			...
			rightIconButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					DefaultListModel listModel = (DefaultListModel) getAttributesList()
							.getModel();
					int firstIndex = -1;
					...
					if (values.length > 0) {
						...
						for (int i = 0; i < values.length; i++) {
							((DefaultListModel) getAttributesAddedList()
									.getModel()).addElement(values[i]);
					...
					if (listModel.size() > 0) {
						if (firstIndex >= listModel.size())
							getAttributesList().setSelectedIndex(
									listModel.size() - 1);
						else {
			...
			leftIconButton.setToolTipText(PluginServices.getText(this,
			...
			leftIconButton.addActionListener(new ActionListener() {
				 ...
				 */
				public void actionPerformed(ActionEvent e) {
					Object[] values = getAttributesAddedList()
							.getSelectedValues();

					DefaultListModel listModel = (DefaultListModel) getAttributesAddedList()
							.getModel();
					int firstIndex = -1;
					...
					if (values.length > 0) {
						...
						for (int i = 0; i < values.length; i++) {
							((DefaultListModel) getAttributesList().getModel())
									.addElement(values[i]);
							listModel.removeElement(values[i]);
						}
					}
					...
					if (listModel.size() > 0) {
						if (firstIndex >= listModel.size())
							getAttributesAddedList().setSelectedIndex(
									listModel.size() - 1);
						else {
							getAttributesAddedList().setSelectedIndex(
									firstIndex);
			...
			doubleRightIcon = PluginServices.getIconTheme().get(
					"double-right-arrow-icon");
			doubleLeftIcon = PluginServices.getIconTheme().get(
					"double-left-arrow-icon");
			doubleRightIconButton = new JButton(doubleRightIcon);
			doubleRightIconButton.setToolTipText(PluginServices.getText(this,
			...
			doubleRightIconButton.addActionListener(new ActionListener() {
				 ...
				 */
				public void actionPerformed(ActionEvent e) {
					Object[] values = ((DefaultListModel) getAttributesList()
							.getModel()).toArray();
			...
			doubleLeftIconButton.setToolTipText(PluginServices.getText(this,
			...
			doubleLeftIconButton.addActionListener(new ActionListener() {
				 ...
				 */
				public void actionPerformed(ActionEvent e) {
					Object[] values = ((DefaultListModel) getAttributesAddedList()
							.getModel()).toArray();
					...
					if (values.length > 0) {
						for (int i = 0; i < values.length; i++) {
							((DefaultListModel) getAttributesList().getModel())
									.addElement(values[i]);
							((DefaultListModel) getAttributesAddedList()
									.getModel()).removeElement(values[i]);
						}
					}
	...
	/**
	 * <p>
	 * This method initializes reloadIconButton.
	 * </p>
	 * 
	...
	private JButton getReloadIconButton() {
		if (reloadIconButton == null) {
			...
			reloadIconButton.setToolTipText(PluginServices.getText(this,
			...
			reloadIconButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (getLayersComboBox().getItemCount() > 0) {
						FLayerWrapper fW = (FLayerWrapper) getLayersComboBox()
								.getItemAt(0);
						((DefaultListModel) getAttributesList().getModel())
								.removeAllElements();
						((DefaultListModel) getAttributesAddedList().getModel())
								.removeAllElements();
	...
	/**
	 * <p>
	 * Sets the GUI components to <code>b</code>.
	 * </p>
	 * 
	 * @param b
	 *            <code>true</code> to enable the GUI components,
	...
	/**
	 * <p>
	 * Creates the <code>Spring</code> layout of the south center panel.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * This method initializes southEastPanel.
	 * </p>
	 * 
	 * @return javax.swing.JPanel
	 */
	...
	/**
	 * <p>
	 * This method initializes southEastCenterPanel.
	 * </p>
	 * 
	 * @return javax.swing.JPanel
	 */
	...
	/**
	 * <p>
	 * This method initializes saveGeomPropertyButton.
	 * </p>
	 * 
	 * @return javax.swing.JButton
	 */
	private JButton getSaveGeomPropertyButton() {
		if (saveGeomPropertyButton == null) {
			...
			saveGeomPropertyButton.setToolTipText(PluginServices.getText(this,
	...
	/**
	 * <p>
	 * This method initializes addedAttributesScrollPane.
	 * </p>
	 * 
	 * @return javax.swing.JScrollPane
	 */
	private JScrollPane getAddedAttributesScrollPane() {
		if (addedAttributesScrollPane == null) {
			addedAttributesScrollPane = new JScrollPane(
					getAttributesAddedList());
	...
	/**
	 * <p>
	 * This method initializes attributesAddedList.
	 * </p>
	 * 
	 * @return javax.swing.JList
	 */
	private JList getAttributesAddedList() {
		if (attributesAddedList == null) {
			...
			DefaultListModel listModel = ((DefaultListModel) attributesAddedList
					.getModel());
			...
			listModel.addListDataListener(new ListDataListener() {
				 ...
				 * 
				 * @see
				 * javax.swing.event.ListDataListener#contentsChanged(javax.
				 ...
				 * 
				 * @see
				 * javax.swing.event.ListDataListener#intervalAdded(javax.swing
				 * .event.ListDataEvent)
				 */
				 ...
				 * 
				 * @see
				 * javax.swing.event.ListDataListener#intervalRemoved(javax.
				 * swing.event.ListDataEvent)
				 */
					...
					.addListSelectionListener(new ListSelectionListener() {
						...
						public void valueChanged(ListSelectionEvent e) {
							if (!e.getValueIsAdjusting()) {
								Object[] values = attributesAddedList
										.getSelectedValues();
								if (values.length == 1) {
	...
	/**
	 * <p>
	 * This method initializes newColumnCheckBox.
	 * </p>
	 * 
	...
	private JCheckBox getNewColumnCheckBox() {
		if (newColumnCheckBox == null) {
			newColumnCheckBox = new JCheckBox(PluginServices.getText(this,
					"New_field"));
			newColumnCheckBox.setToolTipText(PluginServices.getText(this,
	...
	/**
	 * <p>
	 * Updates the component that has the name of the field selected.
	 * </p>
	 */
	private void updateColumnNameFromField() {
		...
		if (getNewColumnCheckBox().isSelected()) {
			...
			try {
				DefaultComboBoxConfigurableLookUpModel model = ((DefaultComboBoxConfigurableLookUpModel) ((JComboBoxConfigurableLookUp) getNameOfColumnCombo())
						.getModel());
				model.removeAllElements();

				FLyrVect layer = ((FLyrVect) ((FLayerWrapper) getLayersComboBox()
				...
				if (model.getSize() > 0) {
					if ((currentField.getName() == null)
							|| (((DefaultComboBoxConfigurableLookUpModel) getNameOfColumnCombo()
									.getModel()).getIndexOf(currentField
									.getName()) == -1))
						getNameOfColumnCombo().setSelectedItem(
				...
				JOptionPane.showMessageDialog(null,
						PluginServices.getText(null, "Failed_loading_fields"),
	...
	private void updateAttributesToCurrentField() {
		if (currentField != null) {
			...
			if (currentColumnNameComponent instanceof JTextFieldWithSCP) {
				...
				currentField
						.setName(applyReplaces(((String) getNameOfColumnCombo()
								.getSelectedItem())));
			}
		}
	}
	...
	/**
	 * <p>
	 * Applies the different kinds of replacement defined in
	 * {@link StringUtilitiesExtension StringUtilitiesExtension} to
	 * <code>s</code>.
	 * </p>
	 * 
	 * @param s
	...
	private String applyReplaces(String s) {
		return StringUtilitiesExtended
				.replaceAllAccents(StringUtilitiesExtended
						.replaceAllCedilla(StringUtilitiesExtended
								.replaceAllNWithTilde(s)));
	}

	/**
	 * <p>
	 * Updates the current field properties with the configuration in the GUI.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * This method initializes nameOfColumnText.
	 * </p>
	 * 
	...
	private JTextFieldWithSCP getNameOfColumnText() {
		if (nameOfColumnText == null) {
			...
			nameOfColumnText.setToolTipText(PluginServices.getText(this,
	...
	/**
	 * <p>
	 * This method initializes nameOfColumnCombo.
	 * </p>
	 * 
	...
	private JComboBoxConfigurableLookUp getNameOfColumnCombo() {
		if (nameOfColumnCombo == null) {
			...
			DefaultComboBoxConfigurableLookUpModel model = ((DefaultComboBoxConfigurableLookUpModel) ((JComboBoxConfigurableLookUp) nameOfColumnCombo)
					.getModel());
			model.setShowAllItemsInListBox(true);
			nameOfColumnCombo.setPreferredSize(new Dimension(215, 22));
			nameOfColumnCombo.setToolTipText(PluginServices.getText(this,
					"name_of_the_new_property_TOOLTIP_HTML_explanation"));
		}
					...
					if (e.getStateChange() == ItemEvent.SELECTED) {
						try {
							...
							NotificationManager.showMessageError(PluginServices
									.getText(this, "problem_loading_layers"),
									dE);
						}
					}
				}
	 ...
	 * 
	 * @return <code>true</code> if there is any layer added that can be
	...
	private boolean refreshVisibleVectorLayers() {
		...
		// If there is only one layer -> disable the possibility of selection by
		// the user
	...
	/**
	 * <p>
	 * Updates "layersComboBox" with the name of visible layers down
	 * <code>node</code>.
	 * </p>
	 * 
	 * @param node
	 ...
	 * 
	 * @return <code>true</code> if some layer added can be edited, otherwise
	 *         <code>false</code>
	 */
		...
		if (node instanceof FLayers) {
			...
			if ((node.isVisible()) && (node instanceof FLyrVect)
				...
				getLayersComboBox().addItem(
						new FLayerWrapper((FLyrVect) node, level, bIcon,
								getLayerIcon(node)));
	...
	/**
	 * <p>
	 * Updates attrsListModel with the geometric attributes according the shape
	 * type of the current selected layer.
	 * </p>
	 * 
	...
	private void refreshFields(FLyrVect seletedLayer) {
		try {
			...
			case FShape.NULL:
				break;
			case FShape.POINT:
				geomInfo = GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "X_coordinate"),
						getNewFieldName(fieldNames,
				...
				geomInfo = GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "Y_coordinate"),
						getNewFieldName(fieldNames,
				...
				geomInfo = GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "Z_coordinate"),
						getNewFieldName(fieldNames,
				...
				break;
			case FShape.LINE:
				attrsListModel.addElement(GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "Length"),
						getNewFieldName(fieldNames,
								PluginServices.getText(this, "Length")),
						FShape.LINE));
				break;
			case FShape.POLYGON:
				...
				attrsListModel.addElement(geomInfo);
				geomInfo = GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "Perimeter"),
						getNewFieldName(fieldNames,
								PluginServices.getText(this, "Perimeter")),
						FShape.POLYGON);
			...
			case FShape.TEXT:
				break;
			case FShape.MULTI: // Can have points, multipoints, lines and
								// polygons
				/* POINT */
				geomInfo = GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "X_coordinate"),
				...
				/* MULTIPOINT */
				attrsListModel.addElement(GeomInfoFactory.createGeomInfo(
						PluginServices.getText(this, "Number_of_points"),
						getNewFieldName(fieldNames,
								PluginServices.getText(this, "N_points")),
						FShape.MULTIPOINT));
				break;
			...
			JOptionPane.showMessageDialog(this,
					PluginServices.getText(this, "Failed_loading_fields"),
					PluginServices.getText(this, "Error"),
					JOptionPane.ERROR_MESSAGE);
		}
	}

	private String getNewFieldName(HashSet currentFieldNames,
			String proposedName) {
	...
	public WindowInfo getWindowInfo() {
		if (viewInfo == null) {
			...
			viewInfo.setTitle(PluginServices
					.getText(this, "add_geometric_info"));
	 ...
	 * component of the <code>AddGeometricInfoPanel</code> panel.
	 * </p>
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class AdaptedAcceptCancelPanel extends AcceptCancelPanel {
		...
		/**
		 * <p>
		 * Creates a new <code>AdaptedAcceptCancelPanel</code>
		 * </p>
		 */
		...
		private ActionListener getOKAction() {
			...
			return new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					/* 1- Initial tasks */
					/*
					 * 1.1- If layer is in edition, saves it and ends the
					 * edition
					 */
					FLyrVect layer = ((FLayerWrapper) getLayersComboBox()
							.getSelectedItem()).getLayer();
					...
					if (wasBeingEdited) {
						try {
							...
							NotificationManager.showMessageError(PluginServices
									.getText(this, "Failed_saving_the_layer"),
									ex);
					...
					final ArrayList activeLayers = getActiveLayerIndexes(view
							.getTOC().getTree());

					/* 2- Creates the process */
					AddGeometricInfoProcess iprocess = new AddGeometricInfoProcess(
							PluginServices
									.getText(this,
											"Add_geometric_information_to_layer_process"),
							PluginServices.getText(this,
									"Ongoing_process_please_wait"), view,
							layer, ((DefaultListModel) getAttributesAddedList()
									.getModel()).toArray());
					...
					iTask.getProgressPanel().addComponentListener(
							new ComponentAdapter() {
								...
								public void componentHidden(ComponentEvent e) {
									...
									/*
									 * 7- Refreshes all the associated project
									 * tables
									 */
									ProjectTable pt = f_iprocess
											.getLayerProjectTable();
									if (pt != null) {
									...
									/*
									 * 9- Writes in the gvSIG log the results of
									 * the process
									 */
									String text = "\n- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -\n"
											+ PluginServices
									...
									/*
									 * 10- If the process has been canceled ->
									 * refreshes the view
									 */
									...
									/*
									 * 11- Refreshes the toolbars and their
									 * controls
									 */
									PluginServices.getMainFrame()
											.enableControls();
		 ...
		 * 
		 * @throws DriverException
		 *             if fails the driver associated to the layer
		 * @throws ReadDriverException
		 * @throws InitializeWriterException
		 * @throws StopWriterVisitorException
		 * @throws EditionException
		 *             if fails a edition process with the layer
		 */
		private void saveLayer(FLyrVect layer) throws DriverException,
				ReadDriverException, InitializeWriterException,
	...
	private class FLayerWrapper extends JPanel {
		 ...
		 * 
		 * @param layer
		 *            the vector layer to be wrappered
		 * @param level
		 *            the level in the tree of the layer wrappered
		 * @param branchIcon
		 *            icon that represents the branch
		 * @param leafIcon
		 *            icon that represents the leaf
		 */
		public FLayerWrapper(FLyrVect layer, int level, ImageIcon branchIcon,
				Icon leafIcon) {
			super();
	...
	/**
	 * <p>
	 * Gets a list with the indexes of the active layers in the tree.
	 * </p>
	 * 
	 * @param tree
	 *            the {@link TOC TOC} tree with teh active layers
	 * 
		...
		try {
			for (int row = 0; row < tree.getRowCount(); row++) {
				...
				if (node instanceof DefaultMutableTreeNode) {
					userObject = ((DefaultMutableTreeNode) node)
							.getUserObject();
			...
			JOptionPane.showMessageDialog(this, PluginServices.getText(this,
					"Failed_saving_active_layer_indexes"), PluginServices
	...
	private void restoreActiveLayers(JTree tree, ArrayList rows) {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, PluginServices.getText(this,
					"Failed_restoring_active_layers"), PluginServices.getText(
