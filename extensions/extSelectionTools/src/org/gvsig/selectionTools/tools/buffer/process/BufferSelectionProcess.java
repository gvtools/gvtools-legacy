...
//									
//									width = (f_width * relationDistanceUnitsToMeters) * MapContext.CHANGEM[8]; // (units selected to meters) * conversion to º
						}
						else {
							width = f_width * relationDistanceUnitsToMeters;
						}
...
//									width = Geo.getDecimalDegrees(width);
					...
					if (mapControl.getCrs() instanceof ProjectedCRS) {
						...
						log.addLine("    " + PluginServices.getText(null, "Buffer_width") + ": " + width + " m");
						log.addLine("    " + PluginServices.getText(null, "Buffer_width") + ": " + width / MapContext.CHANGEM[8] + " º");
					}
