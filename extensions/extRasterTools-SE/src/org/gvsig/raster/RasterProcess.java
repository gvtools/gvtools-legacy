 ...
 */
public abstract class RasterProcess implements IIncrementable,
	...
	public void setCancelable(boolean enabled) {
		cancellable = enabled;
		getIncrementableTask().getButtonsPanel().setEnabled(
				ButtonsPanel.BUTTON_CANCEL, enabled);
	}
	 ...
	 * explicita y obtenerlos directamente de la tabla Hash cuando vayan a
	 * usarse. Esto queda a elección del programador. En caso de hacerse una
	 * carga de parámetros explicita esta llamada debería hacerse justo despues
	 * del constructor de la clase que contendrá el proceso.
	 */
	...
	/**
	 * Obtención de un objeto de resultado. Este objeto deberá ser el mismo que
	 * el que se pasa por parámetro en el "end" de IProcessActions. Este método
	 * es util cuando la tarea no se lanza en un Thread (ejecutamos process
	 * directamente) y queremos recoger el resultado sin registrarnos al evento
	...
	/**
	 * Activa o desactiva el interfaz de progreso en el lanzamiento de la tarea
	 * como un thread.
	 * 
	 * @param active
	 *            true para activarlo o false para desactivarlo
	 */
	...
	/**
	 * Obtiene el tiempo que tardó en ejecutarse la tarea la última vez que se
	 * procesó
	 */
	public long getTime() {
		return time;
	...
	/**
	 * Obtiene el objeto para ejecutar acciones de la cola de procesos de
	 * ejecución exclusiva.
	 * 
	 * @param IProcessActions
	 */
	...
	/**
	 * Un evento de cancelado es enviado a la tarea cuando actionCanceled es
	 * activado. Para ello se creará un objeto CancelEvent y se asignará a la
	 * tarea en ejecución. Esta lo procesará cuando pueda e interrumpirá el
	 * proceso.
	 */
	...
	public byte getByteParam(String name) {
		Object value = taskParams.get(name);
		return (value != null && value instanceof Byte) ? ((Byte) value)
				.byteValue() : 0;
	}
	...
	public float getFloatParam(String name) {
		Object value = taskParams.get(name);
		return (value != null && value instanceof Float) ? ((Float) value)
				.floatValue() : 0F;
	}
	...
	public int[] getIntArrayParam(String name) {
		Object value = taskParams.get(name);
		return (value != null && value instanceof int[]) ? ((int[]) value)
				: null;
	}
	...
	public FLyrRasterSE getLayerParam(String name) {
		Object value = taskParams.get(name);
		return (value != null && value instanceof FLyrRasterSE) ? ((FLyrRasterSE) value)
				: null;
	}
	...
	public Extent getExtentParam(String name) {
		Object value = taskParams.get(name);
		return (value != null && value instanceof Extent) ? ((Extent) value)
				: null;
	}
