...
/**
 * Clase que contiene el panel con los elementos para añadir una serie de
 * intervalos. La selección de intervalos puede ser seleccionando el número de
 * estos para una distancia definida entre un máximo y un mínimo. En este caso
 * se calculará automáticamente la distancia entre los tramos. También puede ser
 * calculada a partir de una distancia dada. En este caso calculará
 * automáticamenet el número de tramos para la longitud total.
 * 
 * El controlador está metido dentro del mismo panel por lo que tendrá también
 * una referencia al modelo de datos.
 * 
 * 06/08/2008
 * 
 * @author Nacho Brodin nachobrodin@gmail.com
 */
public class StretchSelectorPanel extends BasePanel implements ActionListener,
		DataInputContainerListener, Observer {
	...
	/**
	 * Obtiene el componente botón de selección de creación de tramos por valor
	 * del intervalo
	 * 
	 * @return JRadioButton
	 */
	public JRadioButton getIntervalSize() {
		if (jRadioIntervalSize == null) {
			jRadioIntervalSize = new JRadioButton(PluginServices.getText(this,
					"tamano_intervalo"), true);
			jRadioIntervalSize.setBorder(BorderFactory.createEmptyBorder(0, 0,
					0, 0));
	...
	/**
	 * Obtiene el componente botón de selección de creación de tramos por número
	 * de intervalos
	 * 
	 * @return JRadioButton
	 */
	public JRadioButton getIntervalNumber() {
		if (jRadioIntervalNumber == null) {
			jRadioIntervalNumber = new JRadioButton(PluginServices.getText(
					this, "n_intervalos"));
			jRadioIntervalNumber.setBorder(BorderFactory.createEmptyBorder(0,
					0, 0, 0));
	...
	public double getInterval() {
		...
		if (getIntervalSize().isSelected()) {
			if (value > Math.abs(getData().getMaximum()
					- getData().getMinimum()))
				value = Math.abs(getData().getMaximum()
						- getData().getMinimum());
			if (value <= 0)
				value = 1;
		} else {
