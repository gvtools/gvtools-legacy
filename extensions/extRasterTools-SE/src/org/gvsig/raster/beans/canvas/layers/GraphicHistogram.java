...
/**
 * Gráfica que representa un histograma con los valores pasados en el
 * constructor
 * 
 * 14-oct-2007
 * 
...
public class GraphicHistogram extends DrawableElement {
	...
	protected void paint(Graphics g) {
		...
		case TYPE_FILL:
			...
			for (int i = 0; i < valuesToDraw.length; i++) {
				int x1 = (int) Math.round(((double) i - 0.5D)
						* (width / (valuesToDraw.length - 1.0D)));
				int x2 = (int) Math.round(((double) i + 0.5D)
						* (width / (valuesToDraw.length - 1.0D)));
				...
				g.setColor(color);
				g.fillRect(x1, y1, x2 - x1, canvas.getCanvasMaxY() - border
						- y1);
			}
			break;
		default:
			...
			for (int i = 1; i < valuesToDraw.length; i++) {
				int x1 = (int) Math
						.round(canvas.getCanvasMinX()
								+ ((i - 1.0D) * (width / (valuesToDraw.length - 1.0D))));
				int x2 = (int) Math.round(canvas.getCanvasMinX()
						+ (i * (width / (valuesToDraw.length - 1.0D))));
				g.drawLine(x1, valueToPixelY(valuesToDraw[i - 1]), x2,
						valueToPixelY(valuesToDraw[i]));
			}
			break;
		}
	}
	...
	/**
	 * Asigna el tipo de gráfica a mostrar. El tipo de gráfica está definida por
	 * las constantes TYPE definidas en esta clase. Por defecto siempre se
	 * muestra el tipo LINE
	 * 
	 * @param type
	 */
	...
	/**
	 * Asigna el tipo de vista para el histograma a mostrar. No hace un
	 * repintado
	 * 
	 * @param type
	 */
	...
	/**
	 * Calculamos el histograma a dibujar en cuestion. Los tipos son:
	 * <p>
	 * VIEW_LINEAL<br>
	 * VIEW_ACUMMULATED<br>
	 * VIEW_LOGARITHMIC
	...
	private double[] recalcHistogram(int type) {
		...
		case VIEW_ACUMMULATED:
			...
			if (valuesAcummulated.length >= 1) {
				...
				for (int i = 1; i < histogramValues.length; i++)
					valuesAcummulated[i] = histogramValues[i]
							+ valuesAcummulated[i - 1];
			}
		...
		case VIEW_LOGARITHMIC:
			...
			for (int i = 0; i < histogramValues.length; i++)
				valuesLogarithmic[i] = java.lang.Math.log(histogramValues[i]
						- min + 1.0);
		...
		case VIEW_ACUMMULATEDLOG:
			...
			if (valuesAcummulatedLog.length >= 1) {
				...
				for (int i = 1; i < histogramValues.length; i++)
					valuesAcummulatedLog[i] = histogramValues[i]
							+ valuesAcummulatedLog[i - 1];
			}
			...
			for (int i = 0; i < valuesAcummulatedLog.length; i++)
				valuesAcummulatedLog[i] = java.lang.Math
						.log(valuesAcummulatedLog[i] - min + 1.0);

			convertToPercent(valuesAcummulatedLog);

			return valuesAcummulatedLog;

			// Si no es logaritmica ni acumulada, lo tratamos como si fuera
			// lineal
		default:
			// Si ya estan calculados, no los vuelve a calcular
			if (valuesLineal != null)
				return valuesLineal;
