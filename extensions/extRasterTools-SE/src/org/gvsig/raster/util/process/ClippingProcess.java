 ...
 * para aplicar un recorte a una capa y guardarlo en disco. Muestra una barra de
 * incremento informativa.
 * 
 * @version 24/04/2007
...
public class ClippingProcess extends RasterProcess {
	 ...
	 * <LI>viewname: Nombre de la vista sobre la que se carga la capa al acabar
	 * el proceso</LI>
	 ...
	 * <LI>onelayerperband: booleano que informa de si escribimos una banda por
	 * fichero de salida o todas en un fichero</LI>
	 * <LI>interpolationmethod: Método de interpolación.</LI>
	 * <LI>affinetransform: Transformación que informa al dataset de salida de
	 * su referencia geografica</LI>
	...
	private void saveToRmf(String fileName) {
		...
		if (Configuration
				.getValue("nodata_transparency_enabled", Boolean.FALSE)
				.booleanValue()) {
			try {
				RasterDataset.saveObjectToRmfFile(
						fileName,
						NoData.class,
						new NoData(rasterSE.getNoDataValue(), rasterSE
								.getNoDataType(), rasterSE.getDataType()[0]));
		...
		if (colorTableFilter != null) {
			...
			try {
				RasterDataset.saveObjectToRmfFile(fileName, ColorTable.class,
						(ColorTable) gridPalette);
			} catch (RmfSerializerException e) {
				RasterToolsUtil.messageBoxError("error_salvando_rmf", this, e);
			}
		}
	...
	public void process() throws InterruptedException {
		...
		try {
			...
			if (interpolationMethod != BufferInterpolation.INTERPOLATION_Undefined) {
				try {
					if (pValues != null) {
						if (RasterBuffer.isBufferTooBig(
								new double[] { pValues[0], pValues[3],
										pValues[2], pValues[1] },
								drawableBands.length))
							bufferFactory.setReadOnly(true);
						bufferFactory.setAreaOfInterest(pValues[0], pValues[3],
								pValues[2] - pValues[0], pValues[1]
										- pValues[3]);
						...
						if (!rasterSE
							...
							bufferFactory.setReadOnly(true);
						bufferFactory.setAreaOfInterest(wcValues[0],
								wcValues[1],
					...
					RasterToolsUtil
							.messageBoxError(
									"No se ha podido asignar la vista al inicial el proceso de recorte.",
									this, e);
				}
				...
				if (buffer != null)
					buffer = ((RasterBuffer) buffer).getAdjustedWindow(
				...
				try {
					if (RasterBuffer.isBufferTooBig(new double[] { 0, 0,
							resolutionWidth, resolutionHeight },
							drawableBands.length))
						bufferFactory.setReadOnly(true);
					if (pValues != null)
						...
						bufferFactory.setAreaOfInterest(wcValues[0],
								wcValues[1], wcValues[2], wcValues[3],
								resolutionWidth, resolutionHeight);
					buffer = bufferFactory.getRasterBuf();
					if (buffer == null) {
						RasterToolsUtil
			...
			if ((selectedRois != null) && (!bufferFactory.isReadOnly())) {
				if (selectedRois.size() > 0) {
					...
					if (pValues != null) {
						...
						despX = (int) dsetCopy.worldToRaster(
								new Point2D.Double(wcValues[0], wcValues[1]))
								.getX();
						despY = (int) dsetCopy.worldToRaster(
								new Point2D.Double(wcValues[0], wcValues[1]))
								.getY();
					}
			...
			if (oneLayerPerBand) {
				...
				for (int i = 0; i < drawableBands.length; i++) {
					...
					if (params == null)
						...
						p = params;
					grw = GeoRasterWriter.getWriter(writerBufferServer,
							fileNames[i], 1, affineTransform,
				...
				if (viewName != null) {
					if (RasterToolsUtil.messageBoxYesOrNot("cargar_toc", this)) {
						try {
							for (int i = 0; i < drawableBands.length; i++) {
								FLayer lyr = RasterToolsUtil.loadLayer(
										viewName, fileNames[i], null);
								if (lyr != null && lyr instanceof FLyrRasterSE)
									((FLyrRasterSE) lyr).setRois(rasterSE
											.getRois());
							}
						} catch (RasterNotLoadException e) {
							RasterToolsUtil.messageBoxError("error_load_layer",
									this, e);
						}
					}
				}
				for (int i = 0; i < drawableBands.length; i++) {
					if (externalActions != null)
						externalActions.end(new Object[] {
								fileName + "_B" + drawableBands[i] + ".tif",
								new Long(milis[i]) });
				}
			} else {
				...
				if (params == null) {
					...
					params = GeoRasterWriter.getWriter(finalFileName)
							.getParams();
				} else
					finalFileName = fileName;
				grw = GeoRasterWriter.getWriter(writerBufferServer,
						finalFileName, buffer.getBandCount(), affineTransform,
						buffer.getWidth(), buffer.getHeight(),
			...
			RasterToolsUtil.messageBoxError("error_not_suported_extension",
					this, e);
	...
	/**
	 * Acciones para poner a NoData los pixels que estén fuera de las regiones
	 * de interés seleccionadas.
	 * 
	 * @param buffer
	 */
	private void drawOnlyROIs(IBuffer buffer, ArrayList rois, int despX,
			int despY) {
		for (int i = 0; i < buffer.getWidth(); i++) {
			for (int j = 0; j < buffer.getHeight(); j++) {
				...
				for (int k = 0; k < rois.size(); k++) {
					...
					// TODO: Hacer la comprobacion por coordenadas del mundo en
					// lugar de coordenadas pixel.
	...
	/**
	 * Returns true if there is a layer in the current project using the file
	 * named with fileName.
	 * 
	 * @param fileName
	 * @return
	...
	private boolean isUsingFile(String fileName) {
		...
		for (int i = 0; i < docs.size(); i++) {
			if (docs.get(i) instanceof ProjectViewBase) {
				FLayers lyrs = ((ProjectViewBase) docs.get(i)).getMapContext()
						.getLayers();
					...
					if (lyrs.getLayer(j) instanceof FLyrRasterSE) {
						...
						if (lyr.getDataSource() != null)
							if (lyr.getDataSource().getDataset(0)[0] != null)
								if (lyr.getDataSource().getDataset(0)[0]
										.getFName().equals(fileName))
									return true;
					}
				}
			}
	...
	private void cutFinalize(String fileName, long milis) {
		...
		if (viewName != null) {
			if (RasterToolsUtil.messageBoxYesOrNot("cargar_toc", this)) {
				...
				try {
					FLayer lyr = RasterToolsUtil.loadLayer(viewName, fileName,
							null);
					if (lyr != null && lyr instanceof FLyrRasterSE)
	...
	public int getPercent() {
		if (buffer != null) {
			BufferInterpolation interpolation = ((RasterBuffer) buffer)
					.getLastInterpolation();

			if (interpolation != null)
				if ((interpolation.getPercent() > 0)
						&& (interpolation.getPercent() < 99))
					return interpolation.getPercent();
		}

		return (writerBufferServer != null) ? writerBufferServer.getPercent()
				: 0;
	}
