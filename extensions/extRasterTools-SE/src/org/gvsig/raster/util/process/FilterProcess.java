...
public class FilterProcess extends RasterProcess {
	 ...
	 */
	private DatasetColorInterpretation getColorIntepretation(IBuffer buffer,
			Grid grid) {
		...
		do {
			...
			if (grid.getFilterList().isActive("colortable")) {
				colorInterpretation = new DatasetColorInterpretation(
						new String[] { DatasetColorInterpretation.RED_BAND,
								DatasetColorInterpretation.GREEN_BAND,
								DatasetColorInterpretation.BLUE_BAND });
				break;
			}

			// Si el numero de bandas coincide asignamos la misma interpretacion
			// que tenia antes
			if ((rendering != null)
					&& (buffer.getBandCount() == rasterDataSource
							.getBandCount())) {
				colorInterpretation = rasterDataSource.getColorInterpretation();
				break;
			}

			String[] colorInterp = new String[rasterDataSource
					.getColorInterpretation().getValues().length];

			for (int i = 0; i < rasterDataSource.getColorInterpretation()
					.getValues().length; i++) {
				if (rasterDataSource.getColorInterpretation().getValues()[i]
						.equals(DatasetColorInterpretation.UNDEF_BAND)) {
					colorInterp[i] = DatasetColorInterpretation.GRAY_BAND;
					continue;
				}
				colorInterp[i] = rasterDataSource.getColorInterpretation()
						.getValues()[i];
			}
	...
	public void process() throws InterruptedException {
		...
		try {
			...
			if (lyr != null
					&& !lyr.isActionEnabled(IRasterLayerActions.REMOTE_ACTIONS))
			...
			if (rendering != null) {
				...
				if ((renderBands.length == 3)
						&& (renderBands[0] == renderBands[1])
			...
			DatasetColorInterpretation colorInterpretation = null;
			if ((rendering != null && rendering.existColorTable())
					|| buffer.getBandCount() > 1)
				colorInterpretation = new DatasetColorInterpretation(
						new String[] { DatasetColorInterpretation.RED_BAND,
								DatasetColorInterpretation.GREEN_BAND,
								DatasetColorInterpretation.BLUE_BAND });
				...
				for (int i = 0; i < renderBands.length; i++) {
					...
					case 0:
						ci[i] = DatasetColorInterpretation.RED_BAND;
						break;
					case 1:
						ci[i] = DatasetColorInterpretation.GREEN_BAND;
						break;
					case 2:
						ci[i] = DatasetColorInterpretation.BLUE_BAND;
						break;
					default:
						ci[i] = DatasetColorInterpretation.UNDEF_BAND;
					}
				}
				colorInterpretation = new DatasetColorInterpretation(ci);
			} else
				colorInterpretation = new DatasetColorInterpretation(
						new String[] { DatasetColorInterpretation.GRAY_BAND });

			// Si la imagen original tenía una banda de transparencia se
			// asignará esta. Si los filtros generados
			...
			if (rasterDataSource.getTransparencyFilesStatus() != null
					&& rasterDataSource.getTransparencyFilesStatus()
							.getAlphaBandNumber() >= 0) {
				bufferFactory.setDrawableBands(new int[] { rasterDataSource
						.getTransparencyFilesStatus().getAlphaBandNumber() });
				bufferFactory.setAreaOfInterest();
				IBuffer alpha = bufferFactory.getRasterBuf();
				if (grid.getFilterList().getAlphaBand() != null)
					alpha = Transparency.merge(alpha, grid.getFilterList()
							.getAlphaBand());
				writerBufferServer.setAlphaBuffer(alpha);
				// Asignamos la interpretación de color de la banda alpha
				DatasetColorInterpretation alphaI = new DatasetColorInterpretation(
						new String[] { DatasetColorInterpretation.ALPHA_BAND });
				colorInterpretation.addColorInterpretation(alphaI);
				nbands++;
			} else if (grid.getFilterList().getAlphaBand() != null) {
				writerBufferServer.setAlphaBuffer(grid.getFilterList()
						.getAlphaBand());
				// Asignamos la interpretación de color de la banda alpha
				DatasetColorInterpretation alphaI = new DatasetColorInterpretation(
						new String[] { DatasetColorInterpretation.ALPHA_BAND });
			...
			geoRasterWriter = GeoRasterWriter.getWriter(writerBufferServer,
					filename, nbands, rasterDataSource.getAffineTransform(0),
					buffer.getWidth(), buffer.getHeight(),
					buffer.getDataType(), GeoRasterWriter.getWriter(filename)
							.getParams(), null);

			if (rendering == null)
				colorInterpretation = getColorIntepretation(buffer, grid);

			geoRasterWriter.setColorBandsInterpretation(colorInterpretation
					.getValues());
			...
			if (Configuration.getValue("nodata_transparency_enabled",
					Boolean.FALSE).booleanValue()) {
				try {
					RasterDataset.saveObjectToRmfFile(filename, NoData.class,
							new NoData(rasterDataSource.getNoDataValue(),
									rasterDataSource.isNoDataEnabled() ? 2 : 0,
									rasterDataSource.getDataType()[0]));
	...
	/**
	 * Sustituye la lista de filtros de filterList por la que le pasamos en
	 * forma de ArrayList
	 * 
	 * @param filterList
	 ...
	 */
	public static void addSelectedFilters(RasterFilterList filterList,
			ArrayList listFilterUsed) throws FilterTypeException {
		filterList.clear();
		...
		for (int i = 0; i < listFilterUsed.size(); i++) {
			...
			IRasterFilterListManager filterManager = stackManager
					.getManagerByFilterClass(aux.getFilterClass());
