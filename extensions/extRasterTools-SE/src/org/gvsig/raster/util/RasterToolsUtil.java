...
public class RasterToolsUtil {
	...
	/**
	 * Obtiene la lista de capas del TOC y devuelve las raster. Si hay capas
	 * agrupadas lo tiene en cuenta y mira dentro de la agrupación.
	 * 
	 * @param srcLyrs
	 *            FLayers de la vista
	 * @param destLyrs
	 *            Lista de capas
	 * @return ArrayList con la lista de capas raster agrupadas o no. El orden
	 *         en que aparecen en la lista es de abajo a arriba las que aparecen
	 *         en el TOC.
	 */
	public static ArrayList getRasterLayerList(FLayers srcLyrs,
			ArrayList destLyrs) {
		if (destLyrs == null)
	...
	/**
	 * Obtiene el nombre de la vista donde está cargada la capa que se pasa por
	 * parámetro
	 * 
	 * @param layer
	 *            Capa cargada en una vista
	...
	/**
	 * Devuelve la traducción de un texto. En caso de no existir la traducción
	 * al idioma seleccionado devolverá la cadena de entrada.
	 * 
	 * @param parent
	 *            Ventana padre que contiene el objeto con la traducción
	 * @param text
	 *            Texto a traducir
	 * @return Texto traducido o cadena de entrada en caso de no encontrar una
	 *         traducción
	 */
	...
	/**
	 * Selecciona los controles del panel de propiedades a partir de los
	 * parámtros obtenidos del driver. Este método realiza una transformación
	 * entre Params obtenido del driver de escritura y los parámetros del panel
	 * de propiedades.
	 * 
	 * @param panel
	 ...
	 * @param notTakeIntoAccount
	 *            Nombre de parámetros que no hay que tener en cuenta. Si es
	 *            null se tienen en cuenta todos.
	 */
	public static void loadPropertiesFromWriterParams(
			PropertiesComponent pComp, Params params,
		...
		for (int i = 0; i < params.getNumParams(); i++) {
			...
			// Miramos si el parámetro coincide con alguno en la lista de
			// parametros que no hay que
			...
			case Params.CHOICE:
				...
				types = new Object[] {
						new Integer(PropertiesComponent.TYPE_COMBO), list };
				pComp.addValue(name, key, new Integer(selectedValue), types);
				break;
			case Params.SLIDER:
				types = new Object[] {
						new Integer(PropertiesComponent.TYPE_SLIDER),
						new Integer(p.list[0]), new Integer(p.list[1]) };
				pComp.addValue(name, key, p.defaultValue, types);
				break;
			default:
				pComp.addValue(getText(null, params.getParam(i).id),
						params.getParam(i).id, params.getParam(i).defaultValue,
						null);
				break;
			}
		}
	 ...
	 */
	public static void loadWriterParamsFromPropertiesPanel(
			PropertiesComponent pComp, Params params) {
			...
			for (int iValue = 0; iValue < values.size(); iValue++) {
				...
				if (p.id.compareTo(prop.getKey()) == 0) {
					...
					case Params.CHECK:
						...
						break;
					case Params.CHOICE:
						p.defaultValue = ((Integer) prop.getNewValue());// p.list[((Integer)
																		// prop.getNewValue()).intValue()];
						break;
					case Params.SLIDER:
						try {
	 ...
	 * 
	 * @param path
	 *            Ruta a comprobar los permisosv
	 * @param pluginObject
	 *            si es distinto de null se obtiene un mensaje de advertencia y
	 *            sirve como parámetro para getText de la traducción. Si es null
	 *            no se mostrará ventana de advertencia
			...
			if (pluginObject != null)
				JOptionPane
						.showMessageDialog((Component) PluginServices
								.getMainFrame(), PluginServices.getText(
								pluginObject, "error_escritura"));
			return false;
		}
	}
	 ...
	 * @param parentWindow
	 *            Ventana desde la que se lanza el dialogo
	 * @return El botón seleccionado por el usuario. true si ha seleccionado
	 *         "si" y false si ha seleccionado "no"
	 */
	public static boolean messageBoxYesOrNot(String msg, Object parentWindow) {
		...
		int n = JOptionPane.showOptionDialog(
				...
				"<html>"
						+ PluginServices.getText(parentWindow, msg).replaceAll(
								"\n", "<br>") + "</html>",
	 ...
	 * 
	 * @param msg
	 *            Mensaje a mostrar en el dialogo. Identificador de la cadena a
	 *            traducir
	 * @param parentWindow
	 *            Ventana desde la que se lanza el dialogo
	 */
	 ...
	 * @param parent
	 *            Objeto que hizo disparar el mensaje
	 * @param exception
	 *            Excepcion que ha sido recogida
	 */
	public static void debug(String msg, Object parent, Exception exception) {
		if (parent != null)
			Logger.getLogger(parent.getClass().getName()).debug(
					PluginServices.getText(parent, msg), exception);
	}

	/**
	 * Muestra un dialogo de error con un texto y un botón de aceptar. El error
	 * es registrado en el log de gvSIG con la excepcion que se le pase por
	 * parametro
	 * 
	 * @param msg
	 *            Mensaje a mostrar en el dialogo.
	...
	/**
	 * Muestra un dialogo de error con un texto y un botón de aceptar. Se le
	 * pasa como último parámetros una lista de excepciones que serán guardadas
	 * en el log
	 * 
	 * @param msg
	 *            Mensaje a mostrar en el dialogo.
	 ...
	 * 
	 * @param viewName
	 *            Nombre de la vista donde ha de cargarse. Si vale null se
	 *            cargará en la primera vista que encuentre que esté activa. Si
	 *            no hay ninguna saltará una excepción.
	 * @param fileName
	 *            Nombre del fichero a cargar. No debe ser nulo nunca.
	 * @param layerName
	 ...
	 *             por algún motivo.
	 */
	public static FLayer loadLayer(String viewName, String fileName,
			String layerName) throws RasterNotLoadException {
		if (fileName == null)
			return null;
		...
		try {
			...
			if (viewName != null) {
				...
				for (int i = 0; i < allViews.length; i++) {
					if (allViews[i] instanceof BaseView
							&& ((BaseView) allViews[i]) == activeWindow) // En
																			// la
																			// primera
																			// vista
																			// activa
				...
				if (theView == null) {
					for (int i = 0; i < allViews.length; i++) {
						if (allViews[i] instanceof BaseView) // En la primera
																// vista
							theView = (BaseView) allViews[i];
					}
				}
			...
			throw new RasterNotLoadException(
					"No se puede hacer un casting de esa IWindow a View.");
		}
		...
		try {
			if (layerName == null) {
				...
				lyr = FLyrRasterSE.createLayer(fileName.substring(
						...
						new File(fileName), theView.getMapControl().getCrs());
			} else {
				lyr = FLyrRasterSE.createLayer(layerName, new File(fileName),
	 ...
	 *             por algún motivo.
	 */
	public static FLayer loadLayer(String viewName, FLayer lyr)
			throws RasterNotLoadException {
		if (lyr == null)
			return null;
	...
	/**
	 * Calculo de las coordenadas de una ventana IWindow para que quede centrada
	 * sobre el MainFrame. Estas coordenadas solo valen para un IWindow ya que
	 * andami mete las ventanas con coordenadas relativas a su ventanta
	 * principal.
	 * 
	 * @param widthWindow
	 *            Ancho de la ventana a añadir
	 * @param heightWindow
	...
	/**
	 * Returns layer name from raster file, checking user layer naming
	 * preferences
	 * 
	 * @param raster_file
	 * @return
