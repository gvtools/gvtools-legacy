...
/**
 * Esta clase almacena el estado de un raster en cuanto a las caracteristicas de
 * opacidad, bandas y filtros. Estas características pueden ser salvadas a un
 * xml y recuperadas por la capa a través de las funciones setXMLEntity y
 * getXMLEntity
 * 
...
public class StatusLayerRaster implements IStatusRaster {
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.StatusRasterInterface#setXMLEntity(com
	 * .iver.utiles.XMLEntity)
	 */
	public void setXMLEntity(XMLEntity xml, IRasterProperties layer)
		...
		for (int i = 0; i < xml.getPropertyCount(); i++) {
			if (xml.getPropertyName(i).startsWith("filter."))
				filters.add(xml.getPropertyName(i) + "="
						+ xml.getPropertyValue(i));
		}

		// Rangos de transparencia
		if (xml.contains("filter.transparency.active")
				&& xml.getBooleanProperty("filter.transparency.active")) {
			int i = 0;
			String value = null;
			while (true) {
				if (xml.contains("filter.transparency.transparencyRange" + i)) {
					value = xml
							.getStringProperty("filter.transparency.transparencyRange"
									+ i);
					int alpha = 0;
					if (value.indexOf("@") != 0) {
						try {
							alpha = Integer.parseInt(value.substring(
			...
			if (xml.contains("filter.transparency.bandnumber"))
				alphaBandNumber = xml
						.getIntProperty("filter.transparency.bandnumber");
		}

		if (xml.contains("raster.opacityLevel")) {
			transparency = xml.getIntProperty("raster.opacityLevel");
			// Esto soluciona un problema de compatibilidad entre branch v10 y
			// HEAD. Eliminar en futuras versiones
			if (nameClass != null
					&& nameClass
							.compareTo("com.iver.cit.gvsig.fmap.layers.StatusLayerRaster") == 0)
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.StatusRasterInterface#getXMLEntity(com
	 * .iver.utiles.XMLEntity)
	 */
	public void getXMLEntity(XMLEntity xml, boolean loadClass,
		...
		if (transp != null) {
			...
			if (transp.getTransparencyRange().size() > 0) {
				for (int i = 0; i < transp.getTransparencyRange().size(); i++)
					xml.putProperty(
							"filter.transparency.transparencyRange" + i,
							""
									+ ((TransparencyRange) transp
			...
			if (transp.getAlphaBandNumber() != -1)
				xml.putProperty("filter.transparency.bandnumber",
						transp.getAlphaBandNumber());
		}
		...
		// proyecto que tiene WCS y no se abre la vista de este) entonces hay
		// que leer los filtros
		// que van a salvarse a disco directamente de la variable filters que es
		...
		if (layer.getRenderFilterList() != null) {
			filterListManager = new RasterFilterListManager(
					layer.getRenderFilterList());
		...
		for (int i = 0; i < l.size(); i++)
			xml.putProperty(getElem((String) l.get(i)),
					getValue((String) l.get(i)));
	}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.StatusRasterInterface#applyStatus(com.
	 * iver.cit.gvsig.fmap.layers.RasterFileAdapter)
	 */
	public void applyStatus(FLyrRasterSE layer)
			throws NotSupportedExtensionException, RasterDriverException,
			FilterTypeException {

		// Eliminamos el fichero inicial y cargamos las bandas si hay para que
		// se carguen
	...
	/**
	 * Método para mantener la compatibilidad con la v10 del realce. El realce
	 * de la v10 no aporta suficiente información por lo que se añade un filtro
	 * de realce generico v1.9
	 * 
	 ...
	 */
	public static void enhancedCompV10(ArrayList<String> filterArguments,
			FLyrRasterSE layer, RasterFilterListManager filterListManager)
			throws FilterTypeException {
		...
		for (int i = 0; i < list.lenght(); i++) {
			...
			if (f instanceof TailTrimFilter
					|| f instanceof LinearEnhancementFilter) {
		...
		for (int i = 0; i < filterArguments.size(); i++) {
			if (((String) filterArguments.get(i))
					.startsWith("filter.enhanced.active=true")) {
				EnhancementStretchListManager elm = new EnhancementStretchListManager(
						filterListManager);
				try {
					elm.addEnhancedStretchFilter(LinearStretchParams
							.createStandardParam(layer.getRenderBands(), 0.0,
									layer.getDataSource().getStatistics(),
									false), layer.getDataSource()
							.getStatistics(), layer.getRender()
							.getRenderBands(), false);
	...
	/**
	 * Al cargar filtros desde un proyecto salvado con la V10 los filtros pueden
	 * estar desordenados. Colocamos los más importantes en su posición
	 * adecuada.
	 * 
	 * @throws FilterTypeException
	 * 
	...
	private void sortFilters(RasterFilterList list) throws FilterTypeException {
		...
		for (int i = 0; i < list.lenght(); i++) {
			RasterFilter f = list.get(i);
			if (f instanceof LinearEnhancementFilter
					&& list.get(0) instanceof TailTrimFilter) {
				list.remove(f.getName());
				list.add(f, 1);
