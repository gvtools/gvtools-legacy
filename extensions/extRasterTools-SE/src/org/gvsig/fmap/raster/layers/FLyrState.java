...
/**
 * Estados de una capa. Cada capa tiene uno o varios estados activos en cada
 * instante. Hay estados no compatibles entre si, es decir, no pueden estar
 * activos a la vez en cada instante. Por ejemplo, una capa no puede estar
 * cerrada y renderizando ya que sus datos no están accesibles para esa
 * operación. Los estados de una capa sir
 * 
 * @version 13/09/2007
...
public class FLyrState implements ILayerState {
	...
	public void enableStopped() {
		if (open == true || awake == true || closed == true) {
			lastBeforeStop = (open == true) ? OPEN : ((closed == true) ? CLOSED
					: ((awake == true) ? AWAKE : UNDEFINED));
