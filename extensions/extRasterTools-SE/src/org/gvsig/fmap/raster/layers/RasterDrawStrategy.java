...
/**
 * Aplica estrategias de dibujado para las capas raster. Hace que las capas que
 * están ocultas por otras no sean dibujadas. Esta estrategia tiene que ser
 * aplicada a capas raster de FMap por falta de eficiencia en el dibujado. Esta
 * clase es totalmente dependiente de FMap y puede ser eliminada en caso de que
 * este no exita. 28/11/2007
 * 
...
public class RasterDrawStrategy {
	...
	/**
	 * Estructura de datos para almacenar la asociación entre el extent de una
	 * capa y su información de transparencia. Esta es necesaria para la
	 * comprobación de si la capa con la que se está intersectando tiene
	 * transparencia o no la tiene. Con esto se puede decidir si dibujar la capa
	 * actual o no. 16/06/2008
	 * 
	 * @author Nacho Brodin nachobrodin@gmail.com
	 */
	...
	/**
	 * Al constructor le pasamos el contexto de dibujado para la toma de
	 * decisión.
	 * 
	 * @param mapContext
	 *            Context
	...
	/**
	 * <P>
	 * Estrategia de dibujado para las capas raster que hace que si una capa
	 * está oculta completamente por otra que no tiene transparencias entonces
	 * esta no será dibujada.
	 * </P>
	 * <P>
	 * La estrategia la calcula solo la primera capa raster que aparezca. El
	 * resto de las capas la preguntaran a esta.
	 * </P>
	 * <P>
	 * <code>Estrategia:</code> Analizamos la lista de capas desde la primera
	 * que se dibuja en el TOC hasta la última
	 * <UL>
	 * <LI>Si la capa tiene rotación se dibuja siempre.</LI>
	 * <LI>Si la capa no está activa en el TOC no se dibuja nunca</LI>
	 * <LI>Si la capa no es transparente y no hay extents en la lista se dibuja
	 * y se añade el extent a la lista</LI>
	 * <LI>Si el extent de la capa intersecta con uno de la lista</LI>
	 * </UL>
	 * Comprobamos si está detrás de un extent de la lista. Si es así no se
	 * dibuja Si no está está detrás de un extent comprobamos si tiene
	 * transparencias Si no tiene transparencias añadimos el extent a la lista
	 * </P>
	 * 
	 * @param lyrs
	 * @throws ReadDriverException
	 * @throws ExpansionFileReadException
	 */
	public void stackStrategy() throws ExpansionFileReadException,
			ReadDriverException {
		...
		// Solo la primera capa calcula la estrategia. Las demás se la preguntan
		// a esta.

		// Calculamos cual es la primera capa raster de FLayers y contamos
		// cuantas capas raster hay
		...
		// Si no es la primera capa se obtiene la estrategia que calculó la
		// primera capa
		if (lyrRaster != listLayers.get(posFirstRasterLayer)) {
			if (!(listLayers.get(posFirstRasterLayer) instanceof FLyrRasterSE))
				return;
			mapContextStrategy.put(mapContext, ((FLyrRasterSE) listLayers
					.get(posFirstRasterLayer)).getRasterStrategy());
			return;
		}
		...
		// La lista de capas en FLayers está en orden inverso a lo que aparece
		// en el TOC, es decir, la última capa
		// del TOC es la primera de la lista de FLayers y la primera del TOC es
		...
		for (int i = (listLayers.size() - 1); i >= 0; i--) {
			...
			// Si no es la primera y no intersecta con ningún extent. Metemos el
			// extent y dibujamos
			LayerIntersection li = areIntersecting(extentList, rLyr);
			if ((i == (listLayers.size() - 1)) || li == null) {
				boolean colorTableAlpha = false;
				if (rLyr.getRenderFilterList() != null) {
					RasterFilter rf = rLyr.getRenderFilterList().get(
							"colortable");
					if (rf != null && rf instanceof ColorTableFilter)
						colorTableAlpha = ((ColorTableFilter) rf)
								.getColorTable().hasAlpha();
				}
				extentList.add(new LayerIntersection(
						rLyr.getFullRasterExtent(), (rLyr.existsAlphaBand()
								|| rLyr.isTransparent() || colorTableAlpha)));
				layersToPaint.put(rLyr, new Boolean(true));
				continue;
			}
	...
	/**
	 * Obtiene la lista de capas del TOC. Si hay capas agrupadas lo tiene en
	 * cuenta y mira dentro de la agrupación.
	 * 
	 * @param srcLyrs
	 * @param destLyrs
	 * @return
	...
	/**
	 * Comprueba si la capa que se pasa por parámetro está oculta por alguno de
	 * los extent de la lista
	 * 
	 * @param extentList
	 ...
	 * @return
	 */
	private LayerIntersection areIntersecting(ArrayList extentList,
			FLyrRasterSE lyr) {
	...
	/**
	 * Obtiene un TreeMap con la lista de capas que se dibujan. Si se al TreeMap
	 * se le pasa como parámetro una capa raster y devuelve null es que no se
	 * dibuja.
	 * 
