...
public class FLyrRasterSE extends FLyrDefault implements IRasterProperties,
		...
		IRasterLayerActions, ILayerState, VisualPropertyListener {
	...
	/**
	 * Tipo de valor no data asociado a la capa. Sirve para diferenciar los
	 * estados seleccionados por el usuario. Siendo estos 'Sin Valor NoData',
	 * 'NoData de Capa'(Por defecto) y 'Personalizado'
	 */
	...
	static {
		...
		// TODO: Problema de dependencia entre appgvSIG y libFMap. La resolución
		// de errores en la ruta de las capas debería ser un mecanismo general.
		FLyrRasterSE.addSolveErrorForLayer(
				NotSupportedExtensionException.class, new FileNotFoundSolve());
	}
		...
		if (params != null && getName() != null) {
			try {
				...
				RasterToolsUtil.messageBoxError(
						"Fallo el estado de open. Closed=" + isClosed()
								+ " Active=" + isOpen(), this, e);
			}
		}
	}
			...
			if (params instanceof String[][]) {
				...
				for (int i = 0; i < files.length; i++)
					for (int j = 0; j < files[i].length; j++)
						dt[i][j] = MultiRasterDataset.open(getCrs(),
								files[i][j]);
	...
	/**
	 * Acciones de inicialización después de que la fuente de datos de la capa
	 * está asignada. El tipo de fuente de datos es variable puede ser
	 * MultiRasterDataset, CompositeDataset u otras que existan e implementen
	 * IRasterDatasource.
	 */
	public void init() throws LoadLayerException {
		...
		try {
			...
			RasterToolsUtil.messageBoxError("Fallo el estado de open. Closed="
					+ isClosed() + " Awake=" + isAwake(), this, e);
		}
	}
	...
	public CoordinateReferenceSystem readProjection() {
		try {
			...
			RasterToolsUtil
					.messageBoxError(
							"Problemas accediendo a getWktProjection. Driver no inicializado",
							this, e);
		}
		return null;
	}
	...
	public void applyNoData() {
		Boolean noDataEnabled = configuration.getValueBoolean(
				"nodata_transparency_enabled", Boolean.FALSE);
		if (noDataEnabled.booleanValue() && getDataSource().isNoDataEnabled()) {
			...
			getDataSource().getTransparencyFilesStatus().setNoData(
					noDataValue.doubleValue());
		} else {
			if (getDataSource() != null
					&& getDataSource().getTransparencyFilesStatus() != null)
				getDataSource().getTransparencyFilesStatus()
						.activeNoData(false);
	...
	private void initFilters() {
		...
		try {
			// Si en la carga del proyecto se cargó una tabla de color asignamos
			// esta
			if (loadedFromProject != null) {
				...
				if (this.getDataSource().getColorTables()[0] != null) {
					GridPalette p = new GridPalette(getDataSource()
							.getColorTables()[0]);
					setLastLegend(p);
					ColorTableListManager ctm = new ColorTableListManager(
							filterManager);
			...
			// Ha habido un error en la asignación de filtros por los que no se
			// añade ninguno.
			RasterToolsUtil.debug(
					"Error añadiendo filtros en la inicialización de capa "
							+ this.getName() + " Datatype="
							+ this.getDataType(), null, e);
		}
	}

	/**
	 * Mira la configuracion para saber si debe cargar un realce o una tabla de
	 * color por defecto
	 * 
	 * @param filterManager
	 * @throws FilterTypeException
	 */
	private void loadEnhancedOrColorTable(RasterFilterListManager filterManager)
			throws FilterTypeException {
		String colorTableName = configuration.getValueString(
				"loadlayer_usecolortable", (String) null);
			...
			try {
				...
				if (getBandCount() == 1) {
					...
					for (int i = 0; i < fileList.size(); i++) {
						...
						String paletteName = ColorTableLibraryPersistence
								.loadPalette(palettesPath,
										(String) fileList.get(i), paletteItems);
						if (paletteName.equals(colorTableName)) {
							...
							colorTable.createPaletteFromColorItems(
									paletteItems, true);
							colorTable.setInterpolated(true);

							colorTable.createColorTableInRange(
									stats.getMinimun(), stats.getMaximun(),
									true);
		...
		/*
		 * EnhancementListManager elm = new
		 * EnhancementListManager(filterManager); elm.addEnhancedFilter(false,
		 * stats, 0.0, getRender().getRenderBands());
		 */
		...
		try {
			elm.addEnhancedStretchFilter(LinearStretchParams
					.createStandardParam(getRenderBands(), 0.0, stats, true),
					stats, getRender().getRenderBands(), false);
		} catch (FileNotOpenException e) {
			// No podemos aplicar el filtro
		} catch (RasterDriverException e) {
			// No podemos aplicar el filtro
	...
	public void draw(BufferedImage image, Graphics2D g, ViewPort vp,
		...
		try {
			...
			if (tStr != null && tStr.get(this) != null
			...
			if (isWithinScale(scale)) {
				if (status != null && firstLoad) {
					if (mustTileDraw) {
						...
						Rectangle r = new Rectangle((int) p.getX(),
								(int) p.getY(), vp.getImageWidth(),
								vp.getImageHeight());
						Tiling tiles = new Tiling(maxTileDrawWidth,
								maxTileDrawHeight, r);
						tiles.setAffineTransform((AffineTransform) vp
								.getAffineTransform().clone());
						for (int tileNr = 0; tileNr < tiles.getNumTiles(); tileNr++) {
							// drawing part
							try {
								ViewPort vport = tiles.getTileViewPort(vp,
										tileNr);
								// g.setClip(tiles.getClip(tileNr).x,
								// tiles.getClip(tileNr).y,
								// tiles.getClip(tileNr).width - 5,
								// tiles.getClip(tileNr).height);
								draw(image, g, vport, cancel);
							} catch (RasterDriverException e) {
								throw new ReadDriverException("", e);
							} catch (InvalidSetViewException e) {
								throw new ReadDriverException(
										"Error al asignar la vista en el draw.",
										e);
							} catch (InterruptedException e) {
								System.out
										.println("Se ha cancelado el pintado");
								// throw new
								// ReadDriverException("Dibujado interrumpido.",
								// e);
							} catch (NoninvertibleTransformException e) {
								throw new ReadDriverException(
										"Error en la transformación", e);
							}
						}
					} else {
						try {
							...
							throw new ReadDriverException(
									"Error al asignar la vista en el draw.", e);
						} catch (InterruptedException e) {
							System.out.println("Se ha cancelado el pintado");
							// throw new
							// ReadDriverException("Dibujado interrumpido.", e);
						}
					}
					try {
						...
						throw new ReadDriverException(
								"Error setting filters from a project.", e);
					} catch (RasterDriverException e) {
						throw new ReadDriverException(
								"Error reading file from a project.", e);
					} catch (FilterTypeException e) {
						throw new ReadDriverException("Error adding filters.",
								e);
					}
					firstLoad = false;
				}
	...
	public Rectangle2D getFullExtent() {
		// TODO:DEPURACION Comentamos !isOpen porque getFullExtent de FLayers da
		// una excepción ya que siempre espera
	...
	public int[] getPixel(int pxx, int pxy) {
		...
		if (!isOpen() || (image == null))
			return argb;
		if (pxx >= 0 && pxx < image.getWidth() && pxy >= 0
				&& pxy < image.getHeight()) {
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.InfoByPoint#queryByPoint
	 * (java.awt.Point)
	 */
	public String queryByPoint(Point p) {
		...
		data += "    Point_WC=\"" + MathUtils.format(posXWC, 3) + " , "
				+ MathUtils.format(posYWC, 3) + "\"\n";
		data += "    RGB=\"" + r + ", " + g + ", " + b + "\"\n";
		double[] cmyk = conv.RGBtoCMYK(r & 0xff, g & 0xff, b & 0xff, 1D);
		data += "    CMYK=\"" + MathUtils.format(cmyk[0], 4) + ", "
				+ MathUtils.format(cmyk[1], 4) + ", "
				+ MathUtils.format(cmyk[2], 4) + ","
				+ MathUtils.format(cmyk[3], 4) + "\"\n";
		double[] hsl = conv.RGBtoHSL(r & 0xff, g & 0xff, b & 0xff);
		hsl[0] = (int) (255.0 * hsl[0] / 360.0 + 0.5);
		hsl[2] = (int) (hsl[2] * 255. + 0.5);
		hsl[1] = (int) (hsl[1] * 255. + 0.5);
		data += "    HSL=\"" + MathUtils.format(hsl[0], 4) + ", "
				+ MathUtils.format(hsl[1], 4) + ", "
				+ MathUtils.format(hsl[2], 4) + "\"\n";
		data += "  />\n";
	...
	/**
	 * Transforma un punto real a coordenadas pixel indicando la banda que es
	 * usada para la transformación. Hay que tener en cuenta que es posible que
	 * todas las transformaciones no sean iguales en todas la bandas porque
	 * puede haber bandas de distinta resolución.
	 * 
	 * @param numberBand
	 * @param pReal
	 * @return
	 * @throws ReadDriverException
	 */
	private Point2D transformPoint(int numberBand, Point2D pReal)
			throws ReadDriverException {
		AffineTransform at = this.getDataSource()
				.getAffineTransform(numberBand);
		...
		try {
			...
			throw new ReadDriverException(
					"Error en la transformación del punto", e);
		}
	}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.InfoByPoint#getInfo(java
	 * .awt.Point, double, com.iver.utiles.swing.threads.Cancellable)
	 */
	public XMLItem[] getInfo(Point p, double tolerance, Cancellable cancel)
		...
		Point2D px = new Point2D.Double();
		if (pReal.getX() > this.getMinX() && pReal.getX() < this.getMaxX()
		...
		data += "    World_Point=\"" + MathUtils.format(pReal.getX(), 3)
				+ " , " + MathUtils.format(pReal.getY(), 3) + "\"\n";
		if (px == null)
			data += "    Pixel_Point=\"Out\"\n";
		else
			data += "    Pixel_Point=\"" + (int) px.getX() + " , "
					+ (int) px.getY() + "\"\n";
		if (getDataType()[0] != IBuffer.TYPE_BYTE)
			data += "    RGB=\"" + rgb[1] + "  " + rgb[2] + "  " + rgb[3]
					+ "\"\n";
		double[] cmyk = conv.RGBtoCMYK(rgb[1] & 0xff, rgb[2] & 0xff,
				rgb[3] & 0xff, 1D);
		data += "    CMYK=\"" + MathUtils.format(cmyk[0], 4) + ", "
				+ MathUtils.format(cmyk[1], 4) + ", "
				+ MathUtils.format(cmyk[2], 4) + ","
				+ MathUtils.format(cmyk[3], 4) + "\"\n";
		double[] hsl = conv.RGBtoHSL(rgb[1] & 0xff, rgb[2] & 0xff,
				rgb[3] & 0xff);
		hsl[0] = (int) (255.0 * hsl[0] / 360.0 + 0.5);
		hsl[2] = (int) (hsl[2] * 255. + 0.5);
		hsl[1] = (int) (hsl[1] * 255. + 0.5);
		...
		try {
			if (px != null) {
				if (getDataType()[0] >= 0 && getDataType()[0] <= 3) {
					for (int i = 0; i < getBandCount(); i++) {
						if (getDataSource().isInside(pReal)) {
							...
							int val = ((Integer) getDataSource().getData(
									(int) pxAux.getX(), (int) pxAux.getY(), i))
									.intValue();
							if (getDataType()[0] == IBuffer.TYPE_BYTE)
								data += (val & 0x000000ff) + "  ";
							else
				...
				if (getDataType()[0] == 4) {
					for (int i = 0; i < getBandCount(); i++) {
						if (getDataSource().isInside(pReal)) {
							Point2D pxAux = transformPoint(i, pReal);
							data += ((Float) getDataSource().getData(
									(int) pxAux.getX(), (int) pxAux.getY(), i))
									.floatValue() + "  ";
						}
					}
				...
				if (getDataType()[0] == 5) {
					for (int i = 0; i < getBandCount(); i++) {
						if (getDataSource().isInside(pReal)) {
							Point2D pxAux = transformPoint(i, pReal);
							data += ((Double) getDataSource().getData(
									(int) pxAux.getX(), (int) pxAux.getY(), i))
									.doubleValue() + "  ";
						}
					}
			...
			throw new ReadDriverException("Fichero no abierto en el dataset",
					ex);
		} catch (InterruptedException e) {
		}
		data += "\"\n";
	...
	/**
	 * Filters a string for being suitable as XML Tag, erasing all not
	...
	public ArrayList getAttributes() {
		...
		if (!isOpen())
			return attr;
		Object[][] a = { { "Filename", dataset.getDataset(0)[0].getFName() },
	 ...
	 * where the project is loaded, but it doesn't mean that the layer can not
	 * be loaded.
	 * 
	 * @return
	 * @throws XMLException
	...
	public XMLEntity getXMLEntityWithoutChecks() throws XMLException {
		...
		if (getFile() != null) {
			...
			xml.putProperty("file",
					pathGenerator.getPath(getFile().getAbsolutePath()));
		}
		xml.putProperty("driverName", "gvSIG Raster Driver");

		// Si no hay ningín Status aplicamos el StatusLayerRaster que se usa por
		// defecto
		if (status == null)
	...
	public void setXMLEntity(XMLEntity xml) throws XMLException {
		for (int i = 0; i < xml.getPropertyCount(); i++) {
			...
			if (key.startsWith("raster.file")) {
				if (xml.getPropertyValue(i).startsWith(
						RasterLibrary.getTemporalPath()))
		...
		try {
			String path = pathGenerator.getAbsolutePath((String) xml
					.getStringProperty("file"));
			if (path != null) {
				params = new File(pathGenerator.getAbsolutePath(xml
						.getStringProperty("file")));
			} else {
				...
				if (claseStr != null && !claseStr.equals("")) {
					try {
						...
						if (status != null) {
							...
							ArrayList color = (ArrayList) filterArguments
									.clone();
							loadedFromProject = ColorTableListManager
									.createColorTableFromArray(color);
						}
	...
	public void print(Graphics2D g, ViewPort viewPort, Cancellable cancel,
			double scale, PrintRequestAttributeSet propeties)
			throws ReadDriverException {
		...
		if (!mustTilePrint) {
			...
			Tiling tiles = new Tiling(maxTilePrintWidth, maxTilePrintHeight,
					g.getClipBounds());
			tiles.setAffineTransform((AffineTransform) viewPort
					.getAffineTransform().clone());

			// Si es la primera lectura salvamos los valores de máximo y mínimo
			// para la aplicación
			// de realce si la imagen es de 16 bits.

			// RasterStats stats = getSource().getFilterStack().getStats();
			// if(stats != null)
			// stats.history.add(stats.new History(getName(),
			// stats.minBandValue, stats.maxBandValue, stats.secondMinBandValue,
			// stats.secondMaxBandValue));
			...
			/*
			 * if(stats != null){
			 * getSource().getFilterStack().getStats().history.clear(); stats =
			 * getSource().getFilterStack().getStats(); }
			 */
		}
	}
	...
	public void _print(Graphics2D g, ViewPort viewPort, Cancellable cancel,
		...
		AffineTransform mat = (AffineTransform) viewPort.getAffineTransform()
				.clone();
			...
			for (stepX = 0; stepX < numW + 1; stepX++) {
				...
				try {
					...
					Rectangle2D.Double rectCuadricula = new Rectangle2D.Double(
							dstPts[0], dstPts[1], dstPts[2] - dstPts[0],
							dstPts[5] - dstPts[3]);
		...
		if (this.getDataSource() != null) {
			...
			try {
				for (int i = 0; i < getDataSource().getDatasetCount(); i++) {
					getDataSource().saveObjectToRmf(i, RasterDataset.class,
							getDataSource().getDataset(i)[0]);
				}
		...
		if (this.isRemoveRasterFlag()) {
			...
			for (int i = 0; i < files.length; i++) {
				...
				if (dirTemp.compareTo(file.getParentFile()) == 0) {
					...
					// Borramos todos los ficheros que puedan tener relacion con
					// el fichero actual
	...
	/**
	 * Asigna el valor del flag que dice si destruimos la memoria del raster al
	 * eliminarlo del TOC o no.
	 * 
	 * @param removeRasterFlag
	 *            The removeRasterFlag to set.
	 */
	 ...
	 */
	public void addFile(String fileName) throws NotSupportedExtensionException,
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.shared.IRasterOperations#getColourInterpretation(int,
	 * int)
	 */
	public String getColorInterpretation(int band, int dataset) {
		if (this.dataset.getDataset(dataset)[0].getColorInterpretation().get(
				band) == null)
			return "Undefined";
		return this.dataset.getDataset(dataset)[0].getColorInterpretation()
				.get(band);
	}
	...
	/**
	 * Metodo para consultar si una capa puede ser un RGB. Suponemos que es un
	 * RGB si el tipo de datos es de tipo byte y su interpretacion de color
	 * tiene asignada los tres colores.
	 * 
	 * @return boolean
	 */
	...
	/**
	 * Obtiene el grid de la capa completa. Hay que tener cuidado porque cuando
	 * se hace esta petición se carga un buffer con todos los datos de la capa.
	 * Este buffer puede ser cacheado o no dependiendo del tamaño de esta.
	 * 
	 * @param interpolated
	 *            true si se solicita un grid interpolado y false si se solicita
	 *            sin interpolar.
	 * @return Grid.
	 * @throws InterruptedException
	 */
	public Grid getFullGrid(boolean interpolated) throws GridException,
	...
	/**
	 * Obtiene el grid de la capa completa. Esta llamada devuelve un buffer de
	 * solo lectura
	 * 
	 * @param interpolated
	 *            true si se solicita un grid interpolado y false si se solicita
	 ...
	 * @throws InterruptedException
	 */
	public Grid getReadOnlyFullGrid(boolean interpolated) throws GridException,
			InterruptedException {
	...
	public int getAlphaBandNumber() {
		if (getDataSource().getColorInterpretation() != null)
			return getDataSource().getColorInterpretation().getBand(
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.Classifiable#addLegendListener
	 * (com.iver.cit.gvsig.fmap.layers.LegendListener)
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.Classifiable#getShapeType
	 * ()
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iver.cit.gvsig.fmap.layers.layerOperations.Classifiable#
	 * removeLegendListener(com.iver.cit.gvsig.fmap.layers.LegendListener)
	 */
	...
	/**
	 * Metodo que obtiene si un punto cae dentro de los límites de la capa o
	 * fuera de ellos.
	 * 
	 * @param p
	 *            Punto a calcular
	...
	/**
	 * Recupera del raster la matriz de transformación que lo situa en cualquier
	 * parte de la vista
	 * 
	...
	/**
	 * Asigna al raster la matriz de transformación para situarlo en cualquier
	 * parte de la vista
	 * 
	 * @param transf
	 */
	...
	/**
	 * Asigna al raster la matriz de transformación para situarlo en cualquier
	 * parte de la vista. Esta versión no guarda en el historico.
	 * 
	 * @param transf
	 */
	 ...
	 * 
	 * @see
	 * org.gvsig.fmap.raster.layers.IRasterLayerActions#isActionEnabled(int)
	 */
	...
	public void setVisible(boolean visibility) {
		...
		/*
		 * Cuando se modifica la visibilidad de una capa raster se hace un
		 * updateDrawVersion de todas las capas raster de ese MapContext. Esto
		 * es porque la estrategia utilizada por RasterDrawStrategy hace que se
		 * cacheen en blanco las capas raster que están ocultas debajo de otras.
		 * Al hacer invisibles las de arriba la cache que estaba en blanco hace
		 * que no se pinte nada. Para evitar esto las marcamos todas como que
		 * han sido modificadas para que se vuelvan a leer.
		 */
		if (getMapContext() != null) {
			ArrayList listLayers = new ArrayList();
			listLayers = RasterDrawStrategy.getLayerList(getMapContext()
	...
	/**
	 * Consulta si tiene aplicada alguna transparencia en la última
	 * renderización o no.
	 * 
	 * @return true si se aplicó alguna transparencia en la última
	 *         renderización.
	 */
	...
	public void setTransparency(int trans) {
		...
		try {
			...
			// Solo asigna la transparencia a la clase padre y no a la
			// renderización
		}
	}
	 ...
	 * 
	 * @param rois
	 *            ArrayList de ROIs a asociar a la capa raster.
	 */
	...
	/**
	 * Si ya tiene una estrategia de dibujado de raster calculada la devuelve,
	 * sino devolverá null.
	 * 
	...
	/**
	 * Devuelve el tipo de valor de NoData asociado a la capa. Sirve para
	 * diferenciar los estados seleccionados por el usuario. Siendo estos '0:
	 * Sin Valor NoData', '1: NoData de Capa'(Por defecto) y '2: Personalizado'
	 */
	/**
	...
	public boolean isRenderingAsGray() {
		...
		if ((renderBands != null) && (renderBands.length == 3)
				&& (renderBands[0] >= 0) && (renderBands[0] == renderBands[1])
				&& (renderBands[1] == renderBands[2]))
			return true;
		return false;
	}
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.grid.render.VisualPropertyListener#actionValueChanged
	 * (org.gvsig.raster.grid.render.VisualPropertyEvent)
	 */
	...
	/**
	 * Ajusta las coordenadas especificadas en el parámetro al área máxima del
	 * raster en píxeles.
	 * 
	 * @param req
	 *            Punto a ajustar dentro del extener del raster
	 */
	public Point2D adjustWorldRequest(Point2D req) {
		Rectangle2D ext = null;

		ext = getFullExtent();
		req.setLocation(Math.max(ext.getMinX(), req.getX()),
				Math.max(ext.getMinY(), req.getY()));
		req.setLocation(Math.min(ext.getMaxX(), req.getX()),
				Math.min(ext.getMaxY(), req.getY()));
		return req;
	}
	...
	public FLayer cloneLayer() throws Exception {
		FLyrRasterSE newLayer = FLyrRasterSE.createLayer(this.getName(),
				params, this.getCrs());
		...
		newLayer.getRender().getFilterList()
				.setEnv(getRender().getFilterList().getEnv());
	...
	/**
	 * When a process is using information of this layer this variable will
