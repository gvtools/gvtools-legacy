 ...
 * aplicada. El resultado visual es la selección como un rectangulo común con el
 * rectangulo transformado superpuesto.
 * 
...
public class TransformedRectangleBehavior extends Behavior {
	...
	public void paintComponent(Graphics g) {
		...
		if (at != null) {
			...
			Point2D.Double lr = new Point2D.Double(r.x + r.width, r.y
					+ r.height);

			Point2D center = new Point2D.Double(((double) r.width) / 2.0,
					((double) r.height) / 2.0);
			AffineTransform T1 = new AffineTransform(1D, 0, 0, 1,
					-center.getX(), -center.getY());
			AffineTransform R1 = new AffineTransform(1D, at.getShearY()
					/ at.getScaleY(), at.getShearX() / at.getScaleX(), 1, 0, 0);
			AffineTransform T2 = new AffineTransform(1D, 0, 0, 1,
					center.getX(), center.getY());
			T2.concatenate(R1);
			T2.concatenate(T1);
			...
			try {
				...
				Point2D.Double ptMin = new Point2D.Double(Math.min(
						Math.min(ul.getX(), ur.getX()),
						Math.min(ll.getX(), lr.getX())), Math.min(
						Math.min(ul.getY(), ur.getY()),
						Math.min(ll.getY(), lr.getY())));
				Point2D.Double ptMax = new Point2D.Double(Math.max(
						Math.max(ul.getX(), ur.getX()),
						Math.max(ll.getX(), lr.getX())), Math.max(
						Math.max(ul.getY(), ur.getY()),
				...
				((Graphics2D) g).drawRect((int) ptMin.getX(),
						(int) ptMin.getY(), (int) w, (int) h);
