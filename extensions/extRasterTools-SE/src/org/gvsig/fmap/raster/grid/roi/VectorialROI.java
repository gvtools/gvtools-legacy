...
/**
 * Clase que representa una región de interes conformada por elementos
 * vectoriales.
 * 
...
public class VectorialROI extends ROI {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.raster.grid.roi.ROI#isInside(double, double, double,
	 * double)
	 */
	public boolean isInside(double x, double y, double w, double h) {
		if ((x + w) < extent.minX() || x > extent.maxX()
				|| (y - h) > extent.maxY() || y < extent.minY())
			return false;
	...
	public boolean isInGrid(int x, int y) {
		...
		if (newX >= 0 && newX <= getNX() && newY >= 0 && newY <= getNY()) {
			...
			double mapX = this.getGridExtent().getMin().getX() + newX
					* cellSize;
			double mapY = this.getGridExtent().getMax().getY() - newY
					* cellSize;
			for (int i = 0; i < geometries.size(); i++) {
				Rectangle2D celda = new Rectangle2D.Double(mapX, mapY
	...
	public void addGeometry(IGeometry geometry) {
		...
		if (extent == null)
			extent = new Extent(geometryBounds.getMinX(),
					geometryBounds.getMaxY(), geometryBounds.getMaxX(),
					geometryBounds.getMinY());
		else
			extent = new Extent(Math.min(geometryBounds.getMinX(),
					extent.minX()), Math.max(geometryBounds.getMaxY(),
					extent.maxY()), Math.max(geometryBounds.getMaxX(),
					extent.maxX()), Math.min(geometryBounds.getMinY(),
					extent.minY()));

		double minX = geometryBounds.getMinX()
				- Math.abs(geometryBounds.getMinX()
						- getGrid().getGridExtent().getMin().getX()) % cellSize;
		double minY = (geometryBounds.getMinY())
				- (Math.abs(geometryBounds.getMinY()
						- getGrid().getGridExtent().getMin().getY()))
				% cellSize;
		double maxX = (geometryBounds.getMaxX() + cellSize)
				- Math.abs(geometryBounds.getMaxX()
						- getGrid().getGridExtent().getMin().getX()) % cellSize;
		double maxY = (geometryBounds.getMaxY() + cellSize)
				- (Math.abs(geometryBounds.getMaxY()
						- getGrid().getGridExtent().getMin().getY()))
				% cellSize;
	 ...
	 * 
	 * @param index
	 *            indice de la geometría a eliminar
	 */
