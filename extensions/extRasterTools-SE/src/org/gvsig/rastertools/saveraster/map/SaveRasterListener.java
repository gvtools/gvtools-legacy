...
/**
 * Extensión de la clase SaveRasterListenerImpl de FMap para poder llamar a
 * métodos de andami o de gvSIG.
 * 
...
public class SaveRasterListener extends SaveRasterListenerImpl {
	 ...
	 */
	/*
	 * public void setRasterLayer(FLyrRaster flyr){ layer = flyr; }
	 */
	...
	public void rectangle(RectangleEvent event) {
		...
		if (Math.abs(fin.getY() - ini.getY()) > 10
				&& Math.abs(fin.getX() - ini.getX()) > 10) {
			...
			try {
				dialog.getTInfDerX().setValue(
						String.valueOf(rect.getMaxX()).substring(0,
								indexPoint + dec));
			...
			try {
				dialog.getTInfDerY().setValue(
						String.valueOf(rect.getMinY()).substring(0,
								indexPoint + dec));
			} catch (IndexOutOfBoundsException ex) {
			...
			try {
				dialog.getTSupIzqX().setValue(
						String.valueOf(rect.getMinX()).substring(0,
								indexPoint + dec));
			} catch (IndexOutOfBoundsException ex) {
			...
			try {
				dialog.getTSupIzqY().setValue(
						String.valueOf(rect.getMaxY()).substring(0,
								indexPoint + dec));
			} catch (IndexOutOfBoundsException ex) {
