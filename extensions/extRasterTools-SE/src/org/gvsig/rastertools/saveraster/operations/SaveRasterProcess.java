...
public class SaveRasterProcess extends RasterProcess {
	...
	public void init() {
		...
		supportImage = getBooleanParam("remotelayers")
	...
	public void process() throws InterruptedException {
		...
		AffineTransform at = new AffineTransform(vpData.getExtent().width()
				/ imgSz.width, 0, 0,
				-(vpData.getExtent().height() / imgSz.height), vpData
						.getExtent().getULX(), vpData.getExtent().getULY());

		String oldFileName = fileName;
		if (supportImage) {
			fileName = fileName.substring(
					0,
					Math.min(fileName.lastIndexOf(File.separator) + 1,
							fileName.length() - 1));
		...
		try {
			...
			if (supportImage) {
				try {
					...
					jp2Copy = new CopyDataset(fileName, oldFileName,
							incrementableTask);
					jp2Copy.copy();
					new File(fileName).delete();
					new File(RasterUtilities.getRMFNameFromFileName(fileName))
							.delete();
			...
			try {
				...
				// Si la ventana se ha cerrado ya es porque ha sido cancelada
				// por lo que
				// producirá esta excepción. En este caso no se lanza la ventana
				// de información
				// de finalización.
			}
			...
			RasterToolsUtil.messageBoxError("memoria_excedida", this,
					new IOException("Out of memory en salvar a raster"));
		} finally {
			if (layers != null) {
				for (int i = 0; i < layers.size(); i++)
	 ...
	 */
	private void write(String name, AffineTransform at, Params writerParams,
			IDataWriter rasterizerLayer) throws IOException,
			InterruptedException {
		RasterTask task = RasterTaskQueue
				.get(Thread.currentThread().toString());
		GeoRasterWriter geoRasterWriter = null;
		try {
			// TODO: FUNCIONALIDAD: Poner los gerWriter con la proyección de la
			// vista
			geoRasterWriter = GeoRasterWriter.getWriter(rasterizerLayer, name,
					3, at, dimension.width, dimension.height,
					IBuffer.TYPE_IMAGE, writerParams, null);
		} catch (NotSupportedExtensionException e) {
			RasterToolsUtil.messageBoxError("extension_no_soportada",
					(Component) PluginServices.getMainFrame(), e);
		...
		geoRasterWriter.setCancellableRasterDriver(new ExternalCancellable(
				incrementableTask));
	...
	private Params getWriterParams(String name) {
		...
		try {
			if (writer == null) {
				// La primera vez que se obtiene el driver
