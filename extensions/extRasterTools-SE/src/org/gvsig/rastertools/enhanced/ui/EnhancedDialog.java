...
public class EnhancedDialog extends JPanel implements IWindow, IWindowListener {
	 ...
	 * 
	 * @param lyr
	 *            Capa raster sobre la que se opera
	 * @param lyrs
	 *            Lista de capas cargadas en el TOC
	 * @param width
	 *            Ancho
	 * @param height
	 *            Alto
	...
	public EnhancedDialog(FLyrRasterSE lyr, int width, int height) {
		...
		filteredPreview.setFilterStatus(this.lyr.getRender().getFilterList()
				.getStatusCloned());
		controlsPanel = new SelectorsPanel(this.lyr,
				graphicsPanel.getInputHistogram());

		this.add(getPreviewBasePanel(), BorderLayout.CENTER);

		listener = new EnhancedListener(controlsPanel, graphicsPanel, this,
				filteredPreview);
		...
		getPreviewBasePanel().getButtonsPanel().addButtonPressedListener(
				listener);
	...
	public PreviewBasePanel getPreviewBasePanel() {
		if (previewBasePanel == null) {
			...
			downPreview.add(getNewOrSaveLayerPanel().getJPanel(),
					gridBagConstraints);
			...
			downPreview.add(getNewOrSaveLayerPanel().getFileNamePanel(),
					gridBagConstraints);

			previewBasePanel = new PreviewBasePanel(
					ButtonsPanel.BUTTONS_APPLYCLOSE, list, controlsPanel,
					downPreview, filteredPreview, lyr);
	...
	public WindowInfo getWindowInfo() {
		...
		if (getPreviewBasePanel().getLayer() != null)
			m_viewinfo.setAdditionalInfo(getPreviewBasePanel().getLayer()
					.getName());
