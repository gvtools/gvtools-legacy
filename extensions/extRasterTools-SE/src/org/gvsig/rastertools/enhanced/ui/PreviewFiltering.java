...
/**
 * Procesado de la imagen para la previsualización. Para poder usar esta clase
 * después de instanciarla habrá que asignarle un valor a showFiltersSelected
 * aunque por defecto está a true por lo que se visualizaran los cambios sin
 * problemas, asigna una lista de filtros inicial (que ya tenga el raster
 * aplicado) con setFilterStatus y asignar los filtros que queramos aplicar con
 * addNewParam (una llamada por cada filtro).
 * 
 * 19/02/2008
 * 
...
public class PreviewFiltering implements IPreviewRenderProcess {
	...
	/**
	 * Asigna el arrayList de filtros inicial. Esto hace que aplique los filtros
	 * que ya existen en el raster. Estos pueden ser obtenidos del render de la
	 * capa de la forma lyr.getRender().getFilterList().getStatusCloned().
	 * 
	 * @param params
	...
	/**
	 * Procesa la imagen con la lista de filtros si el flag showFilterSelected
	 * está a true. Esta función llama a addFilter por cada filtro añadido pero
	 * es applyFilters la encargada de construir la lista.
	 */
	...
	public ArrayList applyFilters(IRasterRendering rendering) {
		...
		for (int i = 0; i < paramsList.size(); i++) {
			...
			for (int j = 0; j < listFilterUsed.size(); j++) {
				if (((ParamStruct) listFilterUsed.get(j)).getFilterName()
						.equals(((ParamStruct) paramsList.get(i))
								.getFilterName())) {
