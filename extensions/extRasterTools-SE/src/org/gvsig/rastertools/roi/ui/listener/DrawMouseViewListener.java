...
public class DrawMouseViewListener implements PolylineListener, PointListener {
	...
	public void polylineFinished(MeasureEvent event) throws BehaviorException {
		...
		if (tablePanel.getMapControl().getCurrentTool()
				.equals("drawPolygonROI"))
		...
		try {
			...
			roiName = (String) tablePanel.getTable().getModel()
				...
				RasterToolsUtil
						.messageBoxError(
								...
								new NullPointerException(
										"ROI no seleccionada DrawMouseViewListener L 61"));
				return;
			}
			...
			ISymbol sym = null;
			Color geometryColor = (Color) tablePanel.getTable().getModel()
					.getValueAt(selectedRow, 4);

			if (tablePanel.getPolygonToolButton().isSelected()) {
				numGeometries = ((Integer) tablePanel.getTable().getModel()
						.getValueAt(selectedRow, 1)).intValue();
				tablePanel
						.getTable()
						.getModel()
						.setValueAt(new Integer(numGeometries + 1),
								selectedRow, 1);
				sym = SymbologyFactory.createDefaultFillSymbol();
				((IFillSymbol) sym).setFillColor(geometryColor);
			} else {
				numGeometries = ((Integer) tablePanel.getTable().getModel()
						.getValueAt(selectedRow, 2)).intValue();
				tablePanel
						.getTable()
						.getModel()
						.setValueAt(new Integer(numGeometries + 1),
			...
			tablePanel.getMapControl().getMapContext().getGraphicsLayer()
					.addGraphic(fGraphic);
	...
	public void point(PointEvent event) throws BehaviorException {
		...
		try {
			...
			roi = (VectorialROI) tablePanel.getROI(roiName);
			roi.addGeometry(geometry);
			int numPoints = ((Integer) tablePanel.getTable().getModel()
					.getValueAt(selectedRow, 3)).intValue();
			tablePanel.getTable().getModel()
					.setValueAt(new Integer(numPoints + 1), selectedRow, 3);
			// tablePanel.getTable().getModel().setValueAt(new
			// Integer(roi.getValues()), selectedRow, 5);
