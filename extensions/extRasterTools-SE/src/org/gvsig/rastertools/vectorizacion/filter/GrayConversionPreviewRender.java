...
/**
 * Clase para el renderizado de la vista previa en la conversión a escala de
 * grises. 10/06/2008
 * 
...
public class GrayConversionPreviewRender implements IPreviewRenderProcess {
	 ...
	 */
	private void addHighPass(RasterFilterListManager filterManager,
	 ...
	 * @throws FilterTypeException
	 */
	private void addMode(RasterFilterListManager filterManager,
			IRasterRendering rendering) throws FilterTypeException {
	 ...
	 * @throws FilterTypeException
	 */
	private void addNoiseReduction(RasterFilterListManager filterManager,
			IRasterRendering rendering) throws FilterTypeException {
		...
		if (data.isPosterizationActive()) {
			...
			try {
				...
				if (lyr.getDataType()[0] == IBuffer.TYPE_BYTE) {
					...
					min = Math.min(
							Math.min(stats.getMin()[0], stats.getMin()[1]),
							stats.getMin()[2]);
					max = Math.max(
							Math.max(stats.getMax()[0], stats.getMax()[1]),
							stats.getMax()[2]);
				}
			...
			// En caso de ser conversión a B/W se asigna el valor de umbral para
			// la posterización
			if (data.getPosterizationLevels() == 2) {
				in[2] = in[3] = lastIn
						+ ((data.getPosterizationThreshold() * (max - min)) / 255);
			} else {
				for (int i = 2; i < in.length - 2; i = i + 2) {
					in[i] = in[i + 1] = lastIn
							+ ((max - min) / data.getPosterizationLevels());
					lastIn = (int) in[i];
				}
			}
