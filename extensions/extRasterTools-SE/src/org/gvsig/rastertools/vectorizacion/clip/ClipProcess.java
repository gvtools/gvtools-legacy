...
public class ClipProcess implements IProcessActions {
	...
	/**
	 * Asigna la clase IProcessActions para las acciones de finalización.
	 * 
	 * @param pa
	 */
	...
	public void clip(ClipData data) throws FilterTypeException {
		...
		if (sourceLayer.getBandCount() > 1)
			clippingProcess.addParam("drawablebands",
					sourceLayer.getRenderBands());
		else
		...
		double newCellSize = sourceLayer.getCellSize()
				/ data.getScaleSelectedValue();
		AffineTransform at = new AffineTransform(newCellSize, 0, 0,
				-newCellSize, data.getWCCoordinates()[0],
				data.getWCCoordinates()[1]);
		clippingProcess.addParam("affinetransform", at);
		if (sourceLayer.getBandCount() < 3)
			clippingProcess.addParam("colorInterpretation",
					DatasetColorInterpretation.createGrayInterpretation());
		else
			clippingProcess.addParam("colorInterpretation",
					DatasetColorInterpretation.createRGBInterpretation());
		double w = Math.abs((data.getWCCoordinates()[0] - data
				.getWCCoordinates()[2]) / sourceLayer.getCellSize());
		double h = Math.abs((data.getWCCoordinates()[1] - data
				.getWCCoordinates()[3]) / sourceLayer.getCellSize());
		clippingProcess.addParam("resolution",
				new int[] { (int) (w * data.getScaleSelectedValue()),
						(int) (h * data.getScaleSelectedValue()) });
	...
	public void end(Object param) {
		...
		try {
			if (param instanceof Object[] && ((Object[]) param).length == 2
					&& ((Object[]) param)[0] instanceof String) {
				clip = FLyrRasterSE.createLayer(
						RasterLibrary.getOnlyLayerName(),
						(String) ((Object[]) param)[0], sourceLayer.getCrs());
			}
