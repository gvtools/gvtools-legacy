 ...
 */
public class OverviewsTocMenuEntry extends AbstractTocContextMenuAction
	...
	public boolean isEnabled(ITocItem item, FLayer[] selectedItems) {
		...
		if (!((FLyrRasterSE) selectedItems[0]).getDataSource()
				.overviewsSupport())
			return false;

		return ((FLyrRasterSE) selectedItems[0])
				.isActionEnabled(IRasterLayerActions.CREATEOVERVIEWS);
	}
	...
	public void execute(ITocItem item, FLayer[] selectedItems) {
		...
		if (lyr instanceof FLyrRasterSE) {
			if (RasterToolsUtil.messageBoxYesOrNot(
					"sobreescribir_datos_overview", this)) {

				try {
					if (((FLyrRasterSE) lyr).getDataSource()
							.getOverviewCount(0) > 0) {
						if (!RasterToolsUtil.messageBoxYesOrNot(
								"sobreescribir_overviews", this))
							return;
					}
				} catch (BandAccessException e) {
					// Actuamos como si no tubiera overviews pero salvamos un
					// Log
					RasterToolsUtil.debug(
							"Error accediendo a la banda en getOverViewCount",
							this, e);
