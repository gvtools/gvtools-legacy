...
public class EnhancedControl implements IProcessActions {
	...
	class BrightnessContrastListener implements ActionListener, SliderListener {
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.gvsig.gui.beans.slidertext.listeners.SliderListener#
		 * actionValueChanged
	 ...
	 * @param tp
	 */
	public EnhancedControl(AbstractPanelGroup panelGroup, EnhancedPanel tp,
			IRasterDataset dataset, FLayer lyr, RasterFilterList rfl) {
	...
	private void setValuesFromFilterToPanel() {
		// BRILLO
		BrightnessFilter bFilter = (BrightnessFilter) filterList
				.getByName(BrightnessFilter.names[0]);
		if (bFilter != null) {
		...
		// CONTRASTE
		ContrastFilter cFilter = (ContrastFilter) filterList
				.getByName(ContrastFilter.names[0]);
		if (cFilter != null) {
		...
		// REALCE LINEAL
		LinearStretchEnhancementFilter eFilter = (LinearStretchEnhancementFilter) filterList
		...
		if (eFilter != null) {
			...
			LinearStretchParams stretchs = (LinearStretchParams) eFilter
					.getParam("stretchs");
			double[] tailTrimList;
			if (stretchs != null)
			...
			double tailTrim = new Double(nValues > 0 ? median / nValues
					: median).doubleValue();
	...
	private void setValuesFromPanelToFilter() throws FilterTypeException,
		...
		// REALCE
		EnhancementStretchListManager eManager = (EnhancementStretchListManager) manager
		...
		if (ePanel.getActive().isSelected()) {
			if (dataset != null && dataset.getDataSource() != null) {
				...
				if (lyr instanceof IRasterRendering)
					renderBands = ((IRasterRendering) lyr).getRender()
							.getRenderBands();
				...
				try {
					if (!stats.isCalculated() && lyr instanceof FLyrRasterSE)
						...
						if (ePanel.isTailTrimCheckSelected()) {
							LinearStretchParams leParams = LinearStretchParams
									.createStandardParam(((FLyrRasterSE) lyr)
											.getRenderBands(), (double) (ePanel
											.getTrimValue() / 100D), stats, rgb);
							eManager.addEnhancedStretchFilter(leParams, stats,
									renderBands, ePanel.isRemoveEndsSelected());
						} else {
							filterList.remove(TailTrimFilter.class);
							LinearStretchParams leParams = LinearStretchParams
									.createStandardParam(((FLyrRasterSE) lyr)
											.getRenderBands(), 0, stats, rgb);
							eManager.addEnhancedStretchFilter(leParams, stats,
									renderBands, ePanel.isRemoveEndsSelected());
						}
					}
				} catch (FileNotOpenException e) {
					throw new FilterAddException(
							"No se han podido calcular estadisticas. Error al añadir realce;"
									+ e.getMessage());
	...
	private void endActionsForFilterSettings() throws FilterTypeException {
		ArrayList listOrder = (ArrayList) panelGroup.getProperties().get(
				"filterOrder");
		ArrayList listCopy = filterList.getStatusCloned();
		int cont = 0;
		for (int i = 0; i < listOrder.size(); i++) {
			int pos = hasFilter(listCopy,
					((RasterFilter) listOrder.get(i)).getName());
			if (pos != -1) {
	...
	/**
	 * Consulta si el filtro especificado en el parámetro name está dentro de la
	 * lista filter.
	 * 
	 * @param filter
	 *            Lista donde se consulta
	 * @param name
	 *            Nombre a comprobar si está en la lista
	...
	public void saveStatus() {
		panelGroup.getProperties().put("filterStatus",
				filterList.getStatusCloned());
		...
		if (posEnhanced != -1) {
			...
			if (posTailTrim == -1) {
				RasterFilter aux = StatisticsListManager.createTailFilter(0, 0,
						false, null);
			...
			// Insertamos primero el tailtrim y luego el realce para conservar
			// un orden logico
			filterOrder.add(0,
					StatisticsListManager.createTailFilter(0, 0, false, null));
			filterOrder.add(1, EnhancementStretchListManager
					.createEnhancedFilter(null, null, null, false));
		}

		// Si no tiene brillo, lo insertamos
		if (hasFilter(filterOrder, "brightness") == -1)
			filterOrder.add(BrightnessContrastListManager
					.createBrightnessFilter(0));

		// Si no tiene el contraste, lo insertamos
		if (hasFilter(filterOrder, "contrast") == -1)
			filterOrder.add(BrightnessContrastListManager
					.createContrastFilter(0));

		panelGroup.getProperties().put("filterOrder", filterOrder);
	}

	public void restoreStatus() {
		filterList.setStatus((ArrayList) panelGroup.getProperties().get(
				"filterStatus"));
	...
	public void end(Object param) {
		...
		if (param instanceof FLyrRasterSE
				&& ((FLyrRasterSE) param).getDataSource() != null)
		...
		try {
			...
			if (ePanel.isTailTrimCheckSelected()) {
				LinearStretchParams leParams = LinearStretchParams
						.createStandardParam(
								((FLyrRasterSE) lyr).getRenderBands(),
								(double) (ePanel.getTrimValue() / 100D),
								statistics, rgb);
				eManager.addEnhancedStretchFilter(leParams, statistics,
						renderBands, ePanel.isRemoveEndsSelected());
			} else {
				filterList.remove(TailTrimFilter.class);
				LinearStretchParams leParams = LinearStretchParams
						.createStandardParam(
								((FLyrRasterSE) lyr).getRenderBands(), 0,
								statistics, rgb);
				eManager.addEnhancedStretchFilter(leParams, statistics,
						renderBands, ePanel.isRemoveEndsSelected());
			}
			...
			RasterToolsUtil
					.messageBoxError(
							PluginServices.getText(this, "error_adding_stats"),
							this, e);
		} catch (RasterDriverException e) {
			RasterToolsUtil
					.messageBoxError(
