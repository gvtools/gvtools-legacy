...
public class PanSharpeningControl implements ActionListener, SliderListener,
		ChangeListener {
	...
	public PanSharpeningControl(AbstractPanelGroup panelGroup,
			PanSharpeningPanel pP, IRasterDataset dataset,
			IRasterProperties prop, FLayer lyr, RasterFilterList rfl) {
	...
	public void stateChanged(ChangeEvent e) {
		...
		if (e.getSource().equals(pansharpPanel.getJSlider())) {
			pansharpPanel.getJTextField()
					.setText(
							String.valueOf((pansharpPanel.getJSlider()
									.getValue() / 200.0)));
		}

		if (e.getSource().equals(pansharpPanel.getJSlider1())) {
			pansharpPanel.getJTextField1()
					.setText(
							String.valueOf((pansharpPanel.getJSlider1()
									.getValue() / 2)));
		}
	}
	...
	private void setValuesFromPanelToFilter() throws FilterTypeException {
		...
		PanSharpeningListManager bcManager = (PanSharpeningListManager) manager
				.getManagerByClass(PanSharpeningListManager.class);

		// Fichero de salida
		String fileName = RasterLibrary.usesOnlyLayerName();
		fileNameOutput = Utilities.createTempDirectory() + File.separator
				+ fileName + ".tif";
		if (pansharpPanel.getCbActiveSharpening().isSelected()) {
			...
			ArrayList orden = (ArrayList) panelGroup.getProperties().get(
					"renderBands");
			...
			double coef = 0D;
			if (pansharpPanel.getRbHSL().isSelected())
				coef = Double.parseDouble(pansharpPanel.getJTextField()
						.getText());
			int coefBrovey = 0;
			if (pansharpPanel.getRbBrovey().isSelected())
				coefBrovey = Integer.parseInt(pansharpPanel.getJTextField1()
						.getText());
			// Añadir el filtro
			if (posPancromatica != -1)
				bcManager.addPanSharpeningFilter(dataset, posPancromatica,
						orden, 1, method, coef, coefBrovey, fileNameOutput);
			else
	...
	void applyPansharpeningProcess() throws FilterTypeException {
		...
		try {
			filterList.getFilterByBaseClass(PanSharpeningFilter.class)
					.execute();
		...
		try {
			...
			lyr = FLyrRasterSE.createLayer(fileNameOutput.substring(
					fileNameOutput.lastIndexOf(File.separator) + 1, endIndex),
					new File(fileNameOutput), dataset.getDataSource()
							.getDataset(0)[0].getCrs());
