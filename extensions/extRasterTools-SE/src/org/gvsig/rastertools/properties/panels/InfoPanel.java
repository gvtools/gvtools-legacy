...
public class InfoPanel extends AbstractPanel {
	...
	/**
	 * Método que crea el código HTML para la tabla de información general
	...
	public String tablaInfo() {
		...
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "archivo"),
				op.getFileName()[selectedDataSet]);

		long fileSize = op.getFileSize()[selectedDataSet];
		if (op instanceof FLyrRasterSE) {
			FLyrRasterSE rasterSE = (FLyrRasterSE) op;
			fileSize = rasterSE.getDataSource().getDataset(selectedDataSet)[0]
					.getFileSize();
		}

		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "size"),
				RasterUtilities.formatFileSize(fileSize) + " ");
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "ancho_alto"), op.getPxWidth()
						+ " X " + op.getPxHeight());
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "formato"), op.getFileFormat());
		propiedades += setHTMLBasicProperty(PluginServices.getText(this,
				"georref"),
				op.isGeoreferenced() ? PluginServices.getText(this, "si")
						: PluginServices.getText(this, "no"));
		propiedades += setHTMLBasicProperty(PluginServices.getText(this,
				"nbandas"), new Integer(
				op.getBandCountFromDataset()[selectedDataSet]).toString());
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "tipo_dato"),
				RasterUtilities.typesToString(op.getDataType()[0]));
	...
	public String tablaCoord() {
		...
		propiedades += setHTMLBasicProperty(PluginServices.getText(this, "ul"),
				ext.getULX() + ", " + ext.getULY());
		propiedades += setHTMLBasicProperty(PluginServices.getText(this, "lr"),
				ext.getLRX() + ", " + ext.getLRY());
		propiedades += setHTMLBasicProperty(PluginServices.getText(this, "ur"),
				ext.getURX() + ", " + ext.getURY());
		propiedades += setHTMLBasicProperty(PluginServices.getText(this, "ll"),
				ext.getLLX() + ", " + ext.getLLY());
		...
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "tamPixX"),
				String.valueOf(pixelSizeX));
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "tamPixY"),
				String.valueOf(pixelSizeY));
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "rotX"), String.valueOf(rotX));
		propiedades += setHTMLBasicProperty(
				PluginServices.getText(this, "rotY"), String.valueOf(rotY));

		return "<br>" + setHTMLTable(cabecera + propiedades);
	}

	/**
	 * Método que crea la tabla de origen de datos dentro del panel de
	 * Información.
	 * 
	...
	public String tablaOrigen() {
		...
		for (int j = 0; j < op.getBandCountFromDataset()[selectedDataSet]; j++) {
			...
			data += ", ";
			data += "ColorInterp="
					+ op.getColorInterpretation(j, selectedDataSet);
			...
			if (op instanceof FLyrRasterSE) {
				...
				data = "";
				try {
					int count = rasterSE.getDataSource().getDataset(
							selectedDataSet)[0].getOverviewCount(j);
					if (count > 0) {
						for (int i = 0; i < count; i++) {
							if (!data.equals(""))
								data += ", ";
							data += rasterSE.getDataSource().getDataset(
									selectedDataSet)[0].getOverviewWidth(j, i);
							data += "x";
							data += rasterSE.getDataSource().getDataset(
									selectedDataSet)[0].getOverviewHeight(j, i);
						}
	...
	/**
	 * Método para crear la tabla de proyección del raster en el panel de
	 * información de propiedades de raster.
	 * 
	...
	public String tablaMetadatos() {
		...
		if (meta != null) {
			...
			for (int j = 0; j < metadatos.length; j++) {
				...
				propiedades += setHTMLBasicProperty(
						metadatos[j].substring(0, index),
						metadatos[j].substring(index + 1));
			}
		}
	 ...
	 * propiedades de ráster. Se llama cada vez que se actualiza algún dato de
	 * las tablas.
	 * 
	 */
	public void refresh() {
		...
		if (!jComboBoxEvent) {
			...
			for (int i = 0; i < op.getFileCount(); i++) {
				...
				nameFile = nameFile.substring(nameFile
		...
		if ((refresh) && (op.getFileCount() >= 1)) {
			html = setHTMLBody(tablaInfo() + tablaCoord() + tablaOrigen()
					+ tablaProjection() + tablaMetadatos());
		}
