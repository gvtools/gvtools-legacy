 ...
 */
public class Layout extends JPanel implements SingletonWindow,
		ViewPortListener, IWindowListener, CommandListener {
	...
	/**
	 * We use it when we are doing a layout and assigning tags. It is put in
	 * debug when we do a VIEW_TAGS
	 */
	...
	public void drawLayoutPrint(Graphics2D g2) {
		...
		double scale = 0;
		scale = layoutControl.getRect().height
				/ layoutContext.getAtributes().m_sizePaper.getAlto() * 1;
	 ...
	 * characteristics that contains attributes and differentiating if is to
	 * visualize in screen or for print.
	 * 
	 */
	public void obtainRect(boolean isPrint) {
		layoutContext.getAtributes().obtainRect(isPrint,
	...
	public IFFrame openFFrameDialog(IFFrame fframe) {
		...
		if (fframedialog != null) {
			fframedialog.setRectangle(fframe.getBoundingBox(layoutControl
					.getAT()));
	...
	public static Layout createLayout(XMLEntity xml, Project p)
		...
		try {
			layout.layoutContext.setAdjustToGrid(xml
					.getBooleanProperty("isCuadricula"));
			// layout.setName(xml.getStringProperty("m_name"));
			layout.getLayoutContext().setAtributes(
					Attributes.createAtributes(xml.getChild(0)));
			if (xml.contains("isEditable")) {
				layout.layoutContext.setEditable(xml
						.getBooleanProperty("isEditable"));
			}
			if (xml.contains("numBehind")) {
				layout.layoutContext.numBehind = xml
						.getIntProperty("numBehind");
				layout.layoutContext.numBefore = xml
						.getIntProperty("numBefore");
			}
			// layout.layoutContext.getEFS().startComplexCommand();
			// for (int i = 1; i < xml.getChildrenCount(); i++) {
			// try {
			// layout.layoutContext.addFFrame(FFrame.createFFrame(xml.getChild(i),
			// p,
			// layout), true, false);
			...
			for (int i = 1; i < xml.getChildrenCount(); i++) {
				try {
					IFFrame frame = FFrame.createFromXML(xml.getChild(i), p,
							layout);
					layout.layoutContext.addFFrame(frame, true,
							frame.getSelected() == IFFrame.RECT);
				} catch (OpenException e) {
					e.showError();
				}
			}
	...
	public void windowActivated() {
		...
		PluginServices
				...
				.setMessage(
						"units",
						PluginServices.getText(this, layoutContext
	...
	public void layoutToPDF(String suggestedName) {
		...
		JFileChooser jfc = new JFileChooser(PDF_AND_PS_FILECHOOSER,
	...
	public void layoutToPS(String suggestedName) {
		FileFilter pdfFilter = new GenericFileFilter("ps",
				PluginServices.getText(this, "ps"));
	...
	public static boolean getDefaultShowGrid() {
		if (defaultShowGrid == null) {
			...
			if (xml.contains("DefaultShowLayoutGrid")) {
				defaultShowGrid = new Boolean(
						xml.getBooleanProperty("DefaultShowLayoutGrid"));
			} else {
	...
	public static boolean getDefaultAdjustToGrid() {
		if (defaultAdjustToGrid == null) {
			...
			if (xml.contains("DefaultEnableLayoutGrid")) {
				defaultAdjustToGrid = new Boolean(
