...
/**
 * <p>
 * Panel where user will select the layer and its fields to display as a
 * <i>quick information tool</i> on a <code>MapControl</code> object.
 * </p>
 * 
 * @version 06/03/08
 * 
...
public class QuickInfoDataSelectionPanel extends JPanel implements IWindow {
	...
	/**
	 * <p>
	 * Creates a new form where user could select the quick information to
	 * display.
	 * </p>
	 */
	...
	private void initialize() {
		...
		InitQIPanelProcess iprocess = new InitQIPanelProcess(
				PluginServices.getText(this,
						"Initializing_configuration_quick_info_tool"),
				PluginServices.getText(this, "loading_layers_please_wait"),
				this);
		...
		//
		// bIcon = new
		// ImageIcon(QuickInfoDataSelectionPanel.class.getClassLoader().getResource("images/layerGroup.png"));
		// leafIcon = new
		// ImageIcon(QuickInfoDataSelectionPanel.class.getClassLoader().getResource("images/field-leaf-icon.png"));
		// childIcon = new
		// ImageIcon(QuickInfoDataSelectionPanel.class.getClassLoader().getResource("images/field-child-icon.png"));
		// complexIcon = new
		// ImageIcon(QuickInfoDataSelectionPanel.class.getClassLoader().getResource("images/field-complex-icon.png"));
		//
		// add(getNorthPanel());
	...
	/**
	 * <p>
	 * This method initializes layersLabel.
	 * </p>
	 * 
	...
	private JComboBox getLayersComboBox() {
		if (layersComboBox == null) {
			...
			layersComboBox.setPreferredSize(new Dimension(layersComboBox_Width,
					layersComboBox_Height));
					...
					if (e.getStateChange() == ItemEvent.SELECTED) {
						...
						try {
							...
							if (fW != null) {
								...
								if (layer != null) {
									infoLayerSelected = null;
									infoLayerSelected = new QuickInfoFLayerSelected(
											layer);
									getFieldsModeButton().setToolTipText(
											PluginServices.getText(this,
													"Ascending"));
									layersComboBox.setToolTipText("<html>"
											+ PluginServices.getText(this,
													"Layer")
											+ ": "
											+ layer.getName()
											...
											+ ": "
											+ FShapeTypeNames
													.getFShapeTypeName(((FLyrVect) layer)
															.getShapeType())
											+ "</html>");
							...
							getFieldsModeButton().setText(
									PluginServices.getText(this, "Ascending"));
							getFieldsModeButton().setToolTipText(
									PluginServices.getText(this, "Ascending"));
							updateAcceptButton();
						} catch (ReadDriverException dE) {
							NotificationManager.showMessageError(PluginServices
									.getText(null, "problem_loading_layers"),
									dE);
						}
					}
				}
	...
	/**
	 * <p>
	 * This method initializes layerFieldsScrollPane.
	 * </p>
	 * 
	...
	private JScrollPane getLayerFieldsScrollPane() {
		if (layerFieldsScrollPane == null) {
			...
			layerFieldsScrollPane.setBorder(BorderFactory
					.createLineBorder(Color.LIGHT_GRAY));
			layerFieldsScrollPane.setPreferredSize(new java.awt.Dimension(446,
					196));
	...
	/**
	 * <p>
	 * This method initializes optionButtonsPanel.
	 * </p>
	 * 
	 * @return javax.swing.JPanel
	 */
	...
	private JButton getSelectAllButton() {
		if (selectAllButton == null) {
			selectAllButton = new JButton(PluginServices.getText(this,
					"Select_all"));
			selectAllButton.setToolTipText(PluginServices.getText(this,
					"Select_all"));
			...
			selectAllButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					for (int i = 0; i < getFieldsTable().getRowCount(); i++) {
						...
						infoLayerSelected.getLayerFields().put(
								((FieldNameWrapper) fieldsTable.getModel()
										.getValueAt(i, 1)).getField(),
								new Vector());
					}

					selectedFields = fieldsTable.getRowCount();

					for (int i = 0; i < getOptionalFieldsPanel()
							.getComponentCount(); i++) {
						Component child = getOptionalFieldsPanel()
								.getComponent(i);
						if (child instanceof JCheckBox) {
							((JCheckBox) child).setSelected(true);
							infoLayerSelected.getCalculatedLayerFields()
									.put(((JCheckBox) child).getText(),
											new Vector());
							selectedFields++;
						}
					}
	...
	/**
	 * <p>
	 * This method initializes fieldsModeButton.
	 * </p>
	 * 
	 * @return javax.swing.JButton
	 */
	private JButton getFieldsModeButton() {
		if (fieldsModeButton == null) {
			fieldsModeButton = new JButton(PluginServices.getText(this,
					"Ascending"));
			fieldsModeButton.setToolTipText(PluginServices.getText(this,
					"Ascending"));
			...
			fieldsModeButton.addMouseListener(new MouseAdapter() {
				 ...
				 */
				public void mouseReleased(MouseEvent e) {
					if (fieldsModeButton.getText().compareTo(
							PluginServices.getText(this, "Ascending")) == 0) {
						// Sort the values in the second column of the model
						// in descending order
						sortFieldColumns(((DefaultTableModel) getFieldsTable()
								.getModel()), ColumnSorter.ASCENDING);
						fieldsModeButton.setText(PluginServices.getText(this,
								"Descending"));
						fieldsModeButton.setToolTipText(PluginServices.getText(
								this, "Descending"));
					} else {
						if (fieldsModeButton.getText().compareTo(
								PluginServices.getText(this, "Descending")) == 0) {
							sortFieldColumns(
									((DefaultTableModel) getFieldsTable()
											.getModel()),
									ColumnSorter.DESCENDING);
							fieldsModeButton.setText(PluginServices.getText(
									this, "Reload"));
							fieldsModeButton.setToolTipText(PluginServices
									.getText(this, "Reload"));
						} else {
							if (fieldsModeButton.getText().compareTo(
									PluginServices.getText(this, "Reload")) == 0) {
								int index = getLayersComboBox()
										.getSelectedIndex();
								getLayersComboBox().setSelectedIndex(-1);
								getLayersComboBox().setSelectedIndex(index);
								fieldsModeButton.setText(PluginServices
										.getText(this, "Ascending"));
								fieldsModeButton.setToolTipText(PluginServices
										.getText(this, "Ascending"));
							}
						}
	...
	private JButton getUnselectAllButton() {
		if (unselectAllButton == null) {
			unselectAllButton = new JButton(PluginServices.getText(this,
					"Clean_selection"));
			unselectAllButton.setToolTipText(PluginServices.getText(this,
					"Clean_selection"));
			...
			unselectAllButton.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					for (int i = 0; i < getFieldsTable().getRowCount(); i++) {
						...
						infoLayerSelected.getLayerFields().remove(
								((FieldNameWrapper) fieldsTable.getModel()
										.getValueAt(i, 1)).getField());
					}
					...
					for (int i = 0; i < getOptionalFieldsPanel()
						...
						if (child instanceof JCheckBox) {
							...
							infoLayerSelected.getCalculatedLayerFields()
									.remove(((JCheckBox) child).getText());
							selectedFields--;
						}
					}
	...
	/**
	 * <p>
	 * This method initializes optionalFieldsPanel.
	 * </p>
	 * 
	 * @return javax.swing.JPanel
	 */
	private JPanel getOptionalFieldsPanel() {
		if (optionalFieldsPanel == null) {
			...
			optionalFieldsPanel.setBorder(BorderFactory
					.createTitledBorder(PluginServices.getText(this,
							"calculated_fields")));
	...
	/**
	 * <p>
	 * Returns a reference to the table that stores the fields.
	 * </p>
	 * 
	...
	private JTable getFieldsTable() {
		if (fieldsTable == null) {
			...
			JLabel l = (JLabel) fieldsTable.getTableHeader()
					.getDefaultRenderer();
			l.setPreferredSize(new Dimension(15, 20));

			fieldsTable.getColumnModel().getColumn(1)
					.setCellRenderer(new FieldNameRenderer());
			...
			fieldsTable.addMouseListener(new MouseAdapter() {
				...
				public void mouseReleased(MouseEvent e) {
					...
					if (row >= 0) {
						Boolean b = ((Boolean) fieldsTable.getModel()
								.getValueAt(row, 0));

						if (b.booleanValue() == true) {
							fieldsTable.getModel().setValueAt(Boolean.FALSE,
									row, 0);
							infoLayerSelected.getLayerFields().remove(
									((FieldNameWrapper) fieldsTable.getModel()
											.getValueAt(row, 1)).getField());
							selectedFields--;
							updateAcceptButton();
						} else {
							fieldsTable.getModel().setValueAt(Boolean.TRUE,
									row, 0);
							infoLayerSelected.getLayerFields().put(
									((FieldNameWrapper) fieldsTable.getModel()
											.getValueAt(row, 1)).getField(),
	...
	/**
	 * <p>
	 * Enables / disables the <i>accept</i> button according the number of
	 * selected fields.
	 * </p>
	 * 
	 * <p>
	 * If there is no field selected, disables that button, is there is any
	 * selected, enables it.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Gets the table model that lists the fields of the selected layer.
	 * </p>
	 * 
	...
	private TableModel getFieldsTableModel() {
		if (fieldsTableModel == null) {
			...
			((DefaultTableModel) fieldsTableModel).addColumn("",
					new Object[] {});
			((DefaultTableModel) fieldsTableModel).addColumn(
					PluginServices.getText(this, "Name"), new Object[] {});
			((DefaultTableModel) fieldsTableModel).addColumn(
					PluginServices.getText(this, "Type"), new Object[] {});
		}
	...
	/**
	 * <p>
	 * Updates "layersComboBox" with the name of visible layers of the
	 * associated <code>MapControl</code> instance.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Updates recursively the combo box with the layers name and property.
	 * </p>
	 * 
	 * @param node
	 *            parent node
	 * @param level
	 *            <code>node</code> level from the root
	 */
	private void refreshVisibleVectorLayers(FLayer node, int level) {
		if (node instanceof FLayers) {
			...
			if ((node.isVisible()) && (node instanceof FLyrVect)
					&& (layerName != null)) {
				getLayersComboBox().addItem(
						new FLayerWrapper(node, level, getLayerIcon(node)));
			}
		}
	}
	...
	/**
	 * <p>
	 * Returns the icon that represents the layer in the current active view's
	 * TOC.
	 * </p>
	 * 
	 * @param layer
	...
	/**
	 * <p>
	 * Updates "layerFieldsList" with the fields of the selected vector layer.
	 * </p>
	 */
	private void refreshSelectedLayerFields() {
		...
		if (item != null) {
			try {
				FLyrVect selectedLayer = (FLyrVect) ((FLayerWrapper) item)
						.getLayer();

				SelectableDataSource sds = selectedLayer.getRecordset();

				((DefaultTableModel) getFieldsTable().getModel())
						.setRowCount(0);
				fillTable(sds);

				refreshOptionalLayerFields(selectedLayer);
				getFieldsModeButton().getText().compareTo(
						PluginServices.getText(this, "Ascending"));
			} catch (Exception e) {
				NotificationManager.showMessageError(PluginServices.getText(
						null, "problem_loading_fields_of_the_layer"), e);
			}
		}
	}
	...
	/**
	 * <p>
	 * Fills the table with the fields of the layer.
	 * </p>
	 * 
	 * @param sds
	 *            object with the selected data of the data source
		...
		for (int i = 0; i < sds.getFieldCount(); i++) {
			if (sds.getFieldType(i) == Types.STRUCT)
				tableModel.addRow(new Object[] { Boolean.FALSE,
						new FieldNameWrapper(sds.getFieldName(i), null, true),
						SQLTypeNames.getSQLTypeName(sds.getFieldType(i)) });
			else
				tableModel.addRow(new Object[] { Boolean.FALSE,
						new FieldNameWrapper(sds.getFieldName(i), null, false),
						SQLTypeNames.getSQLTypeName(sds.getFieldType(i)) });
		}
	}
	...
	/**
	 * <p>
	 * Updates "optionalFieldsPanel" with the check boxes that are optional
	 * fields for the selected layer, according its type.
	 * </p>
	 * 
	 * @throws DriverException
	 *             if fails working with the data driver
	 */
	private void refreshOptionalLayerFields(FLyrVect seletedLayer)
		...
		case FShape.MULTI: // Can have points, lines and polygons
			getOptionalFieldsPanel().add(
					getNewLabel(PluginServices.getText(this, "Points") + ":"));
			getOptionalFieldsPanel().add(getNewOptionalCheckBox("Coordinates"));
			getOptionalFieldsPanel().add(getNewLabel("  "));
			getOptionalFieldsPanel().add(
					getNewLabel(PluginServices.getText(this, "Lines") + ":"));
			getOptionalFieldsPanel().add(getNewOptionalCheckBox("Length"));
			getOptionalFieldsPanel().add(getNewLabel("  "));
			getOptionalFieldsPanel()
					.add(getNewLabel(PluginServices.getText(this, "Polygons")
							+ ":"));
			getOptionalFieldsPanel().add(getNewOptionalCheckBox("Area"));
			getOptionalFieldsPanel().add(getNewOptionalCheckBox("Perimeter"));
			getOptionalFieldsPanel().add(
					getNewLabel(PluginServices.getText(this, "Multipoints")
							+ ":"));
			getOptionalFieldsPanel().add(
					getNewOptionalCheckBox("Number_of_points"));
			break;
	...
	/**
	 * <p>
	 * Creates a <code>JCheckBox</code> with <code>text</code> as a <i>text</i>
	 * and <i>tool tip text</i> of that component.
	 * </p>
	 * 
	 * @param text
	 *            text to assign
		...
		cb.addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent ev) {
				...
				if (source.isSelected()) {
					infoLayerSelected.getCalculatedLayerFields().put(
							source.getText(), new Vector());
					selectedFields++;
					updateAcceptButton();
				} else {
					infoLayerSelected.getCalculatedLayerFields().remove(
							source.getText());
					selectedFields--;
					updateAcceptButton();
				}
	...
	/**
	 * <p>
	 * Creates a <code>JLabel</code> with <code>text</code> as a <i>text</i> and
	 * <i>tool tip text</i> of that component.
	 * </p>
	 * 
	 * @param text
	 *            text to assign
	...
	private JLabel getNewLabel(String text) {
		JLabel cl = new JLabel("<html><body><p><i>"
				+ PluginServices.getText(this, text) + "</i></p></body></html>");
		cl.setToolTipText("<html><body><p><i>"
				+ PluginServices.getText(this, text) + "</i></p></body></html>");
	 ...
	 * component of the <code>QInfoDataSelectionPanel</code> panel.
	 * </p>
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class AdaptedAcceptCancelPanel extends AcceptCancelPanel {
		...
		private ActionListener getOKAction() {
			...
			return new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					...
					try {
						int type = ((FLyrVect) infoLayerSelected.getLayer())
								.getShapeType();
						...
						case FShape.MULTIPOINT:
							...
							break;
						default:
							getQuickInfoListener().setPixelTolerance(
					...
					mapControl
							.addCombinedTool(new AdaptedMouseMovementBehavior(
									getQuickInfoListener()));
				}
			};
		}
		...
		private QuickInfoListener getQuickInfoListener() {
			if (qIListener == null) {
				qIListener = new QuickInfoListener(mapControl,
						infoLayerSelected,
						QuickInfoListener.DEFAULT_PIXEL_TOLERANCE);
			}
	...
	/**
	 * <p>
	 * Wrappers a <code>FLayer</code> field.</code>
	 * <p>
	 * Represents a leaf as a green icon, a complex feature as a red icon, an
	 * other kind of nodes with a blue icon.
	 * </p>
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class FieldNameWrapper extends JPanel {
		...
		/**
		 * <p>
		 * Creates a new <code>FieldNameWrapper.
		 * </p>
		 * 
		 * @param field
		 *            a layer field to be wrappered
		 * @param ancestors
		 *            ancestors of that layer field
		 * @param hasChildren
		 *            <code>true</code> if has children; otherwise
		 *            <code>false</code>
		 */
		public FieldNameWrapper(String field, String[] ancestors,
				boolean hasChildren) {
			super();
		...
		/**
		 * <p>
		 * Determines if this field has children.
		 * </p>
		 * <p>
		 * This is prepared to <i>complex features</i>, but meanwhile that
		 * functionality isn't supported.
		 * </p>
		 * 
		 * @return <code>true</code> if this field has children; otherwise
		 *         <code>false</code>
		 */
		...
		/**
		 * <p>
		 * Gets the name of the field wrappered.
		 * </p>
		 * 
		...
		/**
		 * <p>
		 * Gets the level of this field in the <i>TOC</i> tree.
		 * </p>
		 * <p>
		 * This is prepared to <i>complex features</i>, but meanwhile that
		 * functionality isn't supported.
		 * </p>
		 * 
		 * @return level of this layer in the <i>TOC</i> tree, 0 if it's the
		 *         root
		 */
		public int getLevel() {
			if (ancestors == null)
	...
	/**
	 * <p>
	 * Wrappers a <code>FLayer</code> overwriting the method
	 * <code>toString()</code> in order to would return the name of the
	 * layer.</code>
	 * 
	 * <p>
	 * Also displays icons and label
	 * </p>
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class FLayerWrapper extends JPanel {
		...
		/**
		 * <p>
		 * Creates a new <code>FLayerWrapper</code>.
		 * </p>
		 * 
		 * @param layer
		 *            the layer to be wrappered
		 * @param level
		 *            the level in the tree of the layer wrappered
		 * @param leafIcon
		 *            icon that represents the leaf
		 */
		public FLayerWrapper(FLayer layer, int level, Icon leafIcon) {
			...
			if (layer instanceof FLyrVect) {
				...
				try {
					layerTypeLabel = new JLabel(
							PluginServices.getText(null, "Type")
									+ ": "
									+ FShapeTypeNames
											.getFShapeTypeName(((FLyrVect) layer)
													.getShapeType()));
				} catch (ReadDriverException e) {
					layerTypeLabel = new JLabel(PluginServices.getText(null,
							"Type")
							+ ": "
							+ PluginServices.getText("", "UNKNOWN"));
		...
		/**
		 * <p>
		 * Creates the <code>Spring</code> layout of this component.
		 * </p>
		 */
		private void doSpringLayout() {
			...
			// Make every component 5 pixels away from the component to its
			// left.
		...
		/**
		 * <p>
		 * Gets the level in the tree of the layer wrappered.
		 * </p>
		 * 
	...
	/**
	 * <p>
	 * Cell renderer of the combo box with information of the visible vector
	 * layers in the current active view.
	 * </p>
	 * 
	...
	private class LayersComboBoxCellRenderer implements ListCellRenderer {
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (isSelected) {
				renderer.setForeground(UIManager
	...
	/**
	 * <p>
	 * Cell renderer of the table with information of the fields of a layer.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Sorts the three columns of the <code>model</code> bearing in mind the
	 * <code>ascending</code> order according the second column, which has the
	 * field names.
	 * </p>
	 * 
	 * @param model
	 *            the model to be sort ordered
	 * @param ascending
	 *            <code>true</code> if it's an ascending order,
	 *            <code>false</code> if its descending
	 */
	public void sortFieldColumns(DefaultTableModel model, boolean ascending) {
		...
		Collections.sort(Arrays.asList(secondColData), new ColumnSorter(
				ascending, ColumnSorter.WITHOUT_CASE_SENSITIVE));
		...
		getFieldsTable().getColumnModel().getColumn(1)
				.setCellRenderer(new FieldNameRenderer());
	}

	/**
	 * <p>
	 * Compares two <code>String</code> bearing in mind <i>ascending order</i>
	 * and <i>case sensitive</i>.
	 * </p>
	 * 
	 * @see Comparator
	 * 
	...
	/**
	 * <p>
	 * Inherits from {@link DefaultTableModel DefaultTableModel} adding support
	 * for having different column renderers.
	 * </p>
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class LayerTypesTableModel extends DefaultTableModel {
		...
		/**
		 * <p>
		 * Instances <code>LayerTypesTableModel</code>.
		 * </p>
		 */
		...
		/*
		 * <p>JTable uses this method to determine the default renderer/ editor
	...
	/**
	 * Process that initializes this component, loading the fields of all
	 * visible vector layers in the current active view.
	 * 
	 * @author Pablo Piqueras Bartolomé (pablo.piqueras@iver.es)
	 */
	private class InitQIPanelProcess extends IncrementableProcess {
		 ...
		 *            importation
		 * @param label
		 *            brief of this process, that will be displayed in the
		 *            dialog
		 * @param qIPanel
		 *            reference to the panel that will initialize
		 */
		public InitQIPanelProcess(String title, String label,
				QuickInfoDataSelectionPanel qIPanel) {
			super(title);
		...
		/**
		 * Sets the object that will display the evolution of this loading
		 * process as a progress dialog.
		 * 
		 * @param iTask
		...
		public void setIncrementableTask(IncrementableTask iTask) {
			...
			iTask.getButtonsPanel().setEnabled(ButtonsPanel.BUTTON_ACCEPT,
					false);
		...
		public synchronized void run() {
			...
			try {
				...
				if (text != null) {
					log.addLine(PluginServices.getText(null, "Percent_of_load")
							+ ": " + getPercent());
					log.addLine(text);
				...
				if (percentage == 100) {
					iTask.getProgressPanel().setPercent(100); // Forces setting
																// the progress
																// bar at 100 %
					iTask.getProgressPanel().setLabel(
							PluginServices.getText(null, "Process_finished"));
					iTask.hideWindow();
				}
		 ...
		 *             any exception produced initializing this component
		 */
		public void process() throws InterruptedException {
			...
			try {
				...
				childIcon = PluginServices.getIconTheme().get(
						"field-child-icon");
				if (childIcon != null)
					childIconLabel = new JLabel(childIcon);

				complexIcon = PluginServices.getIconTheme().get(
						"field-complex-icon");
				percentage = 15;
		 ...
		 * (non-Javadoc)
		 * 
		 * @see org.gvsig.gui.beans.incrementabletask.IncrementableListener#
		 * actionCanceled
		...
		public void actionCanceled(IncrementableEvent e) {
			if (percentage < 100) {
				...
				JOptionPane.showMessageDialog(null, PluginServices.getText(
						this, "Process_finished_wont_be_cancelled"),
	...
	/**
	 * As combined tool, in a CompoundBehavior its "paintComponent" method is
	 * always called. The idea is painting nothing to accelerate the MapControl
	 * painting process.
	 * 
	...
	private class AdaptedMouseMovementBehavior extends MouseMovementBehavior {
		 ...
		 * 
		 * @see
		 * com.iver.cit.gvsig.fmap.tools.Behavior.Behavior#paintComponent(java
		 * .awt.Graphics)
		 */
			...
			for (int i = 0; i < cb_ctools.size(); i++) {
				...
				if (behavior instanceof AdaptedMouseMovementBehavior) {
					...
					if (behavior instanceof CompoundBehavior) {
						if (removeQuickInfoTool(mapControl,
	...
	private static boolean removeQuickInfoTool(MapControl mapControl,
