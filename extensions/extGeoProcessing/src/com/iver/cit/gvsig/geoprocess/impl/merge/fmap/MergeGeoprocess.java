...
/* CVS MESSAGES:
 *
 * $Id: MergeGeoprocess.java 11634 2007-05-15 07:24:19Z cesar $
 * $Log$
...
/**
 * This geoprocess is "equivalent" to UNION relational operator. Output layer
 * will have a geometry for each geometry of input layer.
 * 
...
public class MergeGeoprocess extends AbstractGeoprocess {
	...
	public void checkPreconditions() throws GeoprocessException {
		if (inputLayers == null)
			throw new GeoprocessException(
					"No se han especificado las capas a juntar en un merge");
		if (outputSchemaLayer == null)
			throw new GeoprocessException(
					"Merge: no se ha especificado que capa determinara los atributos");
		int shapeType = -1;
		try {
			shapeType = inputLayers[0].getShapeType();
			for (int i = 0; i < inputLayers.length; i++) {
				if (inputLayers[i] == null)
					throw new GeoprocessException("Merge: la capa " + i
							+ " no está definida -null");
				if (inputLayers[i].getShapeType() != shapeType)
					throw new GeoprocessException(
							"Las capas a juntar deben tener el mismo tipo de geometria");
			}
		} catch (ReadDriverException e) {
			throw new GeoprocessException(
					"Error al tratar de determinar el tipo de geometria de las capas a mezclar");
		}
	}
	...
	public void process() throws GeoprocessException {
		try {
			...
			throw new GeoprocessException(
					"Error al ejecutar el geoproceso merge");
		}
	}
	...
	public ILayerDefinition createLayerDefinition() {
		if (this.resultLayerDefinition == null) {
			try {
				this.resultLayerDefinition = DefinitionUtils
	...
	class MergeMonitorableTask implements IMonitorableTask {
		...
		String MERGE_NOTE = PluginServices.getText(this,
		...
		public String getNote() {
			return MERGE_NOTE + " " + getCurrentStep() + " " + OF + " "
					+ getFinishStep();
		}
		...
		public void run() throws GeoprocessException {
			DeferredFeaturePersisterProcessor processor = new DeferredFeaturePersisterProcessor(
					writer);
			try {
				writer.preProcess();
				...
				finished = true;
				throw new GeoprocessException(
						"Error al preparar la capa resultado");
			}
			...
			for (int i = 0; i < inputLayers.length; i++) {
				...
				try {
					...
					finished = true;
					throw new GeoprocessException(
							"Merge: error al leer la capa "
									+ inputLayers[i].getName());
				} catch (ProcessVisitorException e) {
					finished = true;
					throw new GeoprocessException(
							"Merge: error al procesar la capa "
									+ inputLayers[i].getName());
