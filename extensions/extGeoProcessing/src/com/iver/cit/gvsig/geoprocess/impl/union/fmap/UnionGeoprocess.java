...
/**
 * Union geoprocess is known like "spatial or" because is an overlay geoprocess
 * (it computes intersections between features of two layers) formed for
 * geometries of one layer or another. <br>
 * Algorithm makes these three passes: a) computing intersections and saves them
 * in a temp file. b) computing differences with first layer. c) computing
 * differences with second layer.
 * 
 * @author azabala
 * 
 *         TODO Is very similar to ClipGeoprocess. Build an overlay abstract
 *         class.
 * 
 * 
 */
public class UnionGeoprocess extends AbstractGeoprocess implements
		IOverlayGeoprocess {
	...
	public void checkPreconditions() throws GeoprocessException {
		...
		if (this.writer == null || this.schemaManager == null) {
			throw new GeoprocessException(
					"Operacion de union sin especificar capa de resultados");
		}
		 ...
		 * GeoprocessException("Primera capa de union no es de polígonos"); }
		 * if(overlayLayer.getShapeType() != XTypes.POLYGON &&
		 * overlayLayer.getShapeType() != XTypes.MULTI){ throw new
		 * GeoprocessException("Segunda capa de union no es de polígonos"); } }
		 * catch (ReadDriverException e) { throw new GeoprocessException(
		 * "Error al tratar de chequear si las capas a unir son de polígonos");
		 * }
		 */
	...
	class UnionMonitorableTask implements IMonitorableTask {
		...
		String FIRST_PASS_NOTE = PluginServices.getText(this,
				"Mensaje_primera_pasada_union");
		String SECOND_PASS_NOTE = PluginServices.getText(this,
				"Mensaje_segunda_pasada_union");
		String THIRD_PASS_NOTE = PluginServices.getText(this,
		...
		public String getNote() {
			return currentMessage + " " + getCurrentStep() + " " + OF + " "
					+ getFinishStep();
		}
		...
		public void run() throws GeoprocessException {
			/*
			 * Lo vamos a hacer en tres pasadas: a) intersecciones con el
			 * geoproceso Intersection, guardando en el fichero del usuario. b)
			 * diferencias de first con overlay, guardando en el mismo fichero
			 * c) diferencias de overlay con first, guardando en el mismo
			 * fichero
			 */
			try {
				...
				DeferredFeaturePersisterProcessor featureProcessor = new DeferredFeaturePersisterProcessor(
						writer);
				...
				DifferenceVisitor visitor2 = new DifferenceVisitor(
						overlayLayer, featureProcessor, strategy2,
				...
				if (onlyClipLayerSelection) {
					FBitSet selection = overlayLayer.getRecordset()
							.getSelection();
