...
public class GeoProcessingMergePanel2 extends GridBagLayoutPanel implements
	...
	private void initialize() {
		...
		final String titleText = PluginServices.getText(this,
				"Juntar._Introduccion_de_datos") + ":";
		addComponent(new JLabel(titleText), insets);

		final String inputLayerText = PluginServices.getText(this,
		...
		final LayerSelectorListener layerSelectorListener = new LayerSelectorListener(
				layers);
		...
		final String directoryLayersText = PluginServices.getText(this,
				"Capas_del_directorio") + ":";
		directoryLyrsLabel = new JLabel(directoryLayersText);
		addComponent(directoryLyrsLabel, insets);

		final JButton directoryButton = new JButton(PluginServices.getText(
				this, "Seleccionar_Directorio"));
		...
		final AditionalLayerSelectorListener listener = new AditionalLayerSelectorListener(
				aditionalLayers);
		...
		final JLabel resultSchemaLabel = new JLabel(PluginServices.getText(
				this, "Usar_los_campos_de_la_capa") + ":");

		fieldsJComboBox = new JComboBox();
		addComponent(resultSchemaLabel, fieldsJComboBox,
		...
		final String resultLayerText = PluginServices.getText(this,
				"Cobertura_de_salida") + ":";
	...
	public FLyrVect[] loadLayersInDirectory(final File file,
		...
		for (int i = 0; i < inputLayersToAdd.length; i++) {
			...
			try {
				...
				// we catch any possible (and not declared) exception, because
				// we dont know
				// the correctness of file layers (for example, shp without dbf,
				// etc)
				e.printStackTrace();
			}
		}// for
		if (errors.size() > 0) {
			String errorMsg = PluginServices.getText(this,
	...
	public void layersSelected() {
		...
		final AditionalLayerSelectorListener aditionalListener = new AditionalLayerSelectorListener(
				aditionalLayers);
		directoryLayerList.addListSelectionListener(aditionalListener);
		final AditionalLayersListModel aditionalListModel = new AditionalLayersListModel(
				aditionalLayers);
	...
	private class LayerSelectorListener implements ListSelectionListener {
		...
		public void valueChanged(final javax.swing.event.ListSelectionEvent e) {
			final Object[] layersSelectedNames = inputLayersList
					.getSelectedValues();
	...
	private class AditionalLayerSelectorListener implements
		...
		public void valueChanged(final javax.swing.event.ListSelectionEvent e) {
			final Object[] layersSelectedNames = directoryLayerList
					.getSelectedValues();
			if (layersSelectedNames == null) {
				return;
			}
		...
		public Object getElementAt(final int arg0) {
			if (arg0 < layers.getLayersCount()) {
				...
				if (aditionalLayers == null) {
					...
					final FLyrVect layer = aditionalLayers[arg0
	...
	public IMonitorableTask askForSpatialIndexCreation(final FLyrVect layer) {
		...
		final String confirmDialogText = PluginServices.getText(this,
		...
		final int option = JOptionPane.showConfirmDialog(this,
				confirmDialogText, title, JOptionPane.YES_NO_OPTION,
		...
		if (option == JOptionPane.YES_OPTION) {
			...
			try {
				final CreateSpatialIndexMonitorableTask task = new CreateSpatialIndexMonitorableTask(
						layer);
				return task;
	...
	public boolean askForOverwriteOutputFile(final File outputFile) {
		final String title = PluginServices.getText(this,
