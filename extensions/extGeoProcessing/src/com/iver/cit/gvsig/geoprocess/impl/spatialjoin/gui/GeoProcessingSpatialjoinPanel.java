...
public class GeoProcessingSpatialjoinPanel extends AbstractGeoprocessPanel
	 ...
	 */
	/*
	 * public GeoProcessingSpatialjoinPanel() { super(); initialize(); }
	 */
	/**
	...
	private void changeSelectedItemsJCheckBox1() {
		FLyrVect inputSelectable = (FLyrVect) (layers
				.getLayer((String) secondLayerCombo.getSelectedItem()));
		FBitSet fBitSet = null;
		try {
			fBitSet = inputSelectable.getRecordset().getSelection();
		} catch (ReadDriverException e) {
	...
	private void changeSelectedItemsNumberJLabel1() {
		if (getSecondSelectionCheck().isSelected()) {
			...
			ReadableVectorial va = ((FLyrVect) layers.getLayer(selected))
					.getSource();
			try {
	...
	public JTextField getFileNameResultTextField() {
		if (fileNameResultTextField == null) {
			super.getFileNameResultTextField().setBounds(
					new java.awt.Rectangle(139, 18, 175, 18));
		}
	...
	private JCheckBox getFirstSelectionCheck() {
		if (firstSelectionCheck == null) {
			...
			firstSelectionCheck.setText(PluginServices.getText(this,
					"Usar_solamente_los_elementos_seleccionados"));
			firstSelectionCheck
					.setBounds(new java.awt.Rectangle(6, 31, 320, 24));
			firstSelectionCheck
					.addItemListener(new java.awt.event.ItemListener() {
						public void itemStateChanged(java.awt.event.ItemEvent e) {
							firstLayerSelectionChecked(firstSelectionCheck
									.isSelected());
						}
					});
		}
	...
	private JComboBox getSecondLayerCombo() {
		if (secondLayerCombo == null) {
			...
			secondLayerCombo
					.setBounds(new java.awt.Rectangle(185, 20, 238, 25));
			DefaultComboBoxModel defaultModel = new DefaultComboBoxModel(
					getLayerNames());
	...
	private JCheckBox getSecondSelectionCheck() {
		if (secondSelectionCheck == null) {
			...
			secondSelectionCheck.setText(PluginServices.getText(this,
					"Usar_solamente_los_elementos_seleccionados"));
			secondSelectionCheck.setBounds(new java.awt.Rectangle(7, 79, 300,
					24));
			secondSelectionCheck
					.addItemListener(new java.awt.event.ItemListener() {
						public void itemStateChanged(java.awt.event.ItemEvent e) {
							secondLayerSelectionChecked(secondSelectionCheck
									.isSelected());
						}
					});
		}
	...
	public FLyrVect getFirstLayer() {
		return (FLyrVect) layers.getLayer((String) firstLayerCombo
				.getSelectedItem());
	}

	public FLyrVect getSecondLayer() {
		return (FLyrVect) layers.getLayer((String) secondLayerCombo
				.getSelectedItem());
	}
	...
	private JPanel getTargetLayerPanel() {
		if (targetLayerPanel == null) {
			...
			jLabel5.setText(PluginServices.getText(this,
