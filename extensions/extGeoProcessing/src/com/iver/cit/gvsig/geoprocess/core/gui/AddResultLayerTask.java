...
/**
 * Implementation of IMonitorableTask that add result layer of a geoprocess to
 * the TOC in a background thread
 * 
 * @author azabala
 * 
 */
public class AddResultLayerTask implements IMonitorableTask {
	...
	FLayers layers;
	/**
	 * These are fictitial steps of adding layer to TOC task, to get
	...
	private boolean checkToAdd(FLayer layer) throws GeoprocessException {
		try {
			if (layer instanceof FLyrVect) {
				...
				// TODO Verificar esto si queremos meter en geoprocessing capas
				// raster
				return false;
		...
		try {
			...
			if (checkToAdd(result)) {
				if (result instanceof FLayers) {// TODO PRUEBA
					...
					/*
					 * Checks if root FLayers has these FLayers. If not, adds
					 * childs layers. If true, doesnt add nothing (patch to work
					 * with topology project and referencing module with
					 * auxiliar layers. TODO Re-study
					 */
