...
public class SpatialJoinGeoprocessController extends
	...
	public void setView(IGeoprocessUserEntries viewPanel) {
		this.geoProcessingSpatialjoinPanel = (SpatialJoinPanelIF) viewPanel;
	}
	...
	public boolean launchGeoprocess() {
		...
		if (XTypes.getNumericFieldsNames(secondLayer).length > 0) {
			if (!geoProcessingSpatialjoinPanel.isNearestSelected()) {
				boolean isOk = geoProcessingSpatialjoinPanel
				...
				if (sumarizeFunctions == null || sumarizeFunctions.size() == 0) {
					geoProcessingSpatialjoinPanel.error(PluginServices.getText(
							this, "Error_spjoinmn_sin_funcion"), PluginServices
		...
		if (outputFile.exists()) {
			if (!geoProcessingSpatialjoinPanel
		...
		if (!nearest) {
			if (!secondLayer.isSpatiallyIndexed()) {
				final IMonitorableTask sptIdxTask = geoProcessingSpatialjoinPanel
