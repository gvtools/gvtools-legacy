 ...
 * strategy (in constract with NearestSpatialJoinVisitor, that does a secuential
 * scanning). <br>
 * Which heuristic does this visitor apply? It obtains second layer (target
 * layer in spatial join) full extent, and subdivide it in 4 envelopes. After
 * that, it computes the distance of the geometry we want to join with second
 * layer, and computes 4 distances with each one of the envelopes. Then, it
 * recursively subdivide the Envelope at the shortest distance. <br>
 * This process is repeated recursively until we obtain a nearest envelope of a
 * parametrized dimension. After that, it makes a spatial query with this
 * envelope on the layer B. If this query doesnt return, repeat the spatial
 * query with the envelope that originated this envelope (we take the parent
 * node in the quad-tree structure). <br>
 * A critical aspect is optimization of the number of levels of quad-tree.
 * 
 * If we take very few levels, the spatial query will return a lot of candidates
 * to nearest, so we wont get advantage of this stuff.
 * 
 * If we take a lot of levels, we wont get result in the spatial queries, and
 * we'll have to do a lot of querys.
 * 
 * @author azabala
 * 
 *         FIXME EL ALGORITMO FALLA!!!!!!!! EL QUADTREE ES UNA ESTRUCTURA BUENA
 *         PARA RECTANGULOS, PERO PARA PUNTOS CREO QUE NO FUNCIONA. NO TIENE EN
 *         CUENTA LOS EXTREMOS DE LOS RECTANGULOS
 * 
 */
public class NearestHeuristicSpatialJoinVisitor extends
		NearestSpatialJoinVisitor {

	private QuadTreeUtil quadTree = new QuadTreeUtil();
	/**
	 * Full extent of the layer where we are looking for features to join by
	 * spatial criteria
	 */
	...
	public NearestHeuristicSpatialJoinVisitor(FLyrVect sourceLayer,
		...
		targetLayerEnv = new Envelope(rect.getMinX(), rect.getMaxX(),
	...
	class LookForNearest implements FeatureVisitor {
		...
		/**
		 * min distance of the features processed in the search of nearest
		 * feature
		 */
		...
		Geometry firstG;
		/**
		 * It this selectin is != null, in our search we will only consideer
		 * features selected.
		 */
		FBitSet selection;
	...
	public void visit(IGeometry g, int sourceIndex) throws VisitorException,
		...
		Stack stackOfEnvelopes = quadTree.getNearestEnvelopeOfIdealDimension(
				geometry, targetLayerEnv);
		...
		while ((stackOfEnvelopes.size() > 0)) {
			...
			try {
				...
				throw new ProcessVisitorException(targetRecordset.getName(), e,
						"Error accediendo a los datos buscando el feature mas proximo");
			}
		}// while
