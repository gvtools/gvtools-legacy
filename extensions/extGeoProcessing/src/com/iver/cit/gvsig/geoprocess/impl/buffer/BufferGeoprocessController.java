 ...
 * 
 */
public class BufferGeoprocessController extends AbstractGeoprocessController {
	/**
	 * UI class to read user entries. By using UI interfaces, we are allowing
	 * geoprocessing work with GUI and Command Line UI.
	 */
	...
	public boolean launchGeoprocess() {
		...
		try {
			...
			final String error = PluginServices.getText(this,
					"Error_entrada_datos");
			final String errorDescription = PluginServices.getText(this,
					"Error_seleccionar_resultado");
			bufferPanel.error(errorDescription, error);
			return false;
		}
		if ((outputFile == null)
				|| (outputFile.getAbsolutePath().length() == 0)) {
			final String error = PluginServices.getText(this,
					"Error_entrada_datos");
			final String errorDescription = PluginServices.getText(this,
					"Error_seleccionar_resultado");
		...
		} else if (typePolygonBuffer
				.equals(BufferPanelIF.BUFFER_INSIDE_OUTSIDE)) {
		...
		final int distanceUnits = view.getMapContext().getViewPort()
				.getDistanceUnits();
		...
		try {
			...
			final String error = PluginServices
					.getText(this, "Error_ejecucion");
			final String errorDescription = PluginServices.getText(this,
		...
		final SHPLayerDefinition definition = (SHPLayerDefinition) buffer
				.createLayerDefinition();
		definition.setFile(outputFile);
		final ShpSchemaManager schemaManager = new ShpSchemaManager(
		...
		try {
			...
			final MonitorableDecoratorMainFirst globalTask = new MonitorableDecoratorMainFirst(
					task1, task2);
