...
/* CVS MESSAGES:
 *
 * $Id: BufferVisitor.java 32916 2010-07-01 09:12:40Z vcaballero $
 * $Log$
...
/**
 * Abstract base class to those all FeatureVisitor that computes geometry
 * buffers.
 * 
 * 
 * @author azabala
...
public abstract class BufferVisitor implements FeatureVisitor {
	...
	/**
	 * type of buffer to compute (polygons could have the three types, point and
	 * lines only outside buffer.
	 */
	...
	/**
	 * Flag to decide if simplify or not the input geometries. Actually always
	 * is true, but it could change
	 */
	...
	/**
	 * Verifys if a geometry buffer is null. It is useful when you're working
	 * with internal buffers. If the internal buffer distance is greater than
	 * the geometry radius, the buffer result will be null.
	 * 
	 * @param newGeometry
	 * @return
	 */
	...
	/**
	 * 
	 * Compute a buffer (in function of the params) of the original geometry
	 * 
	...
	public void computeBuffer(Geometry originalGeometry, double bufferDistance,
			int index) throws VisitorException {
		Geometry solution = null;
		Geometry inputParam = originalGeometry;
		/*
		 * When we try to apply large buffer distances, we could get
		 * OutOfMemoryError exceptions. Explanation in
		 * http://lists.jump-project.
		 * org/pipermail/jts-devel/2005-February/000991.html
		 * http://lists.jump-project
		 * .org/pipermail/jts-devel/2005-September/001292.html This problems
		 * hasnt been resolved in JTS 1.7.
		 */
		if (originalGeometry.getDimension() != 0 && simplifyGeometry) {
			// ver si 1/10 de la distancia de buffer
			// es un % de simplificación adecuado
			inputParam = TopologyPreservingSimplifier.simplify(
					originalGeometry, bufferDistance / 10d);
		}
		...
		if (typeOfBuffer == BUFFER_INSIDE_POLY) {
			...
			for (int i = numberOfRadialBuffers; i >= 1; i--) {
				...
				if (previousInteriorRing != null) {
					...
					solution = JTSFacade.difference(newGeometry,
							previousInteriorRing);
				} else {
			...
			for (int i = 1; i <= numberOfRadialBuffers; i++) {
				...
				if (previousExteriorRing == null
					...
					if (collapsedInterior) {
						...
						solution = JTSFacade.difference(out,
								previousExteriorRing);
					} else {
						// Geometry outRing =
						// out.difference(previousExteriorRing);
						// Geometry inRing =
						// previousInteriorRing.difference(in);
						Geometry outRing = JTSFacade.difference(out,
								previousExteriorRing);
						Geometry inRing = JTSFacade.difference(
						...
						stack.push(solution);
						while (stack.size() != 0) {
							GeometryCollection geCol = (GeometryCollection) stack
									.pop();
						...
						MultiPolygon newSolution = geomFact
								.createMultiPolygon(pols);
				...
				IFeature feature = null;
				try {
					feature = createFeature(solution, -1 * distRing, distRing,
							index);
				} catch (ReadDriverException e) {
					e.printStackTrace();
				}
	...
	/**
	 * Creates a feature for a simple buffer process for polygons (the result
	 * feature will have a field with the buffer distance)
	 * 
	 * @param jtsGeo
	 * @param distance
	 * @return
	 ...
	 */
	protected IFeature createFeature(Geometry jtsGeo, double distance, int index)
			throws ReadDriverException {
	 ...
	 * @throws ReadDriverException
	 */
	protected IFeature createFeature(Geometry jtsGeo, double distanceFrom,
			double distanceTo, int index) throws ReadDriverException {
		IGeometry iGeo = FConverter.jts_to_igeometry(jtsGeo);
		Value[] values = null;
		if (!dissolveBuffers) {
			int fieldCount = recordset.getFieldCount();
	...
	/**
	 * Sets BufferResultProcessor, class that has the responsability of process
	 * individual JTS buffered geometries (save it, to cache it, etc)
	 * 
	 * @param processor
	 */
