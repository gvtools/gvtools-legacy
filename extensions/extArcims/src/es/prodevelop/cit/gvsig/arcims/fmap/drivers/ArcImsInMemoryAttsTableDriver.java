...
public class ArcImsInMemoryAttsTableDriver {
	...
	// Logger.getLogger(FMapFeatureArcImsDriver.class.getName());
	public ArcImsInMemoryAttsTableDriver(String tb_name, ArrayList colnames,
			ArrayList coltypes, FMapFeatureArcImsDriver p,
			ArcImsAttributesDataSourceAdapter pad) {
		...
		idSqlFieldName = replaceUnwantedCharacters((String) colnames
				.get(idFieldInd));

		requested = new FBitSet();

		for (int i = 0; i < colnames.size(); i++) {
			sqlColumnNames.add(replaceUnwantedCharacters((String) colnames
					.get(i)));
	...
	// public void addRow(DataSourceFactory dsf, Value[] values) throws
	// DriverException {
	...
	// public void deleteRow(DataSourceFactory dsf, int i) throws
	// DriverException {
	...
	public Value doGetFieldValue(long row, int colind) {
		...
		if (therow[colind] == null) {
			logger.warn("In order to avoid returning element [ "
					+ row
					+ ", "
					+ colind
	...
	/**
	 * Utility methods to avoid issue when server declares attribute names which
	 * contain dot (.) Driver keeps two versions: name used with server
	 * (original) and name used by user (because you cannot use points because
	 * they are not valid in logical expressions)
	 */
