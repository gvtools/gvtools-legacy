...
public class LayerScaleData {
	private static Logger logger = Logger.getLogger(LayerScaleData.class
			.getName());
	...
	/**
	 * Gets the color depending on the type of layer and whether the current
	 * scale is inside the limits or not.
	 * 
	 * @param current
	 *            current scale
	...
	public static long getTrueScaleFromRelativeScaleAndMapUnits(double rs,
		...
		double fromMetersPerPixelToTrueScale = theDpi
				/ ArcImsProtocolHandler.INCHES;
		...
		if (units
				.compareToIgnoreCase(ServiceInfoTags.vMAP_UNITS_DECIMAL_DEGREES) == 0) {
