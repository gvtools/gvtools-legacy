...
public class ArcImsPropsDialog extends JPanel implements IWindow,
	...
	private static Logger logger = Logger.getLogger(ArcImsPropsDialog.class
			.getName());
	...
	private void initialize(FLayer layer) {
		...
		if (layer instanceof FRasterLyrArcIMS) {
			arcImsWizard = getArcImsWizard(
					((FRasterLyrArcIMS) layer).getProperties(), true);
			theServicePanel = arcImsWizard.getImgServicePanel();
		} else {
			if (layer instanceof FFeatureLyrArcIMS) {
				arcImsWizard = getArcImsWizard(
						((FFeatureLyrArcIMS) layer).getProperties(), false);
	...
	private void getNewLayerConfig() {
		...
		if (fLayer instanceof FFeatureLyrArcIMS) {
			((FFeatureLyrArcIMS) fLayer).setLayerQuery(arcImsWizard
					.getLayerQuery());
			((FFeatureLyrArcIMS) fLayer)
					.setName(arcImsWizard.getNewLayerName());
		}
	}
