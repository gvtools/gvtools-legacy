...
public class FFeatureLyrArcIMS extends FLyrVect implements InfoByPoint,
	...
	private static Logger logger = Logger.getLogger(FFeatureLyrArcIMS.class
			.getName());
	private URL host;
	...
	// private SelectableDataSource sds; --> is created like this --> new
	// SelectableDataSource(source.getRecordSet())
	...
	private void _draw(BufferedImage image, Graphics2D g, ViewPort viewPort,
		...
		if (lgnd instanceof IClassifiedVectorLegend) {
			...
			_legflds = ((IClassifiedVectorLegend) lgnd)
		...
		try {
			...
			ReadDriverException de = new ReadDriverException(
					"While getting map (ArrayList of geometries) ", e);
			throw de;
		}
		// restore dubfields
		arcimsStatus.setSubfields(savedFieldNames);
		...
		if (bDrawShapes) {
			...
			try {
				...
				IFeatureIterator it = new AvoidGeometryAndIdFeatureIterator(
						geometries, savedFieldNames.length);
				...
				while (!cancel.isCanceled() && it.hasNext()) {
					...
					FMapFeatureArcImsDriver imsdrv = (FMapFeatureArcImsDriver) getSource()
							.getDriver();
	...
	public String queryByPoint(Point p) throws ReadDriverException {
		...
		try {
			((ArcImsVectorialAdapter) getSource())
					.requestFeatureAttributes(fbs);
		} catch (ArcImsException e) {
			ReadDriverException de = new ReadDriverException(
					"While querying by point ", e);
			throw de;
		}
	...
	private String getFormattedFieldsFromFeature(int j) {
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				fieldName = ArcImsInMemoryAttsTableDriver
						.replaceUnwantedCharacters(fieldName);
			...
			// <FIELDS NOMBRE="LR-542" OBSERVACIO="" TIPO="CAUT3"
			// _SHAPE_="[Geometry]" _ID_="1975" />
			r = r + "/>";
		} catch (ReadDriverException e) {
	...
	public void setSubfieldsInStatus() {
		...
		ServiceInformationLayerFeatures silf = (ServiceInformationLayerFeatures) arcimsStatus
				.getServiceInfo().getLayerById(id);
		FieldInformation fi = (FieldInformation) silf
				.getFieldInformationByType(FieldInformation.ID).get(0);
		subfields[1] = fi.getName();
		fi = (FieldInformation) silf.getFieldInformationByType(
				FieldInformation.SHAPE).get(0);
	...
	public void setXMLEntity(XMLEntity xml) throws XMLException {
		...
		FMapFeatureArcImsDriver drv = new FMapFeatureArcImsDriver(
				host.toString(), service, layerQuery);
		...
		try {
			fext = ((ArcImsFeatureClient) drv.getClient())
					.getLayerExtent(arcimsStatus);
		} catch (Exception e) {
		...
		try {
			setRecordset(((FMapFeatureArcImsDriver) getSource().getDriver())
					.getRecordSet());
		} catch (Exception e) {
	 ...
	 *            "service_name", String (remote service name) "srs", String
	 *            (coordinate system) "layer_id", String (*single* remote
	 *            layer's ID) "layer_name", String (local layer name)
	 * 
	 ...
	 * try {
	 * 
	 * String _layer_id = (String) m.get("layer_id"); String _service_name =
	 * (String) m.get("service_name"); String _layer_name = (String)
	 * m.get("layer_name"); String _srs = (String) m.get("srs");
	 * 
	 * // ----------------------------------
	 * 
	 ...
	 * 
	 * 
	 * FMapFeatureArcImsDriver _drv = new
	 * FMapFeatureArcImsDriver(_true_host.toString(), _service_name, _layer_id);
	 ...
	 * 
	 * setProjectionInStatus(_true_srs.getAbrev()); setHostInStatus(_true_host);
	 * setServiceInStatus(_service_name);
	 * 
	 * String _units = _si.getMapunits(); int _theDpi = _si.getScreen_dpi();
	 * long _scale; if (_silf.getMaxscale()!=-1) { _scale =
	 * LayerScaleData.getTrueScaleFromRelativeScaleAndMapUnits(
	 * _silf.getMaxscale(), _units, _theDpi); setMaxScale((double) _scale); } if
	 * (_silf.getMinscale()!=-1) { _scale =
	 * LayerScaleData.getTrueScaleFromRelativeScaleAndMapUnits(
	 * _silf.getMinscale(), _units, _theDpi); setMinScale((double) _scale); }
	 * 
	 * setServiceInformationInStatus(_si); Vector _ids = new Vector();
	 * _ids.add(_layer_id); setLayerIdsInStatus((Vector) _ids.clone());
	 * setSubfieldsInStatus();
	 * 
	 * setHost(_true_host); setService(_service_name);
	 * setServiceType(ServiceInfoTags.vFEATURESERVICE); setTransparency(0);
	 * setLayerQuery(_layer_id); setProjection(_true_srs); setName(_layer_name);
	 * 
	 * Rectangle2D _fext = ((ArcImsFeatureClient)
	 * _drv.getClient()).getLayerExtent(getArcimsStatus());
	 * _drv.setFullExtent(_fext); // individualLayers[i].setF.
	 * setFullExtent(((ArcImsProtImageHandler)
	 ...
	 * _adapter.setOriginalVectorialAdapter(_oldadapter); _drv.declareTable();
	 * 
	 * setSource(_adapter); _adapter.setDriver(_drv);
	 * 
	 * getSource().setDriver(_drv); setInitialLegend();
	 * setShapeType(_adapter.getShapeType()); setRecordset(_drv.getRecordSet());
	 * // ------ -------------
	 * 
	 * if ((_si.getFeaturecoordsys() == null) ||
	 * (_si.getFeaturecoordsys().equals(""))) {
	 * _si.setFeaturecoordsys(_true_srs.
	 * getAbrev().substring(ServiceInfoTags.vINI_SRS.length()).trim());
	 * logger.warn("Server provides no SRS. "); } } catch (Exception e) { throw
	...
	private class AvoidGeometryAndIdFeatureIterator implements IFeatureIterator {
		...
		public IFeature next() throws ReadDriverException {
			index++;
			if (index > size) {
				logger.error("Out of index in AuxFeatureIterator: "
						+ (index - 1));
				return null;
			} else {
