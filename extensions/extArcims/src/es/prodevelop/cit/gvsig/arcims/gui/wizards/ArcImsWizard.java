...
public class ArcImsWizard extends WizardPanel {
	private static Logger logger = Logger.getLogger(ArcImsWizard.class
			.getName());
	...
	private FFeatureLyrArcIMSCollection createArcImsFeatureLayer(boolean sep) {
		...
		try {
			layer = dataSource.createArcImsFeatureLayer(host, serviceName,
					layerQuery, nombreTema, getMapCtrl().getMapContext()
							.getCrs(), sep);
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	...
	/**
	 * Creates an ArcIMS raster layer. This method will use the ArcImsWizardData
	...
	private FRasterLyrArcIMS createArcImsRasterLayer() {
		...
		try {
			// the value of these parameters was set by other gui objects
			// (panels, etc)
		...
		// boolean imagefmtok = ((FMapRasterArcImsDriver)
		// layer.getDriver()).testFormat(status, fmt);
		// if (!imagefmtok) {
	...
	private void fillImageServiceTab() {
		FMapRasterArcImsDriver drv = (FMapRasterArcImsDriver) this.dataSource
				.getDriver();
		...
		this.imgServicePanel
				.setDetailsPanelServiceNameInBorder(this.serviceName);
		...
		for (int i = 0; i < si.getLayers().size(); i++) {
			if (si.getLayers().get(i) instanceof ServiceInformationLayerImage) {
				ServiceInformationLayerImage sii = (ServiceInformationLayerImage) si
						.getLayers().get(i);
				this.imgServicePanel
						.addLayerToAvailableList(new LayersListElement(sii,
								map_units, _dpi));
			}
			...
			if (si.getLayers().get(i) instanceof ServiceInformationLayerFeatures) {
				ServiceInformationLayerFeatures sif = (ServiceInformationLayerFeatures) si
						.getLayers().get(i);
				this.imgServicePanel
						.addLayerToAvailableList(new LayersListElement(sif,
								map_units, _dpi));
			}
		}
	...
	private void fillFeatureServiceTab() {
		FMapFeatureArcImsDriver drv = (FMapFeatureArcImsDriver) this.dataSource
				.getDriver();
		ArcImsClientP cli = (ArcImsClientP) drv.getClient();
