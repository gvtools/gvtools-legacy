...
public class ArcImsAttributesDataSourceAdapter implements
		ArcImsAlphanumericDataSource {
	private static Logger logger = Logger
			.getLogger(ArcImsAttributesDataSourceAdapter.class.getName());
	...
	public ArcImsAttributesDataSourceAdapter(ArrayList col_Names,
		...
		ArcImsInMemoryAttsTableDriver dataDriver = new ArcImsInMemoryAttsTableDriver(
				tableName, col_Names, col_Types, parentDriver, this);
		...
		try {
			...
			// String sqlStr =
			// ArcImsSqlUtils.getSqlSentenceCreateTableWithFieldsAndTypes(tableName,
			// col_Names, col_Types, dataDriver.getRowIndexFieldName());
			...
			for (int i = 0; i < ids.length; i++) {
				...
				// dataDriver.addInIndexToIdHashMap(rwcount, ((IntValue)
				// id).intValue());
				// logger.debug("Adding id = " + ((IntValue) id).intValue() +
				// " in row = " + rwcount);
				dataDriver.addInIdToIndexHashMap(((IntValue) id).intValue(),
						rwcount);

				// dsFactory.executeSQL(sqlStr,
	...
	public long[] getWhereFilter() throws IOException {
		...
		// String message =
		// "'Where' filter not used in method: ArcImsAttributesDataSourceAdapter.getWhereFilter()";
	...
	public ValueCollection getPKValue(long arg0) throws ReadDriverException {
		...
		try {
			...
			ReadDriverException ex = new ReadDriverException(
					"While doing getPKValue: " + e.getMessage(), e);
			throw ex;
		}
	...
	public String getPKName(int arg0) throws ReadDriverException {
		if (arg0 == 0) {
			...
			ReadDriverException de = new ReadDriverException("pk_error",
					new Exception("In get PK name"));
	...
	public Value[] getRow(long arg0) throws ReadDriverException {
		...
		if (parentDriver.getDataTable().isNonRequestedRow((int) arg0)) {
			...
			FBitSet fbs = parentDriver.getDataTable()
					.getThisAndNonRequestedUpTo((int) arg0, rowsPerRequest);
		...
		try {
			...
			ReadDriverException de = new ReadDriverException(e.getMessage(),
					new Exception());
			logger.error("While getting row ", de);
			throw de;
		}
	...
	public Value getFieldValue(long row, int colind) throws ReadDriverException {
		if (parentDriver.getDataTable().isNonRequestedRow((int) row)) {
			...
			// FBitSet fbs = dataDriver.getThisAndNonRequestedUpTo((int) row,
			// rowsPerRequest);
			try {
	...
	public void updateRow(Value[] new_row, String[] fld_query, int rowind)
		...
		// || ((new_row.length == getFieldCount()) &&
		// (fld_query[0].compareTo("#ALL#") == 0)))) {
	...
	public int getRowId(int row) throws DriverException {
		...
		// System.out.println("dataDriver.getRowId(dsFactory, " + row + ") = " +
		// dataDriver.getRowId(dsFactory, row));
		// return 0;
	...
	public BitSet getNonRequestedFromHere(int start, int total) {
		return parentDriver.getDataTable().getThisAndNonRequestedUpTo(start,
				total);
	}
