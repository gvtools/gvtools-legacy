...
public class Annotation_TaskCreate extends AbstractMonitorableTask {
	...
	public void run() throws Exception {
		...
		} else if (duplicate.equals(PluginServices.getText(this,
				"duplicate.none"))) {
			va.start();
					...
					if (geom != null) {
						if (geom.getGeometryType() != FShape.POINT) {
							Point2D p = FLabel.createLabelPoint((FShape) geom
									.getInternalShape());
							geom = ShapeFactory.createPoint2D(p.getX(),
									p.getY());
						}
		...
		if (reader != null) {
			...
			if (res == JOptionPane.YES_OPTION) {
				...
				Annotation_Layer la = Annotation_Layer
						.createLayerFromVect((FLyrVect) newLayer);
				// LayerListener[] layerListeners =
				// newLayer.getLayerListeners();
				//
		...
		try {
			if (name == Annotation_Mapping.TEXT) {
				if (pos != -1) {
					...
					if (s != null && !s.equals("")) {
						...
						values[i] = ValueFactory
								.createValue(Annotation_Mapping.DEFAULTTEXT);
					}

				} else {
					values[i] = ValueFactory.createValueByType(
							String.valueOf(Annotation_Mapping.DEFAULTTEXT),
							type);
				}
			} else if (name == Annotation_Mapping.COLOR) {
				if (pos != -1) {
					values[i] = ValueFactory
							.createValue(((NumericValue) antValues[pos])
									.intValue());
				} else {
					values[i] = ValueFactory.createValueByType(
							String.valueOf(Annotation_Mapping.DEFAULTCOLOR),
							type);
				}
				...
				if (pos != -1) {
					...
					values[i] = ValueFactory.createValueByType(
							String.valueOf(Annotation_Mapping.DEFAULTHEIGHT),
							type);
				}
			} else if (name == Annotation_Mapping.TYPEFONT) {
				if (pos != -1) {
					values[i] = ValueFactory.createValue(antValues[pos]
							.toString());
				} else {
					values[i] = ValueFactory.createValueByType(
							String.valueOf(Annotation_Mapping.DEFAULTTYPEFONT),
							type);
				}
				...
				if (pos != -1) {
					...
					values[i] = ValueFactory
							.createValueByType(
									String.valueOf(Annotation_Mapping.DEFAULTSTYLEFONT),
									type);
				}
				...
				if (pos != -1) {
					...
					values[i] = ValueFactory.createValueByType(
							String.valueOf(Annotation_Mapping.DEFAULTROTATE),
							type);
				}
			}
	 ...
	 *             DOCUMENT ME!
	 */
	private void duplicate() throws InitializeDriverException,
	...
	private void modifyMap(IGeometry geom, int index)
		...
		// textValue=Annotation_Mapping.DEFAULTTEXT;
		// }
