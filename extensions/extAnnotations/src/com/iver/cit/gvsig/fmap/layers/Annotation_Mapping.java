...
public class Annotation_Mapping {
	...
	public Annotation_Mapping() {
		...
		if (xml.contains(DEFAULT_ANNOTATION_COLOR)) {
			String stringColor = xml
					.getStringProperty(DEFAULT_ANNOTATION_COLOR);
	...
	public static void addAnnotationMapping(Annotation_Layer al)
		...
		if (am.getColumnText() == -1) {
			throw new ReadDriverException(al.getName(), new RuntimeException(
					"This does not seem an annotation layer"));
		}
	}

	public static void storeValues(Color fontColor, String text,
			String fontType, int fontStyle, int fontHeight, int fontRotate) {
		XMLEntity xml = ps.getPersistentXML();

		xml.putProperty(DEFAULT_ANNOTATION_COLOR,
				StringUtilities.color2String(fontColor));
		xml.putProperty(DEFAULT_ANNOTATION_TEXT, text);
		xml.putProperty(DEFAULT_ANNOTATION_TYPEFONT, fontType);
		xml.putProperty(DEFAULT_ANNOTATION_STYLEFONT, fontStyle);
		xml.putProperty(DEFAULT_ANNOTATION_HEIGHT, fontHeight);
		xml.putProperty(DEFAULT_ANNOTATION_ROTATE, fontRotate);
	}
}
