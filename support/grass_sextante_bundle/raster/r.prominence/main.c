...
main (int argc, char *argv[])
{
	...
	module->description = "Calculates prominence statistic";
	...
	parm.input->description = "Raster map for which to calculate index";
	...
	flag.localnorm->description = "Local data normalization";
	
	flag.globalnorm = G_define_flag ();
	flag.globalnorm->key = 'g';
	flag.globalnorm->description = "Global data normalization";
	
	flag.rect = G_define_flag ();
	flag.rect->key = 'r';
	flag.rect->description = "Square neighborhood shape (instead of circular)";
	...
	if ((flag.localnorm->answer) && (flag.globalnorm->answer)) {
		G_fatal_error ("Please choose either local OR global normalization.");
	}
	...
	if (flag.localnorm->answer) {
		...
		if (fd < 0) {
			G_fatal_error ("Could not open output map for normalization pass!\n");
		}
		G_strcpy (sysstr, parm.output->answer);
		G_strcat (sysstr, ".localnorm");
		fd_out = G_open_raster_new (sysstr, DCELL_TYPE);
		if ( fd_out < 0 ) {
			G_fatal_error ("Could not open temporary map for normalization pass (write access)!\n");
		}	
	...
	if (flag.globalnorm->answer) {
		/* normalise data over total map */
		if (!flag.quiet->answer) {
			fprintf (stdout,"\nNormalizing data by map maximum:\n");
			fflush (stdout);
		}
		/* re-open output map for reading */
		fd = G_open_cell_old (parm.output->answer, G_mapset());	
