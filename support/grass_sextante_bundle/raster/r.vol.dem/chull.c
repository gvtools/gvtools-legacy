...
/*-------------------------------------------------------------------*/
int make3DHull ( long int *px, long int *py, long int *pz, int num_points, FILE *tmpfile )
{
	int error;
	...
	if (DEBUG > 1) check = BTRUE;	
	if (DEBUG > 2) debug = BTRUE;
...
int  VolumeSign( tFace f, tVertex p )
{
   ...
   if ( debug )
      fprintf(stderr,"Face=%6x; Vertex=%d: vol(int) = %li, vol(double) = %.f\n",
