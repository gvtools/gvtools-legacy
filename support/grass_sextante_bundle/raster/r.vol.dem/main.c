...
int isRGB (char *triplet, unsigned short int *R, unsigned short int *G, unsigned short int *B) {
	R = NULL;
	G = NULL;
	B = NULL;
	return (0);
...
void check_zrange ( int *fd, DCELL **dcell, int nfiles ) {
	...
	row = 0;
  	if (G_get_d_raster_row (fd[0], dcell[0], row) < 0) {
		G_fatal_error(_("Could not read row number %i from map %s."), row, parm.maps->answers[0]);		    
	}
	top = dcell[0][0];	
	topDEM = 0;
	for (i = 0; i < nfiles; i ++) {
		for (row=0; row < nrows; row ++) {
	  		if (G_get_d_raster_row (fd[i], dcell[i], row) < 0) {
				G_fatal_error(_("Could not read row number %i from map %s."), row, parm.maps->answers[i]);		    
			}
	...
	fflush (stdout);	
	if ( DEBUG )
		fprintf (stdout, "BOTTOM = %.2f in %s\n", bottom, parm.maps->answers[bottomDEM]);	
...
void check_topologies ( int *fd, DCELL **dcell, int nfiles ) {
	...
	if ( parm.error->answer != NULL ) {
		...
		/* KILL ME */
		fprintf (stderr, "DEBUG: Topology error mapping is an untested feature.\n");
		/* KILL ME */
		...
		if ( f_error < 0 ) {
			G_fatal_error ("Could not create new CELL raster map for mapping topology errors.");
		}
				...
				for (j = i+1; j < nfiles; j ++) {
					if (dcell[j][col] <= dcell[i][col]) {
						...
						G_warning(_("Z value in %s undershoots Z value in %s at cells %i,%i, \ncoordinates: E=%f, N=%f.\n"), 
							parm.maps->answers[j], parm.maps->answers[i], col, row, 
	...
	if ( highestDEMValue == -1 ) {
		...
		if ( highestDEMValue == (nfiles-1) ) {
			...
			if ( flag.fillnull->answer ) {
				...
				stopat = dcell[highestDEMValue][col] + (window.tb_res+(window.tb_res*0.5));
			}
		}
	}		
	...
	if ( lowestDEMValue == -1 ) {
		...
		if ( lowestDEMValue == 0 ) {
			...
			if ( flag.fillnull->answer ) {
				...
				stopat = dcell[lowestDEMValue][col] - (window.tb_res+(window.tb_res*0.5));
			}
		}
	}					
...
void interpolate ( int *fd, DCELL **dcell, int nfiles, int nvalues, double ***ascii ) {
			...
			for ( i = 1; i <NSLICES; i ++) {
				if ( isValue (flist[i])) {
					if ( nvalues == 0 ) {
						...
						ascii[i][row][col] = atof (parm.values->answers [flist[i]]);
					}
				} else {
...
void output_ascii ( double ***ascii ) {
	...
	if (tmpfile == NULL) {
		G_fatal_error (_("Could not create temporary file for writing ASCII output (%s)"), TMPFILE);
	}
	...
	for ( i=0; i < NSLICES; i ++ ) {
		for ( j=0; j < nrows; j ++ ) {
			for ( k=0; k < ncols; k ++ ) {
				...
				fprintf (tmpfile,"%f ", ascii[(NSLICES-1)-i][(nrows-1)-j][k]); 
			}
		fprintf (tmpfile,"\n");
		}
	}
...
void output_ascii_points ( double ***ascii ) {
	...
	tmpfile = fopen (tmp, "w+");
	if (tmpfile == NULL) {
		G_fatal_error (_("Could not create temporary file for writing ASCII output (%s)"), tmp);
	}
	...
	fprintf (tmpfile,"#   v.in.ascii in=%s.points.txt out=%s -z x=1 y=2 z=3 cat=0 columns='x double, y double, z double, w double' fs=tab\n",
	...
	for ( i=0; i < NSLICES; i ++ ) {
		for ( j=0; j < nrows; j ++ ) {
			for ( k=0; k < ncols; k ++ ) {
				...
				if ( w != DNULLVALUE ) {
					x = window.west   + ( window.ew_res * k ) + ( window.ew_res * 0.5 );
					y = window.south  + ( window.ns_res * j ) + ( window.ns_res * 0.5 );
					z = window.top - ( window.tb_res * i ) - ( window.tb_res * 0.5 );						
	...
	sprintf (tmp, "v.in.ascii in=%s.points.txt out=%s -z x=1 y=2 z=3 cat=0 columns='x double, y double, z double, w double' fs=tab\n",
			parm.output->answer, parm.output->answer);
...
void output_ascii_hulls ( double ***ascii, int nvalues, int nfiles ) {
	...
	for ( l=0; l < nfiles; l++) {
		...
		for ( i=0; i < NSLICES; i ++ ) {
			for ( j=0; j < nrows; j ++ ) {
				for ( k=0; k < ncols; k ++ ) {
					...
					w = ascii[i][(nrows-1)-j][k];
					if ( w == label ) {
						px[curPoint] = ((long int) window.west   + ( window.ew_res * k ) 
							+ ( window.ew_res * 0.5 )) * (PRECISION);
						py[curPoint] = ((long int) window.south  + ( window.ns_res * j ) 
							+ ( window.ns_res * 0.5 )) * (PRECISION);
						pz[curPoint] = ((long int) window.top    - ( window.tb_res * i ) 
		...
		if ( error < 0 ) {
			...
			fprintf (stdout, "DEBUG: Simple planar hulls not implemented yet.\n");
			...
			if ( l == 0 ) {
				...
				fprintf (tmpfile, "DEBUG: Simple planar hulls not implemented yet.\n");
		...
		if ( error > -1 ) {
			...
			sprintf (tmp, "v.in.ascii -z in=%s.hull.%i.txt out=%s_hull_%i form=standard", parm.output->answer, l, parm.output->answer, l);
...
main (int argc, char *argv[])
{
	...
	module->description = "Creates a 3D raster model (voxels) from a series of raster DEMs";
	...
	parm.maps->description = "Input DEMs (at least 2) in raster format. Bottom DEM first";	
	...
	parm.colors->description = "List of RRR:GGG:BBB color triplets, one for each layer";
	...
	parm.error->description = "Raster map to represent topology errors in input DEMs";
	...
	flag.vtk->key = 'v';
	flag.vtk->description = "Generate a .vtk file for visualisation with e.g. paraview";	
	...
	if ( nvalues > 0 ) {
		if ( nvalues != nfiles ) {
			G_fatal_error (_("Number of values (%i) does not match number of DEMs (%i)!"), nvalues, nfiles);
		}
	}
	...
	if ( ncolors > 0 ) {
		...
		if ( ncolors != nfiles ) {
			G_fatal_error (_("Number of RGB triplets (%i) does not match number of DEMs (%i)!"), ncolors, nfiles);
		}
		for ( i=0; i < ncolors; i ++ ) {
			if ( !isRGB (parm.colors->answers[i], &R[i], &G[i], &B[i]) ) {
				G_fatal_error (_("%s is not a valid RRR:GGG:BBB code!"), parm.colors->answers[i]);
			}
		}
	}
	...
	if ( NSLICES < 1 ) {
		G_fatal_error("This would produce < 1 slice. Adjust active 3d region resolution.");
	}
	if (DEBUG)  {
		fprintf (stdout, "%i DEMS, NSLICES = %i (%f)\n", nfiles, NSLICES, dslices);
		fflush (stdout);
	}
	...
	sprintf (sys,"r3.in.ascii in=%s out=%s nv=%f", TMPFILE, parm.output->answer, DNULLVALUE);
	if (DEBUG > 0 ) {
		fprintf (stdout, "%s\n", sys);
	}
	error = system ( sys );
