...
TESTING
- TEST THE INCLUDE OPTION and category (int) type input/output!!!
- Valgrind & test on Windows + Mac OS X
- use higher resolution data (0.0625m!) with oversampling
- try smaller and larger neighborhoods
- try very small setting for distance
- try smaller pixels (new rasterization of Vrable data with oversampling)
- increase min points (should not penalize processing time much)
- play with "power" param

NEXT VERSION:
- introduce secondary raster maps:
	 - sensor IDs: If a cell has the same sensor ID as the cell to be interpolated, then it should get higher weight
	 - slope: ???
	 - TODO item: mem for secondary maps needs to be included in estimation
...
void (*GET_STATS) (unsigned long, unsigned long, double, double, double*, unsigned long, stats_struct*);
void (*COLLECT_DATA) (double, double, double, double, double*, unsigned long, stats_struct*);
...
long int estimate_mem_needed ( long int cols, char *method )
{
 ...
 * types of information to be collected. In addition, there is an
 * unfiltered and a filtered version. The filtered version will be used
 * if the user has supplied a range and/or list of values to be included
 * in the operation.
 */

void collect_values_unfiltered ( 	double val1, double val2, double min, double max,
									double *include_lst, unsigned long include_lst_len,
									stats_struct *stats )
{
								...
								stats_struct *stats)
{
	...
	if ( val1 >= min && val1 <= max ) {
		if ( include_lst_len > 0 ) {
			for (i=0; i < include_lst_len; i++) {
				if ( include_lst[i] == val1 ) {
					collect_values_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
				}
			}
		} else {
			collect_values_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
...
void collect_values_and_weights_unfiltered ( 	double val1, double val2, double min, double max,
												double *include_lst, unsigned long include_lst_len,
												stats_struct *stats )
{
	stats->values[stats->num_values]=val1;
...
void collect_values_and_weights_filtered ( 	double val1, double val2, double min, double max,
											double *include_lst, unsigned long include_lst_len,
											stats_struct *stats )
{
	unsigned long i;
	...
	if ( val1 >= min && val1 <= max ) {
		if ( include_lst_len > 0 ) {
			for (i=0; i < include_lst_len; i++) {
				if ( include_lst[i] == val1 ) {
					collect_values_and_weights_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
				}
			}
		} else {
			collect_values_and_weights_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
...
void collect_values_and_frequencies_unfiltered ( 	double val1, double val2, double min, double max,
													double *include_lst, unsigned long include_lst_len,
													stats_struct *stats )
{
	unsigned long i;
...
void collect_values_and_frequencies_filtered ( 	double val1, double val2, double min, double max,
												double *include_lst, unsigned long include_lst_len,
												stats_struct *stats )
{
	unsigned long i;
	...
	if ( val1 >= min && val1 <= max ) {
		if ( include_lst_len > 0 ) {
			for (i=0; i < include_lst_len; i++) {
				if ( include_lst[i] == val1 ) {
					collect_values_and_frequencies_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
				}
			}
		} else {
			collect_values_and_frequencies_unfiltered (val1,val2,min,max,include_lst,include_lst_len,stats);
 ...
 * This is needed for calculating median and medoid statistics.
 */
		...
		for ( j = 0; j < DATA_WIDTH; j ++ ) {
	    	...
	    	if ( !IS_NULL(cell) ) {
	    		...
	    		if ( WEIGHTS[i][j] != -1.0 ) {
	    			...
	    			COLLECT_DATA (cell_value, WEIGHTS[i][j],min,max,include_lst,include_lst_len,stats);
	    		}
	    	}
...
void get_statistics_wmean (	unsigned long row_index, unsigned long col,
							double min, double max, double *include_lst, unsigned long include_lst_len,
							stats_struct *stats)
{
	unsigned long i;
	double total_weight;

	read_neighborhood (	row_index, col, min, max, include_lst, include_lst_len, stats );
	...
	stats->result = 0.0;
	for (i = 0; i < stats->num_values; i ++) {
		stats->result += (stats->weights[i] * stats->values[i]) /
										total_weight;
	}
}
					...
					stats_struct *stats)
{
	...
	if ( stats->num_values % 2  != 0 ) {
		/* odd number of elements: result is average of the two central values */
		stats->result = (stats->values[stats->num_values/2]+stats->values[(stats->num_values/2)+1])/2.0;
	} else {
		/* even number of elements: result is the central element */
...
/*
 * NEIGHBORHOOD STATISTICS FUNCTION MEDOID
 * Simple, unweighted medoid. Identical to median, except that for an even number of data points,
 * the medoid will be the lower (smaller element) in the sorted data list.
 */
void get_statistics_medoid (unsigned long row_index, unsigned long col,
					double min, double max, double *include_lst, unsigned long include_lst_len,
					stats_struct *stats)
{
	read_neighborhood (	row_index, col, min, max, include_lst, include_lst_len, stats );
	...
	if ( stats->num_values % 2  != 0 ) {
		/* odd number of elements: result is the first of the two central values */
 ...
 */
void interpolate_row ( 	unsigned long row_index, unsigned long cols, int preserve,
						double min, double max,
						unsigned long min_points,
						double *include_lst, unsigned long include_lst_len,
						stats_struct *stats, int write_err )
{
	...
	/* process all cells on current row */
	cell_input = get_input_row(row_index);
	...
	for ( j = 0; j < cols; j++ ) {
		int done = 0;
		if ( preserve ) {
			/* if -p flag is given, we copy the original data through to the result */
			if ( !IS_NULL(cell_input) ) {
				WRITE_CELL_VAL (cell_input, cell_output);
				if ( write_err )
					G_set_raster_value_f(err_output,(FCELL)0.0, FCELL_TYPE);
				/* done with this cell, skip to the next one! */
				done = 1;
			}
		}
		/* still need to replace this cell with an interpolated value? */
		if (!done) {
			/* get neighborhood statistics */
			GET_STATS (row_index, j, min, max, include_lst, include_lst_len, stats);
			/* enough interpolating points in input map? */
			if ( stats->num_values < min_points ) {
		...
		/* advance cell input and output pointers by one cell size */
		cell_input += CELL_IN_SIZE;
 ...
 * passed as "1" and all cells within the neighborhood will be set to "1.0".
 * In both casses, all cells outside the neighborhood will be set to "-1.0".
 */
void build_weights_matrix (double radius, double power, double res_x, double res_y, int center_flag, int constant)
{
    ...
    /* center cell is not part of the matrix, include only
       if -c flag has been given */
    if ( center_flag ) {
...
int main(int argc, char *argv[])
{
    ...
    int projection;
    double *include_list = NULL;
    unsigned long include_list_length;
	...
	struct Option
	    *input, *output, *error,
	    *radius, *method, *power, *min, *max, *include, *minpts;
        struct Flag
            *print_w, *print_m, *center, *preserve, *single_precision;
    } parm;
    ...
    parm.radius->description = _("Maximum distance (in map units) for interpolation");

    parm.method = G_define_option();
    parm.method->key = "method";
    parm.method->key_desc = "name";
    parm.method->required = YES;
    parm.method->multiple = NO;
    parm.method->type = TYPE_STRING;
    parm.method->description = _("Statistic for interpolated cell values");
    parm.method->options = "wmean,mean,median,medoid,mode";
    parm.method->answer = "wmean";
    ...
    parm.include = G_define_option();
    parm.include->key = "include";
    parm.include->key_desc = "value";
    parm.include->required = NO;
    parm.include->multiple = YES;
    parm.include->type = TYPE_DOUBLE;
    parm.include->description = _("List of values to include in interpolation (exclude rest)");
    ...
    parm.minpts->key = "points";
    parm.minpts->key_desc = "value";
    parm.minpts->required = YES;
    parm.minpts->multiple = NO;
    parm.minpts->type = TYPE_INTEGER;
    parm.minpts->answer = "12";
    parm.minpts->description = _("Minimum number of points for interpolation");

    parm.print_w = G_define_flag();
    parm.print_w->key = 'w';
    parm.print_w->description = _("Just print the spatial weights matrix");

    parm.print_m = G_define_flag();
    parm.print_m->key = 'm';
    parm.print_m->description = _("Just print estimated memory usage");

    parm.preserve = G_define_flag();
    parm.preserve->key = 'p';
    parm.preserve->description = _("Preserve original input data");

    parm.center = G_define_flag();
    parm.center->key = 'c';
    parm.center->description = _("Include value of center cell (not for 'wmean')");
    ...
    if ( projection == PROJECTION_LL ) {
    	G_fatal_error (_("Data in lat/lon format is not supported."));
    }
    ...
    min_points = atol (parm.minpts->answer);
    write_error = 0;
    ...
    double max_dist = sqrt ( pow((cols*res_x),2) + pow((rows*res_y),2) ); 
    if ( radius > max_dist ) {
        G_warning (_("Maximum distance too large. Adjusted to '%.6f' (diagonal of current region)."), max_dist);
        radius = max_dist;
    }
    if ( min_points < 1 ) {
        G_fatal_error (_("Minimum number of points must be at least '1'."));
    }
    if ( min_points > ((DATA_WIDTH*DATA_HEIGHT)-1)) {
    	G_fatal_error (_("Specified minimum number of points unreachable with current settings."));
    }
    ...
    if ( parm.center->answer && !strcmp (parm.method->answer,"wmean") ) {
    	G_warning (_("The center cell is always excluded in mode 'wmean'. Ignoring '-c' flag."));
    	parm.center->answer=0;
    }
    if ( parm.power->answer && strcmp (parm.method->answer,"wmean") ) {
    	G_warning (_("The 'power' option has no effect for all modes except 'wmean'."));
    	parm.center->answer=0;
    }
    ...
    if (parm.print_w->answer) {
        if ( !strcmp (parm.method->answer,"wmean") ) {
        	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 0);
        }
        if ( !strcmp (parm.method->answer,"mean") ) {
        	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 1);
        }
        if ( !strcmp (parm.method->answer,"median") ) {
        	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 1);
        }
        if ( !strcmp (parm.method->answer,"medoid") ) {
        	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 1);
        }
        if ( !strcmp (parm.method->answer,"mode") ) {
        	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 1);
        }
    	print_weights_matrix (DATA_HEIGHT, DATA_WIDTH);
        /* continue only if "-m" flag has also been given */
        if ( !parm.print_m->answer )
        	exit (0);
    }
    ...
    /* build list of values to include */
    char *endptr;
    include_list_length = 0;
    if ( parm.include->answer!=NULL ) {
    	while (parm.include->answers[include_list_length]) {
    		include_list_length++;
    	}
    	include_list = G_malloc (sizeof(double)*include_list_length);
    	for ( i=0; i < include_list_length; i++) {
    		errno = 0;
    		include_list[i]=strtod (parm.include->answers[i],&endptr);
    		if ( errno !=0 || *endptr != '\0' ) {
    			G_fatal_error (_("Bad include list: '%s' is not a valid number."),parm.include->answers[i]);
    		}
    	}
    }
    ...
    if ( IN_TYPE == CELL_TYPE ) {
    	if ( 	(!strcmp (parm.method->answer,"wmean")) || (!strcmp (parm.method->answer,"mean"))
    			|| (!strcmp (parm.method->answer,"median"))) {
    		G_warning (_("Input data type is integer but interpolation method is '%s'."), parm.method->answer);
    		if ( parm.single_precision->answer ) {
    			OUT_TYPE = FCELL_TYPE;
    ...
    G_message (_("Input data type is '%s' and output data type is '%s'."),
    		data_type_string_in, data_type_string_out );

    /* initialize data type dependent cell handling functions */
	init_cell_funcs();

    /* just print projected mem usage if user wants it so */
    G_message ("Minimal estimated memory usage is %.3f MB.", ((double) estimate_mem_needed(cols,parm.method->answer))/1024/1024);
    if ( parm.print_m->answer ) {
    	exit (0);
    }
     ...
     * */
	CELL_INPUT = G_malloc ( sizeof (CELL_IN_PTR_SIZE) * WINDOW_HEIGHT );
    for ( i = 0; i < WINDOW_HEIGHT; i ++ ) {
    	CELL_INPUT[i]= G_malloc ( sizeof (CELL_IN_SIZE) * (cols + (PADDING_WIDTH*2)));
    }
    for ( i = 0; i < WINDOW_HEIGHT; i ++ ) {
	 ...
	 */
	CELL_INPUT_HANDLES = G_malloc (sizeof(CELL_IN_PTR_SIZE)*WINDOW_HEIGHT);
    ...
    if ( !strcmp (parm.method->answer,"medoid") ) {
    	build_weights_matrix (radius, power, res_x, res_y, parm.center->answer, 1);
    	GET_STATS = &get_statistics_medoid;
    	if ( filter_min == 1 || filter_max == 1 ) {
    		COLLECT_DATA=&collect_values_and_frequencies_filtered;
    	} else {
    		COLLECT_DATA=&collect_values_and_frequencies_unfiltered;
	...
	for ( i = 0; i <= PADDING_HEIGHT; i++ ) {
	    ...
	    interpolate_row (	row_idx, cols, parm.preserve->answer,
							min, max, min_points,
							include_list, include_list_length, &cell_stats, write_error );
    ...
    sprintf (hist.edhist[0],"Settings: method=%s, distance=%.3f, power=%.3f",
    		parm.method->answer, radius, power );
    sprintf (hist.edhist[1],"          min=%.3f, max=%.3f, min. points=%lu",
    		min, max, min_points );
    if ( include_list ) {
        sprintf (hist.edhist[2],"(some input values have been excluded)");
    }
