...
int main(int argc, char *argv[])
{
  ...
  struct Flag *replace, *from, *quiet, *mask, *silent;
  ...
  type->description= "Type of viewshed analysis";
  ...
  /* check if maps exist */
  if ( G_find_cell ( input->answer, "" ) == NULL ) {
  	G_fatal_error ( "Input elevation map '%s' does not exist in current database.\n", input->answer );
  }

  if ( patt_map->answer != NULL ) {
 	if ( G_find_cell ( patt_map->answer, "" ) == NULL ) {
  		G_fatal_error ( "Input mask map '%s' does not exist in current database.\n", patt_map->answer );
  	}
  }

  if ( patt_map_v->answer != NULL ) {
  	if ( G_find_cell ( patt_map_v->answer, "" ) == NULL ) {
  		G_fatal_error ( "Input mask map '%s' does not exist in current database.\n", patt_map_v->answer );
  	}
  }
  ...
  Segment_named_outfile  (output->answer, current_mapset, &new,
			  ...
			  module->overwrite, quiet->answer, fract, CELL_TYPE);
  ...
  if (strcmp (type->answer, "sites") == 0)  {
    ...
    if ( sites->answer == NULL ) {
    	G_fatal_error ("You have to specify a vector map for option '%s' for analysis type 'sites'.\n ", sites->key );
    }
