...
public class JNCSFile implements JNCSProgressiveUpdate {
	...
	private native int ECWSetView(int i, int ai[], int j, int k, int l, int i1,
			double d, double d1, double d2, double d3, int j1, int k1);
	...
	/**
	 * Carga la libreria libecw.so que contiene el wrapper y llama a la función
	 * de inicialización
	 * 
	...
	static void initClass() throws JNCSNativeLibraryException {
		...
		try {
			...
			if (i != 0) {
				System.err
						.println("JNCSFile classes found on PATH failed to initialize correctly. Attempting to locate other libecw.so....");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * 
	 * @param flag
	 *            parámetro para la función NCScbmCloseFileViewEx
	 */
	...
	public void addProgressiveUpdateListener(
			JNCSProgressiveUpdate jncsprogressiveupdate) {
	...
	/**
	 * Crea una vista en un fichero ecw abierto dando las coordenadas de la
	 * vista
	 * 
	 * @param nBands
	 *            Número de bandas en bandList
	 * @param bandList
	 *            Array de índices de bandas
	 * @param width
	 *            ancho de la vista en pixels
	 * @param height
	 *            alto de la vista en pixels
	 * @param tlx
	 *            coordenada X arriba-izquierda de la vista
	 * @param tly
	 *            coordenada Y arriba-izquierda de la vista
	 * @param brx
	 *            coordenada X abajo-derecha de la vista
	 * @param bry
	...
	/**
	 * Asigna la vista pasando por parámetros los pixeles de inicio y fin y
	 * coordenadas de georreferenciación solicitadas- El cliente de esta función
	 * tendrá que convertir las coordenadas reales en pixeles.
	 */
	public int setView(int nBands, int posBands[], int iniX, int iniY,
			int endX, int endY, double tlX, double tlY, double brX, double brY,
			int bufW, int bufH) throws JNCSFileNotOpenException,
			JNCSInvalidSetViewException {
		...
		int l = ECWSetView(nBands, posBands, iniX, iniY, endX, endY, tlX, tlY,
				brX, brY, bufW, bufH);
		if (l != 0) {
	 ...
	 *            alto de la vista en pixels
	 * @param dWorldTLX
	 *            coordenada X arriba-izquierda)
	 * @param dWorldTLY
	 *            coordenada Y arriba-izquierda
	 * @param dWorldBRX
	 *            coordenada X abajo-derecha)
	 * @param dWorldBRY
	...
	public int setView(int i, int ai[], double d, double d1, double d2,
		...
		int l = ECWSetView(i, ai, jncsdatasetpoint.x, jncsdatasetpoint.y,
				jncsdatasetpoint1.x - 1, jncsdatasetpoint1.y - 1, d, d1, d2,
				d3, j, k);
		if (l != 0) {
	...
	/**
	 * Obtiene una cadena que corresponde a un error a través del entero que lo
	 * representa
	 * 
