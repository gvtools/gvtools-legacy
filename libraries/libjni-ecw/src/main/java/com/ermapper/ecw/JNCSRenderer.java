...
public class JNCSRenderer extends JNCSFile {
	...
	public int setView(int i, int ai[], int j, int k, int l, int i1, int j1,
			int k1) throws JNCSFileNotOpenException,
		...
		if (i1 > super.height - 1)
			k2 = super.height - 1;
		int l2 = (int) Math.round(((double) (l1 - j) / (double) (l - j))
				* (double) j1);
		int i3 = (int) Math.round(((double) (i2 - k) / (double) (i1 - k))
				* (double) k1);
		int j3 = (int) Math.round(((double) (j2 - j) / (double) (l - j))
				* (double) j1);
		int k3 = (int) Math.round(((double) (k2 - k) / (double) (i1 - k))
				* (double) k1);
	...
	public int setView(int i, int ai[], double d, double d1, double d2,
			double d3, int j, int k) throws JNCSFileNotOpenException,
			JNCSInvalidSetViewException {
		...
		double d6 = d2;
		double d7 = d3;
		jncsdatasetpoint.x = (int) Math.floor((d4 - super.originX)
				/ super.cellIncrementX);
		jncsdatasetpoint.y = (int) Math.floor((d5 - super.originY)
				/ super.cellIncrementY);
		jncsdatasetpoint1.x = (int) Math.ceil((d6 - super.originX)
				/ super.cellIncrementX);
		jncsdatasetpoint1.y = (int) Math.ceil((d7 - super.originY)
				/ super.cellIncrementY);
		JNCSWorldPoint jncsworldpoint = convertDatasetToWorld(
				jncsdatasetpoint.x, jncsdatasetpoint.y);
		JNCSWorldPoint jncsworldpoint1 = convertDatasetToWorld(
				jncsdatasetpoint1.x, jncsdatasetpoint1.y);
		d4 = jncsworldpoint.x;
		d5 = jncsworldpoint.y;
		...
		if (jncsdatasetpoint1.x > super.width - 1)
			d6 = super.originX + (double) (super.width - 1)
					* super.cellIncrementX;
		if (jncsdatasetpoint1.y > super.height - 1)
			d7 = super.originY + (double) (super.height - 1)
					* super.cellIncrementY;
		if (jncsdatasetpoint.x < 0 && jncsdatasetpoint1.x < 0
				|| jncsdatasetpoint.x > super.width - 1
				&& jncsdatasetpoint1.x > super.width - 1) {
			bHaveValidSetView = false;
			return 1;
		}
		if (jncsdatasetpoint.y < 0 && jncsdatasetpoint1.y < 0
				|| jncsdatasetpoint.y > super.height - 1
				&& jncsdatasetpoint1.y > super.height - 1) {
			bHaveValidSetView = false;
			return 1;
		}
	...
	public void refreshUpdate(int i, int j, double d, double d1, double d2,
			double d3) {
	...
	private boolean ecwReadImage(int i, int j, double d, double d1, double d2,
			double d3) {
		Image image = null;
		boolean flag = false;
		...
		try {
			...
			if (flag) {
				memImSource = null;
				memImSource = new MemoryImageSource(i, j, ecwColorModel,
						pRGBArray, 0, i);
			} else {
			...
			System.out.print(String.valueOf(String.valueOf((new StringBuffer(
					"readImageRGBA failed :")).append(jncsexception.toString())
					.append("\n"))));
			bHaveValidSetView = false;
			boolean flag1 = false;
			return flag1;
	...
	public void drawImage(Graphics g, int i, int j, int k, int l, double d,
		...
		if (!super.progressive && bHaveValidSetView)
			ecwReadImage(dRendererSetViewWidth, dRendererSetViewHeight,
					dRendererSetViewWorldTLX, dRendererSetViewWorldTLY,
					dRendererSetViewWorldBRX, dRendererSetViewWorldBRY);
		...
		synchronized (ecwImage) {
			if (bHave1_2VM)
				if ((double) transparencyValue == 1.0D)
					;
			calculateDeviceCoords(i, j, i + k, j + l, d, d1, d2, d3,
					outputDeviceCoords);
			int i1 = (int) Math.round(outputDeviceCoords[2]
					- outputDeviceCoords[0]);
			int j1 = (int) Math.round(outputDeviceCoords[3]
					- outputDeviceCoords[1]);
			int k1 = ecwImage.getWidth(imageobserver);
			int l1 = ecwImage.getHeight(imageobserver);
			if (i1 != k1 || j1 != l1) {
				if (i1 > k1 && j1 > l1) {
					double d4 = outputDeviceCoords[0] >= (double) i ? outputDeviceCoords[0]
							: i;
					double d5 = outputDeviceCoords[1] >= (double) j ? outputDeviceCoords[1]
							: j;
					double d6 = outputDeviceCoords[2] <= (double) (i + k) ? outputDeviceCoords[2]
							: i + k;
					double d7 = outputDeviceCoords[3] <= (double) (j + l) ? outputDeviceCoords[3]
							: j + l;
					double ad[] = calculateImageCoords(outputDeviceCoords[0],
							outputDeviceCoords[1], outputDeviceCoords[2],
							outputDeviceCoords[3], k1, l1, d4, d5, d6, d7);
					ad[0] = Math.floor(ad[0]);
					ad[1] = Math.floor(ad[1]);
					ad[2] = Math.ceil(ad[2]);
					ad[3] = Math.ceil(ad[3]);
					double d8 = dRendererWorldTLX
							+ ((dRendererWorldBRX - dRendererWorldTLX) / (double) k1)
							* ad[0];
					double d9 = dRendererWorldTLY
							+ ((dRendererWorldBRY - dRendererWorldTLY) / (double) l1)
							* ad[1];
					double d10 = dRendererWorldTLX
							+ ((dRendererWorldBRX - dRendererWorldTLX) / (double) k1)
							* ad[2];
					double d11 = dRendererWorldTLY
							+ ((dRendererWorldBRY - dRendererWorldTLY) / (double) l1)
							* ad[3];
					double d12 = ((outputDeviceCoords[2] - outputDeviceCoords[0]) * (d8 - dRendererWorldTLX))
							/ (dRendererWorldBRX - dRendererWorldTLX)
							+ outputDeviceCoords[0];
					double d13 = ((outputDeviceCoords[3] - outputDeviceCoords[1]) * (d9 - dRendererWorldTLY))
							/ (dRendererWorldBRY - dRendererWorldTLY)
							+ outputDeviceCoords[1];
					double d14 = ((outputDeviceCoords[2] - outputDeviceCoords[0]) * (d10 - dRendererWorldTLX))
							/ (dRendererWorldBRX - dRendererWorldTLX)
							+ outputDeviceCoords[0];
					double d15 = ((outputDeviceCoords[3] - outputDeviceCoords[1]) * (d11 - dRendererWorldTLY))
							/ (dRendererWorldBRY - dRendererWorldTLY)
							+ outputDeviceCoords[1];
					g.drawImage(ecwImage, (int) Math.round(d12),
							(int) Math.round(d13), (int) Math.round(d14),
							(int) Math.round(d15), (int) Math.floor(ad[0]),
							(int) Math.floor(ad[1]), (int) Math.ceil(ad[2]),
							(int) Math.ceil(ad[3]), imageobserver);
				} else {
					g.drawImage(ecwImage,
							(int) Math.round(outputDeviceCoords[0]),
							(int) Math.round(outputDeviceCoords[1]),
							(int) Math.round(outputDeviceCoords[2]),
							(int) Math.round(outputDeviceCoords[3]), 0, 0, k1,
							l1, imageobserver);
				}
			} else {
				g.drawImage(ecwImage, (int) Math.round(outputDeviceCoords[0]),
						(int) Math.round(outputDeviceCoords[1]), imageobserver);
			}
			if (bHave1_2VM)
				if ((double) transparencyValue == 1.0D)
					;
	...
	private static final double[] calculateImageCoords(double d, double d1,
	...
	private final void calculateDeviceCoords(int i, int j, int k, int l,
			double d, double d1, double d2, double d3, double ad[]) {
	...
	private final void convertImageToWorld(int i, int j, double d, double d1,
			double ad[]) {
		ad[0] = dRendererWorldTLX
				+ ((dRendererWorldBRX - dRendererWorldTLX) / (double) i) * d;
		ad[1] = dRendererWorldTLY
				+ ((dRendererWorldBRY - dRendererWorldTLY) / (double) j) * d1;
	}
