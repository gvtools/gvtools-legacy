...
public class URLRetrieveTest extends TestCase {
	private final static String tempDirectoryPath = System
	...
	RetrieveListener listener1 = new RetrieveListener() {
		public void transferEventReceived(RetrieveEvent event) {
			...
			if (e == RetrieveEvent.REQUEST_FINISHED
					|| e == RetrieveEvent.REQUEST_FAILED
					|| e == RetrieveEvent.REQUEST_CANCELLED)
				finished1 = true;
		}
	};
	...
	public void setUp() {
		...
		System.out.println("\n\n\nSetting up test..");
		queue = new RetrieveQueue(
				"http://192.168.0.223/cgi-bin/mapserv_46.exe?map=c:\\ms4w\\Apache\\cgi-bin\\demo.map");
	...
	public void testCancelling() {
		...
		boolean more = true;
		while (!queue.isEmpty()) {
			if (more && System.currentTimeMillis() - time > 1000) { // wait 1
																	// seconds
																	// and
																	// cancel
				task2.cancel();
	...
	public void testCocurrentTransfers() {
		finished1 = finished2 = finished3 = finished4 = false;
		System.out
				.println("tests to merge two or more equivalent transfers into one");
		RequestManager manager = RequestManager.getInstance();
