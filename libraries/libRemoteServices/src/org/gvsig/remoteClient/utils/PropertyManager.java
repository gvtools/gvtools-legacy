...
public class PropertyManager {
	...
	/**
	 * Gets a properties object If the the property files are not yet loaded,
	 * then loads them first
	 * 
		...
		for (int i = 0; i < s_propertyFileNames.length; i++) {
			try {
				InputStream input = loader
						.getResourceAsStream(s_propertyFileNames[i]);
	...
	// Loads the property files
	/*
	 * public static synchronized void saveProperties(String propsName) throws
	 * IOException { try { //ClassLoader loader =
	 * PropertyManager.class.getClassLoader(); //InputStream input =
	 * loader.getResourceAsStream(propsName); FileOutputStream output = new
	 * FileOutputStream(propsName); Properties props = new Properties();
	 * props.store(output, propsName); output.close(); } catch(Exception e) {
	 * System
