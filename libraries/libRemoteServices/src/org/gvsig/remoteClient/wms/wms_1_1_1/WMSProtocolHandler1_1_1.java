 ...
 * </p>
 */
public class WMSProtocolHandler1_1_1 extends
	...
	/**
	 * <p>
	 * Parse the xml data retrieved from the WMS, it will parse the WMS
	 * Capabilities
	 * </p>
	 * 
	 */
	 ...
	 * </p>
	 */
	private void parseServiceTag(KXmlParser parser) throws IOException,
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(CapabilitiesTags.NAME) == 0) {
					...
					if (value != null) {
						...
						CapabilitiesTags.KEYWORDLIST) == 0)
						|| (parser.getName().compareTo(
								CapabilitiesTags.CONTACTINFORMATION) == 0)) {
					parser.skipSubTree();
				}
				break;
			case KXmlParser.END_TAG:
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(CapabilitiesTags.REQUEST) == 0) {
					...
					// TODO:
					// Add to serviceInfo the supported formats for the
					// exceptions????
					...
					// //Add all the SRS that the layer supports to the
					// WMSProtocolHandler if they dont exist already
					// for (i=0;i<layer.getAllSrs().size();i++)
					// {
					// // if
					// (!layer.srs.contains(layer.getAllSrs().elementAt(i)))
					// // {
	...
	private void parseRequestTag(KXmlParser parser) throws IOException,
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName()
						.compareTo(CapabilitiesTags.GETCAPABILITIES) == 0) {
					serviceInfo.operations.put(
							CapabilitiesTags.GETCAPABILITIES, null);
				} else if (parser.getName().compareTo(CapabilitiesTags.GETMAP) == 0) {
					// put a null to this key?
					// or leave it like it was with a CODE in a vector
					// specifying this operation.
					...
					serviceInfo.operations.put(CapabilitiesTags.GETFEATUREINFO,
							null);
					parseGetFeatureInfoTag(parser);
				} else if (parser.getName().compareTo(
						CapabilitiesTags.DESCRIBELAYER) == 0) {
					// serviceInfo.operations.put(WMSProtocolHandler.DESCRIBELAYER_OPERATION)
					serviceInfo.operations.put(CapabilitiesTags.DESCRIBELAYER,
							null);
				} else if (parser.getName().compareTo(
						CapabilitiesTags.GETLEGENDGRAPHIC) == 0) {
					serviceInfo.operations.put(
							CapabilitiesTags.GETLEGENDGRAPHIC, null);
	...
	/**
	 * <p>
	 * Parses the GetFeatureInfoTag tag
	 * </p>
	 */
	 ...
	 * </p>
	 */
	private void parseGetLegendGraphicTag(KXmlParser parser)
			throws IOException, XmlPullParserException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				CapabilitiesTags.GETLEGENDGRAPHIC);
		currentTag = parser.next();
	...
	protected String parseException(byte[] data) {
		...
		try {
			...
			// Comentar temporalmente para subsanar el hecho de que SimonCit me
			// devuelve las capabilities en un GetLegendGraphic!!!
			kxmlParser.require(KXmlParser.START_TAG, null,
			...
			while (!end) {
				...
				case KXmlParser.END_TAG:
					if (kxmlParser.getName().compareTo(
							ExceptionTags.EXCEPTION_ROOT) == 0)
						end = true;
					break;
				}
				if (!end) {
