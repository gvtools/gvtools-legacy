...
public class WMSLayer1_1_1 extends org.gvsig.remoteClient.wms.WMSLayer {
	...
	/**
	 * <p>
	 * Extents defined for the layer in the capabilities doc
	 * </p>
	 */
	...
	public ArrayList getDimensions() {
		...
		for (int i = 0; i < dimensions.size(); i++) {
			...
			if (extent != null) {
				((WMSDimension) dimensions.get(i))
						.setDimensionExpression(extent.getExtentExpression());
			}
		}
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(CapabilitiesTags.LAYER) == 0) {
					...
					addSrs(bbox.getSrs());
						...
						CapabilitiesTags.LATLONBOUNDINGBOX) == 0) {
					bbox = new BoundaryBox();
				...
				// don't use the scalehint param to set scaleMin and scaleMax,
				// because
				// - its semantincs is different to MinScaleDenominator and
				// MaxScaleDenominator (wms 1.3.0)
				// - they are quite useless as they are defined in wms 1.1.1
				// - they break the usage of scaleMin and scaleMax
				// else if
				// (parser.getName().compareTo(CapabilitiesTags.SCALEHINT)==0)
				// {
