...
public abstract class WMSProtocolHandler {
	...
	/**
	 * parses the data retrieved by the WMS in XML format. It will be mostly the
	 * WMS Capabilities, but the implementation will be placed in the handler
	 * implementing certain version of the protocol.
	 * 
	 */
	 ...
	 * be implemented by each specific version because the XML from the server
	 * will be parsed and presented by a well known structure.
	 */
	...
	/**
	 * <p>
	 * Builds a GetCapabilities request that is sent to the WMS the response
	 * will be parse to extract the data needed by the WMS client
	 * </p>
	 * 
	 * @param override
	...
	public void getCapabilities(WMSStatus status, boolean override,
		...
		try {
			...
			File f = Utilities.downloadFile(request, "wms_capabilities.xml",
					cancel);
			if (f == null)
				return;
			clear();
	...
	/**
	 * <p>
	 * It will send a GetFeatureInfo request to the WMS Parsing the response and
	 * redirecting the info to the WMS client
	 * </p>
	 * TODO: return a stored file instead a String.
	 */
	public String getFeatureInfo(WMSStatus status, int x, int y,
			int featureCount, ICancellable cancel) {
		...
		try {
			...
			File f = Utilities.downloadFile(request, "wms_feature_info.xml",
					cancel);
			if (f == null)
				return "";
			...
			if ((outputFormat == null) || (outputFormat.indexOf("xml") != -1)
				...
				if (kxmlParser.getName().compareTo(ServiceException) == 0) {
					sb.append("<INFO>")
							.append(parseException(output.toString().getBytes()))
							.append("</INFO>");
					return sb.toString();
				...
				// if (kxmlParser.getName().compareTo(ServiceException)==0)
				// {
				// sb.append("<INFO>").append(parseException(
				// output.toString().getBytes())).append("</INFO>");
				// return sb.toString();
				// }
				// else if
				// (kxmlParser.getName().compareToIgnoreCase("ERROR")==0)
				// return output.toString();
				// else
				...
				// return sb.toString();
				// Para que funcione con el GetFeatureInfo Viewer generico hay
				// que devolver:
				return output.toString();
			}
			...
			if (output.toString().toLowerCase().indexOf("xml") != -1) {
				...
				sb.append("<INFO>").append("Info format not supported")
						.append("</INFO>");
				return sb.toString();
			}
		} catch (Exception e) {
	...
	/**
	 * <p>
	 * Builds a GetMap request that is sent to the WMS the response (image) will
	 * be redirect to the WMS client
	 * </p>
	 */
	public byte[] _getMap(WMSStatus status) throws ServerErrorException,
			WMSException {
		URL request = null;
		try {
			// TODO:
			// pass this buildXXXRequest to the WMSProtocolHandlerXXX: The
			// request can depend on the WMS version.
			...
			for (int i = is.read(buffer); i > 0; i = is.read(buffer)) {
				// Creates a new buffer to contain the previous readed bytes and
				// the next bunch of bytes
				byte[] buffered = new byte[readed + i];
				for (int j = 0; j < buffered.length; j++) {
					if (j < readed) {
						// puts the previously downloaded bytes into the image
						// buffer
						buffered[j] = imageBytes[j];
					} else {
						// appends the recently downloaded bytes to the image
						// buffer.
	...
	public File getLegendGraphic(WMSStatus status, String layerName,
			ICancellable cancel) throws ServerErrorException, WMSException {
		URL request = null;
		try {
			request = new URL(buildGetLegendGraphicRequest(status, layerName));
			System.out.println(request);
			File f = Utilities.downloadFile(request, "wmsGetLegendGraphic",
					cancel);
			if (f == null)
				return null;
	...
	public File getMap(WMSStatus status, ICancellable cancel)
		...
		try {
			...
			if (kxmlParser.getEventType() != KXmlParser.END_DOCUMENT) {
				kxmlParser.require(KXmlParser.START_TAG, null,
						ExceptionTags.EXCEPTION_ROOT);
				tag = kxmlParser.nextTag();
				while (tag != KXmlParser.END_DOCUMENT) {
					switch (tag) {
					case KXmlParser.START_TAG:
						if (kxmlParser.getName().compareTo(
								ExceptionTags.SERVICE_EXCEPTION) == 0) {
	...
	protected String buildGetFeatureInfoRequest(WMSStatus status, int x, int y) {
		...
		req.append(onlineResource).append(symbol)
				.append("REQUEST=GetFeatureInfo&SERVICE=WMS&");
		req.append("QUERY_LAYERS=").append(
				Utilities.Vector2CS(status.getLayerNames()));
		req.append("&VERSION=").append(getVersion())
				.append("&INFO_FORMAT=application/vnd.ogc.gml&");
		req.append(getPartialQuery(status)).append("&x=" + x + "&y=" + y);
		// this parameter sets the max number of features per layer to be
		// returned.
		// we set it to avoid the bug in mapserver that takes this number like
		// max number of total features.
		req.append("&FEATURE_COUNT=10000");
		return req.toString().replaceAll(" ", "%20");
	}

	/**
	 * @return string that represents the url for getting the wms legend If the
	 *         layer has the object layer-->style-->legendurl that url will be
	 *         returned otherwise builds the getLegendGraphic according to the
	 *         OGC WMS Specifications
	 * 
	 */
	private String buildGetLegendGraphicRequest(WMSStatus status,
			String layerName) {
		...
		req.append(
				onlineResource + symbol
						+ "REQUEST=GetLegendGraphic&SERVICE=WMS&VERSION=")
				.append(getVersion());
		req.append("&LAYER=" + layerName).append("&TRANSPARENT=TRUE")
	...
	private String buildMapRequest(WMSStatus status) {
		...
		req.append(
				onlineResource + symbol + "REQUEST=GetMap&SERVICE=WMS&VERSION=")
	...
	public String getPartialQuery(WMSStatus status) {
		...
		req.append("LAYERS=" + Utilities.Vector2CS(status.getLayerNames()))
				...
				.append("&FORMAT=" + status.getFormat()).append("&STYLES=");
	...
	/**
	 * Inner class that represents the description of the WMS metadata. The
	 * first part of the capabilities will return the service information from
	 * the WMS, this class will hold this information.
	 * 
	 */
	public class ServiceInformation {
		...
		/*
		 * public String map_online_resource = null; public String
		 * feature_online_resource = null;
		 */
