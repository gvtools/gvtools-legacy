...
public class WMSLayer1_3_0 extends org.gvsig.remoteClient.wms.WMSLayer {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(CapabilitiesTags.LAYER) == 0) {
					...
					if (lyr.getName() != null)
						...
						CapabilitiesTags.ATTRIBUTION) == 0) {
					...
					// TODO:
					// comentar esto y añadir solo los SRS o CRS que incluyan un
					// extent...
					value = parser.nextText();
					if (value != null) {
					...
					// value =
					// parser.getAttributeValue("",CapabilitiesTags.RESX);
					// if ((value != null) && (Utilities.isNumber(value)))
					// bbox.setYmax(Double.parseDouble(value));
					// value =
					// parser.getAttributeValue("",CapabilitiesTags.RESY);
					// if ((value != null) && (Utilities.isNumber(value)))
					// bbox.setYmax(Double.parseDouble(value));
					...
					addSrs(bbox.getSrs());
						...
						CapabilitiesTags.EX_GEOGRAPHICBOUNDINGBOX) == 0) {
					// minimum bounding rectangle in decimal degrees of the area
					// covered by the layer.
					bbox = parseEXGeographicBBTag(parser);
					addBBox(bbox);
					setLatLonBox(bbox);
					addSrs(bbox.getSrs());
				} else if (CapabilitiesTags.MAX_SCALE_DENOMINATOR
						.compareToIgnoreCase(parser.getName()) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
						setScaleMax(Double.parseDouble(value));
				} else if (CapabilitiesTags.MIN_SCALE_DENOMINATOR
						.compareToIgnoreCase(parser.getName()) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
	...
	/**
	 * <p>
	 * Parses the EX_GeographicBoundingBox
	 * </p>
	 */
	private BoundaryBox parseEXGeographicBBTag(KXmlParser parser)
			throws IOException, XmlPullParserException {
		int currentTag;
		boolean end = false;
		...
		parser.require(KXmlParser.START_TAG, null,
				CapabilitiesTags.EX_GEOGRAPHICBOUNDINGBOX);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						CapabilitiesTags.WESTBOUNDLONGITUDE) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
						bbox.setXmin(Double.parseDouble(value));
				} else if (parser.getName().compareTo(
						CapabilitiesTags.EASTBOUNDLONGITUDE) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
						bbox.setXmax(Double.parseDouble(value));
				} else if (parser.getName().compareTo(
						CapabilitiesTags.NORTHBOUNDLATITUDE) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
						bbox.setYmax(Double.parseDouble(value));
				} else if (parser.getName().compareTo(
						CapabilitiesTags.SOUTHBOUNDLATITUDE) == 0) {
					value = parser.nextText();
					if ((value != null) && (Utilities.isNumber(value)))
						bbox.setYmin(Double.parseDouble(value));
				}
