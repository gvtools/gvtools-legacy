...
public class WFSProtocolHandlerFactory {
	 ...
	 * 
	 * @param caps
	 *            Capabilities con la respuesta del servidor
	 * @param clients
	 *            Iterador de conjunto ordenado descendientemente
	 * 
	 ...
	 * 
	 */
	private static String getDriverVersion(String version, Iterator clients)
	 ...
	 * 
	 * @param host
	 *            maquina con la que se negocia
	 * 
	 * @return instancia de un cliente capaz de negociar con el host que se pasa
	 *         como parámetro
	 */
	public static WFSProtocolHandler negotiate(String host)
		...
		try {
			String highestVersionSupportedByServer = getSuitableWFSVersion(
					host, "");
			if (supportedVersions.contains(highestVersionSupportedByServer)) {
				...
				// we start the negotiation process in which we have to get the
				// higest version
				...
				while (iVersion.hasNext()) {
					...
					if (res == 0) { // Si es la misma que nuestra version
						...
						throw new Exception("Server Version too high: "
								+ wfsVersion);
					} else { // Si es menor que nuestra version
						// Obtenemos la primera version menor o igual que
						// tengamos
						String lowerVersion = WFSProtocolHandlerFactory
								.getDriverVersion(wfsVersion, iVersion);

						if (lowerVersion == null) { // Si no hay ninguna
							throw new Exception("Lowest server version is "
									+ wfsVersion);
						} else {
							if (lowerVersion.equals(wfsVersion)) {
								return createVersionDriver(lowerVersion);
							} else { // Si hay una version menor que la que
										// retorno el servidor
								// iV = lower;
							}
						}
	...
	/**
	 * Sends a GetCapabilities to the WFS server to get the version if the
	 * version parameter is null, the WFS will return the highest version
	 * supported if not it will return the lower highest version than the one
	 * requested.
	 * 
	 * @param host
	 * @param version
	 ...
	 */
	private static String getSuitableWFSVersion(String host, String _version)
			throws ConnectException, IOException {
		String request = WFSProtocolHandler
				.buildCapabilitiesSuitableVersionRequest(host, _version);
		String version = new String();
		Reader reader = null;
		try {
			File file = Utilities.downloadFile(new URL(request),
					"wfs-suitable-version", null);
	...
	private static WFSProtocolHandler createVersionDriver(String version) {
		try {
			...
			driver = Class.forName("org.gvsig.remoteClient.wfs.wfs_" + version
					+ ".WFSProtocolHandler" + version);
			return (WFSProtocolHandler) driver.newInstance();
		} catch (Exception e) {
			e.printStackTrace();
			// throw new Exception("WFSDriverFactory. Unknown driver version " +
			// e);
			return null;
		}
	}
