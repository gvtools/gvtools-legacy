...
/**
 * WCSClient managing the low-level comunication to the server. It is used as a
 * bridge between a standard WCS server and other high-level clients.
 * 
...
public class WCSClient extends org.gvsig.remoteClient.RasterClient {
	...
	public boolean connect(boolean override, ICancellable cancel) {
		try {
			...
			describeCoverage(null, override, cancel); // TODO falta posar el
														// onlineresource del
														// describe coverage
			return true;
	...
	/**
	 * Sends a GetCapabilities request using the properties contained by the
	 * status. If status is null, then it uses the default configuration.
	 * 
	 * @param override
	 * @param WCSStatus
	 *            , containing the status properties
	 */
	private void getCapabilities(WCSStatus status, boolean override,
			ICancellable cancel) {
		handler.getCapabilities(status, override, cancel);
	}

	/**
	 * Sends a DescribeCoverage request using the properties contained by the
	 * status.
	 * 
	 * @param override
	 ...
	 *            , containing the status properties
	 */
	private void describeCoverage(WCSStatus status, boolean override,
			ICancellable cancel) {
		handler.describeCoverage(status, override, cancel);
		// check it was response or if we need to perform a specific
		// DescribeCoverage for each coverage
	...
	/**
	 * Returns the title of the service. The title is a human-readable string
	 * format used to label the service connection.
	 * 
	 * @return String
	 */
	...
	/**
	 * Returns a brief description of the service, it is a human-readable
	 * string.
	 * 
	 * @return String
	 */
	...
	/**
	 * Returns a hash table containing the WCSCoverage's produced at parse time
	 * using the coverage names as the Hashtable's keys
	 * 
	 * @return Hashtable.
	 */
	...
	/**
	 * Given a coverage name and the CRS name, it returns the extent defined by
	 * the server in the DescribeCoverage document.
	 * 
	 * @param coverageName
	 * @param crs
	 * @return Rectangle2D
	...
	public Rectangle2D getExtent(String coverageName, String crs) {
		BoundaryBox bbox = (BoundaryBox) ((WCSCoverage) layerPool
				.get(coverageName)).getBbox(crs);
		;
		if (bbox == null)
			return null;
	...
	/**
	 * Sends the GetCoverage request according to the settings passed in the
	 * status argument.
	 * 
	 * @param status
	 * @return File
	...
	public File getCoverage(WCSStatus status, ICancellable cancel)
			throws ServerErrorException, WCSException {
