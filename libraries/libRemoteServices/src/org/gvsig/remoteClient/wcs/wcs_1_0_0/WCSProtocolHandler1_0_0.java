...
public class WCSProtocolHandler1_0_0 extends WCSProtocolHandler {
	...
	/**
	 * <p>
	 * Parse the xml data retrieved from the WCS, it will parse the WCS
	 * Capabilities
	 * </p>
	 * 
	 */
		...
		try {
			...
			if (parser.getEventType() != KXmlParser.END_DOCUMENT) {
				parser.require(KXmlParser.START_TAG, null,
						CapabilitiesTags.WCS_CAPABILITIES_ROOT1_0_0);
				tag = parser.nextTag();
					...
					case KXmlParser.START_TAG:
						if (parser.getName()
							...
							parseCapabilityTag(parser);
								...
								CapabilitiesTags.WCS_CONTENTMETADATA) == 0) {
	 ...
	 * 
	 * @see
	 * org.gvsig.remoteClient.wcs.WCSProtocolHandler#parseDescribeCoverage(java
	 * .io.File)
	 */
	public boolean parseDescribeCoverage(File f) {
		...
		try {
			...
			if (parser.getEventType() != KXmlParser.END_DOCUMENT) {
				parser.require(KXmlParser.START_TAG, null,
						DescribeCoverageTags.COVERAGE_DESCRIPTION);
				tag = parser.nextTag();
				while (tag != KXmlParser.END_DOCUMENT) {
					switch (tag) {
					case KXmlParser.START_TAG:
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareToIgnoreCase(CapabilitiesTags.NAME) == 0) {
					serviceInfo.name = parser.nextText();
						...
						CapabilitiesTags.WCS_LABEL) == 0) {
					serviceInfo.title = parser.nextText();
				} else if (parser.getName().compareTo(
						CapabilitiesTags.WCS_DESCRIPTION) == 0) {
					serviceInfo.abstr = parser.nextText();
				} else if (parser.getName().compareTo(
						CapabilitiesTags.WCS_KEYWORDS) == 0) {
					parser.skipSubTree();
				} else if (parser.getName().compareTo(
						CapabilitiesTags.ACCESSCONSTRAINTS) == 0) {
					parser.skipSubTree();
			...
			case KXmlParser.START_TAG:
				if (parser.getName()
							...
							CapabilitiesTags.DCPTYPE);
					{
						...
						CapabilitiesTags.DESCRIBECOVERAGE) == 0) {
					currentTag = parser.nextTag();
					parser.require(KXmlParser.START_TAG, null,
							CapabilitiesTags.DCPTYPE);
					{
						...
						CapabilitiesTags.GETCOVERAGE) == 0) {
					currentTag = parser.nextTag();
					parser.require(KXmlParser.START_TAG, null,
							CapabilitiesTags.DCPTYPE);
					{
	...
	private void parseContentMetadataTag(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				CapabilitiesTags.WCS_CONTENTMETADATA);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						"wcs:" + CapabilitiesTags.WCS_COVERAGEOFFERINGBRIEF) == 0) {
