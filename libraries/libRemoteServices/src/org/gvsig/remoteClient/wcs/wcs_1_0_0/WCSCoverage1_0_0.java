...
public class WCSCoverage1_0_0 extends WCSCoverage {
	...
	public void parse(KXmlParser parser) throws XmlPullParserException,
		...
		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.COVERAGE_OFFERING);
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(DescribeCoverageTags.NAME) == 0) {
					...
					System.out.println("Parsing ... " + value);
						...
						DescribeCoverageTags.LABEL) == 0) {
					value = parser.nextText();
					if (value != null)
						setTitle(value);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.LONLAT_ENVELOPE) == 0) {
					BoundaryBox bBox = parseLonLatGMLEnvelope(parser);
					bBox.setSrs(DescribeCoverageTags.WGS84);
					addBBox(bBox);
					setLonLatBox(bBox);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.DOMAINSET) == 0) {
					parseDomainSet(parser);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.RANGESET) == 0) {
					parseRangeSet(parser);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.SUPPORTED_CRSS) == 0) {
					parseSupportedCRSS(parser);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.SUPPORTED_FORMATS) == 0) {
					setNativeFormat(parser.getAttributeValue("",
							DescribeCoverageTags.NATIVE_FORMAT));
					parser.nextTag();
					parser.require(KXmlParser.START_TAG, null,
							DescribeCoverageTags.FORMATS);
					while (parser.getName().compareTo(
							DescribeCoverageTags.FORMATS) == 0) {
						...
						DescribeCoverageTags.SUPPORTED_INTERPOLATIONS) == 0) {
					...
					while (!bInterpolationsEnd) {
						...
						case KXmlParser.START_TAG:
							if (parser.getName().compareTo(
									DescribeCoverageTags.INTERPOLATION_METHOD) == 0) {
					...
					if (!parser.getName().equals("rangeSet")) // just a patch to
																// avoid too
																// much messages
			...
			case KXmlParser.END_TAG:
				if (parser.getName().compareTo(
						DescribeCoverageTags.COVERAGE_OFFERING) == 0)
					end = true;
				break;
			case KXmlParser.TEXT:
				if (parser.getName() != null)
	...
	private void parseSupportedCRSS(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.SUPPORTED_CRSS);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						DescribeCoverageTags.REQUEST_RESPONSE_CRSS) == 0) {
					String[] values = parser.nextText().split(SEPARATOR);
					for (int i = 0; i < values.length; i++) {
						addSRSs(values[i]);
					}
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.NATIVE_CRS) == 0) {
					setNativeSRS(parser.nextText());
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.REQUEST_CRSS) == 0) {
					// TODO
					System.out.println("Skiped " + parser.getName());
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.RESPONSE_CRSS) == 0) {
					// TODO
					System.out.println("Skiped " + parser.getName());
				} else {
	...
	private void parseRangeSet(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.RANGESET);
		currentTag = parser.next();
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(DescribeCoverageTags.NAME) == 0)
					...
					setRangeSetLabel(parser.nextText());
						...
						DescribeCoverageTags.AXISDESCRIPTION) == 0) {
	...
	private AxisDescription parseAxisDescription(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.AXISDESCRIPTION);
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(DescribeCoverageTags.NAME) == 0) {
					...
					while (!valuesEnd) {
						...
						case KXmlParser.START_TAG:
							if (parser.getName().compareTo(
									DescribeCoverageTags.SINGLEVALUE) == 0) {
								as.addSingleValues(parser.nextText());
							} else if (parser.getName().compareTo(
									DescribeCoverageTags.INTERVAL) == 0) {
								// TODO: this is a patch. The rigth solution is
								// change the method as.setInterval(String[])
								// where interval is composed by min and max
								// This patch assumes interval is composed by
								// tags "min" and "max" as integer values
								// as.setInterval(parser.nextText());
								int min = 1;
								int max = 1;
								parser.nextTag();
								if (parser.getName().compareTo("min") == 0) {
									min = new Integer(parser.nextText())
											.intValue();
								}
								parser.nextTag();
								if (parser.getName().compareTo("max") == 0) {
									max = new Integer(parser.nextText())
											.intValue();
								}
								for (int k = min; k <= max; k++) {
									as.addSingleValues(new Integer(k)
											.toString());
								}
							} else if (parser.getName().compareTo(
									DescribeCoverageTags.DEFAULT) == 0) {
						...
						DescribeCoverageTags.NULLVALUES) == 0) {
					// TODO
					System.err
							.println("Missing NULL VALUE PARSING on WCS 1.0.0 Client");
				} else {
					System.out.println("Unrecognized " + parser.getName());
				}
	...
	private void parseDomainSet(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.DOMAINSET);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						DescribeCoverageTags.SPATIALDOMAIN) == 0) {
					parseSpatialDomain(parser);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.TEMPORALDOMAIN) == 0) {
	...
	private void parseTemporalDomain(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.TEMPORALDOMAIN);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						DescribeCoverageTags.GML_TIMEPOSITION) == 0) {
					addTimePosition(parser.nextText());
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.TIMEPERIOD) == 0) {
					...
					while (!timePeriodEnd) {
						...
						case KXmlParser.START_TAG:
							if (parser.getName().compareTo(
									DescribeCoverageTags.BEGINPOSITION) == 0) {
								myTimePeriod[0] = parser.nextText();
							} else if (parser.getName().compareTo(
									DescribeCoverageTags.ENDPOSITION) == 0) {
								myTimePeriod[1] = parser.nextText();
							} else if (parser.getName().compareTo(
									DescribeCoverageTags.TIMERESOLUTION) == 0) {
	...
	private void parseSpatialDomain(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		boolean end = false;

		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.SPATIALDOMAIN);
		currentTag = parser.next();
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(
						DescribeCoverageTags.GML_ENVELOPE) == 0) {
					BoundaryBox bBox = parseGMLEnvelope(parser);
					addBBox(bBox);
				} else if (parser.getName()
						.compareTo(DescribeCoverageTags.GRID) == 0) {
					parseGrid(parser);
				} else if (parser.getName().compareTo(
						DescribeCoverageTags.RECTIFIEDGRID) == 0) {
	...
	private void parseGrid(KXmlParser parser) throws XmlPullParserException,
			IOException {
		int currentTag;
		String value;
		boolean end = false;
		parser.require(KXmlParser.START_TAG, null, DescribeCoverageTags.GRID);
		rg = new RectifiedGrid(Integer.parseInt(parser.getAttributeValue("",
				DescribeCoverageTags.DIMENSION)));
		...
		while (!end) {
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(DescribeCoverageTags.GML_LIMITS) == 0) {
					parser.nextTag();
					if (parser.getName().compareTo(
							DescribeCoverageTags.GML_GRIDENVELOPE) == 0) {
						...
						while (!gridEnd) {
							...
							case KXmlParser.START_TAG:
								...
								if (parser.getName().compareTo(
										DescribeCoverageTags.GML_LOW) == 0) {
									value = parser.nextText();
									...
									rg.addLowGridEnvelopLimit(limits);
										...
										DescribeCoverageTags.GML_HIGH) == 0) {
									value = parser.nextText();
									String[] ss = value.split(SEPARATOR);
									int[] limits = new int[ss.length];
									for (int i = 0; i < limits.length; i++) {
						...
						DescribeCoverageTags.GML_AXISNAME) == 0) {
	...
	private void parseRectifiedGrid(KXmlParser parser)
			throws XmlPullParserException, IOException {
		int currentTag;
		String value;
		boolean end = false;
		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.RECTIFIEDGRID);
		rg = new RectifiedGrid(Integer.parseInt(parser.getAttributeValue("",
				DescribeCoverageTags.DIMENSION)));
			...
			case KXmlParser.START_TAG:
				if (parser.getName().compareTo(DescribeCoverageTags.GML_LIMITS) == 0) {
					...
					rg.addAxisName(parser.nextText());
						...
						DescribeCoverageTags.GML_ORIGIN) == 0) {
					parser.nextTag();
					if (parser.getName()
							.compareTo(DescribeCoverageTags.GML_POS) == 0) {
						value = parser.nextText();
						String[] ss = value.split(SEPARATOR);
						...
						DescribeCoverageTags.OFFSETVECTOR) == 0) {
					value = parser.nextText();
					String[] ss = value.split(SEPARATOR);
	...
	private BoundaryBox parseLonLatGMLEnvelope(KXmlParser parser)
			throws XmlPullParserException, IOException {
		...
		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.LONLAT_ENVELOPE);
		currentTag = parser.next();
	...
	private BoundaryBox parseGMLEnvelope(KXmlParser parser)
			throws XmlPullParserException, IOException {
		BoundaryBox bBox = new BoundaryBox();
		int currentTag;
		String value;
		...
		parser.require(KXmlParser.START_TAG, null,
				DescribeCoverageTags.GML_ENVELOPE);
