...
public abstract class WCSProtocolHandler extends OGCProtocolHandler {
	...
	/**
	 * <p>
	 * Builds a GetCapabilities request that is sent to the WCS the response
	 * will be parse to extract the data needed by the WCS client.
	 * </p>
	 * 
	 * @param override
	 *            , if true the cache is ignored
	 */
	public void getCapabilities(WCSStatus status, boolean override,
			ICancellable cancel) {
		URL request = null;
		try {
		...
		try {
			...
			File f = Utilities.downloadFile(request, "wcs_capabilities.xml",
					cancel);
			if (f != null)
				parseCapabilities(f);
		} catch (Exception e) {
	...
	/**
	 * Builds a complete URL-string that can be used to send a GetCapabilities
	 * request.
	 * 
	 * @return String
	 */
	private String buildCapabilitiesRequest(WCSStatus status) {
		...
		req.append(onlineResource).append(symbol)
				.append("REQUEST=GetCapabilities&SERVICE=WCS&");
		req.append("VERSION=").append(getVersion()).append("&EXCEPTIONS=XML");
		return req.toString();
	}

	/**
	 * Builds a complete URL-string that can be used to send a DescribeCoverage
	 * request. If status is null, then default settings are used.
	 * 
	 * @param WCSStatus
	 * @return String
	 */
	private String buildDescribeCoverageRequest(WCSStatus status) {
		...
		req.append(onlineResource).append(symbol)
				.append("REQUEST=DescribeCoverage&SERVICE=WCS&");
	...
	/**
	 * Send a DescribeCoverage request using the settings passed in the status
	 * argument. If status is null, then default settings are used.
	 * 
	 * @param override
	 * @return String
	 */
	public void describeCoverage(WCSStatus status, boolean override,
			ICancellable cancel) {
		URL request = null;
		try {
		...
		try {
			...
			File f = Utilities.downloadFile(request,
					"wcs_describeCoverage.xml", cancel);
			if (f != null)
	...
	public File getCoverage(WCSStatus status, ICancellable cancel)
		...
		try {
			// TODO:
			// pass this buildXXXRequest to the WCSProtocolHandlerXXX: The
			// request can depend on the WCS version.
	...
	private String buildCoverageRequest(WCSStatus status) {
		...
		req.append(onlineResource + symbol + "service=WCS&version=")
				.append(getVersion()).append("&request=GetCoverage&");
	...
	public String getPartialQuery(WCSStatus status) {
		...
		req.append((status.getTime() != null) ? "TIME=" + status.getTime() : "")
				...
				.append((status.getDepth() != null) ? "&DEPTH="
						+ status.getDepth() : "")
				...
				.append((status.getParameters() != null) ? "&"
	...
	public static String buildCapabilitiesSuitableVersionRequest(String _host,
		...
		if ((_version != null) && (_version.compareTo("") != 0))
			_host += "REQUEST=GetCapabilities&SERVICE=WCS&VERSION=" + _version
					+ "&EXCEPTIONS=XML";
		else
