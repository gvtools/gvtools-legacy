...
public class WCSProtocolHandlerFactory {
	 ...
	 *         como parámetro
	 */
	public static WCSProtocolHandler negotiate(String host)
		...
		try {
			String highestVersionSupportedByServer = getSuitableWCSVersion(
					host, "");
			if (supportedVersions.contains(highestVersionSupportedByServer)) {
				...
				while (iVersion.hasNext()) {
					...
					if (res == 0) { // Si es la misma que nuestra version
						...
						// tengamos
						String lowerVersion = WCSProtocolHandlerFactory
								.getDriverVersion(wcsVersion, iVersion);
	...
	/**
	 * Sends a GetCapabilities to the WCS server to get the version if the
	 * version parameter is null, the WCS will return the highest version
	 ...
	 */
	private static String getSuitableWCSVersion(String host, String _version)
			throws ConnectException, IOException {
		String request = WCSProtocolHandler
		...
		try {
			...
			if (kxmlParser.getEventType() != KXmlParser.END_DOCUMENT) {
				if ((kxmlParser.getName().compareTo(
						CapabilitiesTags.WCS_CAPABILITIES_ROOT1_0_0) == 0)) {
	...
	private static WCSProtocolHandler createVersionDriver(String version) {
		try {
			...
			driver = Class.forName("org.gvsig.remoteClient.wcs.wcs_" + version
					+ ".WCSProtocolHandler" + version);
			return (WCSProtocolHandler) driver.newInstance();
		} catch (Exception e) {
			e.printStackTrace();
			// throw new Exception("WCSDriverFactory. Unknown driver version " +
			// e);
			return null;
		}
	}
