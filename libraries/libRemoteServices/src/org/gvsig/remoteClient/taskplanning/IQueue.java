 ...
 * plannify which is the next, the previous,.. task to be done. However, if you
 * don't want to write anything special, just guess a simple FIFO queue<br>
 * </p>
 * 
...
public interface IQueue {
	...
	/**
	 * Adds a new task to the queue. The place where the new task will be put
	 * its left to the concrete implementation of this interface.
	 * 
	 * @param IRunnableTask
	 *            task
	 */
	...
	/**
	 * Sets the TaskPlanner that will decide which of the tasks in the queue
	 * will be executed next. A null value should represent a FIFO planner.
	 * 
	 * @param planner
	 */
	...
	/**
	 * Causes the execution of this queue to be paused. The task currently in
	 * execution finishes and after it the planner will not issue more tasks
	 * until resume() is invoked.
	 */
	void pause();

	/**
	 * Causes the execution of this queue to be resumed. The execution will
	 * continue with the next task issued by the planner. It has no effect if
	 * the queue was not paused yet.
	 */
	void resume();
