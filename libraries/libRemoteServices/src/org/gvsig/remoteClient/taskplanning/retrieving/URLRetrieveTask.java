...
/**
 * Clase para bajar ficheros en un thread independiente. La idea (y parte del
 * código) está tomada de EarthFlicks
 * 
...
public class URLRetrieveTask implements IRunnableTask {
	...
	public void execute() {
		...
		try {
			...
			if (!cancelled) {
				long readed = 0;
				for (int i = is.read(buffer); !cancelled && i > 0; i = is
						.read(buffer)) {
			...
			if (cancelled) {
				...
				synchronized (this) {
					RequestManager.getInstance().addDownloadedURLRequest(
							request, f.getAbsolutePath());
				}
			}
 ...
 * (non-Javadoc)
 * 
 * @see java.lang.Runnable#run() / public void run() { try { URL url =
 * request.getUrl(); this.status = Status.CONNECTING; this.begin();
 * 
 * { // Retrieve the contents. int numBytesRead = 0; //
 * System.out.println("File being retrieved. " + this.getUrl().toString());
 * java.net.URLConnection connection = url.openConnection();
 * connection.setAllowUserInteraction(true); java.io.InputStream incoming =
 * connection.getInputStream(); this.contentLength =
 * connection.getContentLength(); this.contentType =
 * connection.getContentType();
 * 
 * java.io.File destFile = java.io.File.createTempFile("Cq_",
 * makeSuffix(this.contentType)); destFile.deleteOnExit(); // It's copied later
 * if it's to be persisted. this.destination = destFile.getPath();
 ...
 * 
 * try { while (!Thread.currentThread().isInterrupted() && numBytesRead >= 0) {
 * numBytesRead = incoming.read(buffer); if (numBytesRead > 0) { this.bytesRead
 * += numBytesRead; outgoing.write(buffer, 0, numBytesRead); this.update(); } }
 * } finally { if (incoming != null) try {incoming.close();} catch
 * (java.io.IOException e) {}; // TODO: log it maybe if (outgoing != null) try
 * {outgoing.close();} catch (java.io.IOException e) {}; // TODO: log it maybe }
 * this.status = Status.POSTPROCESSING; } } catch (Exception e) { this.status =
 * Status.ERROR; this.error = e; e.printStackTrace(); } finally { if
 * (Thread.currentThread().isInterrupted()) this.status = Status.INTERRUPTED;
 * this.update(); this.inThreadEnd(); if (this.status == Status.POSTPROCESSING)
 * this.status = Status.DONE; this.end(); } }
 */
...
// } finally {
// if (incoming != null)
// try {incoming.close();} catch (java.io.IOException e) {}; // TODO: log it
// maybe
// if (outgoing != null)
// try {outgoing.close();} catch (java.io.IOException e) {}; // TODO: log it
// maybe
// }
