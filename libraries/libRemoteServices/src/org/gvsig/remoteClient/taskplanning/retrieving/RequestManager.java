...
/**
 * pa administrar les tasques (la història aquella de que hi haja una cola per a
 * cada servidor)
 * 
...
public class RequestManager {
	...
	public URLRetrieveTask addURLRequest(URLRequest request,
		...
		try {
			...
			// TODO canviar per a quetorne el Request antic, que la request
			// guarde el
			...
			if (f != null) {
				...
				System.out.println(request.getUrl() + " is cached at '"
						+ f.getAbsolutePath() + "'");

				// get this server's task queue
				RetrieveQueue serverQueue = (RetrieveQueue) getQueue(request
						.getHost());

				// Look up the previous cached jobs

				URLRetrieveTask workingTask = serverQueue
						.getURLPreviousRequest(request);
				if (workingTask == null) {
					// Task already done. Notify listener
					if (debug)
						System.err.println("done job found: "
								+ request.getUrl());
				...
				if (!tempDir.exists())
					tempDir.mkdir();
				String fileNamePrefix = tempDirectoryPath + File.separator
						+ host + "-";
				...
				// TODO
				// jo ací comprovaria quin protocol, mètode, host, etc... i
				// crearia un
				// objecte que tractara la descàrrega segons el mètode que li
				// toca.
				...
				// Enqueue the request and the listener will be notified when
				// done.
	...
	private final String tempDirectoryPath = System
