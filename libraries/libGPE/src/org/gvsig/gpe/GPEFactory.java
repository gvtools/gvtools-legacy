 ...
 */
/**
 * API entry point to create parsers and writers based on MIME-Type
 * identification.
 * <p>
 * This factory uses a SPI (Service Provider Interface) mechanism to look up for
 * {@link GPEParser} and {@link IGPEWriterHandlerImplementor} implementations
 * that can deal with a requested MIME-Type.
 * </p>
 * <h2>Implementation Lookup</h2>
 * <p>
 * The SPI mechanism keeps parser implementations decoupled from client
 * applications allowing the transparent interchange of implementation, thus
 * defining a format plugin system.
 * </p>
 * <p>
 * Parser and Writer implementations registers themselves lazily by providing a
 * small configuration file bundled together with the rest of the implementation
 * resources. This configuration file consist of a text file at a specific
 * location and with an specific file name, this factory look up mechanism will
 * search for in order to find out the implementation class names.
 * </p>
 * <p>
 * To register a GPEParser implememntation, a file named
 * <code>org.gvsig.gpe.parser.GPEParser</code> shall exist in the class path
 * (hence the ability to have multiple files equally namded budled in different
 * jar files), in the implementation's <code>META-INF/services</code> folder.
 * </p>
 * <p>
 * To register a Writer handler implementation, the same approach shall be
 * followed with a file named
 * <code>META-INF/services/org.gvsig.gpe.writer.IGPEWriterHandlerImplementor</code>
 * </p>
 * <p>
 * The content of both files for a given implementation consists of full
 * qualified class names, one per line. For example, an hypotetical
 * <code>MyFormatParser</code> in the package <code>org.mycompany.gpe</code> and
 * bundled in a jar file called <code>myformat.jar</code> shall provide the
 * following resources:
 * 
 * <pre>
 * <code>
 ...
 * <code>META-INF/services/org.gvsig.gpe.parser.GPEParser</code> shall be a
 * single line of text with the <code>org.mycompany.gpe.MyFormatParser</code>
 * class name.
 * 
...
public class GPEFactory {
	 ...
	 * <p>
	 * NOTE: since GPEParser is an abstract and statefull class rather than an
	 * interface, this method shall stay private in order to ensure we return a
	 * new instance for the required format every time.
	 * </p>
	 * 
	 ...
	 * <p>
	 * NOTE: since GPEWriter is an abstract and statefull class rather than an
	 * interface, this method shall stay private in order to ensure we return a
	 * new instance for the required format every time.
	 * </p>
	 * 
	...
	private static Iterator availableWriters() {
		Iterator providers = sun.misc.Service
				.providers(IGPEWriterHandlerImplementor.class);
		return providers;
	}

	/**
	 * Create a new parser from a class name. This method can be used if the
	 * class name is known but, the common method to get a parser is using the
	 * mime type. *
	 * 
	 * @param prefferredImplClassName
	 *            The name of the class that implements {@link GPEParser}
	 * @return A parser for a concrete format
	...
	/**
	 * Create a new parser from a parser name. This method can be used if the
	 * parser name is known but, the common method to get a parser is using the
	 * mime type.
	 * 
	 * @param name
	 *            The name of the parser, that is returned by the
	 *            {@link GPEParser#getName} method
	 * @return A parser for a concrete format
	 * @throws ParserCreationException
	 *             If it is not possible to create a parser
	 */
	...
	/**
	 * Create a new parser from a {@link URI}. Each parser has a method named
	 * {@link GPEParser#accept(URI)} that return true if the parser can be open
	 * the file.
	 * <p>
	 * This method retrieve all the registered parsers and invoke the
	 * <code>accept</code> method. The first parser that retuns true is
	 * returned. It means that if there are more that one parser to open a
	 * concrete file the first parser found will be returned.
	 * </p>
	 * 
	 * @param uri
	 ...
	 * {@link GPEParser#getFormat()} that returns the mimetype that the parser
	 * can read. One parser only supports one mimetype.
	 * <p>
	 * This method retrieve all the parsers and returns the first parser that is
	 * able to read the mimetype. If there are more parsers that can open the
	 * file will not be used.
	 * </p>
	 * 
	 * @param mimeType
	 *            The mimetype of the file to open
	 * @return A parser that can parse the mimetype.
	 * @throws ParserCreationException
	 *             If it is not possible to create a parser
	 * @see <a
	 *      href="http://www.iana.org/assignments/media-types/">http://www.iana.org/assignments/media-types/</a>
	 */
	public static GPEParser createParserByMimeType(String mimeType)
			throws ParserCreationException {
		Iterator providers = availableParsers();
		while (providers.hasNext()) {
			GPEParser parser = (GPEParser) providers.next();
	...
	/**
	 * Create a new writer from a writer name. This method can be used if the
	 * writer name is known but, the common method to get a writer is using the
	 * mime type.
	 * 
	 * @param name
	 *            The name of the writer, that is returned by the
	 *            {@link GPEWriterHandler#getName} method
	 * @return A writer for a concrete format
	...
	/**
	 * Create a new writer from a class name. This method can be used if the
	 * class name is known but, the common method to get a writer is using the
	 * mime type. *
	 * 
	 * @param prefferredImplClassName
	 ...
	 *             If it is not possible to create a writer
	 */
	public static GPEWriterHandler createWriterByClass(
			String prefferredImplClassName)
			throws WriterHandlerCreationException {
		Class prefferredImplClass = null;
		try {
			prefferredImplClass = Class.forName(prefferredImplClassName);
		...
		Iterator providers = availableWriters();
		while (providers.hasNext()) {
			IGPEWriterHandlerImplementor next = (IGPEWriterHandlerImplementor) providers
					.next();
			if (prefferredImplClass.isInstance(next)) {
	 ...
	 * {@link GPEWriterHandler#getFormat()} that returns the mimetype that the
	 * writer can write. One writer only supports one mimetype.
	 * <p>
	 * This method retrieve all the writers and returns the first one that is
	 * able to write the mimetype. If there are more writer that can write the
	 * format will not be used.
	 * </p>
	 * 
	 * @param mimeType
	 *            The mimetype of the file to write
	 * @return A writer that can write the mimetype.
	 * @throws WriterHandlerCreationException
	 *             If it is not possible to create a writer
	 * @see <a
	 *      href="http://www.iana.org/assignments/media-types/">http://www.iana.org/assignments/media-types/</a>
	...
	/**
	 * Return <code>true</code> if there is a parser that is able to open the
	 * file or <code>false</code> instead.
	 * 
	 * @param uri
	 *            The file to open
	 * @return <code>true</code> if there is a parser that is able to open the
