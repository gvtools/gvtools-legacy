...
public class SchemaUtils {
	 ...
	 * @param tagName
	 *            Node name
	 * @return <code>true</code> if the node has this name
	 */
	public static boolean matches(Node n, String namespace, String tagName) {
		String nodeName = n.getNodeName().substring(
				n.getNodeName().indexOf(":") + 1, n.getNodeName().length());
		int index = n.getNodeName().indexOf(":");
		String nodeNSURI = null;
		if (index > 0) {
			nodeNSURI = n.getNodeName().substring(0,
					n.getNodeName().indexOf(":"));
		}
		if (nodeNSURI == null) {
	 ...
	 * @return <code>true</code> or <code>false</code> according to the above
	 *         description.
	 */
	public static boolean matches(QName qname1, QName qname2) {
		return objNullEq(qname1.getNamespaceURI().toUpperCase(), qname2
				.getNamespaceURI().toUpperCase())
				&& objNullEq(qname1.getLocalPart().toUpperCase(), qname2
	...
	/**
	 * Search a Element with a specified type and a specified attribute name
	 * 
	 * @param element
	 *            Parent element
	 * @param tagName
	 ...
	 * @return
	 */
	public static Element searchChildByAttributeName(Element element,
			String tagName, String targetNamespace, String attributeName) {
		NodeList nodeList = element.getChildNodes();
		for (int i = 0; i < nodeList.getLength(); i++) {
			Node node = nodeList.item(i);
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				if (SchemaUtils.matches(node, SchemaTags.XS_NS, tagName)) {
					String name = ((Element) node)
