...
public class GeometryAsserts {
	...
	public static void lineString(LineString lineString, double[] x,
			double[] y, double[] z) {
		...
		if (lineString != null) {
			for (int i = 0; i < lineString.getCoordinatesNumber(); i++) {
				...
				TestCase.assertEquals(
						new Double(lineString.geCoordinateAt(i, 0)),
						new Double(x[i]));
				TestCase.assertEquals(
						new Double(lineString.geCoordinateAt(i, 1)),
						new Double(y[i]));
				TestCase.assertEquals(
						new Double(lineString.geCoordinateAt(i, 2)),
						new Double(z[i]));
				// }
			}
		}
	...
	public static void linearRing(LinearRing linearRing, double[] x,
			double[] y, double[] z) {
		TestCase.assertNotNull(linearRing);
		if (linearRing != null) {
			for (int i = 0; i < linearRing.getCoordinatesNumber(); i++) {
				TestCase.assertEquals(
						new Double(linearRing.geCoordinateAt(i, 0)),
						new Double(x[i]));
				TestCase.assertEquals(
						new Double(linearRing.geCoordinateAt(i, 1)),
						new Double(y[i]));
				TestCase.assertEquals(
						new Double(linearRing.geCoordinateAt(i, 2)),
						new Double(z[i]));
			}
		}
	}

	public static void polygon(Polygon polygon, double[] x, double[] y,
			double[] z) {
		TestCase.assertNotNull(polygon);
		if (polygon != null) {
			for (int i = 0; i < polygon.getCoordinatesNumber(); i++) {
				TestCase.assertEquals(new Double(polygon.geCoordinateAt(i, 0)),
						new Double(x[i]));
				TestCase.assertEquals(new Double(polygon.geCoordinateAt(i, 1)),
						new Double(y[i]));
				TestCase.assertEquals(new Double(polygon.geCoordinateAt(i, 2)),
						new Double(z[i]));
			}
		}
	}
