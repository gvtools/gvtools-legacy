 ...
 */
/**
 * This test parses some XSD files and calculates the used memory for each of
 * them. The parsing process must to be implemented by the classes tha inherit
 * from this.
 * 
...
public abstract class MemoryUsageTest extends TestCase {
	...
	/**
	 * Parser the same XSD file the number of times specified by TIMES_TO_LOOP
	 * 
	 * @param file
	 *            XSD file to parse
	 * @throws Exception
	...
	private void parseSchemaXTimes(String file) throws Exception {
		...
		System.out.println(usedMemory + " " + sufix + " used to parse " + file
				+ " wich size is " + new File(file).length() / KB + " KB in "
				+ usedTime + " milis");
	}
	...
	public float parseSchemaMemory(String file) throws Exception {
		...
		float usedMemory = (totalAfter - freeAfter)
