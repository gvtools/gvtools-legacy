...
/**
 * Buffer para el almacenamiento de líneas de la imágén. Esta clase es
 * instanciada desde C para que se pueda acceder a sus datos desde el cliente
 * java.
 * 
...
public class GdalBuffer {
	...
	public int getSize() {
		for (int i = 0; i < 5; i++) {
			if (buffByte != null)
				return buffByte.length;
			if (buffShort != null)
				return buffShort.length;
			if (buffInt != null)
				return buffInt.length;
			if (buffFloat != null)
				return buffFloat.length;
			if (buffDouble != null)
				return buffDouble.length;
			if (buffRPalette != null)
				return buffRPalette.length;
		}
		return 0;
	}
