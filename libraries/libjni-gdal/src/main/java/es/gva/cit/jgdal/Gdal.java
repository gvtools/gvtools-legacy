...
public class Gdal extends JNIBase {
	...
	private native int getGeoTransformNat(long cPtr,
			GeoTransform adfgeotransform);

	private native int setGeoTransformNat(long cPtr,
			GeoTransform adfgeotransform);
	...
	private native String getColorInterpretationNameNat(long cPtr,
			int colorInterp);
	 ...
	 */
	public void open(String pszFilename, int access) throws GdalException,
			IOException {
	 ...
	 * @throws GdalException
	 * @param hBand
	 *            Entero que corresponde al número de banda que se quiere
	 *            recuperar
	 ...
	 * 
	 * @return Devuelve un entero con la longitud de la imagen en el eje X en
	 *         pixels.
	 * @throws GdalException
	 */
	 ...
	 * 
	 * @return Devuelve un entero con la longitud de la imagen en el eje Y en
	 *         pixels.
	 * @throws GdalException
	 */
	 ...
	 * Obtiene el número de bandas de la imagen
	 * 
	 * @return Devuelve un entero con el número de bandas que contiene la
	 *         imagen.
	 * @throws GdalException
	 */
	...
	/**
	 * Obtiene el vector geoTransform de la imagen que contiene los valores
	 * Origen y pixelSize
	 * 
	 * @return Devuelve un vector de doubles que contiene los valores de
	 *         coordenadas de origen y pixelSize.
	 * @throws GdalException
	 */
	public GeoTransform getGeoTransform() throws GdalException {
		...
		if (getGeoTransformNat(cPtr, gt) < 0)
			throw new GdalException(
					"Error en getGeoTransform(). No se han obtenido valores para geoTransform.");
		else {
			return gt;
		}
	}
	...
	public String getDriverShortName() throws GdalException {
		...
		if (shortName == null)
			throw new GdalException(
					"Error en getDriverShortName(). No ha podido obtenerse el driver");
		else
			return shortName;
	}

	/**
	 * Añade el vector geoTransform a la imagen que contiene los valores Origen
	 * y pixelSize
	 * 
	 * @return Devuelve un vector de doubles que contiene los valores de
	 *         coordenadas de origen y pixelSize.
	...
	public void setGeoTransform(GeoTransform gt) throws GdalException {
		if (cPtr == 0)
			throw new GdalException("No se ha podido acceder al archivo.");
		if (gt == null)
			throw new GdalException("el objeto " + gt.getClass().getName()
					+ " es null");
	 ...
	 * 
	 * @return Devuelve un String con los datos del sistema de coordenadas de
	 *         referencia.
	 * @throws GdalException
	 */
	 ...
	 * 
	 * @return Devuelve un entero con el numero de bandas que contiene la
	 *         imagen.
	 * @throws GdalException
	 */
	...
	/**
	 * Asigna la proyección especificada en la cadena que se le pasa por
	 * parámetro.
	 * 
	 * @param proj
	 *            proyección
	...
	public void setProjection(String proj) throws GdalException {
		...
		if (res < 0)
			throw new GdalException(
					"Error en setProjection(). No se ha podido asignar la proyección.");
	}

	/**
	 * Obtiene la cadena que representa el tipo de banda de color. Los tipos
	 * posibles son:
	 * <UL>
	 * <LI>0 = "Undefined"</LI>
	 * <LI>1 = "Gray";</LI>
	 ...
	 * @throws GdalException
	 */
	public String getColorInterpretationName(int colorInterp)
			throws GdalException {
		if ((colorInterp < 0) || (colorInterp > 16))
			throw new GdalException(
					"Valor de interpretacion de color fuera de rango");

		String bandTypeName = getColorInterpretationNameNat(cPtr, colorInterp);

		if (bandTypeName == null)
			throw new GdalException(
					"Error en getColorInterpretationName(). No ha podido obtenerse el tipo de banda de color");
		else
			return bandTypeName;
	}
