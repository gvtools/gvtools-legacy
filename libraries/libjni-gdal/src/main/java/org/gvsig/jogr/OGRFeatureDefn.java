...
public class OGRFeatureDefn extends JNIBase {
	...
	private native void setGeomTypeNat(long cPtr, String gtype); // OGRwkbGeometryType
																	// OGRFeatureDefn
	 ...
	 * 
	 * @param cPtr
	 *            dirección de memoria al objeto OGRFeatureDefn de C.
	 */
	...
	public String getName() throws OGRException {
		...
		if (name == null)
			throw new OGRException(
					"Error en getName(). No se ha podido obtener el nombre.");
		return name;
	}
	...
	public String getGeomType() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getGeomType(). El constructor ha fallado.");

		int tipo = getGeomTypeNat(cPtr);

		if (tipo < 0 || tipo > 16)
			throw new OGRException(
					"Error en getGeomType(). No se ha podido obtener un tipo de geometria valido.");
		...
		case 1:
			tipogeom = "wkbPoint";
			break; /* rest are standard WKB type codes */
		case 2:
		...
		case 8:
			tipogeom = "wkbNone";
			break; /* non-standard, for pure attribute records */
		case 9:
			tipogeom = "wkbLinearRing";
			break; /* non-standard, just for createGeometry() */
		case 10:
			tipogeom = "wkbPoint25D";
			break; /* 2.5D extensions as per 99-402 */
		case 11:
	...
	public OGRFieldDefn getFieldDefn(int i) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getFieldDefn(). El constructor ha fallado.");
	...
	public int getFieldIndex(String field) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getFieldIndex(). El constructor ha fallado.");

		if (field == null)
			throw new OGRException(
					"Error en getFieldIndex(). Parámetro invalido.");

		int index = getFieldIndexNat(cPtr, field);

		if (index == 0)
			throw new OGRException(
					"Error en getFieldIndex(). No se ha podido obtener un índice valido.");
		return index;
	...
	public void addFieldDefn(OGRFieldDefn fd) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en addFieldDefn(). El constructor ha fallado.");

		if (fd == null || fd.getPtro() <= 0)
			throw new OGRException(
					"Error en addFieldDefn(). La referencia del parámetro OGRFieldDefn no es valida.");
	...
	public void setGeomType(String gtype) throws OGRException { // OGRwkbGeometryType
																// OGRFeatureDefn

		if (cPtr == 0)
			throw new OGRException(
					"Error en setGeomType(). El constructor ha fallado.");

		boolean ok = false;
		if (gtype != null
				&& (gtype.equals("wkbUnknown") || gtype.equals("wkbPoint")
						...
						|| gtype.equals("wkbMultiPolygon25D") || gtype
							.equals("wkbGeometryCollection25D")))
			ok = true;
	...
	public OGRFeatureDefn cloneFeatureDefn() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en cloneFeatureDefn(). El constructor ha fallado.");

		long ptr_fd = cloneFeatureDefnNat(cPtr);

		if (ptr_fd == 0)
			throw new OGRException(
					"Error en cloneFeatureDefn(). No se ha podido obtener un valor de referencia valido para OGRFeatureDefn.");
	...
	public static OGRFeatureDefn createFeatureDefn(String pszName)
			throws OGRException {

		if (pszName == null)
			throw new OGRException(
					"Error en createFeatureDefn(). Parámetro invalido.");
	...
	public static void destroyFeatureDefn(OGRFeatureDefn fd)
			throws OGRException {

		if (fd == null || fd.getPtro() == 0)
			throw new OGRException(
					"Error en destroyFeatureDefn(). Parámetro invalido.");
