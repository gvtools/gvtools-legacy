...
public class OGRSFDriverRegistrar extends JNIBase {
	...
	public OGRDataSource open(String fte, boolean bUpdate, OGRSFDriver pdriver)
			throws OGRException {
		...
		if (fte == null || fte.equals(""))
			throw new OGRException(
					"Fallo en parametro de entrada de OGRDataSource.open");

		long res = openNat(fte, bU);

		if (res == 0 || driver == 0)
			throw new OGRException(
					"Error en open(). No se ha podido abrir la imágen.");
		else {
	...
	public OGRSFDriver getDriver(int i) throws OGRException {
		...
		if (drv == 0)
			throw new OGRException(
					"Error en getDriver(). No se ha podido obtener el driver indicado.");
