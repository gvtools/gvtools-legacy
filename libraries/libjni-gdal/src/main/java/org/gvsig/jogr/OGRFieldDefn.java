...
public class OGRFieldDefn extends JNIBase {
	...
	private native void set(long cPtr, String s, OGRFieldType ftipe, int i,
			int j, String justification);
	 ...
	 * 
	 * @param cPtr
	 *            dirección de memoria al objeto OGRFieldDefn de C.
	 */
	...
	public OGRFieldType getType() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getType(). El constructor ha fallado.");
		...
		if (tipo >= 0) {
			...
			throw new OGRException(
					"Error en getType(). No se ha podido obtener un tipo valido.");
	...
	public String getNameRef() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getNameRef(). El constructor ha fallado.");

		String ref = getNameRefNat(cPtr);
		if (ref == null)
			throw new OGRException(
					"Error en getNameRef(). No se ha podido obtener un nombre de referencia valido.");
	...
	public String getFieldTypeName(OGRFieldType tipo) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getFieldTypeName(). El constructor ha fallado.");

		String ref = getFieldTypeNameNat(cPtr, tipo.getType());

		if (ref == null)
			throw new OGRException(
					"Error en getFieldTypeName(). No se ha podido obtener un nombre de tipo de campo valido.");
	...
	public void set(String s, OGRFieldType ftipe, int i, int j,
			String justification) throws OGRException {
