...
public class OGRSFDriver extends JNIBase {
	...
	private native long copyDataSourceNat(long cPtr, long poSrcDS,
	...
	public String getName() throws OGRException {
		...
		if (name == null)
			throw new OGRException(
					"Error en getName(). No se ha podido obtener el nombre del driver.");
		return name;
	}
	...
	public OGRDataSource open(String pszName, boolean bUpdate)
			throws OGRException {
		if (cPtr == 0)
			throw new OGRException("Fallo al acceder al dato.");
		...
		if (ptr_ds == 0)
			throw new OGRException(
					"Error en open(). No se ha podido obtener un dataset valido.");
	...
	/**
	 * @param capability
	 *            Capacidad del driver a testear
	 *            <UL>
	 *            <LI>"RandomRead"</LI>
	...
	public boolean testCapability(String capability) throws OGRException {
		...
		if (capability == null || capability.equals(""))
			throw new OGRException(
					"Error en testCapability(). El parámetro pasado a la función no es valido.");
	...
	public OGRDataSource createDataSource(String pszName) throws OGRException {
		...
		if (pszName == null || pszName.equals(""))
			throw new OGRException(
					"Error en createDataSource(). El parámetro pasado a la función no es valido.");

		long ptr_ds = createDataSourceNat(cPtr, pszName);

		if (ptr_ds <= 0)
			throw new OGRException(
					"Error en createDataSource(). No se ha podido obtener un dataset valido.");
	...
	public void deleteDataSource(String pszName) throws OGRException {// Excepciones
		...
		if (pszName == null || pszName.equals(""))
			throw new OGRException(
					"Error en deleteDataSource(). El parámetro pasado a la función no es valido.");
	...
	public OGRDataSource copyDataSource(OGRDataSource poSrcDS,
			String pszNewName, String[] papszOptions) throws OGRException {
		if (poSrcDS == null || pszNewName == null || pszNewName.equals(""))
			throw new OGRException(
					"Error en deleteDataSource().Algún parámetro pasado a la función no es valido.");
		...
		long ptr_ds = copyDataSourceNat(cPtr, poSrcDS.getPtro(), pszNewName,
				papszOptions);

		if (ptr_ds == 0)
			throw new OGRException(
					"Error en copyDataSource(). No se ha podido obtener un dataset valido.");
