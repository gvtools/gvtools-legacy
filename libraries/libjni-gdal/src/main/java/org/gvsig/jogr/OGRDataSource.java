...
public class OGRDataSource extends JNIBase {
	 ...
	 * 
	 * @param cPtr
	 *            dirección de memoria al objeto OGRDataSource de C.
	 */
	...
	public String getName() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getName(). El constructor ha fallado.");

		String name = getNameNat(cPtr);

		if (name == null)
			throw new OGRException(
					"Error en getName(). No se ha podido obtener el nombre del datasource.");
	...
	public OGRLayer getLayer(int i) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getLayer(). El constructor no tuvo exito");
		...
		if (layer == 0)
			throw new OGRException(
					"Error en getLayer(). No se ha podido obtener la capa indicada.");
	...
	public OGRLayer getLayerByName(String name) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getLayerByName(). El constructor no tuvo exito");

		if (name == null || name.equals(""))
			throw new OGRException(
					"Error en getLayerByName(). Parámetro no valido");

		long ptr_name = getLayerByNameNat(cPtr, name);

		if (ptr_name == 0)
			throw new OGRException(
					"Error en getLayerByName(). No se ha podido obtener un puntero a un OGRLayer valido.");
	...
	public void deleteLayer(int iLayer) throws OGRException { // Excepciones
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en deleteLayer(). El constructor no tuvo exito");

		if (iLayer < 0)
			throw new OGRException(
					"Error en deleteLayer(). Parámetro no valido");
	...
	public int testCapability(String odr) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en testCapability(). El constructor no tuvo exito");

		if (odr == null || odr.equals(""))
			throw new OGRException(
					"Error en testCapability(). Parámetro no valido");
		...
		if (res == 0)
			throw new OGRException(
					"Error en testCapability(). No se ha podido obtener un valor de retorno valido.");
	...
	public OGRLayer createLayer(String pszName,
			OGRSpatialReference poSpatialRef, String eGType, // OGRwkbGeometryType
																// OGRFeatureDefn
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en createLayer(). El constructor no tuvo exito");

		if (pszName == null || pszName.equals(""))
			throw new OGRException(
					"Error en createLayer(). Parámetros en la función no validos.");

		if (poSpatialRef == null || poSpatialRef.getPtro() == 0)
			throw new OGRException(
					"Error en createLayer(). Parámetro OGRSpatialReference no valido.");

		ptro_sr = poSpatialRef.getPtro();

		long ptr_layer = createLayerNat(cPtr, pszName, ptro_sr, eGType,
				papszOptions);
		if (ptr_layer == 0)
			throw new OGRException(
					"Error en createLayer(). No se ha podido obtener una referencia valida a un OGRLayer.");
	...
	public OGRLayer copyLayer(OGRLayer poSrcLayer, String pszNewName,
			String[] papszOptions) throws OGRException {
		if (cPtr == 0)
			throw new OGRException(
					"Error en copyLayer(). El constructor no tuvo exito");

		if (pszNewName == null || pszNewName.equals("") || poSrcLayer == null)
			throw new OGRException(
					"Error en copyLayer(). Parámetros en la función no validos.");

		if (poSrcLayer.getPtro() == 0)
			throw new OGRException(
					"Error en copyLayer(). Referencia del parámetro OGRSpatialReference invalida.");

		long ptr_layer = copyLayerNat(cPtr, poSrcLayer.getPtro(), pszNewName,
				papszOptions);
		if (ptr_layer == 0)
			throw new OGRException(
					"Error en copyLayer(). No se ha podido obtener una referencia valida a un OGRLayer.");
	...
	public OGRStyleTable getStyleTable() throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en getStyleTable(). El constructor no tuvo exito");

		long ptr_styletable = getStyleTableNat(cPtr);
		if (ptr_styletable == 0)
			throw new OGRException(
					"Error en getStyleTable(). No se ha podido obtener una referencia valida a un OGRStyleTable.");
	...
	public OGRLayer executeSQL(String pszStatement,
			OGRGeometry poSpatialFilter, String pszDialect) throws OGRException {

		if (cPtr == 0)
			throw new OGRException(
					"Error en executeSQL(). El constructor no tuvo exito");

		if (pszStatement == null || pszStatement.equals("")
				|| poSpatialFilter == null)
			throw new OGRException(
					"Error en executeSQL(). Parámetros en la función no validos.");

		if (poSpatialFilter.getPtro() == 0)
			throw new OGRException(
					"Error en executeSQL(). Referencia del parámetro OGRGeometry invalida.");

		long ptr_layer = executeSQLNat(cPtr, pszStatement,
				poSpatialFilter.getPtro(), pszDialect);
		if (ptr_layer == 0)
			throw new OGRException(
					"Error en executeSQL(). No se ha podido obtener una referencia valida a un OGRLayer.");
	...
	public void releaseResultSet(OGRLayer poResultsSet) throws OGRException {
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en poResultsSet(). El constructor no tuvo exito");

		if (poResultsSet == null)
			throw new OGRException(
					"Error en releaseResultSet(). Parámetro en la función no valido.");

		if (poResultsSet.getPtro() == 0)
			throw new OGRException(
					"Error en poResultsSet(). Referencia del parámetro OGRLayer invalida.");
	...
	public void syncToDisk() throws OGRException {// Excepciones
		...
		if (cPtr == 0)
			throw new OGRException(
					"Error en syncToDisk(). El constructor no tuvo exito");
