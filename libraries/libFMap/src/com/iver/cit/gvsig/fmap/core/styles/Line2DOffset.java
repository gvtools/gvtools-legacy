			...
			while (!pi.isDone()) {
				...
				case PathIterator.SEG_MOVETO:
					if (from == null) {
						...
						/*
						 * Puede significar un agujero en un polígono o un salto
						 * en una linea. Entonces, consumimos los segmentos que
						 * llevamos y empezamos un nuevo polígono o una nueva
						 * linea.
						 */
						try {
							if (p instanceof FPolygon2D) {
								offsetSegments.append(
										offsetAndConsumeClosedSegments(offset,
												segments), false);
							} else {
								offsetSegments.append(
										offsetAndConsumeSegments(offset,
												segments), false);
							}
				...
				case PathIterator.SEG_CLOSE:
					...
					try {
						offsetSegments
								.append(offsetAndConsumeClosedSegments(offset,
										segments), false);
					} catch (NotEnoughSegmentsToClosePathException e) {
						Logger.getLogger(Line2DOffset.class).error(
								e.getMessage(), e);
	...
	private static GeneralPathX offsetAndConsumeSegments(double offset,
		...
		for (int i = 0; i < segmentCount; i++) {
			...
			if (i < segmentCount - 1) {
				...
				try {
					pEnd = eq.resolve(eq1);
				} catch (ParallelLinesCannotBeResolvedException e) { // Las
																		// lineas
																		// son
																		// paralelas
																		// y NO
					...
					gpx.append(new Line2D.Double(pIni, pEnd), true); // añadimos
																		// una
																		// linea
																		// hasta
																		// el
																		...
																		// primer
																		// segmento
					// y asignamos como punto final el principio del siguiente
					// segmento
					// para que en la siguiente iteración lo tome como punto
					// inicial.
					pIni = pEnd;
	...
	private static GeneralPathX offsetAndConsumeClosedSegments(double offset,
		...
		if (segmentCount > 1) {
			...
			for (int i = 0; i < segmentCount; i++) {
				...
				LineEquation offsetLine = new LineEquation(theta, x0, y0, x1,
						y1);
				offsetLines.put(segment, offsetLine);
			}
			...
			for (int i = 0; i < segmentCount; i++) {
				LineSegment segment = segments.get(0);
				LineEquation eq = offsetLines.get(segment);
				if (i == 0) { // Calculo de la intersección entre el primer
								// segmento y el último
					LineEquation eq0 = offsetLines.get(segments
							.get(segmentCount - 1));
					try {
						pIni = eq0.resolve(eq);
					} catch (ParallelLinesCannotBeResolvedException e) { // Las
				...
				if (i < segmentCount - 1) {
					...
					try {
						...
						gpx.append(new Line2D.Double(pIni, pEnd), true); // añadimos
																			...
																			// primer
																			// segmento
						// y asignamos como punto final el principio del
						// siguiente segmento
						// para que en la siguiente iteración lo tome como punto
						// inicial.
						pIni = pEnd;
...
class LineEquation {
	...
	public LineEquation(double theta, double x, double y, double xEnd,
			double yEnd) {
		// this.theta = Math.tan(theta); //Esto es un error, no podemos
		// confundir el angulo de la recta con su pendiente
		this.theta = theta;
	...
	public Point2D resolve(LineEquation otherLine)
		...
		if (Math.abs(this.x - this.xEnd) < 0.00001) { // Esta linea es vertical
			X = this.xEnd;
			if (Math.abs(otherLine.x - otherLine.xEnd) < 0.00001) {// La otra
																	// linea es
																	// vertical
				if (Math.abs(this.x - otherLine.x) < 0.00001) { // Son la misma
																// linea,
																// devolvemos el
																// primer punto
																// de la otra
																// linea.
					Y = otherLine.y;
				} else { // No son la misma linea, entonces son paralelas,
							// excepcion.
					throw new ParallelLinesCannotBeResolvedException(this,
							otherLine);
				}
			} else if (Math.abs(otherLine.y - otherLine.yEnd) < 0.00001) { // La
																			// otra
																			// linea
																			// es
																			// horizontal
		...
		} else if (Math.abs(this.y - this.yEnd) < 0.00001) { // Esta linea es
																// horizontal
			Y = this.yEnd;
			if (Math.abs(otherLine.y - otherLine.yEnd) < 0.00001) { // La otra
																	// linea es
																	// horizontal
				if (Math.abs(this.y - otherLine.y) < 0.00001) { // Son la misma
																// linea,
																// devolvemos el
																// primer punto
																// de la otra
			...
			} else if (Math.abs(otherLine.x - otherLine.xEnd) < 0.00001) {// La
																			// otra
																			// linea
																			// es
																			// vertical
			...
			if (Math.abs(otherLine.y - otherLine.yEnd) < 0.00001) { // La otra
			...
			} else if ((Math.abs(otherLine.m - this.m) < 0.00001)) { // Tienen
																		// la
																		// misma
																		// pendiente
				...
				X = (mTimesX - this.y - otherLine.m * otherLine.x + otherLine.y)
						/ (this.m - otherLine.m);
...
// private static GeneralPathX offsetAndConsumeSegments(double offset,
// ArrayList<LineSegment> segments) {
// Hashtable<LineSegment, LineEquation> offsetLines = new Hashtable<LineSegment,
// LineEquation>();
// int segmentCount = segments.size();
// // first calculate offset lines with the starting point
// for (int i = 0; i < segmentCount; i++) {
...
// private static GeneralPathX offsetAndConsumeClosedSegments(double offset,
// ArrayList<LineSegment> segments) throws
// ParallelLinesCannotBeResolvedException, NotEnoughSegmentsToClosePathException
// {
// int segmentCount = segments.size();
// if (segmentCount > 1) {
// GeneralPathX openPath = offsetAndConsumeSegments(offset, segments);
...
// public LineEquation(double theta, double x, double y, double xEnd, double
// yEnd, double offset) {
// this.theta = theta;
// this.x = x;
// this.y = y;
...
// public Point2D resolve(LineEquation otherLine) throws
// ParallelLinesCannotBeResolvedException {
// double X;
// double Y;
...
// X = (thetaTimesX - this.y - otherTanTheta*otherLine.x + otherLine.y) /
// (tanTheta - otherTanTheta);
...
// public ParallelLinesCannotBeResolvedException(LineEquation eq1, LineEquation
// eq2) {
