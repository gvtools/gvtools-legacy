...
public class FLiteShape implements FShape, Cloneable {
	 ...
	 */
	public FLiteShape(Geometry geom, AffineTransform at,
			MathTransform mathTransform, boolean generalize, double maxDistance)
	 ...
	 */
	public FLiteShape(Geometry geom, AffineTransform at,
			MathTransform mathTransform, boolean generalize)
			throws TransformException, FactoryException {
		if (geom != null)
		...
		xScale = (float) Math
				.sqrt((affineTransform.getScaleX() * affineTransform
						.getScaleX())
						+ (affineTransform.getShearX() * affineTransform
								.getShearX()));
		yScale = (float) Math
				.sqrt((affineTransform.getScaleY() * affineTransform
						.getScaleY())
	...
	private void transformGeometry(Geometry geometry)
		...
		if (mathTransform == null || mathTransform.isIdentity()) {
			...
			MathTransformFactory factory = new DefaultMathTransformFactory();
			factory.createConcatenatedTransform(mathTransform, factory
		...
		if (geometry instanceof GeometryCollection) {
			...
			transformer.transform(((Point) geometry).getCoordinateSequence(),
					mathTransform);
	...
	public void reProject(MathTransform ct) {
		// TODO
		/*
		 * Point2D pt = new Point2D.Double(); for (int i = 0; i < numCoords;
		 * i+=2) { pt.setLocation(pointCoords[i], pointCoords[i+1]); pt =
		 * ct.convert(pt,null); pointCoords[i] = pt.getX(); pointCoords[i+1] =
		 * pt.getY(); }
		 */
