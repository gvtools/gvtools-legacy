...
public class VectorialEditableDBAdapter extends VectorialEditableAdapter
	...
	private class MyIterator implements IFeatureIterator {
		...
		public MyIterator(Rectangle2D r, String strEPSG)
		...
		public MyIterator(String[] fields, CoordinateReferenceSystem newCrs)
				throws ReadDriverException {
		...
		public boolean hasNext() throws ReadDriverException {
										...
										// iterador y salta un fallo
			{
				...
				if (bOriginalCursorOpened) {
					...
					// Aquí suponemos que el orden es el original. Si no, no
					// funcionará.
					if (delRows.get(originalIndex)) // Si está borrado
					{
						...
						while (featIt.hasNext()) {
							...
							// calculatedIndex =
							// getCalculatedIndex(originalIndex);
							if (delRows.get(originalIndex) == false) // Si NO
																		// está
																		// borrado
							{
								bFound = true;
						...
						if (bFound == false) // Todos los últimos están
												// borrados.
						{
							bOriginalCursorOpened = false; // para que busque en
															// el fichero de
															// expansión
							feat = null;
						}
					} // if delRows
					if (bOriginalCursorOpened) // Si todavía quedan features por
												// leer, y no están borradas
					{
						calculatedIndex = originalIndex; // getCalculatedIndex(originalIndex);
						Integer integer = new Integer(calculatedIndex);
						if (!relations.containsKey(integer)) { // Si no está en
																// el
																// fichero de
																// expansión
			...
			if (!bOriginalCursorOpened) {
				...
				// de expansión
				while ((idFromExpansion < expansionFile.getSize())
						&& (feat == null)) {
					IRowEdited rowEd = expansionFile.getRow(idFromExpansion);
					IFeature aux = (IFeature) rowEd.getLinkedRow();
					Integer calculated = (Integer) mapFID2index
							.get(aux.getID());
					calculatedIndex = calculated.intValue();
					System.out.println("El elemento idFromExpansion = "
							+ idFromExpansion + " es " + aux.getID());

					// Revisamos los borrados
					if (delRows.get(calculatedIndex) == true) {
						boolean bFound = false;
						while ((!bFound)
								&& (idFromExpansion < expansionFile.getSize() - 1)) {
							// calculatedIndex++;
							idFromExpansion++;
							rowEd = expansionFile.getRow(idFromExpansion);
							aux = (IFeature) rowEd.getLinkedRow();

							Integer auxCalculated = (Integer) mapFID2index
									.get(aux.getID());
							calculatedIndex = auxCalculated.intValue();
							// Si no está borrado y es una entidad válida, que
							// está siendo usada (no es algo que está en el
							// expansionFile sin usarse)
							if ((delRows.get(calculatedIndex) == false)
									&& (relations.containsKey(auxCalculated))) {
								bFound = true;
								calculated = auxCalculated;
								break;
							} else {
								System.out
										.println("El elemento idFromExpansion = "
												+ idFromExpansion
												+ " está borrado");
							}
						}
					...
					if (relations.containsKey(calculated)) {
						Integer realExpansionIndex = (Integer) relations
								.get(calculated);
			...
			if (calculatedIndex == -1)
				...
				if (feat == null) {
					if (idFromExpansion == expansionFile.getSize())
						...
						System.err
								.println("ERROR DE ENTREGA DE FEATURE EN hasNext del Iterador");
				}
				/*
				 * if (delRows.get(calculatedIndex)) feat = null;
				 */
				return true;
			}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#getFeatures(
	 * java.awt.geom.Rectangle2D, java.lang.String)
	 */
	public IRowEdited[] getFeatures(Rectangle2D r, String strEPSG)
			throws ReadDriverException, ExpansionFileReadException {
		ArrayList aux = new ArrayList();
		IFeatureIterator featIt = getFeatureIterator(r, strEPSG, getLyrDef()
				.getFieldNames());
		int numEntities = 0;
		while (featIt.hasNext()) {
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#startEdition()
	 */
	...
	public IFeatureIterator getFeatureIterator(Rectangle2D r, String strEPSG,
		...
		return new MyIterator(r, alphaNumericFieldsNeeded,
				ProjectionUtils.getCRS(strEPSG));
	}
	...
	public IFeatureIterator getFeatureIterator(String[] fields,
		...
		// return new DefaultFeatureIterator(this, projection, newProjection,
		// fields);
	}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#doAddRow(com
	 * .iver.cit.gvsig.fmap.core.IRow)
	 */
	public int doAddRow(IRow feat, int sourceType) throws ReadDriverException,
		...
		// Integer posInExpansionFile = (Integer) relations.get(new
		// Integer(calculatedIndex));
		Integer virtual = new Integer(calculatedIndex); // calculatedIndex es
														// igual al numero de
														// shapes originales +
														// el numero de
														// entidades añadidas.
		// es decir, virtual es el calculatedIndex (no tiene en cuenta los
		// borrados)
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#doModifyRow(int,
	...
	public int doModifyRow(int calculatedIndex, IRow feat, int sourceType)
		...
		int posAnteriorInExpansionFile = super.doModifyRow(calculatedIndex,
				feat, sourceType); // devolverá -1 si es original
		...
		// {
		// AssertionError err = new
		// AssertionError("Fallo al modificar la fila. ID viejo=" +
		// theIDoriginal + " ID nuevo = " + feat.getID());
		// err.printStackTrace();
		// }
		// hashFIDtoExpansionFile.put(feat.getID(), new
		// Integer(posInExpansionFile));
		mapFID2index.put(feat.getID(), virtual);
		// mapIndex2FID.put(virtual, feat.getID());
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#doRemoveRow(int)
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.edition.VectorialEditableAdapter#undoAddRow(int)
	 */
