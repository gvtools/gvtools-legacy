...
public class WKBParser2 {
	...
	protected IGeometry parseGeometry(ByteBuffer data) {
		...
		case WKBConstants.wkbPoint:
			result1 = ShapeFactory
					.createPoint2D(parsePoint(data, haveZ, haveM));
			break;
		case WKBConstants.wkbLineString:
			result1 = ShapeFactory.createGeometry(parseLineString(data, haveZ,
					haveM));
			break;
		case WKBConstants.wkbPolygon:
			result1 = ShapeFactory.createGeometry(parsePolygon(data, haveZ,
					haveM));
			break;
		...
		case WKBConstants.wkbMultiLineString:
			if (haveM) {
				...
				result1 = ShapeFactory
						.createGeometry(parseMultiLineString(data));
			}
			break;
	...
	private FPolygon2D parseLinearRing(ByteBuffer data, boolean haveZ,
			boolean haveM) {
		FPoint2D[] points = parsePointArray(data, haveZ, haveM);
		GeneralPathX gp = new GeneralPathX();
		gp.moveTo(points[0].getX(), points[0].getY());
	...
	private FPolygon2D parsePolygon(ByteBuffer data, boolean haveZ,
			boolean haveM) {
		int count = data.getInt();
	...
	private FPolyline2D parseMultiLineString(ByteBuffer data) {
		...
		return new FPolyline2D(gp);// strings[0]; //new
	...
	private GeneralPathX parseGeneralPath(ByteBuffer data) {
		...
		for (int i = 0; i < count; i++) {
			...
			/*
			 * FPoint2D[] result = new FPoint2D[count2]; for (int i = 0; i <
			 * count; i++) { result[i] = parsePoint(data, haveZ, haveM); }
			 * return result;
			 */
			/*
			 * FPoint2D p = parsePoint(data, gHaveZ, gHaveM);
			 * gp.moveTo(p.getX(), p.getY()); for (int j = 1; j < count2; j++) {
			 * p = parsePoint(data, gHaveZ, gHaveM); gp.lineTo(p.getX(),
			 * p.getY()); }
			 */
	...
	/**
	 * Devuelve el GeneralPathX compuesto por todos los GeneralPath de todas las
	 * geometrías que componen el elemento.
	 * 
	 * @param geometries
	 *            Lista de geometrías.
	 * 
