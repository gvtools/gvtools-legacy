...
/**
 * Clase con m�todos est�ticos para dibujar sobre el Graphics que se les
 * pase como par�metro.
 * 
 * Esta clase deber�a ser privada. Las clases que la usan son GraphicLayer y
 * AnnotationStrategy, pero hay que revisarlas para que no sea necesario. Lo
 * m�s urgente ser�a lo del dibujado de textos, para que sea Hay que quitar
 * las dependecias de FSymbol, y trabajar SIEMPRE con ISymbol. Recordar: Que sea
 * ISymbol el que renderiza. extensible el s�mbolo a usar. NOTA: Ver tambi�n
 * comentario en ISymbol
 * 
 * @author fjp
 */
public class FGraphicUtilities {
	...
	public static void DrawSymbol(Graphics2D g2, AffineTransform mT2,
		...
		// System.out.println("r = " + r.toString() + " Color preview:" +
		// symbol.m_Color.toString());
		...
		case FConstant.SYMBOL_TYPE_POINT:
			...
			if (symbol.getFont() != null) {
				...
				FGraphicUtilities.DrawLabel(g2, mT, shp, symbol, new FLabel(
						"Abcd"));
		...
		case FShape.MULTI:
			...
			// Punto:
			shp = new FPoint2D(r3.x + (r3.width / 2), r3.y + (r3.height / 2));
		...
		case FConstant.SYMBOL_TYPE_TEXT:
			...
			symbol.setFontSizeInPixels(true);
			FGraphicUtilities
					.DrawLabel(g2, mT, shp, symbol, new FLabel("Abcd"));
			symbol.setFontSizeInPixels(bAux);
			break;
		}
	}
	...
	/**
	 * Dibuja el shape que se pasa como par�metro con las caracter�sticas
	 * que aporta el s�mbolo sobre el Graphics2D.
	 * 
	 * @param g2
	 *            Graphics2D sobre el que dibujar.
	 * @param mT
	...
	public static void DrawShape(Graphics2D g2, AffineTransform mT, FShape shp,
		...
		/*
		 * if (shp.getShapeType()>=FShape.Z){ type=shp.getShapeType()-FShape.Z;
		 * }
		 */
		switch (type) {
	 ...
	 *            FShape a dibujar.
	 * @param theSymbol
	 *            S�mbolo para aplicar.
	 * @param theLabel
	 *            FLabel que contiene el texto que se debe dibujar.
	 */
	public static void DrawLabel(Graphics2D g2, AffineTransform mT, FShape shp,
		...
		/*
		 * if (m_labelValues[numReg].length() > 0) { TextLayout layout = new
		 * TextLayout(m_labelValues[numReg], font, frc); layout.draw(g2, x, y);
		 * }
		 */
		if (shp == null) {
			return;
		...
		// Las etiquetas que pongamos a nulo ser� porque no la queremos
		// dibujar.
		...
		case FShape.LINE:
			...
			//
			if (theLabel.getOrig() == null) // Calculamos el punto y la
											// orientaci�n solo la primera vez
			{
			...
			// theSymbol.m_Font.createGlyphVector(g2.getFontRenderContext(),
			// theLabel);
			// Shape txtShp = TextPathLayout.layoutGlyphVector(theGlyphs,
			// shp.m_Polyline,TextPathLayout.ALIGN_MIDDLE);
			// g2.draw(txtShp);
		...
		case FShape.POLYGON:
			...
			if (theLabel.getOrig() == null) // Calculamos el punto solo la
											// primera vez
			{
				...
				theLabel.setOrig(new Point2D.Double(((FPoint2D) pLabel).getX(),
		...
		case FLabel.CENTER_TOP:
			g2.drawString(theLabel.getString(), 0 - (width / 2), 0 - height);
		...
		case FLabel.RIGHT_CENTER:
			g2.drawString(theLabel.getString(), 0 - width, 0 - (height / 2));
	...
	public static void DrawAnnotation(Graphics2D g2, AffineTransform at,
		 ...
		 * break;
		 * 
		 * case FLabel.LEFT_CENTER: float height = metrics.getMaxAscent();
		 * g2.drawString(s, 0, 0 - (height / 2));
		 * 
		 * break;
		 * 
		 * case FLabel.LEFT_TOP: height = metrics.getMaxAscent();
		 * g2.drawString(s, 0, 0 - height);
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_BOTTOM: float width = metrics.stringWidth(s);
		 * g2.drawString(s, 0 - (width / 2), 0 - 3);
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_CENTER: height = metrics.getMaxAscent(); width =
		 * metrics.stringWidth(s); g2.drawString(s, 0 - (width / 2), 0 - (height
		 * / 2));
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_TOP: width = metrics.stringWidth(s); height =
		 * metrics.getMaxAscent(); g2.drawString(s, 0 - (width / 2), 0 -
		 * height);
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_BOTTOM: width = metrics.stringWidth(s);
		 * g2.drawString(s, 0 - width, 0 - 3);
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_CENTER: width = metrics.stringWidth(s); height =
		 * metrics.getMaxAscent(); g2.drawString(s, 0 - width, 0 - (height /
		 * 2));
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_TOP: width = metrics.stringWidth(s); height =
		 * metrics.getMaxAscent(); g2.drawString(s, 0 - width, 0 - height);
		 * 
		 * break; }
		 */
		// /Rectangle2D
		// borde=vp.fromMapRectangle(theLabel.getBoundBox());//theLabel.getBoundingBox();
	...
	private static void drawSymbolPoint(Graphics2D g2, AffineTransform mT,
		...
		case FConstant.SYMBOL_STYLE_MARKER_CIRCLE: // Circulito
			...
			if (theSymbol.getColor() != null) {
				g2.fillOval(rectAux.x, rectAux.y, rectAux.width, rectAux.height);
			}

			if (theSymbol.isOutlined()) {
				g2.setColor(theSymbol.getOutlineColor());
				g2.drawOval(rectAux.x, rectAux.y, rectAux.width, rectAux.height);
			}
		...
		case FConstant.SYMBOL_STYLE_FILL_SOLID:
			...
			if (theSymbol.isOutlined()) {
				g2.setColor(theSymbol.getOutlineColor());
				g2.drawRect(rectAux.x, rectAux.y, rectAux.width, rectAux.height);
			}
		...
		case FConstant.SYMBOL_STYLE_MARKER_TRIANGLE: // Triangulo
			...
			genPath.moveTo(x - (int) (radio_simbolo * 0.866), y
					+ (int) (radio_simbolo * 0.5));
			genPath.lineTo(x + (int) (radio_simbolo * 0.866), y
					+ (int) (radio_simbolo * 0.5));
		...
		case 34: // TrueType marker
			...
			/*
			 * lf.lfHeight = -radioSimbolo_en_pixels; angulo =
			 * pSimbolo->m_Rotation; // En radianes, de -pi a pi angulo = -180.0
			 * * angulo / PI;
			 * 
			 * lf.lfEscapement = (long) angulo*10; lf.lfOrientation = (long)
			 * angulo*10;
			 * 
			 * fuente.CreateFontIndirect(&lf); pOldFont =
			 * pDC->SelectObject(&fuente);
			 * 
			 * pDC->TextOut(pAPI.x, pAPI.y+radioSimbolo_en_pixels/2,elChar,1);
			 * 
			 * pDC->SelectObject(pOldFont); fuente.DeleteObject();
			 * 
			 * break;
			 */
		case FConstant.SYMBOL_STYLE_MARKER_IMAGEN: // Icono
		{
			if (theSymbol.getIcon() != null) {
				...
				if (!theSymbol.isSizeInPixels()) {
					...
					h = (theSymbol.getIcon().getHeight(null) * w)
							/ theSymbol.getIcon().getWidth(null);
				 ...
				 * g2.setColor(Color.WHITE); g2.fillRect(rectAux.x, rectAux.y,
				 * rectAux.width, rectAux.height); }
				 */
				g2.drawImage(theSymbol.getIcon(), rectAux.x, rectAux.y,
						rectAux.width, rectAux.height,
						theSymbol.getImgObserver());
			} else {
		...
		/*
		 * DrawIconEx(pDC->m_hDC, pAPI.x-(pSimbolo->m_widthIco/2),
		 * pAPI.y-(pSimbolo->m_heightIco/2), pSimbolo->m_hIcon,
		 * pSimbolo->m_widthIco, pSimbolo->m_heightIco, 0 , NULL, DI_NORMAL);
		 * break;
		 */
	...
	public static void DrawHandlers(Graphics2D g, AffineTransform at,
		...
		for (int i = 0; i < handlers.length; i++) {
			...
			g.drawRect((int) (point.getX() - 5), (int) (point.getY() - 5), 10,
					10);
			g.drawString("" + i, (int) (point.getX() - 5),
					(int) (point.getY() - 5));
		}
	}

	public static void DrawVertex(Graphics2D g, AffineTransform at,
			Handler handlers) {
		// for (int i = 0; i < handlers.length; i++) {
		Point2D point = handlers.getPoint();
		at.transform(point, point);
		g.setColor(Color.black);
		g.drawLine((int) point.getX() - 2, (int) point.getY() - 10,
				(int) point.getX() - 2, (int) point.getY() + 10);
		g.drawLine((int) point.getX() + 2, (int) point.getY() - 10,
				(int) point.getX() + 2, (int) point.getY() + 10);
		g.drawLine((int) point.getX() - 10, (int) point.getY() - 2,
				(int) point.getX() + 10, (int) point.getY() - 2);
		g.drawLine((int) point.getX() - 10, (int) point.getY() + 2,
				(int) point.getX() + 10, (int) point.getY() + 2);
		g.setColor(Color.red);
		g.drawLine((int) point.getX() - 1, (int) point.getY() - 10,
				(int) point.getX() - 1, (int) point.getY() + 10);
		g.drawLine((int) point.getX() + 1, (int) point.getY() - 10,
				(int) point.getX() + 1, (int) point.getY() + 10);
		g.drawLine((int) point.getX() - 10, (int) point.getY() - 1,
				(int) point.getX() + 10, (int) point.getY() - 1);
		g.drawLine((int) point.getX() - 10, (int) point.getY() + 1,
				(int) point.getX() + 10, (int) point.getY() + 1);
