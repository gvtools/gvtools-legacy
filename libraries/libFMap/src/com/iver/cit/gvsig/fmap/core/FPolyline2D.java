...
public class FPolyline2D implements FShape {
		...
		if (gp.intersects(r)) {
			...
			while (!theIterator.isDone()) {
				...
				if (theType == PathIterator.SEG_MOVETO) {
					...
					Line2D l = new Line2D.Double(pAnt.getX(), pAnt.getY(),
							theData[0], theData[1]);
					if (l.intersectsLine(l1.getX1(), l1.getY1(), l1.getX2(),
							l1.getY2())
							|| l.intersectsLine(l2.getX1(), l2.getY1(),
									l2.getX2(), l2.getY2())
							|| l.intersectsLine(l3.getX1(), l3.getY1(),
									l3.getX2(), l3.getY2())
							|| l.intersectsLine(l4.getX1(), l4.getY1(),
									l4.getX2(), l4.getY2())
							|| r.intersectsLine(l)) {
						bool = true;
					}
	...
	public Handler[] getSelectHandlers() {
		...
		while (!gpi.isDone()) {
			...
			// g.fillRect((int)(theData[0]-3),(int)(theData[1]-3),6,6);
			/*
			 * if (!(this instanceof FPolygon2D && isFirst)){ handlers.add(new
			 * PointSelHandler(i,theData[0], theData[1])); i++; } isFirst=false;
			 */
			switch (theType) {
