 ...
 * 
 */
public class AttrInTableLabelingStrategy implements ILabelingStrategy,
		CartographicSupport {
	...
	public void draw(BufferedImage image, Graphics2D g, ViewPort viewPort,
			Cancellable cancel, double _dpi) throws ReadDriverException {
		if (idTextField == -1)
			return;
		...
		if (zoom == null
				|| (zoom.isUserDefined() && (scale <= zoom.getMaxScale()) && (scale >= zoom
						.getMinScale()))) {
			try {
				...
				if ((idTextField == -1)
						|| (idTextField >= recordSet.getFieldCount())) {
					System.err
							.println("Ha habido un error. Se ha perdido el campo de etiquetado. Probablemente por quitar un join o edici�n externa.");
					return;
				}

				for (int i = bs.nextSetBit(0); i >= 0 && !cancel.isCanceled(); i = bs
						.nextSetBit(i + 1)) {
					...
					if (useFixedSize) {
						...
						try {
							size = ((NumericValue) vv[idHeightField])
									.doubleValue();// * fontScaleFactor;
							...
							Logger.getAnonymousLogger().warning(
									"Null text height value for text '"
					...
					size = CartographicSupportToolkit.getCartographicLength(
							this, size, viewPort, _dpi);
					...
					if (useFixedColor) {
						...
						try {
							color = new Color(
									((NumericValue) vv[idColorField])
											.intValue());
						} catch (ClassCastException ccEx) {
							if (!NullValue.class.equals(vv[idColorField]
									.getClass())) {
								throw new ReadDriverException("Unknown", ccEx);
							}
							// a null value
					...
					if (idRotationField != -1) {
						...
						if (!(vv[idRotationField] instanceof NullValue))
							rotation = -Math
									.toRadians(((NumericValue) vv[idRotationField])
											.doubleValue());
					}
				...
				Logger.getAnonymousLogger().log(Level.SEVERE,
						"Could not get the layer extent.\n" + e.getMessage());
			} catch (ExpansionFileReadException e) {
				Logger.getAnonymousLogger().log(
						Level.SEVERE,
						"Could not draw annotation in the layer" + "\""
								+ layer.getName()
	...
	public XMLEntity getXMLEntity() {
		...
		if (getColorFont() != null)
			xml.putProperty("Color",
					StringUtilities.color2String(getColorFont()));
	...
	public String getTextField() throws ReadDriverException {
		if (idTextField == -1)
			return null;
		try {
			String fieldName = ((SelectableDataSource) layer.getRecordset())
					.getFieldAlias(idTextField);
			return fieldName;
		} catch (ArrayIndexOutOfBoundsException e) {
			// Probablmente hemos quitado un join y la leyenda se basaba en un
	...
	public void setXMLEntity(XMLEntity xml) {
		...
		if (xml.contains("fontName")) {
			Font font = new Font(xml.getStringProperty("fontName"),
		...
		if (xml.contains("useFixedColor")) {
			...
			fixedColor = StringUtilities.string2Color(xml
					.getStringProperty("fixedColor"));
		}
	...
	public void setLayer(FLayer layer) {
		...
		try {
			...
			if (textFieldName != null) {
				...
				// textFieldName=textFieldName.substring(5,
