...
public class ArcJoin implements CustomQuery {
	...
	/**
	 * @see com.hardcode.gdbms.engine.customQuery.CustomQuery#evaluate(com.hardcode.gdbms.engine.data.DataSource[],
	 *      com.hardcode.gdbms.engine.instruction.Expression[])
	 */
	public OperationDataSource evaluate(DataSource[] tables, Expression[] values)
			throws QueryException {
		if (tables.length != 2)
			throw new QueryException("ArcJoin sólo opera con 2 tablas");
		if (values.length != 2)
			throw new QueryException("Se esperan dos expresiones de campo");
		...
		if (fieldName0 == null)
			throw new QueryException(
					"El valor debe ser una referencia a columna:"
							+ Utilities.getText(((Adapter) values[0])
									.getEntity()));
		String fieldName1 = values[1].getFieldName();
		if (fieldName1 == null)
			throw new QueryException(
					"El valor debe ser una referencia a columna:"
							+ Utilities.getText(((Adapter) values[1])
									.getEntity()));
		...
		try {
			...
			if (index0 == -1)
				throw new QueryException("No existe el campo: " + fieldName0);
			int index1 = tables[1].getFieldIndexByName(fieldName1);
			if (index1 == -1)
				throw new QueryException("No existe el campo: " + fieldName1);
			 ...
			 * idx.start(); for (int i = 0; i < tables[1].getRowCount(); i++) {
			...
			for (int i = 0; i < tables[0].getRowCount(); i++) {
				...
				if (pi == null) {
					...
					try {
						if (((BooleanValue) v.equals(tables[1].getFieldValue(
								pi.intValue(), index1))).getValue()) {
							result[i] = pi.intValue();
						}
					} catch (IncompatibleTypesException e1) {
						throw new QueryException(
								"Los tipos de datos son incompatibles: "
										+ tables[0].getFieldType(index0)
										+ " - "
										+ tables[1].getFieldType(index1), e1);
					}
				}
			}
			/*
			 * for (int i = 0; i < tables[0].getRowCount(); i++) { Value v =
			 * tables[0].getFieldValue(i, index0); PositionIterator pi =
			 * idx.getPositions(v); boolean any = false; while (pi.hasNext()){
			 * int pos = pi.next(); try { if
			 * (((BooleanValue)v.equals(tables[1].getFieldValue(pos,
			 * index1))).getValue()){ result[i] = pos; any = true; } } catch
			 * (IncompatibleTypesException e1) { throw new
			 * QueryException("Los tipos de datos son incompatibles: " +
			 * tables[0].getFieldType(index0) + " - " +
			 * tables[1].getFieldType(index1), e1); } } if (!any) result[i] =
			 * -1; }
			 */
			tables[0].stop();
			tables[1].stop();
