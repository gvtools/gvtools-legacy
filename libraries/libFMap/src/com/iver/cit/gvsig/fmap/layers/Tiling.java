...
/**
 * Cálculo de Partes (Tiles) en las que se divide un raster grande. Se usa para
 * imprimir rasters y capas raste remotas (WMS).
 * 
 * Para no pedir imagenes demasiado grandes, vamos a hacer lo mismo que hace
 * EcwFile: chunkear. Llamamos a drawView con cuadraditos más pequeños del
 * BufferedImage ni caso, cuando se imprime viene con null código original de
 * Fran Peñarrubia
 * 
...
public class Tiling {
	...
	public Tiling(int tileW, int tileH, Rectangle r) {
		...
		for (stepY = 0; stepY < numRows; stepY++) {
			...
			for (stepX = 0; stepX < numCols; stepX++) {
				...
				tile[tileCnt] = new Rectangle(xProv, yProv, anchoAux + 1,
						altoAux + 1);
	...
	/**
	 * Calcula el tamaño máximo de tile controlando que ningún tile tenga menos
	 * de MIN_SIZE pixeles
	 * 
	 * @param tileW
	 *            Ancho del tile
	 ...
	 *            Alto del tile
	 * @param r
	 *            Rectangulo que define el area de la imagen
	 */
	...
	private void calcViewPort(ViewPort viewPort)
		...
		viewPortList[0].setExtent(new Rectangle2D.Double(viewPort
				.getAdjustedExtent().getMinX(), viewPort.getAdjustedExtent()
				.getMaxY() - tileWcH, tileWcW, tileWcH));
			...
			for (int stepX = 0; stepX < numCols; stepX++) {
				...
				if (vpCnt > 0) {
					...
					if ((xt + wt) > viewPort.getAdjustedExtent().getMaxX())
						wt = Math.abs(viewPort.getAdjustedExtent().getMaxX()
								- xt);

					viewPortList[vpCnt] = viewPort.cloneViewPort();
					viewPortList[vpCnt].setImageSize(getTile(vpCnt).getSize());
					viewPortList[vpCnt].setExtent(new Rectangle2D.Double(xt,
							yt, wt, ht));
