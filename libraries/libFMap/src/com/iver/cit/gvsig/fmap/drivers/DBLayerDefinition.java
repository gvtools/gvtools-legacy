...
public class DBLayerDefinition extends LayerDefinition {
	...
	private String fieldGeometry; // (gid y the_geom), pero por si acaso se
									// cambian
	...
	public void setFieldID(String fieldID) {
		...
		/*
		 * boolean bFound = false; for (int i=0; i< fieldsDesc.length; i++) { if
		 * (fieldsDesc[i].getFieldName().compareToIgnoreCase(fieldID) == 0)
		 * bFound = true; } if (bFound != true) { int length =
		 * fieldsDesc.length; FieldDescription[] aux = new
		 * FieldDescription[length+1]; System.arraycopy(fieldsDesc, 0, aux, 0,
		 * length); FieldDescription f = new FieldDescription(); aux[length] =
		 * field }
		 */
	}
	 ...
	 *            Array to search on.
	 * @param fieldName
	 *            Name of the field we want to search.
	 ...
	 * FieldDescription array, it will try to create gid1, etc. As it will check
	 * if exists a field with that name in fieldsDesc, fieldsDesc must be filled
	...
	/**
	 * @deprecated Better use getCompoundTableName to deal with schemas. This
	 *             method should be used only by drivers when it is necessary to
	 *             distinguish between table and schema.
	 * @return
	 */
	...
	/**
	 * @return "schema"."tableName" or only "tableName" if schema is not
	 *         defined. schema and tableName are quoted
	 */
