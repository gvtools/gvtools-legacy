...
public class GeometryUtilities {
		...
		for (int i = 0; i < parts.size(); i++) {
			...
			if (isCCW(xs, ys)) {
				if (proj instanceof ProjectedCRS) {
					...
					area -= getCoordsArea(vp, xs, ys,
							new Point2D.Double(xs[xs.length - 1].doubleValue(),
									ys[ys.length - 1].doubleValue()));
				} else {
	...
	public static double getLength(ViewPort vp, IGeometry geom) {
		...
		for (int j = 0; j < parts.size(); j++) {
			...
			for (int i = 0; i < (xsys[0].length - 1); i++) {
				...
				Point2D p = new Point2D.Double(xsys[0][i].doubleValue(),
						xsys[1][i].doubleValue());
				Point2D p2 = new Point2D.Double(xsys[0][i + 1].doubleValue(),
						xsys[1][i + 1].doubleValue());
				dist = vp.distanceWorld(p, p2);
				distAll += dist;
			}
			int distanceUnits = vp.getDistanceUnits();
			perimeter += distAll
					/ MapContext.getDistanceTrans2Meter()[distanceUnits];
		}
		return perimeter;
	...
	/**
	 * <p>
	 * Returns an array list containing the parts that compose a multipolygon or
	 * multilyne. Each element of the array list is an array (Double[][])
	 * containing the X and Y coordinates for each polygon part.
	 * </p>
	 * 
	 * <p>
	 * I think it wll be clearer with an example:
	 * 
	 * <pre>
	 * ArrayList parts = GeometryUtilities.getParts(geom);
	...
	public static boolean isCCW(Double[] xs, Double[] ys) {
		...
		for (int i = 0; i < ys.length; i++) {
			Coordinate coord = new Coordinate(xs[i].doubleValue(),
					ys[i].doubleValue());
			coordList.add(coord);
		}
	...
	/**
	 * Returns the area of a polygon, whose coordinates are defined in a
	 * geographic (latitude/longitude) CRS.
	 * 
	 * @param xs
	 * @param ys
	 * @return
	...
	/**
	 * Calculates the area for a polygon whose coordinates are defined in a
	 * projected CRS.
	 * 
	 * @param aux
	 *            �ltimo punto.
	 ...
	 * @return �rea.
	 */
	private static double getCoordsArea(ViewPort vp, Double[] xs, Double[] ys,
			Point2D point) {
		Point2D aux = point;
		double elArea = 0.0;
		Point2D pPixel;
		...
		return (elArea / (Math.pow(
				MapContext.getAreaTrans2Meter()[vp.getDistanceArea()], 2)));
	}
	...
	public static IGeometry closeLine(IGeometry line) {
		...
		double[] newCoords = new double[6]; // Can receive as much 6 coordinates
											// (3 points)
		...
		// Only can close (convert to polygons) the lines which are composed by
		// more than 2 points
		if (numPoints < 3)
			return null;
