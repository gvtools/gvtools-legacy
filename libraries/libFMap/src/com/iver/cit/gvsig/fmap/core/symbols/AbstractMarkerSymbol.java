 ...
 */
public abstract class AbstractMarkerSymbol extends AbstractSymbol implements
		IMarkerSymbol {
	...
	public void drawInsideRectangle(Graphics2D g,
		...
		if (properties == null)
			draw(g, scaleInstance,
					new FPoint2D(r.getCenterX(), r.getCenterY()), null);
		else {
			double originalSize = getSize();
			double size = originalSize;
			PrintQuality pq = (PrintQuality) properties.get(PrintQuality.class);
			...
			setSize(size);
			print(g, scaleInstance,
					new FPoint2D(r.getCenterX(), r.getCenterY()), properties);
			setSize(originalSize);
		}
	}
