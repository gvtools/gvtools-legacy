...
public class FSpline2D extends FPolyline2D {
	...
	private static GeneralPathX getGeneralPathX(Point2D[] ps) {
		...
		if (ps[0].getX() == ps[ps.length - 1].getX()
				&& ps[0].getY() == ps[ps.length - 1].getY())
			gpx.closePath();
		return gpx;
	}
	...
	static class Spline {
		...
		/**
		 * The constructor calculates the second derivatives of the
		 * interpolating function at the tabulated points xi, with xi = (i,
		 * y[i]). Based on numerical recipes in C,
		...
		public Spline(double y[]) {
			...
			for (int i = 1; i < n - 1; i++) {
				y2[i] = -1.0 / (4.0 + y2[i - 1]);
				u[i] = (6.0 * (y[i + 1] - 2.0 * y[i] + y[i - 1]) - u[i - 1])
						/ (4.0 + y2[i - 1]);
			}
			for (int i = n - 2; i >= 0; i--) {
