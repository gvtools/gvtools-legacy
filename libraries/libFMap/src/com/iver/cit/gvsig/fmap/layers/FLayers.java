...
public class FLayers extends FLyrDefault implements VectorialData,
	...
	protected Map orderedLayers = Collections
	...
	// *
	// * @param fmap reference to the <code>MapContext</code> instance that
	// contains this node of layers
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#setAllVisibles
	 * (boolean)
	 */
	...
	/**
	 * Adds a layer on an specified position in this node. If there is a
	 * previous layer that has been not added the layer is saved and will bi
	 * added where the previous layer will be added.
	 * 
	 * @param pos
	 *            position in the inner list where the layer will be added
	 * @param layer
	 *            a layer
	 */
			...
			if (orderedLayers.size() > 0) {
				...
				while (it.hasNext()) {
					...
					if (position == layers.size()) {
						FLayer orderedLayer = (FLayer) orderedLayers
								.get(position);
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#addLayer
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#moveTo
	 * (int, int)
	 */
	public void moveTo(int from, int to) throws CancelationException {
		int newfrom = layers.size() - from - 1;
		int newto = layers.size() - to - 1;
		if (newfrom < 0 || newfrom >= layers.size() || newto < 0
				|| newto >= layers.size())
			return;
		FLayer aux = (FLayer) layers.get(newfrom);
		callLayerMoving(LayerPositionEvent.createLayerMovingEvent(aux, newfrom,
				newto));
		layers.remove(newfrom);
		layers.add(newto, aux);
		this.updateDrawVersion();
		callLayerMoved(LayerPositionEvent.createLayerMovedEvent(aux, newfrom,
				newto));
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#removeLayer
	 * (com.iver.cit.gvsig.fmap.layers.FLayer)
	 */
	...
	public void replaceLayer(String layerName, FLayer layer)
		...
		for (int i = 0; i < layers.size(); i++) {
			...
			if (lyr.getName().compareToIgnoreCase(layerName) == 0) {
				...
				callLayerAdding(LayerCollectionEvent
						.createLayerAddingEvent(layer));

				layers.add(i, layer);
				layer.setParentLayer(this);

				callLayerAdded(LayerCollectionEvent
						.createLayerAddedEvent(layer));
				break;
			}
		}
	}
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#getVisibles
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#getLayer
	 * (int)
	 */
	 ...
	 * 
	 * <p>
	 * In <code>result</code> always will be at least one layer.
	 * </p>
	 * 
	 * @param layer
	 *            the layer we are looking for
	 * @param result
	 *            an array list that will have the results of the search
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#getLayersCount
	 * ()
	 */
	 ...
	 * shared object <code>cancel</code>.
	 * </p>
	 * 
	 * <p>
	 ...
	 *            Java(tm) platform
	 * @param viewPort
	 *            the information for drawing the layers
	 * @param cancel
	 ...
	 *         drawn or were <code>null</code>; otherwise, the
	 *         <code>group</code>
	 * 
	 ...
	 */
	public ComposedLayer print_old(Graphics2D g, ViewPort viewPort,
			Cancellable cancel, double scale,
			PrintRequestAttributeSet properties, ComposedLayer group)
			throws ReadDriverException {
		double dpi = 72;
		...
		// TODO: We have to check when we have to call the drawLabels method
		// when exists a ComposedLayer group.
		for (int i = 0; i < layers.size(); i++) {
			...
			try {
				...
				if (group != null) {
					if (lyr instanceof FLayers) {
						group = ((FLayers) lyr).print_old(g, viewPort, cancel,
								scale, properties, group);
					} else {
						...
						if (lyr instanceof ILabelable
								...
								&& ((ILabelable) lyr).getLabelingStrategy()
										.shouldDrawLabels(scale)) {
							group.add(lyr);
						} else {
							...
							if (group == null) {
								if (lyr instanceof FLayers) {
									group = ((FLayers) lyr).print_old(g,
											viewPort, cancel, scale,
											properties, group);
								} else {
									...
									if (lyr instanceof ILabelable
											&& ((ILabelable) lyr).isLabeled()
											&& ((ILabelable) lyr)
													.getLabelingStrategy() != null
											&& ((ILabelable) lyr)
													.getLabelingStrategy()
													.shouldDrawLabels(scale)) {
										ILabelable lLayer = (ILabelable) lyr;
										lLayer.drawLabels(null, g, viewPort,
												cancel, scale, dpi);
									}
								}
							} else {
					...
					if (group == null) {
						if (lyr instanceof FLayers) {
							group = ((FLayers) lyr).print_old(g, viewPort,
									cancel, scale, properties, group);
						} else {
							...
							if (lyr instanceof ILabelable
								...
								lLayer.drawLabels(null, g, viewPort, cancel,
										scale, dpi);
							}
						}
					} else {
				...
				String mesg = Messages.getString("error_printing_layer") + " "
						+ lyr.getName() + ": " + e.getMessage();
	...
	public Rectangle2D getFullExtent() {
		...
		for (Iterator iter = layers.iterator(); iter.hasNext();) {
			...
			try {
				...
				e.printStackTrace();// TODO hay que revisar para determinar el
									// comportamiento que espera el usuario.
			}
		}
	 ...
	 * 
	 * @param e
	 *            a layer collection event with the new layer
	 */
	 ...
	 * 
	 * @param event
	 *            a layer collection event with the layer being removed
	 * 
	 * @throws CancelationException
	 *             any exception produced during the cancellation of the driver.
	 */
	 ...
	 * 
	 * @param e
	 *            a layer collection event with the layer removed
	 */
	...
	/**
	 * <p>
	 * Returns an entity that represents this collection of layers stored as a
	 * tree-node with children that are also layers.
	 * </p>
	 * 
	 * <p>
	 * The root node has the same properties that
	 * <code>FlyrDefault#getXMLEntity()</code> returns, and adds:
	 * <ul>
	 * <li><i>numLayers</i> : number of layers of this collection (direct
	 * children of this node)
	 * <li><i>LayerNames</i> : an array list with the name of the layers of this
	 * collection (direct children of this node)
	 * <code>FLayer.getXMLEntity()</code>
	 * </ul>
	 * </p>
	 * 
	 * <p>
	 * All XML elements returned represent the information about this layer.
	 * </p>
	 * 
	 * @return an XML entity with information to this collection of layers
	 * @throws com.iver.cit.gvsig.fmap.layers.XMLException
	 *             if there is any error creating the XML from the layers.
	 */
	 ...
	 * 
	 * <p>
	 * This root node has the same properties that return
	 * <code>FlyrDefault#getXMLEntity()</code> adding:
	 * <ul>
	 * <li><i>numLayers</i> : number of first-level layers of this collection
	 * <li><i>LayerNames</i> : an array list with the name of the first-level
	 * layers of this collection <code>FLayer.getXMLEntity()</code>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.VectorialData#process(
	 * com.iver.cit.gvsig.fmap.operations.strategies.FeatureVisitor,
	 * com.iver.cit.gvsig.fmap.layers.FBitSet)
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#setAllActives
	 * (boolean)
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.layerOperations.LayerCollection#getActives
	 * ()
	 */
	 ...
	 * 
	 * @see
	 * com.iver.cit.gvsig.fmap.layers.FLyrDefault#addLayerListener(com.iver.
	 * cit.gvsig.fmap.layers.LayerListener)
	 */
	...
	public ImageIcon getTocImageIcon() {
		return new ImageIcon(
				MapControl.class.getResource("images/layerGroup.png"));
	}
	 ...
	 * there hasn't been errors.
	 * </p>
	 * 
	 * @see CopyOfFLayers#addLayerFromXML(XMLEntity, String)
	 * 
	 * @param xml
	 *            tree-node structure with information about layers
	 * @param name
	 ...
	 * <p>
	 * Adds the layer with the information in an XML entity and the specified
	 * name, to this collection of layers.
	 * </p>
	 * 
	 * <p>
	 * This method really executes the addition, considering the kind of layer (
	 * <code>FLyrVect</code>, <code>FLyrAnnotation</code>,
	 * <code>FLyrRaster</code>, a collection of layers (<code>FLayers</code>),
	 * or another kind of layer (<code>FLayer</code>)), and the driver in the
	 * layer.
	 * </p>
	 * 
	 * @param xml
	...
	private void addLayerFromXML(XMLEntity xml, String name)
		...
		throw new RuntimeException(
				"Create a schema, generate Java sources and rewrite this method");
		...
		// //TODO VCN FLyrAnnotation es un parche para no tener que duplicar
		// todo el c�digo de aq� y de los diferentes m�todos de
		// LayerFactory,
		// //ya que los drivers de una FLyrAnnotation no sabemos cual es puede
		// ser cualquier Driver Vectorial.
		...
		// try {
		// // d =
		// LayerFactory.getDM().getDriver(xml.getStringProperty("driverName"));
		// } catch (DriverLoadException e1) {
		// throw new DriverLayerException(name,e1);
		// }
		// String
		// path=pathGenerator.getAbsolutePath((String)xml.getStringProperty("file"));
		// if (path!=null){
		// // layer = LayerFactory.createLayer(name,
		// // new
		// File(pathGenerator.getAbsolutePath((String)xml.getStringProperty("file"))),
		// // crs);
		...
		// try {
		// // driver = (IVectorialDatabaseDriver)
		// LayerFactory.getDM().getDriver(driverName);
		// //Hay que separar la carga de los datos del XMLEntity del load.
		// if (xml.firstIndexOfChild("className",
		// "com.iver.cit.gvsig.fmap.layers.SelectionSupport")==2)
		...
		// layer.setAvailable((((DefaultJDBCDriver)driver).getConnection() !=
		// null));
		...
		// {
		// String className2 =
		// xml.getChild(classChild).getStringProperty("className");
		// try {
	...
	/**
	 * <p>
	 * Sets the <code>MapContext</code> that contains this layer node.
	 * </p>
	 * 
	 * @param mapContext
	 *            the <code>MapContext</code> that contains this layer node
	 */
	...
	public void beginDraw(Graphics2D g, ViewPort viewPort) {
		LayerDrawEvent beforeEvent = new LayerDrawEvent(this, g, viewPort,
				LayerDrawEvent.LAYER_BEFORE_DRAW);
		fmap.fireLayerDrawingEvent(beforeEvent);
	}

	public void endDraw(Graphics2D g, ViewPort viewPort) {
		LayerDrawEvent afterEvent = new LayerDrawEvent(this, g, viewPort,
				LayerDrawEvent.LAYER_AFTER_DRAW);
