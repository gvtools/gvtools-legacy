...
public class UtilFunctions {
																			...
																			// g)
	{
		...
		/*
		 * g.setColor(Color.RED); ((Graphics2D) g).draw(resul);
		 * g.setColor(Color.BLUE); ((Graphics2D) g).draw(rBounds);
		 ...
		 * g.drawString("C", (int) xC, (int) yC); g.draw3DRect((int)xC, (int)
		 * yC, 2, 2, true);
		 */
	...
	public static Point2D[] getPerpendicular(Point2D p1, Point2D p2,
			Point2D perp) {
		if ((p2.getY() - p1.getY()) == 0) {
			return new Point2D[] { new Point2D.Double(perp.getX(), 0),
	...
	public static Point2D getUnitVector(Point2D p1, Point2D p2) {
		Point2D paux = new Point2D.Double(p2.getX() - p1.getX(), p2.getY()
				- p1.getY());
	 ...
	 * 
	 * @param p1
	 *            primer punto del círculo cuyo centro se quiere obtener
	 * @param p2
	 *            segundo punto del círculo cuyo centro se quiere obtener
	 * @param p3
	...
	public static Point2D getCenter(Point2D p1, Point2D p2, Point2D p3) {
		...
		Point2D[] perp1 = getPerpendicular(
				p1,
				p2,
				new Point2D.Double((p1.getX() + p2.getX()) / 2, (p1.getY() + p2
						.getY()) / 2));
		Point2D[] perp2 = getPerpendicular(
				p2,
				p3,
				new Point2D.Double((p2.getX() + p3.getX()) / 2, (p2.getY() + p3
						.getY()) / 2));
	 ...
	 * 
	 * @param angle1
	 *            angulo en radianes. Debe ser positivo y no dar ninguna vuelta
	 *            a la circunferencia
	 * @param angle2
	 *            angulo en radianes. Debe ser positivo y no dar ninguna vuelta
	 *            a la circunferencia
	...
	/**
	 * Devuelve el punto de la recta que viene dada por los puntos p1 y p2 a una
	 * distancia radio de p1.
	 * 
	 * @param p1
	 *            DOCUMENT ME!
	 * @param p2
	 ...
	 * el arco porque los puntos están alineados o los 3 puntos no son distintos
	 * 
	 * @param p1
	 * @param p2
	 * @param p3
	...
	/**
	 * Obtiene un arco a partir del centro del arco y punto inicio y punto final
	 ...
	 * @return Arco
	 */
	public static Arc2D createArc2points(Point2D center, Point2D init,
			Point2D end) {
	...
	/**
	 * Devuelve el punto a una distancia radio del punto p1 y aplicandole un
	 * ángulo an. una distancia radio de p1.
	 * 
	 * @param p1
	 *            DOCUMENT ME!
	 * @param p2
	...
	public static boolean isLowAngle(Point2D antp, Point2D lastp,
		...
		try {
			 ...
			 * System.out.println("angleDiff 2 1= "
			 * +angleDistance(angle2,angle1));
			 * System.out.println("angleDiff 1 2= "
			 * +angleDistance(angle1,angle2)); if
			 * (angleDistance(angle2,angle1)>Math.PI){
			 * 
			 * if (RobustCGAlgorithms.isCCW(coords)) {
			 * System.out.println("izquierda,arriba,true"); return true; } else{
			 * System.out.println("izquierda,arriba,false"); } }else { if
			 * (!RobustCGAlgorithms.isCCW(coords)) {
			 * System.out.println("izquierda,abajo,true"); return true; } else{
			 * System.out.println("izquierda,abajo,false"); } } }else if
			 * (lastp.getX()>antp.getX()){
			 */
	...
	public static void rotateGeom(IGeometry geometry, double radAngle,
	...
	public static void scaleGeom(IGeometry geometry, Point2D basePoint,
			double sx, double sy) {
		AffineTransform at = new AffineTransform();
