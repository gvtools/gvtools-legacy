...
/**
 * S�mbolo utilizado para guardar las caracter�sticas que se deben de
 * aplicar a los Shapes a dibujar.
 * 
...
public class FSymbol implements ISymbol, ISLDCompatible {
	...
	/**
	 * Si <code>m_bUseFontSize</code> viene a false entonces m_FontSize viene en
	 ...
	 * Es �til cuando est�s etiquetando y no quieres que se dibuje el
	 * s�mbolo que te sirve de base para etiquetar.
	 */
	...
	public String m_LinePattern = "0"; // Solo para poder mostrarlo cuando vamos
										// a seleccionar un s�mbolo
	...
	private boolean m_bUseSize; // Si est� a true, m_Size viene en coordenadas
								// de mundo real.
	 ...
	 * 
	 * @param tipoSymbol
	 *            Tipo de S�mbolo. case FConstant.SYMBOL_TYPE_POINT: case
	 *            FConstant.SYMBOL_TYPE_POINTZ: case
	 *            FConstant.SYMBOL_TYPE_MULTIPOINT: m_bUseSize = true; // Esto
	 *            es lo primero que hay que hacer siempre
	 * 
	 *            // para evitar un StackOverflow m_useOutline = false;
	 *            setStyle(FConstant.SYMBOL_STYLE_MARKER_SQUARE); setSize(5);
	 *            //pixels
	 * 
	 *            break;
	 * 
	 *            case FConstant.SYMBOL_TYPE_LINE: case
	 *            FConstant.SYMBOL_TYPE_POLYLINEZ: case
	 *            FConstant.SYMBOL_TYPE_POLYGONZ: setStroke(new BasicStroke());
	 *            setStyle(FConstant.SYMBOL_STYLE_LINE_SOLID);
	 * 
	 *            break;
	 * 
	 *            case FConstant.SYMBOL_TYPE_FILL: setStroke(new BasicStroke());
	 *            setStyle(FConstant.SYMBOL_STYLE_FILL_SOLID);
	 * 
	 *            break; case FShape.MULTI: m_bUseSize = true; setStroke(new
	 *            BasicStroke()); setStyle(FConstant.SYMBOL_STYLE_FILL_SOLID);
	 * 
	 *            // setStyle(FConstant.SYMBOL_STYLE_MARKER_SQUARE); setSize(5);
	 *            //pixels break;
	 * 
	 *            case FConstant.SYMBOL_TYPE_TEXT: setStroke(new BasicStroke());
	 *            setStyle(FConstant.SYMBOL_STYLE_TEXT_NORMAL); setFont(new
	 *            Font("Dialog",Font.PLAIN,12)); break;
	 */
	...
	public static FSymbol getSymbolForSelection(FSymbol sym) {
		...
		// 050215, jmorell: Si en los drivers cambiamos el estilo, aqu�
		// tenemos que
	...
	/**
	 * Se usa para el s�mbolo de selecci�n. Es una forma r�pida de clonar
	 * un s�mbolo, sin hacerlo via XML. Vicente, no lo borres!!!
	 * 
	 * @return
	 */
	...
	private void createSymbol(int tipoSymbol, Color c) {
		...
		// MULTISIMBOLO
		// Tipo de simbolo
		m_symbolType = tipoSymbol; // Para no recalcular el pixel, no usamos los
									// set
	 ...
	 * 
	 * @param xml
	 *            xml que contiene la informaci�n para crear el s�mbolo.
	 * 
	...
	public static FSymbol createFromXML03(XMLEntity xml) {
		...
		// System.out.println("createFromXML: m_Style=" +
		// xml.getIntProperty("m_Style"));

		symbol.setOutlined(xml.getBooleanProperty("m_useOutline"));

		if (xml.contains("m_Color")) {
			symbol.setColor(StringUtilities.string2Color(xml
					.getStringProperty("m_Color")));
		}

		if (xml.contains("m_outlineColor")) {
			symbol.setOutlineColor(StringUtilities.string2Color(xml
					.getStringProperty("m_outlineColor")));
		}

		if (xml.contains("m_FontColor")) {
			symbol.setFont(new Font(xml.getStringProperty("fontname"), xml
					.getIntProperty("fontstyle"), (int) xml
					.getFloatProperty("m_FontSize")));
			symbol.setFontColor(StringUtilities.string2Color(xml
					.getStringProperty("m_FontColor")));
		...
		if (xml.contains("m_Fill")) {
			// TODO: Si es un Fill de tipo imagen, deber�amos recuperar la
			// imagen.
			String strFill = xml.getStringProperty("m_Fill");
			if (strFill.compareTo("WithFill") == 0)
				symbol.setFill(FSymbolFactory.createPatternFill(
						symbol.getStyle(), symbol.getColor()));
			else
		...
		if (xml.contains("m_BackColor")) {
			symbol.m_BackColor = StringUtilities.string2Color(xml
					.getStringProperty("m_BackColor"));
		}

		if (xml.contains("m_BackFill")) {
			symbol.m_BackFill = StringUtilities.string2Color(xml
					.getStringProperty("m_BackFill"));
		}
	...
	public static FSymbol createFromXML(XMLEntity xml) {
		...
		if (symbol.m_LinePattern.compareTo("0") == 0) {
			...
			symbol.setStroke(new BasicStroke(lineWidth, BasicStroke.CAP_ROUND,
					BasicStroke.JOIN_BEVEL, 1.0f, toArray(symbol.m_LinePattern,
							lineWidth), 0));
		}
	 ...
	 * 
	 * @param m_outlineColor
	 *            The m_outlineColor to set.
	 */
	 ...
	 * 
	 * @param m_bUseFontSize
	 *            The m_bUseFontSize to set.
	 */
	...
	public String toSLD() {
		...
		try {
			...
			case FConstant.SYMBOL_TYPE_POINT:
				...
				if (this.m_Color != null) {
					...
					xmlBuilder.writeTag(SLDTags.CSSPARAMETER,
							...
							SLDTags.NAME_ATTR, SLDTags.FILL_ATTR);
					xmlBuilder.closeTag();
				}
				xmlBuilder.closeTag();
				...
				// TODO: Ver como a�adir un texto a cada feature de una capa
				// de puntos...
			...
			case FConstant.SYMBOL_TYPE_LINE:
				...
				// Add geometry tag with the column that gives the geometric
				// infrmation
				...
				if (this.m_Color != null) {
					...
					if (getStroke() != null) {
						xmlBuilder.writeTag(SLDTags.CSSPARAMETER, Float
								.toString(((BasicStroke) getStroke())
										.getLineWidth()), SLDTags.NAME,
								SLDTags.STROKE_WIDTH_ATTR);
					}
			...
			case FConstant.SYMBOL_TYPE_FILL:
				...
				if (this.m_outlineColor != null) {
					xmlBuilder.openTag(SLDTags.STROKE);
					xmlBuilder.writeTag(SLDTags.CSSPARAMETER, SLDUtils
							.convertColorToHexString(this.m_outlineColor),
							SLDTags.NAME_ATTR, SLDTags.STROKE_ATTR);
					if (getStroke() != null) {
						xmlBuilder.writeTag(SLDTags.CSSPARAMETER, Float
								.toString(((BasicStroke) getStroke())
										.getLineWidth()), SLDTags.NAME,
			...
			case FShape.MULTI:
				if (this.getFont() != null) {
					...
					// FGraphicUtilities.DrawLabel(g2, mT, shp, symbol,new
					// FLabel("Abcd"));
	...
	public void drawInsideRectangle(Graphics2D g2,
	...
	public void print(Graphics2D g, AffineTransform at, FShape shape,
		...
		if (stroke != null && stroke.getLineWidth() != 0) {
			...
			this.setStroke(new BasicStroke((int) d, BasicStroke.CAP_ROUND,
