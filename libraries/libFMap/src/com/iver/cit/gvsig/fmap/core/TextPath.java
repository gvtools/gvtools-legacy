...
/**
 * <p>
 * Class that represents baseline of a string and allows the baseline to be
 * composed as contiguous segments with distinct slope each.<br>
 * </p>
 * 
 * <p>
 * Once a TextPath is created for a string it is possible to know where the
 * character at a determined position in the string is placed and rotated.<br>
 * </p>
 * 
...
public class TextPath {
	...
	private char[] text;
	/**
	 * An array which contains the calculated positions for the glyphs Each row
	 * represents a glyph, and it contains the X coord, the Y coord, and the
	 * rotation angle
	 */
	...
	private char[] wordSeparators = { ' ' }; // in the future, separators might
												// be provided as parameter

	/**
	 * <p>
	 * Creates a new instance of TextPath with the current graphics context.<br>
	 * </p>
	 * 
	 * <p>
	 * Given a <b>Graphics2D</b>, TextPath can know which Font and
	 * FontRenderContext is in use. So, it can calculate the position and
	 * rotation of each character in <b>char[] text</b> based in the path
	 * defined by the <b>FShape path</b> argument.
	 * </p>
	 * 
	 * @param g
	 *            , Graphics2D
	...
	public TextPath(Graphics2D g, FShape path, char[] text, Font font,
		...
		/*
		 * java 6 code TODO keep this!! if (kerning) { HashMap<TextAttribute,
		 * Object> attrs = new HashMap<TextAttribute, Object>();
	...
	protected FShape softenShape(FShape shape, GlyphVector gv) {
		...
		float interval = (float) gv.getVisualBounds().getWidth()
				/ (gv.getNumGlyphs() * 3);
		...
		for (float curPos = 0; curPos < pl.lengthOfPath(); curPos = curPos
				+ interval) {
			...
			if (!bufferComplete) {
				...
				if (nextPos == controlPoints) {
					...
					for (int i = 1; i < controlPoints / 2 - 1; i++) {
						// calculate the points from the origin of the geometry
						// to the first interpolated point
				...
				if (!movedTo) {
					correctedPath.moveTo(xsum / controlPoints, ysum
							/ controlPoints);
					movedTo = true;
				} else {
					correctedPath.lineTo(xsum / controlPoints, ysum
							/ controlPoints);
				}
		...
		if (bufferComplete) {
			/**
			 * calculate the points from the last interpolated point to the end
			 * of the geometry
			 */
			...
			for (int i = 0; i < controlPoints / 2 - 1; i++) {
				// calculate the points from the last interpolated point to the
				// end of the geometry
	...
	private double[][] computeAtRight(GlyphVector gv, PathLength pl, char[] text) {
		...
		for (int i = numGlyphs - 1; i >= 0; i--) {
			if (correction && charDistance >= 0) {
				...
				if (i < numGlyphs - 1) {
					// correct distance according to angle between current and
					// previous glyph
					int turn = Angle.getTurn(previousAngle, angle);
					if (turn == 1) { // if turn is positive => increase distance
						float auxDistance = charDistance
								- (float) (charWidth * 2.5f
										* Angle.diff(previousAngle, angle) / Math.PI);
						float auxAngle = pl.angleAtLength(auxDistance);
						if (Angle.getTurn(previousAngle, auxAngle) == 1) { // ensure
																			// new
																			// position
																			// also
																			// has
					...
					} else if (turn == -1) { // if turn is negative => decrease
												// distance
						float auxDistance = charDistance
								+ (float) (charWidth * 0.9f
										* Angle.diff(previousAngle, angle) / Math.PI);
						float auxAngle = pl.angleAtLength(auxDistance);
						if (Angle.getTurn(previousAngle, auxAngle) == -1) { // ensure
																			// new
			...
			if (wordSpacing != DEFAULT_WORD_SPACING
					&& isWordSeparator(
							text[gv.getGlyphCharIndex(glyphsConsumed)],
							wordSeparators)) {
				charWidth = wordSpacing;
			} else {
				charWidth = Math.max(gv.getGlyphMetrics(glyphsConsumed)
						.getAdvance(), characterWidth);
		...
		/**
		 * Calculate 2D positions for the glyphs from the calculated linear
		 * distances
		 */
		for (int i = numGlyphs - 1; i >= 0; i--) {
			float anchor = (rightToLeft) ? charAnchors[charAnchors.length - 1
					- i] : charAnchors[i];
			Point2D p = pl.pointAtLength(anchor);
			if (p == null) {
				if (i < numGlyphs - 1) { // place in a straight line the glyphs
											// that don't fit in the shape
					pos[i][0] = pos[i + 1][0]
							+ (charAnchors[i] - charAnchors[i + 1])
							* Math.cos(pos[i + 1][2]);
					pos[i][1] = pos[i + 1][1]
							+ (charAnchors[i] - charAnchors[i + 1])
							* Math.sin(pos[i + 1][2]);
	...
	private double[][] computeAtLeft(GlyphVector gv, PathLength pl, char[] text) {
		...
		for (int i = 0; i < gv.getNumGlyphs(); i++) {
			...
			if (correction && charDistance <= lengthOfPath) {
				...
				if (i > 0) {
					...
					} else if (turn == -1) { // if turn is negative => increase
												// distance
		...
		for (int i = 0; i < charAnchors.length; i++) {
			...
			Point2D p = pl.pointAtLength(anchor);
			if (p == null) {
				if (i > 0) { // place in a straight line the glyphs that don't
								// fit in the shape
					pos[i][0] = pos[i - 1][0]
							+ (charAnchors[i] - charAnchors[i - 1])
							* Math.cos(pos[i - 1][2]);
	 ...
	 * @param path
	 */
	private void computeAtMiddle(FontRenderContext frc, char[] text, Font font,
			PathLength pl) {
		if (text.length == 0) {
	 ...
	 * 
	 * <p>
	 * <b>double[0]</b> Position in X in the screen
	 * </p>
	 * <p>
	 * <b>double[1]</b> Position in Y in the screen
	 * </p>
	 * <p>
	 * <b>double[2]</b> Angle of the character.
	 * </p>
	 * 
	 * @return
	 */
