...
public class DbaseFileWriterNIO {
	 ...
	 */
	public DbaseFileWriterNIO(DbaseFileHeaderNIO header, FileChannel out)
			throws IOException {
	...
	public void write(Object[] record) throws IOException {
		/*
		 * if (record.length != header.getNumFields()) { throw new
		 * DbaseFileException("Wrong number of fields " + record.length +
	...
	private String fieldString(Object obj, final int col) {
		...
		case 'c':
			o = formatter.getFieldString(fieldLen,
					(obj == null || obj instanceof NullValue) ? NULL_STRING
							: ((StringValue) obj).getValue());
			break;
		case 'L':
		case 'l':
			o = (obj == null || obj instanceof NullValue) ? "F"
		...
		case 'G':
			...
			break;
		/*
		 * case 'N': case 'n': // int? if (header.getFieldDecimalCount(col) ==
		 * 0) {
		 * 
		 * o = formatter.getFieldString( fieldLen, 0, (Number) (obj == null ?
		 * NULL_NUMBER : Double.valueOf(obj.toString())) ); break;
		 * 
		 * }
		 */
		case 'N':
		...
		case 'f':
			...
			o = formatter.getFieldString(fieldLen,
					header.getFieldDecimalCount(col), number);
			break;
		case 'D':
		case 'd':
			if (obj == null || obj instanceof NullValue)
	...
	public void setCharset(Charset charset) {
		if (!Charset.isSupported(charset.name())) {
			Logger.getLogger(this.getClass()).error(
					"Charset:  " + charset.name() + "  not supported!");
		} else {
