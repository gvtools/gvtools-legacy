...
public class AnnotationStrategy extends DefaultStrategy {
	private static Logger logger = Logger.getLogger(AnnotationStrategy.class
			.getName());
		...
		try {
			...
			if (lyrAnnotation.getISpatialIndex() != null) {
				if (isSpatialIndexNecessary(elExtent)) {
					lstIndexes = lyrAnnotation.getISpatialIndex().query(
							elExtent);
					sc = lstIndexes.size();
			...
			for (int numReg = 0; numReg < sc; numReg++) {
				 ...
				 * numOriginal
				 * =((EditableAdapter)lyrAnnotation.getSource()).getCalculatedIndex
				 * (numOriginal);
				 */
				...
				Rectangle2D r = null;
				if (inPixels
						&& lyrAnnotation.getMapping().getColumnHeight() == -1) {
					r = getDefaultBoundBoxinPixels(metrics, theLabel.getOrig(),
							theLabel.getString());
				} else {
					r = getBoundBox(theLabel.getOrig(),
							(float) theLabel.getHeight(),
							theLabel.getJustification(), theLabel.getString());
				}
				theLabel.setBoundBox(r);

				if (XRectangle2D.intersectInclusive(elExtent, r)) {
					FPoint2D p = new FPoint2D(
							viewPort.fromMapPoint(new Point2D.Double(r.getX(),
									r.getY())));
					...
					if (lyrAnnotation.isSpatialCacheEnabled()) {
						if (cache.getMaxFeatures() >= cache.size()) {
							...
							IGeometry geo = ShapeFactory.createPoint2D(
									r.getX(), r.getY());
							Rectangle2D re = new Rectangle2D.Double(r.getX(),
									r.getY(), 1, 1);
							cache.insert(re, geo);
						}
					}
	 ...
	 * @return
	 */
	public Rectangle2D getBoundBox(Point2D p, float hp, int justification,
			String s) {
		// Rectangle2D bounding=null;
		if (((FLyrAnnotation) capa).isInPixels()) {
			graphics.setFont(graphics.getFont().deriveFont(hp));
		} else {
			float alturaPixels = (float) ((hp * viewPort.getAffineTransform()
					.getScaleX()) * FLabel.SQUARE);
		...
		/*
		 * switch (justification) { case FLabel.LEFT_BOTTOM:
		 * bounding=justification(p, width,height, 0, 0);
		 * 
		 * break;
		 * 
		 * case FLabel.LEFT_CENTER: bounding=justification(p, width,height, 0,
		 * -(height / 2));
		 * 
		 * break;
		 * 
		 * case FLabel.LEFT_TOP: bounding=justification(p,width,height, 0,
		 * -height);
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_BOTTOM: bounding=justification(p, width,height,
		 * -(width / 2), -dist);
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_CENTER: bounding=justification(p, width,height,
		 * -(width / 2), -(height / 2));
		 * 
		 * break;
		 * 
		 * case FLabel.CENTER_TOP: bounding=justification(p, width,height,
		 * -(width / 2), -height);
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_BOTTOM: bounding=justification(p, width,height,
		 * -width, -dist);
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_CENTER: bounding=justification(p, width,height,
		 * -width, -(height / 2));
		 * 
		 * break;
		 * 
		 * case FLabel.RIGHT_TOP: bounding=justification(p, width,height,
		 * -width, -height);
		 * 
		 * break; }
		 * 
	...
	public FBitSet queryByShape(IGeometry g, int relationship)
		...
		for (int i = 0; i < lstRecs.size(); i++) {
			...
			IGeometry geom = getGeometry(((FLyrAnnotation) capa)
					.getLabel(index).getBoundBox());
			if (trans != null) {
	...
	public FBitSet queryByRect(Rectangle2D rect) throws ReadDriverException,
		...
		for (int i = 0; i < lstRecs.size(); i++) {
			...
			// System.out.println("Rectángulo de la geometría = "+
			// geom.getBounds2D());
