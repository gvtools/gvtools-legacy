...
public class AnnotationEditableAdapter extends VectorialEditableAdapter {
	...
	public int doAddRow(IRow feat, int sourceType) throws ReadDriverException,
		...
		// label.setBoundBox(r);
		AnnotationStrategy strategy = (AnnotationStrategy) lyrAnnotation
				.getStrategy();
		Rectangle2D r = strategy.getBoundBox(label.getOrig(),
				(float) label.getHeight(), label.getJustification(),
				label.getString());
		label.setBoundBox(r);
	...
	public int doModifyRow(int calculatedIndex, IRow feat, int sourceType)
		...
		if (!relations.containsKey(integer)) {
			...
			} // Aquí hay que comprobar, si se quiere, el resto de columnas, si
				// son iguales a la de Texto.
			...
			label.setBoundBox(new Rectangle2D.Double(p.getX(), p.getY(),
					rLabelAnt.getWidth(), rLabelAnt.getHeight()));
			...
			FLabel label = (FLabel) labelExpansion.getLabel(
					((Integer) relations.get(new Integer(calculatedIndex)))
							.intValue()).clone();
			Value value = feat.getAttribute(mapping.getColumnText());
			Rectangle2D rLabelAnt = (Rectangle2D) label.getBoundBox().clone();
			label.setString(value.toString());
			...
			label.setBoundBox(new Rectangle2D.Double(p.getX(), p.getY()
					+ rLabelAnt.getHeight(), rLabelAnt.getWidth(), rLabelAnt
					.getHeight()));
			Rectangle2D rLabel = label.getBoundBox();
			label.setOrig(p);
			fmapSpatialIndex.delete(rLabelAnt, new Integer(calculatedIndex));
	...
	public int undoModifyRow(int calculatedIndex,
		...
		try {
			if (previousExpansionFileIndex == -1) {
				...
				boolean cancel = fireBeforeModifyRow(dfAnt, calculatedIndex,
						sourceType);
				if (cancel)
					return -1;
				...
				this.fmapSpatialIndex
						.insert(rAnt, new Integer(calculatedIndex));
