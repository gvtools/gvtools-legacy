 ...
 * <p>
 * The <code>WKTReader</code> allows extracting <code>Geometry</code> objects
 * from either input streams or internal strings. This allows it to function as
 * a parser to read <code>Geometry</code> objects from text blocks embedded in
 ...
 * that <code>MultiPoints</code> are represented by
 * <code>MULTIPOINT ( ( x y), (x y) )</code> , but the examples show
 * <code>MultiPoint</code>s as <code>MULTIPOINT ( x y, x y )</code> . Other
...
public class WKTParser {
	 ...
	 *         empty array if "EMPTY" is the next element returned by the
	 *         stream.
	 * @throws IOException
	...
	private double getNextNumber(StreamTokenizer tokenizer) throws IOException,
		...
		case StreamTokenizer.TT_EOL:
			throw new ParseException(
					"Expected number but encountered end of line");
	 ...
	 *             if an I/O error occurs
	 */
	private String getNextCloser(StreamTokenizer tokenizer) throws IOException,
			ParseException {
		String nextWord = getNextWord(tokenizer);
		if (nextWord.equals(")")) {
			return nextWord;
	...
	private String getNextWord(StreamTokenizer tokenizer) throws IOException,
		...
		case StreamTokenizer.TT_EOL:
			throw new ParseException(
					"Expected word but encountered end of line");
		case StreamTokenizer.TT_NUMBER:
	 ...
	 *             if an I/O error occurs
	 */
	private IGeometry readGeometryTaggedText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
		...
		/*
		 * else if (type.equals("GEOMETRYCOLLECTION")) { return
		 * readGeometryCollectionText(tokenizer); }
		 */
	 ...
	 *             if an unexpected token was encountered
	 */
	private FGeometry readPointText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals("EMPTY")) {
			return null;
	 ...
	 *             if an unexpected token was encountered
	 */
	private FGeometry readLineStringText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
	 ...
	 *             if an unexpected token was encountered
	 */
	private IGeometry readMultiPointText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
	 ...
	 *             if an I/O error occurs
	 */
	private FGeometry readPolygonText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals("EMPTY")) {
			return null;
		...
		return shell; // geometryFactory.createPolygon(shell, (LinearRing[])
						// holes.toArray(array));
	}
	 ...
	 *             if an unexpected token was encountered
	 */
	private FGeometry readMultiLineStringText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
		...
		return lineString; // geometryFactory.createMultiLineString((LineString[])
							// lineStrings.toArray(array));
	}
	 ...
	 */
	// TODO:
	private IGeometry readMultiPolygonText(StreamTokenizer tokenizer)
			throws IOException, ParseException {
		String nextToken = getNextEmptyOrOpener(tokenizer);
		if (nextToken.equals("EMPTY")) {
			return null;
		...
		/*
		 * polygons.add(polygon); nextToken = getNextCloserOrComma(tokenizer);
		 * while (nextToken.equals(",")) { polygon = readPolygonText(tokenizer);
		 * polygons.add(polygon); nextToken = getNextCloserOrComma(tokenizer); }
		 */
		// Polygon[] array = new Polygon[polygons.size()];
		return polygon; // geometryFactory.createMultiPolygon((Polygon[])
	...
	// TODO:
	/*
	 * private GeometryCollection readGeometryCollectionText(StreamTokenizer
	 * tokenizer) throws IOException, ParseException { String nextToken =
	 * getNextEmptyOrOpener(tokenizer); if (nextToken.equals("EMPTY")) { return
	 * geometryFactory.createGeometryCollection(new Geometry[]{}); } ArrayList
	 * geometries = new ArrayList(); Geometry geometry =
	 * readGeometryTaggedText(tokenizer); geometries.add(geometry); nextToken =
	 * getNextCloserOrComma(tokenizer); while (nextToken.equals(",")) { geometry
	 * = readGeometryTaggedText(tokenizer); geometries.add(geometry); nextToken
	 * = getNextCloserOrComma(tokenizer); } Geometry[] array = new
	 * Geometry[geometries.size()]; return
