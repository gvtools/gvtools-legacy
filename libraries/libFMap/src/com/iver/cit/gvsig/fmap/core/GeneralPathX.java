...
public class GeneralPathX implements Shape, Cloneable, Serializable {
	...
	public boolean isClosed() {
		PathIterator theIterator = getPathIterator(null, FConverter.FLATNESS); // polyLine.getPathIterator(null,
																				// flatness);
	 ...
	 * not multi-threaded safe, which means that this <code>GeneralPathX</code>
	 * class does not guarantee that modifications to the geometry of this
	 * <code>GeneralPathX</code> object do not affect any iterations of that
	 ...
	 * <code>GeneralPathX</code> class does not guarantee that modifications to
	 * the geometry of this <code>GeneralPathX</code> object do not affect any
	...
	/**
	 * Use this function to ensure you get real polygons or holes En JTS, con
	 * bCCW = false obtienes un polígono exterior. Nota: Solo se le da la vuelta
	 * (si es que lo necesita) al polígono exterior. El resto, por ahora, no se
	 * tocan. Si se necesita tenerlos en cuenta, habría que mirar si están
	 * dentro del otro, y entonces revisar que tiene un CCW contrario al
	 * exterior.
	 * 
	 * @param bCCW
	 *            true if you want the GeneralPath in CCW order
	 * @return true si se le ha dado la vuelta. (true if flipped) TODO: TERMINAR
	 *         ESTO!! NO ESTÁ COMPLETO!! NO sirve para multipoligonos
	 */
	public boolean ensureOrientation(boolean bCCW) {
		...
		for (i = 0; i < numTypes; i++) {
			...
			switch (type) {
			case SEG_MOVETO:
				c1 = new Coordinate(pointCoords[pointIdx],
						pointCoords[pointIdx + 1]);
				cInicio = c1;
				coordList.add(c1, true);
				if (i > 0)
					bFirstList = false;
			...
			case SEG_QUADTO:
				c1 = new Coordinate(pointCoords[pointIdx],
						pointCoords[pointIdx + 1]);
				coordList.add(c1, true);
				c2 = new Coordinate(pointCoords[pointIdx + 2],
						pointCoords[pointIdx + 3]);
			...
			case SEG_CUBICTO:
				...
				c2 = new Coordinate(pointCoords[pointIdx + 2],
						pointCoords[pointIdx + 3]);
				coordList.add(c2, true);
				c3 = new Coordinate(pointCoords[pointIdx + 4],
						pointCoords[pointIdx + 5]);
