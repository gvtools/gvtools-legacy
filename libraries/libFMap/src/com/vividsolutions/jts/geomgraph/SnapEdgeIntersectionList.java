...
public class SnapEdgeIntersectionList extends EdgeIntersectionList {
	...
	public class SnapEdgeIntersectNode extends Node implements Comparable {
		...
		public SnapEdgeIntersectNode(Coordinate arg0, EdgeEndStar arg1,
				EdgeIntersection edgeIntersection) {
			super(arg0, arg1);
		...
		// Esto me vale para Nodos a secas, pero para el caso de intersecciones
		// no....
		// porque tienen mas informacion (segmento asociado, etc.)
		// Por ejemplo, primer y ultimo punto de un poligono cerrado no deben
		// ser el
		...
		public boolean equals(Object obj) {
			...
			return (other.coord.distance(this.coord) <= snapTolerance)
					...
					&& (Math.abs(other.edgeIntersection.dist
							- this.edgeIntersection.dist) <= snapTolerance);
		}
	...
	/*
	 * El codigo de verificacion de snap está por todas partes. Llevar a una
	 * clase auxiliar si procede
	 */
	...
	public SnapEdgeIntersectionList(SnappingEdge edge) {
		...
		nodeMap = new SnappingNodeMap(new NodeFactory(), snapTolerance) {
			public Node addNode(Node n) {
				...
				SnapEdgeIntersectNode eNode = (SnapEdgeIntersectNode) super.nodeMap
						.get(newNode);
				if (eNode != null) {
			...
			/*
			 * Por un lado quiero la rapidez del HashMap, y la posibilidad de
			 * resolver conflictos a partir de equals...
			 * 
			 * Pero cuando me pidan las intersecciones ordenadas, es necesario
			 * hacerlo según el sentido de los vertices (de mas cercana a mas
			 * lejana al vertice 0)
			 */
	...
	public EdgeIntersection add(Coordinate intPt, int segmentIndex, double dist) {
		...
		SnapEdgeIntersectNode solution = (SnapEdgeIntersectNode) nodeMap
				.addNode(newNode);
	...
	Edge createSplitEdge(EdgeIntersection ei0, EdgeIntersection ei1) {
		...
		boolean useIntPt1 = ei1.dist > snapTolerance
				|| !(ei1.coord.distance(lastSegStartPt) <= snapTolerance);// Con
																			// esto
																			// se
																			// dejaria
																			// de
		...
		if (useIntPt1)
			pts[ipt] = ei1.coord;
		return new SnappingEdge(pts, new Label(this.edge.getLabel()),
				snapTolerance);
	}
