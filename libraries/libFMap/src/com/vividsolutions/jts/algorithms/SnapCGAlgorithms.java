...
public class SnapCGAlgorithms extends CGAlgorithms {
	...
	public static boolean isPointInRing(Coordinate p, Coordinate[] ring,
		...
		SnapLineIntersector lineIntersector = new SnapLineIntersector(
				snapTolerance);
		...
		for (int i = 1; i < ring.length; i++) {
			...
			// if (((p1.y > (p.y - snapTolerance)) && (p2.y <= (p.y +
			// snapTolerance))) ||
			// ((p2.y > (p.y - snapTolerance)) && (p1.y <= (p.y) +
			// snapTolerance))) {//si no se cumple, no pueden intersectar
			//
			// double x1 = p1.x - p.x;
			// double y1 = p1.y - p.y;
			// double x2 = p2.x - p.x;
	...
	public static boolean snapEquals2D(Coordinate a, Coordinate b,
			double snapTolerance) {
