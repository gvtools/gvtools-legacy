...
/**
 * Integration test to test that Fill symbols always draw in the same place
...
public class TestIFillSymbol extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		for (int i = 0; i < drawFillSymbols.length; i++) {
			drawFillSymbols[i] = (IDrawFillSymbol) ((Class) getClassesToTest()
					.get(i)).newInstance();
		}
		...
		this.symbols = (IFillSymbol[]) symbols.toArray(new IFillSymbol[symbols
				.size()]);
	...
	public final void testNullColorAvoidsFilling() {
		...
		for (int i = 0; i < symbols.length; i++) {
			...
			for (int s = 0; s < sizes.length; s++) {
				...
				Point2D firstPoint = new Point2D.Float((float) (centerP.getX()
						- sizes[s] / 2 - 3), (float) (centerP.getY() - sizes[s]
						/ 2 - 3));
				Point2D lastPoint = new Point2D.Float((float) (centerP.getX()
						+ sizes[s] / 2 + 3), (float) (centerP.getY() + sizes[s]
						/ 2 + 3));
				...
				for (int j = 0; j < drawFillSymbols.length; j++) {
					if (drawFillSymbols[j].isSuitableFor(newSymbol)) {
						newSymbol = drawFillSymbols[j]
								.makeSymbolTransparent(newSymbol);
						break;
					}
				}

				newSymbol.draw(g, new AffineTransform(), new FPolygon2D(gpx),
						null);

				try {

					File dstDir = new File(System.getProperty("java.io.tmpdir")
							+ "/prova-imatges/");
					if (!dstDir.exists())
						dstDir.mkdir();
					ImageIO.write(bi, "png", new File(dstDir.getAbsoluteFile()
							+ File.separator + name + "_size_" + sizes[s]
							+ ".png"));
				} catch (IOException e) {
					e.printStackTrace();
					fail();
				...
				for (int j = 0; j < bi.getWidth(); j++) {
					for (int k = 0; k < bi.getHeight(); k++) {
						if (isInsideShape(new FPolygon2D(gpx), j, k)) {
							assertEquals(
									"Failed, the fill is being painted when the color is null '"
