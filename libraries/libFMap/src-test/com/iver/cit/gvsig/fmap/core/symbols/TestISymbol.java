...
public class TestISymbol extends TestCase {
	...
	public static TestSuite suite() {
		...
		TestSuite suite = new TestSuite(
				"Integration test for com.iver.cit.gvsig.fmap.core.ISymbol");
	...
	public static void addSymbolTest(AbstractSymbolTestCase symbolTestClass) {
		if (classesToTest == null)
			classesToTest = new ArrayList<AbstractSymbolTestCase>();
	...
	public void testPointSuitability() {
		final IGeometry dummyPointGeom = ShapeFactory
				.createPoint2D(new FPoint2D());
		for (int i = 0; i < symbols.length; i++) {
			// text symbols are suitable for everything
			if (symbols[i] instanceof ITextSymbol) {
				assertTrue(getNameForSymbol(symbols[i])
						+ " should be suitable for " + dummyPointGeom,
			...
			if (symbols[i] instanceof IMarkerSymbol) {
				...
				assertFalse(getNameForSymbol(symbols[i])
						+ " should NOT be suitable for " + dummyPointGeom,
						symbols[i].isSuitableFor(dummyPointGeom));
			}
		}
	}
	...
	public void testLineSuitability() {
		final IGeometry dummyLineGeom = ShapeFactory
				.createPolyline2D(new GeneralPathX());
		for (int i = 0; i < symbols.length; i++) {
			// text symbols are suitable for everything
			if (symbols[i] instanceof ITextSymbol) {
				assertTrue(getNameForSymbol(symbols[i])
						+ " should be suitable for " + dummyLineGeom,
			...
			if (symbols[i] instanceof ILineSymbol) {
				...
				assertFalse(getNameForSymbol(symbols[i])
						+ " should NOT be suitable for " + dummyLineGeom,
						symbols[i].isSuitableFor(dummyLineGeom));
			}
		}
	}
	...
	public void testPolygonSuitability() {
		final IGeometry dummyPolygonGeom = ShapeFactory
		...
		for (int i = 0; i < symbols.length; i++) {
			...
			if (symbols[i] instanceof ITextSymbol) {
				assertTrue(getNameForSymbol(symbols[i])
						+ " should be suitable for " + dummyPolygonGeom,
			...
			if (symbols[i] instanceof IFillSymbol) {
				...
				assertFalse(getNameForSymbol(symbols[i])
						+ " should NOT be suitable for " + dummyPolygonGeom,
						symbols[i].isSuitableFor(dummyPolygonGeom));
			}
		}
	}
	...
	public void testGeneralSuitability() {
		...
		for (int i = 0; i < symbols.length; i++) {
			// text symbols are suitable for everything
			if (symbols[i] instanceof ITextSymbol) {
				assertTrue(symbols[i].getClassName()
						+ " should be suitable for " + null,
	...
	/**
	 * ensures that any symbol that is suitable for markers declares its type as
	 * FShape.POINT or FShape.MULTI
	 * 
	 */
	public void testMarkerSymbolTypeDefinition() {
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i] instanceof IMarkerSymbol) {
				assertTrue("Symbol no. " + i + " '"
						+ getNameForSymbol(symbols[i]) + "'",
	...
	/**
	 * ensures that any symbol that is suitable for lines declares its type as
	 * FShape.LINE or FShape.MULTI
	 * 
	 */
	public void testLineSymbolTypeDefinition() {
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i] instanceof ILineSymbol) {
				assertTrue(
						"Symbol no. " + i + " '" + getNameForSymbol(symbols[i])
								+ "'",
	...
	/**
	 * ensures that any symbol that is suitable for fills declares its type as
	 * POLYGON or FShape.MULTI
	 * 
	 */
	...
	/**
	 * ensures that any symbol has a description in its persistence (very
	 * important)
	 */
	public void testDescription() {
		for (int i = 0; i < symbols.length; i++) {
			assertTrue(getNameForSymbol(symbols[i])
					+ " does not declare a description in its XMLEntity",
	...
	public void testIsShapeVisible() {
		for (int i = 0; i < symbols.length; i++) {
			assertTrue(
					getNameForSymbol(symbols[i])
							+ " does not declare the isShapeVisible field in its XMLEntity",
	...
	public void testSymbolSelfDefinition() {
		for (int i = 0; i < symbols.length; i++) {
			...
			final ISymbol cloneSymbol = SymbologyFactory.createSymbolFromXML(
					theSymbol.getXMLEntity(), null);
			assertTrue(theSymbol.getClassName()
		...
		for (int i = 0; i < symbols.length; i++) {
			if (symbols[i] instanceof IMarkerSymbol) {
				...
				assertTrue(
						"Symbol no. "
								...
								+ " '"
								+ getNameForSymbol(symbols[i])
								+ "' RGB value mismatch for getOnePointRBG() and getColor().getRGB()",
						symbols[i].getOnePointRgb() == marker.getColor()
								.getRGB());
			...
			if (symbols[i] instanceof IFillSymbol) {
				...
				if (!outlined)
					...
					fill.getOutline().setLineColor(
							new Color(random.nextInt() >> 8));
				assertTrue(
						"Symbol no. "
								+ i
								+ " '"
								+ getNameForSymbol(symbols[i])
								+ "' RGB value mismatch for getOnePointRBG() and getColor().getRGB()",
						symbols[i].getOnePointRgb() == ((outlined) ? fill
								.getOutline().getColor().getRGB() : fill
								.getFillColor().getRGB()));
			}
			...
			if (symbols[i] instanceof ITextSymbol) {
				...
				assertTrue(
						"Symbol no. "
								...
								+ getNameForSymbol(symbols[i])
								+ "' RGB value mismatch for getOnePointRBG() and getColor().getRGB()",
						symbols[i].getOnePointRgb() == text.getTextColor()
								.getRGB());
			}
	...
	public void testSymbolForSelection() {
		for (int i = 0; i < symbols.length; i++) {
			assertNotNull("Symbol no. " + i + " '"
					+ getNameForSymbol(symbols[i])
					+ " does not define any derived symbol for selection",
					symbols[i].getSymbolForSelection());
		}
	}
