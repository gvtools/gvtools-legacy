...
/**
 * Integration test to test that Marker symbols always draw in the same place
...
public class TestIMarkerSymbol extends TestCase {
	...
	private static final double sizes[] = new double[] { 200, 100, 50, 30, 16,
			5, 3, 2,
		...
		for (int i = 0; i < symbols.length; i++) {
			for (int j = 0; j < sizes.length; j++) {
				...
				Rectangle wrappingRect = new Rectangle(
						...
						(int) (sizes[j]), (int) (sizes[j]));

				assertFalse("fails sizing marker, too big (" + name + ", "
						+ sizes[j] + "px)",
						isOutsideRect(bi, wrappingRect, OUTTER_TOLERANCE));
				assertTrue("fails sizing marker, too small (" + name + ", "
						+ sizes[j] + "px) \n"
						+ "\t - forgot to enable ANTIALIASING?",
						fitsInsideRect(bi, wrappingRect, INNER_TOLERANCE));
			}
		}
	}
	...
	public void testDrawingOffset() {
		...
		for (int i = 0; i < symbols.length; i++) {
			for (int j = 0; j < sizes.length; j++) {
				...
				if (xOffset - sizes[j] < 0)
					xOffset = sizes[j];
				if (yOffset - sizes[j] < 0)
					yOffset = sizes[j];

				// ensure the symbol will completely fit inside the image
				if (xOffset + sizes[j] > sz.width)
					xOffset = sz.width - sizes[j];
				if (yOffset + sizes[j] > sz.height)
					yOffset = sz.height - sizes[j];
	 ...
	 * the value for Rotation attribute at persistence (getXMLEntity and
	 * setXMLEntity)
	 */
	public void testRotationDeclaration() {
		for (int i = 0; i < symbols.length; i++) {
			...
			assertTrue(
					TestISymbol.getNameForSymbol(symbols[i])
							+ " does not declare \"rotation\" attribute in its XMLEntity",
					xml.contains("rotation"));

			assertTrue(
					TestISymbol.getNameForSymbol(symbols[i])
							+ "'s getRotation()'s value does not match the value declared in its XMLEntity",
					xml.getDoubleProperty("rotation") == symbols[i]
							.getRotation());
	...
	private boolean fitsInsideRect(BufferedImage bi, Rectangle wrappingRect,
			int tolerance) {
		int myTolerance = Math.min(tolerance, wrappingRect.width);
		Rectangle myRect;
		if (myTolerance >= INNER_TOLERANCE) {
			myRect = new Rectangle(wrappingRect.x + myTolerance, wrappingRect.y
					+ myTolerance, wrappingRect.width - myTolerance
					- myTolerance, wrappingRect.height - myTolerance
					- myTolerance);
		} else {
			myRect = new Rectangle(-1, -1, 0, 0); // just ensure it does not
													// disturb the check
		}

		for (int i = 0; i < bi.getWidth(); i++) {
			for (int j = 0; j < bi.getHeight(); j++) {
				if (wrappingRect.contains(i, j) && !myRect.contains(i, j)) {
	...
	private boolean isOutsideRect(BufferedImage bi, Rectangle wrappingRect,
			int tolerance) {
		Rectangle myRect = new Rectangle(wrappingRect.x - tolerance,
				wrappingRect.y - tolerance, wrappingRect.width + tolerance
						+ tolerance, wrappingRect.height + tolerance
						+ tolerance);
		for (int i = 0; i < bi.getWidth(); i++) {
			for (int j = 0; j < bi.getHeight(); j++) {
...
// / paste this piece of code to produce some png files to check visually what
// is happening
