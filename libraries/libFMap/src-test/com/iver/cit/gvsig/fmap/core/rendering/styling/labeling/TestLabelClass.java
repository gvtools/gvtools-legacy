...
public class TestLabelClass extends TestCase {
	/*
	 * private ITextSymbol textSymbol; private String labelExpression; private
	 * boolean isVisible = true; private String[] texts; private int priority;
	 * private double scale = 1; private String sqlQuery;
	 */

	private static final String[] names = new String[] { "aName", "otherName",
			"30949Mock", };

	private static final Dimension[] sizes = new Dimension[] {
			new Dimension(200, 150),
			new Dimension(100, 100),
			new Dimension(300, 100), // he detectat que esta falla en svg, pero
										// crec que es per batik
			new Dimension(300, 180), new Dimension(300, 600),
			new Dimension(100, 200), new Dimension(10, 10), };
	...
	/**
	 * Tests if LabelClass always return a text symbol, since it is who actually
	 * draws the texts.
	 */
	public void testTextSymbol() {
		LabelClass lc = new LabelClass();
		assertNotNull(
				"LabelClass.getTextSymbol() returns null when doing only new LabelClass()",
				lc.getTextSymbol());

		lc.setTextSymbol(null);
		assertNotNull(
				"LabelClass.getTextSymbol() returns null after calling "
						+ "LabelClass.setTextSymbol(null). The label class must provide always "
						+ "a ITextSymbol.", lc.getTextSymbol());
	}

	public void testDefaultUnit() throws Exception {
		int u = CartographicSupportToolkit.DefaultMeasureUnit;
		for (int i = -1 /* -1 == pixel */; i < MapContext
				.getDistanceTrans2Meter().length; i++) {
			CartographicSupportToolkit.DefaultMeasureUnit = i;
			LabelClass lc = new LabelClass();
			assertTrue(
					"fails creating with system's context's default measure unit",
					lc.getUnit() == CartographicSupportToolkit.DefaultMeasureUnit);
			assertTrue(
					"LabelClass's XMLEntity's unit attribute is not correctly persisted",
...

		{
			...
			LabelClass lc = new LabelClass();
			assertTrue(
					"fails creating with system's context's default measure reference system",
					lc.getReferenceSystem() == CartographicSupportToolkit.DefaultReferenceSystem);

			assertTrue(
					"LabelClass's XMLEntity referenceSystem attribute is not correctly persisted",
					lc.getXMLEntity().getIntProperty("referenceSystem") == lc
							.getReferenceSystem());
		}
	...
	/**
	 * Ensures that sizing the label style causes the LabelClass exactly fits
	 * into its premises. It iterates over all the sizes defined in the field
	 * <b>sizes</b>, and all the backgrounds defined in <b>backgrounds</b>
	 * field.
	 */
		...
		for (ILabelStyle aux : testLabelStyles) {
			for (int i = 0; i < sizes.length; i++) {
				...
				LabelLocationMetrics llm = new LabelLocationMetrics(
						new Point2D.Double(sz.getWidth() / 4,
								sz.getHeight() / 4), 0, true);

				// new blank buffered image
				BufferedImage bi = new BufferedImage(sz.width * 2,
						sz.height * 2, BufferedImage.TYPE_INT_ARGB);
				...
				lc.draw(graphics, llm, new FPoint2D(llm.getAnchor()) /*
																	 * Although
																	 * for the
																	 * test it
																	 * is not
				...
				Rectangle wrappingRect = new Rectangle((int) Math.round(sz
						.getWidth() / 4), (int) Math.round(sz.getHeight() / 4),
						(int) Math.round(sz.getWidth()), (int) Math.round(sz
								.getHeight()));

				// assertFalse("fails sizing LabelClass, too big ("+sz.width+"x"+sz.height+"px)",
				// isOutsideRect(bi, wrappingRect, (int) OUTTER_TOLERANCE ));
				// assertTrue("fails sizing marker, too small ("+sz.width+"x"+sz.height+"px) \n"
				// +
				// "\t - forgot to enable ANTIALIASING?", fitsInsideRect(bi,
				// wrappingRect, INNER_TOLERANCE));
				try {
					fitsInOutBounds(bi, wrappingRect, INNER_TOLERANCE,
							OUTTER_TOLERANCE);
					ImageIO.write(bi, "png",
							new File(TMP_DIR + "LabelClass_" + sz.width + "x"
									+ sz.height + "_" + sty.getDescription()
									+ ".png"));
				} catch (IOException e) {
					e.printStackTrace();
				} catch (AssertionError e) {
					fail(sz.getWidth() + "x" + sz.getHeight() + "px, fails: "
							+ e.getMessage());
				}
	...
	private void fitsInOutBounds(BufferedImage bi, Rectangle bounds,
			float innerTolerance, float outterTolerance) {
		Rectangle2D bigBounds = new Rectangle2D.Double(bounds.getX()
				- outterTolerance, bounds.getY() - outterTolerance,
				bounds.getWidth() + (outterTolerance + outterTolerance),
				bounds.getHeight() + (outterTolerance + outterTolerance));

		Rectangle2D smallBounds = new Rectangle2D.Double(bounds.getX()
				+ outterTolerance, bounds.getY() + outterTolerance,
				bounds.getWidth() - (outterTolerance + outterTolerance),
				bounds.getHeight() - (outterTolerance + outterTolerance));
