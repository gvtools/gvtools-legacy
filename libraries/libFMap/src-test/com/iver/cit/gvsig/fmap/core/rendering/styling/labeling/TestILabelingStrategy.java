...
public class TestILabelingStrategy extends TestCase {
	...
	private static ArrayList<Class<? extends ILabelingStrategy>> getClassesToTest() {
		if (classesToTest == null) {
			...
			TestILabelingStrategy
					.addLabelingStrategyToTest(AttrInTableLabelingStrategy.class);
		}
	...
	/**
	 * The main purpose of this method is to create new strategies instances to
	 * be used for other test strategies.
	 * 
	 * @return
	 * @throws FieldNotFoundException
	 */
	public static ILabelingStrategy[] getNewStrategiesInstances()
			throws FieldNotFoundException {
		ILabelingStrategy[] strategies = new ILabelingStrategy[getClassesToTest()
				.size()];
		for (int i = 0; i < strategies.length; i++) {

			try {
				strategies[i] = (ILabelingStrategy) (getClassesToTest().get(i))
						.newInstance();
	...
	// /**
	// * Returns the labeling strategy currently in use. The labeling strategy
	// handles
	...
	//
	// /**
	// * Sets the labeling strategy that will be used the next time the the draw
	// is invoked.
	...
	//
	// /**
	// * Sets the PlacementConstraints that will determine where to place the
	// labels. The change will take effect next time the draw(...) strategy is
	// invoked.
	// * @param constraints
	// */
	...
	// */
	// public void draw(BufferedImage mapImage, Graphics2D mapGraphics, ViewPort
	// viewPort,
	// Cancellable cancel, double dpi) throws ReadDriverException;
	//
	// /**
	// * Applies the printer properties to the rendering process to match its
	// attributes.
	// * The result is manifested in the Graphics2D g which is the object sent
	// to the printer.
	// * @param g
	// * @param viewPort
	// * @param cancel
	...
	//
	// /**
	// * Returns a non-null String[] containing the names of the fields involved
	// in the
	// * labeling. If this strategy contains more than one LabelClass the result
	// is an
	// * array with all the names of the fields used by all the LabelClass, with
	// no duplicates.
	// * @return
	// */
