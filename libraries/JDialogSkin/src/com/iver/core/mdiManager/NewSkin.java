...
public class NewSkin extends Extension implements MDIManager{
    ...
    private void centerWindow(IWindow win) {
    	JDialog dialog = (JDialog) fws.getComponent(win);
 ...
 /*   private void addJFrame(IWindow p) {
    ...
    public class FrameListener extends WindowAdapter {
        ...
        public void windowClosing(WindowEvent e) {
            ...
            if (lastWindow != null) {
                ...
                JDialog frame = fws.getFrame(lastWindow);
                frame.toFront();
            }	
        }
    ...
    public boolean closeSingletonWindow(Class viewClass, Object model) {
        JDialog frame = sws.getFrame(viewClass, model);
        if (frame == null)
            return false;
        closeJFrame(frame);
        return true;
    ...
    public boolean closeSingletonWindow(Object model) {
        JDialog[] frames = sws.getFrames(model);
        if (frames.length == 0)
            return false;
    ...
    public void refresh(IWindow win) {
    	Component frame = fws.getComponent(win);
    	if (frame!=null)
