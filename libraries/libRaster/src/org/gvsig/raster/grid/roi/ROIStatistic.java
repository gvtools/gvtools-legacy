...
public class ROIStatistic {
	...
	public void calculateStatistics() throws GridException,
		...
		for (int iBand = 0; iBand < bandCount; iBand++) {
			if (values[iBand] > 0) {
				...
				variance[iBand] = variance[iBand] / (double) values[iBand]
			...
			for (int l = 0; l < roi.getNX(); l++) {
				for (int i = 0; i < bandCount; i++) {
					for (int j = i; j < bandCount; j++) {
						...
						if (!roi.isNoDataValue(valorBandai)
								&& !roi.isNoDataValue(valorBandaj)) {
		...
		for (int iBand = 0; iBand < bandCount; iBand++)
			for (int jBand = 0; jBand < bandCount; jBand++)
				if (iValues[iBand][jBand] > 1)
					varCov[iBand][jBand] = dSum[iBand][jBand]
							/ (double) (iValues[iBand][jBand]);
				else
	 ...
	 * 
	 * @return true si las estadisticas del grid están calculadas y false si no
	 *         lo están
	 */
	...
	/**
	 * Asigna el flag que informa si las estadisticas están calculadas para un
	 * grid concreto
	 * 
	 * @param calc
	 *            true si están calculadas y false si no lo están
	 */
