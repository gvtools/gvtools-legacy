 ...
 * This should be used when the window extent 'fits' into the structure
 * (coordinates and cellsize) of the grid, so it is faster than using a grid
 * wrapper with interpolation
 * 
 * Upon construction, cellsizes are not checked, so they are assumed to be
 * equal. Use a QueryableGridWindow to safely create a GridWrapper better than
...
public class GridNotInterpolated extends GridReader {
	...
	/**
	 * Crea un objeto lector a partir de un buffer de datos y el extent de la
	 * extensión completa y de la ventana accedida.
	 * 
	 * @param rb
	 *            Buffer de datos
	 * @param layerExtent
	 ...
	 */
	public GridNotInterpolated(RasterBuffer rb, GridExtent layerExtent,
	...
	/**
	 * Crea un objeto lector a partir de una fuente de datos y el extent de la
	 * extensión completa y de la ventana accedida. La fuente de datos no tiene
	 * bandas asignadas ni área de interés. Estas son calculadas a partir de los
	 * extent pasados por parámetro.
	 * 
	 * @param ds
	 *            Fuente de datos
	 * @param layerExtent
	 ...
	 *            Número de bandas del origen
	 */
	public GridNotInterpolated(BufferFactory ds, GridExtent layerExtent,
			GridExtent windowExtent, int[] bands) {
	...
	private void init() {
		...
		iWindowMinX = (int) ((windowExtent.minX() - layerExtent.minX()) / windowExtent
				.getCellSizeX());
		iWindowMinY = (int) ((layerExtent.maxY() - windowExtent.maxY()) / windowExtent
				.getCellSizeY());

		dMinX = Math.min(Math.max(windowExtent.minX(), layerExtent.minX()),
				layerExtent.maxX());
		dMinY = Math.min(Math.max(windowExtent.minY(), layerExtent.minY()),
				layerExtent.maxY());
		dMaxX = Math.max(Math.min(windowExtent.maxX(), layerExtent.maxX()),
				layerExtent.minX());
		dMaxY = Math.max(Math.min(windowExtent.maxY(), layerExtent.maxY()),
				layerExtent.minY());

		iBufMinX = (int) Math.floor((dMinX - windowExtent.minX())
				/ windowExtent.getCellSizeX())
				+ iWindowMinX;
		iBufMinY = (int) Math.floor((dMaxY - windowExtent.maxY())
				/ windowExtent.getCellSizeY())
				+ iWindowMinY;
		;
		iBufMaxX = (int) Math.floor((dMaxX - windowExtent.minX())
				/ windowExtent.getCellSizeX())
				+ iWindowMinX;
		iBufMaxY = (int) Math.floor((dMinY - windowExtent.maxY())
				/ windowExtent.getCellSizeY())
				+ iWindowMinY;
		...
		try {
			...
			bufferFactory.setAreaOfInterest(iBufMinX, iBufMinY, m_iWidth,
					m_iHeight);
	 ...
	 * @return true si la coordenada X e Y pasada por parámetro cae dentro del
	 *         grid y false si cae fuera.
	 */
	...
	public byte getCellValueAsByte(int x, int y)
		...
		try {
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY))
				return rasterBuf.getElemByte(y - m_iOffsetY, x - m_iOffsetX,
						bandToOperate);
			else
				return (byte) rasterBuf.getNoDataValue();
		} catch (ArrayIndexOutOfBoundsException e) {
			throw new RasterBufferInvalidAccessException("Access to data type "
					+ dataType + "invalid");
	...
	public short getCellValueAsShort(int x, int y)
		...
		try {
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY))
				return rasterBuf.getElemShort(y - m_iOffsetY, x - m_iOffsetX,
						bandToOperate);
			else
	...
	public int getCellValueAsInt(int x, int y)
		...
		try {
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY))
				return rasterBuf.getElemInt(y - m_iOffsetY, x - m_iOffsetX,
						bandToOperate);
			else
	...
	public float getCellValueAsFloat(int x, int y)
		...
		try {
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY))
				return rasterBuf.getElemFloat(y - m_iOffsetY, x - m_iOffsetX,
						bandToOperate);
			else
	...
	public double getCellValueAsDouble(int x, int y)
		...
		try {
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY))
				return rasterBuf.getElemDouble(y - m_iOffsetY, x - m_iOffsetX,
						bandToOperate);
			else
		...
		try {
			if (dataType == RasterBuffer.TYPE_DOUBLE) {
				...
				return (double) rasterBuf.getElemByte(x, y, 0);
					...
					| (dataType == RasterBuffer.TYPE_USHORT)) {
			...
			if (isInRasterBuf(x - m_iOffsetX, y - m_iOffsetY)) {
				try {
					...
					throw new RasterBufferInvalidAccessException(
							"Access to data type " + dataType + "invalid");
				} catch (NullPointerException e) {
					throw new RasterBufferInvalidException("Null Buffer");
				}
				return b;
