...
public class ConvolutionByteFilter extends ConvolutionFilter {
	...
	public void process(int col, int line) throws InterruptedException {
		...
		for (int band = 0; band < raster.getBandCount(); band++) {
			/*
			 * TODO: Mirar de cambiar el sistema de recorrido de pixels, para
			 * tener metodos circulares, repetitivos o de descarte.
			 * 
			 * int posx = 0; int posy = 0; for (int j = -semiLado; j <=
			 * semiLado; j++) { for (int i = -semiLado; i <= semiLado; i++) {
			 * posx = col + i; posy = line + j; if (posx < 0) posx=0; if (posy <
			 * 0) posy=0; if (posx >= width) posx=width - 1; if (posy >= height)
			 * posy=height - 1;
			 * 
			 * ventana[i + semiLado][j + semiLado] = raster.getElemByte(posy,
			 * posx, band) & 0xff; } }
			 */
			...
			if ((col - semiLado >= 0) && (line - semiLado >= 0)
				...
				for (int j = -semiLado; j <= semiLado; j++)
					for (int i = -semiLado; i <= semiLado; i++)
						ventana[i + semiLado][j + semiLado] = raster
								.getElemByte(line + j, col + i, band) & 0xff;
				...
				rasterResult.setElem(line, col, band,
						(byte) raster.getElemByte(line, col, band));
		}
	}
