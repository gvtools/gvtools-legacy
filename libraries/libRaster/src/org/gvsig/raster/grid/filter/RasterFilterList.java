...
public class RasterFilterList {
	...
	/**
	 * Asigna un listener a la lista que será informado cuando cambie un filtro
	 * o posición en la lista.
	 * 
	 * @param listener
	 *            FilterListListener
	 */
	...
	private void callFilterListChanged(Object obj) {
		for (int i = 0; i < filterListListener.size(); i++) {
			...
			((FilterListChangeListener) filterListListener.get(i))
					.filterListChanged(ev);
		}
	}

	/**
	 * Añade un parámetro a la lista de filtros. Estos parámetros luego pueden
	 * ser utilizados por los managers que se registren
	 * 
	 * @param key
	 *            Nombre del parámetro que coincide con el nombre de la clase.
	 * @param value
	 *            Objeto
	 */
	 ...
	 * 
	 * @param key
	 *            Identificador del parámetro. Coincide con el nombre de la
	 *            clase del parámetro.
	 */
	...
	public int getOutDataType() {
		if (list.size() > 0)
			return ((RasterFilter) list.get(list.size() - 1))
	...
	/**
	 * Obtiene el filtro apilado de la posición i o null si el indice es
	 * incorrecto
	 * 
	 * @param i
	 ...
	 * @param name
	 *            Nombre de filtro
	 * @return Filtro en caso de que exista un filtro apilado de ese tipo o null
	 *         si no hay ninguno.
	 */
	...
	/**
	 * Obtiene el primer filtro de la lista que es instancia de la clase pasada
	 * por parámetro
	 * 
	 * @param baseFilterClass
	 *            Filtro base
	 ...
	 * @param position
	 *            Posición a asignar
	 * @return Devuelve true si el filtro existia y lo ha movido y false si no
	 *         lo ha hecho.
	 */
	...
	private void executeFilterByDataType(int dataType)
		...
		for (int i = 0; i < list.size(); i++) {
			...
			// cerciorarse de si el tipo del filtro es totalmente el correcto o
			// hay
			// que recrearlo. Ejemplo:
			// Si el filtro que tenemos antes de preprocesar es de tipo Byte y
			// la
			// entrada de datos es de tipo float, reconstruir solo este filtro
			// para
			...
			// Si el filtro genera una banda alpha se mezcla con la que han
			// generado otros
			if (filter.getResult("alphaBand") != null) {
				if (alphaBand != null)
					alphaBand = Transparency.merge(alphaBand,
							(IBuffer) filter.getResult("alphaBand"));
				else
	...
	/**
	 * Guarda el estado de la lista de filtros en una pila, que se podrá ir
	 * recuperando con popStatus()
	 */
	...
	/**
	 * Recupera un estado guardado con antelación mediante el método
	 * pushStatus()
	 */
