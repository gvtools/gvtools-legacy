...
public class FirstDerivativeFilter extends RasterFilter {
	public static String[] names = new String[] { "sobel", "roberts",
			"prewitt", "freichen" };
	...
	static final double prewittH[][] = { { 1, 0, -1 }, { 1, 0, -1 },
			{ 1, 0, -1 } };
	static final double prewittV[][] = { { -1, -1, -1 }, { 0, 0, 0 },
			{ 1, 1, 1 } };
	static final Kernel prewittHKernel = new Kernel(prewittH);
	static final Kernel prewittVKernel = new Kernel(prewittV);

	static final double freiChenH[][] = { { -1, -1.4D, -1 }, { 0, 0, 0 },
			{ 1, 1.4D, 1 } };
	static final double freiChenV[][] = { { -1, 0, 1 }, { -1.4D, 0, 1.4D },
			{ -1, 0, 1 } };
	// static final double freiChenH[][]=
	// {{-1,-1.4142D,-1},{0,0,0},{1,1.4142D,1}};
	// static final double freiChenV[][]=
	// {{-1,0,1},{-1.4142D,0,1.4142D},{-1,0,1}};
	...
	public Params getUIParams(String nameFilter) {
		...
		params.setParam("Umbral", new Integer(umbral), Params.SLIDER,
				new String[] { "0", "255", "0", "1", "25" }); // min, max, valor
																// defecto,
																// intervalo
																// pequeño,
																// intervalo
																// grande;
		params.setParam("Compare", new Boolean(compare), Params.CHECK, null);
		params.setParam("FilterName", getName(), -1, null);
		return params;
	}
