...
public class RGBToCMYKByteFilter extends RGBToCMYKFilter {
	...
	public void process(int col, int line) throws InterruptedException {
		...
		if (value[0] == 0 && value[1] == 0 && value[2] == 2)
			System.out.println();
		double[] cmyk = colorConversion.RGBtoCMYK(value[0] & 0xff,
				value[1] & 0xff, value[2] & 0xff, 1);
		if (out == IBuffer.TYPE_BYTE) {
			double[] rgb = colorConversion.CMYKtoRGB(cmyk[0], cmyk[1], cmyk[2],
					cmyk[3]);
			for (int band = 0; band < 3; band++)
				value[band] = (byte) (rgb[band] * 255);

			// System.out.println();
			// System.out.println("+" + cmyk[0] + " " + cmyk[1] + " " + cmyk[2]
			// + " " + cmyk[3]);
			/*
			 * value[0] = (byte)(((byte) (cmyk[0] * 100. )) & 0xff); value[1] =
			 * (byte)(((byte) (cmyk[1] * 100. )) & 0xff); value[2] =
			 * (byte)(((byte) (cmyk[2] * 100. )) & 0xff); value[3] =
			 * (byte)(((byte) (cmyk[3] * 100. )) & 0xff);
			 */
			/*
			 * value[0] = ((byte) Math.round(cmyk[0] * 100)); value[1] = ((byte)
			 * Math.round(cmyk[1] * 100)); value[2] = ((byte) Math.round(cmyk[2]
			 * * 100)); value[3] = ((byte) Math.round(cmyk[3] * 100));
			 */
