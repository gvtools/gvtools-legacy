...
public class EnhancementListManager implements IRasterFilterListManager {
	...
	/**
	 * Registra EnhancementListManager en los puntos de extension de
	 * RasterFilter
	 */
	 ...
	 * 
	 * @param removeEnds
	 *            eliminar extremos en los máximos y mínimos
	 * @param stats
	 ...
	 */
	public void addEnhancedFilter(boolean removeEnds, IStatistics stats,
			double tailTrim, int[] renderBands) throws FilterTypeException {
		try {
			if (tailTrim == 0) { // En este caso siempre es necesario el máximo
									// y
				// mínimo
			...
			RasterFilter filter = createEnhancedFilter(removeEnds, stats,
					tailTrim, renderBands);
			if (filter != null)
	...
	public static RasterFilter createEnhancedFilter(boolean removeEnds,
			IStatistics stats, double tailTrim, int[] renderBands) {
	...
	public ArrayList getStringsFromFilterList(ArrayList filterList,
		...
		if (rf instanceof LinearEnhancementFilter) {
			...
			filterList.add("filter.enhanced.tailTrim="
					+ ((LinearEnhancementFilter) rf).getTailTrim().toString());
			...
			filterList
					.add("filter.enhanced.remove="
							+ ((LinearEnhancementFilter) rf).getRemoveEnds()
									.toString());
		}
		...
		if (classFilter.equals(LinearEnhancementFilter.class)) {
			...
			for (int i = 0; i < params.getNumParams(); i++) {
				...
				if (params.getParam(i).id.equals("TailTrim")
					...
					tailTrim = ((Double) params.getParam(i).defaultValue)
							.doubleValue() / 100.0;
			}

			addEnhancedFilter(removeEnds,
