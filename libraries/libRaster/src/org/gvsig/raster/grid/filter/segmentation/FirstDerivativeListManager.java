...
public class FirstDerivativeListManager implements IRasterFilterListManager {
	...
	/**
	 * Registra FirstDerivativeListManager en los puntos de extension de
	 * RasterFilter
	 */
	 ...
	 */
	public void addFirstDerivativeFilter(int umbral, boolean compare,
			String Name) throws FilterTypeException {
	...
	public ArrayList getStringsFromFilterList(ArrayList filterList,
		...
		if (rf instanceof FirstDerivativeFilter) {
			...
			filterList.add("filter.firstDerivative.umbral="
					+ firstDerivative.umbral);
			filterList.add("filter.firstDerivative.compare="
					+ firstDerivative.compare);
			filterList.add("filter.firstDerivative.filterName="
					+ firstDerivative.getName());
		}
		...
		if ((fil.startsWith("filter.firstDerivative.active"))
			...
			for (int prop = 0; prop < filters.size(); prop++) {
				...
				if (elem.startsWith("filter.firstDerivative.umbral")) {
					umbral = Integer.parseInt(RasterFilterListManager
							.getValue(elem));
					filters.remove(prop);
					prop--;
				}
				if (elem.startsWith("filter.firstDerivative.compare")) {
					compare = Boolean.getBoolean(RasterFilterListManager
							.getValue(elem));
					filters.remove(prop);
					prop--;
				}
		...
		if (classFilter.equals(FirstDerivativeFilter.class)) {
			...
			for (int i = 0; i < params.getNumParams(); i++) {
				if (params.getParam(i).id.equals("Umbral")
					...
					umbral = ((Integer) params.getParam(i).defaultValue)
							.intValue();
				if (params.getParam(i).id.equals("Compare")
						&& params.getParam(i).defaultValue instanceof Boolean)
					compare = ((Boolean) params.getParam(i).defaultValue)
							.booleanValue();
