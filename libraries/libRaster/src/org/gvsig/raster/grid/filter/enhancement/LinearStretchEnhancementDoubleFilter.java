...
/**
 * Filtro de realce lineal para tipos de datos double. El realce es aplicado por
 ...
 */
public class LinearStretchEnhancementDoubleFilter extends
	 ...
	 */
	private void processValue(double p, Stretch data, int col, int line,
		...
		if (data.scale != null) {
			...
			for (int i = 0; i < data.scale.length; i++) {
				if ((p > data.stretchIn[i] && p <= data.stretchIn[i + 1])
					...
					p = (double) ((((double) p) - data.stretchIn[i])
							* data.scale[i] + data.offset[i]);
					break;
				}
			}
