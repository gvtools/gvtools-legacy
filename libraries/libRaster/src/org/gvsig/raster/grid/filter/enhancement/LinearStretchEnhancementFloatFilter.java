...
/**
 * Filtro de realce lineal para tipos de datos float. El realce es aplicado por
 ...
 */
public class LinearStretchEnhancementFloatFilter extends
	 ...
	 */
	private void processValue(float p, Stretch data, int col, int line,
		...
		if (data.scale != null) {
			...
			for (int i = 0; i < data.scale.length; i++) {
				if ((p > data.stretchIn[i] && p <= data.stretchIn[i + 1])
					...
					p = (float) ((((double) p) - data.stretchIn[i])
