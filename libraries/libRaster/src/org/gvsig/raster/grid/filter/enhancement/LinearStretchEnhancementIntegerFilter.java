...
/**
 * Filtro de realce lineal para tipos de datos int. El realce es aplicado por
 ...
 */
public class LinearStretchEnhancementIntegerFilter extends
		...
		if (data.scale != null) {
			...
			for (int i = 0; i < data.scale.length; i++) {
				if ((p > data.stretchIn[i] && p <= data.stretchIn[i + 1])
					...
					p = (int) ((((double) p) - data.stretchIn[i])
