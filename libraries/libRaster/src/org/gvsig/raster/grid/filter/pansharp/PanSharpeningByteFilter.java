...
public class PanSharpeningByteFilter extends PanSharpeningFilter {
	...
	/**
	 * Filtro de pansharpening aplicado a un buffer Multiespectral y
	 * Pancromatico de tipo byte El buffer resultante es de tipo byte
	 */
	...
	/**
	 * Se toman los buffers multiespectral y pancromatico para la aplicacion del
	 * filtro.
	 */
	public void init() {
		...
		try {
			...
			BufferFactory bfFactoryMultiespc = new BufferFactory(
					dataset.getDataSource());
			Extent adjustedExtent = RasterUtilities.calculateAdjustedView(
					dataset.getDataSource().getExtent(), dataset
							.getDataSource().getAffineTransform(0),
					new Dimension(widthMultiespec, heightMultiespec));
			bfFactoryMultiespc.setDrawableBands(bandas);
			try {
				bfFactoryMultiespc.setAreaOfInterest(adjustedExtent.getULX(),
						adjustedExtent.getULY(), adjustedExtent.getLRX(),
						adjustedExtent.getLRY(), widthMultiespec,
						heightMultiespec);
			...
			BufferFactory bfFactoryPancr = new BufferFactory(
					dataset.getDataSource());
			Extent adjustedExtentP = RasterUtilities.calculateAdjustedView(
					dataset.getDataSource().getDataset(pancrName).getExtent(),
					dataset.getDataSource().getDataset(pancrName)
							.getAffineTransform(), new Dimension(widthPancr,
							heightPancr));
			bfFactoryPancr.setDrawableBands(new int[] { posPancromatica });
			try {
				bfFactoryPancr.setAreaOfInterest(adjustedExtentP.getULX(),
						adjustedExtentP.getULY(), adjustedExtentP.getLRX(),
						adjustedExtentP.getLRY(), widthPancr, heightPancr);
			} catch (RasterDriverException e) {
				e.printStackTrace();
			} catch (InvalidSetViewException e) {
				e.printStackTrace();
	...
	/**
	 * Validamos que haya alguna otra banda además de la pancromática y que la
	 * pancromática sea de mayor resolución que las demás.
	 */
	private void checkInput() {

		exec = true;
		if (heightMultiespec > heightPancr || widthMultiespec > widthPancr
				|| heightMultiespec == 0 || widthMultiespec == 0)
			exec = false;
		String ficheros[] = dataset.getFileName();
		for (int i = 0; i < ficheros.length; i++) {
			if (ficheros[i] != pancrName) {
				if (dataset.getDataSource().getDataset(ficheros[i]).getHeight() != heightMultiespec
						|| dataset.getDataSource().getDataset(ficheros[i])
								.getWidth() != widthMultiespec) {
					exec = false;
					break;
				}
			}
	 ...
	 * posPancromatica: posicion de la banda pancromatica dataset: bandOrden:
	 * array con las bandas que se visualizan alpha: parametro alpha coef:
	 * parametro coef coefBrovey: parametro co filenameOutput: ruta con el
	 * nombre del ficheo de salida(fichero temporal) heightPancr: height de la
	 * imagen Pancromatica widthPancr : width de la imagen Pancromatica
	 * widthMultiespec: width de la imagen multiespectral heightMultiespec:
	 * heigth de la imagen multiespectral
	 * 
	 */
	private void loadParam() {
		...
		pancrName = dataset.getDataSource().getBands().getBand(posPancromatica)
				.getFileName();
		...
		for (int i = 0; i < ficherosNames.length; i++) {
			if (ficherosNames[i] != pancrName) {
				heightMultiespec = dataset.getDataSource()
						.getDataset(ficherosNames[i]).getHeight();
				widthMultiespec = dataset.getDataSource()
						.getDataset(ficherosNames[i]).getWidth();
			}
		}
	...
	/**
	 * Aplica la operación de refinamiento sobre el buffer Multiespectral que
	 * contiene el RGB que es pasado por parámetro utilizando la pancromática.
	 * 
	 * @param bufferInput
	 *            Buffer rgb
	 * @param length
	 *            longitud del buffer de la pancromatica utilizado
	 * @param iLine
	 *            línea leida de la imagen multiespectral
	 * @return
	...
	private void processIHS(byte[][] bufferInput, int iLine)
		...
		for (int iElem = 0; iElem < bufferInput[0].length; iElem++) {
			xyz = colorConversion.RGBtoHSL(
					(int) (bufferInput[0][iElem] & 0xff),
					(int) (bufferInput[1][iElem] & 0xff),
					(int) (bufferInput[2][iElem] & 0xff));
			for (int j = 0; j < relX; j++) {
				for (int k = 0; k < relY; k++) {
					line = iLine * relX + j;
					col = iElem * relY + k;
					xyz[2] = (((bufferPancr.getElemByte(line, col, 0) & 0xff)) / 255.0)
							+ coef;
					tmp[0] = (int) (255.0 * xyz[0] / 360.0 + 0.5);
					tmp[2] = (int) (xyz[2] * 255. + 0.5);
					tmp[1] = (int) (xyz[1] * 255. + 0.5);
					uvw = colorConversion.HSLtoRGB(tmp[0], tmp[1], tmp[2]);
