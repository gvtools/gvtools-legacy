...
public class GridExtent extends Extent {
	 ...
	 *            cell size
	 */
	public GridExtent(double minX, double minY, double maxX, double maxY,
			double dCellSize) {
	...
	public boolean fitsIn(GridExtent extent) {
		...
		if ((extent.getCellSizeX() != this.getCellSizeX())
				|| (extent.getCellSizeY() != this.getCellSizeY())) {
			return false;
		}
	...
	public boolean equals(GridExtent extent) {
		return (minX() == extent.minX() && maxX() == extent.maxX()
				...
				&& cellSizeX == extent.getCellSizeX() && cellSizeY == extent
					.getCellSizeY());
	}
	 ...
	 * getMin().setLocation(Math.min(layer.getMinX(), minX()),
	 * Math.min(layer.getMinY(), minY()));
	 * getMax().setLocation(Math.max(layer.getMaxX(), maxX()),
	 * Math.max(layer.getMaxY(), maxY()));
	 * 
