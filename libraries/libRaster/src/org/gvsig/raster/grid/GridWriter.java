...
public class GridWriter {
	 ...
	 *            Extent del grid
	 * @param dataType
	 *            Tipo de dato del grid
	 * @param rasterBuf
	 *            Buffer de datos del grid
	 */
	 ...
	 * Con ellos se creará el GridExtent y el buffer de datos para escribir.
	 * 
	 * @param iNX
	 *            Ancho en pixeles
	 * @param iNY
	 ...
	 *            Tamaño de celda
	 * @param dMinX
	 *            Coordenada mínima en X
	 * @param dMinY
	 *            Coordenada mínima en Y
	 * @param dataType
	 *            Tipo de dato
	 * @param bands
	 *            Bandas a escribir
	 */
	public GridWriter(int iNX, int iNY, double dCellSize, double dMinX,
			double dMinY, int dataType, int[] bands) {
		...
		rasterBuf = RasterBuffer.getBuffer(dataType, iNX, iNY, bands.length,
				true);
	}
	...
	public void setCellValue(int iX, int iY, byte value)
			throws OutOfGridException, InterruptedException {
		if (isCellInGrid(iX, iY))
			rasterBuf.setElem(iY, iX, bandToOperate, value);
		else
			throw new OutOfGridException("Point:(" + iX + "," + iY
					+ ") out of grid.");
	}

	public void setCellValue(int iX, int iY, short value)
			throws OutOfGridException, InterruptedException {
		if (isCellInGrid(iX, iY))
			rasterBuf.setElem(iY, iX, bandToOperate, value);
		else
	...
	public void setCellValue(int iX, int iY, int value)
			throws OutOfGridException, InterruptedException {
		if (isCellInGrid(iX, iY))
			rasterBuf.setElem(iY, iX, bandToOperate, value);
		else
	...
	public void setCellValue(int iX, int iY, float value)
			throws OutOfGridException, InterruptedException {
		if (isCellInGrid(iX, iY))
			rasterBuf.setElem(iY, iX, bandToOperate, value);
		else
	...
	public void setCellValue(int iX, int iY, double value)
			throws OutOfGridException, InterruptedException {
		if (isCellInGrid(iX, iY))
			rasterBuf.setElem(iY, iX, bandToOperate, value);
		else
	...
	public void setNoData(int iX, int iY) throws InterruptedException {
		if (isCellInGrid(iX, iY)) {
			...
			case IBuffer.TYPE_BYTE:
				rasterBuf.setElem(iY, iX, bandToOperate,
						(byte) rasterBuf.getNoDataValue());
				break;
			case IBuffer.TYPE_SHORT:
				rasterBuf.setElem(iY, iX, bandToOperate,
						(short) rasterBuf.getNoDataValue());
				break;
			case IBuffer.TYPE_INT:
				rasterBuf.setElem(iY, iX, bandToOperate,
						(int) rasterBuf.getNoDataValue());
				break;
			case IBuffer.TYPE_FLOAT:
				rasterBuf.setElem(iY, iX, bandToOperate,
						(float) rasterBuf.getNoDataValue());
				break;
			case IBuffer.TYPE_DOUBLE:
				rasterBuf.setElem(iY, iX, bandToOperate,
						(double) rasterBuf.getNoDataValue());
				break;
			}
		}
	}
	 ...
	 * Esta llamada afecta solo a la banda 0 del raster y es especial para MDT's
	 * y para conservar la compatibilidad con código que la usa de esta forma.
	 * 
	 * @param sFile
	 *            Nombre del fichero
	 * @throws IOException
	 ...
	 */
	public void ExportToArcViewASCIIFile(String sFile) throws IOException,
	...
	public void ExportToArcViewASCIIFile(String sFile, int band)
		...
		fout.write("nodata_value "
				+ Float.toString((float) rasterBuf.getNoDataValue()));
		fout.newLine();

		switch (rasterBuf.getDataType()) {
		case IBuffer.TYPE_BYTE:
			for (int i = 0; i < extent.getNY(); i++) {
				for (int j = 0; j < extent.getNX(); j++)
					fout.write(Byte.toString(rasterBuf.getElemByte(i, j, band)/*
																			 * [j
																			 * ]
																			 * [
																			 * i
		...
		case IBuffer.TYPE_SHORT:
			for (int i = 0; i < extent.getNY(); i++) {
				for (int j = 0; j < extent.getNX(); j++)
					fout.write(Short.toString(rasterBuf
							.getElemShort(i, j, band)/* [j][i] */) + " ");
				fout.newLine();
			}
			break;
		case IBuffer.TYPE_INT:
			for (int i = 0; i < extent.getNY(); i++) {
				for (int j = 0; j < extent.getNX(); j++)
					fout.write(Integer.toString(rasterBuf
							.getElemInt(i, j, band)/* [j][i] */) + " ");
				fout.newLine();
			}
			break;
		case IBuffer.TYPE_FLOAT:
			for (int i = 0; i < extent.getNY(); i++) {
				for (int j = 0; j < extent.getNX(); j++)
					fout.write(Float.toString(rasterBuf
							.getElemFloat(i, j, band)/* [j][i] */) + " ");
				fout.newLine();
			}
			break;
		case IBuffer.TYPE_DOUBLE:
			for (int i = 0; i < extent.getNY(); i++) {
				for (int j = 0; j < extent.getNX(); j++)
					fout.write(Double.toString(rasterBuf.getElemDouble(i, j,
							band)/* [j][i] */) + " ");
				fout.newLine();
			}
			break;
	...
	/**
	 * Consulta si el grid tiene la posición indicada por parámetro o si esta
	 * cae fuera del área.
	 * 
	 * @param iX
	 *            Posición X
	...
	public boolean isCellInGrid(int iX, int iY) {
		return (iX >= 0 && iX < extent.getNX() && iY >= 0 && iY < extent
				.getNY());
	}
