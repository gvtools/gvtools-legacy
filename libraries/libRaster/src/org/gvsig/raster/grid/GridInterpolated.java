...
/**
 * A grid wrapper that performs interpolation to calculate cell values. This
 * should be used when the window extent does not 'fit' into the structure
 * (coordinates and cellsize) of the grid.
 * 
 * @author Victor Olaya
 */
...
public class GridInterpolated extends GridReader {
	 ...
	 */
	public GridInterpolated(RasterBuffer rb, GridExtent layerExtent,
	 ...
	 *            Número de bandas del origen
	 */
	public GridInterpolated(BufferFactory ds, GridExtent layerExtent,
			GridExtent windowExtent, int[] bands) {
	...
	private void init() {
		...
		m_dCellSizeX = bufferFactory.getDataSource().getAffineTransform(0)
				.getScaleX();
		m_dCellSizeY = bufferFactory.getDataSource().getAffineTransform(0)
				.getScaleY();
	...
	private double _getValueAt(int x, int y)
			throws RasterBufferInvalidAccessException,
			RasterBufferInvalidException, InterruptedException {
		double dX = windowExtent.minX() + windowExtent.getCellSize()
				* (x + 0.5);
		double dY = windowExtent.maxY() - windowExtent.getCellSize()
				* (y + 0.5);
		return getValueAt(dX, dY);
	}

	/**
	 * Devuelve el valor interpolado para unas coordenadas de la imagen no
	 * enteras la distancia se toma al vertice superior izquierdo del pixel
	 * 
	 * @throws InterruptedException
	 * */
	public double _getValueAt(double x, double y)
			throws RasterBufferInvalidAccessException,
			RasterBufferInvalidException, InterruptedException {
		int x_int, y_int;
	...
	private double _getValueInverseDistance(int x, int y, double dx, double dy)
		...
		if (dx > 0.0 || dy > 0.0) {
			...
			dValue = _getCellValueInLayerCoords(x + 1, y + 1);
			if (!isNoDataValue(dValue)) {
				d = 1.0 / Math.sqrt((1.0 - dx) * (1.0 - dx) + (1.0 - dy)
						* (1.0 - dy));
				z += d * dValue;
				n += d;
			}
	...
	public byte[] getBandsValuesAsByte(int x, int y) {
		// TODO: FUNCIONALIDAD: getBandsValuesAsByte:Obtener los valores
		// interpolados en todas las bandas
		return null;
	}

	public short[] getBandsValuesAsShort(int x, int y) {
		// TODO: FUNCIONALIDAD: getBandsValuesAsShort:Obtener los valores
		// interpolados en todas las bandas
		return null;
	}

	public int[] getBandsValuesAsInt(int x, int y) {
		// TODO: FUNCIONALIDAD: getBandsValuesAsInt:Obtener los valores
		// interpolados en todas las bandas
		return null;
	}

	public float[] getBandsValuesAsFloat(int x, int y) {
		// TODO: FUNCIONALIDAD: getBandsValuesAsFloat:Obtener los valores
		// interpolados en todas las bandas
		return null;
	}

	public double[] getBandsValuesAsDouble(int x, int y) {
		// TODO: FUNCIONALIDAD: getBandsValuesAsDouble:Obtener los valores
		// interpolados en todas las bandas
		return null;
	}
}
