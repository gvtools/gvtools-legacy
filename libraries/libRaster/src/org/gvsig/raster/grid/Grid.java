...
/**
 * Clase que representa una rejilla de datos raster. Este tipo de grid tiene el
 * interfaz necesario de acceso a la información raster para aplicaciones de
 * analisis raster. Contiene métodos de acceso al dato tanto en lectura como en
 * escritura y encapsula operaciones entre grids. Además contiene objetos que
 * contienen las características asociadas a ese grid. Sobre un grid pueden
 * aplicarse también operaciones de filtrado.
 * 
 * Podemos crear un Grid de cinco formas distitas:
 * <UL>
 * <LI>A partir de una fuente de datos (cargada en el constructor). Se hará una
 * petición del extent pasado por parámetro con las bandas seleccionadas por
 * parámetro.</LI>
 * <LI>A partir de un extensión de capa, una extensión de vista, un tipo de dato
 * y un número de datos crea un Grid vacio util para escritura.</LI>
 * <LI>A partir de una fuente de datos (cargados). Datasource se usa como buffer
 * de datos cargados y se selecciona si queremos el reader interpolado.</LI>
 * <LI>A partir de una fuente de datos (cargada en el constructor). Se hará una
 * petición del extent completo de la fuente con las bandas seleccionadas por
 * parámetro.</LI>
 * <LI>A partir de una fuente de datos (cargada en el constructor). Se hará una
 * petición del extent completo de la fuente con todas las bandas disponibles.</LI>
 * </UL>
 * 
...
public class Grid implements IQueryableGrid, IWritableGrid {
	...
	/**
	 * Crea un grid a partir de un MultiRasterDataset. Usa el extent de la
	 * fuente de datos como extent completo y el extent pasado como parámetro
	 * como extensión de ventana.
	 * 
	 * Cuando se construye el reader se carga el buffer con la extensión
	 * definida en windowExtent y las bandas especificadas en bands.
	 * 
	 * @param IRasterDataSource
	 *            datasets que proporcionan los datos
	 * @param bands
	 *            número de bandas requeridas
	 * @param windowExtent
	...
	public Grid(IRasterDataSource datasets, int[] bands, GridExtent windowExtent)
		...
		double cellSize[] = calcCellSize(bufferFactory.getDataSource(),
				bufferFactory.getSourceWidth(), bufferFactory.getSourceHeight());
		layerExtent = new GridExtent(bufferFactory.getDataSource().getExtent(),
				cellSize[0], cellSize[1]);
		if (bufferFactory.getDataSource() != null
				&& bufferFactory.getDataSource().getDataType() != null)
		...
		if (this.windowExtent.fitsIn(layerExtent))
			reader = new GridNotInterpolated(bufferFactory, layerExtent,
					this.windowExtent, bands);
		else {
			reader = new GridInterpolated(bufferFactory, layerExtent,
					this.windowExtent, bands);
		}
	...
	/**
	 * Carga el buffer de datos desde el reader para poder escribir sobre los
	 * datos de la ventana original. Esto es útil cuando el GridWriter se crea
	 * asociado a un raster pero el buffer está vacio, por ejemplo en el caso de
	 * crear un GridInterpolated. Un GridInterpolated no carga ningún buffer en
	 * memoria sino que accede a los datos en disco a medida que se le van
	 * solicitando. Por ello, no es posible modificarlos sino cargamos un buffer
	 * con los datos previamente.
	 */
	public void loadWriterData() throws GridException {
		rasterBuf = RasterBuffer.getBuffer(dataType, reader.getNX(),
				reader.getNY(), bands.length, true);
		...
		try {
			...
			case IBuffer.TYPE_BYTE:
				for (x = 0; x < getNX(); x++) {
					for (y = 0; y < getNY(); y++)
						writer.setCellValue(x, y,
								(byte) (reader.getCellValueAsByte(x, y)));
				}
				break;
			case IBuffer.TYPE_SHORT:
				for (x = 0; x < getNX(); x++) {
					for (y = 0; y < getNY(); y++)
						writer.setCellValue(x, y,
								(short) (reader.getCellValueAsShort(x, y)));
				}
				break;
			case IBuffer.TYPE_INT:
				for (x = 0; x < getNX(); x++) {
					for (y = 0; y < getNY(); y++)
						writer.setCellValue(x, y,
								(int) (reader.getCellValueAsInt(x, y)));
				}
				break;
			case IBuffer.TYPE_FLOAT:
				for (x = 0; x < getNX(); x++) {
					for (y = 0; y < getNY(); y++)
						writer.setCellValue(x, y,
								(float) (reader.getCellValueAsFloat(x, y)));
				}
				break;
			case IBuffer.TYPE_DOUBLE:
				for (x = 0; x < getNX(); x++) {
					for (y = 0; y < getNY(); y++)
						writer.setCellValue(x, y,
								(double) (reader.getCellValueAsDouble(x, y)));
				}
				break;
			}
		} catch (RasterBufferInvalidException e) {
			throw new GridException("Buffer de datos no válido " + x + " " + y,
					e);
		} catch (OutOfGridException e1) {
			throw new GridException("Acceso fuera de los límites del Grid " + x
					+ " " + y, e1);
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("Acceso al grid no válido " + x + " " + y,
					e);
	...
	/**
	 * Crea un grid vacio a partir de un extent y un tipo de datos. Se crea un
	 * buffer vacio en el que puede escribirse a través del writer. Los datos
	 * escritos pueden ser consultados con el reader. No tiene una fuente de
	 * datos asociada ya que es un grid vacio basicamente para escritura.
	 * 
	 * @param layerExtent
	 *            Tamaño completo de la capa
	 * @param windowExtent
	 *            Ventana de datos.
	 * @param dataType
	 *            Tipo de datos del buffer
	 * @param bands
	 *            número de bandas requeridas y orden de dibujado en el buffer
	 */
	public Grid(GridExtent layerExtent, GridExtent windowExtent, int dataType,
			int[] bands) throws RasterBufferInvalidException {
		this.windowExtent = windowExtent;
		this.layerExtent = layerExtent;
		this.dataType = dataType;

		rasterBuf = RasterBuffer.getBuffer(dataType, layerExtent.getNX(),
				layerExtent.getNY(), bands.length, true);

		if (windowExtent.fitsIn(layerExtent))
			reader = new GridNotInterpolated(rasterBuf, layerExtent,
					windowExtent, bands);
		else
			reader = new GridInterpolated(rasterBuf, layerExtent, windowExtent,
					bands);
		writer = new GridWriter(layerExtent, dataType, rasterBuf);
		init(null);
	}
	...
	/**
	 * Crea un grid a partir de un BufferFactory. El buffer debe estar cargado
	 * de datos y el extent de este viene definido en el parámetro windowExtent.
	 * 
	 * @param bufferFactory
	 *            Fuente de datos
	 * @param windowExtent
	 *            Extent de los datos cargados en bufferFactory
	 * @param notInterp
	 *            Si es true fuerza a que el reader sea sin interpolación. Si es
	 *            false decide si el reader es interpolado o no a partir de los
	 *            extents de capa y la ventana seleccionada.
	 */
	public Grid(BufferFactory bufferFactory, boolean notInterp) {
		double cellSize[] = calcCellSize(bufferFactory.getDataSource(),
				bufferFactory.getSourceWidth(), bufferFactory.getSourceHeight());
		this.layerExtent = new GridExtent(bufferFactory.getDataSource()
				.getExtent(), cellSize[0], cellSize[1]);
		if (bufferFactory.getDataSource() != null
				&& bufferFactory.getDataSource().getDataType() != null)
			dataType = bufferFactory.getDataSource().getDataType()[0];
		...
		if (notInterp) {
			...
			if (windowExtent.fitsIn(layerExtent))
				...
				reader = new GridInterpolated(rasterBuf, layerExtent,
						windowExtent, bands);
		}
		writer = new GridWriter(layerExtent, dataType, rasterBuf);
		init(bufferFactory);
	...
	/**
	 * Crea un grid a partir de un BufferFactory. Se hará una petición del
	 * extent completo de la fuente. bufferFactory tiene asociada una fuente de
	 * datos pero se ignorará si tiene el buffer lleno. La instanciación de
	 * GridNotInterpolated hará que se haga la petición para cargar raster
	 * completo.
	 * 
	 * @param bufferFactory
	 *            Fuente de datos
	...
	public Grid(BufferFactory bufferFactory, int[] bands)
		...
		double cellSize[] = calcCellSize(bufferFactory.getDataSource(),
				bufferFactory.getSourceWidth(), bufferFactory.getSourceHeight());
		windowExtent = layerExtent = new GridExtent(bufferFactory
				.getDataSource().getExtent(), cellSize[0], cellSize[1]);
		if (bufferFactory.getDataSource() != null
				&& bufferFactory.getDataSource().getDataType() != null)
			dataType = bufferFactory.getDataSource().getDataType()[0];
		this.bands = bands;
	 ...
	 * datos pero se ignorará si tiene el buffer lleno. La instanciación de
	 * GridNotInterpolated hará que se haga la petición para cargar raster
	 * completo. El buffer se cargará con todas las bandas disponibles.
	 * 
	 * @param bufferFactory
	 *            Fuente de datos
	 */
	public Grid(BufferFactory bufferFactory)
			throws RasterBufferInvalidException {
		// La petición es del raster completo
		double cellSize[] = calcCellSize(bufferFactory.getDataSource(),
				bufferFactory.getSourceWidth(), bufferFactory.getSourceHeight());
	...
	/**
	 * Calcula el tamaño de celda a partir de un dataset, un ancho y un alto en
	 * pixeles
	 * 
	 * @return
	 */
	...
	/**
	 * Selecciona la forma de obtener datos con o sin interpolación. Si pasamos
	 * el parámetro false al aplicar un método de consulta de datos este será
	 * aplicado sin interpolación.
	 * 
	 * @param interpolation
	 */
	public void switchToInterpolationMethod(boolean interpolation) {
		// GridExtent layer = new GridExtent(0, 0, rasterBuf.getWidth(),
		// rasterBuf.getHeight(), 1);
		if (interpolation)
			reader = new GridInterpolated(rasterBuf, layerExtent, windowExtent,
					bands);
		else
	...
	private void init(BufferFactory ds) {
		...
		if (ds == null)
			...
			Transparency transp = ds.getDataSource()
			...
			for (int iPal = 0; iPal < ds.getColorTables().length; iPal++) {
				if (ds.getColorTables()[iPal] != null)
					this.palette[iPal] = new GridPalette(
							ds.getColorTables()[iPal]);
			}
		}
		filterList = new RasterFilterList();
		if (ds != null) {
			filterList.addEnvParam("IStatistics", ds.getDataSource()
					.getStatistics());
		...
		for (i = 0; i < 8; i++) {
			m_dDist[i] = Math.sqrt(m_iOffsetX[i] * dCellSize * m_iOffsetX[i]
					* dCellSize + m_iOffsetY[i] * dCellSize * m_iOffsetY[i]
					* dCellSize);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.grid.IWritableGrid#assign(org.gvsig.fmap.dataaccess.
	 * BufferFactory)
	 */
	public void assign(BufferFactory bufferFactory) throws GridException {
		Grid window;
		try {
			window = new Grid(bufferFactory.getDataSource(), bands,
					windowExtent);
			write(window);
	...
	/**
	 * Sobreescribe los datos del grid actual con los datos del grid pasado por
	 * parámetro
	 * 
	 * @param g
	 *            Grid desde donde se obtienen los datos
	 */
	...
	public void assignNoData() throws InterruptedException {
		try {
			...
			// No hacemos nada. El tipo de dato al que se accede no es
			// controlado por el usuario por lo
					...
					for (int x = 0; x < g.getNX(); x++) {
						for (int y = 0; y < g.getNY(); y++) {
							writer.setCellValue(
									x,
									y,
									reader.getCellValueAsByte(x, y)
											+ g.getCellValueAsByte(x, y));
						}
					}
					break;
				case IBuffer.TYPE_SHORT:
					for (int x = 0; x < g.getNX(); x++) {
						for (int y = 0; y < g.getNY(); y++) {
							writer.setCellValue(
									x,
									y,
									reader.getCellValueAsShort(x, y)
											+ g.getCellValueAsShort(x, y));
						}
					}
					break;
				case IBuffer.TYPE_INT:
					for (int x = 0; x < g.getNX(); x++) {
						for (int y = 0; y < g.getNY(); y++) {
							writer.setCellValue(
									x,
									y,
									reader.getCellValueAsInt(x, y)
											+ g.getCellValueAsInt(x, y));
						}
					}
					break;
				case IBuffer.TYPE_FLOAT:
					for (int x = 0; x < g.getNX(); x++) {
						for (int y = 0; y < g.getNY(); y++) {
							writer.setCellValue(
									x,
									y,
									reader.getCellValueAsFloat(x, y)
											+ g.getCellValueAsFloat(x, y));
						}
					}
					break;
				case IBuffer.TYPE_DOUBLE:
					for (int x = 0; x < g.getNX(); x++) {
						for (int y = 0; y < g.getNY(); y++) {
							writer.setCellValue(
									x,
									y,
									reader.getCellValueAsDouble(x, y)
											+ g.getCellValueAsDouble(x, y));
						}
					}
					break;
				}
	...
	public void addToCellValue(int x, int y, byte value) throws GridException,
		...
		try {
			writer.setCellValue(x, y,
					(byte) (reader.getCellValueAsByte(x, y) + value));
		} catch (OutOfGridException e) {
			throw new GridException("");
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("");
	...
	public void addToCellValue(int x, int y, short value) throws GridException,
		...
		try {
			writer.setCellValue(x, y,
					(short) (reader.getCellValueAsShort(x, y) + value));
		} catch (OutOfGridException e) {
			throw new GridException("");
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("");
	...
	public void addToCellValue(int x, int y, int value) throws GridException,
		...
		try {
			writer.setCellValue(x, y,
					(int) (reader.getCellValueAsInt(x, y) + value));
		} catch (OutOfGridException e) {
			throw new GridException("");
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("");
	...
	public void addToCellValue(int x, int y, float value) throws GridException,
		...
		try {
			writer.setCellValue(x, y,
					(float) (reader.getCellValueAsFloat(x, y) + value));
		} catch (OutOfGridException e) {
			throw new GridException("");
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("");
	...
	public void addToCellValue(int x, int y, double value)
		...
		try {
			writer.setCellValue(x, y,
					(double) (reader.getCellValueAsDouble(x, y) + value));
		} catch (OutOfGridException e) {
			throw new GridException("");
		} catch (RasterBufferInvalidAccessException e) {
			throw new GridException("");
	...
	private boolean getSubMatrix3x3(int x, int y, double subMatrix[])
		...
		if (isNoDataValue(z)) {
			...
			for (i = 0; i < 4; i++) {
				iDir = 2 * i;
				z2 = getCellValueAsDouble(x + m_iOffsetX[iDir], y
						+ m_iOffsetY[iDir]);
				if (!isNoDataValue(z2)) {
					subMatrix[i] = z2 - z;
				} else {
					z2 = getCellValueAsDouble(x + m_iOffsetX[(iDir + 4) % 8], y
							+ m_iOffsetY[(iDir + 4) % 8]);
					if (!isNoDataValue(z2)) {
						subMatrix[i] = z - z2;
					} else {
		...
		try {
			if (getSubMatrix3x3(x, y, zm)) {
				...
				if (G != 0.0)
					...
					dAspect = H > 0.0 ? DEG_270_IN_RAD
							: (H < 0.0 ? DEG_90_IN_RAD : -1.0);
				return dAspect;
			} else
				return rasterBuf.getNoDataValue();
		} catch (GridException e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.grid.IQueryableGrid#getDirToNextDownslopeCell(int,
	 * int)
	 */
	...
	public int getDirToNextDownslopeCell(int x, int y,
			boolean bForceDirToNoDataCell) throws GridException,
			InterruptedException {
	...
	/**
	 * Obtiene la paleta para el caso de un MDT. Esta función evita el tener que
	 * obtener un array de paletas y buscar en el cuando se trata de un caso
	 * simple de MDT de una sola banda. Comprueba que se trata de un raster
	 * monobanda con paleta asociada antes de devolverla.
	 * 
	 * @return GridPalette asociada al Grid
	 */
	public GridPalette getMDTPalette() {
		if (rasterBuf != null && rasterBuf.getBandCount() == 1
				&& palette != null)
			return palette[0];
		return null;
	}
	...
	/**
	 * Obtiene el número de bandas del grid o 0 si no tiene buffer de datos
	 * asociado.
	 * 
	...
	/**
	 * Asigna la banda sobre la que se realizan las operaciones. Por defecto es
	 * la banda 0 con lo que para el uso de MDTs no habrá que modificar este
	 * valor.
	 * 
	 * @param band
	 *            Banda sobre la que se realizan las operaciones.
	 */
