...
/**
 * Clase principal de la libreria. En ella se definen variables globales con
 * información de uso general, así como acciones a realizar al arracar la
 * librería. El método que contiene las acciones de arranque es wakeUp. Las
 * tareas principales de este método son de registro de drivers de lectura y
 * escritura y eliminación del directorio de temporales.
 * 
...
public class RasterLibrary {
	...
	/**
	 * En la generación automática de clases esta variable representa el número
	 * de clases en las que se hace la división.
	 */
	public static int defaultNumberOfClasses = 64;

	/**
	 * En la genración de las paletas de color, esta variable representa el
	 * número de colores en las que se hace la división para generar la paleta
	 * nueva. Con esto conseguimos velocidad de procesamiento. Cuanto menor sea,
	 * peor será la calidad representada de la imagen.
	 */
	public static int defaultNumberOfColors = 256;
	/**
	 * Tamaño de bloque en los procesos que recorren un raster completo a base
	 * de ventanas con recorrido descendente. Esta variable indica la altura de
	 * dicho bloque. Por lo tanto cada bloque será de raster.width X
	 * blockHeight. Tipicamente recorridos de este tipo se usan para el calculo
	 * de estadísticas, histogramas, salvado a raster, etc... Es importante para
	 * el buen funcionamiento que este bloque sea potencia de dos.
	 */
	public static int blockHeight = 512;

	// *************CACHE*******************
	/**
	 * Tamaño aproximado de caché en Megas. Si este valor es alto cabrán muchas
	 * páginas en memoria a la vez y si es bajo cabrán pocas. Hay que tener en
	 * cuenta que al instanciar se convertira en bytes para su mejor
	 * tratamiento. Al llamar al constructor esta variable contendrá el tamaño
	 * exacto de la cache en bytes. El tamaño aquí especificado es aproximado.
	 * Este variará dependiendo de los parámetros del raster a cachear ya que
	 * las páginas deben tener una altura potencia de 2.
	 */
	public static long cacheSize = 25;
	/**
	 * Tamaño máximo de la página en Megas. Hay que tener en cuenta que al
	 * instanciar se convertira en bytes para su mejor tratamiento. Al llamar al
	 * constructor esta variable contendrá el tamaño exacto de la página en
	 * bytes
	 */
	...
	/**
	 * Directorio temporal para la caché. Si gastamos el mismo que andami este
	 * se ocupará de gestionar su destrucción al cerrar gvSIG.
	 */
	public static String tempCacheDirectoryPath = System
			.getProperty("java.io.tmpdir") + File.separator + "tmp-andami";
	/**
	 * Ruta o rutas donde busca jars con clases que incorporen elementos nuevos
	 * que extiendan otros ya existentes. Estos pueden ser drivers o filtros.
	 */
	public static String[] pathExtensions = { "." + File.separator };

	/**
	 * Valor noData por defecto para la librería. En caso de no tener un valor
	 * asociado al raster se usará este.
	 */
	...
	public static void wakeUp() {
		...
		ExtensionPoint.getExtensionPoint("RasterReader").setDescription(
		...
		ExtensionPoint.getExtensionPoint("RasterWriter").setDescription(
				"Raster Writer Classes");
		...
		ExtensionPoint.getExtensionPoint("RasterFilter").setDescription(
				"Raster Filter Classes");

		// Invoca las llamadas estáticas de cada clase para registrarlas en los
		// puntos de extensión
		...
		ExtensionPoint.getExtensionPoint("Serializer").setDescription(
				"Raster Serializer Classes");
	 ...
	 * automática. Cuando alguien crea una capa nueva, si esta no tiene nombre
	 * especifico, obtiene su nombre mediante este método. La siguiente vez que
	 * se llame dará un nombre distinto. El nombre de la capa será NewLayer_
	 * seguido de un contador de actualización automática cada vez que se usa.
	 * 
	...
	public static String usesOnlyLayerName() {
		...
		for (int i = 0; i < propetiesListeners.size(); i++) {
			if (propetiesListeners.get(i) instanceof PropertyListener)
				((PropertyListener) propetiesListeners.get(i))
						.actionValueChanged(new PropertyEvent(oldValue,
								"NewLayer", newValue, oldValue));
		}
		return newValue;
	}
	 ...
	 * Es solo para consulta. La siguiente vez que se llama a getOnlyLayerName o
	 * usesOnlyLayerName devolverá el mismo nomnbre.
	 * 
	 * @return Nombre único para la capa.
	 */
	...
	/**
	 * Esta función crea el directorio para temporales y devuelve la ruta de
	 * este
	 * 
	 * @return
	 */
	 ...
	 * 
	 * //Cargamos sobre jarList todos los File correspondientes a los jar
	 * contenidos en pathExtensions File[] jarList = null; for (int iPath = 0;
	 * iPath < pathExtensions.length; iPath++) { File directory = new
	 * File(pathExtensions[iPath]); if (directory.isDirectory() &&
	 * directory.canRead()) { jarList = directory.listFiles(new FileFilter() {
	 * public boolean accept(File pathname) { return
	 * (pathname.getName().toUpperCase().endsWith(".JAR")); } }); } }
	 * 
	 * //Creamos las URL URL[] urls = new URL[jarList.length];
	 * 
	 * for (int j = 0; j < jarList.length; j++) { try { urls[j] = new
	 * URL("file:" + jarList[j]); } catch (MalformedURLException e) {
	 * Logger.getLogger
	 * (RasterLibrary.class.getName()).debug("Error formando la URL, jar incorrecto"
	 * , e); } }
	 * 
	 * //Comprobamos que no haya clases repetidas ZipFile[] jarFiles = new
	 * ZipFile[jarList.length]; for (int i = 0; i < jarList.length; i++) { try {
	 * jarFiles[i] = new ZipFile(jarList[i].getPath());
	 * 
	 * Enumeration entradas = jarFiles[i].entries();
	 * 
	 * while (entradas.hasMoreElements()) { ZipEntry file = (ZipEntry)
	 * entradas.nextElement(); String fileName = file.getName();
	 * 
	 ...
	 * '.');
	 * 
	 * if (clasesJar.get(fileName) != null) { throw new
	 * JarException("CLASES REPETIDAS: " + fileName + " " + " en " +
	 * jarFiles[i].getName() + " y en " + ((ZipFile)
	 * clasesJar.get(fileName)).getName()); }
	 * 
	 * clasesJar.put(fileName, jarFiles[i]); } } catch (ZipException e) { throw
	 * new IOException(" Jar: " + jarList[i].getPath() + ": " + jarFiles[i]); }
