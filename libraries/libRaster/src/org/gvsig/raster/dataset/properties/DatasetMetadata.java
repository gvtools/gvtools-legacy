 ...
 * 
 * NODATA_VALUES=255 255 255 1 AREA_OR_POINT=Point 4 TIFFTAG_XRESOLUTION=72 4
 * TIFFTAG_YRESOLUTION=72 4 TIFFTAG_RESOLUTIONUNIT=2 (pixels/inch) 4
 * 
 * STATISTICS_MINIMUM=0 STATISTICS_MAXIMUM=221 STATISTICS_MEAN=35.910196078431
 * STATISTICS_MEDIAN=30 STATISTICS_MODE=0 STATISTICS_STDDEV=29.609849452294
 * STATISTICS_HISTONUMBINS=256 STATISTICS_HISTOMIN=0 STATISTICS_HISTOMAX=255
 * LAYER_TYPE=athematic STATISTICS_HISTOBINVALUES=30285675|0|0|22050| ...
 * 
...
public class DatasetMetadata {
	...
	public DatasetMetadata(String[] metadata,
			DatasetColorInterpretation colorInterpr) {
	...
	public TransparencyRange parserNodataByBand() {
		...
		// Esta comprobación es para los raster con paleta (gif). Cuando se
		// trate la paleta y no se usen como
		// imagenes de una banda habrá que cambiar esto
		if ((colorInterpr.length() == 1)
				&& colorInterpr.get(0).equals("Palette"))
			return null;

		if (noDataByBand == null)
			return null;

		// Si todos los valores nodata por banda son -1 es que no hay ninguno
		// asignado
		int count = 0;
	...
	/**
	 * Parsea los metadatos NODATA_VALUES si existe alguno y devuelve un objeto
	 * TransparencyRange.
	 * 
	 * @param nodata
	 * @return Vector de enteros con los valores RGBA o null si no tiene este
	 *         metadato o no es parseable en este formato.
	 */
