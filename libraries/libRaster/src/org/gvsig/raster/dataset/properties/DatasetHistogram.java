...
/**
 * Clase para la gestión de histogramas de un raster. Es la encargada del
 * calculo de un histograma total o parcial de un raster a partir de los datos
 * de disco. Además también es la encargada de gestionar salvar este histograma
 * a .rmf. En caso de que solicite un histograma del raster completo este irá a
 * buscarlo al fichero rmf asociado antes de calcularlo por si ya existise. Al
 * realizar el calculo del histograma de la imagen completa este será salvado en
 * el fichero .rmf asociado al raster.
 * 
...
public class DatasetHistogram {
	...
	public Histogram getHistogram() throws FileNotOpenException,
		...
		try {
			if (dataset != null) {
				...
				try {
					histogram = (Histogram) dataset.loadObjectFromRmf(
				...
				histogram = new Histogram(dataset.getBandCount(), dataset
						.getStatistics().getMin(), dataset.getStatistics()
						.getMax(), dataset.getDataType()[0]);
			...
			// La vista se selecciona automáticamente no debería darse esta
			// excepción
		}
		return null;
	}
	 ...
	 */
	private Histogram getHistogramByDataType() throws InvalidSetViewException,
			FileNotOpenException, RasterDriverException, InterruptedException {
		RasterTask task = RasterTaskQueue
				.get(Thread.currentThread().toString());
		percent = 0;
				...
				for (int iBand = 0; iBand < dataset.getBandCount(); iBand++)
					for (int col = 0; col < dataset.getWidth(); col++)
						for (int row = 0; row < hB; row++)
							histogram.incrementPxValue(iBand,
									(double) bBlock[iBand][row][col]);
				break;
