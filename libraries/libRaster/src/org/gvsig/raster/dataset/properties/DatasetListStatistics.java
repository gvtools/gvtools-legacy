...
public class DatasetListStatistics extends DatasetStatistics {
	 ...
	 */
	public static DatasetListStatistics loadDatasetListStatistics(
			IRasterDataSource datasource) throws RmfSerializerException {
		DatasetStatistics[] stats = new DatasetStatistics[datasource
				.getDatasetCount()];
		for (int i = 0; i < datasource.getDatasetCount(); i++) {
			DatasetStatistics statFile = ((RasterDataset) datasource
					.getDataset(i)[0]).getStatistics();
			stats[i] = (DatasetStatistics) ((RasterDataset) datasource
					.getDataset(i)[0]).loadObjectFromRmf(
					DatasetStatistics.class, statFile);

			// Con que un dataset no tenga la estadistica calculada se pone a no
			// calculado el DatasetListStatistics
	...
	public void calcFullStatistics() throws FileNotOpenException,
		...
		try {
			...
			throw new RasterDriverException(
					"Error en el acceso al array de máximos y mínimos");
		}

		calculated = true;
		// t2 = new Date().getTime();
		// System.out.println("Estadisticas MultiFile: " + ((t2 - t1) / 1000D) +
		// ", secs.");
	}
	...
	/**
	 * Construye todos los datos del DatasetStadistics actual a partir de toda
	 * la lista de DatasetStatistics que tiene este objeto.
	 * 
	 * @param len
	...
	private void constructStats(int len) throws ArrayIndexOutOfBoundsException {
		...
		for (int i = 0; i < statList.length; i++) {
			...
			for (int j = 0; j < statList[i].getMax().length; j++) {
				...
				secondMaxByteUnsigned[count] = statList[i]
						.getSecondMaxByteUnsigned()[j];
				secondMinByteUnsigned[count] = statList[i]
						.getSecondMinByteUnsigned()[j];
