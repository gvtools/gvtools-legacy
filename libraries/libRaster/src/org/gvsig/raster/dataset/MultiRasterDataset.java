...
public class MultiRasterDataset implements IRasterDataSource {
	...
	// TODO: FUNCIONALIDAD: Contructores igual a RasterDataset + String[]
	// nameFiles
	public MultiRasterDataset() {
	}

	/**
	 * Crea un objeto MultiRasterDataset nuevo con los mismos ficheros que el
	 * actual.
	 * 
	...
	public IRasterDataSource newDataset() {
		try {
			...
			MultiRasterDataset multiRasterDataset = MultiRasterDataset.open(
					getDataset(0)[0].getCrs(), fileList[0]);
	 ...
	 * identificador del dataset. Este objeto puede ser una ruta a un fichero en
	 * disco. En este caso la extensión del fichero servirá para buscar el
	 * driver que lo gestiona. Si proporcionamos un array de cadenas se tratarán
	 * como la ruta a N ficheros de disco. También puede ser un buffer de datos
	 * en memoria o cualquier otro objeto que pueda aceptar un driver.
	 * 
	 * @param crs
	 *            PRoyección
	 * @param datasetOpenParam
	 *            Parámetros al driver
	...
	public static MultiRasterDataset open(CoordinateReferenceSystem crs,
		...
		if (datasetOpenParam instanceof String[]) {
			...
			for (int dataset = 0; dataset < param.length; dataset++)
				try {
					rmd.addDataset(new RasterDataset[] { RasterDataset.open(
							crs, param[dataset]) });
				} catch (FileNotFoundInListException e) {
	...
	/**
	 * Añade un fichero a la lista que componen el multi raster. El array solo
	 * debe tener un elemento ya que solo tiene sentido para este caso con un
	 * elemento. ¡Ojo!, añadir más elementos al array no hace que se añadan
	 * varios datasets a la lista.
	 * 
	 * @param f
	 *            Añade el elemento 0 a la lista.
	 */
	public void addDataset(RasterDataset[] f)
			throws FileNotFoundInListException {
		if (f.length != 1)
			throw new FileNotFoundInListException("Error in list.");
		if (findDataset(f[0]))
			throw new FileNotFoundInListException(
					"The file already is in list.");
		files.add(f[0]);
		addBands(f[0]);
		if (stats == null)
	...
	/**
	 * Añade un fichero a la lista que componen el multi raster a partir de su
	 * nombre. El array solo debe tener un elemento ya que solo tiene sentido
	 * para este caso con un elemento. ¡Ojo!, añadir más elementos al array no
	 * hace que se añadan varios datasets a la lista.
	 * 
	 * @param f
	 *            Añade el elemento 0 a la lista.
	 */
	...
	/**
	 * Añade el fichero a lista de georrasterfiles y sus bandas a la lista de
	 * bandas
	 * 
	 * @param grf
	 */
	...
	public void removeDataset(RasterDataset file) {
		for (int i = 0; i < files.size(); i++) {
			if (((RasterDataset) files.get(i)).getFName().equals(
					file.getFName())) {
				files.remove(i);
	 ...
	 * @param file
	 *            Fichero búscado.
	 * @return true si se ha hallado el fichero y false si no se ha encontrado
	 */
	 ...
	 * 
	 * @param i
	 *            Para un MultiRasterDataset el parámetro es ignorado
	 * @param j
	 *            Para un MultiRasterDataset el parámetro es ignorado
	...
	private void initBufferToNoData(IBuffer raster, BandList bandList)
		...
		for (int i = 0; i < bandList.getDrawableBandsCount(); i++) {
			...
			case IBuffer.TYPE_BYTE:
				raster.assign(i, raster.getByteNoDataValue());
				break;
			case IBuffer.TYPE_SHORT:
				raster.assign(i, raster.getShortNoDataValue());
				break;
			case IBuffer.TYPE_INT:
				raster.assign(i, raster.getIntNoDataValue());
				break;
			case IBuffer.TYPE_FLOAT:
				raster.assign(i, raster.getFloatNoDataValue());
				break;
			case IBuffer.TYPE_DOUBLE:
				raster.assign(i, raster.getNoDataValue());
				break;
			}
		}
	}
	...
	/**
	 * A partir de la lista de bandas que dice como cargar el buffer se crean
	 * tantos IBuffer como ficheros intervienen . Cada IBuffer corresponde a un
	 * dataset del RasterMultiDataset y en ellos se reserva memoria solo para
	 * las bandas que vayan a ser cargadas. Las otras se asignaran a la banda
	 * NotValid.
	 * 
	 * @param bl
	 *            Lista de bandas
	...
	private IBuffer[] mallocBuffersDatasets(BandList bl, int width, int height) {
		...
		for (int i = 0; i < getDatasetCount(); i++) {
			if (forceToMemory)
				buffers[i] = RasterBuffer.getMemoryBuffer(
						getDataset(i)[0].getDataType()[0], width, height,
						getDataset(i)[0].getBandCount(), false);
			else
				buffers[i] = RasterBuffer.getBuffer(
						getDataset(i)[0].getDataType()[0], width, height,
						getDataset(i)[0].getBandCount(), false);

			// Asignamos las bandas de cada fichero que no se pintan a null y
			// las que se pintan se reserva memoria
			...
			for (int j = 0; j < getDataset(i)[0].getBandCount(); j++) {
				if (bl.getBufferBandToDraw(name, j) == null)
					...
					buffers[i].mallocOneBand(getDataset(i)[0].getDataType()[0],
							width, height, j);
			}
		}
		return buffers;
	...
	/**
	 * Mezcla los buffers de los dataset que forman el RasterMultiDataset sobre
	 * un solo buffer con las directrices que marca la lista de bandas. Esta
	 * función es la que realiza el switch de las bandas.
	 * 
	 * @param b
	 *            Buffer sobre el que se mezcla
	 * @param bDataset
	 *            Buffers que corresponden a los datasets
	 * @param bandList
	 *            Objeto que contiene la información de que bandas de los
	 *            dataset se escriben sobre que banda del buffer.
	 * @throws InterruptedException
	 */
	private void mergeBuffers(IBuffer b, IBuffer[] bDataset, BandList bandList)
			throws InterruptedException {
		for (int iDataset = 0; iDataset < getDatasetCount(); iDataset++) { // Ojo!
																			// Los
																			// datasets
																			// están
																			// en
			...
			for (int iBand = 0; iBand < getDataset(iDataset)[0].getBandCount(); iBand++) {
				...
				if (posToDraw != null) {
					for (int i = 0; i < posToDraw.length; i++) {
						...
						case IBuffer.TYPE_BYTE:
							b.assignBand(posToDraw[i],
									bDataset[iDataset].getBand(iBand));
							break;
						case IBuffer.TYPE_SHORT:
							b.assignBand(posToDraw[i],
									bDataset[iDataset].getBand(iBand));
	 ...
	 * No aplica supersampleo ni subsampleo sino que devuelve una matriz de
	 * igual tamaño a los pixeles de disco.
	 * 
	 * @param x
	 *            Posición X superior izquierda
	 * @param y
	 *            Posición Y superior izquierda
	 * @param w
	 *            Ancho en coordenadas reales
	 * @param h
	 *            Alto en coordenadas reales
	 * @param adjustToExtent
	 *            Flag que dice si el extent solicitado debe ajustarse al extent
	 *            del raster o no.
	 * @param bandList
	 * @return Buffer de datos
	 */
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
			double lry) throws InvalidSetViewException, InterruptedException,
			RasterDriverException {

		// Leemos pixels completos aunque el valor obtenido sea decimal. Esto se
		// consigue redondeando
		// por arriba el más alto y por abajo el menor y luego restandolos

		Point2D p1 = ((RasterDataset) files.get(0))
				.worldToRaster(new Point2D.Double(ulx, uly));
		Point2D p2 = ((RasterDataset) files.get(0))
				.worldToRaster(new Point2D.Double(lrx, lry));

		// Para el valor mayor redondeamos por arriba. Para el valor menor
		// redondeamos por abajo.
		double p1X = (p1.getX() > p2.getX()) ? Math.ceil(p1.getX()) : Math
				.floor(p1.getX());
		double p1Y = (p1.getY() > p2.getY()) ? Math.ceil(p1.getY()) : Math
				.floor(p1.getY());
		double p2X = (p2.getX() > p1.getX()) ? Math.ceil(p2.getX()) : Math
				.floor(p2.getX());
		double p2Y = (p2.getY() > p1.getY()) ? Math.ceil(p2.getY()) : Math
				.floor(p2.getY());
		...
		width = (int) Math.abs(p1X - p2X);
		height = (int) Math.abs(p1Y - p2Y);
		...
		if (isReadOnly()) {
			RasterBuffer rb = RasterBuffer.getReadOnlyBuffer(getDataType()[0],
					width, height, getBandCount());
			if (rb instanceof RasterReadOnlyBuffer) {
				try {
					((RasterReadOnlyBuffer) rb).setBufferParams(this,
							(int) p1.getX(), (int) p1.getY(), (int) p2.getX(),
							(int) p2.getY(), bandList);
		...
		if (forceToMemory) // Fuerza siempre buffer en memoria
			raster = RasterBuffer.getMemoryBuffer(getDataType()[0], width,
					height, mallocNBands, false);
		else
			raster = RasterBuffer.getBuffer(getDataType()[0], width, height,
					mallocNBands, false);
		...
		IBuffer[] bufferDatasets = mallocBuffersDatasets(bandList, width,
				height);

		// Si hemos redondeado los pixeles de la petición (p1 y p2) por arriba y
		// por abajo deberemos calcular un extent mayor
		// equivalente a los pixeles redondeados.
		Point2D wc1 = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double(p1X, p1Y));
		Point2D wc2 = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double(p2X, p2Y));

		for (int i = 0; i < getDatasetCount(); i++)
			bufferDatasets[i] = ((RasterDataset) files.get(i)).getWindowRaster(
					wc1.getX(), wc1.getY(), wc2.getX(), wc2.getY(), bandList,
					bufferDatasets[i]);
	...
	public IBuffer getWindowRaster(double ulx, double uly, double w, double h,
			boolean adjustToExtent) throws InvalidSetViewException,
			InterruptedException, RasterDriverException {
		// El incremento o decremento de las X e Y depende de los signos de
		// rotación y escala en la matriz de transformación. Por esto
		// tenemos que averiguar si lrx es x + w o x -w, asi como si lry es y +
		// h o y - h
		Extent ext = getExtent();
		Point2D pInit = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double(0, 0));
		Point2D pEnd = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double((int) getWidth(),
						(int) getHeight()));

		double wRaster = Math.abs(pEnd.getX() - pInit.getX());
		double hRaster = Math.abs(pEnd.getY() - pInit.getY());
		double lrx = (((int) (ext.getULX() - wRaster)) == ((int) ext.getLRX())) ? (ulx - w)
				: (ulx + w);
		double lry = (((int) (ext.getULY() - hRaster)) == ((int) ext.getLRY())) ? (uly - h)
				: (uly + h);
		...
		int height = (int) Math.abs(Math.ceil(p2.getY())
				- Math.floor(p1.getY()));
		...
		if (isReadOnly()) {
			...
			if (rb instanceof RasterReadOnlyBuffer) {
				try {
					((RasterReadOnlyBuffer) rb).setBufferParams(this,
							(int) Math.floor(p1.getX()),
							(int) Math.floor(p1.getY()),
							(int) Math.ceil(p2.getX()),
							(int) Math.ceil(p2.getY()), bandList);
				} catch (FileNotExistsException e) {
					// Esto no debe darse ya que se comprueba al hacer el open.
					return null;
				} catch (NotSupportedExtensionException e) {
		...
		// Si no vamos a ajustar el extent al raster inicializamos el buffer a
		// noData ya que este puede ser
		...
		// equivalente a los pixeles redondeados.
		Point2D wc1 = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double(Math.floor(p1.getX()), Math
						.floor(p1.getY())));
		Point2D wc2 = ((RasterDataset) files.get(0))
				.rasterToWorld(new Point2D.Double(Math.ceil(p2.getX()), Math
						.ceil(p2.getY())));
		for (int i = 0; i < getDatasetCount(); i++)
			bufferDatasets[i] = ((RasterDataset) files.get(i)).getWindowRaster(
					wc1.getX(), wc1.getY(), Math.abs(wc2.getX() - wc1.getX()),
					Math.abs(wc2.getY() - wc1.getY()), bandList,
					bufferDatasets[i], adjustToExtent);
		// bufferDatasets[i] = ((RasterDataset)files.get(i)).getWindowRaster(x,
		// y, w, h, bandList, bufferDatasets[i], adjustToExtent);
	...
	/**
	 * Obtiene una ventana de datos de la imagen a partir de coordenadas reales.
	 * Aplica supersampleo o subsampleo en función del tamaño del buffer. Esta
	 * operación la gestiona el driver.
	 * 
	 * @param minX
	 *            Valor mínimo de la X en coordenadas reales
	 * @param minY
	 *            Valor mínimo de la Y en coordenadas reales
	 * @param maxX
	 *            Valor máximo de la X en coordenadas reales
	 * @param maxY
	 *            Valor máximo de la Y en coordenadas reales
	 * @param bufWidth
	 *            ancho del buffer lde datos
	 * @param bufHeight
	 *            alto del buffer de datos
	 * @param adjustToExtent
	 *            Flag que dice si el extent solicitado debe ajustarse al extent
	 *            del raster o no.
	 * @param bandList
	 ...
	 */
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
			double lry, int bufWidth, int bufHeight, boolean adjustToExtent)
		...
		if (isReadOnly()) {
			RasterBuffer rb = RasterBuffer.getReadOnlyBuffer(getDataType()[0],
					bufWidth, bufHeight, getBandCount());
			if (rb instanceof RasterReadOnlyBuffer) {
				try {
					((RasterReadOnlyBuffer) rb).setBufferParams(this,
							(int) p1.getX(), (int) p1.getY(), (int) p2.getX(),
		...
		IBuffer raster = null;
		if (forceToMemory) // Fuerza siempre buffer en memoria
			raster = RasterBuffer.getMemoryBuffer(getDataType()[0], bufWidth,
					bufHeight, mallocNBands, false);
		else
			raster = RasterBuffer.getBuffer(getDataType()[0], bufWidth,
					bufHeight, mallocNBands, false);
		...
		// Reservamos memoria para los buffers por dataset
		IBuffer[] bufferDatasets = mallocBuffersDatasets(bandList, bufWidth,
				bufHeight);
		for (int i = 0; i < getDatasetCount(); i++)
			bufferDatasets[i] = ((RasterDataset) files.get(i)).getWindowRaster(
					ulx, uly, lrx, lry, bufWidth, bufHeight, bandList,
					bufferDatasets[i], adjustToExtent);
	 ...
	 *            Posición Y superior izquierda
	 * @param w
	 *            Ancho en coordenadas pixel
	 * @param h
	 *            Alto en coordenadas pixel
	 * @param bandList
	 * @return Buffer de datos
	 */
	public IBuffer getWindowRaster(int x, int y, int w, int h)
			throws InvalidSetViewException, InterruptedException,
			RasterDriverException {
		if (x < 0 || y < 0 || w > ((RasterDataset) files.get(0)).getWidth()
				|| h > ((RasterDataset) files.get(0)).getHeight())
		...
		if (isReadOnly()) {
			...
			if (rb instanceof RasterReadOnlyBuffer) {
				try {
					((RasterReadOnlyBuffer) rb).setBufferParams(this, x, y, x
							+ w, y + h, bandList);
				} catch (FileNotExistsException e) {
					// Esto no debe darse ya que se comprueba al hacer el open.
					return null;
		...
		IBuffer raster = null;
		if (forceToMemory) // Fuerza siempre buffer en memoria
			raster = RasterBuffer.getMemoryBuffer(getDataType()[0], w, h,
					bandList.getDrawableBandsCount(), false);
		else
			raster = RasterBuffer.getBuffer(getDataType()[0], w, h,
					bandList.getDrawableBandsCount(), false);
		...
		for (int i = 0; i < getDatasetCount(); i++)
			bufferDatasets[i] = ((RasterDataset) files.get(i)).getWindowRaster(
					x, y, w, h, bandList, bufferDatasets[i]);
	 ...
	 * 
	 * @param x
	 *            Posición X superior izquierda en pixels
	 * @param y
	 *            Posición Y superior izquierda en pixels
	 * @param w
	 *            Ancho en pixels
	 * @param h
	 *            Alto en pixels
	 * @param bufWidth
	 *            ancho del buffer de datos
	 * @param bufHeight
	 *            alto del buffer de datos
	 * @param bandList
	 * @return Buffer de datos
	...
	public IBuffer getWindowRaster(int x, int y, int w, int h, int bufWidth,
		...
		if (forceToMemory) // Fuerza siempre buffer en memoria
			raster = RasterBuffer.getMemoryBuffer(getDataType()[0], bufWidth,
					bufHeight, bandList.getDrawableBandsCount(), false);
		else
			raster = RasterBuffer.getBuffer(getDataType()[0], bufWidth,
					bufHeight, bandList.getDrawableBandsCount(), false);
		...
		for (int i = 0; i < getDatasetCount(); i++)
			bufferDatasets[i] = ((RasterDataset) files.get(i)).getWindowRaster(
					x, y, w, h, bufWidth, bufHeight, bandList,
					bufferDatasets[i]);
	...
	private void adjustPoints(Point2D ul, Point2D lr) {
		...
		lr.setLocation(
				(c > 0.95 || c < 0.05) ? Math.round(lr.getX()) : lr.getX(),
	...
	public double getCellSize() {
		try {
			Extent e = getExtent();
			double dCellsize = (e.getMax().getX() - e.getMin().getX())
					/ getWidth();
			return dCellsize;
	...
	/**
	 * Obtiene fichero de la posición i. En un MultiRasterDataset el array
	 * devuelto será de un solo elemento por lo que solo tendrá sentido la
	 * posición 0.
	 * 
	 * @param i
	 *            Posición del fichero a obtener.
	...
	public double getMinX() {
		...
		for (int i = 0; i < files.size(); i++) {
			double aux = ((RasterDataset) files.get(i)).getExtent().getMin()
					.getX();
			if (aux < minx)
				minx = aux;
		}
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#getTransparencyFilesStatus()
	 */
	public Transparency getTransparencyFilesStatus() {
		if (files.size() <= 0)
			return null;
		Transparency t = ((RasterDataset) files.get(0))
				.getTransparencyDatasetStatus();
		for (int i = 1; i < files.size(); i++) {
			Transparency t1 = ((RasterDataset) files.get(i))
					.getTransparencyDatasetStatus();
			try {
				t.merge(t1);
	...
	/**
	 * Obtiene la paleta correspondiente a uno de los ficheros que forman el
	 * GeoMultiRasterFile
	 * 
	 * @param i
	 *            Posición del raster
	...
	/**
	 * Obtiene la lista de paletas correspondiente a todos los ficheros que
	 * forman el GeoMultiRasterFile
	 * 
	 * @return Paleta asociada a este o null si no tiene. Una posición null en
	 *         el array también indica que para ese fichero no hay paletas
	 *         asociadas.
	 */
	...
	/**
	 * Obtiene la paleta correspondiente al nombre del fichero pasado por
	 * parámetro.
	 * 
	 * @param fileName
	 *            Nombre del fichero
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#rasterToWorld(java.awt.geom
	 * .Point2D)
	 */
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#worldToRaster(java.awt.geom
	 * .Point2D)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.raster.dataset.IRasterDataSource#calcSteps(double, double,
	 * double, double, double, double, int, int)
	 */
	public double[] calcSteps(double dWorldTLX, double dWorldTLY,
			double dWorldBRX, double dWorldBRY, double nWidth, double nHeight,
			int bufWidth, int bufHeight) {
		return ((RasterDataset) files.get(0)).calcSteps(dWorldTLX, dWorldTLY,
				dWorldBRX, dWorldBRY, nWidth, nHeight, bufWidth, bufHeight);
	}
	...
	public Object getData(int x, int y, int band)
		...
		for (int i = 0; i < files.size(); i++) {
			if (((RasterDataset) files.get(i)).getFName().equals(file))
				return ((RasterDataset) files.get(i)).getData(x, y,
						posList[band]);
		}
		return null;
	}
	...
	/**
	 * Obtiene el objeto que contiene la interpretación de color del
	 * MultiRasterDataset
	 * 
	...
	public DatasetColorInterpretation getColorInterpretation() {
		...
		for (int i = 0; i < files.size(); i++) {
			ci.addColorInterpretation(((RasterDataset) files.get(i))
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.driver.datasetproperties.IHistogramable#getHistogram()
	 */
	...
	/**
	 * Obtiene el número de dataset para un número de banda y la posición de la
	 * banda dentro de ese dataset.
	 * 
	 * @param bandNumber
	 *            Número de banda
	 * @return Dataset que corresponde a la banda pedida y número de banda
	 *         dentro del dataset (dataset, banda)
	 */
	private int[] getDatasetFromBandNumber(int bandNumber) {
		...
		for (int i = 0; i < getDatasetCount(); i++) {
			...
			if (cont > bandNumber) {
				int lastBands = (cont - getDataset(i)[0].getBandCount()); // Suma
																			// de
																			// las
																			// bandas
																			// de
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#setAffineTransform(java.awt
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.raster.dataset.IRasterDataSource#saveObjectToRMF(int,
	 * java.lang.Class, java.lang.Object)
	 */
	public void saveObjectToRmf(int file, Class class1, Object value)
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#loadObjectFromRMF(java.lang
	 * .Class, java.lang.Object)
	 */
	public Object loadObjectFromRmf(Class class1, Object value)
			throws RmfSerializerException {
