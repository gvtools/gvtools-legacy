 ...
 * banda del raster.
 * </P>
 * \<ColorInterpretation\> \<BandCount\>3\</BandCount\><BR>
 * \<Band\>Red\</Band\><BR>
 * \<Band\>Green\</Band\><BR>
 * \<Band\>Blue\</Band\><BR>
...
public class ColorInterpretationRmfSerializer extends ClassSerializer {
	...
	/**
	 * Registra ColorInterpretationRmfSerializer en los puntos de extension de
	 * Serializer
	 */
	public static void register() {
		ExtensionPoint point = ExtensionPoint.getExtensionPoint("Serializer");
		point.register("ColorInterpretation",
				ColorInterpretationRmfSerializer.class);
	}
				...
				while (tag != KXmlParser.END_DOCUMENT) {
					...
					case KXmlParser.START_TAG:
						if (parser.getName().compareTo(MAIN_TAG) == 0) {
							int nBands = Integer.valueOf(
									parserString(parser, BANDCOUNT, null))
									.intValue();
