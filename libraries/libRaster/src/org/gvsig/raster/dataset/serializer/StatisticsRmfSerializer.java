...
/**
 * <P>
 * Clase para convertir a XML las estadisticas y obtener las estadísticas desde
 * XML. Esta clase implementa el interfaz IRmfBlock con los métodos de escritura
 * y lectura. Estos serán utilizados por el gestor de ficheros RMF para escribir
 * y leer datos.
 * </P>
 * <P>
...
public class StatisticsRmfSerializer extends ClassSerializer {
	...
	/**
	 * Devuelve el objeto estadisticas de un dataset, en caso de no existir, lo
	 * crea.
	 * 
	 * @return
	 */
	 ...
	 * @throws IOException
	 */
	private long[] parserStatBandValues(KXmlParser parser, int band,
			double[] max, double[] min, double[] sndmax, double[] sndmin,
			double[] maxRGB, double[] minRGB, double[] sndmaxRGB,
			double[] sndminRGB, double[] mean, double[] variance)
					...
					case KXmlParser.START_TAG:
						if (parser.getName() != null) {
							if (parser.getName().compareTo(MAIN_TAG) == 0) {
								bandCount = Integer.parseInt(parserString(
										parser, BANDCOUNT, null));
								if (max == null) {
								...
								for (int i = 0; i < bandCount; i++)
									parserStatBandValues(parser, i, max, min,
											secondMax, secondMin, maxRGB,
											minRGB, secondMaxRGB, secondMinRGB,
											mean, variance);
							}
						}
						break;
	...
	public String write() {
		...
		for (int i = 0; i < getDatasetStatistics().getBandCount(); i++) {
			...
			if (getDatasetStatistics().getSecondMax() != null)
				putProperty(b, SNDMAX,
						getDatasetStatistics().getSecondMax()[i], 2);
			if (getDatasetStatistics().getSecondMin() != null)
				putProperty(b, SNDMIN,
						getDatasetStatistics().getSecondMin()[i], 2);
			if (getDatasetStatistics().getMaxByteUnsigned() != null)
				putProperty(b, MAXRGB, getDatasetStatistics()
						.getMaxByteUnsigned()[i], 2);
			if (getDatasetStatistics().getMinByteUnsigned() != null)
				putProperty(b, MINRGB, getDatasetStatistics()
						.getMinByteUnsigned()[i], 2);
			if (getDatasetStatistics().getSecondMaxByteUnsigned() != null)
				putProperty(b, SNDMAXRGB, getDatasetStatistics()
						.getSecondMaxByteUnsigned()[i], 2);
			if (getDatasetStatistics().getSecondMinByteUnsigned() != null)
				putProperty(b, SNDMINRGB, getDatasetStatistics()
						.getSecondMinByteUnsigned()[i], 2);
			...
			if (getDatasetStatistics().getVariance() != null)
				putProperty(b, VARIANCE,
						getDatasetStatistics().getVariance()[i], 2);
			b.append("\t</" + BAND + ">\n");
		}
		for (int i = 0; i < getDatasetStatistics().getTailTrimCount(); i++) {
			if (getDatasetStatistics().getTailTrimValue(i) != null) {
				b.append("\t<" + TAILTRIM + ">\n");
				putProperty(b, KEY, ((Double) getDatasetStatistics()
						.getTailTrimValue(i)[0]).doubleValue(), 2);
				putProperty(b, VALUE, ((Double) getDatasetStatistics()
						.getTailTrimValue(i)[1]).doubleValue(), 2);
				b.append("\t</" + TAILTRIM + ">\n");
			}
		}
		b.append("</" + MAIN_TAG + ">\n");
