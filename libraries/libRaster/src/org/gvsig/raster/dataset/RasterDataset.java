 ...
 * Actua tambien como una 'Fabrica', ocultando al cliente la manera en que se ha
 * implementado ese manejo. Una clase nueva que soportara un nuevo tipo de
 * raster tendría que registrar su extensión o extensiones usando el método @see
...
public abstract class RasterDataset extends GeoInfo {
	 ...
	 * 
	 * @param crs
	 *            Proyección en la que está el raster.
	 * @param fName
	 *            Nombre del fichero.
	...
	public static RasterDataset open(CoordinateReferenceSystem crs, Object param)
		...
		if (param instanceof String)
			idFormat = RasterUtilities
					.getExtensionFromFileName(((String) param));
		...
		if (param instanceof MemoryRasterDriverParam) {
			...
			if (idFormat != null) {
				ExtensionPoint extensionPoint = ExtensionPoint
						.getExtensionPoint("RasterReader");
		...
		try {
			...
			throw new RasterDriverException(
					"Error NoSuchMethodException in open", e);
		} catch (IllegalArgumentException e) {
			throw new RasterDriverException(
					"Error IllegalArgumentException in open", e);
		} catch (InstantiationException e) {
			throw new RasterDriverException(
					"Error InstantiationException in open", e);
		} catch (IllegalAccessException e) {
			throw new RasterDriverException(
					"Error IllegalAccessException in open", e);
		} catch (InvocationTargetException e) {
			throw new NotSupportedExtensionException(
					"Error in open. Problemas con las librerías.", e);
		}
		return grf;
	}

	/**
	 * Acciones de inicilización comunes a todos los drivers. Este método debe
	 * ser llamado explicitamente por el constructor de cada driver. Estas son
	 * acciones de inicialización que se ejecutan después del constructor de
	 * cada driver. Las acciones que hayan de ser realizadas antes se definen en
	 * el constructor de RasterDataset.
	 */
	protected void init() {
	}
	...
	/**
	 * Tipo de fichero soportado. Devuelve true si el tipo de fichero
	 * (extension) está soportado, si no devuelve false.
	 * 
	 * @param fName
	 *            Fichero raster
	...
	public static boolean fileIsSupported(String fName) {
		ExtensionPoint extensionPoint = ExtensionPoint
				.getExtensionPoint("RasterReader");
		return extensionPoint.existKey(RasterUtilities
				.getExtensionFromFileName(fName));
	}
	...
	/**
	 * Carga un fichero raster. Puede usarse para calcular el extent e
	 ...
	 */
	protected void loadFromRmf(RmfBlocksManager manager)
			throws ParsingException {
		if (!manager.checkRmf())
			return;
		...
		NoDataRmfSerializer noDataSerializer = new NoDataRmfSerializer(
				new NoData(noData, RasterLibrary.NODATATYPE_LAYER));
		...
		if (noDataSerializer.getResult() != null) {
			noData = ((org.gvsig.raster.datastruct.NoData) noDataSerializer
					.getResult()).getValue();
		...
		if (colorInterpSerializer.getResult() != null) {
			DatasetColorInterpretation ci = (DatasetColorInterpretation) colorInterpSerializer
					.getResult();
			setColorInterpretation(ci);
			if (ci.getBand(DatasetColorInterpretation.ALPHA_BAND) != -1)
				getTransparencyDatasetStatus().setTransparencyBand(
						ci.getBand(DatasetColorInterpretation.ALPHA_BAND));
		}
	...
	/**
	 * Obtiene el valor del raster en la coordenada que se le pasa. El valor
	 * será Double, Int, Byte, etc. dependiendo del tipo de raster.
	 * 
	 * @param x
	 *            coordenada X
	 * @param y
	 ...
	 * @param lrx
	 *            Posición X inferior derecha
	 * @param lry
	 *            Posición Y inferior derecha
	 * @param rasterBuf
	 *            Buffer de datos
	 * @param bandList
	 * @return Buffer de datos
	 */
	abstract public IBuffer getWindowRaster(double ulx, double uly, double lrx,
	 ...
	 * @return Buffer de datos
	 */
	abstract public IBuffer getWindowRaster(double x, double y, double w,
			double h, BandList bandList, IBuffer rasterBuf,
			boolean adjustToExtent) throws InterruptedException,
			RasterDriverException;

	/**
	 * Obtiene una ventana de datos de la imagen a partir de coordenadas reales.
	 * Se aplica supersampleo o subsampleo dependiendo del tamaño del buffer
	 * especificado.
	 * 
	 * @param minX
	 *            Posición mínima X superior izquierda
	 * @param minY
	 *            Posición mínima Y superior izquierda
	 * @param maxX
	 *            Posición máxima X inferior derecha
	 * @param maxY
	 *            Posición máxima Y inferior derecha
	 * @param bufWidth
	 ...
	 * @return Buffer de datos
	 */
	abstract public IBuffer getWindowRaster(double minX, double minY,
			double maxX, double maxY, int bufWidth, int bufHeight,
	 ...
	 * @return Buffer de datos
	 */
	abstract public IBuffer getWindowRaster(int x, int y, int w, int h,
	 ...
	 */
	abstract public IBuffer getWindowRaster(int x, int y, int w, int h,
			int bufWidth, int bufHeight, BandList bandList, IBuffer rasterBuf)
			throws InterruptedException, RasterDriverException;

	abstract public int getBlockSize();

	/**
	 * Obtiene el objeto que contiene los metadatos. Este método debe ser
	 * redefinido por los drivers si necesitan devolver metadatos.
	 * 
	 * @return
	 */
	 ...
	 * RMF se devuelve este, sino se buscará en la cabecera del raster o
	 * metadatos de este. Si finalmente no encuentra ninguno se devuelve el
	 * valor por defecto definido en la libreria.
	 * 
	 * @return
	 */
	...
	/**
	 * Pone el valor original de noData. Primero lo consulta del valor del
	 * metadata y luego del RMF.
	 */
	public void resetNoDataValue() {
		/**
		 * Intentamos recuperar el valor del metadatas, en caso de no
		 * encontrarlo le asignamos el noData por defecto
		 */
		noDataEnabled = false;
		do {
			if (getMetadata() == null) {
	...
	/**
	 * Obtiene el objeto paleta. Esta paleta es la que tiene adjunta el fichero
	 * de disco. Si es null este objeto quiere decir que no tiene paleta para su
	 * visualización.
	 * 
	 * @return Palette
	 */
	...
	/**
	 * Define el objeto paleta. Si se define null quiere decir que no tiene
	 * paleta para su visualización.
	 * 
	 * @param value
	 */
	 ...
	 * Si el buffer es de mayor tamaño que el raster (supersampleo) quiere decir
	 * que por cada pixel de buffer se repiten varios del raster. Esta función
	 * calcula el número de pixels de desplazamiento en X e Y que corresponden
	 * al primer pixel del buffer en la esquina superior izquierda. Esto es
	 * necesario porque la coordenada solicitada es real y puede no caer sobre
	 * un pixel completo. Este calculo es util cuando un cliente quiere
	 * supersamplear sobre un buffer y que no se lo haga el driver
	 * automáticamente.
	 * 
	 * @param dWorldTLX
	...
	public double[] calcSteps(double dWorldTLX, double dWorldTLY,
		...
		int x = (int) ((tl.getX() > br.getX()) ? Math.floor(br.getX()) : Math
				.floor(tl.getX()));
		int y = (int) ((tl.getY() > br.getY()) ? Math.floor(br.getY()) : Math
				.floor(tl.getY()));

		double a = (tl.getX() > br.getX()) ? (Math.abs(br.getX() - x)) : (Math
				.abs(tl.getX() - x));
		double b = (tl.getY() > br.getY()) ? (Math.abs(br.getY() - y)) : (Math
				.abs(tl.getY() - y));
	 ...
	 * @throws InterruptedException
	 */
	abstract public Object readCompleteLine(int line, int band)
			throws InvalidSetViewException, FileNotOpenException,
			RasterDriverException, InterruptedException;

	/**
	 * Lee un bloque completo de datos del raster y devuelve un array
	 * tridimensional del tipo correcto. Esta función es util para una lectura
	 * rapida de todo el fichero sin necesidad de asignar vista.
	 * 
	 * @param pos
	 *            Posición donde se empieza a leer
	 * @param blockHeight
	 *            Altura máxima del bloque leido
	 * @return Object que es un array tridimendional del tipo de datos del
	 *         raster. (Bandas X Filas X Columnas)
	 * @throws InvalidSetViewException
	 * @throws FileNotOpenException
	 * @throws RasterDriverException
	 */
	 ...
	 * @return
	 */
	abstract public int getOverviewCount(int band) throws BandAccessException,
			RasterDriverException;
	 ...
	 * @return
	 */
	abstract public int getOverviewWidth(int band, int overview)
	 ...
	 * @return
	 */
	abstract public int getOverviewHeight(int band, int overview)
			throws BandAccessException, RasterDriverException;
	...
	public RmfBlocksManager getRmfBlocksManager() {
		if (rmfBlocksManager == null) {
			String fileRMF = RasterUtilities
					.getNameWithoutExtension(getFName()) + ".rmf";
	...
	/**
	 * Informa de si el punto en coordenadas del mundo real pasado por parámetro
	 * cae dentro del raster o fuera. Para este calculo cogeremos el punto a
	 * averiguar si está dentro del raster y le aplicaremos la transformación
	 * inversa de la transformación afín aplicada. Una vez hecho esto ya se
	 * puede comprobar si está dentro de los límites del extent del raster.
	 * 
	 * @param p
	 *            Punto a comprobar en coordenadas reales
	 * @return true si el punto está dentro y false si está fuera.
	 */
	public boolean isInside(Point2D p) {
		// Realizamos los calculos solo si el punto está dentro del extent de la
		// imagen rotada, así nos ahorramos los calculos
		...
		try {
			...
			if (pt.getX() >= 0 && pt.getX() < getWidth() && pt.getY() >= 0
					&& pt.getY() < getHeight())
				return true;
		} catch (NoninvertibleTransformException e) {
			return false;
	...
	public boolean isRotated() {
		if (externalTransformation.getShearX() != 0
				|| externalTransformation.getShearY() != 0)
			return true;
		return false;
	}
	...
	/**
	 * Devuelve un serializador que tenga un constructor con la clase pasada por
	 * parametro
	 * 
	 * @throws RmfSerializerException
	 */
	static private Class getSerializerClass(Class class1)
			throws RmfSerializerException {
		ExtensionPoint extensionPoint = ExtensionPoint
				.getExtensionPoint("Serializer");
		...
		while (iterator.hasNext()) {
			...
			if (entry != null) {
				...
				try {
					...
					// Si no encuentra un constructor para esta clase, seguirá
					// buscando en los demás elementos
				}
			}
		}
		throw new RmfSerializerException(
				"No se ha encontrado la clase para el serializador");
	}

	/**
	 * Construye un serializador que soporte la clase class1. El constructor se
	 * invoca con el parametro value.
	 * 
	 * @param class1
	 * @param value
	 * @return
	 * @throws RmfSerializerException
	 */
	static private ClassSerializer getSerializerObject(Class class1,
			Object value) throws RmfSerializerException {
		try {
			Class[] args = { class1 };
			Constructor constructor = getSerializerClass(class1)
					.getConstructor(args);
			Object[] args2 = { value };
			return (ClassSerializer) constructor.newInstance(args2);
		} catch (Exception e) {
			throw new RmfSerializerException(
					"No se ha podido crear el serializador para el Rmf", e);
		}
	}

	/**
	 * Guarda en el RMF que trata el BlocksManager el valor value usando el
	 * serializador que soporta la clase class1.
	 * 
	 * @param blocksManager
	 * @param class1
	 * @param value
	 * @throws RmfSerializerException
	 */
	static private void saveObjectToRmfFile(RmfBlocksManager blocksManager,
			Class class1, Object value) throws RmfSerializerException {
		ClassSerializer serializerObject = getSerializerObject(class1, value);

		if (serializerObject == null)
			throw new RmfSerializerException(
					"No se ha podido encontrar el serializador para el Rmf");

		if (!blocksManager.checkRmf())
			throw new RmfSerializerException(
					"Error al comprobar el fichero Rmf");
		...
		try {
			...
			throw new RmfSerializerException(
					"Error al escribir el fichero Rmf", e);
		}
	 ...
	 * serializador que trata las clases class1.
	 * 
	 ...
	 * @throws RmfSerializerException
	 */
	static public void saveObjectToRmfFile(String file, Class class1,
			Object value) throws RmfSerializerException {
	...
	/**
	 * Guarda en el fichero file (en formato RMF) el objecto value usando el
	 * serializador que trata la misma clase que el objeto value.
	 * 
	 * Si el fichero RMF no existe se crea. Ha de ser un RMF de formato valido.
	 * 
	 * @param file
	 ...
	 * @throws RmfSerializerException
	 */
	static public void saveObjectToRmfFile(String file, Object value)
			throws RmfSerializerException {
		saveObjectToRmfFile(file, value.getClass(), value);
	}

	/**
	 * Guarda en el RMF el objecto actual en caso de que exista un serializador
	 * para el. El tipo del objeto se especifica en el parametro class1. Esto
	 * nos puede permitir poder poner a null un valor y encontrar su
	 * serializador.
	 * 
	 * @param value
	 * @throws RmfSerializerException
	 */
	public void saveObjectToRmf(Class class1, Object value)
			throws RmfSerializerException {
		saveObjectToRmfFile(getRmfBlocksManager(), class1, value);
	}

	/**
	 * Carga un objecto desde un serializador del tipo class1. Usa value para
	 * iniciar dicho serializador
	 * 
	 * @param class1
	 * @param value
	 * @return
	 * @throws RmfSerializerException
	 */
	static private Object loadObjectFromRmfFile(RmfBlocksManager blocksManager,
			Class class1, Object value) throws RmfSerializerException {
		ClassSerializer serializerObject = getSerializerObject(class1, value);
	 ...
	 * @throws RmfSerializerException
	 */
	static public Object loadObjectFromRmfFile(String file, Class class1,
			Object value) throws RmfSerializerException {
		String fileRMF = RasterUtilities.getNameWithoutExtension(file) + ".rmf";
		RmfBlocksManager blocksManager = new RmfBlocksManager(fileRMF);
	...
	/**
	 * Carga un objecto desde un serializador usando el tipo del mismo objeto
	 * pasado por parametro. Usa value para iniciar dicho serializador
	 * 
	 * @param value
	 * @return
	 * @throws RmfSerializerException
