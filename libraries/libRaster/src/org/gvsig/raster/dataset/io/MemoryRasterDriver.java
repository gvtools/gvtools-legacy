...
public class MemoryRasterDriver extends RasterDataset {
	...
	public static void register() {
		...
		point.register(new MemoryRasterDriverParam().getFormatID(),
				MemoryRasterDriver.class);
	}
	 ...
	 */
	public MemoryRasterDriver(CoordinateReferenceSystem crs, Object buf)
			throws NotSupportedExtensionException {
		super(null, null);
		setParam(buf);
		...
		if (extent != null) {
			...
			ownTransformation = new AffineTransform(psX, 0, 0, psY,
					extent.minX(), extent.maxY());
		} else
			ownTransformation = new AffineTransform(1, 0, 0, -1, 0,
					buffer.getHeight());
	...
	/**
	 * Obtiene la orientación de la imagen a partir del signo del tamaño de
	 * pixel para poder asignarlo en el setView. Esto es util para poder conocer
	 * como debe leerse la image, de abajo a arriba, de arriba a abajo, de
	 * izquierda a derecha o de derecha a izquierda. La posición habitual es la
	 * que el pixel size en X es positivo y en Y negativo leyendose en este caso
	 * las X de menor a mayor y las Y de mayor a menor. Los casos posibles son:
	 * <UL>
	 ...
	 */
	/*
	 * private boolean[] getOrientation(){ boolean[] orientation = {true,
	 * false}; return orientation; }
	 */
	...
	/**
	 * Ajusta los puntos pasados por parámetro a los límites del buffer. Es
	 * decir si alguno excede los límites por arriba o por abajo los ajusta.
	 * 
	 * @param begin
	 *            Punto inicial
	 * @param end
	...
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
		...
		case IBuffer.TYPE_BYTE:
			writeByteBuffer(rasterBuf, 1, 1, begin, bandList);
			break;
		case IBuffer.TYPE_SHORT:
			writeShortBuffer(rasterBuf, 1, 1, begin, bandList);
			break;
		case IBuffer.TYPE_INT:
			writeIntBuffer(rasterBuf, 1, 1, begin, bandList);
			break;
		case IBuffer.TYPE_FLOAT:
			writeFloatBuffer(rasterBuf, 1, 1, begin, bandList);
			break;
		case IBuffer.TYPE_DOUBLE:
			writeDoubleBuffer(rasterBuf, 1, 1, begin, bandList);
			break;
		}
		return rasterBuf;
	}
	...
	public IBuffer getWindowRaster(double minX, double minY, double maxX,
		...
		// Relación entre el número de pixels del buffer origen (area
		// seleccionada) y el destino
		...
		switch (buffer.getDataType()) {
		case IBuffer.TYPE_BYTE:
			writeByteBuffer(rasterBuf, stepX, stepY, begin, bandList);
			break;
		case IBuffer.TYPE_SHORT:
			writeShortBuffer(rasterBuf, stepX, stepY, begin, bandList);
			break;
		case IBuffer.TYPE_INT:
			writeIntBuffer(rasterBuf, stepX, stepY, begin, bandList);
			break;
		case IBuffer.TYPE_FLOAT:
			writeFloatBuffer(rasterBuf, stepX, stepY, begin, bandList);
			break;
		case IBuffer.TYPE_DOUBLE:
			writeDoubleBuffer(rasterBuf, stepX, stepY, begin, bandList);
			break;
		}

		/*
		 * int xPx = 0, yPx = 0; for (int iBand = 0; iBand <
		 * rasterBuf.getBandCount(); iBand++) { yPx = 0; for(double row =
		 * begin.getY(); yPx < bufHeight; row += stepY) { xPx = 0; for(double
		 * col = begin.getX(); xPx < bufWidth; col += stepX) {
		 * switch(buffer.getDataType()){ case IBuffer.TYPE_BYTE:
		 * rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemByte((int)row,
		 * (int)col, iBand)); break; case IBuffer.TYPE_SHORT:
		 * rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemShort((int)row,
		 * (int)col, iBand)); break; case IBuffer.TYPE_INT:
		 * rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemInt((int)row,
		 * (int)col, iBand)); break; case IBuffer.TYPE_FLOAT:
		 * rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemFloat((int)row,
		 * (int)col, iBand)); break; case IBuffer.TYPE_DOUBLE:
		 * rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemDouble((int)row,
		 * (int)col, iBand)); break; } xPx ++; } yPx ++; } }
		 */
		return rasterBuf;
	}

	/**
	 * Escribe sobre el buffer pasado por parámetro los valores solicitados,
	 * desde el buffer de la clase. Los valores se solicitan a través de los
	 * parámetros. En ellos se especifica el tamaño del buffer de destino, las
	 * bandas a escribir, el punto inicial en coordenadas pixel (double) y el
	 * incremento.
	 * 
	 * @param rasterBuf
	 *            Buffer donde se escriben los datos
	 * @param stepX
	 *            Incremento en X. Cada vez que se escribe un pixel en X se
	 *            incrementa el contador en stepX pixels. Esto es necesario ya
	 *            que el buffer destino no tiene porque tener el mismo ancho que
	 *            el de origen. Este valor suele ser ancho_buffer_origen /
	 *            ancho_buffer_destino.
	 * @param stepY
	 *            Incremento en Y. Cada vez que se escribe un pixel en Y se
	 *            incrementa el contador en stepY pixels. Esto es necesario ya
	 *            que el buffer destino no tiene porque tener el mismo alto que
	 *            el de origen. Este valor suele ser alto_buffer_origen /
	 *            alto_buffer_destino.
	 * @param begin
	 *            pixel donde se comienza a leer en el buffer de origen. Este
	 *            valor es decimal ya que no tiene porque empezar a leerse al
	 *            principio del pixel. Esto es util cuando se supersamplea.
	 * @throws InterruptedException
	 */
	private void writeByteBuffer(IBuffer rasterBuf, double stepX, double stepY,
			Point2D begin, BandList bandList) throws InterruptedException {
		int xPx = 0, yPx = 0;
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++) {
			int[] drawableBands = bandList.getBufferBandToDraw(this.getFName(),
					iBand);
			...
			for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
				yPx = 0;
				for (double row = begin.getY(); (yPx < rasterBuf.getHeight() && row < buffer
						.getHeight()); row += stepY) {
					xPx = 0;
					for (double col = begin.getX(); (xPx < rasterBuf.getWidth() && col < buffer
							.getWidth()); col += stepX) {
						rasterBuf
								.setElem(yPx, xPx, iBand, buffer.getElemByte(
										(int) row, (int) col, iBand));
						xPx++;
					}
					yPx++;
				}
	 ...
	 * @throws InterruptedException
	 */
	private void writeShortBuffer(IBuffer rasterBuf, double stepX,
			double stepY, Point2D begin, BandList bandList)
			throws InterruptedException {
		int xPx = 0, yPx = 0;
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++) {
			int[] drawableBands = bandList.getBufferBandToDraw(this.getFName(),
			...
			for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
				...
				for (double row = begin.getY(); yPx < rasterBuf.getHeight(); row += stepY) {
					...
					for (double col = begin.getX(); xPx < rasterBuf.getWidth(); col += stepX) {
						rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemShort(
								(int) row, (int) col, iBand));
						xPx++;
					}
					yPx++;
	 ...
	 * @throws InterruptedException
	 */
	private void writeIntBuffer(IBuffer rasterBuf, double stepX, double stepY,
			Point2D begin, BandList bandList) throws InterruptedException {
		int xPx = 0, yPx = 0;
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++) {
			int[] drawableBands = bandList.getBufferBandToDraw(this.getFName(),
			...
			for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
				...
				for (double row = begin.getY(); yPx < rasterBuf.getHeight(); row += stepY) {
					xPx = 0;
					for (double col = begin.getX(); xPx < rasterBuf.getWidth(); col += stepX) {
						rasterBuf.setElem(yPx, xPx, iBand,
								buffer.getElemInt((int) row, (int) col, iBand));
						xPx++;
					}
					yPx++;
				}
	 ...
	 * @throws InterruptedException
	 */
	private void writeFloatBuffer(IBuffer rasterBuf, double stepX,
			double stepY, Point2D begin, BandList bandList)
			throws InterruptedException {
		int xPx = 0, yPx = 0;
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++) {
			...
			for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
				...
				for (double row = begin.getY(); yPx < rasterBuf.getHeight(); row += stepY) {
					xPx = 0;
					for (double col = begin.getX(); xPx < rasterBuf.getWidth(); col += stepX) {
						rasterBuf.setElem(yPx, xPx, iBand, buffer.getElemFloat(
								(int) row, (int) col, iBand));
						xPx++;
					}
					yPx++;
	 ...
	 * @throws InterruptedException
	 */
	private void writeDoubleBuffer(IBuffer rasterBuf, double stepX,
			double stepY, Point2D begin, BandList bandList)
			throws InterruptedException {
		int xPx = 0, yPx = 0;
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++) {
			...
			for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
				...
				for (double row = begin.getY(); yPx < rasterBuf.getHeight(); row += stepY) {
					xPx = 0;
					for (double col = begin.getX(); xPx < rasterBuf.getWidth(); col += stepX) {
						rasterBuf.setElem(yPx, xPx, iBand, buffer
								.getElemDouble((int) row, (int) col, iBand));
						xPx++;
					}
					yPx++;
				}
	...
	public IBuffer getWindowRaster(int x, int y, int w, int h,
		...
		setView(new Extent(RasterUtilities.getMapRectFromPxRect(getExtent()
				.toRectangle2D(), getWidth(), getHeight(),
				...
				for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
					for (int line = y; line < (y + h); line++)
						for (int col = x; col < (x + w); col++)
							rasterBuf.setElem((line - y), (col - x),
									drawableBands[drawBands], buffer
											.getElemByte(line, col,
													drawableBands[drawBands]));
				}
			} else if (buffer.getDataType() == IBuffer.TYPE_SHORT) {
				for (int drawBands = 0; drawBands < drawableBands.length; drawBands++) {
					for (int line = y; line < (y + h); line++)
	...
	public IBuffer getWindowRaster(int x, int y, int w, int h, int bufWidth,
		...
		switch (buffer.getDataType()) {
		case IBuffer.TYPE_BYTE:
			writeByteBuffer(rasterBuf, stepX, stepY, new Point2D.Double(x, y),
					bandList);
			break;
		case IBuffer.TYPE_SHORT:
			writeShortBuffer(rasterBuf, stepX, stepY, new Point2D.Double(x, y),
					bandList);
			break;
		case IBuffer.TYPE_INT:
			writeIntBuffer(rasterBuf, stepX, stepY, new Point2D.Double(x, y),
					bandList);
			break;
		case IBuffer.TYPE_FLOAT:
			writeFloatBuffer(rasterBuf, stepX, stepY, new Point2D.Double(x, y),
					bandList);
			break;
		case IBuffer.TYPE_DOUBLE:
			writeDoubleBuffer(rasterBuf, stepX, stepY,
					new Point2D.Double(x, y), bandList);
			break;
		}
		return rasterBuf;
	}
	...
	public Object readCompleteLine(int line, int band)
		...
		switch (buffer.getDataType()) {
		case IBuffer.TYPE_BYTE:
			return buffer.getLineFromBandByte(line, band);
		case IBuffer.TYPE_SHORT:
			return buffer.getLineFromBandShort(line, band);
		case IBuffer.TYPE_INT:
			return buffer.getLineFromBandInt(line, band);
		case IBuffer.TYPE_FLOAT:
			return buffer.getLineFromBandFloat(line, band);
		case IBuffer.TYPE_DOUBLE:
			return buffer.getLineFromBandDouble(line, band);
		}
		return null;
	}
