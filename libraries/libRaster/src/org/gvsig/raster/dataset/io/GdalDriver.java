...
public class GdalDriver extends RasterDataset {
	...
	public static void register() {
		...
		point.register("pgm", GdalDriver.class); // Ficheros PNM en escala de
													// grises
	 ...
	 */
	public GdalDriver(CoordinateReferenceSystem crs, Object param)
			throws NotSupportedExtensionException {
		super(crs, param);
		try {
			if (param instanceof String) {
				...
				setParam(param);
				// TODO: FUNCIONALIDAD: Formatos gestionados por gdal que no
				// tienen extensión. Estos tendrán un objeto
				// IRegistrableRasterFormat Por ej: Grass
			}
		...
		for (int i = 0; i < dt.length; i++)
			dt[i] = RasterUtilities.getRasterBufTypeFromGdalType(file
					.getDataType()[i]);
		setDataType(dt);
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.RasterDataset#translateFileName(java.lang.String
	 * )
	 */
	...
	public Object readCompleteLine(int line, int band)
		...
		try {
			...
			throw new RasterDriverException(
					"Error reading data from Gdal library");
		}
	}
	...
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
		...
		try {
			file.readWindow(rasterBuf, bandList, viewRequest.getULX(),
					viewRequest.getULY(), viewRequest.getLRX(),
					viewRequest.getLRY(), rasterBuf.getWidth(),
					rasterBuf.getHeight(), true);
		} catch (GdalException e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.GeoRasterFile#getWindowRaster(double, double,
	...
	public IBuffer getWindowRaster(double ulx, double uly, double w, double h,
		...
		try {
			file.readWindow(rasterBuf, bandList, viewRequest.getULX(),
					viewRequest.getULY(), viewRequest.getLRX(),
					viewRequest.getLRY(), rasterBuf.getWidth(),
					rasterBuf.getHeight(), adjustToExtent);
		} catch (GdalException e) {
			throw new RasterDriverException("Error reading data");
		}
	...
	public IBuffer getWindowRaster(double minX, double minY, double maxX,
		...
		Point2D ul = new Point2D.Double(viewRequest.getULX(),
				viewRequest.getULY());
		Point2D lr = new Point2D.Double(viewRequest.getLRX(),
				viewRequest.getLRY());
		ul = worldToRaster(ul);
		lr = worldToRaster(lr);
		...
		try {
			file.readWindow(rasterBuf, bandList, viewRequest.getULX(),
					viewRequest.getULY(), viewRequest.getLRX(),
					viewRequest.getLRY(), width, height, bufWidth, bufHeight,
					adjustToExtent);
		} catch (GdalException e) {
			throw new RasterDriverException("Error reading data");
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.GeoRasterFile#getWindowRaster(int, int, int,
	...
	public IBuffer getWindowRaster(int x, int y, int w, int h, int bufWidth,
		...
		try {
			setView(new Extent(RasterUtilities.getMapRectFromPxRect(getExtent()
					.toRectangle2D(), getWidth(), getHeight(),
					new Rectangle2D.Double(x, y, w, h))));
			file.readWindow(rasterBuf, bandList, x, y, w, h, bufWidth,
					bufHeight);
		} catch (GdalException e) {
			throw new RasterDriverException("Error reading data");
		}
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.RasterDataset#setAffineTransform(java.awt.geom
	 * .AffineTransform)
	 */
	...
	public int getOverviewWidth(int band, int overview)
		...
		try {
			...
			return file.getRasterBand(band + 1).getOverview(overview)
					.getRasterBandXSize();
		} catch (GdalException e) {
			throw new RasterDriverException("");
		}
