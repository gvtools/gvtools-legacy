...
public class MrSidNative extends MrSIDImageReader {
	...
	/**
	 * Último extent de la ventana seleccionada por el usuario. Este extent
	 * corresponde al de la imagen, no al del viewport de la vista.
	 */
	...
	/**
	 * Array de transformaciones afines donde cada elemento es la transformación
	 * correspondiente a una overview
	 */
	...
	private void init(String fName) throws MrSIDException, IOException {
		...
		// Creamos una matriz de transformación por nivel de overview con el
		// factor de escala de esta overview
		...
		for (int i = 1; i < noverviews; i++) {
			...
			// System.out.println("Over:" + i + " zoom:" +
			// LTIUtils.levelToMag(i) + " dims:" + dims[0] + "X" + dims[1]);
			overviewsTransformation[i] = new AffineTransform(scaleFactorX, 0,
					0, scaleFactorY, 0, 0);
		}
	...
	/**
	 * Calcula el overview a usar de la imagen y el viewport a partir del ancho,
	 * alto y coordenadas del mundo real
	 * 
	 * @param dWorldTLX
	...
	public void setView(double ulx, double uly, double lrx, double lry,
		...
		Point2D tl = new Point2D.Double(Math.min(a.getX(), b.getX()), Math.min(
				a.getY(), b.getY()));
		Point2D br = new Point2D.Double(Math.max(a.getX(), b.getX()), Math.max(
				a.getY(), b.getY()));
			...
			if (WITH_OVERVIEWS && ((noverviews - 1) > 0)) {
				for (int i = (noverviews - 1); i > 0; i--) {
					...
					if (dims[0] > (this.getWidth() * viewportScaleX)) {
						...
						tl.setLocation(Math.min(tl.getX(), br.getX()),
								Math.min(tl.getY(), br.getY()));
						overviewsTransformation[i].transform(tl, tl);
						// Hemos asignado currentFull solo para calcular tl en
						// relación al tamaño de la overview
						currentFullWidth = width;
						currentFullHeight = height;
						currentViewX = tl.getX();
						currentViewY = tl.getY();
	...
	public LTISceneBuffer readBuffer(int x, int y, int SceneWidth,
			int SceneHeight) throws MrSIDException {
	 ...
	 * currentView a través de la libreria de MrSid. Esta escena es cargada
	 * sobre un buffer y asignada al parámetro de salida.
	 * 
	 * @param line
	 *            Escena leida
	 * @throws MrSIDException
	 *             Lanzada si ocurre un error en la lectura de la escena
	 */
	public void readScene(int[] line, RasterTask task) throws MrSIDException,
			InterruptedException {
		// Posición de inicio de la escena en entero para la petición a la
		// libreria
		...
		try {
			SceneWidth = (int) Math
					.ceil((((double) currentViewWidth) / viewportScaleX) + 1);
			if (SceneWidth > currentFullWidth)
				SceneWidth = currentFullWidth;
			SceneHeight = (int) Math
					.ceil((((double) currentViewHeight) / viewportScaleY) + 1);
				...
				try {
					...
					try {
						...
						try {
							...
							SceneWidth--;
							try {
								buffer = readBuffer(x, y, SceneWidth,
										SceneHeight);
			...
			if ((dataType == LTIDataType.LTI_DATATYPE_UINT8)
				...
				// Desplazamiento para la X y la Y leidas. Estas tienen efecto
				// cuando un pixel no empieza a visualizarse
				// justo en su esquina superior izquierda y tiene que ser
				// cortado en la visualización.
	...
	public void readWindow(IBuffer buf, BandList bandList, int x, int y, int w,
			int h) throws MrSIDException, InterruptedException {
	 ...
	 * @param sizeX
	 *            Tamaño X de la imagen
	 * @param sizeY
	 *            Tamaño Y de la image
	 * @param band
	 *            Número de bandas
