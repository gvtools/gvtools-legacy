...
public class GdalNative extends Gdal {
	...
	/**
	 * Vectores que contiene los desplazamientos de un pixel cuando hay
	 * supersampling. , es decir el número de pixels de pantalla que tiene un
	 * pixel de imagen. Como todos los pixeles no tienen el mismo ancho y alto
	 * ha de meterse en un array y no puede ser una variable. Además hay que
	 * tener en cuenta que el primer y último pixel son de distinto tamaño que
	 * el resto.
	 */
	...
	private void init(String fName) throws GdalException, IOException {
		...
		if (getPtro() == -1)
			throw new GdalException(
					"Error en la apertura del fichero. El fichero no tiene un formato válido.");
		...
		// Asignamos la interpretación de color leida por gdal a cada banda.
		// Esto
		// nos sirve para saber que banda de la imagen va asignada a cada banda
		// de
		// visualización (ARGB)
		...
		for (int i = 0; i < getRasterCount(); i++) {
			...
			String colorInt = getColorInterpretationName(rb
					.getRasterColorInterpretation());
			metadata.setNoDataValue(
					i,
					(rb.getRasterNoDataValue() == -9999.0) ? RasterLibrary.defaultNoDataValue
							: rb.getRasterNoDataValue());
			...
			if (rb.getRasterColorTable() != null && palette == null) {
				...
				palette.createPaletteFromGdalColorTable(rb
						.getRasterColorTable());
		...
		try {
			...
			ownTransformation = new AffineTransform(trans.adfgeotransform[1],
					trans.adfgeotransform[4], trans.adfgeotransform[2],
					trans.adfgeotransform[5], trans.adfgeotransform[0],
					trans.adfgeotransform[3]);
			...
			// Transformación para ficheros sin georreferenciación. Se invierte
			// la Y
	...
	/**
	 * Obtiene el flag que informa de si el raster tiene valor no data o no en
	 * una banda concreta.
	 * 
	 * @return true si tiene valor no data en esa banda y false si no lo tiene
	 * @param band
	 *            Posición de la banda a consultar (0..n)
	 * @throws GdalException
	 */
	 ...
	 * 
	 * @param dt
	 *            entero que representa el tipo de dato
	 */
	...
	/**
	 * Obtiene un punto 2D con las coordenadas del raster a partir de uno en
	 * coordenadas del punto real. Supone rasters no girados
	 * 
	 * @param pt
	 *            punto en coordenadas del punto real
	 * @return punto en coordenadas del raster
	 */
	public Point2D worldToRasterWithoutRot(Point2D pt) {
		Point2D p = new Point2D.Double();
		AffineTransform at = new AffineTransform(
				externalTransformation.getScaleX(), 0, 0,
				externalTransformation.getScaleY(),
				externalTransformation.getTranslateX(),
				externalTransformation.getTranslateY());
		try {
	...
	/**
	 * Obtiene un punto del raster en coordenadas pixel a partir de un punto en
	 * coordenadas reales.
	 * 
	 * @param pt
	...
	/**
	 * Calcula el overview a usar. Hay que tener en cuenta que tenemos que tener
	 * calculadas las variables viewPortScale, currentFullWidth y
	 * currentFulHeight
	 * 
	 * @param coordenada
	 *            pixel expresada en double que indica la posición superior
	 *            izquierda
	 * @throws GdalException
	 */
	private void calcOverview(Point2D tl, Point2D br) throws GdalException {
		...
		if (gdalBands[0].getOverviewCount() > 0) {
			...
			for (int i = gdalBands[0].getOverviewCount() - 1; i > 0; i--) {
				...
				if (ovb.getRasterBandXSize() > getRasterXSize()
						* viewportScaleX) {
					currentOverview = i;
					viewportScaleX *= ((double) width / (double) ovb
							.getRasterBandXSize());
					viewportScaleY *= ((double) height / (double) ovb
							.getRasterBandYSize());
	...
	public void setView(double dWorldTLX, double dWorldTLY, double dWorldBRX,
			double dWorldBRY, int nWidth, int nHeight) throws GdalException {
		currentFullWidth = width;
		currentFullHeight = height;
		...
		// Para lectura del renderizado (ARGB). readWindow selecciona las bandas
		// que necesita.
		...
		/*
		 * gdalBands[0] = getRasterBand(rBandNr); gdalBands[1] = gdalBands[0];
		 * gdalBands[2] = gdalBands[1];
		 * 
		 * if(getRasterCount() >= 2) { gdalBands[1] = getRasterBand(gBandNr);
		 * gdalBands[2] = gdalBands[1]; } if(this.getRasterCount() >= 3)
		 * gdalBands[2] = getRasterBand(bBandNr); if(colorInterpr.isAlphaBand())
		 * gdalBands[3] = getRasterBand(aBandNr);
		 * 
		 * assignDataTypeFromGdalRasterBands(gdalBands);
		 * 
		 * if (currentOverview > 0) { gdalBands[0] =
		 * gdalBands[0].getOverview(currentOverview); if(getRasterCount() >= 2)
		 * { gdalBands[1] = gdalBands[1].getOverview(currentOverview); }
		 * if(this.getRasterCount() >= 3) gdalBands[2] =
		 * gdalBands[2].getOverview(currentOverview);
		 * if(colorInterpr.isAlphaBand()) gdalBands[3] =
		 * gdalBands[3].getOverview(currentOverview);
		 * 
		 * }
		 */
	}
	...
	/**
	 * Selecciona bandas y overview en el objeto GdalRasterBand[] para el número
	 * de bandas solicitado.
	 * 
	 * @param nbands
	 *            Número de bandas solicitado.
	 * @throws GdalException
	 */
	 ...
	 * 
	 * @param line
	 *            Buffer donde se cargan los datos
	 * @param initOffset
	 *            Desplazamiento inicial desde el margen inzquierdo. Esto es
	 *            necesario para cuando se supersamplea ya que cada pixel de
	 *            imagen ocupa muchos pixeles de pantalla y puede empezar a
	 *            dibujarse por la izquierda a mitad de pixel
	 * @param gdalBuffer
	 *            Buffer con la línea de datos original
	 */
	private void readLine(byte[][] line, double initOffset,
			GdalBuffer[] gdalBuffer) {
		double j = 0D;
		int i = 0;
		for (int iBand = 0; iBand < gdalBuffer.length; iBand++) {
			for (i = 0, j = initOffset; i < currentViewWidth
					&& j < gdalBuffer[0].getSize(); i++, j += stepX) {
	 ...
	 *            Buffer con la línea de datos original
	 */
	private void readLine(short[][] line, double initOffset,
			GdalBuffer[] gdalBuffer) {
		double j = 0D;
		int i = 0;
		for (int iBand = 0; iBand < gdalBuffer.length; iBand++) {
	 ...
	 *            Buffer con la línea de datos original
	 */
	private void readLine(int[][] line, double initOffset,
			GdalBuffer[] gdalBuffer) {
		double j = 0D;
		int i = 0;
		for (int iBand = 0; iBand < gdalBuffer.length; iBand++) {
	 ...
	 *            Buffer donde se cargan los datos
	 * @param initOffset
	 *            Desplazamiento inicial desde el margen izquierdo. Esto es
	 *            necesario para cuando se supersamplea ya que cada pixel de
	 *            imagen ocupa muchos pixeles de pantalla y puede empezar a
	 *            dibujarse por la izquierda a mitad de pixel
	 * @param gdalBuffer
	 *            Buffer con la línea de datos original
	 */
	private void readLine(float[][] line, double initOffset,
			GdalBuffer[] gdalBuffer) {
		double j = 0D;
		int i = 0;
		for (int iBand = 0; iBand < gdalBuffer.length; iBand++) {
	 ...
	 *            Buffer con la línea de datos original
	 */
	private void readLine(double[][] line, double initOffset,
			GdalBuffer[] gdalBuffer) {
		double j = 0D;
		int i = 0;
		for (int iBand = 0; iBand < gdalBuffer.length; iBand++) {
	...
	/**
	 * Lee una línea completa del raster y devuelve un array del tipo correcto.
	 * Esta función es util para una lectura rapida de todo el fichero sin
	 * necesidad de asignar vista.
	 * 
	 * @param nLine
	 *            Número de línea a leer
	 * @param band
	 *            Banda requerida
	 * @return Object que es un array unidimendional del tipo de datos del
	 *         raster
	 * @throws GdalException
	 */
	public Object readCompleteLine(int nLine, int band) throws GdalException {
		...
		gdalBuf = gdalBand.readRaster(0, nLine, getRasterXSize(), 1,
				getRasterXSize(), 1, dataType[band]);
		...
		if (dataType[band] == GDT_CInt16 || dataType[band] == GDT_CInt32
				|| dataType[band] == GDT_CFloat32
				|| dataType[band] == GDT_CFloat64)
			return null;
	...
	/**
	 * Lee una bloque completo del raster y devuelve un array tridimensional del
	 * tipo correcto. Esta función es util para una lectura rapida de todo el
	 * fichero sin necesidad de asignar vista.
	 * 
	 * @param nLine
	 *            Número de línea a leer
	 * @param band
	...
	public Object readBlock(int pos, int blockHeight) throws GdalException,
		...
		if (dataType[0] == GDT_Byte) {
			...
			for (int iBand = 0; iBand < gdalBuf.length; iBand++) {
				gdalBuf[iBand] = gdalBand[iBand].readRaster(0, pos, nX,
						blockHeight, nX, blockHeight, dataType[0]);
				for (int iRow = 0; iRow < blockHeight; iRow++) {
					for (int iCol = 0; iCol < nX; iCol++)
						buf[iBand][iRow][iCol] = gdalBuf[iBand].buffByte[iRow
								* nX + iCol];
			...
			return buf;
		} else if (dataType[0] == GDT_CInt16 || dataType[0] == GDT_Int16
				|| dataType[0] == GDT_UInt16) {
			...
			for (int iBand = 0; iBand < gdalBuf.length; iBand++) {
				...
				for (int iRow = 0; iRow < blockHeight; iRow++) {
					for (int iCol = 0; iCol < nX; iCol++)
						buf[iBand][iRow][iCol] = gdalBuf[iBand].buffShort[iRow
								* nX + iCol];
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
			...
			return buf;
		} else if (dataType[0] == GDT_CInt32 || dataType[0] == GDT_Int32
				|| dataType[0] == GDT_UInt32) {
			...
			for (int iBand = 0; iBand < gdalBuf.length; iBand++) {
				...
				for (int iRow = 0; iRow < blockHeight; iRow++) {
					for (int iCol = 0; iCol < nX; iCol++)
						buf[iBand][iRow][iCol] = gdalBuf[iBand].buffInt[iRow
								* nX + iCol];
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
			...
			for (int iBand = 0; iBand < gdalBuf.length; iBand++) {
				...
				for (int iRow = 0; iRow < blockHeight; iRow++) {
					for (int iCol = 0; iCol < nX; iCol++)
						buf[iBand][iRow][iCol] = gdalBuf[iBand].buffFloat[iRow
								* nX + iCol];
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
			...
			for (int iBand = 0; iBand < gdalBuf.length; iBand++) {
				...
				for (int iRow = 0; iRow < blockHeight; iRow++) {
					for (int iCol = 0; iCol < nX; iCol++)
						buf[iBand][iRow][iCol] = gdalBuf[iBand].buffDouble[iRow
								* nX + iCol];
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
	...
	public void readLine(Object line) throws GdalException {
		...
		if (gdalBands[0].getRasterColorTable() != null) {
			palette = new ColorTable();
			palette.createPaletteFromGdalColorTable(gdalBands[0]
					.getRasterColorTable());
	 ...
	 * que empezar a escribir en caso de que este sea mayor que los datos a
	 * leer.
	 * 
	 * @param dWorldTLX
	 *            Posición X superior izquierda en coord reales
	 * @param dWorldTLY
	 *            Posición Y superior izquierda en coord reales
	 * @param dWorldBRX
	 *            Posición X inferior derecha en coord reales
	 * @param dWorldBRY
	 *            Posición Y inferior derecha en coord reales
	 * @param nWidth
	 *            Ancho en pixeles del buffer
	 * @param nHeight
	 *            Alto en pixeles del buffer
	 * @return desplazamiento dentro del buffer en X e Y
	 */
	private int[] calcStepBuffer(Extent dataExtent, int nWidth, int nHeight,
			int[] stpBuffer) {
		Extent imageExtent = getExtentWithoutRot();
		Extent ajustDataExtent = RasterUtilities.calculateAdjustedView(
				dataExtent, imageExtent);
		if (!RasterUtilities.compareExtents(dataExtent, ajustDataExtent)) {
			Point2D p1 = worldToRasterWithoutRot(new Point2D.Double(
					ajustDataExtent.minX(), ajustDataExtent.maxY()));
			Point2D p2 = worldToRasterWithoutRot(new Point2D.Double(
					ajustDataExtent.maxX(), ajustDataExtent.minY()));
			Point2D p3 = worldToRasterWithoutRot(new Point2D.Double(
					dataExtent.minX(), dataExtent.maxY()));
			// Point2D p4 = worldToRasterWithoutRot(new
			// Point2D.Double(dataExtent.maxX(), dataExtent.minY()));
			// Ese es el ancho y alto q tendría el buffer en caso de haberse
			// ajustado
			int w = (int) Math
					.abs(Math.ceil(p2.getX()) - Math.floor(p1.getX()));
			int h = (int) Math
					.abs(Math.floor(p1.getY()) - Math.ceil(p2.getY()));
	 ...
	 * 
	 * @param buf
	 *            Buffer donde se almacenan los datos
	 * @param bandList
	 *            Lista de bandas que queremos leer y sobre que bandas del
	 *            buffer de destino queremos escribirlas
	 * @param dWorldTLX
	 *            Posición X superior izquierda en coord reales
	 * @param dWorldTLY
	 *            Posición Y superior izquierda en coord reales
	 ...
	 * @throws GdalException
	 */
	public void readWindow(IBuffer buf, BandList bandList, double ulx,
			double uly, double lrx, double lry, int nWidth, int nHeight,
			boolean adjustToExtent) throws GdalException, InterruptedException {
		...
		// Si el buffer no se ajusta al extent entonces calculamos en que
		// posición comienza a escribirse dentro del buffer
		...
		if (!adjustToExtent) {
			...
			if (y < 0)
				y = 0;
			readData(buf, bandList, x, y, wh[0], wh[1], wh[0], wh[1], 0, 0,
					stpBuffer);
			return;
		}

		readData(buf, bandList, x, y, nWidth, nHeight, nWidth, nHeight, 0, 0,
				stpBuffer);
	}

	/**
	 * Lee una ventana de datos con resampleo a partir de coordenadas reales.
	 * Este método lee la ventana de una vez cargando los datos de un golpe en
	 * el buffer. Las coordenadas se solicitan en coordenadas del mundo real por
	 * lo que estas pueden caer en cualquier parte de un pixel. Esto se hace más
	 * evidente cuando supersampleamos en la petición, es decir el buffer de de
	 * mayor tamaño que el número de pixels solicitado.
	 * 
	 * Para resolver esto escribiremos con la función readRaster los datos sobre
	 * un buffer mayor que el solicitado. Después calcularemos el desplazamiento
	 * en pixels dentro de este buffer de mayor tamaño hasta llegar a la
	 * coordenada real donde comienza la petición real que ha hecho el usuario.
	 * Esto es así porque cuando supersampleamos no queremos los pixeles del
	 * raster de disco completos sino que en los bordes del buffer quedan
	 * cortados.
	 * 
	 * @param buf
	 *            Buffer donde se almacenan los datos
	 ...
	 *            Posición Y inferior derecha en coord reales
	 * @param nWidth
	 *            Ancho en pixeles de la petición
	 * @param nHeight
	 *            Alto en pixeles de la petición
	 * @param bufWidth
	 *            Ancho del buffer
	 * @param bufHeight
	 *            Alto del buffer
	 ...
	 */
	public void readWindow(IBuffer buf, BandList bandList, double ulx,
			double uly, double lrx, double lry, double nWidth, double nHeight,
		...
		/*
		 * if(bufWidth > Math.ceil(nWidth)){ stpX = (int)(((tl.getX() - x) *
		 * bufWidth) / nWidth); bufWidth = (int)((Math.abs(endX - x) * bufWidth)
		 * / nWidth); } if(bufHeight > Math.ceil(nHeight)){ stpY =
		 * (int)(((tl.getY() - y) * bufHeight) / nHeight); bufHeight =
		 * (int)((Math.abs(endY - y) * bufHeight) / nHeight); }
		 */
		...
		if (!adjustToExtent) {
			...
			readData(buf, bandList, x, y, wh[0], wh[1], bufWidth, bufHeight, 0,
					0, stpBuffer);
			return;
		}
		...
		readData(buf, bandList, x, y, (int) nWidth, (int) nHeight, bufWidth,
				bufHeight, stpX, stpY, stpBuffer);
	}
	...
	private void adjustPoints(Point2D ul, Point2D lr) {
		...
		ul.setLocation(
				(a > 0.95 || a < 0.05) ? Math.round(ul.getX()) : ul.getX(),
				(b > 0.95 || b < 0.05) ? Math.round(ul.getY()) : ul.getY());
		lr.setLocation(
				(a > 0.95 || a < 0.05) ? Math.round(lr.getX()) : lr.getX(),
				(b > 0.95 || b < 0.05) ? Math.round(lr.getY()) : lr.getY());
	}

	/**
	 * Lee una ventana de datos sin resampleo a partir de coordenadas en
	 * pixeles.
	 * 
	 * @param buf
	 *            Buffer donde se almacenan los datos
	 ...
	 * @throws GdalException
	 */
	public void readWindow(IBuffer buf, BandList bandList, int x, int y, int w,
			int h) throws GdalException, InterruptedException {
	...
	/**
	 * Lee una ventana de datos con resampleo a partir de coordenadas en
	 * pixeles. Este método lee la ventana de una vez cargando los datos de un
	 * golpe en el buffer.
	 * 
	 * @param buf
	 *            Buffer donde se almacenan los datos
	 ...
	 */
	public void readWindow(IBuffer buf, BandList bandList, int x, int y, int w,
			int h, int bufWidth, int bufHeight) throws GdalException,
			InterruptedException {
		gdalBands = new GdalRasterBand[getRasterCount()];
		...
		int[] stpBuffer = new int[] { 0, 0, buf.getWidth(), buf.getHeight() };
		readData(buf, bandList, x, y, w, h, bufWidth, bufHeight, 0, 0,
				stpBuffer);
	}

	/**
	 * Asigna el tipo de datos de las bandas a partir de una lista de
	 * GdalRasterBands
	 * 
	 * @param gdalBands
	 * @throws GdalException
	 */
	private void assignDataTypeFromGdalRasterBands(GdalRasterBand[] gdalBands)
			throws GdalException {
	...
	/**
	 * Lee una ventana de datos sin resampleo a partir de coordenadas en
	 * pixeles. Esta función es usuada por readWindow para coordenadas reales y
	 * readWindow en coordenadas pixel.
	 * 
	 * @param buf
	 *            Buffer donde se almacenan los datos
	 * @param bandList
	 ...
	 *            Alto del buffer
	 * @param stepX
	 *            Desplazamiento en pixeles en X a partir de la posición x. Este
	 *            desplazamiento es util cuando hay un supersampleo ya que puede
	 *            ser que de los pixeles que están en el borde izquierdo de la
	 *            petición solo queramos una parte de ellos.
	 * @param stepY
	 *            Desplazamiento en pixeles en Y a partir de la posición y. Este
	 *            desplazamiento es util cuando hay un supersampleo ya que puede
	 *            ser que de los pixeles que están en el borde superior de la
	 *            petición solo queramos una parte de ellos.
	 * @param stepBuffer
	 *            El buffer puede empezar a escribirse a partir de un pixel
	 *            determinado y acabar de escribir antes de fin de buffer. Este
	 *            parámetro indica el desplazamiento desde el inicio del buffer
	 *            y la posición final.
	 *            <UL>
	 ...
	 * @throws GdalException
	 */
	private void readData(IBuffer buf, BandList bandList, int x, int y, int w,
			int h, int bufWidth, int bufHeight, int stpX, int stpY,
			int[] stepBuffer) throws GdalException, InterruptedException {
		...
		for (int iBand = 0; iBand < gdalBands.length; iBand++) {
			int[] drawableBands = bandList.getBufferBandToDraw(
					RasterUtilities.getFormatedRasterFileName(fileName), iBand);
			if (drawableBands == null
					|| (drawableBands.length == 1 && drawableBands[0] == -1))
				continue;
			int init = (int) ((bufWidth * stpY) + stpX); // Pos inicial.
															// Desplazamos stpX
															// pixels hacia la
															// derecha y bajamos
															// stpY lineas
			int pos = init;
			gdalBuf = gdalBands[iBand].readRaster(x, y, w, h, bufWidth,
					bufHeight, dataType[iBand]);
			if (dataType[iBand] == Gdal.GDT_Byte) {
				for (int line = stepBuffer[1]; line < stepBuffer[3]/*
																	 * buf.getHeight
																	 * ()
																	 */; line++) {
					pos = (int) ((bufWidth * (line - stepBuffer[0])) + init);
					for (int col = stepBuffer[0]; col < stepBuffer[2]/*
																	 * buf.getWidth
																	 * ()
																	 */; col++) {
			...
			} else if ((dataType[iBand] == Gdal.GDT_UInt16)
					|| (dataType[iBand] == Gdal.GDT_Int16)
					|| (dataType[iBand] == Gdal.GDT_CInt16)) {
			...
			} else if ((dataType[iBand] == Gdal.GDT_UInt32)
					|| (dataType[iBand] == Gdal.GDT_Int32)
					|| (dataType[iBand] == Gdal.GDT_CInt32)) {
				for (int line = stepBuffer[1]; line < stepBuffer[3]; line++) {
					pos = (int) ((bufWidth * (line - stepBuffer[0])) + init);
					for (int col = stepBuffer[0]; col < stepBuffer[2]; col++) {
	 ...
	 * @throws GdalException
	 */
	private void readDataByLine(IBuffer buf, BandList bandList, int x, int y,
			int w, int yMax) throws GdalException, InterruptedException {
		GdalBuffer gdalBuf = null;
		int rasterBufLine;
		RasterTask task = RasterTaskQueue
				.get(Thread.currentThread().toString());
		...
		for (int iBand = 0; iBand < gdalBands.length; iBand++) {
			...
			if (dataType[iBand] == Gdal.GDT_Byte) {
				for (int line = y; line < yMax; line++) {
					gdalBuf = gdalBands[iBand].readRaster(x, line, w, 1, w, 1,
							dataType[iBand]);
					rasterBufLine = line - y;
					buf.setLineInBandByte(gdalBuf.buffByte, rasterBufLine,
							iBand);
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
				...
				for (int line = y; line < yMax; line++) {
					gdalBuf = gdalBands[iBand].readRaster(x, line, w, 1, w, 1,
							dataType[iBand]);
					rasterBufLine = line - y;
					buf.setLineInBandShort(gdalBuf.buffShort, rasterBufLine,
							iBand);
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
				...
				for (int line = y; line < yMax; line++) {
					gdalBuf = gdalBands[iBand].readRaster(x, line, w, 1, w, 1,
							dataType[iBand]);
					rasterBufLine = line - y;
					buf.setLineInBandFloat(gdalBuf.buffFloat, rasterBufLine,
							iBand);
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
				...
				for (int line = y; line < yMax; line++) {
					gdalBuf = gdalBands[iBand].readRaster(x, line, w, 1, w, 1,
							dataType[iBand]);
					rasterBufLine = line - y;
					buf.setLineInBandDouble(gdalBuf.buffDouble, rasterBufLine,
							iBand);
					if (task.getEvent() != null)
						task.manageEvent(task.getEvent());
				}
	...
	/**
	 * Obtiene el valor de un pixel determinado por las coordenadas x e y que se
	 * pasan por parámetro
	 * 
	 * @param x
	 ...
	 * @return Array de Object donde cada posición representa una banda y el
	 *         valor será Integer en caso de ser byte, shot o int, Float en caso
	 *         de ser float y Double en caso de ser double.
	 */
		...
		try {
			...
			for (int i = 0; i < getRasterCount(); i++) {
				...
				case 0:
					break; // Sin tipo
				case 1:
					data[i] = new Integer(r.buffByte[0]); // Buffer byte (8)
					break;
				case 2: // Buffer short (16)
				case 3:
					data[i] = new Integer(r.buffShort[0]); // Buffer short (16)
					break;
				case 4: // Buffer int (32)
				case 5:
					data[i] = new Integer(r.buffInt[0]); // Buffer int (32)
					break;
				case 6:
					data[i] = new Float(r.buffFloat[0]); // Buffer float (32)
					break;
				case 7:
					data[i] = new Double(r.buffDouble[0]); // Buffer double (64)
					break;
				}
			}
			return data;
	...
	/**
	 * Calcula el extent en coordenadas del mundo real sin rotación. Solo
	...
	/**
	 * Asigna una transformación que es aplicada sobre la que ya tiene el propio
	 * fichero
	 * 
	 * @param t
	 */
