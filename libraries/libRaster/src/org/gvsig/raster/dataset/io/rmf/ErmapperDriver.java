 ...
 */
public class ErmapperDriver extends RasterDataset implements
	 ...
	 */
	public ErmapperDriver(CoordinateReferenceSystem crs, Object param)
			throws NotSupportedExtensionException {
	 ...
	 */
	public GeoInfo load() {
		ownTransformation = new AffineTransform(file.cellIncrementX, 0, 0,
				file.cellIncrementY, file.originX, file.originY);
	 ...
	 * 
	 * @see
	 * org.gvsig.fmap.driver.RasterDataset#setView(org.gvsig.fmap.raster.Extent)
	 */
	 ...
	 * int[] stpBuffer) { Extent imageExtent = getExtentWithoutRot(); Extent
	 * ajustDataExtent = RasterUtilities.calculateAdjustedView(dataExtent,
	 * imageExtent); if(!RasterUtilities.compareExtents(dataExtent,
	 * ajustDataExtent)) { Point2D p1 = worldToRaster(new
	 * Point2D.Double(ajustDataExtent.minX(), ajustDataExtent.maxY())); Point2D
	 * p2 = worldToRaster(new Point2D.Double(ajustDataExtent.maxX(),
	 * ajustDataExtent.minY())); Point2D p3 = worldToRaster(new
	 * Point2D.Double(dataExtent.minX(), dataExtent.maxY())); Point2D p4 =
	 * worldToRaster(new Point2D.Double(dataExtent.maxX(), dataExtent.minY()));
	 * //Ese es el ancho y alto q tendría el buffer en caso de haberse ajustado
	 * int w = (int)Math.abs(Math.ceil(p2.getX()) - Math.floor(p1.getX())); int
	 * h = (int)Math.abs(Math.floor(p1.getY()) - Math.ceil(p2.getY()));
	 * 
	 * stpBuffer[0] = (int)(p1.getX() + (-p3.getX())); stpBuffer[1] =
	 * (int)(p1.getY() + (-p3.getY())); stpBuffer[2] = stpBuffer[0] + w;
	 * stpBuffer[3] = stpBuffer[1] + h; return new int[]{w, h}; } return new
	 * int[]{nWidth, nHeight}; }
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.RasterDataset#getWindowRaster(double, double,
	 * double, double, org.gvsig.fmap.driver.BandList,
	 * org.gvsig.fmap.driver.IBuffer, boolean)
	 */
	public IBuffer getWindowRaster(double ulx, double uly, double w, double h,
		...
		/*
		 * if(!adjustToExtent){ int[] wh = calcStepBuffer(selectedExtent, wPx,
		 * hPx, stpBuffer); if(x < 0) x = 0; if(y < 0) y = 0; readData(buf,
		 * bandList, x, y, wh[0], wh[1], wh[0], wh[1], 0, 0, stpBuffer); return;
		 * }
		 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.RasterDataset#getWindowRaster(int, int, int,
	 * int, org.gvsig.fmap.driver.BandList, org.gvsig.fmap.driver.IBuffer)
	 */
	 ...
	 * 
	 * @see org.gvsig.fmap.driver.RasterDataset#getWindowRaster(double, double,
	 * double, double, int, int, org.gvsig.fmap.driver.BandList,
	 * org.gvsig.fmap.driver.IBuffer, boolean)
	 */
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
		...
		loadBuffer(viewRequest, bufWidth, bufHeight, rasterBuf, bandList,
				stpBuffer);
		return rasterBuf;
	}
	 ...
	 * 
	 * @see org.gvsig.fmap.driver.RasterDataset#getWindowRaster(int, int, int,
	 * int, int, int, org.gvsig.fmap.driver.BandList,
	 * org.gvsig.fmap.driver.IBuffer)
	 */
	...
	/**
	 * Carga el buffer con las bandas RGB del raster con los parámetros
	 * especificados de extensión y tamaño de buffer. El problema de ecw es que
	 * solo podemos leer 3 bandas de una vez ya que solo disponemos de una
	 * llamada readLineRGBA. Para leer más bandas tendremos que hacer multiples
	 * llamadas a setView para leer 3 cada vez.
	 * 
	 * Si por ejemplo tenemos un ecw de 6 bandas [0, 1, 2, 3, 4, 5] y queremos
	 * cargar un buffer con el siguiente orden [0, -, 2, -, 4, -] La variable
	 * readBandsFromECW hará la llamada a setView con los valores [0, 2, 4, 0,
	 * 0, 0]. La función drawRGB será la encargada de hacer el switch para
	 * obtener [0, -, 2, -, 4, -].
	 * 
	 * Bug#1: Si se dispone de un ecw de más de tres bandas podemos llamar a
	 * setView con readBandsFromECW con el orden que queramos, por ejemplo [3,
	 * 2, 5, 1, 0] pero para ecw de 3 bandas la llamada con las bandas cambiadas
	 * no hace caso. El caso de readBandsFromECW = [2, 0, 1] será tomado siempre
	 * como [0, 1, 2].
	 * 
	 ...
	 *            Buffer de datos
	 */
	private void loadBuffer(Extent selectedExtent, int bufWidth, int bufHeight,
			IBuffer rasterBuf, BandList bandList, int[] stpBuffer)
			throws InterruptedException, RasterDriverException {
		try {
			RasterTask task = RasterTaskQueue.get(Thread.currentThread()
					.toString());

			// Leemos el raster desde la librería

			// Extent ext = getExtent();//new Extent(file.originX, file.originY,
			// file.originX + (file.width * file.cellIncrementX), file.originY +
			// (file.height * file.cellIncrementY));
			// selectedExtent =
			// RasterUtilities.calculateAdjustedView(selectedExtent, ext);
			// selectedExtent =
			// RasterUtilities.calculateAdjustedView(selectedExtent,
			// this.getAffineTransform(), new Dimension(file.width,
			// file.height));
			...
			for (int i = 0; i < file.numBands; i++) {
				int[] bandsToDraw = bandList.getBand(i)
			...
			// Si el ancho y alto pedido no coincide con el estimado es que se
			// intenta resamplear.
			// Otros drivers soportan el resampleo pero no es el caso. Hay que
			// hacer la petición y luego remuestrear.

			/*
			 * if((selectedExtent.width()/file.cellIncrementX) ||
			 * (selectedExtent.height()/file.cellIncrementY)) {
			 * 
			 * }
			 */

			if (bufWidth > Math.abs(selectedExtent.width()
					/ file.cellIncrementX))
				bufWidth = (int) Math.round(Math.abs(selectedExtent.width()
						/ file.cellIncrementX));
			if (bufHeight > Math.abs(selectedExtent.height()
					/ file.cellIncrementY))
				bufHeight = (int) Math.round(Math.abs(selectedExtent.height()
						/ file.cellIncrementY));
			file.setView(file.numBands, readBandsFromECW,
					selectedExtent.minX(), selectedExtent.maxY(),
					selectedExtent.maxX(), selectedExtent.minY(), bufWidth,
					bufHeight);
	...
	private void drawRGB(IBuffer rasterBuf, int[] pRGBArray, int[] readBands,
			BandList bandList, RasterTask task) throws JNCSException,
					...
					if (i == 0) {
						for (int line = 0; line < rasterBuf.getHeight(); line++) {
							try {
								...
								System.err
										.println("Error: fuera de rango en la petición de línea");
							}
						}
						return;
					}
		...
		if (bandR >= 0 && bandG >= 0 && bandB >= 0) {
			for (int line = 0; line < rasterBuf.getHeight(); line++) {
				try {
					file.readLineRGBA(pRGBArray);
					for (int col = 0; col < pRGBArray.length; col++) {
						rasterBuf.setElem(line, col, bandR,
								(byte) ((pRGBArray[col] & 0x00ff0000) >> 16));
						rasterBuf.setElem(line, col, bandG,
								(byte) ((pRGBArray[col] & 0x0000ff00) >> 8));
						rasterBuf.setElem(line, col, bandB,
								(byte) (pRGBArray[col] & 0x000000ff));
					}
				} catch (JNCSException exc) {
				}
	...
	public Object readCompleteLine(int line, int band)
			...
			RasterDriverException {
		if (line < 0 || line >= file.height || band < 0
				|| band >= getBandCount())
		...
		Extent e = new Extent(begin.getX(), begin.getY(), end.getX(),
				end.getY());

		try {
			int[] value = new int[file.width];
			file.setView(file.numBands, readBandsFromECW, e.minX(), e.maxY(),
					e.maxX(), e.minY(), file.width, 1);
			...
			throw new FileNotOpenException(
					"Error en jecw: JNCSFileNotOpenException");
		} catch (JNCSInvalidSetViewException e1) {
			throw new FileNotOpenException(
					"Error en jecw: JNCSInvalidSetViewException");
	...
	public Object readBlock(int pos, int blockHeight)
		...
		try {
			int[] value = new int[file.width];
			file.setView(file.numBands, readBandsFromECW, e.minX(), e.maxY(),
					e.maxX(), e.minY(), file.width, blockHeight);
	...
	public Object getData(int x, int y, int band)
		...
		try {
			...
			if (file.numBands <= 3) {
				...
				case 0:
					return new Integer(
							(((value[0] & 0x00ff0000) >> 16) & 0xffffffff));
				case 1:
					return new Integer(
							(((value[0] & 0x0000ff00) >> 8) & 0xffffffff));
				case 2:
	...
	public void refreshUpdate(int arg0, int arg1, double arg2, double arg3,
			double arg4, double arg5) {
	}

	public void refreshUpdate(int arg0, int arg1, int arg2, int arg3, int arg4,
			int arg5) {
	}
