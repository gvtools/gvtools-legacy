 ...
 * 
 * Puede exportar un fichero desde un GeoRasterFile en cualquier formato
 * soportado por los drivers de lectura a uno en formato Ecw.
 * 
 * Puede salvar a disco en formato Ecw obteniendo los datos que van siendo
 * servidos desde el cliente. Este cliente debe implementar un IDataWriter o
 * tener un objeto que lo implemente. Inicialmente le pasará los parámetros de
 * la imagen de salida y cuando el driver comience a escribir le irá solicitando
 * más a través del método readData de IDataWriter. El cliente será el que lleve
 * el control de lo que va sirviendo y lo que le queda por servir.
 * 
...
public class ErmapperWriter extends GeoRasterWriter {
	...
	public static void register() {
		...
		if (os.startsWith("2.4")) {
			...
			features = new WriteFileFormatFeatures("Ecw", "ecw",
					new int[] { 3 }, null, ErmapperWriter.class);
			fileFeature.put("ecw", features);
		}
		point.register("jp2", ErmapperWriter.class);
		features = new WriteFileFormatFeatures("Jpeg2000", "jp2",
				new int[] { 3 }, null, ErmapperWriter.class);
	...
	public void loadParams() {
		...
		driverParams.setParam("compression", new Integer(10), Params.SLIDER,
				new String[] { "1", "20", "10", "1", "5" }); // min, max, valor
																// defecto,
																// intervalo
																// pequeño,
																// intervalo
		...
		driverParams.setParam("format", new Integer(4), Params.CHOICE,
	...
	/**
	 * Constructor para la lectura de datos desde el objeto cliente a partir de
	 * un viewport dado.
	 * 
	 * @param dataWriter
	 *            Objeto que sirve datos para el escritor
	 * @param outFileName
	 *            Fichero de salida
	 * @param nBands
	 *            Número de bandas
	...
	public ErmapperWriter(IDataWriter dataWriter, String outFileName,
			...
			CoordinateReferenceSystem crs, Boolean geo) throws EcwException,
			IOException {

		// La libreria de ECW no se traga caracteres raros al escribir. Los
		// cambiamos por X de momento y ya se apaña el usuario

		String ext = RasterUtilities.getExtensionFromFileName(outFileName);
		String fname = outFileName.substring(
				outFileName.lastIndexOf(File.separator) + 1,
				outFileName.lastIndexOf(".")).replaceAll("[^a-zA-Z0-9_]", "X");
	...
	/**
	 * Inicialización de los parámetros del compresor que serán obtenidos de
	 * PxRaster.
	 * 
	 * @throws EcwException
	 */
	private void init() throws EcwException {
		percent = 0;
		int comp = ((Integer) ((Param) driverParams.getParamById("compression")).defaultValue)
				.intValue();
		...
		/*
		 * boolean georef = new
		 * Boolean(((Param)driverParams.getParamById("georef"
		 * )).defaultValue).booleanValue(); if (georef) {
		 */
	...
	/**
	 * Convierte la cadena que representa el formato en un valor númerico que
	 * entiende el driver.
	 * 
	 * @param format
	 *            Cadena que representa el formato
	...
	/**
	 * A partir de un elemento que contiene una propiedad y un valor lo parsea y
	 * asigna el valor a su variable.
	 * 
	 * @param propValue
	 *            elemento con la forma propiedad=valor
	 */
	...
	/**
	 * Asigna propiedades al driver a partir de un vector de strings donde cada
	 * elemento tiene la estructura de propiedad=valor.
	 * 
	 * @param props
	 *            Propiedades
	 */
	...
	public void fileWrite() throws IOException {
		/*
		 * try { compressclient.NCSEcwCompressOpen(false);
		 * compressclient.NCSEcwCompress(readerObj); } catch (EcwException e) {
		 * e.printStackTrace(); }
		 */
		// TODO: FUNCIONALIDAD: Compresión a partir de un dataset. De momento no
		// es necesario y es posible que nunca lo sea.
	}
	...
	public void dataWrite() throws IOException, InterruptedException {
		if (dataWriter == null) {
			throw new IOException(
					"No se ha obtenido un objeto para la lectura valido.");
		}
	...
	public void setWkt(String wkt) {
		// TODO: FUNCIONALIDAD: La asignación de proyección en Ecw falla
		// haciendo petar la aplicación.
		/*
		 * if(compressclient != null && wkt != null && wkt.compareTo("unknown")
		 * != 0) { WktUtils wktutil = new WktUtils(wkt);
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.GeoRasterWriter#setCancellableRasterDriver(org
	 * .gvsig.raster.dataset.io.IExternalCancellable)
	 */
...
/**
 * Clase que se encarga de la lectura de datos que serán escritos. Hereda de
 * JniObject para asegurar para asegurar que el interfaz de la libreria tiene
 * acceso a variables necesarias para la petición de datos cuando vacia el
 * buffer. Implementa ReadCallBack para obligar escribir el método loadBuffer
 * encargado de servir los datos cuando el buffer se vacia.
 * 
 */
class Reader extends JniObject implements ReadCallBack {
	...
	private int countLine = 1; // Contador de líneas procesadas en cada lectura
								// de bloque
	byte[][] buf = null;
	byte[] bufband1 = null;
	byte[] bufband2 = null;
	...
	private int lineasBloqueFinal = 0; // Número de líneas leidas del bloque
										// final
	...
	/**
	 * Constructor para que los datos sean servidos desde el cliente a través de
	 * un IDataWriter.
	 * 
	 * @param dataWriter
	 *            Objeto servidor de datos del driver
	 * @param compressclient
	 *            Objeto que representa al compresor ecw
	 * @param width
	 *            Ancho de la imagen
	 * @param height
	 *            Alto de la imagen
	 * @param blockSizeRead
	 *            Altura del bloque en la lectura
	 * @param nBands
	 *            Número de bandas
	 */
	...
	/**
	 * Lectura de bandas llamando al objeto cliente para que nos cargue el
	 * buffer
	 * 
	 * @param width
	 *            Ancho de la imagen
	 ...
	 */
	/*
	 * private void readBands(int width, int height) { if(dataType ==
	 * IBuffer.TYPE_IMAGE) dataBuffer = dataWriter.readARGBData(width, height,
	 * 0); if(dataType == IBuffer.TYPE_BYTE) { byte[][] b =
	 * dataWriter.readByteData(width, height);
	 * 
	 * } }
	 */
	...
	public void loadBuffer() {
		...
		// si se leen bloques completos lineasLeidas es == al tamaño de bloque
		// sino
	...
	/**
	 * Carga el buffer de datos desde elementos de tipo entero con contenido
	 * ARGB
	 * 
	 * @param data
	...
	private void loadBufferFromImageDataType(int[] data) {
		for (int iBand = 0; iBand < this.nBands; iBand++) {
			for (int pos = 0; pos < width; pos++) {
				switch (iBand) {
				case 0:
					compressclient.buffer[pos + (width * iBand)] = (byte) ((data[pos
							+ (width * countLine)] & 0xff0000) >> 16);
					break;
				case 1:
					compressclient.buffer[pos + (width * iBand)] = (byte) ((data[pos
							+ (width * countLine)] & 0xff00) >> 8);
					break;
				case 2:
					compressclient.buffer[pos + (width * iBand)] = (byte) (data[pos
							+ (width * countLine)] & 0xff);
					break;
				}
	...
	/**
	 * Carga el buffer de datos a comprimir desde un buffer byte[][] donde la
	 * primera dimensión es el número de bandas y la segunda los elementos en
	 * lista (Ancho X Alto)
	 * 
	 * @param b
	 *            Buffer de datos de tipo byte[][]
	 */
