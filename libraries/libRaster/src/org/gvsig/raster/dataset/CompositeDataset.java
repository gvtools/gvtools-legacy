...
public class CompositeDataset implements IRasterDataSource {
	 ...
	 *            Número de columnas
	 */
	public CompositeDataset(MultiRasterDataset[][] mos)
		...
		/*
		 * if(!validateMosaic()) { this.mosaic = null; throw new
		 * MosaicNotValidException("Extends no validos para montar un mosaico");
		 * }
		 */
		init();
	}
	...
	/**
	 * Método que elimina crea un array bidimensional eliminando los nulos del
	 * que tiene el que se le pasa por parámetro
	 * 
	 * @param values
	 * @return MultiRasterDataset
	 */
	private MultiRasterDataset[][] deleteNullValues(
			MultiRasterDataset[][] values) {
		int n = values.length;
	...
	/**
	 * Valida que los extends del mosaico sean validos, es decir, que sean
	 * correlativos formando la matriz. Además también valida que el tamaño de
	 * pixel coincida en todos los raster que forman el mosaico.
	 * 
	 * @param mos
	 * @throws MosaicNotValidException
	 */
				...
				for (int col = 0; col < m; col++) {
					if (col < (m - 1) && mosaic[row][col] != null) {
						...
						if (((int) a.maxX()) != ((int) b.minX()))
							return false;
						double psx = MathUtils.clipDecimals(
								mosaic[row][col].getPixelSizeX(), 2);
						double psx1 = MathUtils.clipDecimals(
								mosaic[row][col + 1].getPixelSizeX(), 2);
						double psy = MathUtils.clipDecimals(
								mosaic[row][col].getPixelSizeY(), 2);
						double psy1 = MathUtils.clipDecimals(
								mosaic[row][col + 1].getPixelSizeY(), 2);
						if (psx != psx1 || psy != psy1)
							return false;
						if (mosaic[row][col].getBandCount() != mosaic[row][col + 1]
								.getBandCount())
							return false;
					}
				}
				...
				for (int row = 0; row < n; row++) {
					if (row < (n - 1) && mosaic[row][col] != null) {
						...
						double psx = MathUtils.clipDecimals(
								mosaic[row][col].getPixelSizeX(), 2);
						double psx1 = MathUtils.clipDecimals(
								mosaic[row + 1][col].getPixelSizeX(), 2);
						double psy = MathUtils.clipDecimals(
								mosaic[row][col].getPixelSizeY(), 2);
						double psy1 = MathUtils.clipDecimals(
								mosaic[row + 1][col].getPixelSizeY(), 2);
						if (psx != psx1 || psy != psy1)
							return false;
						if (mosaic[row][col].getBandCount() != mosaic[row + 1][col]
								.getBandCount())
							return false;
					}
				}
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#addDataset(org.gvsig.raster
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#addDataset(java.lang.String[])
	 */
		...
		if (mosaic != null) {
			...
			if (fileName.length == (n * m)) {
				for (int i = 0; i < n; i++) {
					for (int j = 0; j < m; j++) {
						MultiRasterDataset mrd = new MultiRasterDataset();
						mrd.addDataset(new RasterDataset[] { RasterDataset
								.open(null, fileName[i * n + j]) });
					}
				}
			}
			init();
	...
	/**
	 * Acciones de inicialización cuando se crea el objeto o se añaden nuevos
	 * dataset a este
	 */
	private void init() {
				...
				if (row != 0 && col != 0) { // El primero ya está añadido
					for (int i = 0; i < mosaic[0][0].getBandCount(); i++) {
						if (mosaic[row][col] != null)
							bandList.getBand(i).setAdditionalName(
									mosaic[row][col].getBands().getBand(i)
											.getFileName());
					}
				}
			}
	...
	public double[] calcSteps(double dWorldTLX, double dWorldTLY,
		...
		if (mosaic != null)
			return mosaic[0][0].calcSteps(dWorldTLX, dWorldTLY, dWorldBRX,
					dWorldBRY, nWidth, nHeight, bufWidth, bufHeight);
		return null;
	}
	...
	public IRasterDataSource newDataset() {
		...
		for (int row = 0; row < n; row++)
			for (int col = 0; col < m; col++)
				if (mosaic[row][col] != null)
					mrd[row][col] = (MultiRasterDataset) mosaic[row][col]
							.newDataset();
	...
	public Extent getExtent() {
		...
		if (mosaic != null && mosaic[0][0] != null) {
			...
			return new Extent(new Point2D.Double(ulx, uly), new Point2D.Double(
					lrx, lry), new Point2D.Double(urx, ury),
	...
	/**
	 * Obtiene el ancho del mosaico completo en píxeles, esto es la suma de
	 * todos los raster que componen la extensión.
	 */
	...
	/**
	 * Obtiene el alto del mosaico completo en píxeles, esto es la suma de todos
	 * los raster que componen la extensión.
	 */
	...
	/**
	 * Obtiene el dataset cuyas coordenadas contienen el punto pasado por
	 * parámeto
	 * 
	 * @param x
	 *            Coordenada X a comprobar
	 * @param y
	 *            Coordenada Y a comprobar
	...
	public Point2D getDatasetByCoords(double x, double y) {
		...
		for (int row = 0; row < n; row++) {
			for (int col = 0; col < m; col++) {
				if (mosaic[row][col] != null
						&& RasterUtilities.isInside(new Point2D.Double(x, y),
								mosaic[row][col].getExtent(),
								mosaic[row][col].getAffineTransform(0)))
	...
	/**
	 * Obtiene la lista de datasets del mosaico que intersectan con el extent
	 * proporcionado
	 * 
	 * @param ulx
	 ...
	 */
	private MultiRasterDataset[][] getDatasetListInArea(double ulx, double uly,
			double lrx, double lry) throws NoninvertibleTransformException {
		int n = mosaic.length;
		int m = mosaic[0].length;
		...
		for (int row = 0; row < n; row++)
			for (int col = 0; col < m; col++)
				if (mosaic[row][col] != null
						&& RasterUtilities.intersects(new Extent(ulx, uly, lrx,
								lry), mosaic[row][col].getExtent(),
								mosaic[row][col].getAffineTransform(0))) {
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#isInside(java.awt.geom.Point2D
	 * )
	 */
	...
	public boolean isRotated() {
		if (getAffineTransform(0).getShearX() != 0
				|| getAffineTransform(0).getShearY() != 0)
			return true;
		return false;
	}
	...
	/**
	 * Genera un buffer de datos único a partir de una matriz de buffers donde
	 * puede haber elementos con valor nulo.
	 * 
	 * @return
	...
	public IBuffer generateBuffer(IBuffer[][] bufList, int drawableBands)
		...
		// Creamos el buffer
		IBuffer raster = RasterBuffer.getBuffer(bufList[0][0].getDataType(),
				nCols, nRows, drawableBands, true);
	 ...
	 *            Buffer de destino
	 * @param col
	 *            Columna del buffer de destino donde se empieza a escribir
	 * @param row
	 *            Fila del buffer de destino donde se empieza a escribir
	 * @return array con los valores que representan la última fila y última
	 *         columna que se escribieron
	 * @throws InterruptedException
	 */
			...
			for (int band = 0; band < origin.getBandCount(); band++)
				for (int row = 0; row < origin.getHeight(); row++)
					for (int col = 0; col < origin.getWidth(); col++)
						try {
							dest.setElem(row + r, col + c, band,
									origin.getElemByte(row, col, band));
			...
			for (int band = 0; band < origin.getBandCount(); band++)
				for (int row = 0; row < origin.getHeight(); row++)
					for (int col = 0; col < origin.getWidth(); col++)
						try {
							dest.setElem(row + r, col + c, band,
									origin.getElemShort(row, col, band));
						} catch (ArrayIndexOutOfBoundsException e) {
							break;
						}
			break;
			...
			for (int band = 0; band < origin.getBandCount(); band++)
				for (int row = 0; row < origin.getHeight(); row++)
					for (int col = 0; col < origin.getWidth(); col++)
						try {
							dest.setElem(row + r, col + c, band,
									origin.getElemFloat(row, col, band));
						} catch (ArrayIndexOutOfBoundsException e) {
							break;
						}
			break;
			...
			for (int band = 0; band < origin.getBandCount(); band++)
				for (int row = 0; row < origin.getHeight(); row++)
					for (int col = 0; col < origin.getWidth(); col++)
						try {
							dest.setElem(row + r, col + c, band,
									origin.getElemDouble(row, col, band));
						} catch (ArrayIndexOutOfBoundsException e) {
							break;
						}
			break;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.raster.dataset.IRasterDataSource#getWindowRaster(double,
	...
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
			...
			RasterDriverException {
		try {
			MultiRasterDataset[][] datasetList = getDatasetListInArea(ulx, uly,
					lrx, lry);
			int n = mosaic.length;
			int m = mosaic[0].length;
			...
			for (int row = 0; row < n; row++)
				for (int col = 0; col < m; col++)
					if (datasetList[row][col] != null)
						bufferList[row][col] = datasetList[row][col]
								.getWindowRaster(ulx, uly, lrx, lry);
			...
			throw new InvalidSetViewException(
					"No se ha podido aplicar la transformación inversa para esa vista.");
		}
	}
		...
		try {
			...
			for (int row = 0; row < n; row++)
				for (int col = 0; col < m; col++)
					if (datasetList[row][col] != null)
						bufferList[row][col] = datasetList[row][col]
								.getWindowRaster(ulx, uly, ulx + w, uly - h);
			IBuffer endBuffer = generateBuffer(bufferList, mosaic[0][0]
					.getBands().getDrawableBandsCount());
			for (int row = 0; row < n; row++)
				for (int col = 0; col < m; col++)
	 ...
	 * 
	 * @see org.gvsig.raster.dataset.IRasterDataSource#getWindowRaster(double,
	 * double, double, double, int, int, boolean)
	 */
	public IBuffer getWindowRaster(double ulx, double uly, double lrx,
		...
		try {
			...
			MultiRasterDataset[][] datasetList = getDatasetListInArea(
			...
			for (int row = 0; row < n; row++)
				for (int col = 0; col < m; col++)
					if (datasetList[row][col] != null) {
						int[] values = getLocalRequest((int) px1.getX(),
								(int) px1.getY(),
								(int) Math.abs(px2.getX() - px1.getX()),
								(int) Math.abs(px2.getY() - px1.getY()), row,
								col);
						if (values != null) {
							int bW = (int) Math.round((Math.abs(values[2]
									- values[0]) * bufWidth)
									/ Math.abs(px2.getX() - px1.getX()));
							int bH = (int) Math.round((Math.abs(values[3]
									- values[1]) * bufHeight)
									/ Math.abs(px2.getY() - px1.getY()));
							int wTile = ((int) Math.abs(values[2] - values[0]) > (int) datasetList[row][col]
									.getWidth()) ? (int) datasetList[row][col]
									.getWidth() : (int) Math.abs(values[2]
									- values[0]);
							int hTile = ((int) Math.abs(values[3] - values[1]) > (int) datasetList[row][col]
									.getHeight()) ? (int) datasetList[row][col]
									.getHeight() : (int) Math.abs(values[3]
									- values[1]);
							bufferList[row][col] = datasetList[row][col]
									.getWindowRaster(values[0], values[1],
											wTile, hTile, bW, bH);
						}
					}
			IBuffer endBuffer = generateBuffer(bufferList, mosaic[0][0]
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.raster.dataset.IRasterDataSource#getWindowRaster(int, int,
	 * int, int)
	 */
	public IBuffer getWindowRaster(int x, int y, int w, int h)
		...
		try {
			...
			Point2D p2 = mosaic[0][0].rasterToWorld(new Point2D.Double(x + w, y
					+ h));
			MultiRasterDataset[][] datasetList = getDatasetListInArea(
					p1.getX(), p1.getY(), p2.getX(), p2.getY());
			int n = mosaic.length;
			...
			for (int row = 0; row < n; row++) {
				for (int col = 0; col < m; col++) {
					if (datasetList[row][col] != null) {
						...
						if (values != null) {
							int bW = Math
									.round((Math.abs(values[2] - values[0]) * bufWidth)
											/ w);
							int bH = Math.round((Math
									.abs(values[3] - values[1]) * bufHeight)
									/ h);
							int wTile = ((int) Math.abs(values[2] - values[0]) > (int) datasetList[row][col]
									.getWidth()) ? (int) datasetList[row][col]
									.getWidth() : (int) Math.abs(values[2]
	...
	/**
	 * Convierte una petición global del mosaico en coordenadas pixel a una
	 * local al tile concreto que se está tratando
	 * 
	 * @param x
	 *            Posición X de la petición a comprobar
	 * @param y
	 *            Posición Y de la petición a comprobar
	 * @param w
	 *            Ancho de la petición a comprobar
	 * @param h
	 *            Alto de la petición a comprobar
	 * @param r
	 *            File del tile
	 * @param c
	 *            Columna del tile
	 * @return cuatro valores correspondientes a la x1, y1, x2, y2 de la
	 *         petición referente al tile.
	 */
	private int[] getLocalRequest(int x, int y, int w, int h, int r, int c) {
		...
		if (isInside(x, y, r, c))
			...
			else if (getTileFromPixelPoint(x, y)[1] == c) // Está en la misma
															// columna
		...
		if (isInside(fx, fy, r, c))
			...
			if (getTileFromPixelPoint(fx, fy)[0] == r)
				p2 = new Point2D.Double(mosaic[r][c].getWidth(), getLocalPixel(
						fx, fy).getY());
			else if (getTileFromPixelPoint(fx, fy)[1] == c)
				p2 = new Point2D.Double(getLocalPixel(fx, fy).getX(),
						mosaic[r][c].getHeight());
			else
				p2 = new Point2D.Double(mosaic[r][c].getWidth(),
						mosaic[r][c].getHeight());
		}
		if (p1 != null && p2 != null)
			return new int[] { (int) p1.getX(), (int) p1.getY(),
					(int) p2.getX(), (int) p2.getY() };
		return null;
	}

	/**
	 * Consulta si una petición en coordenadas pixel de la imagen completa pasa
	 * a través de un tile de posición r, c
	 * 
	 * @param x
	 *            Posición X de la petición a comprobar
	 * @param y
	 ...
	 */
	private boolean requestGoThroughTile(int x, int y, int w, int h, int r,
			int c) {
		...
		// Intersección entre el área pedida y el tile. Si intersectan se
		// devuelve true
		if (((x <= p1.getX() && w >= p1.getX()) || (x <= p2.getX() && w >= p2
				.getX()))
				&& ((y <= p1.getY() && h >= p1.getY()) || (y <= p2.getY() && h >= p2
						.getY())))
			return true;
		return false;
	}
	 ...
	 * 
	 * @param x
	 *            Posición X del punto a comprobar
	 * @param y
	 *            Posición Y del punto a comprobar
	 ...
	 *            Posición Y del punto a comprobar
	 * @param r
	 *            Fila del tile a comprobar
	 * @param c
	 *            Columna del tile a comprobar
	...
	/**
	 * Obtiene la coordenada pixel local para un raster dada la coordenada pixel
	 * global
	 * 
	 * @param x
	 *            Coordenada X global
	...
	/**
	 * Obtiene la coordenada pixel global para un raster dada la coordenada
	 * pixel local
	 * 
	 * @param x
	 *            Coordenada X local
		...
		try {
			for (int i = 0; i < histogram.length; i++) {
				for (int j = 0; j < histogram[i].length; j++) {
					histogram[i][j] = new DatasetListHistogram(mosaic[i][j])
							.getHistogram();
					if (i != 0 || j != 0)
	...
	public Object getData(int x, int y, int band)
		...
		if (posTile != null) {
			RasterDataset dataset = mosaic[posTile[0]][posTile[1]]
					.getDataset(0)[0];
			Point2D localPixel = getLocalPixel(x, y);
			return dataset.getData((int) localPixel.getX(),
					(int) localPixel.getY(), band);
		}
		return null;
	}
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#saveGeoPointsToRmf(org.gvsig
	 * .raster.datastruct.GeoPointList)
	 */
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#getDataset(java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * org.gvsig.raster.dataset.IRasterDataSource#getNameDatasetStringList(int,
	 * int)
	 */
