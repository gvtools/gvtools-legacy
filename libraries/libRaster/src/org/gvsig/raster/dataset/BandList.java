...
/**
 * Esta clase gestiona una lista de bandas que puede pertenecer a un fichero (la
 * tiene un Dataset) o puede ser una lista de bandas de multiples ficheros (la
 * tiene un MultiRasterDataset). Las bandas son almacenadas en un array donde la
 * posición en este es el número de banda.
 * 
...
public class BandList {
	 ...
	 * 
	 * @param file
	 *            Fichero al que pertenece la banda buscada.
	 * @param pos
	 *            Posición que ocupa en el fichero.
	 * @return true si se ha hallado la banda y false si no se ha encontrado
	 */
	...
	/**
	 * Obtiene la lista de bandas a pintar sobre el buffer de salida en forma de
	 * array. Cada elemento del array es una banda del RasterBuf de salida que
	 * ha de dibujarse y el valor que contiene ese elemento es la banda de la
	 * imagen que va pintada en esa banda del RasterBuf.
	 * <table border=1>
	 * <tr>
	 * <td>Elem</td>
	 * <td>Valor</td>
	 ...
	 * </tr>
	 * </table>
	 * El RasterBuf tendra en la banda 0 dibujada la banda 1 de este
	 * GeoRasterFile, en la banda 1 la 0 del dataset, en la banda 2 no habrá
	 * ninguna de este dataset, en la 3 la 0 y en la 4 la 2 de este
	 * GeoRasterFile.
	 * 
		...
		for (int dataImageBand = 0; dataImageBand < nBandsDataImage; dataImageBand++) {
			...
			for (int band = 0; band < bands.size(); band++) {
				if (((Band) bands.get(band)) != null
					...
					for (int dest = 0; dest < ((Band) bands.get(band))
							.getBufferBandListToDraw().length; dest++) {
						if (dataImageBand == ((Band) bands.get(band))
								.getBufferBandListToDraw()[dest])
	...
	/**
	 * Checkea si alguna banda de la lista se estça dibujando en el buffer de
	 * salida.
	 * 
	...
	/**
	 * Añade la lista de bandas pasada por parámetro a la lista actual. Si
	 * alguna banda ya existe no la añade y continua.
	 * 
	 * @param bl
	 *            Lista de bandas
	 */
	 ...
	 * 
	 * @param name
	 *            Nombre de las bandas a eliminar
	 */
	 ...
	 * cuando se haga un update. Especificamos a través de los parámetros para
	 * que posición del RasterBuf irá dibujada con que banda del fichero de
	 * imagen.
	 * 
	 * @param posRasterBuf
	...
	public void addDrawableBand(int posRasterBuf, int imageBand) {
		try {
			((Band) bands.get(imageBand))
					.setPositionToDrawInBuffer(posRasterBuf);
	 ...
	 * 
	 * @param i
	 *            Posición de la banda a obtener.
	 * @return Banda.
	 */
	...
	/**
	 * A partir de un nombre de fichero y un número de banda obtiene la banda o
	 * bandas del buffer de salida donde se pinta.
	 * 
	 * @param fileName
	 *            Nombre de fichero
	 * @param band
	 *            Número de banda de la imagen
	...
	/**
	 * Obtiene el tipo de dato de las bandas. Esta llamada supone que todos los
	 * tipos de dato de las bandas son igual por lo que devolverá el primero.
	 * 
	...
	public void show() {
		...
		for (int i = 0; i < getBandCount(); i++) {
			...
			System.out
					.println("DataType: " + ((Band) getBand(i)).getDataType());
			System.out
					.println("FileName: " + ((Band) getBand(i)).getFileName());
			System.out
					.println("Position: " + ((Band) getBand(i)).getPosition());
			if (((Band) getBand(i)).getBufferBandListToDraw() != null) {
				System.out.print("Band Dst: ");
				for (int j = 0; j < ((Band) getBand(i))
						.getBufferBandListToDraw().length; j++)
					System.out.print(((Band) getBand(i))
