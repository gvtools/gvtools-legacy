...
/**
 * Rectangulo de pixeles. Para cada tipo de datos java hay un buffer distinto
 * donde cada elemento es accedido de la siguiente forma: [banda][fila][columna]
 * m[1][2][0] = cte;-> Sustituye el elemento de la fila 2 de la banda 1 columna
 * 0 m[1][0] = array; -> Sustituye la fila 0 de la banda 1 m[0] = matriz
 * cuadrada; -> Sustituye la banda entera.
 * 
 */
public abstract class RasterBuffer implements IBuffer {
	...
	/**
	 * Variable estática que si está a false desactiva el uso de caché. Puede
	 * ser usada por un cliente para cargar siempre los datos en memoria.
	 * independientemente de su tamaño.
	 */
	public static boolean cacheOn = true;
	/**
	 * Fuerza la carga de los datos en caché independientemente de su tamaño. Su
	 * uso suele ser util solo para depuración. Su valor por defecto y
	 * recomendado es siempre false.
	 */
	public static boolean forceToLoadInCache = false;
	/**
	 * Fuerza la carga de los datos en caché de solo lectura independientemente
	 * de su tamaño. Su uso suele ser util solo para depuración. Su valor por
	 * defecto y recomendado es siempre false.
	 */
	public static boolean forceToLoadInReadOnlyCache = false;
	/**
	 * Valor con el que se rellena una banda no valida del buffer. Una banda no
	 * valida es la que no tiene datos asignados y tampoco puede ser null. Todas
	 * las bandas no validas de un buffer apuntan por referencia a la misma
	 * banda.
	 */
	...
	/**
	 * Genera instancias del buffer de datos adecuado al tamaño del raster. Si
	 * no hay muchos datos (menos de cacheMemorySize) creará un buffer en
	 * memoria. Si hay más de esta cantidad entonces crearemos un buffer
	 * cacheado (RasterCache). A partir de la cantidad señalada por
	 * multicacheMemorySize haremos un buffer cacheado donde cada página no
	 * ocupa todo el ancho del raster ya que este será muy grande. La gestión de
	 * una cache donde cada pagina ha de partir una línea lleva una complejidad
	 * añadida.
	 * 
	 * @param dataType
	 *            Tipo de dato
	 ...
	 *            Banda
	 * @param flag
	 *            En caso de buffers de memoria este flag a true significa que
	 *            se reserva la memoria para el buffer de forma normal y si está
	 *            a false no se reserva por lo que la reserva deberá ser
	 *            posterior.
	 ...
	 */
	public static RasterBuffer getBuffer(int dataType, int width, int height,
			int bandNr, boolean malloc)
	/*
	 ...
	 */{
		...
		if (cacheOn) {
			long size = (long) ((long) RasterUtilities
					.getBytesFromRasterBufType(dataType)
					* (long) width
					* (long) height * (long) bandNr) / 1024L;
			long ms1 = RasterLibrary.cacheSize * 1024L;
			if (size <= ms1)
				return new RasterMemoryBuffer(dataType, width, height, bandNr,
						malloc);
			else
				return new RasterCache(dataType, width, height, bandNr);
		} else
	...
	/**
	 * Genera una instancia del buffer de solo lectura. Este buffer consta de
	 * una cache y unos apuntadores a las páginas en disco. Cuando se accede a
	 * los datos se carga en memoria la página pedida.
	 * 
	 * @param dataType
	 *            Tipo de dato
	 * @param width
	 ...
	 *            Banda
	 */
	public static RasterBuffer getReadOnlyBuffer(int dataType, int width,
			int height, int bandNr) {
		return new RasterReadOnlyBuffer(dataType, width, height, bandNr);
	}
	 ...
	 *            posterior.
	 */
	public static RasterBuffer getMemoryBuffer(int dataType, int width,
			int height, int bandNr, boolean malloc) {
		return new RasterMemoryBuffer(dataType, width, height, bandNr, malloc);
	}

	/**
	 * Devuelve true si el tamaño del dataset es menor que el de la caché y
	 * false si no lo es.
	 * 
	 * @param datasource
	 *            Fuente de datos
	...
	/**
	 * Dadas unas coordenadas pixel y un número de bandas, esta función
	 * comprueba si el tamaño de ventana que va a generarse supera el tamaño de
	 * la caché
	 * 
	 * @param coords
	 *            Coordenadas pixel del raster
	 * @param bands
	 *            Número de bandas
	 * @param ds
	 * @return true si la ventana supera el tamaño de la caché o false si no lo
	 *         supera.
	 */
	 ...
	 *         supera.
	 */
	public static boolean isBufferTooBig(double[] coords, double resolution,
			int bands) {
	...
	/**
	 * Obtiene el tipo de dato. Los tipos de dato posibles están definidos en
	 * IRaster.
	 * 
	...
	/**
	 * Asigna el tipo de dato. Los tipos de dato posibles están definidos en
	 * IRaster.
	 * 
	 * @param dataType
	...
	/**
	 * Replica la banda de una posición sobre otra. Si la banda de destino no
	 * existe se crea nueva. Si la posición de la banda de destino está
	 * intercalada entre bandas que ya existen las otras se desplazan hacia
	 * abajo, NO se machacan los datos de ninguna. Los datos se replican por
	 * referencia por lo que al modificar la banda original las del resto
	 * quedarán afectadas.
	 * 
	 * @param orig
	 *            . Posición de la banda de origen.
	 * @param dest
	 *            . Posición de la banda destino
	 */
	public abstract void replicateBand(int orig, int dest);

	/**
	 * Cambia bandas de posición. Las posiciones deben existir como bandas del
	 * raster. Cada elemento del array representa una banda existente en el
	 * buffer (de longitud rasterBuf.length) y el valor contenido dentro la
	 * banda que le corresponde. Por ejemplo si pasamos un array {1, 0, 3, 2}
	 * significa que el buffer tiene cuatro bandas y que cambiamos la 0 por la 1
	 * y la 2 por la 3. Un array {0, 1, 2, 3} en el mismo caso no produciría
	 * nigún cambio.
	 * 
	 * Si quisieramos asignar en un buffer monobanda su banda a la segunda
	 * posición habria que insertar una vacia, por ejemplo con addBandFloat(0,
	 * null) se insertaria una banda nula en la posición 0 y la banda que estaba
	 * en la 0 pasaría a la segunda.
	 * 
	 */
	...
	/**
	 * Ajusta el área del grid a un ancho y un alto dado en pixeles. Este ajuste
	 * se hará en relación a un método de interpolación definido en el
	 * parámetro.
	 * 
	 * @param w
	 ...
	 * @param interpolation
	 *            Método de interpolación que se usará en el ajuste.
	 */
	...
	public Histogram getHistogram() throws InterruptedException {
		...
		hist = new Histogram(getBandCount(), limits[0], limits[1],
				getDataType());
			...
			for (int row = 0; row < getHeight(); row++) {
				...
				case IBuffer.TYPE_BYTE:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								(double) (getElemByte(row, col, iBand)));
					break;
				case IBuffer.TYPE_SHORT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								(double) (getElemShort(row, col, iBand)));
					break;
				case IBuffer.TYPE_INT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								(double) (getElemInt(row, col, iBand)));
					break;
				case IBuffer.TYPE_FLOAT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								(double) getElemFloat(row, col, iBand));
					break;
				case IBuffer.TYPE_DOUBLE:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								getElemDouble(row, col, iBand));
					break;
				}
				...
				progressHistogram = ((iBand * getHeight() + row) * 100)
						/ (getHeight() * getBandCount());
			}
		}
	...
	public void resetPercent() {
		switch (process) {
		case HISTOGRAM_PROCESS:
			progressHistogram = 0;
		case INTERPOLATION_PROCESS:
			progressInterpolation = 0;
		}
	}
