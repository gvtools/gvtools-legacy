 ...
 */
/**
 * Esta clase representa un buffer de datos cacheado. El estar en cache
 * significa que solo una parte de los datos estarán en memoria y que gestiona,
 * dependiendo de las peticiones, que partes hay que cargar a memoria y en que
 * momento. Para esto el buffer no se llena de golpe sino que utiliza una clase
 * que sirve los datos desde la fuente. Esta clase servidora de datos debe
 * implementar el interfaz ICacheDatasetSourcepara servir datos de la forma
 * requerida.
 * 
...
public class RasterCache extends RasterBuffer {
	...
	public class CacheBand extends RasterBand {
		...
		public void setFileName(int numBand) {
			for (int i = 0; i < cacheDataServer.length; i++)
				((CacheDataServer) cacheDataServer[i])
						.setName(null, numBand, i);
		}
	}
	...
	/**
	 * Borra la caché cuando se elimina el objeto RasterCache ya que los trozos
	 * ya no pueden ser referenciados.
	 */
	...
	public void setLineDouble(double[][] data, int line)
			throws InterruptedException {
		if (exists(line)) {
			cache.getAccessPage().setLineDouble(data,
					(line & cache.getOffset()));
		}
	}
	...
	public void getElemByte(int line, int col, byte[] data)
		...
		cache.getAccessPage()
				.getElemByte((line & cache.getOffset()), col, data);
	}
	...
	public void getElemShort(int line, int col, short[] data)
		...
		cache.getAccessPage().getElemShort((line & cache.getOffset()), col,
				data);
	}
	...
	public void getElemFloat(int line, int col, float[] data)
		...
		cache.getAccessPage().getElemFloat((line & cache.getOffset()), col,
				data);
	}
	...
	public void getElemDouble(int line, int col, double[] data)
		...
		cache.getAccessPage().getElemDouble((line & cache.getOffset()), col,
				data);
	}
	...
	public IBuffer getBandBuffer(int iBand) {
		RasterCache rasterCache = new RasterCache(getDataType(), getWidth(),
				getHeight(), 1);
	...
	public void assign(int band, byte value) throws InterruptedException {
		for (int line = 0; line < height; line++) {
			boolean beginLine = true; // Para acelerar solo comprobará si la
										// página está en caché cada vez que
										// empieza una línea
	...
	public IBand getBand(int band) {
		...
		try {
			...
			// TODO: EXCEPCIONES: Modificar el manejo de excepciones de
			// RasterBuffer para q lance las apropiadas con caché
			return null;
		} catch (InterruptedException e) {
			return null;
		}
	...
	public void copyBand(int nBand, IBand band) {
		if (band instanceof CacheBand) {
			...
			for (int iPage = 0; iPage < cb.cacheDataServer.length; iPage++) {
				cb.cacheDataServer[iPage] = new CacheDataServer(null, nBand,
						iPage);
				String path = ((CacheBand) band).cacheDataServer[iPage]
						.getPath();
				File f = new File(path);
				if (f.exists()) {
					try {
						RasterUtilities.copyFile(path,
								cb.cacheDataServer[iPage].getPath());
					} catch (FileNotFoundException e) {
						// TODO: EXCEPCIONES: Modificar el manejo de excepciones
						// de RasterBuffer para q lance las apropiadas con caché
						e.printStackTrace();
					} catch (IOException e) {
						e.printStackTrace();
					}
	...
	public IBand createBand(byte defaultValue) throws InterruptedException {
		PageBandBuffer pageBuffer = new PageBandBuffer(getDataType(),
				getWidth(), cache.getHPag(), 1, true, 0);
		IBand band = null;
		try {
	...
	public void assignBandToNotValid(int iBand) {
		...
		case IBuffer.TYPE_BYTE:
			loadPage(new Byte((byte) getNotValidValue()), pageBuffer);
			break;
		case IBuffer.TYPE_SHORT:
			loadPage(new Short((short) getNotValidValue()), pageBuffer);
			break;
		case IBuffer.TYPE_INT:
			loadPage(new Integer((int) getNotValidValue()), pageBuffer);
			break;
		case IBuffer.TYPE_FLOAT:
			loadPage(new Float((float) getNotValidValue()), pageBuffer);
			break;
		case IBuffer.TYPE_DOUBLE:
			loadPage(new Double((double) getNotValidValue()), pageBuffer);
			break;
		}
	 ...
	 * 
	 * @param pageBuffer
	 *            Pagina de cache cargada de datos
	 * @param numBand
	 *            Número de banda a la que representa
	 * @return IBand
	 * @throws IOException
	 * @throws InterruptedException
	 */
	private IBand createBand(PageBandBuffer pageBuffer)
	...
	/**
	 * Carga la página con el valor pasado por parámetro. El valor será del
	 * mismo tipo de dato que el buffer actual.
	 * 
	 * @param value
	 *            Valor a inicializar
	 * @param pageBuffer
