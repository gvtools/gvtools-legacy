...
/**
 * Esta clase representa la estructura en memoria de la cache. Contiene el
 * número de páginas así como los subconjuntos en las que se divide. Tendrá
 * también datos como en ancho y alto de cada página y tamaño en memoria de
 * esta.
 * 
...
public class CacheStruct {
	...
	private int[] possibleHeights = { 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024,
	...
	private int nPags = 0;
	/**
	 * Número de estructuras PageBandBuffer que tendrá el objeto PageBuffer.
	 * Cada una de estas corresponde a una banda del buffer cacheada.
	 */
	...
	/**
	 * Número de bits para el desplazamiento de una página. Por ejemplo, una
	 * página de 64 líneas tendrá un bitsPag = 6
	 */
	...
	/**
	 * Calcula los parámetros del tamaño de caché y de página, asi como la
	 * altura de las mismas y el número de grupos que salen en caché.
	 * 
	 * @param nBands
	 ...
	 *            ancho de cada línea completa del raster.
	 */
	public CacheStruct(int nBands, int dataType, int dataSourceWidth,
			int dataSourceHeight) {
		init(nBands, dataType, dataSourceWidth, dataSourceHeight);
	}

	/**
	 * Constructor de una sola banda y sin el calculo de parámetros. Estos deben
	 * ser asignados posteriorimente. Esta llamada es usada en casos especiales
	 * de creación de buffers cacheados.
	 */
	public CacheStruct() {
	}
	 ...
	 */
	public CacheStruct(int nBands, int dataType, int dataSourceWidth,
			int dataSourceHeight, long cacheSize, long pageSize) {
	...
	private void init(int nBands, int dataType, int dataSourceWidth,
		...
		if (dataType == IBuffer.TYPE_BYTE) {
			dataSizePerBand = 1;
		} else if ((dataType == IBuffer.TYPE_SHORT)
				| (dataType == IBuffer.TYPE_USHORT)) {
			dataSizePerBand = 2;
		...
		for (int i = (possibleHeights.length - 1); i >= 0; i--) {
			long size = (long) dataSourceWidth * (long) possibleHeights[i]
					* (long) dataSizePerBand;
	...
	/**
	 * Asigna el número de bits por página para poder calcular el desplazamiento
	 * binario de la dirección de acceso de la petición. Es decir si se solicita
	 * un dato en la línea 36 (en decimal) del raster 100100(en binario) y el
	 * desplazamiento es 4 bits el número de página resultante será 10(en
	 * binario) 2 (en decimal)
	 * 
	 * @param bitsPag
	 */
	...
	/**
	 * Obtiene el valor del desplazamiento Para extraer el desplazamiento de una
	 * dirección (línea de raster) hay que hacer una operación And con con la
	 * altura de la página -1. Por ejemplo, una página de 16 líneas de altura el
	 * desplazamiento será 16 - 1 = 15 porque 15 en binario es 1111.
	 * 
	 * Si queremos acceder a la linea del raster número 83 (1010011) realizando
	 * la operación And con el valor del desplazamiento obtenemos (0001111 &
	 * 1010011 = 0000011), es decir el valor 3 en decimal. Esto quiere decir que
	...
	/**
	 * Asigna el valor del desplazamiento. Para extraer el desplazamiento de una
	 * dirección (línea de raster) hay que hacer una operación And con con la
	 * altura de la página -1. Por ejemplo, una página de 16 líneas de altura el
	 * desplazamiento será 16 - 1 = 15 porque 15 en binario es 1111.
	 * 
	 ...
	 * cuenta que cada banda se trata como una estructura de datos en memoria
	 * distinta para poder hacer switch con las bandas de una forma sencilla
	 * 
	 ...
	 * 
	 * @param tamaño
	 *            de una página de caché correspondiente a una banda
	 */
