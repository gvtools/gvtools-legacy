 ...
 * 
 * @deprecated Esta clase fue creada para que CacheDataServer salvara sus datos
 *             a tif. Se ha cambiado por la escritura de los bytes en disco
 *             porque era más óptimo.
 * 
 *             NO DEBE BORRARSE porque se usa para test pero no debería
 *             utilizarse para el trabajo normal de escritura.
 * 
...
public class WriterBufferCompleteServer implements IDataWriter {
		...
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++)
			for (int j = 0; j < buffer.getHeight(); j++)
				for (int i = 0; i < buffer.getWidth(); i++)
					b[iBand][j * buffer.getWidth() + i] = buffer.getElemByte(j,
							i, iBand);
		return b;
	}
	...
	public short[][] readShortData(int sizeX, int sizeY)
		...
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++)
			for (int j = 0; j < buffer.getHeight(); j++)
				for (int i = 0; i < buffer.getWidth(); i++)
					b[iBand][j * buffer.getWidth() + i] = buffer.getElemShort(
							j, i, iBand);
		return b;
	}
	...
	public int[][] readIntData(int sizeX, int sizeY)
		...
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++)
			for (int j = 0; j < buffer.getHeight(); j++)
				for (int i = 0; i < buffer.getWidth(); i++)
					b[iBand][j * buffer.getWidth() + i] = buffer.getElemInt(j,
							i, iBand);
		return b;
	}
	...
	public float[][] readFloatData(int sizeX, int sizeY)
		...
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++)
			for (int j = 0; j < buffer.getHeight(); j++)
				for (int i = 0; i < buffer.getWidth(); i++)
					b[iBand][j * buffer.getWidth() + i] = buffer.getElemFloat(
							j, i, iBand);
		return b;
	}
	...
	public double[][] readDoubleData(int sizeX, int sizeY)
		...
		for (int iBand = 0; iBand < buffer.getBandCount(); iBand++)
			for (int j = 0; j < buffer.getHeight(); j++)
				for (int i = 0; i < buffer.getWidth(); i++)
					b[iBand][j * buffer.getWidth() + i] = buffer.getElemDouble(
							j, i, iBand);
		return b;
	}
}