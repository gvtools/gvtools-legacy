...
/**
 * Esta clase representa una página de cache.Cada página de caché está compuesta
 * por varios objetos PageBandBuffer donde cada uno es una página que
 * corresponde a una banda de raster cacheada.
 * 
...
public class PageBuffer implements IBuffer {
	 ...
	 * @param malloc
	 */
	public PageBuffer(int dataType, int width, int height, int bandNr,
			boolean malloc, int nHddPags) {
		pageBandBuffer = new PageBandBuffer[bandNr];
		for (int i = 0; i < pageBandBuffer.length; i++)
			pageBandBuffer[i] = new PageBandBuffer(dataType, width, height, 1,
					malloc, i);
	}
	 ...
	 * necesarios. Para esto recorre todas las bandas de la página llamando al
	 * load de cada una.
	 * 
	 * @param nPag
	 ...
	 * recorre todas las bandas de la página llamando al save de cada una.
	 * 
	 * @param nPag
	...
	public Histogram getHistogram() throws InterruptedException {
		...
		for (int iBand = 0; iBand < getBandCount(); iBand++) {
			for (int row = 0; row < getHeight(); row++) {
				...
				case IBuffer.TYPE_BYTE:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
								(double) (getElemByte(row, col, iBand) & 0xff));
					break;
				case IBuffer.TYPE_SHORT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(
								iBand,
								(double) (getElemShort(row, col, iBand) & 0xffff));
					break;
				case IBuffer.TYPE_INT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(
								iBand,
								(double) (getElemInt(row, col, iBand) & 0xffffffff));
					break;
				case IBuffer.TYPE_FLOAT:
					for (int col = 0; col < getWidth(); col++)
						hist.incrementPxValue(iBand,
				...
				if (task.getEvent() != null)
					task.manageEvent(task.getEvent());
				percent = ((iBand * getHeight() + row) * 100)
