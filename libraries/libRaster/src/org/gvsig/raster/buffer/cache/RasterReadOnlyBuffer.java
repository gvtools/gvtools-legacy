...
public class RasterReadOnlyBuffer extends RasterBuffer {
	...
	/**
	 * BufferFactory para las cargas de páginas. Lleva asociado un
	 * IRasterDatasource de un raster de disco.
	 */
	...
	/**
	 * Constructor. Asigna las variables de inicialización y crea la estructura
	 * de la caché con los datos pasados.
	 * 
	 * @param dataType
	 *            Tipo de dato
	 * @param width
	...
	/**
	 * Asigna los parametros relativos al buffer. Estos son el nombre del
	 * fichero asociado y la extensión asignada
	 * 
	 * @param fileName
	 ...
	 *            Extensión
	 */
	public void setBufferParams(IRasterDataSource datasource, Extent ext,
			BandList bandList) throws InvalidSetViewException,
			FileNotExistsException, NotSupportedExtensionException {
		this.bandList = bandList;
		this.dataset = dataset.newDataset();

		if (ext.minX() < dataset.getExtent().minX()
				|| ext.minY() < dataset.getExtent().minY()
				|| ext.maxX() > dataset.getExtent().maxX()
				|| ext.maxY() > dataset.getExtent().maxY())
	 ...
	 * 
	 * @param dataset
	 *            Dataset del fichero asociado
	 * @param minX
	 *            Coordenada pixel X mínima
	 * @param minY
	 *            Coordenada pixel Y mínima
	 * @param maxX
	 *            Coordenada pixel X máxima
	 * @param maxY
	 *            Coordenada pixel Y máxima
	 * @exception InvalidSetViewException
	 * @exception FileNotExistsException
	 * @exception NotSupportedExtensionException
	 */
	public void setBufferParams(IRasterDataSource dataset, int minX, int minY,
			int maxX, int maxY, BandList bandList)
			throws InvalidSetViewException, FileNotExistsException,
	...
	/**
	 * Acciones de inicialización. Crear el buffer de memoria y la lista de
	 * bandas, así comno los bits empleados por cada página de memoria y el
	 * desplazamiento que supone para el calculo de direcciones.
	 */
	private void init() {
		...
		// Creamos el bufferFactory asociado a la carga de páginas. Cada página
		// se fuerza
		...
		nPages = (int) Math.ceil((double) height
				/ (double) RasterLibrary.blockHeight);
	...
	public Extent[] calcExtentPages(RasterDataset dataset, int nPages) {
		...
		double h = (RasterLibrary.blockHeight * dataset.getExtent().height())
				/ dataset.getHeight();
		...
		try {
			...
			if (page == null) {
				if (bufWidth == 0 && bufHeight == 0) {
					if (pag == nPages - 1)
						bFactory.setAreaOfInterest(minX,
								(RasterLibrary.blockHeight * pag) + minY, w,
								heightLastPage);
					else
						bFactory.setAreaOfInterest(minX,
								(RasterLibrary.blockHeight * pag) + minY, w,
								RasterLibrary.blockHeight);
				} else {
					int hPage = Math.round((RasterLibrary.blockHeight * h)
							/ bufHeight);
					if (pag == nPages - 1) {
						int hLastPage = Math.round((heightLastPage * bufHeight)
								/ h);
						bFactory.setAreaOfInterest(minX, (hPage * pag) + minY,
								w, hLastPage, bufWidth, heightLastPage);
					} else
						bFactory.setAreaOfInterest(minX, (hPage * pag) + minY,
								w, hPage, bufWidth, RasterLibrary.blockHeight);
				}
				...
				if (bufWidth == 0 && bufHeight == 0) {
					if ((pag + 1) == nPages - 1)
						bFactory.setAreaOfInterest(minX,
								(RasterLibrary.blockHeight * (pag + 1)) + minY,
								w, heightLastPage);
					else
						bFactory.setAreaOfInterest(minX,
								(RasterLibrary.blockHeight * (pag + 1)) + minY,
								w, RasterLibrary.blockHeight);
				} else {
					int hPage = Math.round((RasterLibrary.blockHeight * h)
							/ bufHeight);
					if (pag == nPages - 1) {
						int hLastPage = Math.round((heightLastPage * bufHeight)
								/ h);
						bFactory.setAreaOfInterest(minX, (hPage * (pag + 1))
								+ minY, w, hLastPage, bufWidth, heightLastPage);
					} else
						bFactory.setAreaOfInterest(minX, (hPage * (pag + 1))
								+ minY, w, hPage, bufWidth,
								RasterLibrary.blockHeight);
				}
				...
				if (pag == loadedSecondPage)
					...
					if (bufWidth == 0 && bufHeight == 0) {
						if (pag == nPages - 1)
							bFactory.setAreaOfInterest(minX,
									(RasterLibrary.blockHeight * pag) + minY,
									w, heightLastPage);
						else
							bFactory.setAreaOfInterest(minX,
									(RasterLibrary.blockHeight * pag) + minY,
						...
						int hPage = Math
								.round((RasterLibrary.blockHeight * (maxY - minY))
										/ bufHeight);
						if (pag == nPages - 1) {
							int hLastPage = Math
									.round((heightLastPage * bufHeight) / h);
							bFactory.setAreaOfInterest(minX, (hPage * pag)
									+ minY, w, hLastPage, bufWidth,
									heightLastPage);
						} else
							bFactory.setAreaOfInterest(minX, (hPage * pag)
									+ minY, w, hPage, bufWidth,
			...
			// Al interrumpir la carga de datos de un buffer o producirse una
			// excepción page quedará como null.
			// Tenemos que asignarle algún valor para que las últimas consultas
			// no den error.
			if (page == null) {
				page = RasterBuffer.getBuffer(dataType, width, height, nBands,
						true);
				loadedPage = pag;
			}
		}
