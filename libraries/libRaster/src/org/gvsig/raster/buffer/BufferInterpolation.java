...
public class BufferInterpolation {
	 ...
	 * Promedia el valor de dos pixeles contiguos.
	 * 
	 * @param w
	 *            Nuevo ancho
	 * @param h
	...
	public RasterBuffer adjustRasterNearestNeighbourInterpolation(int w, int h)
		...
		RasterBuffer rasterBuf = RasterBuffer.getBuffer(buffer.getDataType(),
				w, h, buffer.getBandCount(), true);
				...
				if (w <= buffer.width) { // submuestreo
					for (int iRow = 0; iRow < buffer.height; iRow++) {
						for (int iCol = 0; iCol < buffer.width; iCol++)
							rasterBuf.setElem((int) (iRow * stepY),
									(int) (iCol * stepX), bands[iBand],
									buffer.getElemByte(iRow, iCol, iBand));
					...
					for (int iRow = 0; iRow < h; iRow++) {
						for (int iCol = 0; iCol < w; iCol++)
							rasterBuf.setElem(iRow, iCol, bands[iBand], buffer
									.getElemByte((int) (iRow / stepY),
											(int) (iCol / stepX), iBand));
						percent = (iBand * buffer.height + iRow) * multip;
						if (task.getEvent() != null)
							task.manageEvent(task.getEvent());
					}
				...
				if (w <= buffer.width) { // submuestreo
					for (int iRow = 0; iRow < buffer.height; iRow++) {
						for (int iCol = 0; iCol < buffer.width; iCol++)
							rasterBuf.setElem((int) (iRow * stepY),
									(int) (iCol * stepX), bands[iBand],
									buffer.getElemDouble(iRow, iCol, iBand));
						percent = (iBand * buffer.height + iRow) * multip;
						if (task.getEvent() != null)
							task.manageEvent(task.getEvent());
					}
				...
				if (w <= buffer.width) { // submuestreo
					for (int iRow = 0; iRow < buffer.height; iRow++) {
						for (int iCol = 0; iCol < buffer.width; iCol++)
							rasterBuf.setElem((int) (iRow * stepY),
									(int) (iCol * stepX), bands[iBand],
									buffer.getElemFloat(iRow, iCol, iBand));
						percent = (iBand * buffer.height + iRow) * multip;
						if (task.getEvent() != null)
							task.manageEvent(task.getEvent());
					}
				...
				if (w <= buffer.width) { // submuestreo
					for (int iRow = 0; iRow < buffer.height; iRow++) {
						for (int iCol = 0; iCol < buffer.width; iCol++)
							rasterBuf.setElem((int) (iRow * stepY),
									(int) (iCol * stepX), bands[iBand],
									buffer.getElemInt(iRow, iCol, iBand));
						percent = (iBand * buffer.height + iRow) * multip;
						if (task.getEvent() != null)
							task.manageEvent(task.getEvent());
					}
				...
				if (w <= buffer.width) { // submuestreo
					for (int iRow = 0; iRow < buffer.height; iRow++) {
						for (int iCol = 0; iCol < buffer.width; iCol++)
							rasterBuf.setElem((int) (iRow * stepY),
									(int) (iCol * stepX), bands[iBand],
									buffer.getElemShort(iRow, iCol, iBand));
						percent = (iBand * buffer.height + iRow) * multip;
						if (task.getEvent() != null)
							task.manageEvent(task.getEvent());
					}
	...
	/**
	 * Ajusta el raster al ancho y alto solicitado ajustando con una
	 * interpolación bilineal. Promedia el valor de cuatro pixeles adyacentes.
	 * <P>
	 * Para cada pixel del raster A:(x, y) obtiene el B:(x + 1, y), C:(x, y +
	 * 1), D:(x + 1, y + 1) Para cada valor del kernel se calcula un valor 'd'
	 * que es un peso dependiendo de su posición. Este peso depende de la
	 * posición del pixel destino dentro del origen. La posición del pixel
	 * destino en el origen es un valor decimal que puede ir de 0 a 1. Si está
	 * muy pegado a la esquina superior izquierda estará cercano a 0 y si está
	 * muy pegado a la esquina inferior derecha estará cercano a 1. Este valor
	 * está representado por 'dx' y 'dy'.
	 * </P>
	 * <P>
	 ...
	 * cuatro pixeles. El valor final del pixel será 'z/n', es decir un promedio
	 * del valor de los cuatro teniendo en cuenta el peso de cada uno.
	 * </P>
	 * 
	 * @param w
	 *            Nuevo ancho del buffer de salida
	 * @param h
	 *            Nuevo alto del buffer de salida
	 */
			...
			case RasterBuffer.TYPE_BYTE:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[] kernel = getKernelByte(((int) posX),
									((int) posY), iBand);
							rasterBuf.setElem(
									iRow,
									iCol,
									iBand,
									(byte) ((byte) getBilinearValue(dx, dy,
											kernel) & 0xff));
			...
			case RasterBuffer.TYPE_SHORT:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[] kernel = getKernelShort(((int) posX),
									((int) posY), iBand);
							rasterBuf.setElem(
									iRow,
									iCol,
									iBand,
									(short) ((short) getBilinearValue(dx, dy,
											kernel) & 0xffff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
			...
			case RasterBuffer.TYPE_INT:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[] kernel = getKernelInt(((int) posX),
									((int) posY), iBand);
							rasterBuf.setElem(
									iRow,
									iCol,
									iBand,
									(int) ((int) getBilinearValue(dx, dy,
											kernel) & 0xff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
			...
			case RasterBuffer.TYPE_FLOAT:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[] kernel = getKernelFloat(((int) posX),
									((int) posY), iBand);
							rasterBuf.setElem(iRow, iCol, iBand,
									(float) getBilinearValue(dx, dy, kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						posX += pxSize;
			...
			case RasterBuffer.TYPE_DOUBLE:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[] kernel = getKernelDouble(((int) posX),
									((int) posY), iBand);
							rasterBuf.setElem(iRow, iCol, iBand,
									(double) getBilinearValue(dx, dy, kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						posX += pxSize;
	...
	/**
	 * Ajusta el raster al ancho y alto solicitado ajustando con una
	 * interpolación de distancia inversa. Asigna el valor de un pixel en
	 * función inversa de la distancia.
	 * <P>
	 * Para cada pixel del raster A:(x, y) obtiene el B:(x + 1, y), C:(x, y +
	 * 1), D:(x + 1, y + 1) Para cada valor del kernel se calcula un valor 'd'
	 * que es un peso dependiendo de su posición. Este peso será dependiente de
	 * la posición del pixel destino dentro del origen. La posición del pixel
	 * destino en el origen es un valor decimal que puede ir de 0 a 1. Si está
	 * muy pegado a la esquina superior izquierda estará cercano a 0 y si está
	 * muy pegado a la esquina inferior derecha estará cercano a 1. Este valor
	 * está representado por 'dx' y 'dy'. En este caso, y a diferencia del
	 * método bilinear el peso vendrá representado por la inversa de la
	 * distancia entre la posición dentro del pixel y el origen del mismo.
	 * </P>
	 * <P>
	 * Los pesos aplicados son a
	 * <UL>
	...
	public RasterBuffer adjustRasterInverseDistanceInterpolation(int w, int h)
		...
		double pxSize = (double) buffer.width / (double) w;
		RasterBuffer rasterBuf = RasterMemoryBuffer.getBuffer(
				buffer.getDataType(), w, h, buffer.getBandCount(), true);
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							...
							rasterBuf.setElem(
									...
									iCol,
									iBand,
									(byte) ((byte) getInverseDistanceValue(dx,
											dy, kernel) & 0xff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							...
							rasterBuf.setElem(
									...
									iCol,
									iBand,
									(short) ((short) getInverseDistanceValue(
											dx, dy, kernel) & 0xffff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							...
							rasterBuf.setElem(
									...
									iCol,
									iBand,
									(int) ((int) getInverseDistanceValue(dx,
											dy, kernel) & 0xff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							...
							rasterBuf.setElem(
									...
									iCol,
									iBand,
									(float) getInverseDistanceValue(dx, dy,
											kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							...
							rasterBuf.setElem(
									...
									iCol,
									iBand,
									(double) getInverseDistanceValue(dx, dy,
											kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
	...
	/**
	 * Ajusta el raster al ancho y alto solicitado ajustando con una
	 * interpolación BSpline. Promedia el valor de cuatro pixeles adyacentes.
	 * 
	 * @param w
	 *            Nuevo ancho
	 * @param h
			...
			case RasterBuffer.TYPE_BYTE:
				for (int iRow = 0; iRow < h; iRow++) {
					...
					for (int iCol = 0; iCol < w; iCol++) {
						dx = posX - ((int) posX);
						try {
							double[][] kernel = get4x4Submatrix(((int) posX),
									((int) posY), iBand);
							if (kernel == null) {
								double[] k = getKernelByte(((int) posX),
										((int) posY), iBand);
								rasterBuf.setElem(
										iRow,
										iCol,
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(byte) ((byte) getBSplineValue(dx, dy,
												kernel) & 0xff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							double[][] kernel = get4x4Submatrix(((int) posX),
									((int) posY), iBand);
							if (kernel == null) {
								double[] k = getKernelShort(((int) posX),
										((int) posY), iBand);
								rasterBuf.setElem(
										iRow,
										iCol,
										iBand,
										(short) ((short) getBilinearValue(dx,
												dy, k) & 0xffff));
							} else
								rasterBuf.setElem(
										iRow,
										iCol,
										iBand,
										(short) ((short) getBSplineValue(dx,
												dy, kernel) & 0xffff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							double[][] kernel = get4x4Submatrix(((int) posX),
									((int) posY), iBand);
							if (kernel == null) {
								double[] k = getKernelInt(((int) posX),
										((int) posY), iBand);
								rasterBuf
										.setElem(
												iRow,
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							double[][] kernel = get4x4Submatrix(((int) posX),
									((int) posY), iBand);
							if (kernel == null) {
								double[] k = getKernelFloat(((int) posX),
										((int) posY), iBand);
								rasterBuf.setElem(iRow, iCol, iBand,
										(float) getBilinearValue(dx, dy, k));
							} else
								rasterBuf
										.setElem(
												iRow,
					...
					for (int iCol = 0; iCol < w; iCol++) {
						...
						try {
							double[][] kernel = get4x4Submatrix(((int) posX),
									((int) posY), iBand);
							if (kernel == null) {
								double[] k = getKernelDouble(((int) posX),
										((int) posY), iBand);
								rasterBuf.setElem(iRow, iCol, iBand,
										(double) getBilinearValue(dx, dy, k));
							} else
								rasterBuf
										.setElem(
												iRow,
	...
	/**
	 * Ajusta el raster al ancho y alto solicitado ajustando con una
	 * interpolación de spline bicubica.
	 * 
	 * @param w
	 *            Nuevo ancho
	 * @param h
						...
						try {
							...
							if (kernel == null) {
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(byte) ((byte) getBicubicSplineValue(
												dx, dy, kernel) & 0xff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						...
						try {
							...
							if (kernel == null) {
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(short) ((short) getBicubicSplineValue(
												dx, dy, kernel) & 0xffff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						...
						try {
							...
							if (kernel == null) {
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(int) ((int) getBicubicSplineValue(dx,
												dy, kernel) & 0xffffffff));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						posX += pxSize;
						...
						try {
							...
							if (kernel == null) {
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(float) getBicubicSplineValue(dx, dy,
												kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
						...
						try {
							...
							if (kernel == null) {
								...
								rasterBuf.setElem(
										...
										iCol,
										iBand,
										(double) getBicubicSplineValue(dx, dy,
												kernel));
						} catch (ArrayIndexOutOfBoundsException e) {
							// System.out.println(posX + " " + posY);
						}
	...
	private double getBicubicSplineValue(double dx, double dy, double[][] kernel) {
		...
		for (i = 0; i < 4; i++) {
			...
			c[i] = kernel[1][i] + b1 * dx + b2 * (dx * dx) + b3
					* (dx * dx * dx);
		}
	...
	/**
	 * Calcula los valores N y Z para el método bilinear y obtiene el valor del
	 * pixel como Z / N
	 * 
	 * @param dx
	...
	/**
	 * Calcula los valores N y Z para el método de distancia inversa y calcula
	 * el valor del pixel como Z / N.
	 * 
	 * @param dx
	 *            distancia en X desde el centro del pixel hasta el punto. Es un
	 *            valor entre 0 y 1
	 ...
	 * la imagen , por ejemplo en los bordes devuelve null.
	 * 
	 * @param x
	 * @param y
	 * @param band
			...
			for (ix = 0, px = x - 1; ix < 4; ix++, px++) {
				if (!buffer.isInside(px, py))
					...
					case IBuffer.TYPE_BYTE:
						z_xy[ix][iy] = (buffer.getElemByte(py, px, band) & 0xff);
						break;
					case IBuffer.TYPE_SHORT:
						z_xy[ix][iy] = (buffer.getElemShort(py, px, band) & 0xffff);
						break;
					case IBuffer.TYPE_INT:
						z_xy[ix][iy] = (buffer.getElemInt(py, px, band) & 0xffffffff);
						break;
					case IBuffer.TYPE_FLOAT:
						z_xy[ix][iy] = buffer.getElemFloat(py, px, band);
						break;
					case IBuffer.TYPE_DOUBLE:
						z_xy[ix][iy] = buffer.getElemDouble(py, px, band);
						break;
					}
				}
			}
	 ...
	 * y), (x + 1, y), (x, y + 1), (x + 1, y + 1). Si los pixeles x + 1 o y + 1
	 * se salen del raster de origen se tomará x e y.
	 * 
	 * @param x
