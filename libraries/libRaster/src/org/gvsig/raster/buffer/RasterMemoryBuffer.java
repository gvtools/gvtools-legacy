...
/**
 * Implementación del buffer de datos en memoria. Contiene las operaciones
 * necesarias para acceso a datos raster implementando IBuffer y su
 * almacenamiento de datos está basado en arrays tridimensionales. Existe un
 * array para cada tipo de dato pero al instanciarse solo se usará uno de ellos,
 * el que corresponda al tipo de dato del raster manejado. Esto quiere decir que
 * cada RasterMemoryBuffer solo puede contener bandas de un solo tipo de dato
 * donde la variable dataType especificará de que tipo de dato se trata.
 * 
...
public class RasterMemoryBuffer extends RasterBuffer {
	 ...
	 * @param orig
	 */
	public RasterMemoryBuffer(int dataType, int width, int height, int bandNr,
			boolean malloc) {
		if (malloc)
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.dataaccess.buffer.RasterBuffer#malloc(int, int, int,
	 * int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.dataaccess.buffer.RasterBuffer#mallocOneBand(int,
	 * int, int, int)
	 */
	...
	public IBuffer getBandBuffer(int iBand) {
		RasterMemoryBuffer rmb = new RasterMemoryBuffer(dataType, width,
				height, 1, false);
		if (dataType == TYPE_BYTE)
	...
	public IBand createBand(byte defaultValue) {
		...
		case RasterBuffer.TYPE_BYTE:
			ByteBand bb = new ByteBand(width, height, false);
			bb.buf = createByteBand(width, height, defaultValue);
			return bb;
		case RasterBuffer.TYPE_SHORT:
			ShortBand sb = new ShortBand(width, height, false);
			sb.buf = createShortBand(width, height, defaultValue);
			return sb;
		case RasterBuffer.TYPE_INT:
			IntBand ib = new IntBand(width, height, false);
			ib.buf = createIntBand(width, height, defaultValue);
			return ib;
		case RasterBuffer.TYPE_FLOAT:
			FloatBand fb = new FloatBand(width, height, false);
			fb.buf = createFloatBand(width, height, defaultValue);
			return fb;
		case RasterBuffer.TYPE_DOUBLE:
			DoubleBand db = new DoubleBand(width, height, false);
	...
	public double[][] createDoubleBand(int width, int height,
			double defaultValue) {
	...
	/*
	 * public void replaceBandByte(int pos, byte[][] data) { if(pos >=
	 * byteBuf.length) return; byteBuf[pos] = data; }
	 * 
	 * public void replaceBandShort(int pos, short[][] data) { if(pos >=
	 * shortBuf.length) return; shortBuf[pos] = data; }
	 * 
	 * public void replaceBandInt(int pos, int[][] data) { if(pos >=
	 * intBuf.length) return; intBuf[pos] = data; }
	 * 
	 * public void replaceBandFloat(int pos, float[][] data) { if(pos >=
	 * floatBuf.length) return; floatBuf[pos] = data; }
	 * 
	 * public void replaceBandDouble(int pos, double[][] data) { if(pos >=
	 * doubleBuf.length) return; doubleBuf[pos] = data; }
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.IBuffer#copyBand(int,
	 * org.gvsig.fmap.dataaccess.buffer.IBand)
	 */
	public void copyBand(int nBand, IBand band) {
		switch (band.getDataType()) {
		case RasterBuffer.TYPE_BYTE:
			byteBuf[nBand] = new ByteBand(band.getHeight(), band.getWidth(),
					true);
			...
			break;
		case RasterBuffer.TYPE_DOUBLE:
			doubleBuf[nBand] = new DoubleBand(band.getHeight(),
					band.getWidth(), true);
			...
			break;
		case RasterBuffer.TYPE_FLOAT:
			floatBuf[nBand] = new FloatBand(band.getHeight(), band.getWidth(),
					true);
		...
		case RasterBuffer.TYPE_USHORT:
		case RasterBuffer.TYPE_SHORT:
			shortBuf[nBand] = new ShortBand(band.getHeight(), band.getWidth(),
					true);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.gvsig.fmap.driver.IBuffer#assignBand(int,
	 * org.gvsig.fmap.dataaccess.buffer.IBand)
	 */
	...
	public IBuffer cloneBuffer() {
		...
		if (byteBuf != null || shortBuf != null || intBuf != null
				|| floatBuf != null || doubleBuf != null)
			malloc = true;
		RasterMemoryBuffer rmb = new RasterMemoryBuffer(dataType, width,
				height, nBands, malloc);
		for (int iBand = 0; iBand < nBands; iBand++) {
			for (int row = 0; row < height; row++) {
				for (int col = 0; col < width; col++) {
					if (byteBuf != null)
						rmb.setElem(row, col, iBand,
								getElemByte(row, col, iBand));
					if (shortBuf != null)
						rmb.setElem(row, col, iBand,
								getElemShort(row, col, iBand));
					if (intBuf != null)
						rmb.setElem(row, col, iBand,
								getElemInt(row, col, iBand));
					if (floatBuf != null)
						rmb.setElem(row, col, iBand,
								getElemFloat(row, col, iBand));
					if (doubleBuf != null)
						rmb.setElem(row, col, iBand,
