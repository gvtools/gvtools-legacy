...
public class Histogram {
	/**
	 * Variable que representa el histograma. La primera dimensión es el número
	 * de bandas y la segunda un array de clases. En el caso básico de un RGB
	 * una clase tendrá el valor mínimo igual al máximo.
	 */
	 ...
	 *            Número de bandas
	 * @param nClasses
	 *            Número de clases en que hacemos la división
	 * @param min
	...
	private void constructorHistogramImpl(double[] min, double[] max,
			int nClasses) {
	 ...
	 * 
	 * @param hist
	 * @deprecated Con los nuevos maximos y minimos por banda, habria que
	 *             estudiar la viabilidad de este metodo y si se comporta
	 *             correctamente
	 */
	 ...
	 * 
	 * @return array bidimensional donde el primer elemento es el valor del
	 *         pixel o rango y el segundo el número de elementos que aparecen.
	 */
	public HistogramClass[][] getHistogram() {
		...
		for (int i = 0; i < table.length; i++) {
			for (int j = 0; j < table[i].length; j++) {
				HistogramClass hc = new HistogramClass(min[i]
						+ ((j * (max[i] - min[i])) / (nClasses - 1)), min[i]
						+ (((j + 1) * (max[i] - min[i])) / (nClasses - 1)));
	 ...
	 * 
	 * @param band
	 *            Banda o obtener la longitud
	 * @return entero con la longitud de la banda rangos de valores y
	 *         DataclassList.
	 */
	 ...
	 * 
	 * @return entero que representa el número de valores o clases del
	 *         histograma
	 */
	 ...
	 * 
	 * @param band
	 *            Valor del pixel o clase a asignar
	 * @param px
	 *            Valor del pixel
	 * @param value
	 *            Valor a asignar
	 ...
	 *            Valor del pixel o clase a asignar
	 * @param pos
	 *            Posicion dentro de la clase. Ejemplo 0..63
	 * @param value
	 *            Valor a asignar
	 */
	 ...
	 * 
	 * @param band
	 *            Número de banda del valor a recuperar
	 * @param px
	 *            Pixel o valor de la clase del valor a recuperar
	 * @return valor
	 */
	...
	/**
	 * Calculo de estadísticas a partir de un histograma. El resultado de la
	 * función es un array bidimensional donde el primer índice inndica la
	 * estadistica y el segundo el número de banda.
	 * 
	 * <UL>
	 * <LI>mínimo</LI>
	 * <LI>máximo</LI>
	 ...
	 * @param histogram
	 * @param bandas
	 *            solicitadas. Cada elemento del vector representa una banda. Si
	 *            está a true se calcula la estadistica para esa banda y si está
	 *            a false no se calculará.
	 * @return
	 */
	 ...
	 * @param histogram
	 * @param beginPos
	 *            Posición de inicio del histograma para contabilizar
	 *            estadisticas
	 * @param endPos
	 *            Posición de fin del histograma para contabilizar estadisticas
	 * @param bandas
	 *            solicitadas. Cada elemento del vector representa una banda. Si
	 *            está a true se calcula la estadistica para esa banda y si está
	 *            a false no se calculará.
	 * @return
	 */
	public double[][] getBasicStats(double beginPos2, double endPos2,
			boolean[] bands) {
		if (histogram == null)
			getHistogram();
		...
		// Contamos el número de bandas para las cuales se calcula la
		// estadística
		int bandCount = 0;
		...
		double[] average = new double[bandCount]; // Valor de pixel medio
													// (Media)
		double[] middle = new double[bandCount]; // Mediana
		double[] nPixelsBand = new double[bandCount];// Número de pixels por
														// banda

		int showBandCounter = 0; // Contador de bandas de las que hay calcular
									// la
									// estadistica
			...
			if (bands[iBand]) {
				for (int i = beginPos; i <= endPos; i++) {
					...
					if (histogram[iBand][i].getValue() != 0
							&& histogram[iBand][i].getMin() < min[showBandCounter])
						min[showBandCounter] = histogram[iBand][i].getMin();

					// Calculo del máximo
					if (histogram[iBand][i].getValue() != 0
							&& histogram[iBand][i].getMax() > max[showBandCounter])
						max[showBandCounter] = histogram[iBand][i].getMin();

					// Calculo del número de pixeles
					nPixelsBand[showBandCounter] += histogram[iBand][i]
							.getValue();

					average[showBandCounter] += histogram[iBand][i].getValue()
							* ((histogram[iBand][i].getMax() + histogram[iBand][i]
									.getMin()) / 2);
				}
				// Calculo de la media
				try {
				...
				for (i = beginPos; aux <= stopPos; i++) {
					...
					try {
						...
						Logger.getLogger(getClass().getName()).debug(
								"Acceso a un valor de histogram (Banda: "
										+ iBand + " Posición: " + i
										+ ") fuera de límites. (NBandas: "
										+ histogram.length + " Valores:"
										+ histogram[iBand].length + ")", e);
					}
				}
				middle[showBandCounter] = (histogram[iBand][i - 1].getMax() + histogram[iBand][i - 1]
						.getMin()) / 2;
	...
	/**
	 * Obtiene la tabla de valores normalizada. Divide todos los elementos por
	 * el número de pixeles total.
	 * 
	...
	/**
	 * Obtiene la tabla de valores normalizada y acumulada. Divide todos los
	 * elementos por el número de pixeles total.
	 * 
	 * @return tabla de valores normalizada
	 */
	public static double[][] convertTableToNormalizeAccumulate(
			long[][] tableToConvert) {
	...
	public static Histogram convertHistogramToRGB(Histogram histogram) {
		...
		Histogram histogramNew = new Histogram(histogram.getNumBands(), mins,
				maxs, IBuffer.TYPE_BYTE);
