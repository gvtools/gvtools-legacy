...
/**
 * Clase LegendgvSIG para la exportacion e importacion de ficheros rmf. Permite
 * coger cualquier tipo de rmf generado con gvSIG y cargar de ellos solo la
 * tabla de color o sustituir su tabla de color sin perder el rmf.
 * 
 * @version 20/11/2007
...
public class LegendgvSIG extends RasterLegendIO {
	...
	public ColorTable read(File input) throws IOException {
		try {
			return (ColorTable) RasterDataset.loadObjectFromRmfFile(
					input.toString(), ColorTable.class, null);
		} catch (RmfSerializerException e) {
			throw new IOException(
					"No se ha podido leer del RMF la tabla de color");
		}
	}
	...
	public void write(ColorTable colorTable, File output) throws IOException {
		try {
			RasterDataset.saveObjectToRmfFile(output.toString(),
					ColorTable.class, colorTable);
		} catch (RmfSerializerException e) {
			throw new IOException(
					"No se ha podido guardar en el RMF la tabla de color");
		}
	}
}