...
/**
 * Este test prueba el acceso a datos a traves de un grid. A partir de una
 * fuente de datos (cargada en el constructor del Grid). 1-F32 Monobanda: Se
 * hará una petición del extent completo de la fuente seleccionando tres bandas.
 * 2-Comprobamos el tipo de datos el ancho y el alto 3-Modifica algunos valores
 * del grid y se comprueba que las modificaciones han sido efectivas.
 * 4-Cambiamos la banda a operar y obtenemos un dato comprobando que tiene que
 * devolver NoData
 * 
 * 1-RGB: Se hará una petición del extent completo de la fuente seleccionando
 * tres bandas. 2-Comprobamos el tipo de datos el ancho y el alto 3-Modifica
 * algunos valores del grid y se comprueba que las modificaciones han sido
 * efectivas. 4-Cambiamos la banda a operar y obtenemos un dato comprobando que
 * el valor que devuelve es correcto 5 -Comprobamos que si nos salimos devuelve
 * un NoData
 * 
...
public class TGReadingFullDatasource extends TestCase {
	...
	/**
	 * Compara todos los elementos del buffer con los del grid para comprobar la
	 ...
	 */
	private void testCellBufferFloat(Grid g, int band)
		...
		for (int i = 0; i < nRows; i++) {
			for (int j = 0; j < nCols; j++)
				assertEquals((int) g.getCellValueAsFloat(j, i), (int) ds1
						.getRasterBuf().getElemFloat(i, j, band));
		}
	}
	 ...
	 * @throws InterruptedException
	 */
	private void testCellBufferByte(Grid g, int band)
			throws RasterBufferInvalidAccessException, GridException,
			InterruptedException {
		...
		for (int i = 0; i < nRows; i++) {
			for (int j = 0; j < nCols; j++)
				assertEquals((int) g.getCellValueAsByte(j, i), (int) ds2
						.getRasterBuf().getElemByte(i, j, band));
		}
	}
}
