...
/**
 * Test para probar las interpolaciones de un buffer de datos completo. El test
 * creará un buffer a partir de una imagen de 870x870 y aplicará una reducción
 * usando los métodos de interpolación implementados. Después comparará el
 * resultado con imagenes ya generadas y correctas que se encuentran en el
			...
			for (int i = 0; i < b1.getHeight(); i++) {
				for (int j = 0; j < b1.getWidth(); j++) {
					...
					case IBuffer.TYPE_BYTE:
						assertEquals(b1.getElemByte(i, j, k),
								b2.getElemByte(i, j, k));
						break;
					case IBuffer.TYPE_SHORT:
						assertEquals(b1.getElemShort(i, j, k),
								b2.getElemShort(i, j, k));
						break;

					case IBuffer.TYPE_INT:
						assertEquals(b1.getElemInt(i, j, k),
								b2.getElemInt(i, j, k));
						break;

					case IBuffer.TYPE_FLOAT:
						assertEquals((int) b1.getElemFloat(i, j, k),
								(int) b2.getElemFloat(i, j, k));
						break;

					case IBuffer.TYPE_DOUBLE:
						assertEquals((int) b1.getElemDouble(i, j, k),
								(int) b2.getElemDouble(i, j, k));
						break;
					}
				}
			}
