...
/**
 * Test para comprobar la inicialización de la caché. El constructor de
 * CacheStruct es el encargado de crear la estructura de cache. Esta clase
 * comprueba que esta creación sea correcta. Comprueba altura de página, número
 * de estas, y tamaño de las mismas, así como el de la cache completa.
 * 
...
public class TestStructCache extends TestCase {
	...
	public void testStack() {
		...
		if (showCode)
			print(cs, "Size = 20M; Pags Per Group = 5; Page size = 2M", 2048,
					2048, IBuffer.TYPE_BYTE);
		...
		if (showCode)
			print(cs, "Size = 20M; Pags Per Group = 5; Page size = 2M", 5000,
					5000, IBuffer.TYPE_INT);
		...
		if (showCode)
			print(cs, "Size = 20M; Pags Per Group = 5; Page size = 2M", 42000,
					42000, IBuffer.TYPE_FLOAT);
		...
		if (showCode)
			print(cs, "Size = 20M; Pags Per Group = 5; Page size = 2M", 105000,
					105000, IBuffer.TYPE_FLOAT);
		assertEquals(cs.getHPag(), 4);
