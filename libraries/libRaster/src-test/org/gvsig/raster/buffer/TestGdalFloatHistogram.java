 ...
 * coma flotante de 28x25 pixels. Se crean clases de tres tipo de intervalos
 * distintos y se testean los valores obtenidos en el histograma resultante. El
 * último test comprueba la generación automática de clases.
 * 
...
public class TestGdalFloatHistogram extends TestCase {
	...
	public void testStack() {
		 ...
		 * 
		 * try { histogram = rmd.getHistogram(); } catch (HistogramException e)
		 * { e.printStackTrace(); }
		 */
		// testHist1(histogram);
	 ...
	 * 
	 * private void print(Histogram histogram) { for (int i = 0; i <
	 * histogram.getNumBands(); i++) { for (int j = 0; j <
	 * histogram.getBandLenght(i); j++) {
	 ...
	 */
	/*
	 * private void printData() { IBuffer raster = ds.getRasterBuf(); for(int
	 * line = 0; line < raster.getHeight(); line++) { for(int col = 0; col <
	 * raster.getWidth(); col++) System.out.print("(" +
	 * raster.getElemFloat(line, col, 0) + ")"); System.out.println(); } }
	 */
}