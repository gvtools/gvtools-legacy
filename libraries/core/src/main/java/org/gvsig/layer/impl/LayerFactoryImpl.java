...
public class LayerFactoryImpl implements LayerFactory {
	...
	public Layer createLayer(Source source) {
		VectorialLayer ret = newVectorialLayer();
		ret.setSource(source);

		return ret;
	}

	private VectorialLayer newVectorialLayer() {
		return new VectorialLayer(featureSourceCache.get(),
	...
	public Layer createLayer(LayerType xml) {
		if (xml instanceof CompositeLayerType) {
			...
			VectorialLayer layer = newVectorialLayer();
			layer.setXML(xml);
