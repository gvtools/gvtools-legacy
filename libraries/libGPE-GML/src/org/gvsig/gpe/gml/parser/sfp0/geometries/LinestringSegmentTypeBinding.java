...
public class LinestringSegmentTypeBinding extends GeometryBinding {
	public Object parse(IXmlStreamReader parser, GPEDefaultGmlParser handler)
		...
		/********************************************************************
		 * Falta ver como pasar la información del tipo de interpolacion * en el
		 * ContentHandler o en un hashtable de info para el usuario * de la
		 * libreria, o crear una entrada de ContentHandler para cada * uno. *
		 ********************************************************************/
		...
		while (!endFeature) {
			...
			case IXmlStreamReader.START_ELEMENT:
				if (CompareUtils.compareWithNamespace(tag, GMLTags.GML_POSLIST)) {
					PosListTypeIterator coordinatesIterator = handler
							.getProfile().getPosListTypeBinding();
					coordinatesIterator.initialize(parser, handler,
							GMLTags.GML_LINESTRINGSEGMENT);
					linestringsegment = handler.getContentHandler()
							.startLineString(id, coordinatesIterator, srsName);
				}
				break;
