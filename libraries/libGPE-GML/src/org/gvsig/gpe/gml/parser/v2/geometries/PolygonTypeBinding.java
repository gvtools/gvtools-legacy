...
public class PolygonTypeBinding extends GeometryBinding {
	...
	protected Object parseTag(IXmlStreamReader parser,
		...
		if (CompareUtils.compareWithNamespace(tag, GMLTags.GML_OUTERBOUNDARYIS)) {
			OuterBoundaryIsTypeBinding coordinatesBinding = handler
					.getProfile().getOuterBoundaryIsTypeBinding();
			ICoordinateIterator coordinatesIterator = coordinatesBinding.parse(
					parser, handler);
			...
			InnerBoundaryIsTypeBinding coordinatesBinding = handler
					.getProfile().getInnerBoundaryIsTypeBinding();
			ICoordinateIterator coordinatesIterator = coordinatesBinding.parse(
					parser, handler);
