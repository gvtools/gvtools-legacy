	...
	public Object parse(IXmlStreamReader parser, GPEDefaultGmlParser handler)
		...
		while (!endFeature) {
			...
			case IXmlStreamReader.START_ELEMENT:
				...
				if (CompareUtils.compareWithNamespace(tag,
					...
					bbox = handler.getContentHandler().startBbox(id,
						...
						.compareWithNamespace(tag, GMLTags.GML_POS)) {
					PosTypeIterator coordinatesIterator = handler.getProfile()
							.getPosTypeBinding();
					coordinatesIterator.initialize(parser, handler,
							GMLTags.GML_POS);
					bbox = handler.getContentHandler().startBbox(id,
					...
					return bbox;
						...
						GMLTags.GML_LOWERCORNER)) {
					LowerCornerTypeBinding coordinatesIterator = handler
							.getProfile().getLowerCornerTypeBinding();
			...
			case IXmlStreamReader.END_ELEMENT:
				if (CompareUtils
						.compareWithNamespace(tag, GMLTags.GML_ENVELOPE)) {
					endFeature = true;
