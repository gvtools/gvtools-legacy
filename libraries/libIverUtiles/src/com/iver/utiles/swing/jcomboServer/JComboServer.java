...
/**
 * This class is a user interface component that looks equal to a JComboBox
 * component, but it contains ServerData objects and it is able to show them in
 * last access order.It has methods to add ServerData objects and to retrieve
 * them.
 * 
...
public class JComboServer extends JComboBox {
	 ...
	 * 
	 * @param servers
	 *            Array with all the rervers to show
	 */
	...
	/**
	 * This static function return the ordered position of a server. The
	 * algorithm to order can be changed.
	 * 
	 * 
	 * @return Number with the new position
	 * @param servers
	 *            Array that contains all the servers
	 * @param serverPos
	 *            Server position
	 */
	private static int getServerPosition(ServerData[] servers, int serverPos) {
		int pos = 0;
		for (int i = 0; i < servers.length; i++) {
			if (!(servers[serverPos].getServerAddress().equals(servers[i]
					.getServerAddress()))) {
				if (servers[serverPos].getLastAccess().before(
						servers[i].getLastAccess())) {
					pos++;
				}
				if (servers[serverPos].getLastAccess().equals(
						servers[i].getLastAccess())) {
					if (serverPos < i) {
						pos++;
					}
