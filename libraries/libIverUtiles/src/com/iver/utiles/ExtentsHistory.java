...
public class ExtentsHistory {
	 ...
	 * 
	 * @param ext
	 *            Rectángulo que se añade al histórico
	 */
...
/*
 * public class Extents{ private final int NUMREC=20; private
 * Rectangle2D.Double[] extents; private int num=0; public Extents(){ extents =
 * new Rectangle2D.Double[NUMREC]; } public void put (Rectangle2D.Double ext){
 * if((ext!=null)&&((num<1)||(ext!=extents[num-1]))){ if (num<(NUMREC)){
 * extents[num]=ext; num=num+1; }else{ for (int i=0;i<(NUMREC-1);i++){
 * extents[i]=extents[i+1]; } extents[num-1]=ext; } } }
 * 
 * public Rectangle2D.Double get(){ if(num>1){ Rectangle2D.Double ext =
 * extents[num-2]; return ext; }
 * 
 * return null; } public Rectangle2D.Double getPrev(){ if(num>1){
 * Rectangle2D.Double ext = extents[num-2]; num=num-1; return ext; }
 * 
 * return null; } }
 */
