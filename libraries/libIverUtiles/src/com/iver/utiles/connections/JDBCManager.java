...
public class JDBCManager extends JPanel {
	...
	public JDBCManager(ConnectionTrans[] ct) {
		...
		/*
		 * try { Class.forName("com.mysql.jdbc.Driver", true,
		 * JDBCManager.class.getClassLoader()); } catch (ClassNotFoundException
		...
		if (menuDel == null) {
			...
			menuDel.addActionListener(new java.awt.event.ActionListener() {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					connectionDB.delPersistence(ctTree.getHost() + "_"
							+ ctTree.getName());
				}
			});
		}
	...
	private JTabbedPane getParameters() {
		if (tabbedPanel == null) {
			...
			tabbedPanel.addTab(getTranslation("parametros"), null,
					getPparameters(), null);
			// tabbedPanel.addTab(getTraduction("opciones"), null,
			// getPopciones(), null);
		}
		return tabbedPanel;
	}
	...
	private JComboBox getCbDrivers() {
		if (cbDrivers == null) {
			...
			/*
			 * Driver[] drivers=null; try { drivers = getDrivers(); } catch
			 * (DriverLoadException e) {
			 * NotificationManager.addError(PluginServices
			 * .getText(this,"Error obteniendo los drivers JDBC"),e); } if
			 * (drivers!=null)
			 */
			...
			cbDrivers.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					initPort(((ConnectionTrans) cbDrivers.getSelectedItem())
							.getPort());
				}
			});
		}
	...
	private JPanel getPServer() {
		if (pServer == null) {
			...
			pServer.setBorder(javax.swing.BorderFactory.createTitledBorder(
					null, getTranslation("servidor"),
	...
	private JPanel getPhost() {
		if (phost == null) {
			...
			phost.setBorder(javax.swing.BorderFactory.createLineBorder(
	...
	private JTextField getTxtPort() {
		if (txtPort == null) {
			...
			ConnectionTrans ct = (ConnectionTrans) getCbDrivers()
					.getSelectedItem();
	...
	private JPanel getPUser() {
		if (pUser == null) {
			...
			pUser.setBorder(javax.swing.BorderFactory.createLineBorder(
					java.awt.Color.gray, 1));
			...
			bTestConnection
					.addActionListener(new java.awt.event.ActionListener() {
						public void actionPerformed(java.awt.event.ActionEvent e) {
							...
							ct.setDriver(getCbDrivers().getSelectedItem()
									.toString());
							...
							ct.setConnBegining(((ConnectionTrans) getCbDrivers()
									.getSelectedItem()).getConnBeginning());
							...
							if (correct)
								JOptionPane.showMessageDialog(
										(Component) JDBCManager.this,
										getTranslation("conexion_correcta"));
						}
					});
		}
	...
	private ConnectionTrans PaneltoConnectTrans() {
		...
		connectionTrans.setDriver(((ConnectionTrans) getCbDrivers()
		...
		connectionTrans.setConnBegining(((ConnectionTrans) getCbDrivers()
				.getSelectedItem()).getConnBeginning());
		return connectionTrans;
	}

	// public Connection getConnection(String host,String name) throws
	// SQLException{
	...
	private void registerConnection(ConnectionTrans connectionTrans) {
		...
		for (int i = 0; i < root.getChildCount(); i++) {
			// Object
			// obj=((DefaultMutableTreeNode)root.getChildAt(i)).getUserObject();

			if (connectionTrans.equals(ctTree)) {
				same = true;
				if (connectionTrans.isConnected() != ctTree.isConnected()
						|| ctTree.isConnected() == false
						|| connectionTrans.isSavePassword() != ctTree
								.isSavePassword()
						|| connectionTrans.isSavePassword() == false) {
	...
	private JTree getTreeConnection() {
		if (treeConnection == null) {
			...
			// Posibilidad de seleccionar de forma aleatoria nodos de la
			// leyenda.
			...
			treeConnection.addMouseListener(new java.awt.event.MouseAdapter() {
				public void mouseClicked(MouseEvent e) {
					...
					if (path != null) {
						nodeSelected = (DefaultMutableTreeNode) path
					...
					if (e.getButton() == MouseEvent.BUTTON1) {
						...
						if (nodeSelected.getUserObject() instanceof ConnectionTrans) {
							ctTree = (ConnectionTrans) nodeSelected
									.getUserObject();
							init(ctTree);
						}
						// / }
	...
	/*
	 * public ViewInfo getViewInfo() { ViewInfo m_viewinfo = new
	 * ViewInfo(ViewInfo.MODALDIALOG);
	...
	private void modifyTreeNode(ConnectionTrans cs, DefaultMutableTreeNode node)
	...
	void setResourceBundle() {
		try {
			resourceBundle = (PropertyResourceBundle) ResourceBundle
					.getBundle("com.iver.utiles.connections.text");
	...
	public static String getTranslation(String key) {
		...
		try {
			...
			// logger.warn(Messages.getString("PluginServices.No_se_encontro_la_traduccion_para")
			// + " " + key);
	...
	/*
	 * private Driver[] getDrivers() throws DriverLoadException{ Class[] classes
	 * = new Class[] { VectorialJDBCDriver.class };
	 * 
	 * ArrayList ret = new ArrayList(); String[] driverNames =
	 * LayerFactory.getDM().getDriverNames();
	 * 
	 * for (int i = 0; i < driverNames.length; i++) { for (int j = 0; j <
	 * classes.length; j++) { if (LayerFactory.getDM().isA(driverNames[i],
	 * classes[j])) { ret.add(LayerFactory.getDM().getDriver(driverNames[i])); }
	 * } }
	 * 
