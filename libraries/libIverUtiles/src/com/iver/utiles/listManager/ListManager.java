...
public class ListManager {
	...
	private void actualizar() {
		...
		if (index != -1) {
			...
			if (btnProperties != null) {
				btnProperties
						.setEnabled(listModel.getElementAt(index) instanceof Propertiable);
			}
		} else {
			if (btnUp != null) {
	...
	public void setBtnProperties(JButton btn) {
		...
		btnProperties.addActionListener(new java.awt.event.ActionListener() {
			public void actionPerformed(java.awt.event.ActionEvent e) {
				int index = list.getSelectedIndex();
				Object prop = listener.getProperties(listModel
						.getElementAt(index));
				((Propertiable) listModel.getElementAt(index))
						.setProperties(prop);
				list.repaint();
			}
		});
