...
public class TestBinarySearchUsingFirstCharacters extends TestCase {
	...
	protected void setUp() throws Exception {
		...
		// Initialize the vector of Items (for test methods without the
		...
		// stringComparatorForItems.setLocaleRules(stringComparatorForItems.new
		// LocaleRules(true, collator));
		// Collections.sort(v1.subList(0, v1.size()), stringComparatorForItems);

		// The algorithm of sort is the default used by Java (that makes some
		// subsets: one for numbers, one for words that starts in big letters,
		// another that stats in small letters, another with 'special'
		// characters, etc)
	...
	public void test1() {
		try {
			text = "";
			list = BinarySearchUsingFirstCharacters
					.doSearchConsideringCaseSensitive(text, v1);
			...
			if (CompareLists.compare(list, results)) {
				System.out
						.println("Op: doSearchConsideringCaseSensitive(String, Vector) -> OK");
			} else {
				System.out
						.println("Op: doSearchConsideringCaseSensitive(String, Vector) -> FAILED");
				fail();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void test2() {
		try {
			text = "ext";
			list = BinarySearchUsingFirstCharacters
					.doSearchConsideringCaseSensitive("ext", v1);
	...
	// /// TEST OPERATION: doSearchWithoutConsideringCaseSensitive(String,
	// Vector) /////
	...
	public void test11() {
		try {
			text = "";
			list = BinarySearchUsingFirstCharacters
					.doSearchIgnoringCaseSensitive(text, v1);
			...
			if (CompareLists.compareIgnoringCaseSensitive(list, results)) {
				System.out
						.println("Op: doSearchIgnoringCaseSensitive(String, Vector) -> OK");
			} else {
				System.out
						.println("Op: doSearchIgnoringCaseSensitive(String, Vector) -> FAILED");
				fail();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	// /// END TEST OPERATION: doSearchWithoutConsideringCaseSensitive(String,
	// Vector) /////
	...
	public void test21() {
		try {
			...
			list = BinarySearchUsingFirstCharacters
					.doSearchConsideringCaseSensitive(text, v2,
							stringComparator);
			...
			if (CompareLists.compare(list, results, stringComparator)) {
				System.out
						.println("Op: doSearchConsideringCaseSensitive(String, Vector, Comparator) -> OK");
			} else {
				System.out
						.println("Op: doSearchConsideringCaseSensitive(String, Vector, Comparator) -> FAILED");
				fail();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void test29_1() {
		try {
			...
			list = BinarySearchUsingFirstCharacters
					.doSearchConsideringCaseSensitive(text, v2,
							stringComparator); // <- Esta búsqueda
	...
	public void test31() {
		try {
			...
			list = BinarySearchUsingFirstCharacters
					.doSearchIgnoringCaseSensitive(text, v2, stringComparator);
			...
			if (CompareLists.compareIgnoringCaseSensitive(list, results,
					stringComparator)) {
				System.out
						.println("Op: doSearchWithoutConsideringCaseSensitive(String, Vector, Comparator) -> OK");
			} else {
				System.out
						.println("Op: doSearchWithoutConsideringCaseSensitive(String, Vector, Comparator) -> FAILED");
				fail();
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	// /// AUXILIAR CLASS THAT REPRESENTS AN ITEM WITH AN STRING VALUE AND
	// IMPLEMENTING THE COMPARABLE INTERFACE /////

	/**
	 * This class is used for represent an item, with an string value, and
	 * implementing the interface Comparable for allow searches.
	 * 
	 * @author Pablo Piqueras Bartolomé (p_queras@hotmail.com)
	 */
		...
		public int compareTo(Object o) {
			if (!(o instanceof Item))
				throw new ClassCastException(
						"Se esperaba un objeto de tipo Item.");
	...
	public class StringComparatorForItems implements Comparator {
		...
		/**
		 * Establece la sensibilidad del comparador a las mayusculas y
		 * minusculas
		 * 
		 * @param b
		 */
		...
		/**
		 * Represents the information needed by <i>StringComparator</i> for use
		 * or not locale-sensitive String comparison-rules in the
		 * <b><i>compare</i></b> method
		 * 
		 * @author Pablo Piqueras Bartolomé
		 */
		public class LocaleRules {
			 ...
			 *            Use locale rules
			 * @param collator
			 *            A reference to an object configurated for
