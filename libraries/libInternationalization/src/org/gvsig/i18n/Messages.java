...
public class Messages {
	...
	private static ArrayList preferredLocales = new ArrayList(); // contains the
																	// ordered
																	// list of
																	// prefered
																	// languages/locales
	...
	/*
	 * The language considered the origin of translations, which will (possibly)
	 * be stored in a property file without language suffix (ie: text.properties
	 * instead of text_es.properties).
	 */
	...
	public static String getText(String key, String callerName) {
		...
		for (int numLocale = 0; numLocale < localeResources.size(); numLocale++) {
			...
			String translation = ((Properties) localeResources.get(numLocale))
					.getProperty(key);
	...
	public static String getText(String key, String[] arguments,
		...
		if (translation != null) {
			try {
				...
				logger.error(callerName + " -- Error formating key: " + key
						+ " -- " + translation);
			}
		}
		return translation;
	...
	public static String getText(String key, String[] arguments, boolean log) {
		...
		if (translation != null) {
			try {
				...
				if (log) {
					logger.error(_CLASSNAME + " -- Error formating key: " + key
							+ " -- " + translation);
				}
			}
		}
	 ...
	 * translations, which will be (possibly) stored in a property file without
	 * language suffix (ie: text.properties instead of text_es.properties).
	 */
