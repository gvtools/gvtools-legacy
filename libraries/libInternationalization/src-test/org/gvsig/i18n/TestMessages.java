	...
	public void testAddResourceFamily() {
		...
		try {
			Messages.addResourceFamily("text", new File(
					"bin-test/org/gvsig/i18n/dataset1/"));
		...
		TestCase.assertEquals("Debe haber al menos una ventana abierta",
				Messages.getText("cascada_enable"));
		...
		try {
			Messages.addResourceFamily("text", new File(
					"bin-test/org/gvsig/i18n/dataset2/"));
		} catch (MalformedURLException e) {
			TestCase.fail("Fichero de recursos no encontrado");
		}
		...
		TestCase.assertEquals("Cascade", Messages.getText("Cascada"));
		// check the new keys
		TestCase.assertEquals("At least one window should be open",
				Messages.getText("cascada_enable"));
		TestCase.assertEquals("Insert first corner point",
				Messages.getText("insert_first_point_corner"));
		TestCase.assertEquals("Capa exportada",
				Messages.getText("capa_exportada"));
