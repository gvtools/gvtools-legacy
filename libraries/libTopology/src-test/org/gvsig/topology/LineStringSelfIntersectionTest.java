...
/* CVS MESSAGES:
 *
 * $Id: LineStringSelfIntersectionTest.java 19639 2008-03-16 12:46:43Z azabala $
 ...
 * 
 * TODO To avoid robustness problems, is strongly encourage to create JTS
 * geometries in gvSIG with a FIXED precision model (limit the number of
 * significant decimals used in computations).
 * 
 * For example:
 * <ol>
...
public class LineStringSelfIntersectionTest extends TestCase {
	...
	public void testRobustLenghtIndexedLine() throws ParseException {
		...
		RobustLengthIndexedLine indexedLine = new RobustLengthIndexedLine(
				lineString);
		...
		if (checker.hasSelfIntersections()) {
			...
			for (int i = 0; i < selfIntersection.length; i++) {
				...
				System.out.println(coord.toString() + " - index = "
						+ computedIndex);
			}
		}
		// if in this point JTS doesnt launch an exception,
		// RobustLenghtIndexedLine
	...
	public void testPolygonSelfIntersected() throws ParseException {
		...
		if (checker.hasSelfIntersections()) {
			...
			// la solucion contiene un LinearRing (generado por polygonizer) y
			// un
			// LineString que ya estaba cerrado pero que por definicion nunca
			// podrá ser closed
	...
	public void testLineStringWithSelfIntersectionIfSnap()
	...
	public void testLineStringWithOneSnapSelfIntersection()
			throws ParseException {
		...
		LineStringSelfIntersectionChecker checker2 = new LineStringSelfIntersectionChecker(
				(LineString) geoms[0]);
