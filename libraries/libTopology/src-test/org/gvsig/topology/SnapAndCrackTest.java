...
/* CVS MESSAGES:
 *
 * $Id: SnapAndCrackTest.java 19325 2008-02-26 20:58:34Z azabala $
...
public class SnapAndCrackTest extends TestCase {
	...
	public void testCrackTwoIntersectingLinesWithPseudonodes()
			throws ParseException {

		// a) Two linestrings that intersects in points that are not nodes
		// (pseudonodes)
		...
		// Crack and Snap only affect to Vertices (it doesnt compute
		// intersections)
		assertTrue(a.equalsIgnoreCase(cracked[0].toText()));
		assertTrue(b.equalsIgnoreCase(cracked[1].toText()));

		// a1) We split these two lines in their intersections
		// (MustNotHavePseudonodes rule)
		...
		LineString[] linASplittedA = splitter.splitSimple(linA,
				intersectionPoints);
		LineString[] linBSplittedB = splitter.splitSimple(linB,
				intersectionPoints);
		assertTrue(linASplittedA.length == 6);
		assertTrue(linBSplittedB.length == 6);
		assertTrue(intersectionPoints.length == 5);

		// in this point, LineStringSplitter could return splitted linestring
		// with
		...
		// b) Two intersecting lines with vertices that must be cracked and
		// snapped

		// b1) vertex 320,180.001 must be cracked in first geometry
		// FIXME When were going to crack a vertex...¿why dont apply the line
		// equation y=mx+b
		// to avoid the pick 300 180, 320 180.001 360 180

		// b2) In 360,120 both lines intersects (this only will be detected in
		// OverlayOp)
		// FIXME This is a real intersection. In the next test use a snap
		// intersection
		...
		// conclusion: to clean a topology we must: a) snap b) crack c) compute
		// intersections
		...
		for (int i = 0; i < snapped.length; i++) {
			...
			for (int j = 0; j < snapped.length; j++) {
				...
				// AQUI NO USAMOS SNAP.
				Coordinate[] inters = EnhancedPrecisionOp
						.intersection(ls1, ls2).getCoordinates();
				...
				multi2.toText();
				// Esto funciona porque son solo dos. Para hacerlo con muchas
				// lineas, hay que mantener
				// para cada lineString una lista con todas las intersecciones,
				// con independencia
				// de con que geomeetrias sea.. (USAR PARA ESTO EL
				// PREPAREDGEOMETRY DE JTS)
			}// for
		}// for
