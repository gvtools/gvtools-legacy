...
public class LayerFactory {
	...
	public static FLyrVect getLineLayerWithCollapsedCoords()
		...
		FeatureCollectionMemoryDriver driver = new FeatureCollectionMemoryDriver(
				"multipuntos", features, definition);

		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("multipuntos", driver, null);
	}

	public static FLyrVect getLyrWithRepeatedCoords() throws ParseException {
		Geometry multiPoint = wktReader
				.read("MULTIPOINT (520 160, 540 180, 540 180, 540 180, 600 180, 580 140, 640 140, 640 140, 640 160)");
		Geometry multiPoint2 = wktReader
				.read("MULTIPOINT (520 160, 540 180, 600 180, 580 140, 640 140,  640 160)");
		LayerDefinition definition = createTestLayerDefinition();
	...
	public static FLyrVect createPointLyrWhichDontPasJtsValidRule() {
		...
		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("puntosJtsInvalidos", driver, null);
	}
	...
	public static FLyrVect createPolygonLayerWhichDontPassJtsValidRule()
			throws ParseException {
		LayerDefinition definition = createTestLayerDefinition();
		...
		// A polygon with a hole which shell has a self-intersection->TODO el
		// paso jts->fmap->jts hace cosas raras
		Geometry pol1 = wktReader
				.read("POLYGON ((80 140, 360 60, 180 340, 200 60, 240 20, 80 140), (220 200, 220 140, 300 100, 260 180, 220 200))");
		IFeature f1 = createTestFeature(pol1, index);
		index++;

		// Non error polygon
		Geometry pol2 = wktReader
				.read("POLYGON ((200 260, 440 80, 500 360, 380 380, 200 260))");
		IFeature f2 = createTestFeature(pol2, index);
		index++;

		// Polygon with selintersecting hole
		Geometry pol3 = wktReader
				.read("POLYGON ((100 80, 240 20, 460 200, 400 380, 220 400, 60 360, 0 220, 100 80), (100 320, 380 220, 60 180, 320 340, 100 320))");
		IFeature f3 = createTestFeature(pol3, index);
		index++;

		// unclosed polygon Por definicion JTS si comprueba que los linearRing
		// sean cerrados en su construccion
		// Geometry pol4 =
		// wktReader.read("POLYGON ((200 260, 440 80, 500 360, 380 380))");
		...
		// it isnt a polygon. its a line (three collinear points: collapsed
		// polygon)
		// Geometry pol5 =
		// wktReader.read("POLYGON ((200 260, 440 80, 200 260))");
		...
		Geometry pol6 = wktReader
				.read("POLYGON ((100 320, 380 220, 60 180, 320 340, 100 320))");
		IFeature f6 = createTestFeature(pol6, index);
		index++;

		// Polygon with hole in CCW order
		Geometry pol7 = wktReader
				.read("POLYGON ((0 0, 1000 0, 1000 1000, 0 1000, 0 0), (100 80, 240 20, 460 200, 400 380, 220 400, 60 360, 0 220, 100 80))");
		IFeature f7 = createTestFeature(pol7, index);
		index++;

		// Polygon with two holes that touch in more than a point
		Geometry pol8 = wktReader
				.read("POLYGON ((40 380, 40 20, 500 20, 500 380, 40 380),"
		...
		// the hole is ccw
		Geometry pol9 = wktReader
				.read("POLYGON ((160 320, 740 80, 740 340, 400 380, 160 320), (440 260, 600 200, 660 300, 500 300, 440 260))");
		IFeature f9 = createTestFeature(pol9, index);
		index++;

		// hole is not full contained by shell
		Geometry pol10 = wktReader
				.read("POLYGON ((160 320, 740 80, 740 340, 400 380, 160 320), (440 260, 500 300, 660 300, 600 200, 440 260))");
		IFeature f10 = createTestFeature(pol10, index);
		index++;

		// shell and hole have the same coordinate sequence
		Geometry pol11 = wktReader
				.read("POLYGON((440 260, 600 200, 660 300, 500 300, 440 260),(440 260, 500 300, 660 300, 600 200, 440 260))");
		...
		features.add(f2);
		features.add(f3);
		features.add(f4);// jts no permite linearRing no cerrados (esto si lo
							// chequea)
		features.add(f5);// jts no permite linearRings con < 3 puntos diferentes
							// y no colineales
		features.add(f6);
		features.add(f7);
		features.add(f8);
		...
		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("poligonos no validos jts", driver, null);

	}

	public static FLyrVect createLyrForIGeometryMustBeClosedTest()
			throws ParseException {
		...
		Geometry unclosedLine = wktReader
				.read("LINESTRING (280 80, 320 140, 360 220, 520 260, 560 200, 600 140, 500 80, 400 80, 260 60)");
		IFeature f1 = createTestFeature(unclosedLine, index++);

		Geometry overShootLine = wktReader
				.read("LINESTRING (140 220, 420 280, 320 380, 160 380, 80 300, 280 180)");
		...
		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("lineas no cerradas", driver, null);

	}

	public static FLyrVect createLyrForLineMustNotHavePseudonodesTest()
			throws ParseException {
		LayerDefinition definition = createTestLayerDefinition();
		definition.setShapeType(FShape.LINE);
		FeatureCollectionMemoryDriver driver = null;
		...
		driver = new FeatureCollectionMemoryDriver(
				"lineas con pseudonodo en 100 10", features, definition);

		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("lineas con pseudonodo en 100 10", driver, null);

	}

	public static FLyrVect createPolygonForNoSelfIntersectTest()
			throws ParseException {
		LayerDefinition definition = createTestLayerDefinition();
		definition.setShapeType(FShape.POLYGON);
		FeatureCollectionMemoryDriver driver = null;
		...
		Geometry line1 = wktReader
				.read("POLYGON ((-13900 -920, -8300 120, -5020 3460, -9480 5660, -15320 2620, -19420 4960, -19300 7600, -14620 8500, -13180 6840, -16380 5460, -22020 6280, -22340 3060, -18860 2120, -14820 4880, -10200 4460, -8680 2640, -8240 1780, -4420 -420, -5140 -2560, -9480 -3640, -12940 -2740, -13900 -920))");
		IFeature f1 = createTestFeature(line1, index++);
		features.add(f1);
		driver = new FeatureCollectionMemoryDriver(
				"poligonos con autointersecciones", features, definition);

		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("poligonos con autointersecciones", driver, null);
	}

	public static FLyrVect createPolygonForMustNotOverlapTest()
			throws ParseException {
		LayerDefinition definition = createTestLayerDefinition();
		definition.setShapeType(FShape.POLYGON);
		FeatureCollectionMemoryDriver driver = null;
		...
		Geometry poly1 = wktReader
				.read("POLYGON ((290 340, 180 280, 610 230, 720 330, 530 400, 300 380, 290 340))");
		IFeature f1 = createTestFeature(poly1, index++);
		features.add(f1);

		Geometry poly2 = wktReader
				.read("POLYGON ((180 210, 550 210, 200 390, 180 210))");
		IFeature f2 = createTestFeature(poly2, index++);
		features.add(f2);

		Geometry poly3 = wktReader
				.read("POLYGON ((820 180, 820 30, 980 30, 1000 180, 820 180))");
		...
		return (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("poligonos que solapan", driver, null);
	}

	public static FLyrVect createPolygonForMustNotHaveGapsTest()
			throws ParseException {
		...
		driver = new FeatureCollectionMemoryDriver("poligonos 1", features,
				definition);

		solution = (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("poligonos que solapan", driver, null);
		return solution;
	}

	public static FLyrVect[] createPolygonForMustNotOverlapWithTest()
			throws ParseException {
		...
		index = 0;
		Geometry poly11 = wktReader
				.read("POLYGON ((20 210, 30 50, 320 30, 330 180, 20 210))");
		IFeature f11 = createTestFeature(poly11, index++);
		features2.add(f11);

		Geometry poly22 = wktReader
				.read("POLYGON ((880 380, 920 230, 1000 210, 840 10, 880 380))");
		IFeature f22 = createTestFeature(poly22, index++);
		features.add(f22);
		driver2 = new FeatureCollectionMemoryDriver("poligonos 1", features2,
				definition);

		solution[1] = (FLyrVect) com.iver.cit.gvsig.fmap.layers.LayerFactory
				.createLayer("poligonos que solapan", driver2, null);
	...
	public static FLyrVect createLayerFor(List<IGeometry> geometries,
		...
		for (int i = 0; i < geometries.size(); i++) {
			...
			DefaultFeature f = new DefaultFeature(geometry, values, i + "");
			features.add(f);
