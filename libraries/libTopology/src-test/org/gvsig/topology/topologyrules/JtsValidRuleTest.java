...
public class JtsValidRuleTest extends TestCase {
	...
	public void testJtsInvalidPointLayer() {
		FLyrVect pointLyr = LayerFactory
				.createPointLyrWhichDontPasJtsValidRule();
	...
	public void testJtsInvalidLineLayer() {
		/*
		 * This line layer has two JTS errors: A line has invalid coords.
	...
	public void testPolygonMustNotHaveSelfIntersectedRing()
	...
	public void testMultiPartShapeIterator() {
		...
		while (shapeIt.hasNext()) {
			...
			List<Point2D[]> coordParts = ShapePointExtractor
					.extractPoints(shape);
			if (i == 0)
	...
	public void testMultiPartShapeIteratorWithCurves() {
		...
		assertTrue(part1.length != 4);// 1 point + 3 control points, but
										// extractPoints returns computed curves
		...
		assertTrue(part2.length != 3);// 1 point + 3 control points, but
										// extractPoints returns computed curves
	...
	public void testPolygonHolesMustNotBeNested() throws ParseException {
		...
		MultiPolygon multiPolygon = NewFConverter
				.toJtsPolygon((FShape) geometry.getInternalShape());
	...
	public void testPolygonHolesMustBeInShell() throws ParseException {
		Geometry pol = wktReader
				.read("POLYGON ((300 320, 240 220, 500 80, 660 300, 580 360, 300 320),"
		...
		PolygonHolesMustBeInShell rule = new PolygonHolesMustBeInShell(null,
				0.01d);
		TestTopologyErrorContainer errorContainer = new TestTopologyErrorContainer();
		rule.setTopologyErrorContainer(errorContainer);
		rule.validateFeature(f);
	...
	public void testJtsInvalidPolygonLayer() throws ParseException {
		FLyrVect polLyr = LayerFactory
				.createPolygonLayerWhichDontPassJtsValidRule();
		...
		// assertTrue(error0.getViolatedRule() instanceof
		// PolygonMustNotHaveSelfIntersectedRings);
