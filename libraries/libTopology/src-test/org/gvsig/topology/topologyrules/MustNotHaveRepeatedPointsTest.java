...
public class MustNotHaveRepeatedPointsTest extends TestCase {
	...
	public void setUp() throws Exception {
		super.setUp();
		multiPoint = wktReader
				.read("MULTIPOINT (520 160, 540 180, 540 180, 540 180, 600 180, 580 140, 640 140, 640 140, 640 160)");
		multiPoint2 = wktReader
				.read("MULTIPOINT (520 160, 540 180, 600 180, 580 140, 640 140,  640 160)");
		lineString = wktReader
				.read("LINESTRING (520 120, 560 180, 560 180, 560 180, 620 160, 660 200, 700 140, 700 140, 700 140, 620 120)");
		polygon = wktReader
				.read("POLYGON ((600 180, 600 180, 600 180, 600 180, 540 200, 480 140, 540 120, 540 120, 680 160, 600 180, 600 180))");
	}
	...
	public void testMustNotHaveRepeatedPointsRule() throws ParseException {
		...
		FLyrVect multiPointLayer = org.gvsig.topology.util.LayerFactory
				.getLyrWithRepeatedCoords();
	...
	public void testSnapRepeatedPointTester() throws ParseException {
		...
		Geometry multiPoint = wktReader
				.read("MULTIPOINT (520 160, 540 180, 540.002 179.999, 540.01 179.999, 600 180, 580 140, 640 140, 640 140, 640 160)");
		Geometry lineString = wktReader
				.read("LINESTRING (520 120, 560 180, 560 180.001, 560.001 180, 620 160, 660 200, 700 140, 700 140, 700 140, 620 120)");
		Geometry polygon = wktReader
				.read("POLYGON ((600 180, 600 179.999, 600.004 180, 600 180, 540 200, 480 140, 540 120, 540 120, 680 160, 600 180, 600 180))");
