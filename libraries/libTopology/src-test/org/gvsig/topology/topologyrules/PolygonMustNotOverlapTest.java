...
public class PolygonMustNotOverlapTest extends TestCase {
	...
	public void testDontOverlaps() throws ParseException,
		...
		FLyrVect lyrWithOverlaps = LayerFactory
				.createPolygonForMustNotOverlapTest();
		lyrWithOverlaps.setCrs(DEFAULT_CRS);

		PolygonMustNotOverlap rule = new PolygonMustNotOverlap(null,
				lyrWithOverlaps, 0.1d);
		...
		PolygonMustNotHaveGaps rule2 = new PolygonMustNotHaveGaps(null, lyr2,
				0.1d);
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				Geometry fGeo = NewFConverter.toJtsGeometry(feature
						.getGeometry());
				geometries.add(fGeo);
