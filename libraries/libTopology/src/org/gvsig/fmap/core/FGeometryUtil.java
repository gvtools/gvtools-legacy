...
public class FGeometryUtil {
	...
	/**
	 * This function closes an unclosed geometry, by inserting a new segment in
	 * the gap between the first and the last point.
	 * 
	 * TODO Insted inserting a new segment, MOVE one of the two points. (those
	 * which is owned by the shortest segment?)
	 * 
	 * @param unclosedGeometry
	 *            unclosed geometry
	 * 
	 * @param snapTolerance
	 *            cluster tolerance
	 * @return
	 */
	public static IGeometry closeGeometry(IGeometry unclosedGeometry,
			double snapTolerance) {
		int shapeType = unclosedGeometry.getGeometryType();

		if (shapeType == FShape.POINT || shapeType == FShape.TEXT
		...
		List<Point2D[]> partsCoords = ShapePointExtractor
				.extractPoints(unclosedGeometry);
		for (int i = 0; i < partsCoords.size(); i++) {
			Point2D[] part = partsCoords.get(i);
		...
		if (unclosedGeometry instanceof FGeometry) {
			...
			if (getXyDimensions(fshape) == 1)
				geomSolution = ShapeFactory
						.createGeometry(new FPolyline2D(gpx));
			else
				geomSolution = ShapeFactory.createGeometry(new FPolygon2D(gpx));
		} else if (unclosedGeometry instanceof FGeometryCollection) {
			// FIXME If its a geometry collection, we need to think what we must
			// to return
			IGeometry[] geoms = new IGeometry[] { ShapeFactory
					.createGeometry(new FPolygon2D(gpx)) };
	 ...
	 * 
	 * @param geometry
	 *            a non closed geometry which have an overshoot
	 * 
	 * @return
	 */
	public static IGeometry removeOvershoot(IGeometry geometry,
		...
		for (int i = 0; i < geoms2process.size(); i++) {
			...
			if (shapeType == FShape.POLYGON || shapeType == FShape.ELLIPSE
					|| shapeType == FShape.POLYGON + FShape.Z) {
				// When we convert from FMap to JTS, if the resulting geometry
				// is a polygon and
				...
				List<Point2D[]> points = ShapePointExtractor
						.extractPoints(nextGeometry);
				List<LinearRing> rings = new ArrayList<LinearRing>();
				for (int j = 0; j < points.size(); j++) {
					Point2D[] pts = points.get(j);
					Coordinate[] coords = JtsUtil.getPoint2DAsCoordinates(pts);
					LineString lineString = JtsUtil.GEOMETRY_FACTORY
							.createLineString(coords);
					LineString correctedLineString = (LineString) JtsUtil
							.removeOverShoot(lineString, clusterTolerance);
					LinearRing correctedRing = JtsUtil
							.toLinearRing(correctedLineString);
				...
				Geometry correctedGeom = JtsUtil.removeOverShoot(jtsGeom,
						clusterTolerance);
	...
	/**
	 * Tells if a vertex is in the boundary of a geometry (perimeter or hole
	 * perimeer)
	 * 
	 * @param geometry
	 * @param point2d
	 * @return
	 */
	public static boolean isInBoundary(IGeometry geometry, Point2D point2d,
			double tolerance) {
		Geometry jtsGeom = NewFConverter.toJtsGeometry(geometry);
		Coordinate coord = new Coordinate(point2d.getX(), point2d.getY());
		Coordinate base = new Coordinate(coord.x - tolerance, coord.y
				- tolerance);
		GeometricShapeFactory shpFactory = new GeometricShapeFactory(
	...
	/**
	 * Inserts a new vertex in a segment of the geometry boundary. It doesnt
	 * moves nothing, it only inserts a new collineaer vertex inside a segment.
	 * 
	 * It is useful to make topological edition, ensuring two geometries shares
	 * a vertex.
	 * 
	 * @param geometry
	 * @param newVertex
		...
		while (shapeIt.hasNext()) {
			...
			for (int i = 0; i < partsCoords.size(); i++) {
				...
				for (int j = 0; j < part.length - 1; j++) {
					...
					IGeometry lineSegment = ShapeFactory
							.createPolyline2D(gpxAux);
	...
	/**
	 * From a given geometry with dimension 1 or 2, it computes a parametric
	 * curve and return a smoothed version of the original geometry.
	 * 
	 * To draw the returned geometry it uses a usual GeneralPath, being the
	 * FLATNESS factor of the returned curve FConverter.FLATNESS.
	 * 
	 * @param originalGeometry
	 * @return
	 */
	public static IGeometry smoothGeometry(IGeometry originalGeometry,
			int curveOption) {
		List<Point2D[]> pointsParts = ShapePointExtractor
				.extractPoints(originalGeometry);
		IGeometry[] smoothedCurves = new IGeometry[pointsParts.size()];
		for (int i = 0; i < pointsParts.size(); i++) {
			FCurve curve = new FCurve(pointsParts.get(i), curveOption);
	...
	public static void drawLineWithArrow(Graphics2D g,
			AffineTransform affineTransform, FShape shp, Color arrowColor,
			Stroke stroke, double lenghtArrow, double widthArrow) {
		...
		while (!theIterator.isDone()) {
			...
			if (theType == PathIterator.SEG_LINETO
					|| theType == PathIterator.SEG_MOVETO)
		...
		double tip1x = mx
				+ (((Mx - mx) * tipLength + (tipWidth / 2) * (my - My)) / Math
						.sqrt((my - My) * (my - My) + (mx - Mx) * (mx - Mx)));
		...
		double tip1y = my
				+ (((My - my) * tipLength - (tipWidth / 2) * (mx - Mx)) / Math
						.sqrt((my - My) * (my - My) + (mx - Mx) * (mx - Mx)));
