...
/**
 * All end points of a line must touch at least another line. ( if only touch
 * one line, its a pseudonode, not a dangle) This rule is checked for the two
 * ends of a line.
 * 
 */
public class LineMustNotHaveDangles extends AbstractTopologyRule implements
	 ...
	 * 
	 */
	public LineMustNotHaveDangles(Topology topology, FLyrVect originLyr,
			double clusterTol) {
		...
		try {
			...
			if (FGeometryUtil.getDimensions(shapeType) != 1
				...
				throw new TopologyRuleDefinitionException(
						"LineMustNotHaveDangles requires a lineal geometry type");
	...
	/**
	 * Utility class to check if a given linestring is a dangle (it has
	 * unconected extremes)
	 * 
	...
	private void process(Geometry geometry, IFeature feature) {
		if (geometry instanceof GeometryCollection) {
			...
			Rectangle2D rect = NewFConverter
					.convertEnvelopeToRectangle2D(lineString
							.getEnvelopeInternal());
			...
			try {
				...
				SnapLineStringSelfIntersectionChecker selfIntChecker = new SnapLineStringSelfIntersectionChecker(
						lineString, clusterTol);
				...
				if (!checker.ends1) {// dangling node
					...
					if (selfIntChecker.hasSelfIntersections()) {
						Coordinate[] selfs = selfIntChecker
								.getSelfIntersections();
						for (int i = 0; i < selfs.length; i++) {
							if (SnapCGAlgorithms.snapEquals2D(
									checker.end1.getCoordinate(), selfs[i],
									clusterTol)) {
					...
					if (!isSelfIntersection) {
						IGeometry errorGeom = ShapeFactory.createPoint2D(
								checker.end1.getX(), checker.end1.getY());
				...
				if (!checker.ends2) {// dangling node
					...
					if (!isSelfIntersection) {
						IGeometry errorGeom = ShapeFactory.createPoint2D(
								checker.end2.getX(), checker.end2.getY());
						TopologyError topologyError = new TopologyError(
								errorGeom, this, feature, topology);
						topologyError.setID(errorContainer.getErrorFid());
						addTopologyError(topologyError);
	...
	private void process2(DangleChecker checker, Geometry intersectingGeom) {
		if (intersectingGeom instanceof LineString) {
			...
			Envelope snapEnvelope = JtsUtil.toSnapRectangle(
					checker.end1.getCoordinate(), clusterTol);
			...
			snapEnvelope = JtsUtil.toSnapRectangle(
					checker.end2.getCoordinate(), clusterTol);
			...
			System.out.println("Encontrado:" + intersectingGeom.toString()
					+ " en regla de dangles");
		}
	}
