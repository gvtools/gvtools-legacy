...
public class LyrMustNotHaveDuplicated extends AbstractTopologyRule implements
		IRuleWithClusterTolerance {

	private static final String RULE_NAME = Messages
			.getText("must_not_have_duplicates");
	...
	public void validateFeature(IFeature feature) {
		...
		try {
			IFeatureIterator neighbours = getOriginLyr().getSource()
			...
			while (neighbours.hasNext()) {
				...
				if (bounds.intersects(rect2)) {
					...
					if (JCSUtil.checkMatchingCriteria(jtsGeom, second,
							clusterTolerance)) {
						TopologyError error = new TopologyError(geom, this,
								feature, getTopology());
