...
public class ExtendDangleToNearestBoundaryPointFix extends
	...
	protected Coordinate getClosestPoint(Geometry original,
			Geometry nearestGeometry, DistanceOp distanceOp) {
	...
	public List<IFeature>[] fixAlgorithm(TopologyError error)
		...
		IFeature nearestFeature = getNearestFeature(jtsDangle, originLyr,
				originFeature);
	...
	private IFeature getNearestFeature(Point jtsDangle, FLyrVect originLyr,
		...
		Rectangle2D dangleRect = new Rectangle2D.Double(jtsDangle.getX(),
				jtsDangle.getY(), 1, 1);
