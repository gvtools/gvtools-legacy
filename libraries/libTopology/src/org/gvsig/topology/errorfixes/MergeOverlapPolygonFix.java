...
public class MergeOverlapPolygonFix extends AbstractTopologyErrorFix implements
		ITopologyErrorFixWithParameters {

	/**
	 * One of the topology error's causing features, which preserve its
	 * geometry.
	 */
	...
	public List<IFeature>[] fixAlgorithm(TopologyError error)
		...
		if (featureToPreserve.getID().equals(firstFeature.getID())) {
			...
			// FIXME USAR JTSUTIL PARA CONTEMPLAR LA POSIBILIDAD DE QUE LLEGUEN
			// MULTIGEOMETRIAS
	...
	public void initialize(TopologyError error) {
		...
		parameters = new GParameter[] { new GFeatureParameter(
				"featureToPreserve", GParameter.FEATURE_PARAM_TYPES,
				error.getFeature1(), this, featureContainers) };
	}
