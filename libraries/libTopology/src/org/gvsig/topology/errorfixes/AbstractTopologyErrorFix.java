...
/**
 * Abstract base class for all those fixes that modifies a topology error's
 * causing feature.
 * 
...
public abstract class AbstractTopologyErrorFix implements ITopologyErrorFix {
	...
	protected EditableAdapter prepareEditableAdapterFromLayer(FLyrVect lyr)
	...
	protected EditableAdapter[] prepareEdition(TopologyError topologyError)
			throws BaseException {
	...
	public void fix(TopologyError topologyError) throws BaseException {
		...
		if (correctedFeatures != null) {
			finalizeEdition(adapters, correctedFeatures,
	...
	/**
	 * It returns a List<IFeature> for each layer related with the topology
	 * error. Index 0 for first layer, Index 1 for second layer, etc
	 * 
	 * @param error
	 * @return
	 * @throws BaseException
	 */
	public abstract List<IFeature>[] fixAlgorithm(TopologyError error)
			throws BaseException;

	public abstract String getEditionDescription();

	protected void finalizeEdition(EditableAdapter[] adapters,
			List<IFeature>[] newFeatures, String editionDescription)
			throws BaseException {
		for (int i = 0; i < adapters.length; i++) {
			...
			for (int j = 0; j < features.size(); j++) {
				...
				// FIXME TopologyError must have the feature index of origin and
				// destination features
				int featureIndex = Integer.parseInt(feature.getID());
				// TODO Probar si la llamada a modifyRow actualiza los features
				// dibujados
				adapter.modifyRow(featureIndex, feature, editionDescription,
