...
public class SplitSelfIntersectingLineFix extends AbstractTopologyErrorFix {
	...
	public List<IFeature>[] fixAlgorithm(TopologyError error)
		...
		for (int i = 0; i < lines.length; i++) {
			...
			// we comment this original code because we have found an
			// inconsistent test case with JTS 1.9
			/*
			 * We have: LINESTRING (-34.89284 126.02532, -25.70532 122.96281,
			 * -21.05876 117.47142, -25.81092 114.30331, -29.82386 120.74514,
			 * -27.60619 128.77101, -21.90359 130.24946, -14.40573 127.18696,
			 * -10.7096 117.15461, -13.5609 116.09858, -18.20746 122.75161,
			 * -6.48546 129.51024, -2.89493 120.95634, 8.40466 126.23653,
			 * 0.80119 126.44773, -8.06951 122.5404, -8.06951 122.5404, -8.06951
			 * 122.5404) MULTIPOINT (-28.914741841726936 124.03284485429363,
			 * -13.723360261472408 125.3366439538489, -4.2628609229577705
			 * 124.21683965700947) JTS sais a.disjoints(b) is true
			 */
			...
			if (!disjoints) {
				...
				for (int j = 0; j < splittedLength; j++) {
					IGeometry newGeometry = NewFConverter
							.toFMap(splittedGeometries[j]);
					solution[j] = new DefaultFeature(newGeometry, attributes,
	...
	public void fix(TopologyError error) throws BaseException {
		...
		if (correctedFeatures != null) {
			...
			adapters[0].removeRow(Integer.parseInt(originalFeature.getID()),
					getEditionDescription(), EditionEvent.GRAPHIC);

			for (int i = 0; i < newFeatures.size(); i++) {
				adapters[0].doAddRow(newFeatures.get(i), EditionEvent.GRAPHIC);
			}
