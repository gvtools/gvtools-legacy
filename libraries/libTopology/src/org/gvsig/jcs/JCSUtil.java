...
public class JCSUtil {
	...
	public static IFeature match(FLyrVect originalLyr,
			IFeature originalFeature, FLyrVect candidateLyr) {
	...
	public static IFeature match(FLyrVect originalLyr,
		...
		FeatureAdapter feature = new FeatureAdapter(originalLyr.getSource(),
				idx);
	...
	static CentroidDistanceMatcher CENTROID_DISTANCE_MATCHER = new CentroidDistanceMatcher();
	static CentroidAligner CENTROID_ALIGNER = new CentroidAligner(
			new HausdorffDistanceMatcher());
	static SymDiffMatcher SYM_DIFF_MATCHER = new SymDiffMatcher();
	static CentroidAligner CENTROID_SYM_DIFF = new CentroidAligner(
			new SymDiffMatcher());
	...
	public static boolean checkMatchingGeometryCollections(
			GeometryCollection a, GeometryCollection b, double clusterTolerance) {
	...
	public static boolean checkMatchingLines(LineString a, LineString b,
			double clusterTolerance) {
		// we apply an epsilon-bande algorithm

		// geometries only match is first point is within cluster tolerance
		// distance
	...
	public static boolean checkMatchingPoints(Point a, Point b,
			double clusterTolerance) {
		return a.distance(b) <= clusterTolerance;
	}

	public static boolean checkMatchingCriteria(Geometry first,
			Geometry second, double clusterTolerance) {
		...
		if (first instanceof Polygon && second instanceof Polygon)
			...
			return checkMatchingLines((LineString) first, (LineString) second,
					clusterTolerance);
		else if (first instanceof Point && second instanceof Point)
			return checkMatchingPoints((Point) first, (Point) second,
					clusterTolerance);
		else if (first instanceof GeometryCollection
				&& second instanceof GeometryCollection)
			return checkMatchingGeometryCollections((GeometryCollection) first,
					(GeometryCollection) second, clusterTolerance);
		else
			return false;
	}

	public static List<IFeature> findDuplicates(FLyrVect lyr,
			IFeature originalFeature, FeatureMatcher matcher) {
	...
	public static void getCoincidentSegments(FLyrVect lyr, FLyrVect lyr2) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
				lyr2.getSource());
		ConflationSession conflationSession = new ConflationSession(fc, fc2);
		FeatureCollection[] coincidenSegments = conflationSession
				.getCoincidentSegments();
	}
	...
	public static void linearMatch(FLyrVect lyr, FLyrVect lyr2) {
		...
		cs.setPrecedenceRuleEngine(new ReferenceDatasetPrecedenceRuleEngine()
				.setReferenceDatasetName("A"));
	...
	public static void diffOperation(FLyrVect lyr, FLyrVect lyr2,
			int diffOperationMode, boolean splitMultiGeomsInComponents) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
				lyr2.getSource());
		DiffGeometryComponents diff = new DiffGeometryComponents(fc, fc2,
		...
		// la primera tiene las geometrias que casan, la segunda las que no
		// casan

	}

	// //Igual que el DIFF GEOMETRY, pero a nivel de segmentos en vez de a nivel
	// de geometrias

	public static void diffSegmentsOperation(FLyrVect lyr, FLyrVect lyr2,
			double tolerance) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
				lyr2.getSource());
		DiffSegmentsWithTolerance diff = new DiffSegmentsWithTolerance(fc, fc2,
				tolerance);
		// el primero tiene los segmentos de A que no están en B, y el segundo
		// viceversa
		FeatureCollection[] diffFC = diff.diff();
	}
	...
	public static void findBoundaryCornerOffsets(FLyrVect lyr, FLyrVect lyr2,
			double clusterTolerance, double offsetTolerance, int maxOffsetAngle) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
				lyr2.getSource());
		...
		OffsetBoundaryCornerFinder finder = new OffsetBoundaryCornerFinder(fc,
				fc2, param);
	...
	public static void findInternalMatchesSegments(FLyrVect lyr,
			double clusterTolerance) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		InternalMatchedSegmentFinder.Parameters param = new InternalMatchedSegmentFinder.Parameters();
		param.distanceTolerance = clusterTolerance;
		InternalMatchedSegmentFinder finder = new InternalMatchedSegmentFinder(
				fc, param);
	...
	public static FLyrVect getGaps(FLyrVect lyr, double clusterTolerance) {
		InternalMatchedSegmentFinder.Parameters msfParam = new InternalMatchedSegmentFinder.Parameters();
		...
		InternalMatchedSegmentFinder msf = new InternalMatchedSegmentFinder(
				new FeatureCollectionAdapter(lyr.getSource()), msfParam);
		FeatureCollection gapsCandidates = msf.getMatchedFeatures();
		FeatureCollectionDataSourceAdapter driver = new FeatureCollectionDataSourceAdapter(
				Messages.getText("GAPS"), gapsCandidates,
				new FeatureSchemaLayerDefinitionAdapter(
						gapsCandidates.getFeatureSchema()));
	...
	public static void alignCoverage(FLyrVect lyr, FLyrVect lyr2,
			double clusterTolerance, double angleTolerance) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
		...
		param.distanceTolerance = clusterTolerance;
		param.angleTolerance = angleTolerance;
		CoverageAligner cvgAligner = new CoverageAligner(fc, fc2,
				new TaskMonitorAdapter());
	...
	public static void removeShortSegments(FLyrVect lyr, double minLength,
			double displacementTolerance) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureShortSegmentRemover remover = new FeatureShortSegmentRemover(fc,
				minLength, displacementTolerance);
		FeatureCollection newFC = remover.process(new TaskMonitorAdapter());
	}

	public static void boundaryMatch(FLyrVect lyr, FLyrVect lyr2,
			double clusterTol, boolean insertRefVertices) {
		FeatureCollectionAdapter fc = new FeatureCollectionAdapter(
				lyr.getSource());
		FeatureCollectionAdapter fc2 = new FeatureCollectionAdapter(
				lyr2.getSource());
	...
	public static void matchLinearFeatures(IFeature linearFeature,
			FLyrVect lyr, double clusterTolerance, double angleTolerance) {
		try {
			IFeatureIterator iterator = lyr.getSource().getFeatureIterator();
			SegmentMatcher segmentMatcher = new SegmentMatcher(
					clusterTolerance, angleTolerance);
			while (iterator.hasNext()) {
				IFeature feature = iterator.next();
				if (feature.getID().equals(linearFeature.getID()))
					continue;
				Geometry geom = NewFConverter.toJtsGeometry(feature
						.getGeometry());
				...
				for (int i = 0; i < lineStrings.length; i++) {
					...
					for (int j = 0; j < pts.length - 1; j++) {
						...
						// TODO Recorrer aqui los segmentos de linearFeature, y
						// comparar
	...
	// targetLyr.getSource());
	//
	// FeatureCollectionAdapter candidatesFcAdapter = new
	...
	//
	// matchPairFeatureCollection =
	// createMatchPairFeatureCollection(targetUnioningFCMatchFinder);
	//
	// matchedTargetsFeatureCollection = createMatchedFeatureCollection(true,
	// targetUnioningFCMatchFinder);
	//
	// unmatchedTargetsFeatureCollection =
	// createUnmatchedFeatureCollection(true);
	//
	// matchedCandidatesFeatureCollection = createMatchedFeatureCollection(
	// false, targetUnioningFCMatchFinder);
	//
	// unmatchedCandidatesFeatureCollection =
	// createUnmatchedFeatureCollection(false);
	//
	// }
	//
	// private FeatureCollection createUnmatchedFeatureCollection(boolean
	// fromTarget) {
	// FeatureCollection originalFC =
	// fromTarget ? targetFeatureCollection : candidateFeatureCollection;
	// FeatureCollection newFC = new
	// FeatureDataset(originalFC.getFeatureSchema());
	...
	// }
	//
	// private FeatureCollection createMatchedFeatureCollection(boolean
	// fromTarget, TargetUnioningFCMatchFinder targetUnioningFCMatchFinder) {
	// AttributeMapping mapping =
	// new AttributeMapping(
	// (fromTarget ? targetFeatureCollection : candidateFeatureCollection)
	// .getFeatureSchema(),
	// new FeatureSchema());
	// //Feature may appear more than once because of unioning. So remove
	// duplicates. [Jon Aquino]
	// TreeSet featureSet = new TreeSet();
	// FeatureCollection newFC = new FeatureDataset(createSchema(mapping,
	// targetUnioningFCMatchFinder != null));
	// for (Iterator i = targetFeatureToMatchesMap.keySet().iterator();
	// i.hasNext();) {
	...
	// newFeature.setAttribute(SCORE_ATTRIBUTE, new
	// Double(matches.getTopScore()));
	// if (targetUnioningFCMatchFinder != null) {
	// newFeature.setAttribute(UNION_ID_ATTRIBUTE,
	// targetUnioningFCMatchFinder.getUnionID(target));
	// }
	...
	public static FeatureMatcher createFeatureMatcher(
			double centroidDistanceWeight, double hasdorffDistanceWeight,
			double symDiffWeight, double symDiffCentroidsWeight,
			double compactnessWeight, double angleWeight) {
