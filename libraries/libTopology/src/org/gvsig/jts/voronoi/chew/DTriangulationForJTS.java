 ...
 * progress.
 * 
 * The TIN's implemented algorithm is incremental delaunay triangulation, which
 * starts with a triangle that contains all mess points, and subdivides it with
 * each new inserted vertex.
 * 
 * 
 * 
 * @author sstein
 ...
 *         Methods of the class are modified versions from DelaunayPanel.java in
 *         DelaunayAp.java.
 */
public class DTriangulationForJTS {
	...
	public DTriangulationForJTS(VoronoiAndTinInputLyr inputLyr,
		...
		this.initialTriangle = new Simplex(new Pnt[] {
				new Pnt(visitor.getArgminx() - (1.5 * dx), visitor.getArgminy()
						- dy), // lower left
				new Pnt(visitor.getArgmaxx() + (1.5 * dx), visitor.getArgminy()
						- dy), // lower right
				new Pnt(visitor.getArgminx() + (dx / 2.0), visitor.getArgmaxy()
						+ 1.5 * dy) }); // center,

		this.lowerLeftPnt = new Pnt(visitor.getArgminx() - (1.5 * dx),
				visitor.getArgminy() - dy);
	 ...
	 */
	public DTriangulationForJTS(VoronoiAndTinInputLyr inputLyr,
			boolean onlySelection, Envelope envelope) throws BaseException {
		MessExtentVertexVisitor visitor = new MessExtentVertexVisitor();
		inputLyr.visitVertices(visitor, onlySelection);
	...
	class CreateDTVertexVisitor implements VertexVisitor {
		...
		public void visit(Point2D vertex) {
			Point pt = JtsUtil.GEOMETRY_FACTORY.createPoint(new Coordinate(
	...
	public void addPoints(VoronoiAndTinInputLyr inputLyr, boolean onlySelection)
	...
	public List<Coordinate[]> getTinTriangles(
	 ...
	 * @return
	 */
	// public List<Geometry> getThiessenPolys(CancellableProgressTask
	// progressMonitor) {
