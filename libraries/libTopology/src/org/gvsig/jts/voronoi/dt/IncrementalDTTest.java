...
public class IncrementalDTTest extends TestCase {
	...
	protected void countTIN(int numberOfPoints) {
		...
		for (int i = 0; i < numberOfPoints; i++) {
			Coordinate x = new Coordinate((Math.random() * 10000),
					(Math.random() * 10000), 0);
			points.add(x);
		...
		System.out.println("pocet trojuhelniku: "
				+ triangulace.number_Triangles);
		System.out.println("");
	...
	public void testRightCountCircle() {
		...
		while (iterTIN.hasNext()) {
			...
			double N = ((Math.pow(T.C.x, 2) - Math.pow(T.A.x, 2)
					+ Math.pow(T.C.y, 2) - Math.pow(T.A.y, 2))
					* (T.B.x - T.A.x) - (Math.pow(T.B.x, 2)
					- Math.pow(T.A.x, 2) + Math.pow(T.B.y, 2) - Math.pow(T.A.y,
					2)) * (T.C.x - T.A.x))
					/ (-2 * ((T.A.y - T.C.y) * (T.B.x - T.A.x) - (T.A.y - T.B.y)
							* (T.C.x - T.A.x)));
			double M = (Math.pow(T.B.x, 2) - Math.pow(T.A.x, 2)
