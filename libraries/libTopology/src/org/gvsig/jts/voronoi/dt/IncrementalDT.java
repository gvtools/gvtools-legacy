...
public class IncrementalDT {
	...
	private boolean duplicitePoint; // true - kdyz nove zadany bod je jiy
									// obsazen v triangulaci
	private Coordinate[] convex; // body convexniho obalu + body lezici na hrane
									// konvexu
	public KDTree triangles = new KDTree(2); // vyvazeny dvourozmerny AVL strom,
												// datova struktura trojuhelniku
	...
	private LinkedList firstPointsXY = new LinkedList(); // souradnice bodu
															// prvniho
															// trojuhelnika
	private boolean firstTriangle = false;
	public int max = 0; // pocet kroku pro nalezeni vnitrniho trojuhelnika,
						// ktery obsahuje novy bod P
	public int maxK = 0; // pocet kroku pro nalezeni trojuhelnika na kraji
							// konvex obalu, ktery obsahuje novy bod P
	...
	public void insertPoint(Coordinate XY) {
		...
		number_Points++;
		if (!firstTriangle) { // vstup prvnich bodu, az mohou vytvorit
								// trojuhelnik
			...
			if (newP.coveredBy(newConvexGeometry.convexHull())) { // vnitrni bod
				...
				if (newConvexGeometry.contains(newP) && !duplicitePoint) // bod
																			// na
																			// hrane
																			// konvex
																			// obalu
	...
	private void createFirstTriangle() {
		...
		if (T.isTriangle()) {
			...
			if (firstPointsXY.size() > 3) { // doplneni triangulace o prvni
											// body, ktere byly v primce
	...
	private TriangleDT ringSearch(PointDT P) {
		...
		for (i = 0; i < 3; i++) {
			if (T.neighbour[i] != null) {
				...
				for (int j = 0; j < 3; j++) {
					if (T.neighbour[i].neighbour[j] != null
							&& T.neighbour[i].neighbour[j] != T) {
						if (left == null) {
	...
	private TriangleDT inSideConvexSearch(TriangleDT T, int i, TriangleDT left,
		...
		while (change) {
			...
			for (int j = 0; j < 3; j++) {
				if ((left.neighbour[j] != null)
						&& left.neighbour[j].containsTwoPoints(A, B))
					return left.neighbour[j];
				if (left.neighbour[j] != null
						&& !wasTested(left.neighbour[j], tested)
						&& left.neighbour[j] != helpTriangle
	...
	private void divideTriangle(PointDT A, boolean pointIsInConvex) {
		...
		if (!duplicitePoint) { // test duplicity
			...
			newTriangles.trian = new TriangleDT(T.A, T.B, A); // vytvoreni tri
																// novych troj.
			...
			if (!pointIsInConvex) { // pro body uvnitr trojuhelnika
				...
				saveTriangleNeighbour(newTriangles.next.trian,
						newTriangles.next.next.trian); // ulozeni sousedu mezi
														// novymi troj.
				saveTriangleNeighbour(newTriangles.trian,
				...
				if (T.neighbour[1] != null)
					saveTriangleNeighbour(T.neighbour[1],
							newTriangles.next.trian);

				if (T.neighbour[2] != null)
					saveTriangleNeighbour(T.neighbour[2],
							newTriangles.next.next.trian);
				...
				for (int i = 0; i < 3; i++)
					if (T.neighbour[i] != null) {
						saveTriangleNeighbour(T.neighbour[i],
								newTriangles.next.trian);
						saveTriangleNeighbour(T.neighbour[i],
								newTriangles.trian);
					}
	...
	private void testNewTriangles(TestingTriangles newTriangles,
		...
		while (newTriangles != null) { // prochazi vsechny nove trojuhelniky
			for (int i = 0; i < 3; i++) { // delaunay test vsech tri sousedu
											// trojuhelnika
				if (newTriangles.trian.neighbour[i] != null) {
					freePointB = searchFreePointOfTwoTriangles(
							newTriangles.trian, newTriangles.trian.neighbour[i]);
					freePointA = searchFreePointOfTwoTriangles(
							newTriangles.trian.neighbour[i], newTriangles.trian);
					if (delaunay(newTriangles.trian.neighbour[i], freePointB)) { // vznik
																					// novych
																					// trojuhelniku,
																					// kdyz
																					// delaunay
						...
						for (int j = 0; j < 3; j++) { // ulozeni sousedu nove
														// vyniklych
														// trojuhelniku
							if ((newTriangles.trian.neighbour[j] != null)
									&& (newTriangles.trian.neighbour[j] != newTriangles.trian.neighbour[i])) {
								saveTriangleNeighbour(
										endOfListNewTriangles.next.trian,
										newTriangles.trian.neighbour[j]);
								saveTriangleNeighbour(
						...
						for (int j = 0; j < 3; j++) {
							if ((newTriangles.trian.neighbour[i].neighbour[j] != null)
								...
								saveTriangleNeighbour(
										endOfListNewTriangles.next.trian,
										newTriangles.trian.neighbour[i].neighbour[j]);
								saveTriangleNeighbour(
										endOfListNewTriangles.trian,
										newTriangles.trian.neighbour[i].neighbour[j]);
							}
						...
						try { // vstup do datove struktury
							...
							triangles
									.delete(newTriangles.trian.neighbour[i].key);
	...
	private PointDT searchFreePointOfTwoTriangles(TriangleDT firstTriangle,
		...
		if (firstTriangle.neighbour[0] == secondTriangle)// najde volny bod,
															// dvou sousedicich
															// trojuhelniku
	...
	private boolean delaunay(TriangleDT T, PointDT A) {
		...
		double R = Math.sqrt(Math.pow((T.A.x - M), 2)
				+ Math.pow((T.A.y - N), 2)); // vypocet polomeru
		if ((R - 0.0001) > (Math.sqrt(Math.pow((A.x - M), 2)
				+ Math.pow((A.y - N), 2)))) // tolerance pro trojuhelniku, ktere
											// jsou soumerne podle osy, kterou
											// prochazi spolecna hrana
			return true;
		else {
			if (R > 0) {
				return false;
	...
	/***
	 * The method for imputing new point into tiangulation. The point isn't
	 * inside convex hull
	 * 
	 * @param PointDT
	 *            P - new point
	...
	private void createConvexPolygon(PointDT P) {
		double nextPointDistance = 0; // distance between two point of convex
										// hull
		...
		newConvex = new Coordinate[convex.length + 1]; // pridani noveho bodu do
														// konvexniho obalu
		...
		} // int i a j udavaji index v poli convex[] kde doslo k roztrzeni
			// convex vstupem noveho bodu

		if (!duplicitePoint) { // kontrola duplicity
			double k = (convex[i].y - convex[j].y)
					/ (convex[i].x - convex[j].x);
			double q1 = convex[i].y - k * convex[i].x; // zjisteni smeru
														// pretrzeni (z i do j
														// nebo obracene
	 ...
	 * 
	 * @param Coordinate
	 *            [] - array of convex points, which generate new convex
	 *            triangles
	 * @param PointDT
	 *            P - new point
	 */
	...
	private void rebuildConvex(Coordinate[] newConvex) {
		...
		for (int w = 0; w < convex.length - 1; w++) {// zde se doplni body ktere
														// lezi na hrane convexu
			isSame = false;
	...
	/*
	 * public void printAllTriangles(){ double[] key=new double[2]; TriangleDT
	 * T; int i=0; key[0]=50; key[1]=50;
	 * System.out.println("pocet TROJUHELNIKU JE"+number_Triangles); try{ do{ //
	 * T=(TriangleDT) triangles.nearest(key); System.out.print(i+" triangle: ");
	 * T.printTriangle(); i++; key=T.key; triangles.delete(key); }while
	 * (T!=null); } catch(Exception e){ System.out.println(e); } }
	 * 
	 * public void getShapeFile() { try{ String URI="c:/temp/triangles_100.shp";
	 ...
	 * AttributeType[] { geom}, "triangle");
	 * 
	 * // feature : instance creation WKTReader wktReader = new WKTReader();
	 * LineString geometry = (LineString) wktReader
	 * .read("LINEARRING (0 0, 10 10, 20 30,12 23, 0 0)"); LineString geometry1
	 * = (LineString) wktReader
	 * .read("LINEARRING (0 0, 30 10, 50 30,125 23, 0 0)");
	 * 
	 * Float width = new Float(10); Feature theRoad = ftRoad.create(new Object[]
	 * { geometry }, "Triangles"); CoordinateReferenceSystem crs =
	 * CRS.decode("EPSG:2065");
	 * 
	 * // datastore creation URL anURL = (new File(URI)).toURL();
	 ...
	 * 
	 * 
	 * // file saving FeatureWriter aWriter =
	 * datastore.getFeatureWriter("triangle", ((FeatureStore)
	 * datastore.getFeatureSource("triangle")) .getTransaction());
	 * 
	 * double[] key=new double[2]; TriangleDT T; int i=0; key[0]=50; key[1]=50;
	 * System.out.println("pocet TROJUHELNIKU JE"+number_Triangles);
	 * 
	 * do{ // T=(TriangleDT) triangles.nearest(key); Feature aNewFeature =
	 * aWriter.next();
	 * 
	 * geometry = (LinearRing) wktReader
	 * .read("LINEARRING ("+T.A.x+" "+T.A.y+","
	 * +T.B.x+" "+T.B.y+","+T.C.x+" "+T.C.y+","+T.A.x+" "+T.A.y+")");
	 * 
	 * aNewFeature.setAttribute(0,geometry);
	 * 
	 * i++; key=T.key; triangles.delete(key); }while (i<number_Triangles);
	 * 
	 * aWriter.write(); aWriter.close(); datastore.forceSchemaCRS( crs );
	 * 
	 * 
	 * 
	 * 
