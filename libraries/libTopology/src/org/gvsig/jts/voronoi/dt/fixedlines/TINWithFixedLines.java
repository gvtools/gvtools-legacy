...
public class TINWithFixedLines {
	...
	/***
	 * The private method for searching a triangle which are intersect by new
	 * hard line
	 * 
	 * @param PointDT
	 *            - coordinate of new hard line
	 */
	...
	private static LinkedList getTrianglesIntersectLine(LineDT line) {
		...
		CoordinateArraySequence newPointP = new CoordinateArraySequence(
				newPoints);
		...
		while (iter.hasNext()) {
			...
			} else { // testovani jestli linie ci bod trojuhelnika je obsazen v
						// pevne hrane
				if (lineContainsPoint(newL, T.A) && !T.A.compare(line.A)
						&& !T.A.compare(line.B)) {
					iter.remove();
					trianglesToChange.add(T);
				} else {
					if (lineContainsPoint(newL, T.B) && !T.B.compare(line.A)
							&& !T.B.compare(line.B)) {
						iter.remove();
						trianglesToChange.add(T);
					} else {
						if (lineContainsPoint(newL, T.C)
								&& !T.C.compare(line.A) && !T.C.compare(line.B)) {
							iter.remove();
							trianglesToChange.add(T);
						}
					}
	 ...
	 * 
	 * @return boolean - true : when the line contains the point P false: when
	 *         the line doesn't contain the point P
	 * 
	 */
	 ...
	 *            - coordinates of the point P
	 * 
	 * @return boolean - true : when the list contains the point P false: when
	 *         the list doesn't contain the point P
	 * 
	 */
	 ...
	 * 
	 * @param LinkedList
	 *            - List of existing TIN triangles
	 * @param PointDT
	 *            A- coordinates of start point of line
	 * @param PointDT
	 *            B- coordinates of end point of line
	 * 
	 * @return LinkedList - return List of point, which generate new TIN around
	 *         hard line without start and end point of hard line
	 * 
	 */

	private static LinkedList getPoints(LinkedList triangles, PointDT A,
			PointDT B) {
		Iterator iter = triangles.iterator();
		...
		PointDT P;
		while (iter.hasNext()) { // vymazani yacatku a konce linie ze seznamu
									// bodu
			P = (PointDT) iter.next();
	...
	/***
	 * The private method for testing, if the old triangles contains new
	 * triangle
	 * 
	 * @param TriangleDT
	 *            - triangle for test
	 * @param LinkedList
	 *            - List of triangles of old triangulation,which will be deleted
	 * 
	...
	private static boolean testIsInside(TriangleDT T, LinkedList trians) {
		...
		while (iter.hasNext()) { // testovani zda teziste noveho trojuhelnika je
									// uvnitr zrusenych trojuhelniku
			if (((TriangleDT) iter.next()).contains(new PointDT(T.key[0],
					T.key[1], 0))) {
				return true;
			}
		}
	 ...
	 * 
	 * @param LinkedList
	 *            left - List of new triangles, which are on the left side of
	 *            the line
	 * @param LinkedList
	 *            right - List of new triangles, which are on the right side of
	 *            the line
	 * @param LinkedList
	 *            trianglesToChange - List of old triangles, which will be
	 *            deleted from triangulation
	 * 
	 */

	private static void testAndAddTrianglesToTIN(LinkedList left,
			LinkedList right, LinkedList trianglesToChange) {
		TriangleDT T;
	...
	/***
	 * The static method for creating new triangles around the fixed lines in
	 * existing triangulation
	 * 
	 * @param LinkedList
	 *            trians - List of triangles, which generate TIN
	 * @param LinkedList
	 *            hLines - List of fixed lines, which will be triangulated
	 * 
	...
	public static LinkedList countTIN(LinkedList trians, LinkedList hLines) {
		...
		LinkedList trianglesToChange; // nalezene trojuhelniky pres ktere
										// prochazi linie
		...
		double yTrans, xTrans; // transfomovane souradnice (shodnostni
								// transformace
								// uhel pooteceni alfa, posunuti 0 v x-ove i v
								// y-ove ose
		while (iter.hasNext()) { // cyklus pro pevne hrany
			...
			trianglesToChange = getTrianglesIntersectLine(line); // ziskani
																	// trojuhelniku,
																	// ktere
																	// protina
																	// pevna
			...
			if (trianglesToChange.size() != 0) {
				points = getPoints(trianglesToChange, line.A, line.B); // ziskani
																		// bodu
																		// z
																		// protnutych
																		// trojuhelniku
				...
				if ((line.B.x - line.A.x) != 0)
					alfa = -1
							* (Math.atan((line.B.y - line.A.y)
									/ (line.B.x - line.A.x)));
				else
					alfa = Math.PI / 2;

				double yTransNullPoints = Math.cos(alfa) * line.A.y
						+ Math.sin(alfa) * line.A.x; // ziskani hodnoty y podle
														// ktere se rozdeluje
														// triangulace na pravou
														// a levou
				System.out.println(yTransNullPoints);
				Iterator it = points.iterator();

				while (it.hasNext()) { // rozdeleni bodu do prave a leve
										// triangulace
					P = (PointDT) it.next();
					yTrans = Math.cos(alfa) * P.y + Math.sin(alfa) * P.x; // shodnostni
																			// transfomace
