...
public class Voronoier {
	...
	private static final Map<String, VoronoiStrategy> strategies = new HashMap<String, VoronoiStrategy>();
	static {
	...
	public static void registerVoronoiStrategy(VoronoiStrategy voronoi,
			String name) {
	...
	public static List<IFeature> createThiessen(VoronoiAndTinInputLyr inputLyr,
			boolean onlySelection, String strategyStr,
			CancellableProgressTask progressMonitor) throws BaseException {
		VoronoiStrategy strategy = strategies.get(strategyStr);
		return strategy.createThiessenPolygons(inputLyr, onlySelection,
				progressMonitor);
	}

	public static List<IFeature> createThiessen(VoronoiAndTinInputLyr inputLyr,
			boolean onlySelection, String strategy) throws BaseException {
		return createThiessen(inputLyr, onlySelection, strategy, null);
	}

	public static List<TriangleFeature> createTIN(
			VoronoiAndTinInputLyr inputLyr, boolean onlySelection,
			String strategy) throws BaseException {
		return createTIN(inputLyr, onlySelection, strategy, null);
	}

	public static List<TriangleFeature> createTIN(
			VoronoiAndTinInputLyr inputLyr, boolean onlySelection,
			String strategyStr, CancellableProgressTask progressMonitor)
			throws BaseException {
		VoronoiStrategy strategy = strategies.get(strategyStr);
		return strategy.createTin(inputLyr, onlySelection, progressMonitor);
	}

	// TODO We could optimize this method if we work with a layer, instead a
	// List<TriangleFeature>,
	// so we could use spatial indexes
	public static List<TriangleFeature> getAdjacentTriangles(
			TriangleFeature feature, List<TriangleFeature> tin) {
		List<TriangleFeature> adjacentsTriangles = new ArrayList<TriangleFeature>();
		FTriangle triangle = (FTriangle) feature.getGeometry()
				.getInternalShape();
		Iterator<TriangleFeature> i = tin.iterator();
		while (i.hasNext()) {
			TriangleFeature candidate = i.next();
			FTriangle otherTriangle = (FTriangle) candidate.getGeometry()
					.getInternalShape();
	...
	private static List<LineString> getVoronoiEdges(List<TriangleFeature> tin) {
		...
		while (it.hasNext()) {
			...
			FTriangle triangleGeo = (FTriangle) triangle.getGeometry()
					.getInternalShape();
			Point2D circumcenter = triangleGeo.getCircumCenter();
			Coordinate jtsCircum = new Coordinate(circumcenter.getX(),
					circumcenter.getY());
			List<TriangleFeature> adjacentTriangles = Voronoier
					.getAdjacentTriangles(triangle, tin);
			Iterator<TriangleFeature> adjacentIterator = adjacentTriangles
					.iterator();
			while (adjacentIterator.hasNext()) {
				TriangleFeature other = adjacentIterator.next();
				FTriangle otherGeo = (FTriangle) other.getGeometry()
						.getInternalShape();
				Point2D otherCircum = otherGeo.getCircumCenter();
				Coordinate jtsOther = new Coordinate(otherCircum.getX(),
						otherCircum.getY());
				Coordinate[] coords = new Coordinate[] { jtsCircum, jtsOther };
				LineString ls = JtsUtil.GEOMETRY_FACTORY
	...
	public static List<Geometry> getThiessenPolys(Polygon bbox,
			List<TriangleFeature> tin, CancellableProgressTask progressMonitor) {
	 ...
	 * @throws BaseException
	 */
	public static Geometry getThiessenBoundingBox(
			VoronoiAndTinInputLyr inputLyr, boolean onlySelection)
			throws BaseException {
		...
		coords[0] = new Coordinate(minX - dx, minY - semiDy); // lowerleft

		coords[1] = new Coordinate(minX + (3 * dx), minY - semiDy); // lowerright

		coords[2] = new Coordinate(minX + (3 * dx), minY + extendedDy); // topright

		coords[3] = new Coordinate(minX - dx, minY + extendedDy); // topleft

		coords[4] = new Coordinate(minX - dx, minY - semiDy);
