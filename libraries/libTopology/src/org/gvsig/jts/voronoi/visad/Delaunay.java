...
public abstract class Delaunay implements java.io.Serializable {
	 ...
	 * triangulation. The exact parameter is an indication of whether the exact
	 * Delaunay triangulation is required. The method chooses from among the
	...
	public static Delaunay factory(float[][] samples, boolean exact)
		...
		if (dim < 2)
			throw new VisADException("Delaunay.factory: "
	...
	/**
	 * scale alters the values of the samples by multiplying them by the mult
	 * factor; copy specifies whether scale should modify the actual samples or
	 * a copy of them.
	 */
	...
	/**
	 * perturb alters the values of the samples by up to epsilon in either
	 * direction, to eliminate triangulation problems such as co-linear points;
	 * copy specifies whether perturb should modify the actual samples or a copy
	 * of them.
	 */
	...
	/**
	 * test checks a triangulation in various ways to make sure it is
	 * constructed correctly; test returns false if there are any problems with
	 * the triangulation. This method is expensive, provided mainly for
	 * debugging purposes.
	 */
	...
	/**
	 * improve uses edge-flipping to bring the current triangulation closer to
	 * the true Delaunay triangulation. pass is the number of passes the
	 * algorithm should take over all edges (however, the algorithm terminates
	 * if no edges are flipped for an entire pass).
	 */
	public void improve(float[][] samples, int pass) throws VisADException {
		...
		if (dim > 2) {
			throw new UnsupportedOperationException(
					"Delaunay.improve: dimension " + "must be 2!");
		}
		int ntris = Tri.length;
		int nrs = samples[0].length;
		for (int i = 1; i < dim; i++) {
	 ...
	 * extension to the Delaunay class should call finish_triang at the end of
	 * its triangulation constructor.
	 */
	...
	/*
	 * public void finish_triang(float[][] samples) throws VisADException { int
	 * dim = samples.length; int dim1 = dim+1; int ntris = Tri.length; int nrs =
	 * samples[0].length; for (int i=1; i<dim; i++) { nrs = Math.min(nrs,
	 * samples[i].length); }
	 * 
	 * if (Vertices == null) { // build Vertices component Vertices = new
	 * int[nrs][]; int[] nverts = new int[nrs]; for (int i=0; i<ntris; i++) {
	 * for (int j=0; j<dim1; j++) nverts[Tri[i][j]]++; } for (int i=0; i<nrs;
	 * i++) { Vertices[i] = new int[nverts[i]]; nverts[i] = 0; } for (int i=0;
	 * i<ntris; i++) { for (int j=0; j<dim1; j++) {
	 * Vertices[Tri[i][j]][nverts[Tri[i][j]]++] = i; } } }
	 * 
	 * if (Walk == null && dim <= 3) { // build Walk component Walk = new
	 * int[ntris][dim1]; for (int i=0; i<Tri.length; i++) { WalkDim: for (int
	 * j=0; j<dim1; j++) { int v1 = j; int v2 = (v1+1)%dim1; Walk[i][j] = -1;
	 * for (int k=0; k<Vertices[Tri[i][v1]].length; k++) { int temp =
	 * Vertices[Tri[i][v1]][k]; if (temp != i) { for (int l=0;
	 * l<Vertices[Tri[i][v2]].length; l++) { if (dim == 2) { if (temp ==
	 * Vertices[Tri[i][v2]][l]) { Walk[i][j] = temp; continue WalkDim; } } else
	 * { // dim == 3 int temp2 = Vertices[Tri[i][v2]][l]; int v3 = (v2+1)%dim1;
	 * if (temp == temp2) { for (int m=0; m<Vertices[Tri[i][v3]].length; m++) {
	 * if (temp == Vertices[Tri[i][v3]][m]) { Walk[i][j] = temp; continue
	 * WalkDim; } } } } // end if (dim == 3) } // end for (int l=0;
	 * l<Vertices[Tri[i][v2]].length; l++) } // end if (temp != i) } // end for
	 * (int k=0; k<Vertices[Tri[i][v1]].length; k++) } // end for (int j=0;
	 * j<dim1; j++) } // end for (int i=0; i<Tri.length; i++) } // end if (Walk
	 * == null && dim <= 3)
	 * 
	 * if (Edges == null && dim <= 3) { // build Edges component
	 * 
	 * // initialize all edges to "not yet found" int edim = 3*(dim-1); Edges =
	 * new int[ntris][edim]; for (int i=0; i<ntris; i++) { for (int j=0; j<edim;
	 * j++) Edges[i][j] = -1; }
	 * 
	 * // calculate global edge values NumEdges = 0; if (dim == 2) { for (int
	 * i=0; i<ntris; i++) { for (int j=0; j<3; j++) { if (Edges[i][j] < 0) { //
	 * this edge doesn't have a "global edge number" yet int othtri =
	 * Walk[i][j]; if (othtri >= 0) { int cside = -1; for (int k=0; k<3; k++) {
	 * if (Walk[othtri][k] == i) cside = k; } if (cside != -1) {
	 * Edges[othtri][cside] = NumEdges; } else { throw new
	 * SetException("Delaunay.finish_triang: " + "error in triangulation!"); } }
	 * Edges[i][j] = NumEdges++; } } } } else { // dim == 3 int[] ptlook1 = {0,
	 * 0, 0, 1, 1, 2}; int[] ptlook2 = {1, 2, 3, 2, 3, 3}; for (int i=0;
	 * i<ntris; i++) { for (int j=0; j<6; j++) { if (Edges[i][j] < 0) { // this
	 * edge doesn't have a "global edge number" yet
	 * 
	 * // search through the edge's two end points int endpt1 =
	 * Tri[i][ptlook1[j]]; int endpt2 = Tri[i][ptlook2[j]];
	 * 
	 * // create an intersection of two sets int[] set = new
	 * int[Vertices[endpt1].length]; int setlen = 0; for (int p1=0;
	 * p1<Vertices[endpt1].length; p1++) { int temp = Vertices[endpt1][p1]; for
	 * (int p2=0; p2<Vertices[endpt2].length; p2++) { if (temp ==
	 * Vertices[endpt2][p2]) { set[setlen++] = temp; break; } } }
	 * 
	 * // assign global edge number to all members of set for (int kk=0;
	 * kk<setlen; kk++) { int k = set[kk]; for (int l=0; l<edim; l++) { if
	 * ((Tri[k][ptlook1[l]] == endpt1 && Tri[k][ptlook2[l]] == endpt2) ||
	 * (Tri[k][ptlook1[l]] == endpt2 && Tri[k][ptlook2[l]] == endpt1)) {
	 * Edges[k][l] = NumEdges; } } } Edges[i][j] = NumEdges++; } // end if
	 * (Edges[i][j] < 0) } // end for (int j=0; j<6; j++) } // end for (int i=0;
	 * i<ntris; i++) } // end if (dim == 3) } // end if (Edges == null && dim <=
	 * 3) }
	 */
