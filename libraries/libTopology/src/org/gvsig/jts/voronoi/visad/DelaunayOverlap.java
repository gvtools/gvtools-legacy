					...
					default: // triangulate first two points
						...
						for (int i = 2; i < glen[qg]; i++) {
							...
							for (itnum = 0; itnum < maxit && !found; itnum++) {
								...
								float p12x = p1x - p2x;
								float p12y = p1y - p2y;
								float c0 = p01x * (p0y - pty) - p01y
										* (p0x - ptx);
								float c1 = p12x * (p1y - pty) - p12y
										* (p1x - ptx);
								float c2 = p02x * (pty - p2y) - p02y
										* (ptx - p2x);
					...
					for (int gy = 0; gy <= broken[curgrid][gx + 1]; gy++) {
						...
						for (int pt = 0; pt < glen[qg]; pt++) {
							...
							for (itnum = 0; itnum < trilen && !located; itnum++) {
								...
								// triangle
								float tval0 = (Bx - Ax) * (Py - Ay) - (By - Ay)
										* (Px - Ax);
								float tval1 = (Cx - Bx) * (Py - By) - (Cy - By)
										* (Px - Bx);
								float tval2 = (Ax - Cx) * (Py - Cy) - (Ay - Cy)
										* (Px - Cx);
