...
/**
 * Moves coordinates of geometries (in a given tolerance) to force them to
 * coincide with coordinates of other geometries in its proximity (the snap
 * tolerance).
 * 
 * Also, it could normalize coordinates of the same geometry. By normalize we
 * mean to force the coherence of isClosed method and coordinates. (for example,
 * LINESTRING(100 100, 200 100, 200 200, 100 200, 100 100.01) return 'true' to
 * isClosed call. After normalize the last coordinate will be (100 100))
 * 
 */
public class GeometrySnapper {
	...
	/**
	 * Snaps the coordinates of many geometries with the same weight (all
	 * coordinates must move)
	 * 
	 * @param geoms
	 * @return
	 */
	...
	/**
	 * Snaps all the coordinates of the given geometries . If many coords are in
	 * the same distance tolerance radius, it applies a weighted average. If one
	 * geometry's weight is 0, it wont participate in the average.
	 * 
	 * @param geoms
	 *            geometries whose coordinates we want to snap
	 * @param weights
	 *            weight of each geometry in the computation of the weighted
	 *            average
	 * 
	 * */
	...
	/**
	 * Similar to the previous method, but only returns the first geometry
	 * snapped with the rest of geometries
	 * 
	 * @param geom2snap
	...
	/**
	 * Snaps the coordinates of the given geometry to avoid coordinates at a
	 * distance lower than the cluster tolerance.
	 * 
	 * @param geom
	 *            original geometry
	 * @return geometry whose vertex has been snapped. Maybe this solution wont
	 *         pass the isValid() JTS test (we dont check for
	 * 
	 * 
	 * 
	 * 
	 * @throws IllegalArgumentException
	 *             if the resulting geometry doesnt pass the main checks of
	...
	public Geometry snap(Geometry geom) throws GeometryCollapsedException {
		...
		if (geom.getGeometryType().equalsIgnoreCase("MultiLineString")) {
			...
			return JtsUtil.GEOMETRY_FACTORY
					.createPolygon(snapedShell, newHoles);
		}

		SnapCoordinateList snapCoordList = new SnapCoordinateList(coords,
				this.snapTolerance);
		...
		try {
			...
			if ((solution.getLength() < this.snapTolerance))
				throw new GeometryCollapsedException(
						"Geometria lineal colapsada al aplicar snap");

			if (solution.getDimension() > 1
					&& (solution.getArea() < snapTolerance * snapTolerance))
				throw new GeometryCollapsedException(
						"Area colapsada a linea al aplicar snap");

			// FIXME The last check if verify if the solution is a JTS valid
			// geometry
			// but to do that we must check this with original geometry
			if (!solution.isValid())
				throw new GeometryCollapsedException(
						"La geometria resultante no es una geometria valida");
		} catch (IllegalArgumentException e) {
			throw new GeometryCollapsedException(
					"JTS no es capaz de construir geometria al snapear sus coordenadas",
					e);
		}
		return solution;
	}
	...
	/**
	 * Creates a geometry of the same type as geom with newPts coordinates. geom
	 * mustn be a GeometryCollection (MultiPolygon, etc.)
	 * 
	 * @param newPts
	 * @param geom
	 * @return
	...
	public Coordinate snapAverage(List<Coordinate> coordinates, int[] weights) {
	...
	/**
	 * If geometry 'a' is not exactly closed, but its extremes are in a snap
	 * radius, its snap the last point to the first point of the geometry
	 * 
	 * @param a
	 * @return
	 */
	public Geometry normalizeExtremes(Geometry a) {
		...
		if (!firstPoint.equals2D(lastPoint)
				&& JtsUtil.isClosed(a, snapTolerance)) {
			CoordinateList coordList = new CoordinateList(coords);
	...
	/**
	 * Snaps coordinates in srcPts with coordinates of snapPts. This method
	 * forces the snap: the points of srcPoints are shifted to snap with points
	 * in snapPts. No average is computed for coordinates.
	 * 
	 * @param srcPts
	 * @param isClosed
	 * @param snapPts
	...
	private Coordinate[] snapTo(Coordinate[] srcPts, boolean isClosed,
			Coordinate[] snapPts) {
