...
/**
 * The CatmullRomSpline is equal to the CardinalSpline with the value of alpha
 * fixed at 0.5. The implementation for the CatmullRomSpline is about 10% faster
 * then the CardinalSpline implementation.
 * 
...
public class CatmullRomSpline extends ParametricCurve {
	...
	protected void eval(double[] p) {
		...
		// Note: The 0.5 does NOT represent alpha. It is a result of the
		// simplification.

		for (int i = 0; i < p.length - 1; i++) {
			p[i] = 0.5
					* ((pt[3][i] - pt[0][i] + 3 * (pt[1][i] - pt[2][i]))
							* t3
							+ (2 * (pt[0][i] + 2 * pt[2][i]) - 5 * pt[1][i] - pt[3][i])
							* t2 + (pt[2][i] - pt[0][i]) * t) + pt[1][i];
		}
	}
