...
/**
 * <p>
 * The natural-cubic-spline is constructed using piecewise third order
 * polynomials which pass through all the control-points specified by the
 * group-iterator. The curve can be open or closed. Figure 1 shows an open curve
 * and figure 2 shows a closed curve.
 * 
 * <p>
 * <center><img align="center" src="doc-files/natcubic1.gif"/></center>
 * 
 * <p>
 * <center><img align="center" src="doc-files/natcubic2.gif"/></center>
 */
public class NaturalCubicSpline extends ParametricCurve {

	/*
	 * The pt array stores the points of the control-path. The data array is
	 * used to store the result of the many calculations.
	 * 
	 * d[0] = w1 For each dimension, 4 arrays are required to store the d[1] =
	 * x1 results of the calculations. d[2] = y1 The length of each array is >=
	 * to the number of points. d[3] = z1 d[4] = w2 d[5] = x2 d[6] = y2 d[7] =
	 * z2 d[8] = a // a, b & c are used (by both open and closed) to store d[9]
	 * = b // the results of the calculations. d[10] = c d[11] = d // only used
	 * for closed cubic curves
	 */
	...
	protected void eval(double[] p) {
		...
		int j = 0;
		for (int i = 0; i < n; i++)
			p[i] = data[j++][ci] + data[j++][ci] * t + data[j++][ci] * t2
					+ data[j++][ci] * t3;
	}
	 ...
	 * and have a group size of at least 2. If these requirements are not met
	...
	public void appendTo(MultiPath mp) {
		...
		for (int i = 0; i < n; i++)
			pt[i] = cp.getPoint(gi.next()).getLocation(); // assign the used
															// points to pt
