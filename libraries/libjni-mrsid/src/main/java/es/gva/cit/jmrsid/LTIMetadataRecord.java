...
public class LTIMetadataRecord extends JNIBase {
	...
	public String getTagName() throws MrSIDException {
		...
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getTagName. La referencia al objeto no es valida.");

		String tagname = getTagNameNat(cPtr);

		if (tagname == null)
			throw new MrSIDException(
					"No se ha devuelto un nombre de tag valido");
	...
	public boolean isVector() throws MrSIDException {
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en isVector. La referencia al objeto no es valida.");
	...
	public int getDataType() throws MrSIDException {
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getDataType. La referencia al objeto no es valida.");

		int res = getDataTypeNat(cPtr);

		if (res < 0)
			throw new MrSIDException(
					"Error en getDataType. No se ha obtenido un tipo de datos valido");

		return res;
	}

	/**
	 * Obtiene el valor de un datos escalar en forma de String. La conversión a
	 * float, entero, ... si fuera necesario es a cargo del cliente
	 * 
	 * @throws MrSIDException
	 * @return dato escalar
	 */
	public String getScalarData() throws MrSIDException {
		...
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getScalarData. La referencia al objeto no es valida.");
	...
	/**
	 * Obtiene el valor del metadato de tipo vector en forma de vector de
	 * strings
	 * 
	 * @throws MrSIDException
	...
	public String[] getVectorData() throws MrSIDException {
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getVectorData. La referencia al objeto no es valida.");
	...
	/**
	 * Obtiene el valor del metadato de tipo array en forma de vector de
	 * strings. Esta función colocará todas las dimensiones del array en forma
	 * de vector
	 * 
	 * @throws MrSIDException
	 * @return vector de strings con los valores del metadato
	...
	public String[] getArrayData() throws MrSIDException {
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getArrayData. La referencia al objeto no es valida.");
	...
	public int getNumDims() throws MrSIDException {
		...
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getNumDims. La referencia al objeto no es valida.");

		int res = getNumDimsNat(cPtr);

		if (res < 0)
			throw new MrSIDException(
					"Error en getNumDims. El número de dimensiones obtenido no es valido.");
	...
	public int[] getDims() throws MrSIDException {
		...
		if (cPtr == 0)
			throw new MrSIDException(
					"Error en getDims. La referencia al objeto no es valida.");

		int[] res = getDimsNat(cPtr);

		if (res == null || res.length == 0)
			throw new MrSIDException(
					"Error en getDims. No se ha podido obtener la longitud de las dimensiones.");
