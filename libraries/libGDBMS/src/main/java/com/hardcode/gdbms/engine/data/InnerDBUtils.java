...
public class InnerDBUtils {
	...
	public static String createDeleteStatement(Value[] pks, String[] names,
		...
		StringBuffer sql = new StringBuffer("DELETE FROM ").append(tableName)
				.append(" WHERE ").append(names[0]).append("=")
				.append(pks[0].getStringValue(vWriter));

		for (int i = 1; i < pks.length; i++) {
			sql.append(" AND ").append(names[i]).append("=")
					.append(pks[i].getStringValue(vWriter));
		}
	...
	public static String getPKIndexStatement(String tableName, String[] pkNames) {
		StringBuffer sql = new StringBuffer("CREATE INDEX ").append("index")
				.append(System.currentTimeMillis()).append(" ON ")
				.append(tableName).append(" (").append(pkNames[0]);
	...
	public static String createUpdateStatement(String tableName, Value[] pk,
		...
		sql.append("UPDATE ").append(tableName).append(" SET ")
				.append(fieldNames[0]).append(" = ")
				.append(values[0].getStringValue(vWriter));

		for (int i = 1; i < fieldNames.length; i++) {
			sql.append(", ").append(fieldNames[i]).append(" = ")
					.append(values[i].getStringValue(vWriter));
		}

		sql.append(" WHERE ").append(pkNames[0]).append(" = ")
				.append(pk[0].getStringValue(vWriter));

		for (int i = 1; i < pkNames.length; i++) {
			sql.append(" AND ").append(pkNames[0]).append(" = ")
					.append(pk[0].getStringValue(vWriter));
		}
