...
public class Renderer {
	...
	/*
	 * public static void render(DataSource ds, ViewPort vp,
	 * StyledLayerDescriptor sldtree) throws DriverException, SLDException{
	 * StyleLayerDescriptorDecorator sld; try { sld = new
	 * StyleLayerDescriptorDecorator(sldtree, ds.getDataSourceFactory()); }
	 * catch (TransformerConfigurationException e1) { throw new
	 * SLDException("Fallo en la estructura SLD", e1); } catch (JAXBException
	 * e1) { throw new SLDException("Fallo en la estructura SLD", e1); }
	 ...
	 * 
	 * for (int i = 0; i < count; i++){ Geometry g = (Geometry)
	 * ds.getFieldValue(i, geomFieldIndex);
	 * 
	 * if (g == null) continue;
	 * 
	 * g.transform(vp.getTransformationMatrix());
	 * 
	 * //Extent boolean any = false; for (int j = 0; j <
	 * namedLayer.getExtentCount(); j++) { if
	 * (g.intersects(namedLayer.getExtent(j))){ any = true; } } if (
	 * (namedLayer.getExtentCount() > 0) && (!any) ){ continue; }
	 * 
	 * //filter try { if (namedLayer.getFilter() != null){ if
	 * (!((BooleanValue)namedLayer
	 * .getFilter().evaluateExpression(i)).getValue()){ continue; } } } catch
	 * (SemanticException e) { throw new RuntimeException(e); }
	 * 
	 * //get the style's Style2D[] styles = namedLayer.getSymbolizers(i); for
	 * (int j = 0; j < styles.length; j++) { g.draw(vp.getGraphics(),
	 * styles[j]); } }
	 * 
	 * ds.stop(); }
	 */
	...
	public static void drawShape(Graphics2D graphics, Shape shape, Style2D style) {
		if (style instanceof MarkStyle2D) {
			...
			renderImage(graphics, coords[0], coords[1],
					(Image) gs2d.getImage(), gs2d.getRotation(),
					gs2d.getOpacity());
	...
	private static void drawWithGraphicsStroke(Graphics2D graphics,
			Shape shape, BufferedImage image) {
	 ...
	 */
	private static void renderImage(Graphics2D graphics, double x, double y,
		...
		graphics.drawImage(image, -image.getWidth(null) / 2,
