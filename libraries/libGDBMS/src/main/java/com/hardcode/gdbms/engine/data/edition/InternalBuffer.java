...
public class InternalBuffer extends DBDataSourceAdapter {
	...
	public InternalBuffer(DataSourceFactory dsf, String[] pkNames,
		...
		try {
			...
			InnerDBUtils
					.execute(databaseName, InnerDBUtils
							.getCreateStatementWithAutonumeric(tableName,
									names, types));
	 ...
	 */
	public void deleteRow(long rowId) throws ReadDriverException,
			WriteDriverException {
		// Get the pk values
		ValueCollection pks = getPKValue(rowId);
		String[] names = getFieldNames();

		String sql = InnerDBUtils.createDeleteStatement(pks.getValues(), names,
				tableName, ValueWriter.internalValueWriter);
	 ...
	 * @throws WriteDriverException
	 */
	public long insertFilledRow(Value[] values) throws ReadDriverException,
			WriteDriverException {
		/*
		...
		String sql = InnerDBUtils.createInsertStatement(tableName, values,
				names, ValueWriter.internalValueWriter);
	...
	public long insertRow(ValueCollection pk) throws WriteDriverException {
		...
		String sql = InnerDBUtils.createInsertStatement(tableName, pks,
				pkNames, ValueWriter.internalValueWriter);
	 ...
	 *             TODO
	 */
	public void setRow(long row, Value[] modifiedRow)
			throws WriteDriverException {
		try {
	...
	public void setFieldValue(long row, int modifiedField, Value modifiedValue)
		...
		try {
			...
			String sql = InnerDBUtils.createUpdateStatement(tableName, pks,
					names, new String[] { getFieldName(modifiedField) },
					new Value[] { modifiedValue },
					ValueWriter.internalValueWriter);
	...
	/**
	 * Rereads the information if has been modified since last refresh
	 * 
	 * @throws ReadDriverException
	 *             TODO
	 */
