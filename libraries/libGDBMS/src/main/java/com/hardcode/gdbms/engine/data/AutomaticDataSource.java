...
public class AutomaticDataSource implements DataSource {
	private static Logger logger = Logger.getLogger(AutomaticDataSource.class
			.getName());
	 ...
	 * @param timeout
	 *            DataSource will close if there is no operation int timeout
	 *            milliseconds
	 */
	...
	private synchronized void open() throws OpenDriverException {
		if (opened) {
			/*
			 * the resetTimer is a long time operation. Will only call it after
			 * a while
			 */
