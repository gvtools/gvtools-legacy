...
public class ViewPort {
	 ...
	 * 
	 * @param width
	 *            width of the image to be obtained
	 * @param height
	 *            heigth of the image to be obtained
	 * @param g
	...
	private void calculateMT() {
		...
		if (rx < ry) {
			adjustedExtent.setRect(xCenter - (extent.getWidth() / 2.0), yCenter
					- ((h / rx) / 2.0), extent.getWidth(), h / rx);
			mt = AffineTransform.getScaleInstance(rx, -rx);
		} else {
			adjustedExtent.setRect(xCenter - ((w / ry) / 2.0), yCenter
					- (extent.getHeight() / 2.0), w / ry, extent.getHeight());
