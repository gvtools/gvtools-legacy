...
public class ArcXMLFeatures extends ArcXML {
	private static Logger logger = Logger.getLogger(ArcXMLFeatures.class
			.getName());

	/**
	 * Creates a complete request in ArcXML for a FeatureService from an
	 * ArcImsStatus; including extent, SRS and so on
	 * 
	...
	public static String getFeatureLayerRequest(ArcImsVectStatus status,
		...
		/**
		 * We suppose that status.getSrs() always will give a string started by
		 * this string
		 */
		String ini_srs = ServiceInfoTags.vINI_SRS;

		/**
		 * Get the minimum distance between points as the mean distance per
		 * pixel
		 */
		...
		return getFeaturesRequest(layerid, // layer to retrieve
				...
				ArcXML.parseNumber(accuracy * GetFeaturesTags.ACCURACY_RATIO,
						ds), // accuracy multiplied by an arbitrary ratio
	...
	/**
	 * Method that buids a proper ArcXML based on basi parameters as fields to
	 * retrieve, where condition, etc.
	 * 
	 * @param layerId
	 * @param subfields
	 * @param where
	...
	private static String getFeaturesRequest(String layerId, int featCount,
		...
		/*
		 * With newxml we get FIELDS tag with inner FIELD tag for every field of
		 * the attribute table, this way, we get attribute="#SHAPE#" instead of
		 * getting #SHAPE# as a name for an attribute
		 */
	...
	public static String getLayerExtentRequest(ArcImsVectStatus status) {
		 ...
		 * <REQUEST> <GET_FEATURES outputmode="newxml" globalenvelope="true"
		 * skipfeatures="true"> <LAYER id="1" /> <SPATIALQUERY subfields="#ALL#"
		 * > <FEATURECOORDSYS id="4326"/> </SPATIALQUERY> </GET_FEATURES>
		 * </REQUEST> </ARCXML>
		 */
	...
	public static String getAttributesRequest(ArcImsVectStatus status,
		...
		/*
		 * <?xml version = '1.0' encoding = 'UTF-8'?> <ARCXML version="1.1">
		 * <REQUEST> <GET_FEATURES outputmode="newxml" geometry="false"
		 * envelope="true" globalenvelope="true"> <LAYER id="1" /> <SPATIALQUERY
		 * subfields="#ALL#" > <FILTERCOORDSYS id="4326"/> <FEATURECOORDSYS
		 * id="23030"/> <SPATIALFILTER relation="area_intersection"> <ENVELOPE
		 * minx="-10" miny="38" maxx="2" maxy="41"/> </SPATIALFILTER>
		 * </SPATIALQUERY> </GET_FEATURES> </REQUEST> </ARCXML>
		 */
	...
	public static String getIdsRequest(ArcImsVectStatus status, int featCount,
		 ...
		 * Finally, we can retrieve the correct ArcXML
		 */
		return getFeaturesRequest((String) status.getLayerIds().get(0), // layer
																		// to
																		// retrieve
