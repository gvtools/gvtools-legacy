...
public class ArcImsRendererFactory {
	private static Logger logger = Logger.getLogger(ArcImsRendererFactory.class
			.getName());
	...
	public Renderer getRenderer(KXmlParser parser) throws ArcImsException {
		...
		if (parser.getName().equals(ServiceInfoTags.tGROUPRENDERER)) {
			...
			render = (SimpleRenderer) parseSimpleRenderer(parser);
				...
				ServiceInfoTags.tSCALEDEPENDENTRENDERER)) {
			...
			render = (ValueMapRenderer) parseValueMapRenderer(parser);
				...
				.equals(ServiceInfoTags.tSIMPLELABELRENDERER)) {
			render = (SimpleLabelRenderer) parseSimpleLabelRenderer(parser);
		} else if (parser.getName().equals(
				ServiceInfoTags.tVALUEMAPLABELRENDERER)) {
	 ...
	 * @throws ArcImsException
	 */
	private ScaleDependentRenderer parseScaleDependentRenderer(KXmlParser parser)
			throws ArcImsException {
		...
		try {
			// We are at SCALEDEPENDENTRENDERER tag, so we have to get it
			// attributes
	...
	private ValueMapRenderer parseValueMapRenderer(KXmlParser parser)
		...
		try {
			...
			while (!end) {
				...
				/*
				 * Once the exits the switch statement we can add the
				 * TypeValueMap into the arraylist of values
				 */
				if (tvm != null) {
					/*
					 * Create and assign the symbol definition passing the
					 * parser
					 */
					parser.nextTag();

					if (ArcImsSymbolFactory.isSymbol(parser.getName())) {
						symb = ArcImsSymbolFactory
								.parseSymbol(parser, featType);
					}
	...
	private ValueMapLabelRenderer parseValueMapLabelRenderer(KXmlParser parser)
		...
		/*
		 * As this render is a type of a ValueMapRenderer, we can use this
		 * method and afterwards get the parameters and values
		 */
