...
public class ArcImsSymbolFactory {
	private static Logger logger = Logger.getLogger(ArcImsSymbolFactory.class
			.getName());
	...
	/**
	 * This method will return valid IArcIMSSymbol from a ArcXML symbol
	 * definition
	 * 
	 ...
	 * @param parser
	 * @param featType
	 *            Type of feature to get the symbol, useful when a default
	 *            symbol has to be created
	 * @return
	 */
	public static IArcIMSSymbol parseSymbol(KXmlParser parser, int featType) {
		...
		// SIMPLEPOLYGONSYMBOL;
			...
			if ((hasBoundary != null) && hasBoundary.equals("false")) {
				...
				String color = parser.getAttributeValue("", strBound
						+ ServiceInfoTags.aCOLOR);

				if (color != null) {
					lsymb.setColor(color);
				}

				String width = parser.getAttributeValue("", strBound
						+ ServiceInfoTags.aWIDTH);

				if (width != null) {
					lsymb.setWidth(width);
				}

				String type = parser.getAttributeValue("", strBound
				...
				String captype = parser.getAttributeValue("", strBound
						+ ServiceInfoTags.aCAPTYPE);
				...
				String jointype = parser.getAttributeValue("", strBound
						+ ServiceInfoTags.aJOINTYPE);
				...
				String transparency = parser.getAttributeValue("", strBound
						+ ServiceInfoTags.aTRANSPARENCY);
			...
			String strFill = "fill";
			String color = parser.getAttributeValue("", strFill
					+ ServiceInfoTags.aCOLOR);
			...
			String trans = parser.getAttributeValue("", strFill
					+ ServiceInfoTags.aTRANSPARENCY);
			...
			String type = parser.getAttributeValue("", strFill
					+ ServiceInfoTags.aTYPE);
		...
		// RASTERMARKERSYMBOL;
			...
			if (url != null) {
				...
				String size = parser.getAttributeValue("",
		...
		// GRADIENTFILLSYMBOL;
			...
			String finishcolor = parser.getAttributeValue("", "finish"
					+ ServiceInfoTags.aCOLOR);
			...
			String startcolor = parser.getAttributeValue("", "start"
					+ ServiceInfoTags.aCOLOR);
