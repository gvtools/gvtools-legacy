...
import com.iver.cit.gvsig.fmap.rendering.AbstractLegend;
...
public class ArcImsFLegendFactory {
     ...
     */
    public ILegend getMainLegend(SelectableDataSource sds) throws ArcImsException {
     ...
     * @throws ArcImsException
     */
    private IVectorLegend getLegend(Renderer render, boolean isMainRender, SelectableDataSource sds)
        throws ArcImsException {
     ...
     * @throws ArcImsException
     */
    private IVectorLegend getLegend(ValueMapRenderer renderer, SelectableDataSource sds)
        throws ArcImsException {
        ...
        //Set the field name
        cleg.setClassifyingFieldNames(new String[] {ArcXML.replaceUnwantedCharacters(fi.getName())});
    ...
    //		return leg;
    //	}
    private IVectorLegend getLegend(GroupRenderer groupRender, SelectableDataSource sds)
        throws ArcImsException {
        ...
        if (auxLabelsOuter!=null && legends.size()>0){
        	...
        	for (int i=0;i<legends.size();i++){
            	...
            	ISymbol simb2 = ((IVectorLegend) legends.get(i)).getDefaultSymbol();
            	
            	if (simb1==simb2){
            		//Null the biggerScale to force the next if statement
            		biggerScale = null;
            		//Merge the auxLabels object into the first legend of the arrayList
//            		IVectorLegend legend = setLabelDefIntoSymbol(auxLabelsOuter,(IVectorLegend) legends.get(0));
...
//            		legends.set(0,legend);
        ...
        if (auxLabelsInner != null) {
        	...
        	// ISymbol sym = finalLegend.getDefaultSymbol();
