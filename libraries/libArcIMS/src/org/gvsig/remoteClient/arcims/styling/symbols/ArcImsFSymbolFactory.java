...
public class ArcImsFSymbolFactory {
	private static Logger logger = Logger.getLogger(ArcImsFSymbolFactory.class
			.getName());
	...
	// This code is copy&paste from
	// com.iver.cit.gvsig.fmap.core.v02.FSymbolFactory;
	static int w;
	...
	protected static SimpleLineSymbol getFSymbol(ArcImsSimpleLineSymbol simb) {
		...
		SimpleLineStyle linestyle = new SimpleLineStyle(Float.parseFloat(simb
				.getWidth()), intCapType, intJoin, 1.0f,
				ArcImsSimpleLineSymbol.toArray(linePattern, lineWidth), 0);
	 ...
	 * railroad symbols, this method will return a simple line with the same
	 * color and width
	 * 
	 * @param symbol
	 * @return
	...
	protected static ISymbol getFSymbol(ArcImsGradientFillSymbol simb) {
		 ...
		 * 
		 * Point2D startP2D = null; Point2D finishP2D = null;
		 * 
		 * double ratio = 150;
		 * 
		 * switch (type) { case FConstant.SYMBOL_STYLE_FILL_UPWARD_DIAGONAL:
		 * startP2D = new Point2D.Double(0.0*ratio,0.0*ratio); finishP2D = new
		 * Point2D.Double(1.0*ratio,1.0*ratio); break; case
		 * FConstant.SYMBOL_STYLE_FILL_DOWNWARD_DIAGONAL: startP2D = new
		 * Point2D.Double(0.0*ratio,1.0*ratio); finishP2D = new
		 * Point2D.Double(1.0*ratio,0.0*ratio); break; case
		 * FConstant.SYMBOL_STYLE_FILL_VERTICAL: startP2D = new
		 * Point2D.Double(0.5*ratio,0.0*ratio); finishP2D = new
		 * Point2D.Double(0.5*ratio,1.0*ratio); break; case
		 * FConstant.SYMBOL_STYLE_FILL_HORIZONTAL: startP2D = new
		 * Point2D.Double(0.0*ratio,0.5*ratio); finishP2D = new
		 * Point2D.Double(1.0*ratio,0.5*ratio); break; }
		 * 
		 * //Set the paint mSimb.setFill((Paint) new
		 * GradientPaint(startP2D,startColor,finishP2D,finishColor)); //Remove
		 * the outline mSimb.setOutlined(false);
		 */
	...
	protected static ISymbol getFSymbol(TextSymbol symb) {
		...
		logger.info("Getting ArcIMS TextSymbol...");
	...
	/**
	 * This method will return a null value as this kind of symbol cannot be
	 * promoted to other supported symbol
	 * 
	 * @param symbol
	 * @return null
	 */
	...
	/**
	 * Method to translate from ArcIMS type to a string of values that will be
	 * parsed
	 * 
	...
	/**
	 * Gets the hue of a color and returns the color with 75% of saturation and
	 * 50% of brightness
	 * 
	 * @param cRef
	 * @return
