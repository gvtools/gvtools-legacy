 ...
 * primero busca en el classpath, luego busca en el directorio del propio plugin
 * en los jars especificados por el xml y en caso de no encontrar la clase pide
 * al PluginClassLoaderManager la lista de plugins que pueden satisfacer la
 * clase e intenta cargarlo con cada un de ellos hasta que lo consigue con uno.
 * 
...
public class DriverClassLoader extends URLClassLoader {
	 ...
	 * 
	 * @param jars
	 *            Array con la ruta de los jars en los que buscará las clases el
	 *            plugin
	 * @param baseDir
	 ...
	 * 
	 * @throws IOException
	 *             Si no se puede leer alguno de los jars
	 * @throws IllegalArgumentException
	 *             Si no se especifica un array de jars
	 * @throws JarException
	 *             Si hay dos clases con el mismo nombre en el plugin
	 */
		...
		try {
			...
			if (c == null) {
				...
				try {
					...
					if (jar == null) {
						Vector cls = (Vector) DriverClassLoaderManager
								.getClassLoaderList(name);

						if (cls != null) {
							for (int i = 0; i < cls.size(); i++) {
								c = ((DriverClassLoader) cls.elementAt(i))
	 ...
	 * Si no se encuentra el recurso ahí se invoca a getResource del classloader
	 * padre, que buscará en el jar de la aplicación. Si ahí tampoco se
	 * encuentra nada se devolverá null.
	 * 
	 * @param res
	 *            Nombre del recurso
	 * 
	 ...
	 * lista de String's con los directorios del path y base es el directorio a
	 * partir del cual se busca dicho recurso. En cada ejecución del método se
	 * toma el primer elemento de res y se busca dicho directorio en el
	 * directorio base. Si se encuentra, será el directorio base para una nueva
	 * llamada.
	 * 
	 * @param base
	 *            Directorio desde donde parte la búsqueda del recurso.
	 * @param res
	 *            Lista de strings con el path del recurso que se quiere
	 *            encontrar
	 * 
	...
	public String getPluginName() {
		String ret = baseDir
				.substring(baseDir.lastIndexOf(File.separatorChar) + 1);
