...
public class NormAlgorithm {
	...
	private NormalizationPattern pat = null;
	 ...
	 */
	public NormAlgorithm(NormalizationPattern _pat) {
		this.pat = _pat;
		...
		// NORMAL CASE
			...
			for (int i = 0; i < this.numFields; i++) {
				...
				if (preChain.length() == 0) {
					...
					fw = this.elements[i].getFieldwidth();
					...
					// Cut chain with separators
						...
						List<String> separators = loadSpecificSeparators(this.elements[i]);
						boolean join = withJoinSeparators(this.elements[i]);
	 ...
	 * @return list with the strings tokenized
	 */
	public static List<String> splitChainByFixedWidth(String chain,
			int[] fieldWidth) {
	...
	public List<String> filterSplitChains(List<String> chains) {
		...
		for (int i = 0; i < this.numFields; i++) {
			...
			nft = this.elements[i].getFieldtype();

			decsep = this.elements[i].getInfieldseparators()
					.getDecimalseparator().trim();
			cdecsep = decsep.compareTo("") == 0 ? (char) 0x20 : decsep
					.charAt(0);
			thosep = this.elements[i].getInfieldseparators()
					.getThousandseparator().trim();
			cthosep = thosep.compareTo("") == 0 ? (char) 0x20 : thosep
					.charAt(0);

			txsep = this.elements[i].getInfieldseparators().getTextseparator()
					.trim();
			...
			// Fill fields empties
			if (chains.size() < this.numFields) {

				for (int j = chains.size(); j < this.numFields; j++) {
					chains.add(j, "");
				}
			}
	...
	public void update(ChangeEvent evt) {
		...
		for (int i = 0; i < this.listeners.size(); i++) {
			((ChangeListener) this.listeners.get(i)).stateChanged(evt);
		}
	}
	 ...
	 * @return
	 */
	private String testSep(String in, char sep) {
		String out = in;
		if (in.indexOf(sep) >= 0) {
			int ind = in.indexOf(String.valueOf(sep));
			out = in.substring(0, ind) + in.substring(ind + 1, in.length());
		}
		return out;
	}
