...
/**
 * <p>
 * Tests the creation of a {@link TreePanel TreePanel} object using
...
public class Test1TreePanel {
	...
	/**
	 * <p>
	 * Test method for the Test1TreePanel.
	 * </p>
	 * 
	 * @param args
	 *            optional arguments
	...
	public static void main(String[] args) {
		try {
			...
			// Class[] panelClass = new
			// Class[Samples_Data.TEST1_CLASSES.length];
			// int i;
			//
			...
			panelGroup = (TreePanel) manager
					.getPanelGroup(Samples_Data.REFERENCE1);
			PanelGroupLoaderFromList loader = new PanelGroupLoaderFromList(
					Samples_Data.TEST1_CLASSES);
			...
			// Dimension(Samples_Data.PANELS_DEFAULT_WIDTH,
			...
			// Begin: Tests adding and removing a panel without "label" -> this
			// shoudn't add that panel
			panels = new ArrayList<IPanel>();
			loader = new PanelGroupLoaderFromList(new Class[] {
					SampleInfoPanel.class, SampleInfoPanel.class,
					SamplePanelWithoutGroupLabel.class });
			loader.loadPanels(panels);
			((AbstractPanel) panels.get(0)).setPreferredSize(new Dimension(600,
					700));
			...
			// End: Tests adding and removing a panel without "label" -> this
			// shoudn't add that panel

			// Begin: Tests adding and removing a panel
			((AbstractPanel) panels.get(1)).setPreferredSize(new Dimension(
					Samples_Data.PANELS_DEFAULT_WIDTH,
					Samples_Data.PANELS_DEFAULT_HEIGHT));
			...
			((AbstractPanel) panels.get(2)).setPreferredSize(new Dimension(
					Samples_Data.PANELS_DEFAULT_WIDTH,
					Samples_Data.PANELS_DEFAULT_HEIGHT));
			...
			panelGroup.setPanelInGUI(repeatedGroupAPanel, false);// test set
																	// false a
																	// panel
																	// with a
																	// repeated
																	// group
			panelGroup.setPanelInGUI((IPanel) oPanels[1], false); // intermediate
																	// element
			((AbstractPanel) oPanels[1]).setInGroupGUI(true); // test change the
																// visibility
																// from a panel
																// child
			((AbstractPanel) oPanels[1]).setInGroupGUI(false); // test change
																// the
																// visibility
																// from a panel
																// child
			((AbstractPanel) oPanels[1]).setInGroupGUI(true); // test change the
																...
																// from a panel
																// child
			panelGroup.setPanelInGUI((IPanel) oPanels[0], false); // first
																	// element
			panelGroup.setPanelInGUI(incorrectAPanel, false); // incorrect
																// element
			panelGroup.setPanelInGUI(panels.get(2), false); // last element (and
															// labelGroup =
															// null)
			panelGroup.setPanelInGUI((IPanel) oPanels[2], false); // intermediate
																	// element
			panelGroup.setPanelInGUI((IPanel) oPanels[2], false); // test set
																	// false two
																	// times the
																	// same node
			panelGroup.setPanelInGUI((IPanel) oPanels[0], true); // first
																	// element
			panelGroup.setPanelInGUI(incorrectAPanel, true); // incorrect
																// element
			panelGroup.setPanelInGUI((IPanel) oPanels[1], true); // intermediate
																	// element
			panelGroup.setPanelInGUI((IPanel) oPanels[1], true); // test set
																	// true two
																	// times the
																	// same node
			panelGroup.setPanelInGUI((IPanel) oPanels[2], true); // intermediate
																	// element
			panelGroup.setPanelInGUI(panels.get(2), true); // last element (and
															// labelGroup =
															// null)
			panelGroup.setPanelInGUI(repeatedGroupAPanel, true); // test set
																	// false a
																	// panel
																	// with a
																	// repeated
			...
			panels.get(1).setInGroupGUI(false); // test change the visibility
												// from a panel child
			if (!panels.get(1).isInGroupGUI()) {
				System.out.println("Test OK: the panel isn't in the GUI");
				System.out.println("Hay " + panelGroup.getPanelInGUICount()
						+ " paneles en GUI, de los "
						+ panelGroup.getPanelCount() + " registrados.");
			} else {
				System.out.println("Test FAILED: the panel is in the GUI");
			}

			panels.get(1).setInGroupGUI(true); // test change the visibility
												// from a panel child
			if (panels.get(1).isInGroupGUI()) {
			...
			panelGroup.addTreeSelectionListener(new TreeSelectionListener() {
				...
				public void valueChanged(TreeSelectionEvent e) {
					...
					System.out
							.println("Selection changed. New selection path: "
			...
			// Begin: Test the methods: 'getDividerLocation()' and
			// 'setDividerLocation(int)'
			System.out
					.println("Divider location before setting it to the position 250 : "
							+ panelGroup.getDividerLocation());
			panelGroup.setDividerLocation(250);
			System.out
					.println("Divider location after setting it to the position 250 : "
							+ panelGroup.getDividerLocation());
			// End: Test the methods: 'getDividerLocation()' and
			// 'setDividerLocation(int)'

			// Begin: Test the methods: 'getDividerSize()' and
			// 'setDividerSize(int)'
			System.out
					.println("Divider size before setting it to the position 10 : "
							+ panelGroup.getDividerSize());
			panelGroup.setDividerSize(10);
			System.out
					.println("Divider size after setting it to the position 10 : "
							+ panelGroup.getDividerSize());
			// End: Test the methods: 'getDividerSize()' and
			// 'setDividerSize(int)'
	...
	/**
	 * <p>
	 * Shows information about the current active panel using the standard
	 * output.
	 * </p>
	 */
	private static void showActivePanelInformation() {
		...
		if (panel == null) {
			System.out
					.println("The node selected of the JTree in the TreePanel object isn't a IPanel .");
		} else {
