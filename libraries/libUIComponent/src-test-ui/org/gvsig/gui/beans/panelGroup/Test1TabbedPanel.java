...
/**
 * <p>
 * Tests the creation of a {@link TabbedPanel TabbedPanel} object using
...
public class Test1TabbedPanel {
	...
	/**
	 * <p>
	 * Test method for the Test1TabbedPanel.
	 * </p>
	 * 
	 * @param args
	 *            optional arguments
	...
	public static void main(String[] args) {
		try {
			...
			System.out
					.println("----------> Begin: Tests adding and removing a panel without \"label\" -> this shoudn't add that panel");
			loader = new PanelGroupLoaderFromList(
					new Class[] { SampleInfoPanel.class });
			...
			System.out
					.println("End: Tests adding and removing a panel without \"label\" -> this shoudn't add that panel <----------");
			...
			((AbstractPanel) oPanels[0]).setPreferredSize(new Dimension(700,
					600));
			System.out.println("Has the new panel '"
					+ ((AbstractPanel) oPanels[0]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[0]).hasChanged());

			String old_Label = ((AbstractPanel) oPanels[0]).getLabel();
			((AbstractPanel) oPanels[0]).setLabel(((AbstractPanel) oPanels[0])
					.getLabel() + " modified");
			System.out.println("Has the panel modified '"
					+ ((AbstractPanel) oPanels[0]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[0]).hasChanged());

			((AbstractPanel) oPanels[0]).setLabel(old_Label);
			((AbstractPanel) oPanels[0]).resetChangedStatus();
			((AbstractPanel) oPanels[0]).setID(((AbstractPanel) oPanels[0])
					.getID() + " modified");
			System.out.println("Has the panel modified '"
					+ ((AbstractPanel) oPanels[0]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[0]).hasChanged());
			...
			((AbstractPanel) oPanels[0]).resetChangedStatus();
			((AbstractPanel) oPanels[0])
					.setLabelGroup(((AbstractPanel) oPanels[0]).getLabelGroup()
							+ " modified");
			System.out.println("Has the panel modified '"
					+ ((AbstractPanel) oPanels[0]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[0]).hasChanged());
			...
			((AbstractPanel) oPanels[0]).resetChangedStatus();
			System.out.println("Has the panel reseted '"
					+ ((AbstractPanel) oPanels[0]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[0]).hasChanged());
			...
			((AbstractPanel) oPanels[0]).setInGroupGUI(false); // test change
																// the
																// visibility
																// from a panel
																// child
			...
			((AbstractPanel) oPanels[0]).setInGroupGUI(true); // test change the
																// visibility
																// from a panel
																// child
			if (((AbstractPanel) oPanels[0]).isInGroupGUI()) {
			...
			panelGroup.setEnabledAt(0, false); // test change the enable status
												// to a panel
			...
			panelGroup.setEnabledAt(0, true); // test change the enable status
												// to a panel
			if (panelGroup.isEnabledAt(0)) {
			...
			panelGroup.setEnabledAt(3, false); // test change the enable status
												// to a panel
			...
			panelGroup.setEnabledAt(3, true); // test change the enable status
												// to a panel
			if (panelGroup.isEnabledAt(3)) {
				System.out.println("Test OK: the panel is enabled.");
			} else {
			...
			// Begin: Test: accept, apply or cancel, after request if has
			// changed?
			System.out
					.println("----------> Begin: Test: accept, apply or cancel, after request if has changed?");
			// Object[] obj_panels = (((Collection<IPanel>)
			// panelGroup.values()).toArray());

			// First part:
			System.out.println("First part:");
			System.out.println("Is the new panel '"
					+ ((AbstractPanel) oPanels[1]).getLabel()
					+ "' always applicable?: "
					+ ((AbstractPanel) oPanels[1]).isAlwaysApplicable());
			System.out.println("Has the new panel '"
					+ ((AbstractPanel) oPanels[1]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[1]).hasChanged());

			((AbstractPanel) oPanels[1]).setLabel(((AbstractPanel) oPanels[1])
					.getLabel() + " modified");
			System.out.println("Has the panel modified '"
					+ ((AbstractPanel) oPanels[1]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[1]).hasChanged());
			...
			System.out.println("Has the panel not modified '"
					+ ((AbstractPanel) oPanels[1]).getLabel() + "' changed?: "
					+ ((AbstractPanel) oPanels[1]).hasChanged());
			panelGroup.accept();
			System.out.println("Has the panel not modified '"
			...
			System.out.println("Has the panel not modified '"
					...
					+ ((AbstractPanel) oPanels[1]).hasChanged());
			System.out
					.println("End: Test: accept, apply or cancel, after request if has changed? <----------");
			...
			panelGroup.addChangeListener(new ChangeListener() {
				...
				public void stateChanged(ChangeEvent evt) {
					...
					System.out
							.println("Selection changed. New selection index: "
									+ jTabbedPanel.getSelectedIndex());
