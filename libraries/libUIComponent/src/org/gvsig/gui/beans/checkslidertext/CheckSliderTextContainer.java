 ...
 */
public class CheckSliderTextContainer extends SliderTextContainer implements
		ActionListener {
	 ...
	 */
	public CheckSliderTextContainer(int min, int max, int defaultPos,
			boolean up, String txt, boolean active) {
	 ...
	 *            Valor por defecto del control.True activo y false desactivo
	 * @param border
	 *            True para mostrar el borde al control y false para ocultarlo
	 * @param showCheck
	 *            True para mostrar el checkbox de activación y desactivación.
	 *            False para ocultarlo
	 */
	public CheckSliderTextContainer(int min, int max, int defaultPos,
			boolean up, String txt, boolean active, boolean border,
			boolean showCheck) {
	...
	private JPanel getPCheck(boolean up) {
		if (pLabel == null) {
			...
			if (!up) {
				...
				if (showCheck) {
					gridBagConstraints1.insets = new java.awt.Insets(0, 10, 13,
							0);
					pLabel.add(getCheck(), gridBagConstraints1);
				} else {
					gridBagConstraints1.insets = new java.awt.Insets(0, 0, 13,
							0);
