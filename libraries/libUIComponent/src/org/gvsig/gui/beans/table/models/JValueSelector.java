...
/**
 * Ventana con el slider para la selección de Alpha en una entrada de la tabla
 * que representa a la paleta de color.
 * 
...
public class JValueSelector extends JPanel {
	 ...
	 * @param valueSelector
	 *            the value-selector to be placed inside the dialog
	 * @param okListener
	...
	public static JDialog createDialog(Component c, String title,
			boolean modal, JValueSelector valueSelector,
			ActionListener okListener, ActionListener cancelListener,
			String initValue) throws HeadlessException {

		return new ValueSelector(c, title, modal, valueSelector, okListener,
				cancelListener, initValue);
	}
...
class ValueSelector extends JDialog implements ActionListener {
	...
	public ValueSelector(Component c, String title, boolean modal,
			...
			ActionListener cancelListener, String initValue)
	...
	private JPanel getPButtons() {
		if (buttons == null) {
			buttons = new JPanel();
			buttons.setPreferredSize(new java.awt.Dimension(widthMargin,
					HBUTTONS));
	...
	public SliderTextContainer getPSlider() {
		if (pSlider == null) {
			...
			pSlider.setPreferredSize(new java.awt.Dimension(widthMargin,
					HSLIDER));
		}
		return pSlider;
	}
	...
	/**
	 * Al aceptar asignamos el valor del slider al botón de alpha y al cancelar
	 * cerramos la ventana
	 */
