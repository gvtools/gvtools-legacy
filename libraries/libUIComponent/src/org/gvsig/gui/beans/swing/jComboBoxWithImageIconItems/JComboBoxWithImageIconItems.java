 ...
 */
public class JComboBoxWithImageIconItems extends JComboBox implements
		Serializable {
	 ...
	 * 
	 * @param aModel
	 *            org.gvsig.gui.beans.swing.jComboBoxWithImageIconItems.
	 *            JComboBoxWithImageIconItems.ComboBoxImageIconModel
	 */
	 ...
	 * 
	 * @param items
	 *            Each item must be an 'ImageIconItemInfo' object
	 */
	...
	/**
	 * Adds an image icon URL path, image icon loaded (or null if not), tool tip
	 * text (tool tip text could be "") and if it's neccessary (it's optional)
	 * another associated value to this item (if there is no associated value,
	 * the 'item_Value' param should be null)
	 * 
	 * @param imageIconPath
	 *            A path
	 * @param imageIcon
	 *            An image icon loaded or null if haven't been loaded
	 * @param imageIconToolTipText
	 *            Text
	 * @param item_Value
	 *            Another extra value associated to the item added (null if no
	 *            extra value)
	 */
	public void addImageIconItem(String imageIconPath, ImageIcon imageIcon,
			String imageIconToolTipText, Object item_Value) {
		// Load the image and stor it into the model of the JComboBox
		this.addItem(new ImageIconItemInfo(imageIconPath, imageIcon,
				imageIconToolTipText, item_Value));
	}

	/**
	 * Adds an image icon URL path, image icon loaded (or null if not), and tool
	 * tip text (tool tip text could be "")
	 * 
	 * @param imageIconItem
	 *            Information for load an image icon and set its tool tip text
	 */
	...
	/**
	 * Adds an image icon URL path, tool tip text (tool tip text could be "")
	 * and the item value If no item matches, does nothing (The image icon
	 ...
	 *            An extra value
	 */
	public void removeImageIconItem(String imageIconPath,
			String imageIconToolTipText, String item_Value) {
		int i = 0;
		...
		while (i < this.getItemCount()) {
			...
			if ((iiii1.getPath().compareTo(imageIconPath) == 0)
					...
					&& (((iiii1.getItemValue() == null) && (item_Value == null)) || (iiii1
							.getItemValue().equals(item_Value)))) {
				this.removeItemAt(i);
	...
	/**
	 * Removes an image icon URL path, tool tip text (tool tip text could be "")
	 * and the item value If no item matches, does nothing (The image icon
	 * parameter is ignored)
	 * 
	 * @param imageIconPath
	 *            Information for remove the item
	 */
	public void removeImageIconItem(ImageIconItemInfo imageIconItem) {
		...
		while (i < this.getItemCount()) {
			...
			if ((iiii1.getPath().compareTo(imageIconItem.getPath()) == 0)
					&& (iiii1.getToolTipText().compareTo(
							imageIconItem.getToolTipText()) == 0)
					&& (((iiii1.getItemValue() == null) && (imageIconItem
							.getItemValue() == null)) || (iiii1.getItemValue()
							.equals(imageIconItem.getItemValue())))) {
				this.removeItemAt(i);
				return; // Only remove the first match
			} else {
				i++;
	...
	/**
	 * Sets some image icons that their information to load is stored in an
	 * array of ImageIconItemInfo
	 * 
	 * @param imagePaths
	 *            An array of ImageIconItemInfo with the URL path, image icon
	 *            loaded (or null if not), and tool tip text to each image
	 */
	...
	/**
	 * Sets some image icons that their information to load is stored in a
	 * Vector of ImageIconItemInfo
	 * 
	 * @param imagePaths
	 *            A Vector of ImageIconItemInfo with the URL path, image icon
	 *            loaded (or null if not), and tool tip text to each image
	 */
	...
	/**
	 * Removes some image icons that their path, tool tip text and item value
	 * are stored in an array If no item matches, does nothing (The image icon
	 * parameter is ignored)
	 * 
	 * @param imageIconPaths
	 *            An array with the ImageIconItemInfo
	 */
		...
		for (i = 0; i < imageIconItems.length; i++) {
			...
			while (j < this.getItemCount()) {
				...
				if ((iiii1.getPath().compareTo(iiii2.getPath()) == 0)
						&& (iiii1.getToolTipText().compareTo(
								iiii2.getToolTipText()) == 0)
						&& (((iiii1.getItemValue() == null) && (iiii2
								.getItemValue() == null)) || (iiii1
								.getItemValue().equals(iiii2.getItemValue())))) {
					this.removeItemAt(j);
	...
	/**
	 * Removes some image icons that their paths, tool tip text and item value
	 * are stored in a Vector (The image icon parameter is ignored)
	 * 
	 * @param imageIconPaths
	 *            A Vector with the ImageIconItemInfo
	 */
	...
	public void setShowImageIconAndTextProperty(boolean b) {
		((ComboBoxImageIconModel) this.getModel())
				.setShowImageIconAndTextProperty(b);
	}
	...
	public boolean getShowImageIconAndTextProperty() {
		return ((ComboBoxImageIconModel) this.getModel())
	...
	public Dimension getJComboBoxRendererPreferredSize() {
		return ((ComboBoxImageIconRenderer) this.getRenderer())
				.getPreferredSize();
	}

	/**
	 * This code has been extracted and modified from the Java Sun
	 * Microsystems's example: 'CustomComboBoxDemo.java' in
	 * http://java.sun.com/docs/books/tutorial/uiswing/components/combobox.html
	 * 
	 * @author Sun Microsystems
	 * @author Pablo Piqueras Bartolomé (p_queras@hotmail.com)
	 */
	private class ComboBoxImageIconRenderer extends JLabel implements
		...
		public Component getListCellRendererComponent(JList list, Object value,
			...
			if (value != null) {
				...
				String toolTipText = ((ImageIconItemInfo) value)
						.getToolTipText();
				...
				if (toolTipText != null) {
					...
					referenceToThisJComboBoxWithImageIconItems
							.setToolTipText(toolTipText);
				}
				...
				if (getShowImageIconAndTextProperty()) {
					if (icon != null) {
						...
						setUhOhText(
								item
										+ " ("
										+ Messages.getText("no_image_available")
										+ ")", list.getFont());
					}
				} else {
					/*
