...
/**
 * <p>
 * Represents a {@link JPanel JPanel} adapted to work with as a group of
 * {@link IPanel IPanel}'s.
 * </p>
 * 
 * @see IPanelGroup
 * 
 ...
 */
public abstract class AbstractPanelGroup extends JPanel implements
		ChangeListener, IPanelGroup, IButtonsPanel {
	/**
	 * <p>
	 * If the last notification received has been
	 * {@linkplain AbstractPanelGroup#accept()}.
	 * </p>
	 * 
	 * @see #accept()
	 */
	...
	/**
	 * <p>
	 * Object that has a a ''semantically'' or ''conceptually'' relation to this
	 * panel.
	 * </p>
	 * 
	 * @see #getReference()
	...
	/**
	 * <p>
	 * Array list with all {@link IPanel IPanel} of this group.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Reference to another <code>IPanelGroup</code> component which contains
	 * this one.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Default constructor of a group of {@link IPanel IPanel}.
	 * </p>
	 * 
	 * @param reference
	 ...
	 * <code>AbstractPanelGroup</code> to execute its particular initialization
	 * tasks.
	 * </p>
	 */
	protected void initialize() {
		...
		// Resizes automatically this and its components, and when a new panel
		// is added, gets adjust to
	...
	public void loadPanels(IPanelGroupLoader loader)
		...
		try {
			...
			listBaseException = addCouldntAddPanelException(listBaseException,
					e);
		}
		...
		if (panels.size() == 0) {
			if (listBaseException == null)
				...
				// If there were other exceptions, launch all them with the
				// 'EmptyPanelGroupException' as the first of them.
		...
		while (iterator.hasNext()) {
			...
			try {
				if (panel.remainsWithItsDefaultPreferredSize()) {
					exception = new PanelWithNoPreferredSizeDefinedException(
							panel.getLabel());
				...
				Logger.getLogger(getClass().getName()).debug(
						Messages.getText("panel_adding_exception"), e);
				listBaseException = addCouldntAddPanelException(
						listBaseException, e);
			} catch (Exception e) {
				Logger.getLogger(getClass().getName()).debug(
						Messages.getText("panel_adding_exception"), e);
				...
				if (panel == null)
					listBaseException = addCouldntAddPanelException(
							listBaseException, new PanelBaseException(e, ""));
				else
					listBaseException = addCouldntAddPanelException(
							listBaseException,
							new PanelBaseException(e, panel.getLabel()));
			}
		}
		...
		if (getPanelCount() == 0) {
			if (listBaseException != null) {
				...
				// 'EmptyPanelGroupException' as the first of them.
				listBaseException = addCouldntAddPanelException(
						listBaseException, new EmptyPanelGroupException());
				throw listBaseException;
			} else {
				throw new EmptyPanelGroupException();
			}
		...
		if (getPanelInGUICount() == 0) {
			if (listBaseException != null) {
				// If there were other exceptions, launch all them with the
				// 'EmptyPanelGroupGUIException' as the first of them.
				listBaseException = addCouldntAddPanelException(
						listBaseException, new EmptyPanelGroupGUIException());
				throw listBaseException;
			} else {
	...
	private ListCouldntAddPanelException addCouldntAddPanelException(
			ListCouldntAddPanelException l, Exception e) {
		if (l == null) {
	 ...
	 * </p>
	 * <p>
	 * One panel belongs this panel group if have the same references, or both
	 * references are <code>null</code>.
	 * </p>
	 * 
	 * @param panel
	 *            the panel to validate
	 * @return <code>true</code> if belong to this group; otherwise
	 *         <code>false</code>
	 */
	...
	public void addPanel(IPanel panel) throws BaseException {
		...
		if (panel.remainsWithItsDefaultPreferredSize()) {
			baseException = new PanelWithNoPreferredSizeDefinedException(
					panel.getLabel());
		} else {
			try {
	 ...
	 * beans.panelGroup.panels.AbstractPanel)
	 */
	...
	/**
	 * <p>
	 * Executes an <i>accept</i>, <i>apply</i>, or <i>cancel</i> action. This
	 * implies notify that action to all panels of the group.
	 * </p>
	 * 
	 * @param action
	...
	private void executeAction(byte action) {
		...
		while (iterator.hasNext()) {
			...
			if ((panel.hasChanged())
					|| ((!panel.hasChanged()) && (panel.isAlwaysApplicable()))) {
				switch (action) {
				case ACCEPT_ACTION:
					accepted = true;
					panel.accept();
	...
	public boolean isEnabledHideDetailsButton() {
		if (parentPanelGroup != null)
			return ((IButtonsPanel) parentPanelGroup)
