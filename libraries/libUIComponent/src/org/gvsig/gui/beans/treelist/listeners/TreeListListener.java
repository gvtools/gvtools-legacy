...
/**
 * Esta clase contiene la lógica que añade elementos desde el arbol a la lista,
 * arrastrando o haciendo doble click sobre el elemento en el arbol, elimina
 * elementos de la lista haciendo doble click sobre ellos o intercambia
 * elementos arrastrando en la lista de una posición a otra. Nacho Brodin
 * (brodin_ign@gva.es)
 */

public class TreeListListener implements MouseListener, MouseMotionListener,
	...
	// Released -> !draggActive. Si enterList y se ha arrastrado y se ha pichado
	// un elemento de la lista -> añade elemento
	...
	// Soltar -> si hay elemento seleccionado estamos dentro de la lista y
	// draggElement es true -> se intercambiar

	/**
	 * Variable que contiene la etiqueta seleccionada y arrastrado hasta la
	 * lista
	 */
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (e.getSource() == tree)
			// Si es doble click y hay algún elemento seleccionado en el arbol
			// lo añadimos
			if (e.getClickCount() == 2)
				insertElement();
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		if (e.getSource() == list) {
			...
			ev.setSourceElement((String) ((DefaultListModel) list.getModel())
					.getElementAt(selectElement));
		}
	}

	public void mouseReleased(MouseEvent e) {
		// Si se ha pinchado sobre un elemento del arbol y se ha arrastrado lo
		// añadimos
		if (enterInList) {
			if (draggLabel != null && draggActive == true) {
				 ...
				 * 
				 * //Lanzamos el evento de añadir un elemento
				 * ev.setElementAdded(draggLabel); elementAdded(ev);
				 * 
				 * model.addElement(draggLabel);
				 */
				draggLabel = null;
		...
		// Si se está arrastrando un elemento de la lista se intercambia por el
		// lugar donde se suelta
		if (draggElement && e.getSource() == list && selectElement != -1) {
			if (list.getSelectedIndex() != selectElement) {
				...
				ev.setDestElement((String) ((DefaultListModel) list.getModel())
						.getElementAt(posDst));
	...
	public void mouseDragged(MouseEvent e) {
		// Al arrastrar desde el arbor se pone draggActive a true se se ha
		// pinchado antes en un elemento
		if (e.getSource() == tree) {
			if (draggLabel != null)
				draggActive = true;
		}
	...
	public void elementRemoved(TreeListEvent e) {
		if (listListeners == null || listListeners.size() == 0)
			return;
		for (int i = 0; i < listListeners.size(); i++)
			((TreeListComponentListener) listListeners.get(i))
					.elementRemoved(e);
	}
