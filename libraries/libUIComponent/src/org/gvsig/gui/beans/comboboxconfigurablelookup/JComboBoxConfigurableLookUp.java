 ...
 * VERSION: BETA
 * <p>
 * A configurable combo box component that lists items that carry out with the
 * criterions of a <code>ILookUp</code> agent, considering the text in its inner
 * editor.
 * </p>
 * 
 * <p>
 ...
 * <li>Uses a {@link DefaultComboBoxConfigurableLookUpModel
 * DefaultComboBoxConfigurableLookUpModel} as a dataModel, which it's also
 * configurable.</li>
 * </ul>
 * </p>
 * 
 ...
 * Configuration flags:
 * <ul>
 * <li><b>OnlyOneColorOnText:</b> uses red if there is no item or only items
 * that matches with the text written according the <code>ILookUp</code> agent,
 * or uses always black color.</li>
 * <li><b>BeepEnabled:</b> rings a beep sound if there is no item that matches
 * with the text written according the <code>ILookUp</code> agent, or not.</li>
 * <li><b>HidePopupIfThereAreNoItems:</b> hides popup (list box) if there is no
 * item that matches with the text written according the <code>ILookUp</code>
 * agent, or uses the default (JComboBox and more) behaviour.</li>
 * <li><b>ToForceSelectAnItem:</b> forces to select an item (if the component
 * looses the focus or user presses the Enter keyboard key), or uses the default
 * (JComboBox and more) behaviour.</li>
 * <li><b>CompleteArrowKeySelection:</b> writes in the editor the value of the
 * element selected by the arrow keys (up or down) if its enabled, otherwise
 * holds the previous text.</li>
 * <li><b>DisplayAllItemsWithArrowButton:</b> <i>true</i>: displays all this
 * component's dataModel items if its <i>popup</i> it's hided, and this
 * component's dataModel listed only the matches <i>(flag:
 * <code>DefaultComboBoxConfigurableLookUpModel.showAllItemsInListBox == false</code>
 * )</i>; otherwise, if this flag isn't enabled, won't display all items in that
 * situation.</li>
 * </ul>
 * </p>
 * 
 ...
 * 
 * <p>
 * <i>More information about the behaviour of it's dataModel in
 * {@link DefaultComboBoxConfigurableLookUpModel
 * DefaultComboBoxConfigurableLookUpModel} .</i>
 * </p>
 * 
 * @see JComboBox
 ...
 * @version 07/02/2008
 */
public class JComboBoxConfigurableLookUp extends JComboBox implements
	...
	/**
	 * <p>
	 * A reference to the dataModel of this component (according to the MVC
	 * <i>(Model-View-Controller)</i> pattern).
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Determines if the arrow button of the GUI has been clicked.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Determines if is showing all items temporally in the GUI .
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Determines if the UI of this component has been updated.
	 * </p>
	 * <p>
	 * This parameter is used by the listener that shows all items when user
	 * presses the <i>arrow button</i> and its enabled the flag
	 * <code>displayAllItemsWithArrowButton</code>.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: use red if there is no item that matches with the text written
	 * according the <code>ILookUp</code> agent, or uses always black colour.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: rings a beep sound if there is no item that matches with the text
	 * written according the <code>ILookUp</code> agent, otherwise no.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: hide popup (list box) if there is no item that matches with the
	 * text written according the <code>ILookUp</code> agent, or uses the
	 * default (JComboBox and more) behaviour.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: forces to select an item (if the component looses the focus or user
	 * presses the Enter keyboard key), or uses the default (JComboBox and more)
	 * behaviour.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: determines if has to write in the editor the value of the element
	 * selected by the arrow keys (up or down) if its enabled, or hold the
	 * previous text if isn't.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Flag: displays all this component's dataModel items if its <i>popup</i>
	 * it's hided, this component's dataModel displayed only the matches, and
	 * this flag enabled; if this flag isn't enabled, won't display all items in
	 * that situation.
	 * </p>
	 */
	...
	/**
	 * <p>
	 * Default constructor with a {@link DefaultComboBoxConfigurableLookUpModel}
	 * as parameter.
	 * </p>
	 * 
	 * @param aModel
	 ...
	 */
	public JComboBoxConfigurableLookUp(
			DefaultComboBoxConfigurableLookUpModel aModel) {
		super(aModel);
	 ...
	 * 
	 * @param items
	 *            An array of objects. All them must implement a <i>'String
	 *            toStrin()'</i> method
	 */
	 ...
	 * 
	 * @param items
	 *            A {@link Vector} of objects. All them must implement a
	 *            <i>'String toStrin()'</i> method
	 */
	...
	/**
	 * <p>
	 * This method sets the start values of inner attributes and creates the
	 * necessary inner objects.
	 * </p>
	 * 
	 * @param dataModel
	 *            dataModel used by this combo box instance
	 */
	...
	protected void configureEditor(ComboBoxEditor newEditor) {
		if (newEditor != null) {
			...
			// Adds the new Focus Listener (tries to remove it if it existed
			// before)
			jTextComponentOfEditor
					.removeFocusListener(this.editorFocusListener);
	...
	protected void configurePopUp(JComboBoxConfigurableLookUp comboBox) {
		...
		BasicComboBoxUI comboBoxUi = (BasicComboBoxUI) comboBoxReference
				.getUI();
		...
		if (arrowMouseListener == null) {
			...
			arrowMouseListener = new MouseAdapter() {
				...
				public void mousePressed(MouseEvent e) {
					...
					// Avoids a bug: when the component is created an shown,
					// first time the arrow button is pressed, the method
					// #isPopupVisible() returns 'true' instead of returning
					// 'false'
					...
					if ((!model.isShowAllItemsInListBox())
							&& (displayAllItemsWithArrowButton)
							&& (!popupVisible)) {
						...
						// Must replace the dataModel for update the data in the
						// popup correctly
						setModel(new DefaultComboBoxConfigurableLookUpModel(
								model.getData()));
	 ...
	 * 
	 * <p>
	 * <b>Warning:</b><i>This method is another important difference from the
	 * JComboBox; and could work badly with some key-maps.</i>
	 * </p>
	 * 
	 * @param comboBox
	 *            A reference of this component
	...
	private void defineEditorKeyListener(JComboBoxConfigurableLookUp comboBox) {
		...
		editorKeyListener = new KeyAdapter() {
			 ...
			 */
			public void keyPressed(KeyEvent ke) // Executed on the Start view
												// state or Search view state
			{
				...
				case KeyEvent.VK_ENTER:
					// Don't allow execute the default instructions because they
					// have a bug (first time we remove some characters, no item
					// will be displayed in the popup)
					ke.consume();

					// Sets the caret position of the text in the document to
					// the end:
					((JTextComponent) comboBoxReference.getEditor()
							.getEditorComponent()).setCaretPosition(document
							.getLength());

					final DefaultComboBoxConfigurableLookUpModel c_model = (DefaultComboBoxConfigurableLookUpModel) comboBoxReference
							.getModel();

					if (toForceSelectAnItem) {
						if (c_model.isShowAllItemsInListBox()) {
							// Select now the first item or the previous
							// selected
							if (c_model.getSelectedItem() == null) {
								if (c_model.getData().size() > 0) {
									if (previousSelected == null)
										previousSelected = c_model
												.getDataAccordingItemsOrder()
												.elementAt(0);
									else {
										((JTextComponent) comboBoxReference
												.getEditor()
												.getEditorComponent())
												.setText(previousSelected
														.toString());
									}
								} else {
									previousSelected = null;
								...
								((JTextComponent) comboBoxReference.getEditor()
										.getEditorComponent())
										.setText(previousSelected.toString());
							}
						} else {
							// Select now the first item or the previous
							// selected
							...
							case 1:
								...
								previousSelected = comboBoxReference
										.getItemAt(0); // Select the first
	...
	private void defineEditorFocusListener(JComboBoxConfigurableLookUp comboBox) {
		...
		editorFocusListener = new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				...
				// Sets the caret position of the text in the document to the
				// end:
				((JTextComponent) comboBoxReference.getEditor()
						.getEditorComponent()).setCaretPosition(document
						.getLength());
				...
				if (toForceSelectAnItem) {
					if (c_model.isShowAllItemsInListBox()) {
						...
						((JTextComponent) comboBoxReference.getEditor()
								.getEditorComponent()).setText(previousSelected
								.toString());
					} else {
						// Select now the first item or the previous selected
	 ...
	 * 
	 * <p>
	 * The editor must be a <code>JTextComponent</code>, and its document a
	...
	public void setEditor(ComboBoxEditor anEditor) {
		...
		JTextComponent jTextComponentOfEditor = (JTextComponent) anEditor
				.getEditorComponent();

		// Adds the new ''PlainDocumentTextFormatter'', adding the listeners of
		// the new editor's document
		PlainDocument old_document = ((PlainDocument) jTextComponentOfEditor
				.getDocument());

		DocumentListener[] documentListeners = old_document
				.getDocumentListeners();
		UndoableEditListener[] undoableEditListeners = old_document
		...
		DocumentListener[] removableDocumentListeners = document
				.getDocumentListeners();
		for (DocumentListener dL : removableDocumentListeners)
			document.removeDocumentListener(dL);

		UndoableEditListener[] removableUndoableEditListeners = document
				.getUndoableEditListeners();
		...
		if (undoableEditListeners.length > 0) {
			if (jTextComponentOfEditor instanceof IEditableText) {
				((IEditableText) jTextComponentOfEditor)
						.addUndoRedoEditListener(new UndoRedoEditListener() {
							/*
							 * (non-Javadoc)
							 * 
							 * @see
							 ...
							 * #operationExecuted(org.gvsig.
							 * gui.beans.editabletextcomponent
							 * .event.UndoRedoEditEvent)
							 */
	...
	public void setModel(ComboBoxModel aModel) {
		// If model isn't DefaultComboBoxConfigurableLookUpModel, sets as model
		// a new DefaultComboBoxConfigurableLookUpModel, with the data
		// of ''aModel'':
	...
	public void setSelectedIndex(int anIndex) {
		if (anIndex < -1 || anIndex >= model.getSize())
			...
			// Selects the item at the given index or clears the selection if
			// the
			// index value is -1.
			setSelectedItem((anIndex == -1) ? null : model
					.getElementAt(anIndex));
	}
	...
	public void setSelectedItem(Object anObject) {
		if ((selectedItemReminder == null)
				|| (!selectedItemReminder.equals(anObject))) {
			if (arrowKeyPressed) {
	...
	/*
	 * <p>Unsupported operation. A <code>JComboBoxConfigurableLookUp</code> is
	 * always editable.</p>
	 * 
	 * (non-Javadoc)
	 * 
	...
	public void removeAllItems() {
		if (!(model instanceof MutableComboBoxModel))
			throw new RuntimeException(
					"Cannot use this method with a non-Mutable data dataModel.");
	...
	protected String paramString() {
		return super.paramString() + "onlyOneColorOnText" + onlyOneColorOnText
				+ "beepEnabled" + beepEnabled + "hidePopupIfThereAreNoItems"
				+ hidePopupIfThereAreNoItems + "toForceSelectAnItem"
				+ toForceSelectAnItem + "completeArrowKeySelection"
				+ completeArrowKeySelection + "displayAllItemsWithArrowButton"
				+ displayAllItemsWithArrowButton + "itemsOrder"
				+ model.getItemsOrder() + "showAllItemsInListBox"
				+ model.isShowAllItemsInListBox() + "localeRules"
				+ model.getLocaleRules() + "caseSensitive"
	...
	public void updateUI() {
		...
		for (int i = 0; i < getComponentCount(); i++) {
			if (getComponent(i) instanceof JButton) {
				((JButton) getComponent(i))
						.addMouseListener(getArrowMouseListener());
				return;
			}
		}
	}
	...
	/**
	 * <p>
	 * Sets if the popup of this component will always remain hided, or can be
	 * shown when is invoked.
	 * </p>
	 * 
	 * @param b
	 *            <code>true</code> if will remain blocked; <code>false</code>
	 *            if can be shown
	 */
	...
	/**
	 * <p>
	 * Returns the 'only_One_Color_On_Text' configuration value of this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: always uses black colour on text.</li>
	 * <li><i>false</i>: by default uses black colour on text, but if text
	 * written by user doesn't match with any item according the
	 * <code>ILookUp</code> agent, text will be on red colour.</li>
	 * </ul>
	 * </p>
	 * 
	 ...
	 * 
	 * @param b
	 *            value for 'only_One_Color_On_Text' configuration flag
	 */
	 ...
	 * Configuration values are:
	 * <ul>
	 * <li><i>true</i>: a beep-sound is listened when no item matches with the
	 * text written according the <code>ILookUp</code> agent.</li>
	 ...
	 * 
	 * @param b
	 *            value for 'beep_Enabled' configuration flag
	 */
	...
	/**
	 * <p>
	 * Returns the 'hidePopupIfThereAreNoItems' configuration value of this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: hides the popup if there are no items when you write.</li>
	 * <li><i>false</i>: default behaviour (if there are no items when you
	 * write, popup remains visible) .</li>
	 * <ul>
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Sets the 'hidePopupIfThereAreNoItems' configuration value for this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: hides the popup if there are no items when you write.</li>
	 * <li><i>false</i>: default behaviour (if there are no items when you
	 ...
	 * 
	 * @param b
	 *            value for 'hidePopupIfThereAreNoItems' configuration flag
	 */
	 ...
	 * Configuration values are:
	 * <ul>
	 * <li><i>true</i>: selects the previous selected item or the first of the
	 * list when user presses the Enter key or when the component loses the
	 * focus .</li>
	 ...
	 * 
	 * @param value
	 *            for 'toForceSelectAnItem' configuration flag
	 */
	...
	/**
	 * <p>
	 * Returns the 'completeArrowKeySelection' configuration value of this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: writes in the editor the value of the element selected
	 * by the arrow keys (up or down).</li>
	...
	/**
	 * <p>
	 * Sets the 'completeArrowKeySelection' configuration value for this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: writes in the editor the value of the element selected
	 * by the arrow keys (up or down).</li>
	 ...
	 * 
	 * @param b
	 *            value for 'completeArrowKeySelection' configuration flag
	 */
	...
	/**
	 * <p>
	 * Returns the 'displayAllItemsWithArrowButton' configuration value of this
	 * component. Configuration values are:
	 * <ul>
	 * <li><i>true</i>: displays all this component's dataModel items if its
	 * <i>popup</i> it's hided, and this component's dataModel listed only the
	 * matches <i>(flag:
	 * <code>DefaultComboBoxConfigurableLookUpModel.showAllItemsInListBox == false</code>
	 * )</i>.</li>
	 ...
	 * 
	 * @param value
	 *            for 'displayAllItemsWithArrowButton' configuration flag
	 */
	...
	/**
	 * <p>
	 * Inner class that inherits of the class PlainDocument, and is used for
	 * manipulate the textWritten that has the document of the editor of this
	 * component.
	 * </p>
	 * 
	 * <p>
	...
	public class PlainDocumentTextFormatter extends PlainDocument {
		 ...
		 * 
		 * @param comboBox
		 *            A reference to the class that contains this.
		 */
		protected void setJComboBoxReference(
				JComboBoxConfigurableLookUp comboBox) {
		...
		public void insertString(int offs, String str, AttributeSet a)
			...
			if (textWritten.length() > 0) {
				if (offs < textWritten.length()) {
					...
					// End of the text
					textWritten = textWritten.substring(offs,
							textWritten.length());

					// Beginning of the text
					if (offs > 0) {
						// Reset (without text written)
						textWritten = old_textWritten.substring(0, offs) + str
								+ textWritten;
					} else {
			...
			if (caretToEnd) {
				// Update the caret position -> at the same place it was or at
				// the end
		...
		public void replace(int offset, int length, String text,
			...
			if (arrowKeyPressed) {
				if (!model.isShowAllItemsInListBox()) {
					if (completeArrowKeySelection) {
						String currentText = ((JTextComponent) comboBoxReference
								.getEditor().getEditorComponent()).getText();

						old_textWritten = textWritten;
						old_caretPosition = ((JTextComponent) comboBoxReference
								.getEditor().getEditorComponent())
								.getCaretPosition()
								+ text.length();
						...
						if ((completeArrowKeySelectionValue == null)
								|| (!currentText.equals(old_textWritten)))
			...
			if (length > 0) {
				...
				if ((offset > 0) && (offset != textWritten.length())) {
					...
					// There is a bug that when text has been removed, the
					// elements in the popup aren seen.
					...
					// Stores the configuration of the flags
					int itemsOrderFlag = model.getItemsOrder();
					boolean showAllItemsInListBox = model
					...
					// Insert the new text
					textWritten = old_textWritten.substring(0, offset)
							+ text
							+ old_textWritten.substring(offset,
									old_textWritten.length());
					...
					// Update the caret position -> at the same place it was or
					// at the end
		...
		public void remove(int offs, int len) throws BadLocationException {
			...
			// There is a bug that when text has been removed, the elements in
			// the popup aren seen.
			// The solution to this bug is add them as a new dataModel
			hidePopup();
			...
			old_caretPosition = Math
					.min(((JTextComponent) comboBoxReference.getEditor()
							.getEditorComponent()).getCaretPosition(), offs);
			...
			// Must replace the dataModel for update the data in the popup
			// correctly
			comboBoxReference
					.setModel(new DefaultComboBoxConfigurableLookUpModel(model
							.getData()));
			...
			// Insert the new text
			textWritten = old_textWritten.substring(0, offs)
					+ textOfReplacement
					+ old_textWritten.substring(offs + len,
							old_textWritten.length());
			...
			// }
			// else {
			// Update the caret position -> at the same place it was or at the
			// end
		...
		/**
		 * <p>
		 * Updates the color of the text in the {@link ComboBoxEditor} of this
		 * component, and rings a beep if there is no item.
		 * </p>
		 * 
		 * <ul>
		 * <li><i>Text in <u>red color</u></i>: if no item matches with the text
		 * written according the <code>ILookUp</code> agent.</li>
		 * <li><i>Text in <u>black color</u></i>: if there is at least one item
		 * matches with the text written according the <code>ILookUp</code>
		 * agent.</li>
		 * </ul>
		 */
		public void updateTextColorAndRingBeep() {
			if (!comboBoxReference.isOnlyOneColorOnText()) {
				if (model.isShowAllItemsInListBox()) {
					// Set text into red color if no items matches with the text
					// written, or in black color if it was in red
					// color and now there is almost one element that starts
					// with the characteres written by the user
					if (textWritten.compareTo("") != 0) {
						if (model.getSelectedItem() == null) {
							comboBoxReference.getEditor().getEditorComponent()
									.setForeground(Color.RED);
							textInRedColor = true;

							// Rings a beep if allowed
							if (beepEnabled)
								comboBoxReference.getToolkit().beep(); // when
																		// available
																		// use:
				...
				if ((textWritten.compareTo("") != 0) && (model.getSize() == 0)) {
					if (beepEnabled)
						comboBoxReference.getToolkit().beep(); // when available
																// use:
																// UIManager.getLookAndFeel().provideErrorFeedback(comboBox);
				}
			}
		...
		/**
		 * <p>
		 * Updates the position of the caret in the {@link ComboBoxEditor} of
		 * this component.
		 * </p>
		 * 
		 * @param position
		...
		public void updateCaretPosition(int position) {
			// If user has selected an item of the popup (using the mouse) ->
			// set the caret position to the end of the text
			if (popupItemSelected) {
				((JTextComponent) comboBoxReference.getEditor()
						.getEditorComponent()).setCaretPosition(textWritten
						.length());
				popupItemSelected = false;
				return;
			}
			...
			if (position > textWritten.length())
				...
				((JTextComponent) comboBoxReference.getEditor()
						.getEditorComponent()).setCaretPosition(position);
			}
		}
		 ...
		 * 
		 * <p>
		 * Sets text to red color if there is no item in the visible list that
		 * the <i>look up</i> agent returned with that text and the items of the
		 * dataModel, and, being enabled the second color in the configuration
		 * the the <code>JcomboBoxConfigurableLookUp</code> agent. Otherwise,
		 * the foreground color of the text will be black.
		 * </p>
		 * 
		 * <p>
		 * This method disables the <i>beep</i> during the process of updating
		 * the color of the text, and restores it after.
		 * </p>
		 */
