...
/**
 * <p>
 * Extra functionality that allows a {@link JTextComponent JTextComponent}
 * having graphical edition options, which allows user to select any: <i>COPY,
 * CUT, PASTE, SELECT ALL, REMOVE, UNDO, REDO</i> in a popup menu.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * <b><i>How select an edition option using the mouse:</i></b> press the right
 * button of the mouse on the text area, and a popup with the options will be
 * displayed. Select and option.<br>
 ...
 * <li><b>REMOVE:</b></li> SUPR (in spanish keyboard, and the equivalent in
 * others). Removes the selected text.
 ...
 * 
 * <p>
 * This component by default stores 100 undo/redo actions. This value can be
 * modified.
 * </p>
 * 
 * <p>
 * A developer would have to re-implement
 * {@linkplain #defineEditorKeyListener() #defineEditorKeyListener()}, creating
 * a new <code>editorKeyListener</code> for changing any fast-access key.
 * </p>
 * 
 * @version 27/02/2008
...
public class EditableTextDecorator implements IEditableText {
	...
	private void initialize() {
		...
		undoRedoLimitActions = DEFAULT_UNDO_REDO_LIMIT_ACTIONS; // By default is
																// 1
	...
	private void createDefaultListeners() {
		// Defines a listener for the PopupMenu of text edition options: when a
		// change is produced in that component, it fires an event
	...
	/**
	 * Defines a listener for the PopupMenu of text edition options: when a
	 * change is produced in that component, it fires an event and this listener
	 * captures and treats it
	 */
	private void defineEditionMenuPropertyChangeListener() {
		editionMenuListener = new PropertyChangeListener() {
			...
			public void propertyChange(PropertyChangeEvent arg0) {
				...
				/*
				 * else { if
				 * (arg0.getPropertyName().equals(JOptionsEditionByMousePopupMenu
				 * .VISIBILITYCHANGED)) { // First True, after False (when false
				 * -> optionsEditorWasVisible = true) if
				 * (!optionsEditionByMouse.isVisible()) optionsEditorWasVisible
				 * = true; else optionsEditorWasVisible = false;
				 * 
				 * } }
				 */
			}
	...
	protected void defineEditorKeyListener() {
		editorKeyListener = new KeyAdapter() {
			...
			{
				// COPY, CUT, PASTE, SELECT ALL, UNDO AND REDO WITH THE KEYBOARD
				// (Combination of keys; REMOVE is implemented after: KV_DELETE
				// (supr spanish key))
				if (ke.isControlDown()) {
					// COPY
		...
		editorMouseListener = new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (e.getButton() == MouseEvent.BUTTON3) {
					...
					// Enable the "Undo" option if there is any previous state
					// to restore
					if (undoManager.canUndo())
						optionsEditionByMouse.setEnabledUndoOption(true);

					// Enable the "Redo" option if there is any later state to
					// restore
					if (undoManager.canRedo())
						optionsEditionByMouse.setEnabledRedoOption(true);

					// Enable the "Copy", "Cut" and "Delete" options if there is
					// text selected
					if (jTextComponent.getCaretPosition() != jTextComponent
							.getCaret().getMark()) {
					...
					// Enable the "Paste" option if there is text on the
					// system's clipboard
					if (jTextComponent.getToolkit().getSystemClipboard()
							.getContents(this).toString().length() > 0)
						optionsEditionByMouse.setEnabledPasteOption(true);//

					// Enable the "Select-All" option (by default it's always
					// enabled)
					optionsEditionByMouse.setEnabledSelectAllOption(true);

					optionsEditionByMouse.setLocation((int) jTextComponent
							.getLocationOnScreen().getX() + e.getX(),
							(int) jTextComponent.getLocationOnScreen().getY()
									+ e.getY());
	...
	/**
	 * Configures the editor {@link ComboBoxEditor ComboBoxEditor} of the text
	 * component.
	 * 
	 * @param document
	...
	private void configureEditor(PlainDocument document) {
		if (jTextComponent != null) {
			...
			// Adds the new Mouse Listener (tries to remove it if it existed
			// before)
	...
	private void configureOptionsEditionByMouse() {
		this.optionsEditionByMouse
				.addPropertyChangeListener(editionMenuListener);
	}
	...
	private void undoOperationLogic() {
		try {
			if (undoManager.canUndo()) {
				...
				fireUndoRedoEditUpdate(new UndoRedoEditEvent(jTextComponent,
						UndoRedoEditEvent.UNDO, previousText, newText));
			}
	...
	private void redoOperationLogic() {
		try {
			if (undoManager.canRedo()) {
				...
				String newText = jTextComponent.getText();
				fireUndoRedoEditUpdate(new UndoRedoEditEvent(jTextComponent,
						UndoRedoEditEvent.REDO, previousText, newText));
			}
	...
	/**
	 * This method is invoked when some text has to be removed: With the
	 * 'Delete' option of the text-edition-popupmenu or with the 'delete'
	 * ('supr' in spanish keyboard) key.
	 */
	private void deleteTextLogic() {
		// Get the new text:
		try {
			PlainDocument document = (PlainDocument) jTextComponent
					.getDocument();
	 ...
	 * 
	 * @see
	 * org.gvsig.gui.beans.editabletextcomponent.IEditableText#removeCaretListener
	 * (org.gvsig.gui.beans.editabletextcomponent.event.UndoRedoEditListener)
	 */
	...
	public UndoRedoEditListener[] getUndoRedoEditListeners() {
		return (UndoRedoEditListener[]) undoRedoEditListeners
				.getListeners(UndoRedoEditListener.class);
	}
