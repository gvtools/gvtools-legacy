...
/**
 * This class is a version of the graphical interface of "FilterDialog" which
 * was made by Fernando and is in 'appgvSIG' project. It's supposed that other
 * classes will extend from this and will add the logic as is in the
 * "FilterDialog" class.
 * 
 * @author Pablo Piqueras Bartolomé (p_queras@hotmail.com)
 */
public class FilterQueryJPanel extends AbstractFilterQueryJPanel implements
		Serializable {
	...
	protected void initialize() {
		...
		this.setBorder(BorderFactory.createTitledBorder(Messages
				.getText("filter_by_sentence")));
		getValuesJList().setCellRenderer(
				new JLabelCellRendererWithTextAsToolTipText());

		// /**
		// * This listener allows show in the tool tip text the value of the
		// item when
		...
		// if (e.getSource() != null) {
		// Object obj =
		// ((JScrollPane)e.getSource()).getViewport().getComponent(0);
		//
		// if (obj instanceof JList) {
		// int index = ((JList)obj).locationToIndex(e.getPoint());
		//
		// if (index >= 0)
		// ((JList)obj).setToolTipText( ((JLabel)
		// ((JList)obj).getModel().getElementAt(index)).getText() );
		// }
		// }
		// }
		//
	...
	protected JPanel getTopJPanel() {
		if (topJPanel == null) {
			...
			// topJPanel.add(getValidateFilterExpressionJCheckBox(),
			// gridBagConstraints);
	...
	protected JPanel getTopCenterJPanel() {
		if (topCenterJPanel == null) {
			...
			topCenterJPanel.setPreferredSize(new Dimension(
					getFilterButtonsJPanel().getPreferredSize().width,
					getFilterButtonsJPanel().getPreferredSize().height
							+ validateFilterExpressionJCheckBoxHeight));
			topCenterJPanel.setLayout(null);

			getFilterButtonsJPanel().setBounds(0, 0,
					getFilterButtonsJPanel().getPreferredSize().width,
					getFilterButtonsJPanel().getPreferredSize().height);
	...
	protected JCheckBox getValidateFilterExpressionJCheckBox() {
		if (validateFilterExpressionJCheckBox == null) {
			...
			validateFilterExpressionJCheckBox.setBounds(5,
					getFilterButtonsJPanel().getPreferredSize().height,
					getFilterButtonsJPanel().getWidth(),
					validateFilterExpressionJCheckBoxHeight);
			validateFilterExpressionJCheckBox.setText(Messages
					.getText("validate_expression"));
			validateFilterExpressionJCheckBox.setToolTipText(Messages
					.getText("validate_filter_expression_explanation"));
		}
			...
			getFilterJScrollPane().setPreferredSize(
					new Dimension(
							getFilterJScrollPane().getPreferredSize().width
									+ difference, getFilterJScrollPane()
