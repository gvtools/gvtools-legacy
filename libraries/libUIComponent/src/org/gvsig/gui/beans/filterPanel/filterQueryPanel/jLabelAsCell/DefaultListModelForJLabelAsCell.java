 ...
 * 
 */
public class DefaultListModelForJLabelAsCell extends DefaultListModel implements
		Serializable {
	private static final long serialVersionUID = 4410627738805243864L;

	/**
	 * Represents the start index of the group of not loaded values (that should
	 * be after the group of loaded values) -2 -> if elements aren't grouped (in
	 * two lists: first for loaded, after for not loaded) -1 -> if there isn't
	 * elements 0 -> if there are only not loaded elements = (capacity() -1) ->
	 * if there are only loaded elements
	 */
	...
	/**
	 * Method that returns true if there is a inner JLabel that has as inner
	 * text the same text as 'patternText'
	 * 
	 * @param patternText
	...
	/**
	 * Returns the position of first JLabel that has as text the same text as
	 * 'patternText', or -1 if no JLabel has that text
	 * 
	 * @param patternText
	 *            An String
	...
	/**
	 * Replaces first element which has the text 'patternText' to a
	 * JLabelValueLoaded new one element with the same text
	 * 
	 * @param patternText
		...
		if (startIndexOfNotLoadedValues == -1) {
			for (int i = 0; i < super.getSize(); i++) {
				if (super.get(i) instanceof JLabelAsCellValueLoaded) {
					JLabelAsCellValueNotLoaded jL = new JLabelAsCellValueNotLoaded(
							((JLabelAsCellValueLoaded) super.get(i)).getText());
					super.remove(i);
					super.add(i, jL);// /// REIMPLEMENTATION OF SOME METHODS OF
										// 'DefaultListModel' /////
				}
			}
		} else {
	...
	public void insertElementAt(Object obj, int index) {
		...
		if ((obj instanceof JLabelAsCellValueNotLoaded) && (index > -1)
				&& (index < startIndexOfNotLoadedValues))
	...
	public Object remove(int index) {
		if ((index > -1) && (index == startIndexOfNotLoadedValues)
				&& (index == (super.capacity() - 1))) {
			startIndexOfNotLoadedValues = -1;
		}
	...
	public void removeRange(int fromIndex, int toIndex) {
		...
		for (int i = 0; i < super.capacity(); i++) {
			if (super.get(i) instanceof JLabelAsCellValueNotLoaded) {
				if (((i == (super.capacity() - 1) || (i < (super.capacity() - 1))
						&& (super.get(super.capacity() - 1) instanceof JLabelAsCellValueNotLoaded))))
	...
	public Object set(int index, Object element) {
		if ((element instanceof JLabelAsCellValueNotLoaded) && (index > -1)
				&& (index < startIndexOfNotLoadedValues))
			startIndexOfNotLoadedValues = index;
