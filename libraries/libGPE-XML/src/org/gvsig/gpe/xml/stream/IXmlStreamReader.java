 ...
 * <p>
 * This interface provides a pull like approach to parsing xml formatted
 * documents for the libGPE library, yet allowing to interchange implementations
 * which can be based, for example, in XML SAX, XML Pull, Binary XML, etc.
 * </p>
 * <p>
 * Note at this time this interface contract is made just of the methods from
 * XmlPullParser that where already used so far in the libGPE-GML and libGPE-KML
 * modules. The intention is to first introduce this abstraction layer and then
 * evolve it as needed to support an intermediate level of abstraction between
 * actual xml parsing and {@link IGPEContentHandler}, while keeping the unit
 * tests passing all the time. So this is work in progress and expected to
 * change dramatically, though ensuring no functional breackages in any time.
 * </p>
 * 
...
public interface IXmlStreamReader {
	...
	/**
	 * Returns the complete (coalesced) value for the attribute at index
	 * {@code i}
	 * 
	 * @param i
	 * @return
	...
	/**
	 * Returns the qualified name (with prefix if applicable) for the current
	 * element.
	 * 
	 * @return the qualified name for the current START_ELEMENT or END_ELEMENT
	 *         event, or <code>null</code> if the current event is not a
